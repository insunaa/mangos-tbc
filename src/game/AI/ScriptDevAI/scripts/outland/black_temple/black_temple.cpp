...
encounter status for use in various Black Temple Scripts SDCategory: Black
Temple EndScriptData */
...
void instance_black_temple::SetData(uint32 type, uint32 data)
{
    ...
    case TYPE_SHADE:
        ...
        if (data != IN_PROGRESS)
            for (ObjectGuid guid : m_creatureGeneratorGuidVector) // notify generators to stop
                                                                  // spawning
    ...
    case TYPE_ILLIDAN:
        if (data == IN_PROGRESS && GetData(TYPE_COUNCIL) != DONE)
            BanPlayersIfNoGm("Player engaged Illidan without killing council and "
        ...
        break;
    default:
        script_error_log("Instance Black Temple: ERROR SetData = %u for type %u "
