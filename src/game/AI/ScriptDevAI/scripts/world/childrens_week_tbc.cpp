...
enum TriggerEntries
{
    ...
    // spells are not needed but kept for brevity - official servers had more
    // power from spell script
...
static std::set<uint32> triggers = {
    NPC_TRIGGER_SPOREGGAR,          NPC_TRIGGER_AUCHINDOUN, NPC_TRIGGER_DARK_PORTAL,        NPC_TRIGGER_AERIS_LANDING,
    NPC_TRIGGER_THRONE_OF_ELEMENTS, NPC_TRIGGER_EXODAR_01,  NPC_TRIGGER_SILVERMOON_01,      NPC_TRIGGER_SILVERMOON_02,
    NPC_TRIGGER_CAVERNS_OF_TIME,    NPC_TRIGGER_EXODAR_02,  NPC_TRIGGER_SILVERMOON_LVL70ETC};
...
struct ChildTbcAI : public PetAI
{
    ChildTbcAI(Creature *creature) : PetAI(creature)
    {
        ...
        AddCustomAction(ACTION_AUCHINDOUN_AND_THE_RING_OF_OBSERVANCE, true,
                        [&]() { HandleAuchindounAndTheRingOfObservance(); });
    ...
    bool IsHorde() const
    {
        return m_creature->GetEntry() == NPC_BLOOD_ELF_CHILD;
    }
    ...
    void MoveInLineOfSight(Unit *who) override
    {
        if (triggers.find(who->GetEntry()) != triggers.end() && m_creature->IsWithinDist(who, 20.f))
        {
            ...
            switch (who->GetEntry())
            {
            case NPC_TRIGGER_SPOREGGAR:
                questId = QUEST_HCHUU_AND_THE_MUSHROOM_PEOPLE;
                break;
            case NPC_TRIGGER_AUCHINDOUN:
                questId = QUEST_AUCHINDOUN_AND_THE_RING_OF_OBSERVANCE;
                break;
            case NPC_TRIGGER_DARK_PORTAL:
                questId = IsHorde() ? QUEST_A_TRIP_TO_THE_DARK_PORTAL_H : QUEST_A_TRIP_TO_THE_DARK_PORTAL_A;
                break;
            case NPC_TRIGGER_AERIS_LANDING:
                questId = QUEST_JHEEL_IS_AT_AERIS_LANDING;
                break;
            case NPC_TRIGGER_THRONE_OF_ELEMENTS:
                questId = QUEST_VISIT_THE_THRONE_OF_THE_ELEMENTS;
                break;
            case NPC_TRIGGER_EXODAR_01:
                questId = QUEST_THE_SEAT_OF_THE_NAARU;
                break;
            case NPC_TRIGGER_SILVERMOON_01:
                questId = QUEST_WHEN_I_GROW_UP;
                break; // location for old script for future
            case NPC_TRIGGER_SILVERMOON_02:
                questId = QUEST_WHEN_I_GROW_UP;
                break; // location for old script for future
            case NPC_TRIGGER_CAVERNS_OF_TIME:
                questId = IsHorde() ? QUEST_TIME_TO_VISIT_THE_CAVERNS_H : QUEST_TIME_TO_VISIT_THE_CAVERNS_A;
                break;
            case NPC_TRIGGER_EXODAR_02:
                questId = QUEST_CALL_ON_THE_FARSEER;
                break;
            case NPC_TRIGGER_SILVERMOON_LVL70ETC:
                questId = QUEST_NOW_WHEN_I_GROW_UP;
                break;
            }
            if (questId)
            {
    ...
    void StartQuestSequence(uint32 questId)
    {
        ...
        case QUEST_HCHUU_AND_THE_MUSHROOM_PEOPLE:
            StartSporeggarScript();
            break;
        case QUEST_NOW_WHEN_I_GROW_UP:
            StartLvl70ETCScript();
            break;
        case QUEST_VISIT_THE_THRONE_OF_THE_ELEMENTS:
            StartThroneScript();
            break;

        case QUEST_AUCHINDOUN_AND_THE_RING_OF_OBSERVANCE:
            StartAuchindounScript();
            break;
        case QUEST_JHEEL_IS_AT_AERIS_LANDING:
            StartAerisLandingScript();
            break;
        case QUEST_THE_SEAT_OF_THE_NAARU:
            StartSeatScript();
            break;
        case QUEST_CALL_ON_THE_FARSEER:
            StartFarseerScript();
            break;
        }
    }
    ...
    void MovementInform(uint32 motionType, uint32 point) override
    {
        ...
        switch (point)
        {
        case POINT_CALL_OF_THE_FARSEER:
            HandleCallOfTheFarseer();
            break;
        case POINT_SEAT:
            HandleSeatOfTheNaaru();
            break;
        case POINT_DARK_PORTAL:
            HandleTripToTheDarkPortal();
            break;
        case POINT_SPOREGGAR:
            HandleHchuuAndTheMushroomPeople();
            break;
        case POINT_THRONE:
            HandleVisitTheThroneOfTheElements();
            break;
        case POINT_LVL70ETC:
            HandleNowWhenIGrowUp();
            break;
        case POINT_AERIS:
            HandleAerisLanding();
            break;
        case POINT_AUCHINDOUN:
            HandleAuchindounAndTheRingOfObservance();
            break;
        }
    }
    ...
    void HandleTripToTheDarkPortal()
    {
        ...
        {
        case 0:
            m_creature->GetMotionMaster()->MovePoint(POINT_DARK_PORTAL, -248.74657f, 959.56885f, 84.364746f,
                                                     FORCED_MOVEMENT_RUN);
        ...
        case 1:
            ...
            if (Unit *spawner = m_creature->GetSpawner())
                spawner->CastSpell(nullptr, IsHorde() ? SPELL_SUMMON_SOLDIER_TRIGGER_H : SPELL_SUMMON_SOLDIER_TRIGGER_A,
                                   TRIGGERED_OLD_TRIGGERED);
            timer = 3500;
            break;
        case 2: {
            CreatureList soldiers;
            GetCreatureListWithEntryInGrid(soldiers, m_creature, IsHorde() ? NPC_HORDE_SOLDIER : NPC_ALLIANCE_SOLDIER,
                                           50.f);
    ...
    // Not like in sniff because broken on retail - reconstructed through
    // guesswork
    ...
    void HandleHchuuAndTheMushroomPeople()
    {
        ...
        case 0:
            ...
            m_creature->GetMotionMaster()->MovePoint(POINT_SPOREGGAR, 215.625f, 8507.509f, 24.274683f,
                                                     FORCED_MOVEMENT_RUN);
            break;
        case 1:
    ...
    void HandleNowWhenIGrowUp()
    {
        ...
        {
        case 0:
            m_creature->GetMotionMaster()->MovePoint(POINT_LVL70ETC, 9495.876f, -7328.124f, 14.374042f,
                                                     FORCED_MOVEMENT_RUN);
            break;
        case 1:
            ...
            break;
        case 2:
            m_creature->GetMotionMaster()->MovePoint(POINT_LVL70ETC, 9507.839f, -7324.838f, 14.1189785f,
                                                     FORCED_MOVEMENT_RUN);
            break;
        case 3:
        ...
        case 8:
            if (Creature *trigger = GetClosestCreatureWithEntry(m_creature, NPC_TRIGGER_SILVERMOON_LVL70ETC, 40.f))
                m_creature->CastSpell(trigger->GetPositionX(), trigger->GetPositionY(), trigger->GetPositionZ(),
                                      SPELL_CHEER, TRIGGERED_NONE);
            timer = 5500;
            break;
        case 9:
            if (Creature *trigger = GetClosestCreatureWithEntry(m_creature, NPC_TRIGGER_SILVERMOON_LVL70ETC, 40.f))
    ...
    void HandleVisitTheThroneOfTheElements()
    {
        ...
        case 0:
            ...
            m_creature->GetMotionMaster()->MovePoint(POINT_THRONE, -716.1864f, 6943.2563f, 32.340847f,
                                                     FORCED_MOVEMENT_RUN);
            break;
        case 1:
    ...
    void HandleAerisLanding()
    {
        ...
        {
        case 0:
            m_creature->GetMotionMaster()->MovePoint(POINT_AERIS, -2080.9805f, 8555.506f, 23.479702f,
                                                     FORCED_MOVEMENT_RUN);
    ...
    void HandleSeatOfTheNaaru()
    {
        ...
        {
        case 0:
            m_creature->GetMotionMaster()->MovePoint(POINT_SEAT, -3895.899f, -11662.84f, -303.9426f,
