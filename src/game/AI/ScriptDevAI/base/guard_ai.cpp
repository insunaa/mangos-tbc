    ...
    if (!m_creature->IsNonMeleeSpellCasted(false))
    {
        ...
        if (m_creature->CanReachWithMeleeAttack(m_creature->GetVictim()))
        {
            ...
            if (m_creature->GetHealthPercent() < 30.0f && !urand(0, 2))
                spellInfo =
                    SelectSpell(m_creature, -1, -1, SELECT_TARGET_ANY_FRIEND, 0, 0, 0, 0, SELECT_EFFECT_HEALING);

            // No healing spell available, select a hostile spell
            if (spellInfo)
                bHealing = true;
            else
                spellInfo = SelectSpell(m_creature->GetVictim(), -1, -1, SELECT_TARGET_ANY_ENEMY, 0, 0, 0, 0,
