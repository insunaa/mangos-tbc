...
bool GossipHello_npc_threshwackonator(Player *pPlayer, Creature *pCreature)
{
    ...
    {
        pPlayer->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_TURN_KEY, GOSSIP_SENDER_MAIN,
...
struct npc_corrupted_furbolgAI : public ScriptedAI
{
    ...
    npc_corrupted_furbolgAI(Creature *creature) : ScriptedAI(creature)
    {
        AddCustomAction(EVENT_BECOME_PURIFIED, true, [&]() {
            ...
            m_creature->GetMotionMaster()->MoveRandomAroundPoint(bowlCoords.GetPositionX(), bowlCoords.GetPositionY(),
                                                                 bowlCoords.GetPositionZ(), 40.f);
        ...
        AddCustomAction(EVENT_START_PURIFICATION, true, [&]() {
            if (!bowlCoords.IsEmpty() && sqrt(bowlCoords.GetDistance(m_creature->GetPosition())) <= 60.f)
            {
                ...
                m_creature->GetNearPointAt(bowlCoords.GetPositionX(), bowlCoords.GetPositionY(),
                                           bowlCoords.GetPositionZ(), m_creature, x, y, z,
                                           m_creature->GetObjectBoundingRadius(), CONTACT_DISTANCE * 2.f, angle + M_PI);
    ...
    virtual bool UpdateFurbolgAI(const uint32 /*diff*/)
    {
        return true;
    }
...
{
    npc_blackwood_warriorAI(Creature *creature) : npc_corrupted_furbolgAI(creature)
    {
        Reset();
    }
...
{
    npc_blackwood_totemicAI(Creature *creature) : npc_corrupted_furbolgAI(creature)
    {
        Reset();
    }
...
bool ProcessEventId_event_purify_food(uint32 /*eventId*/, Object *source, Object *target, bool /*isStart*/)
{
    if (source->GetTypeId() == TYPEID_PLAYER && target->GetTypeId() == TYPEID_GAMEOBJECT)
    {
        ...
        for (int8 i = 0; i < 2; ++i)
        {
            bonfire->GetRandomPoint(bonfire->GetPositionX(), bonfire->GetPositionY(), bonfire->GetPositionZ(), 30.0f, x,
                                    y, z);
            if (Creature *warrior = bonfire->SummonCreature(NPC_BLACKWOOD_WARRIOR, x, y, z, 0.0f,
                                                            TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 120000))
                furbolgs.push_back(warrior);
        }

        // Activate the purification sequence for all hostile furbolgs (prevent
        // attracting already purified ones)
...
bool GOUse_go_furbolg_food(Player *player, GameObject *go)
{
    ...
    else
    {
        ...
        if (Creature *warrior =
                go->SummonCreature(NPC_BLACKWOOD_WARRIOR, x, y, z, 0.0f, TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 120000))
            warrior->AI()->AttackStart(player);
    }
