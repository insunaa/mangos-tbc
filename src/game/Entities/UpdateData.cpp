...
void UpdateData::Compress(void *dst, uint32 *dst_size, void *src, int src_size)
{
    ...
    if (z_res != Z_OK)
    {
        sLog.outError("Can't compress update packet (zlib: deflateInit) Error "
                      "code: %i (%s)",
                      z_res, zError(z_res));
        *dst_size = 0;
        return;
    ...
    {
        sLog.outError("Can't compress update packet (zlib: deflate should report "
    ...
    if (z_res != Z_OK)
    {
        sLog.outError("Can't compress update packet (zlib: deflateEnd) Error "
                      "code: %i (%s)",
                      z_res, zError(z_res));
        *dst_size = 0;
        return;
