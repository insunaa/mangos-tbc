...
struct GameObjectInfo
{
    ...
    bool CannotBeUsedUnderImmunity() const // Cannot be used/activated/looted by players under immunity
                                           // effects (example: Divine Shield)
    {
        switch (type)
        {
        case GAMEOBJECT_TYPE_DOOR:
...
struct GameObjectData
{
    ...
    {
        return urand(uint32(spawntimesecsmin), uint32(spawntimesecsmax));
    }
...
enum class GameObjectActions : uint32
{
    ...
    OPEN_AND_UNLOCK,  // Open + Unlock                    // Sets GO_STATE_ACTIVE
    ...
    CLOSE_AND_LOCK,   // Close + Lock                     // Sets GO_STATE_READY
                      // and
                      // sets GO_FLAG_LOCKED
...
class GameObject : public WorldObject
{
  public:
    ...
    ObjectGuid const GetSpawnerGuid() const
    {
        return m_spawnerGuid;
    }
    void SetSpawnerGuid(ObjectGuid guid)
    {
        m_spawnerGuid = guid;
    }
    ...
    void SetForcedDespawn()
    {
        m_forcedDespawn = true;
    };
    void Refresh();
    void Delete();
    ...
    void SetActionTarget(ObjectGuid guid)
    {
        m_actionTarget = guid;
    };
    ...
    {
        return m_lootGroupRecipientId || !m_lootRecipientGuid.IsEmpty();
    }
    ...
    {
        m_linkedTrap = linkedTrap->GetObjectGuid();
    }
    ...
    float GetCapturePointSliderValue() const
    {
        return m_captureSlider;
    }
    ...
    {
        return 1.f;
    } // to get away with ground collision
    ...
    void GenerateLootFor(Player *player); // used to tie chest loot to encounter
                                          // at the moment of its end
    ...
    GameObjectGroup *GetGameObjectGroup() const
    {
        return m_goGroup;
    }
  ...
  protected:
    ...
    time_t m_respawnTime;  // (secs) time of next respawn (or despawn if GO have
                           // owner()),
    uint32 m_respawnDelay; // (secs) if 0 then current GO state no dependent from
                           // timer
    ...
    GuidSet m_UniqueUsers;  // all players who use item, some items activated
