...
struct boss_warlord_kalithreshAI : public ScriptedAI
{
    ...
    void Reset() override
    {
        ...
        ResetTimer(WARLORD_KALITHRESH_ACTION_WARLORDS_RAGE,
                   GetInitialActionTimer(WARLORD_KALITHRESH_ACTION_WARLORDS_RAGE));
            ...
            if (GetActionReadyStatus(i))
            {
                ...
                case WARLORD_KALITHRESH_ACTION_WARLORDS_RAGE: {
                    ...
                    if (Creature *Distiller = GetClosestCreatureWithEntry(m_creature, NPC_NAGA_DISTILLER, 100.0f))
                    {
                        ...
                        m_creature->SetWalk(false,
                                            true); // Prevent him from slowing down while
                                                   // meleehit/casted upon while starting to move
                ...
                case WARLORD_KALITHRESH_ACTION_IMPALE: {
                    if (Unit *pTarget = m_creature->SelectAttackingTarget(
                            ATTACKING_TARGET_RANDOM, 0, nullptr, SELECT_FLAG_PLAYER | SELECT_FLAG_NOT_IN_MELEE_RANGE))
                    {
