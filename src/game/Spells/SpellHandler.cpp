...
void WorldSession::HandleUseItemOpcode(WorldPacket &recvPacket)
{
    ...
    DETAIL_LOG("WORLD: CMSG_USE_ITEM packet, bagIndex: %u, slot: %u, "
               "spell_index: %u , Item: %u, data length = %u",
...
void WorldSession::HandleOpenItemOpcode(WorldPacket &recvPacket)
{
    ...
    if (pItem->HasFlag(ITEM_FIELD_FLAGS, ITEM_DYNFLAG_WRAPPED)) // wrapped?
    {
        ...
        {
            sLog.outError("Wrapped item %u don't have record in character_gifts "
                          "table and will deleted",
                          pItem->GetGUIDLow());
...
void WorldSession::HandleGameObjectUseOpcode(WorldPacket &recv_data)
{
    ...
    if (!obj->IsSpawned())
    {
        sLog.outError("HandleGameObjectUseOpcode: CMSG_GAMEOBJ_USE for despawned "
    ...
    {
        sLog.outError("HandleGameObjectUseOpcode: CMSG_GAMEOBJ_USE for not allowed "
                      "GameObject type %u (Entry %u), didn't expect this to happen.",
                      obj->GetGoType(), obj->GetEntry());
        return;
    }

    if (obj->HasFlag(GAMEOBJECT_FLAGS,
                     GO_FLAG_LOCKED)) // we should not allow use of a locked GO
        return;
    ...
    {
        sLog.outError("HandleGameObjectUseOpcode: CMSG_GAMEOBJ_USE for "
                      "GameObject (Entry %u) "
                      "with non intractable flag (Flags %u), didn't expect "
                      "this to happen.",
...
void WorldSession::HandleCastSpellOpcode(WorldPacket &recvPacket)
{
    ...
    DEBUG_LOG("WORLD: CMSG_CAST_SPELL, spellId - %u, cast_count: %u data length "
              "= " SIZEFMTD,
...
void WorldSession::HandleSelfResOpcode(WorldPacket & /*recv_data*/)
{
    DEBUG_FILTER_LOG(LOG_FILTER_SPELL_CAST,
                     "WORLD: CMSG_SELF_RES"); // empty opcode
...
void WorldSession::HandleGetMirrorimageData(WorldPacket &recv_data)
{
    ...
    // FIXME: need proper packet structure for 2.x, currently no items and no
    // proper hair colors show.
    ...
    {
        // pCaster may have been nullptr (usually not expected, but may happen at
        // disconnect, etc) OR pCaster is not player, data is taken from
        // CreatureDisplayInfoExtraEntry by model already
        data << (uint8)0;
        data << (uint8)0;
        data << (uint8)0;
        data << (uint8)0;
