...
struct SpellScript
{
    ...
    // called on success inside Spell::finish - for channels this only happens if
    // whole channel went through
    virtual void OnSuccessfulFinish(Spell * /*spell*/) const
    {
    }
    // called at end of Spell::CheckCast - strict is true in Spell::Prepare
    virtual SpellCastResult OnCheckCast(Spell * /*spell*/, bool /*strict*/) const
    {
    ...
    // called on unit target hit before damage deal and proc or after effect
    // execution for targetless, dest, item and go
...
struct AuraCalcData
{
    ...
    Aura *aura; // cannot be used in auras that utilize stacking in checkcast -
                // can be nullptr
    AuraCalcData(Aura *aura, Unit *caster, Unit *target, SpellEntry const *spellProto, SpellEffectIndex effIdx)
        : aura(aura), caster(caster), target(target), spellProto(spellProto), effIdx(effIdx)
    {
    }
};
...
struct AuraScript
{
    ...
    // called during any event that calculates aura modifier amount - caster can
    // be nullptr
    virtual int32 OnAuraValueCalculate(AuraCalcData &data, int32 value) const
    {
        return value;
    }
    // called during done/taken damage calculation
    virtual void OnDamageCalculate(Aura * /*aura*/, Unit * /*victim*/, int32 & /*advertisedBenefit*/,
                                   float & /*totalMod*/) const
    {
    }
    // the following two hooks are done in an alternative fashion due to how they
    // are usually used if an aura is applied before, its removed after, and if
    // some aura needs to do something after aura effect is applied, need to
    // revert that change before its removed called before aura apply and after
    // aura unapply
    ...
    // are usually used if an aura is applied before, its removed after, and if
    // some aura needs to do something after aura effect is applied, need to
    // revert that change before its removed called after aura apply and before
    // aura unapply
    virtual void OnAfterApply(Aura * /*aura*/, bool /*apply*/) const
    {
    }
    // called during proc eligibility checking
    virtual bool OnCheckProc(Aura * /*aura*/, ProcExecutionData & /*data*/) const
    {
        return true;
    }
    ...
    virtual void OnAbsorb(Aura * /*aura*/, int32 & /*currentAbsorb*/, int32 & /*remainingDamage*/,
                          uint32 & /*reflectedSpellId*/, int32 & /*reflectDamage*/, bool & /*preventedDeath*/) const
    {
    }
    ...
    // called on periodic auras which need amount calculation (damage, heal,
    // burn, drain)
...
// note - linux name mangling bugs out if two script templates have same class
// name - avoid it
