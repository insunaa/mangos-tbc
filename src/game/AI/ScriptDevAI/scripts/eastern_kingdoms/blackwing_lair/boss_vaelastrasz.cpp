...
struct boss_vaelastraszAI : public CombatAI
{
    ...
    void HandleIntro()
    {
        ...
        case 0: {
            ...
            // Search for the Blackwing Technicians tormeting Vaelastrasz to make
            // them flee to the next room above the stairs
            ...
            for (CreatureList::const_iterator itr = lTechniciansList.begin(); itr != lTechniciansList.end(); ++itr)
            {
                ...
                // Each fleeing part and despawn is handled in DB, we only need
                // to make them run
                (*itr)->SetWalk(false);

                // The technicians will behave differently depending on they are
                // on the right or left side of Vaelastrasz. We compare their X
                // position to Vaelastrasz X position to sort them out
    ...
    void HandleSpeech()
    {
        ...
        case 0:
            ...
            break;
        case 1:
            // This one is actually 16 seconds but we only go to 10 seconds because
            // he starts attacking after he says "I must fight this!"
...
bool GossipSelect_boss_vaelastrasz(Player *pPlayer, Creature *creature, uint32 /*uiSender*/, uint32 uiAction)
{
    ...
    case GOSSIP_ACTION_INFO_DEF + 1:
        pPlayer->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_VAEL_2, GOSSIP_SENDER_MAIN,
