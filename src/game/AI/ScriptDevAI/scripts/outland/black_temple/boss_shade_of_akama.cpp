...
enum
{
    ...
    SPELL_FIXATE = 40607,             // akama on shade
                                      // SPELL_VERTEX_SHADE_BLACK      = 39833, // used by
                                      // the shade - in c_t_a
...
{
    npc_akamaAI(Creature *creature) : CombatAI(creature, AKAMA_ACTION_MAX), DialogueHelper(aOutroDialogue)
    {
    ...
    void ReceiveAIEvent(AIEventType eventType, Unit *sender, Unit * /*invoker*/, uint32 /*miscValue*/) override
    {
        if (eventType == AI_EVENT_CUSTOM_A)
        {
            ...
            case NPC_ASH_CHANNELER:
                ...
                // Note: the boss should be already slowly moving, but this isn't
                // possible because of the missing stack for the speed debuff
                break;
            }
        }
        else if (eventType == AI_EVENT_CUSTOM_B)
        {
            ...
            if (m_phase == PHASE_CHANNEL && sender->GetEntry() == NPC_SHADE_OF_AKAMA)
            {
                ...
                for (ObjectGuid guid : m_instance->GetGeneratorGuidVector()) // notify generators to
                                                                             // despawn sorcerers
    ...
    void DoSummonBrokenAshtongue()
    {
        ...
        for (auto loc : brokenSpawnLocations)
        {
            if (Creature *pBroken = m_creature->SummonCreature(NPC_ASH_BROKEN, loc.x, loc.y, loc.z, 0,
                ...
                pBroken->GetMotionMaster()->MovePoint(1, brokenDestinationLocations[i].x,
                                                      brokenDestinationLocations[i].y, brokenDestinationLocations[i].z);
            ++i;
        }
    }
...
struct npc_creature_generatorAI : public ScriptedAI
{
    ...
    void TrySummoning() // creature never enters combat, so no point in adding
                        // combat logic
    {
