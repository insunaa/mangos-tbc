...
CombatManeuverReturns PlayerbotPriestAI::DoFirstCombatManeuverPVE(Unit * /*pTarget*/)
{
    if (m_ai.IsHealer())
    {
        ...
        // This is cast on a target, which activates (and switches to another
        // target within the group) upon receiving+healing damage Mana efficient
        // even at one use
...
CombatManeuverReturns PlayerbotPriestAI::DoNextCombatManeuverPVE(Unit *pTarget)
{
    ...
    if (newTarget && !PlayerbotAI::IsNeutralized(newTarget)) // TODO: && party has a tank
    {
        ...
        if (m_ai.GetHealthPercent() < 35 && POWER_WORD_SHIELD > 0 &&
            !m_bot.HasAura(POWER_WORD_SHIELD, EFFECT_INDEX_0) && !m_bot.HasAura(WEAKNED_SOUL, EFFECT_INDEX_0))
        {
            ...
            else if (m_ai.IsHealer()) // Even if any other RETURN_ANY_OK - aside
                                      // from RETURN_CONTINUE
        ...
        // Night Elves priest bot can also cast Elune's Grace to improve his/her
        // dodge rating
        ...
        if (meleeReach)
        {
            // Already healed self or tank. If healer, do nothing else to anger
            // mob
            if (m_ai.IsHealer())
            ...
            if (newTarget->GetHealthPercent() > 25)
            {
                ...
                // Not an elite. You could insert PSYCHIC SCREAM here but in any
                // PvE situation that's 90-95% likely to worsen the situation for
                // the group.
                // ... So please don't.
    ...
    if (m_ai.IsHealer())
    {
        ...
        // Cast Shadow Word:Pain on current target and keep its up (if mana >=
        // 40% or target HP < 15%)
    ...
    case PRIEST_SPEC_HOLY:
        ...
        if (SMITE > 0 && m_ai.In_Reach(pTarget, SMITE) && CastSpell(SMITE, pTarget))
            return RETURN_CONTINUE;
        // if (HOLY_NOVA > 0 && m_ai.In_Reach(pTarget,HOLY_NOVA) && meleeReach &&
        // m_ai.CastSpell(HOLY_NOVA) == SPELL_CAST_OK)
        //     return RETURN_CONTINUE;
        break;
...
CombatManeuverReturns PlayerbotPriestAI::DoNextCombatManeuverPVP(Unit *pTarget)
{
    ...
    case PlayerbotAI::SCENARIO_PVP_DUEL:
        ...
        if (SHADOW_WORD_PAIN && m_ai.In_Reach(pTarget, SHADOW_WORD_PAIN) &&
            CastSpell(SHADOW_WORD_PAIN) & RETURN_ANY_OK) // TODO: Check whether enemy has it active
                                                         // yet
            return RETURN_CONTINUE;

        if (m_ai.GetHealthPercent() < 80 && RENEW && m_ai.In_Reach(pTarget, RENEW) &&
            CastSpell(RENEW) & RETURN_ANY_OK) // TODO: Check whether you have
                                              // renew active on you
            return RETURN_CONTINUE;
...
CombatManeuverReturns PlayerbotPriestAI::HealPlayer(Player *target)
{
    ...
    // Get a free and more efficient heal if needed: low mana for bot or average
    // health for target
    ...
    // if (hp < 40 && PRAYER_OF_HEALING > 0 && m_ai.CastSpell(PRAYER_OF_HEALING,
    // *target) & RETURN_CONTINUE)
    //    return RETURN_CONTINUE;
    // Group heal. Not really useful until a group check is available?
    // if (hp < 50 && CIRCLE_OF_HEALING > 0 && m_ai.CastSpell(CIRCLE_OF_HEALING,
    // *target) & RETURN_CONTINUE)
    //    return RETURN_CONTINUE;
...
// TODO: this and mage's BuffHelper are identical and thus could probably go in
// PlayerbotClassAI.cpp somewhere
