...
struct boss_kalecgosAI : public CombatAI
{
    boss_kalecgosAI(Creature *creature)
        : CombatAI(creature, KALECGOS_ACTION_MAX),
          m_instance(static_cast<instance_sunwell_plateau *>(creature->GetInstanceData()))
    {
        ...
        m_creature->GetCombatManager().SetLeashingCheck(
            [](Unit * /*unit*/, float /*x*/, float y, float /*z*/) { return y < 762 || y > 1076; });
    }
    ...
    void HandleExit()
    {
        ...
        case 0: {
            ...
            m_creature->SetByteValue(UNIT_FIELD_BYTES_1, UNIT_BYTES_1_OFFSET_MISC_FLAGS,
                                     UNIT_BYTE1_FLAG_FLY_ANIM | UNIT_BYTE1_FLAG_ALWAYS_STAND);
...
struct boss_sathrovarrAI : public CombatAI
{
    boss_sathrovarrAI(Creature *creature)
        : CombatAI(creature, SATHROVARR_ACTION_MAX),
          m_instance(static_cast<instance_sunwell_plateau *>(creature->GetInstanceData()))
    {
    ...
    void ReceiveAIEvent(AIEventType eventType, Unit * /*sender*/, Unit *invoker, uint32 /*miscValue*/) override
    {
        ...
        // spawn human Kalec; he starts to attack
            ...
            m_creature->SummonCreature(NPC_KALECGOS_HUMAN, aKalecHumanLoc[0], aKalecHumanLoc[1], aKalecHumanLoc[2],
                                       aKalecHumanLoc[3], TEMPSPAWN_CORPSE_DESPAWN, 0, true);
        else if (eventType == AI_EVENT_CUSTOM_C)
        {
            if (!m_firstShadow)
                return;
...
{
    boss_kalecgos_humanoidAI(Creature *creature)
        : CombatAI(creature, KALEC_HUMAN_ACTION_MAX),
          m_instance(static_cast<instance_sunwell_plateau *>(creature->GetInstanceData()))
    {
...
struct SpectralBlast : public SpellScript
{
    ...
    {
        if (spell->GetCaster()->getThreatManager().getThreatList().size() > 1 &&
            spell->GetCaster()->GetVictim() == target)
            return false;
    ...
    void OnEffectExecute(Spell *spell, SpellEffectIndex /*effIdx*/) const override
    {
        ...
        // Cast the spectral realm effect spell, visual spell and spectral blast
        // rift summoning
    ...
    void OnEffectExecute(Spell *spell, SpellEffectIndex effIdx) const override
    {
        if (effIdx == EFFECT_INDEX_0 && spell->GetUnitTarget())
            if (spell->GetCaster()->GetTypeId() == TYPEID_PLAYER)
                spell->GetUnitTarget()->AI()->SendAIEvent(AI_EVENT_CUSTOM_A, spell->GetCaster(),
                                                          spell->GetUnitTarget());
    }
};
...
struct CurseOfBoundlessAgony : public SpellScript, public AuraScript
{
    bool OnCheckTarget(const Spell *spell, Unit *target, SpellEffectIndex eff) const override
    {
        if (target->IsImmuneToSpell(spell->m_spellInfo, false, (1 << eff)) ||
            target->IsImmuneToSpellEffect(spell->m_spellInfo, eff, false))
            return false;
    ...
    void OnApply(Aura *aura, bool apply) const override
    {
        if (!apply && aura->GetRemoveMode() != AURA_REMOVE_BY_CANCEL &&
            aura->GetTarget()->GetMapId() == 580) // jump only in SWP
        {
    ...
    void OnPeriodicTickEnd(Aura *aura) const override
    {
        ...
        {
        case 0:
            spellId = SPELL_COBA_ANIM_1;
            break;
        case 1:
            spellId = SPELL_COBA_ANIM_2;
            break;
        default:
            spellId = SPELL_COBA_ANIM_3;
            break;
        }
...
struct CrazedRage : public SpellScript
{
    bool OnCheckTarget(const Spell *spell, Unit *target, SpellEffectIndex /*eff*/) const override
    {
        if (target->IsPlayer() || (target->GetEntry() != NPC_KALECGOS_DRAGON && target->GetEntry() != NPC_SATHROVARR) ||
