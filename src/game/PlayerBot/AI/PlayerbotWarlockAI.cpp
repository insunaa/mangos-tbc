...
CombatManeuverReturns PlayerbotWarlockAI::DoNextCombatManeuverPVE(Unit *pTarget)
{
    ...
    // Voidwalker sacrifice gives shield - but you lose the pet (and it's
    // DPS/tank) - use only as last resort for your own health!
    ...
    if (newTarget && !PlayerbotAI::IsNeutralized(newTarget)) // TODO: && party has a tank
    {
        ...
        // Have threat, can't quickly lower it. 3 options remain: Stop attacking,
        // lowlevel damage (wand), keep on keeping on.
        if (newTarget->GetHealthPercent() > 25)
        {
            ...
            if (m_ai.IsElite(newTarget))
            {
                ...
                // no need to check if newTarget can be a player because
                // IsElite() returns false for players
                ...
                return RETURN_NO_ACTION_OK; // do nothing and pray tank gets
                                            // aggro off you
            }

            // Not an elite. You could insert FEAR here but in any PvE situation
            // that's 90-95% likely to worsen the situation for the group. ... So
            // please don't.
    ...
    if (spec)
    {
        ...
        case WARLOCK_SPEC_AFFLICTION:
            if (CORRUPTION && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_SHADOW) &&
                ...
                return RETURN_CONTINUE;
            if (FIRE && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_FIRE) &&
                m_ai.In_Reach(pTarget, FIRE) && !pTarget->HasAura(FIRE) && CastSpell(FIRE, pTarget))
                return RETURN_CONTINUE;
            if (SIPHON_LIFE > 0 && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_SHADOW) &&
                m_ai.In_Reach(pTarget, SIPHON_LIFE) && !pTarget->HasAura(SIPHON_LIFE) &&
                CastSpell(SIPHON_LIFE, pTarget))
                return RETURN_CONTINUE;
...
bool PlayerbotWarlockAI::CheckCurse(Unit *target)
{
    ...
    if (target->GetTypeId() == TYPEID_UNIT)
    {
        ...
        {
            if (CURSE_OF_EXHAUSTION && m_ai.In_Reach(target, CURSE_OF_EXHAUSTION) &&
                !target->HasAura(CURSE_OF_EXHAUSTION))
            {
    ...
    // If bot already put a curse and curse is still active on target: no need to
    // go further
    ...
    // Enemy elite mages have low health but can cast dangerous spells: group
    // safety before bot DPS OR if facing mana user player
...
void PlayerbotWarlockAI::DoNonCombatActions()
{
    ...
    if (shardCount > MAX_SHARD_COUNT || (freeSpace == 0 && shardCount > 1))
        m_bot.DestroyItemCount(SOUL_SHARD, shardCount > MAX_SHARD_COUNT ? shardCount - MAX_SHARD_COUNT : 1, true,
                               false);

    // buff myself DEMON_SKIN, DEMON_ARMOR, FEL_ARMOR - Strongest one available
    // is chosen
    if (FEL_ARMOR)
    {
    ...
    if (CREATE_SOULSTONE)
    {
        ...
        if (!soulStone)
        {
            ...
            if (m_ai.CanStore())
            {
                if (shardCount > 0 && m_bot.IsSpellReady(CREATE_SOULSTONE) &&
                    m_ai.CastSpell(CREATE_SOULSTONE) == SPELL_CAST_OK)
                    return;
            }
        }
        else
        {
            ...
            {
                // TODO: first choice: healer. Second choice: anyone else with
                // revive spell. Third choice: self or master.
    ...
    // Spellstone creation and use (Spellstone dominates firestone completely as
    // I understand it)
