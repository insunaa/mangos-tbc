  ...
  public:
    ...
    */
    ...
    /*!
      ...
      client has to show it's proof first. If the server detects an incorrect
      proof it must abort without showing it's proof. \param sha reference to an
      empty Sha1Hash object
    */
    void Finalize(Sha1Hash &sha);

    BigNumber GetHostPublicEphemeral(void)
    {
        return B;
    };
    BigNumber GetGeneratorModulo(void)
    {
        return g;
    };
    BigNumber GetPrime(void)
    {
        return N;
    };
    BigNumber GetProof(void)
    {
        return M;
    };
    BigNumber GetSalt(void)
    {
        return s;
    };
    BigNumber GetStrongSessionKey(void)
    {
        return K;
    };
    BigNumber GetVerifier(void)
    {
        return v;
    };

    bool SetSalt(const char *new_s);
    void SetStrongSessionKey(const char *new_K)
    {
        K.SetHexStr(new_K);
    };
