...
enum Classes
{
    ...
    CLASS_ROGUE = 4,
    CLASS_PRIEST = 5,
    // CLASS_DEATH_KNIGHT = 6,                             // not listed in DBC,
    // will be in 3.0
    CLASS_SHAMAN = 7,
    CLASS_MAGE = 8,
    CLASS_WARLOCK = 9,
    // CLASS_MONK       = 10,                              // not listed in DBC,
    // will be in 5.0
    CLASS_DRUID = 11,
    // CLASS_DEMON_HUNTER = 12,                            // not listed in DBC,
    // will be in 7.0
};
...
// array index could be used to store class data only Warrior, Paladin and Mage
...
/**
 * The different spell schools that are available, used in both damage
 * calculation and spell casting to decide what should be affected, the
 * SPELL_SCHOOL_NORMAL is the armor, others should be self explanatory.
 *
...
enum Team
{
    ...
    TEAM_INVALID = 1,      // used to invalidate some team depending checks (means not
                           // for both teams)
    HORDE = 67,
    ALLIANCE = 469,
};
...
// some spell that related to ImmuneToDispel or ImmuneToSchool or
// ImmuneToDamage type can't cast to it, some spell_effects that related to
...
enum CreatureTypeFlags
{
    ...
    CREATURE_TYPEFLAGS_GHOST_VISIBLE = 0x00000002, // Creatures which can _also_ be seen when player is a ghost,
    ...
    CREATURE_TYPEFLAGS_UNK5 = 0x00000010,           // controls something in client tooltip
                                                    // related to creature faction
    CREATURE_TYPEFLAGS_UNK6 = 0x00000020,           // may be sound related
    CREATURE_TYPEFLAGS_UNK7 = 0x00000040,           // may be related to attackable / not attackable creatures
    ...
    CREATURE_TYPEFLAGS_CAN_ASSIST = 0x00001000,     // Can aid any player (and group) in combat. Typically seen
                                                    // for escorting NPC's
...
enum DiminishingGroup
{
    ...
    DIMINISHING_CHARM,
    // Mage Specific
    DIMINISHING_DRAGONS_BREATH,              // Should diminish with itself as it is not part
                                             // of DIMINISHING_BLIND_CYCLONE Rogue Specific
    ...
    DIMINISHING_UNSTABLE_AFFLICTION_SILENCE, // The silence from this ability is
                                             // now subject to diminishing
                                             // returns. -
                                             // https://wow.gamepedia.com/Patch_2.3.0
                                             // Shared Class Specific
    DIMINISHING_KNOCKOUT_POLYMORPH_SAPPED,   // Includes all knockout mechanics
                                             // (gouge, maim, repentance),
                                             // polymorph and sap
    DIMINISHING_BLIND_CYCLONE,               // From 2.3.0
    DIMINISHING_DISARM,                      // From 2.3.0
    // DIMINISHING_SILENCE, // From 3.0.8 - "All Silence
    // spells now have diminishing returns." -
    // https://wowwiki.fandom.com/wiki/Patch_3.0.8
...
enum InstanceResetMethod
{
    ...
    INSTANCE_RESET_RESPAWN_DELAY // called from reset scheduler for request reset
                                 // at map unload when map loaded at reset
...
{
    SHAPESHIFT_FLAG_STANCE = 0x00000001,                 // Form allows various player activities, which normally
                                                         // cause "You can't X while shapeshifted." errors (npc/go
                                                         // interaction, item use, etc)
    SHAPESHIFT_FLAG_NOT_TOGGLEABLE = 0x00000002,         // NYI
    SHAPESHIFT_FLAG_PERSIST_ON_DEATH = 0x00000004,       // NYI
    SHAPESHIFT_FLAG_CAN_NPC_INTERACT = 0x00000008,       // Form unconditionally allows talking to NPCs while
...
// expansions installed account with expansion > client supported will rejected
// at connection by client because if client receive unsupported expansion
// level it think that it not have expansion installed and reject
enum Expansions
{
