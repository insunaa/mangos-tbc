...
SD%Complete: 75
SDComment: Thrall reset on server restart is not supported, because of core
limitation. SDCategory: Caverns of Time, Old Hillsbrad Foothills EndScriptData
*/
...
void instance_old_hillsbrad::SetData(uint32 uiType, uint32 uiData)
{
        ...
        if (uiData == IN_PROGRESS)
        {
            ...
            if (m_uiBarrelCount == MAX_BARRELS)
            {
                ...
                if (Player *pPlayer = GetPlayerInMap())
                {
                    ...
                    for (GuidList::const_iterator itr = m_lRoaringFlamesList.begin(); itr != m_lRoaringFlamesList.end();
                         ++itr)
                        DoRespawnGameObject(*itr, 30 * MINUTE);

                    // move the orcs outside the houses
                    float fX, fY, fZ;
                    for (GuidList::const_iterator itr = m_lRightPrisonersList.begin();
                         itr != m_lRightPrisonersList.end(); ++itr)
                    {
                        if (Creature *pOrc = instance->GetCreature(*itr))
                        {
                            pOrc->GetRandomPoint(afInstanceLoc[0][0], afInstanceLoc[0][1], afInstanceLoc[0][2], 10.0f,
                                                 fX, fY, fZ);
                            pOrc->SetWalk(false);
                    ...
                    for (GuidList::const_iterator itr = m_lLeftPrisonersList.begin(); itr != m_lLeftPrisonersList.end();
                         ++itr)
                    {
                        if (Creature *pOrc = instance->GetCreature(*itr))
                        {
                            pOrc->GetRandomPoint(afInstanceLoc[1][0], afInstanceLoc[1][1], afInstanceLoc[1][2], 10.0f,
                                                 fX, fY, fZ);
                            pOrc->SetWalk(false);
                            pOrc->GetMotionMaster()->MovePoint(0, fX, fY, fZ);
                            pOrc->SetStandState(UNIT_STAND_STATE_STAND);
                    ...
                    debug_log("SD2: Instance Old Hillsbrad: SetData (Type: %u Data %u) "
                              "cannot find any pPlayer.",
                              uiType, uiData);
    ...
    case TYPE_THRALL_EVENT:
        ...
        if (uiData == FAIL)
        {
            ...
            // If we already respawned Thrall too many times, the event is failed
            // for good
            else if (m_uiThrallEventCount > MAX_WIPE_COUNTER)
                debug_log("SD2: Instance Old Hillsbrad: Thrall event failed %u times. "
                          "Reset instance required.",
                          m_uiThrallEventCount);
        }
        break;
    case TYPE_DRAKE:
    ...
    case TYPE_EPOCH:
        m_auiEncounter[uiType] = uiData;
        debug_log("SD2: Instance Old Hillsbrad: Thrall event type %u adjusted to "
                  "data %u.",
                  uiType, uiData);
        break;
    }
...
void instance_old_hillsbrad::Load(const char *chrIn)
{
    ...
    // custom reload - if the escort event or the Epoch event are not done, then
    // reset the escort this is done, because currently we cannot handle Thrall
    // relocation on server reset
