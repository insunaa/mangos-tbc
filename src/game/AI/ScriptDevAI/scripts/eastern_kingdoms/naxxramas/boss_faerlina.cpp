...
enum
{
    ...
    // SOUND_RANDOM_AGGRO       = 8955,                     // soundId containing
    // the 4 aggro sounds, we not using this

    SPELL_POISONBOLT_VOLLEY = 28796,
    SPELL_ENRAGE = 28798,
    SPELL_RAIN_OF_FIRE = 28794,
    SPELL_WIDOWS_EMBRACE_1 = 28732, // Cast onto Faerlina by Mind Controlled adds
    SPELL_WIDOWS_EMBRACE_2 = 28797, // Cast onto herself by Faerlina and handle
                                    // all the cooldown and enrage debuff
    SPELL_INSTAKILL_SELF = 28748,
};

static const float resetZ = 266.0f; // Above this altitude, Faerlina is outside
                                    // her room and should reset (leashing)
...
struct boss_faerlinaAI : public CombatAI
{
    ...
    // Widow's Embrace prevents Frenzy and Poison bolt by activating 30 sec spell
    // cooldown If target also has Frenzy, it will remove it and delay next one
    // by 60s
    void SpellHit(Unit * /*caster*/, const SpellEntry *spellEntry) override
    {
        if (spellEntry->Id == SPELL_WIDOWS_EMBRACE_1)
        {
            DoCastSpellIfCan(m_creature,
                             SPELL_WIDOWS_EMBRACE_2); // Start spell category cooldown, delaying
                                                      // next Poison Bolt and Frenzy by 30 secs

            if (m_creature->HasAura(SPELL_ENRAGE))
            {
                m_creature->RemoveAurasDueToSpell(SPELL_ENRAGE);
                ResetCombatAction(FAERLINA_ENRAGE,
                                  60 * IN_MILLISECONDS); // Delay next enrage by 60 sec if
                                                         // Faerlina was already enraged
            }
        }
    }
    ...
    void OnEffectExecute(Spell *spell, SpellEffectIndex effIdx) const override
    {
        if (effIdx == EFFECT_INDEX_1)
        {
            if (Unit *caster = spell->GetCaster())
                caster->CastSpell(nullptr, SPELL_INSTAKILL_SELF,
                                  TRIGGERED_OLD_TRIGGERED); // Self suicide
        }
    }
};
