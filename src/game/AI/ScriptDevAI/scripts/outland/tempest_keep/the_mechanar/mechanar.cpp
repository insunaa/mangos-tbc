...
void instance_mechanar::SetData(uint32 uiType, uint32 uiData)
{
    ...
    if (uiData == TYPE_GYRO_KILL || uiData == TYPE_IRON_HAND)
        if (GetData(TYPE_GYRO_KILL) == DONE && GetData(TYPE_IRON_HAND) == DONE)
            if (GameObject *chest = GetSingleGameObjectFromStorage(
                    instance->IsRegularDifficulty() ? GO_CACHE_OF_THE_LEGION : GO_CACHE_OF_THE_LEGION_H))
    ...
    if (Player *pPlayer = GetPlayerInMap(true, false))
    {
        for (uint8 i = 0; i < MAX_BRIDGE_TRASH; ++i)
        {
            ...
            if (Creature *pTemp = pPlayer->SummonCreature(
                    aBridgeEventLocs[m_uiBridgeEventPhase][i].m_uiSpawnEntry,
                    aBridgeEventLocs[m_uiBridgeEventPhase][i].m_fX, aBridgeEventLocs[m_uiBridgeEventPhase][i].m_fY,
                    aBridgeEventLocs[m_uiBridgeEventPhase][i].m_fZ, aBridgeEventLocs[m_uiBridgeEventPhase][i].m_fO,
                    TEMPSPAWN_DEAD_DESPAWN, 0))
            {
