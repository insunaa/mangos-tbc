...
struct boss_moroesAI : public CombatAI
{
    ...
    // Wrapper to remove the Garrote aura on death and on evade - ToDo: maybe
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case MOROES_ACTION_VANISH: {
            ...
            if (!target) // if no target without garrote found - select any
                         // random
                target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, nullptr, SELECT_FLAG_PLAYER);
            if (!target)
                break;
            target->CastSpell(nullptr, SPELL_TAUNT,
                              TRIGGERED_OLD_TRIGGERED);                  // TODO: Needs to send both packets
