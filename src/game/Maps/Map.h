...
class Map : public GridRefManager<NGridType>
{
  ...
  public:
    ...
    void MessageDistBroadcast(Player const *, WorldPacket const &, float dist, bool to_self,
    ...
    bool IsRemovalGrid(float x, float y) const
    {
        ...
        return (!getNGrid(p.x_coord, p.y_coord) ||
                getNGrid(p.x_coord, p.y_coord)->GetGridState() == GRID_STATE_REMOVAL);
    }
    ...
    // _currently_ spawnmode == difficulty, but this can be changes later, so use
    // appropriate spawnmode/difficult functions for simplify later code support
    ...
    bool isCellMarked(uint32 pCellId) const
    {
    ...
    enum ScriptExecutionParam
    {
        ...
        SCRIPT_EXEC_PARAM_UNIQUE_BY_SOURCE = 0x01, // Start Script only if not yet started (uniqueness identified by
                                                   // id and source)
        SCRIPT_EXEC_PARAM_UNIQUE_BY_TARGET = 0x02, // Start Script only if not yet started (uniqueness identified by
                                                   // id and target)
        SCRIPT_EXEC_PARAM_UNIQUE_BY_SOURCE_TARGET = 0x03, // Start Script only if not yet started (uniqueness identified
                                                          // by id, source and target)
    };
    ...
    Unit *GetUnit(ObjectGuid guid);               // only use if sure that need objects at
                                                  // current map, specially for player case
    WorldObject *GetWorldObject(ObjectGuid guid); // only use if sure that need objects at
                                                  // current map, specially for player case
    // dbguid methods
    ...
    std::map<uint32, uint32> &GetTempCreatures()
    {
        return m_tempCreatures;
    }
    std::map<uint32, uint32> &GetTempPets()
    {
        return m_tempPets;
    }
    ...
    void MonsterYellToMap(CreatureInfo const *cinfo, int32 textId, ChatMsg chatMsg, Language language,
    ...
    TransportSet const &GetTransports()
    {
        return m_transports;
    }

    GraveyardManager &GetGraveyardManager()
    {
        return m_graveyardManager;
    }
    ...
    MapDataContainer &GetMapDataContainer()
    {
        return m_dataContainer;
    }
    MapDataContainer const &GetMapDataContainer() const
    {
        return m_dataContainer;
    }
    WorldStateVariableManager &GetVariableManager()
    {
        return m_variableManager;
    }
    WorldStateVariableManager const &GetVariableManager() const
    {
        return m_variableManager;
    }

    // debug
    std::set<ObjectGuid> m_objRemoveList; // this will eventually eat up too much memory - only
                                          // used for debugging VisibleNotifier::Notify()
                                          // customlog leak
