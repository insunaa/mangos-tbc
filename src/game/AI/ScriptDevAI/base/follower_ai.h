...
class FollowerAI : public ScriptedAI
{
  public:
    ...
    virtual void UpdateFollowerAI(const uint32 diff); // used when it's needed to add code in update
                                                      // (abilities, scripted events, etc)

    void StartFollow(Player *leader, uint32 factionForFollower = 0, const Quest *quest = nullptr);

    void SetFollowPaused(bool paused); // if special event require follow mode
                                       // to hold/resume during the follow
    void SetFollowComplete(bool withEndEvent = false);

    bool HasFollowState(uint32 followState) const
    {
        return (m_followState & followState) != 0;
    }
