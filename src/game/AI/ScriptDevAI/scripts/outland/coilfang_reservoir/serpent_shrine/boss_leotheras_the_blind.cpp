...
enum
{
    ...
    SPELL_CONS_MADNESS = 37749,  // charm spell for the players which didn't kill
                                 // the inner demons during the demon phase
...
struct boss_leotheras_the_blindAI : public CombatAI
{
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case LEOTHERAS_PHASE_2:
            ...
            {
                m_creature->RemoveAurasDueToSpell(SPELL_WHIRLWIND); // whirlwind is removed on entering demon
                                                                    // form
        ...
        case LEOTHERAS_METAMORPHOSIS:
            ...
            {
                if (DoCastSpellIfCan(nullptr, SPELL_METAMORPHOSIS) ==
                    CAST_OK) // TODO: make this stuff happen on cast end
                {
                    m_creature->RemoveAurasDueToSpell(SPELL_WHIRLWIND); // whirlwind is removed on entering demon
                                                                        // form
        ...
        case LEOTHERAS_INSIDIOUS_WHISPER:
            m_creature->GetInstanceData()->SetData(TYPE_LEOTHERAS_EVENT_DEMONS,
                                                   0); // start counter from zero
...
struct InsidiousWhisper : public SpellScript, public AuraScript
{
    ...
    bool OnCheckTarget(const Spell *spell, Unit *target, SpellEffectIndex /*eff*/) const override
    {
        ...
        if (!target->HasFlag(UNIT_FIELD_FLAGS,
                             UNIT_FLAG_PLAYER_CONTROLLED)) // not MCed
            return false;
    ...
    void OnApply(Aura *aura, bool apply) const override
    {
        ...
        if (apply)
        {
            target->CastSpell(target, 37735,
                              TRIGGERED_OLD_TRIGGERED); // Summon Inner Demon
        ...
        if (!apply)
        {
            if (aura->GetRemoveMode() == AURA_REMOVE_BY_EXPIRE) // MC player if inner demon was not
                                                                // killed
            {
                if (Unit *caster = aura->GetCaster())
                {
                    caster->CastSpell(target, 37749,
                                      TRIGGERED_OLD_TRIGGERED); // Consuming Madness
