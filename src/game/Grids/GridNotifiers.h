...
struct VisibleChangesNotifier
{
    ...
    explicit VisibleChangesNotifier(WorldObject &object)
        : i_object(object), m_unvisitedGuids(i_object.GetClientGuidsIAmAt())
    {
    }
    ...
    GuidSet &GetUnvisitedGuids()
    {
        return m_unvisitedGuids;
    }
...
struct SomeSearcher
{
    ...
    SomeSearcher(ResultType& result, Check & check)
        : i_phaseMask(check.GetFocusObject().GetPhaseMask()), i_result(result),
i_check(check) {}
...
// Last accepted by Check GO if any (Check can change requirements at each
// call)
...
// Last accepted by Check Creature if any (Check can change requirements at
// each call)
...
{
    public:
        SomeCheck(SomeObjecType const* fobj, ..some other args) : i_fobj(fobj),
...other inits {} WorldObject const& GetFocusObject() const { return *i_fobj; }
        bool operator()(Creature* u)                    and for other
intresting typs (Player/GameObject/Camera
        {
            return ..(code return true if Object fit to requirenment);
        }
        template<class NOT_INTERESTED> bool operator()(NOT_INTERESTED*) {
return false; } private: SomeObjecType const* i_fobj;                    //
Focus object used for check distance from, phase, so place in world
...
// Success at unit in range, range update for next check (this can be used with
...
{
  public:
    AllGameObjectsMatchingOneEntryInRange(WorldObject const *pObject, std::vector<uint32> const &entries,
                                          float fMaxRange)
...
// Success at gameobject in range of xyz, range update for next check (this can
// be use with GameobjectLastSearcher to find nearest GO)
...
class FriendlyEligibleDispelInRangeCheck
{
  public:
    ...
    Unit const &GetFocusObject() const
    {
        return *i_obj;
    }
    bool operator()(Unit *u)
    {
        ...
        if (!u->IsImmobilizedState() && !u->GetMaxNegativeAuraModifier(SPELL_AURA_MOD_DECREASE_SPEED) &&
            !u->isFrozen() && !u->IsCrowdControlled())
            return false;
  ...
  public:
    ...
    bool operator()(Unit *u)
    {
        return u->IsAlive() && i_obj->IsWithinDistInMap(u, i_range, true, i_ignorePhase) &&
               i_obj->CanAssistSpell(u, i_spellInfo);
    }
...
{
  public:
    AnyFriendlyOrGroupMemberUnitInUnitRangeCheck(Unit const *obj, Unit const *target, Group const *group,
                                                 SpellEntry const *spellInfo, float range)
    ...
    bool operator()(Unit *u)
    {
        ...
        if (i_group)
        {
            ...
            case TYPEID_UNIT: {
                ...
                // the only other possible group members besides players are
                // pets, we exclude anyone else
                if (!creature->IsPet())
                    return false;
    ...
    bool operator()(Unit *currUnit)
    {
        if (currUnit->IsAlive() &&
            (m_source->IsAttackedBy(currUnit) || (m_owner && m_owner->IsAttackedBy(currUnit)) ||
             m_source->IsEnemy(currUnit) || currUnit->IsEnemy(m_source)) &&
        ...
        {
            m_range = m_source->GetDistance(currUnit); // use found unit range as new
                                                       // range limit for next check
            return true;
        }
  ...
  public:
    ...
    bool operator()(Creature *u)
    {
            ...
            (!i_excludeSelf || (&i_obj != u)))
        {
            ...
            if (dist < i_range)
            {
                i_range = dist; // use found unit range as new range limit for next
                                // check
                return true;
            }
...
// Success at unit in range, range update for next check (this can be used with
// CreatureListSearcher to find creatures with given entry)
class AllCreatureEntriesWithLiveStateInObjectRangeCheck
{
  public:
    ...
    bool operator()(Creature *u)
    {
        if (i_entries.find(i_guids ? u->GetDbGuid() : u->GetEntry()) != i_entries.end() &&
    ...
    bool FoundOutOfRange() const
    {
        return i_foundOutOfRange;
    }
...
class AnyPlayerInObjectRangeWithAuraCheck
{
  public:
    ...
    bool operator()(Player *u)
    {
        return u->IsAlive() && i_obj->IsWithinDistInMap(u, i_range) && u->HasAura(i_spellId);
    }
...
class AnyPlayerInCapturePointRange
{
  public:
    ...
    bool operator()(Player *u)
    {
        return u->CanUseCapturePoint() && i_obj->IsWithinDistInMap(u, i_range);
    }
