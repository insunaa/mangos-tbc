...
#ifdef __clang__
// clang-cl doesn't have these hardcoded types available, correct
// ehdata_forceinclude.h that relies on it
...
//
// Declare the static variables of the WheatyExceptionReport class and force
// their initialization before any other static in the program
//
#pragma warning(push)
...
LONG WINAPI WheatyExceptionReport::WheatyUnhandledExceptionFilter(PEXCEPTION_POINTERS pExceptionInfo)
{
    ...
    sprintf(m_szDumpFileName, "%s\\%s_%s_[%u-%u_%u-%u-%u].dmp", crash_folder_path, shortHash, pos, systime.wDay,
            systime.wMonth, systime.wHour, systime.wMinute, systime.wSecond);

    _stprintf(m_szLogFileName, _T("%s\\%s_%s_[%u-%u_%u-%u-%u].txt"), crash_folder_path, shortHash, pos, systime.wDay,
              systime.wMonth, systime.wHour, systime.wMinute, systime.wSecond);

    m_hDumpFile =
        CreateFile(m_szDumpFileName, GENERIC_WRITE, 0, nullptr, OPEN_ALWAYS, FILE_FLAG_WRITE_THROUGH, nullptr);
    ...
    if (m_hDumpFile)
    {
        ...
        if (pExceptionInfo->ExceptionRecord->ExceptionCode == 0xC0000420L &&
        ...
        MiniDumpWriteDump(GetCurrentProcess(), GetCurrentProcessId(), m_hDumpFile,
                          MiniDumpWithIndirectlyReferencedMemory, &info, &additionalStreamInfo, nullptr);
...
template <size_t size> void ToTchar(wchar_t const *src, TCHAR (&dst)[size], std::true_type)
{
    wcstombs_s(nullptr, dst, src, size);
}

template <size_t size> void ToTchar(wchar_t const *src, TCHAR (&dst)[size], std::false_type)
{
    wcscpy_s(dst, src);
}
    ...
    *szVersion = _T('\0');
    ...
    case VER_PLATFORM_WIN32_NT: {
        ...
        if (osvi.dwMajorVersion == 4 && _tcsicmp(szCSDVersion, _T("Service Pack 6")) == 0)
        {
            ...
            if (lRet == ERROR_SUCCESS)
            {
                _stprintf(wszTmp, _T("Service Pack 6a (Version %d.%d, Build %d)"), osvi.dwMajorVersion,
                          osvi.dwMinorVersion, osvi.dwBuildNumber & 0xFFFF);
                _tcsncat(szVersion, wszTmp, cntMax);
            }
            else // Windows NT 4.0 prior to SP6a
            {
                _stprintf(wszTmp, _T("%s (Version %d.%d, Build %d)"), szCSDVersion, osvi.dwMajorVersion,
                          osvi.dwMinorVersion, osvi.dwBuildNumber & 0xFFFF);
                _tcsncat(szVersion, wszTmp, cntMax);
            }
            ::RegCloseKey(hKey);
...
void WheatyExceptionReport::PrintSystemInfo()
{
    ...
    if (_GetProcessorName(sString, std::size(sString)))
        Log(_T("*** Hardware ***\r\nProcessor: %s\r\nNumber Of Processors: ")
            ...
            _T("Limit: %d MB\r\n"),
            sString, SystemInfo.dwNumberOfProcessors, int(MemoryStatus.ullTotalPhys / div),
            int(MemoryStatus.ullAvailPhys / div), int(MemoryStatus.ullTotalPageFile / div));
    else
        Log(_T("*** Hardware ***\r\nProcessor: <unknown>\r\nNumber Of ")
            _T("Processors: ")
            _T("%d\r\nPhysical Memory: %d MB (Available: %d MB)\r\nCommit Charge ")
            _T("Limit: %d MB\r\n"),
            SystemInfo.dwNumberOfProcessors, int(MemoryStatus.ullTotalPhys / div), int(MemoryStatus.ullAvailPhys / div),
            int(MemoryStatus.ullTotalPageFile / div));
...
void WheatyExceptionReport::GenerateExceptionReport(PEXCEPTION_POINTERS pExceptionInfo)
{
    __try
    {
        ...
        DWORD section;
        DWORD_PTR offset;
        GetLogicalAddress(pExceptionRecord->ExceptionAddress, szFaultingModule, sizeof(szFaultingModule), section,
                          offset);
        ...
        {
            Log(_T("\n\rCRITICAL ERROR.\n\r Couldn't initialize the symbol ")
                _T("handler ")
                _T("for process.\n\rError [%s].\n\r\n\r"),
        ...
        if (pExceptionRecord->ExceptionCode == 0xE06D7363 && pExceptionRecord->NumberParameters >= 2)
        {
            ...
            // When _EH_RELATIVE_TYPEINFO is defined, the pointers need to be
            // retrieved with some pointer math
            ...
            CatchableTypeArray const *catchables =
                reinterpret_cast<CatchableTypeArray const *>(resolveExceptionRVA(throwInfo->pCatchableTypeArray));
            CatchableType const *catchable =
                catchables->nCatchableTypes
                    ? reinterpret_cast<CatchableType const *>(resolveExceptionRVA(catchables->arrayOfCatchableTypes[0]))
                    : nullptr;
            TypeDescriptor const *exceptionTypeinfo =
                catchable ? reinterpret_cast<TypeDescriptor const *>(resolveExceptionRVA(catchable->pType)) : nullptr;
            ...
            if (exceptionTypeinfo)
            {
                ...
                std::exception const *exceptionPtr = [](void *object, TypeDescriptor const *typeInfo,
                                                        void *stdExceptionTypeInfo) -> std::exception const * {
                    try
                    {
                        ...
                        // return r;
                        return reinterpret_cast<std::exception const *>(
                            __RTDynamicCast(object, 0, stdExceptionTypeInfo, (void *)typeInfo, false));
                    }
                    catch (...)
                    {
                        return nullptr;
                ...
                // dynamic_cast<type>(variable_that_already_has_that_type) is
                // optimized away by compiler and attempting to call
                // __RTDynamicCast fails for it
                ...
                if (UnDecorateSymbolName(&exceptionTypeinfo->name[1], &undName[0], MAX_SYM_NAME,
...
void WheatyExceptionReport::DumpTypeIndex(DWORD64 modBase, DWORD dwTypeIndex, DWORD_PTR offset, bool &bHandled,
                                          char const *Name, char const * /*suffix*/, bool newSymbol, bool logChildren)
{
    bHandled = false;
    ...
    if (SymGetTypeInfo(m_hProcess, modBase, dwTypeIndex, TI_GET_SYMNAME, &pwszTypeName))
    {
        ...
        if (wcscmp(pwszTypeName, L"std::basic_string<char,std::char_traits<char>,"
                                 L"std::allocator<char> >") == 0)
        {
    ...
    case SymTagData:
        if (SymGetTypeInfo(m_hProcess, modBase, dwTypeIndex, TI_GET_TYPEID, &innerTypeID))
        {
            ...
            case SymTagUDT:
                if (symbolDetails.size() >= WER_MAX_NESTING_LEVEL)
                    logChildren = false;
                DumpTypeIndex(modBase, innerTypeID, offset, bHandled, symbolDetails.top().Name.c_str(), "", false,
                              logChildren);
                break;
            case SymTagPointerType:
                if (Name != nullptr && Name[0] != '\0')
            ...
            default:
                for (DWORD index = 0; index < elementsCount && index < WER_MAX_ARRAY_ELEMENTS_COUNT; index++)
                {
                    ...
                    if (index == 0)
                    {
                        ...
                        {
                            FormatOutputValue(buffer, basicType, length, (PVOID)(offset + length * index),
                                              sizeof(buffer));
                    ...
                    {
                        DumpTypeIndex(modBase, innerTypeID, offset + length * index, elementHandled, "", "", false,
                                      false);
                        if (!elementHandled)
                        {
                            FormatOutputValue(buffer, basicType, length, (PVOID)(offset + length * index),
...
void WheatyExceptionReport::FormatOutputValue(char *pszCurrBuffer, BasicType basicType, DWORD64 length, PVOID pAddress,
                                              size_t bufferSize, size_t countOverride)
{
    __try
    {
        switch (basicType)
...
DWORD_PTR
WheatyExceptionReport::DereferenceUnsafePointer(DWORD_PTR address)
{
    __try
    {
...
{
    // Log current symbol and then add another to the stack to keep the hierarchy
    // format
    PrintSymbolDetail();
