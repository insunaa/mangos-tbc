        ...
        if (m_uiStartEventTimer)
        {
            if (m_uiStartEventTimer <= uiDiff)
            {
                if (Player *pPlayer = m_creature->GetMap()->GetPlayer(m_playerGuid))
                {
                    ...
                    if (Creature *pThug = m_creature->SummonCreature(NPC_OLD_TOWN_THUG, -8672.33f, 442.88f, 99.98f,
                                                                     3.5f, TEMPSPAWN_DEAD_DESPAWN, 300000))
                        pThug->AI()->AttackStart(pPlayer);

                    if (Creature *pThug = m_creature->SummonCreature(NPC_OLD_TOWN_THUG, -8691.59f, 441.66f, 99.41f,
                                                                     6.1f, TEMPSPAWN_DEAD_DESPAWN, 300000))
                        pThug->AI()->AttackStart(pPlayer);
                }
...
#define GOSSIP_ITEM_KAT_1                                                                                              \
    "Pardon the intrusion, Lady Prestor, but Highlord Bolvar suggested that I "                                        \
    "seek your advice."
...
struct npc_squire_roweAI : public npc_escortAI, private DialogueHelper
{
    ...
    void SummonedMovementInform(Creature *summoned, uint32 motionType, uint32 pointId) override
    {
        ...
        // Summoned npc has escort and this can trigger twice if escort state is
        // not checked
    ...
    bool IsStormwindQuestActive() const
    {
        return m_isEventInProgress;
    }
...
{
    // Allow gossip if quest 6402 is completed but not yet rewarded or 6402 is
    // rewarded but 6403 isn't yet completed
         ...
         player->GetQuestStatus(QUEST_THE_GREAT_MASQUERADE) != QUEST_STATUS_COMPLETE))
    {
        ...
        {
            player->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_WINDSOR, GOSSIP_SENDER_MAIN,
...
struct npc_reginald_windsorAI : public npc_escortAI, private DialogueHelper
{
    ...
    void WaypointReached(uint32 pointId) override
    {
        ...
        case 1:
            ...
            if (Creature *jonathan = m_scriptedMap->GetSingleCreatureFromStorage(NPC_JONATHAN))
            {
                ...
                {
                    if (Creature *temp = m_creature->SummonCreature(
                            NPC_GUARD_ROYAL, aGuardLocations[i][0], aGuardLocations[i][1], aGuardLocations[i][2],
                            aGuardLocations[i][3], TEMPSPAWN_TIMED_DESPAWN, 180000))
                        m_guardsGuid[i] = temp->GetObjectGuid();
                }

                jonathan->SetWalk(false);
                jonathan->Unmount();
                jonathan->GetMotionMaster()->MovePoint(0, aMoveLocations[0][0], aMoveLocations[0][1],
                                                       aMoveLocations[0][2]);
            }
            break;
        case 2:
            ...
            break;
        case 4:
            // Periodically triggers 20275 that is used in guardAI_stormwind to
            // trigger random text and emotes
    ...
    void JustDidDialogueStep(int32 entry) override
    {
        ...
        case SAY_JON_DIALOGUE_8:
            ...
            if (Creature *guard = m_creature->GetMap()->GetCreature(m_guardsGuid[4]))
                guard->GetMotionMaster()->MovePoint(1, aMoveLocations[1][0], aMoveLocations[1][1],
                                                    aMoveLocations[1][2]);
            if (Creature *guard = m_creature->GetMap()->GetCreature(m_guardsGuid[3]))
                guard->GetMotionMaster()->MovePoint(2, aMoveLocations[2][0], aMoveLocations[2][1],
                                                    aMoveLocations[2][2]);
            break;
        case SAY_JON_DIALOGUE_9:
            ...
            if (Creature *guard = m_creature->GetMap()->GetCreature(m_guardsGuid[1]))
                guard->GetMotionMaster()->MovePoint(3, aMoveLocations[3][0], aMoveLocations[3][1],
                                                    aMoveLocations[3][2]);
            if (Creature *guard = m_creature->GetMap()->GetCreature(m_guardsGuid[0]))
                guard->GetMotionMaster()->MovePoint(4, aMoveLocations[4][0], aMoveLocations[4][1],
                                                    aMoveLocations[4][2]);
            break;
        ...
        case NPC_JONATHAN:
            ...
            {
                jonathan->SetWalk(true);
                jonathan->GetMotionMaster()->MovePoint(0, aMoveLocations[5][0], aMoveLocations[5][1],
                                                       aMoveLocations[5][2]);
            }
            break;
        ...
        case SAY_BOLVAR_KEEP_2:
            if (Creature *wrynn = m_scriptedMap->GetSingleCreatureFromStorage(NPC_WRYNN))
            {
                ...
                wrynn->GetMotionMaster()->MovePoint(0, aMoveLocations[6][0], aMoveLocations[6][1],
                                                    aMoveLocations[6][2]);

                // Store all the nearby guards, in order to transform them into
                // Onyxia guards
        ...
        case SAY_PRESTOR_KEEP_9:
            if (Creature *bolvar = m_scriptedMap->GetSingleCreatureFromStorage(NPC_BOLVAR))
            {
                ...
                bolvar->GetMotionMaster()->MovePoint(0, aMoveLocations[7][0], aMoveLocations[7][1],
                                                     aMoveLocations[7][2]);
            }
            break;
        case SAY_BOLVAR_KEEP_10:
            if (Creature *bolvar = m_scriptedMap->GetSingleCreatureFromStorage(NPC_BOLVAR))
            {
                ...
                bolvar->SetFactionTemporary(11,
                                            TEMPFACTION_RESTORE_REACH_HOME); // Change Faction so he can
                                                                             // fight Onyxia's Guards
            }
            break;
        ...
        case NPC_BOLVAR:
            ...
            {
                bolvar->SetWalk(true);
                bolvar->GetMotionMaster()->MovePoint(0, aMoveLocations[8][0], aMoveLocations[8][1],
                                                     aMoveLocations[8][2]);
            }
            break;
    ...
    bool IsKeepEventReady() const
    {
        return m_isKeepReady;
    }
...
bool GossipHello_npc_reginald_windsor(Player *player, Creature *creature)
{
    ...
    if (isEventReady && player->GetQuestStatus(QUEST_THE_GREAT_MASQUERADE) != QUEST_STATUS_COMPLETE &&
        player->GetQuestRewardStatus(QUEST_STORMWIND_RENDEZVOUS))
    {
        player->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_REGINALD, GOSSIP_SENDER_MAIN,
                                   GOSSIP_ACTION_INFO_DEF + 1);
