...
class HostileReference : public Reference<Unit, ThreatManager>
{
  public:
    ...
    float getFadeoutThreatReduction() const
    {
        return iFadeoutThreadReduction;
    }
    ...
    // has range attack in this case online = true, but accessable = false
    ...
    {
        return static_cast<HostileReference *>(Reference<Unit, ThreatManager>::next());
    }
    ...
    void SetSuppressabilityToggle()
    {
        m_suppresabilityToggle = true;
    }
    void SetHostileState(HostileState state);
    HostileState GetHostileState() const
    {
        return m_hostileState;
    }

    void SetTauntState(TauntState state)
    {
        m_tauntState = state;
    }
...
class ThreatContainer
{
  ...
  protected:
    ...
    void addReference(HostileReference *hostileReference)
    {
        iThreatList.push_back(hostileReference);
    }
...
class ThreatManager
{
  public:
    ...
    void modifyThreatPercent(Unit *victim,
                             int32 threatPercent); // -101 removes whole ref, -100 sets
                                                   // threat to 0, rest modifies it
    void modifyAllThreatPercent(int32 threatPercent);
    ...
    void setCurrentVictimByTarget(Unit *target); // Used in SPELL_EFFECT_ATTACK_ME to set the current
                                                 // target to the taunter

    void setDirty(bool dirty)
    {
        iThreatContainer.setDirty(dirty);
    }
