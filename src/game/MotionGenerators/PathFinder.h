...
class PathFinder
{
  public:
    ...
    bool calculate(float destX, float destY, float destZ, bool forceDest = false,
                   bool straightLine = false); // transfers coorddinates from global to local
                                               // space if on transport - use other func if
                                               // coords are already in transport space
  ...
  private:
    ...
    std::vector<float> m_cachedPoints; // caching for BuildPointPath - must be
                                       // after m_pointPathLimit
