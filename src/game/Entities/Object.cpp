...
Object::~Object()
{
    ...
    if (m_objectUpdated)
    {
        sLog.outError("Object::~Object (GUID: %u TypeId: %u) deleted but still "
                      "have updated status!!",
...
void Object::BuildCreateUpdateBlockForPlayer(UpdateData *data, Player *target) const
{
    ...
    // DEBUG_LOG("BuildCreateUpdate: update-type: %u, object-type: %u got
...
void Object::BuildMovementUpdate(ByteBuffer *data, uint8 updateFlags) const
{
    *data << uint8(updateFlags); // update flags
    ...
    // 0x4
    if (updateFlags & UPDATEFLAG_HAS_ATTACKING_TARGET) // packed guid (current melee attack
                                                       // target guid)
    {
        if (static_cast<Unit const *>(this)->GetVictim())
        ...
        for (uint16 index = 0; index < m_valuesCount; ++index)
        {
            if (updateMask->GetBit(index))
            {
                if (index == UNIT_NPC_FLAGS)
                {
                    ...
                    if (GetTypeId() == TYPEID_UNIT)
                    {
                        ...
                        if (appendValue & UNIT_NPC_FLAG_FLIGHTMASTER)
                        {
                            ...
                            bounds =
                                sObjectMgr.GetCreatureQuestInvolvedRelationsMapBounds(((Creature *)this)->GetEntry());
                            for (QuestRelationsMap::const_iterator itr = bounds.first; itr != bounds.second; ++itr)
                            {
                                Quest const *pQuest = sObjectMgr.GetQuestTemplate(itr->second);
                ...
                else if (index == UNIT_FIELD_HEALTH || index == UNIT_FIELD_MAXHEALTH)
                {
                    ...
                    // Fog of War: replace absolute health values with
                    // percentages for non-allied units according to settings
                ...
                else if (index == UNIT_FIELD_FLAGS)
                {
                    ...
                    {
                        // Gamemasters should be always able to select units -
                        // remove not selectable flag:
                    ...
                    // Client bug workaround: Fix for missing chat channels when
                    // resuming taxi flight on login Client does not send any
                    // chat joining attempts by itself when taxi flag is on
                ...
                else if (index == UNIT_DYNAMIC_FLAGS)
                {
                    ...
                    if (dynflagsValue & UNIT_DYNFLAG_SPECIALINFO && static_cast<const Unit *>(this)->IsAlive())
                    {
                        ...
                        Unit::AuraList const &mAuraEmpathy =
                            static_cast<const Unit *>(this)->GetAurasByType(SPELL_AURA_EMPATHY);
                        for (Unit::AuraList::const_iterator itr = mAuraEmpathy.begin();
                             !bIsCaster && itr != mAuraEmpathy.end(); ++itr)
                        {
                            bIsEmpathy = true; // Empathy by aura set
                            if ((*itr)->GetCasterGuid() == target->GetObjectGuid())
                                bIsCaster = true; // target is the caster of an
                                                  // empathy aura
                        }
                        if (bIsEmpathy && !bIsCaster) // Empathy by aura, but
                                                      // target is not the caster
                    ...
                    if (GetTypeId() == TYPEID_UNIT)
                    {
                        ...
                        if (creature->IsAlive())
                        {
                            ...
                            {
                                // as creature is in combat we have to manage tap
                                // flags
                                setTapFlags = true;
                            }
                            else
                            {
                                ...
                                // sLog.outString(">> %s is not in combat so not
                                // tapped by %s", this->GetGuidStr().c_str(),
                        ...
                        else
                        {
                            ...
                            if (creature->m_loot && creature->m_loot->CanLoot(target))
                            {
                                ...
                                dynflagsValue = dynflagsValue | UNIT_DYNFLAG_LOOTABLE;
                                // sLog.outString(">> %s is lootable for %s",
                                // this->GetGuidStr().c_str(),
                                // target->GetGuidStr().c_str());
                            }
                            else
                            {
                                // creature is dead but this player cannot loot
                                // it
                                dynflagsValue = dynflagsValue & ~UNIT_DYNFLAG_LOOTABLE;
                                // sLog.outString(">> %s is not lootable for %s",
                                // this->GetGuidStr().c_str(),
                                // target->GetGuidStr().c_str());
                            }
                        ...
                        if (setTapFlags)
                        {
                            ...
                            {
                                // creature is in combat or died and tapped by
                                // this player
                                dynflagsValue = dynflagsValue & ~UNIT_DYNFLAG_TAPPED;
                                // sLog.outString(">> %s is tapped by %s",
                                // this->GetGuidStr().c_str(),
                                // target->GetGuidStr().c_str());
                            }
                            else
                            {
                                // creature is in combat or died but not tapped
                                // by this player
                                dynflagsValue = dynflagsValue | UNIT_DYNFLAG_TAPPED;
                                // sLog.outString(">> %s is not tapped by %s",
                                // this->GetGuidStr().c_str(),
                                // target->GetGuidStr().c_str());
                            }
                        }
                    ...
                    {
                        Unit const *unit = static_cast<const Unit *>(this); // hunters mark effects should only be
                                                                            // visible to owners and not all players
                ...
                else if (index == UNIT_FIELD_FACTIONTEMPLATE)
                {
                    ...
                    // [XFACTION]: Alter faction if detected crossfaction group
                    // interaction when updating faction field:
                    if (this != target && GetTypeId() == TYPEID_PLAYER)
                    {
                        Player const *thisPlayer = static_cast<Player const *>(this);

                        if (sWorld.getConfig(CONFIG_BOOL_ALLOW_TWO_SIDE_INTERACTION_GROUP) &&
                            target->IsInGroup(thisPlayer))
                        {
                            const uint32 targetTeam = target->GetTeam();

                            if (thisPlayer->GetTeam() != targetTeam &&
                                value == Player::getFactionForRace(thisPlayer->getRace()))
                            {
                                switch (targetTeam)
                                {
                                case ALLIANCE:
        ...
        for (uint16 index = 0; index < m_valuesCount; ++index)
        {
            if (updateMask->GetBit(index))
            {
                if (index == CORPSE_FIELD_BYTES_1)
                {
                    ...
                    // [XFACTION]: Alter race field if detected crossfaction
                    // group interaction:
                    if (sWorld.getConfig(CONFIG_BOOL_ALLOW_TWO_SIDE_INTERACTION_GROUP))
                    {
...
{
    sLog.outError("Attempt %s nonexistent value field: %u (count: %u) for object "
...
{
    sLog.outError("Unexpected call of Object::RemoveFromClientUpdateList for "
                  "object (TypeId: %u Update fields: %u)",
                  GetTypeId(), m_valuesCount);
    MANGOS_ASSERT(false);
}
...
{
    sLog.outError("Unexpected call of Object::BuildUpdateData for object "
                  "(TypeId: %u Update fields: %u)",
                  GetTypeId(), m_valuesCount);
    MANGOS_ASSERT(false);
}
...
WorldObject::WorldObject()
    : m_transportInfo(nullptr), m_isOnEventNotified(false), m_visibilityData(this), m_currMap(nullptr), m_mapId(0),
      m_InstanceId(0), m_phaseMask(1), m_isActiveObject(false), m_debugFlags(0), m_transport(nullptr), m_castCounter(0)
{
}

void WorldObject::CleanupsBeforeDelete()
{
    m_events.KillAllEvents(false); // non-delatable (currently casted spells)
                                   // will not deleted now but it will deleted at
...
float WorldObject::GetAngleAt(float x, float y, const WorldObject *obj) const
{
    ...
    // fixed MANGOS_ASSERT(obj != this || PrintEntryError("GetAngle (for
    // self)"));
    if (obj == this)
    {
...
bool WorldObject::IsFacingTargetsBack(const WorldObject *target, float arc /*= M_PI_F*/) const
{
    ...
    // if target is facing the current object then we know its not possible that
    // the current object would be facing the targets back
    if (target->HasInArc(this, arc))
        return false;

    // if current object is not facing the target then we know the current object
    // is not facing the target at all
...
bool WorldObject::IsFacingTargetsFront(const WorldObject *target, float arc /*= M_PI_F*/) const
{
    ...
    // if target is not facing the current object then we know its not possible
    // that the current object would be facing the targets front
    if (!target->HasInArc(this, arc))
        return false;
                                 ...
                                 float minDist /*=0.0f*/, float const *ori /*=nullptr*/) const
{
    ...
    UpdateAllowedPositionZ(rand_x, rand_y,
...
void WorldObject::UpdateGroundPositionZ(float x, float y, float &z) const
{
    ...
    if (new_z > INVALID_HEIGHT)
        z = new_z + 0.05f; // just to be sure that we are not a few pixel under the
                           // surface
}
...
{
    WorldPacket data;
    ChatHandler::BuildChatPacket(data, CHAT_MSG_MONSTER_SAY, text, Language(language), CHAT_TAG_NONE, GetObjectGuid(),
...
void WorldObject::MonsterTextEmote(const char *text, Unit const *target, bool IsBossEmote) const
{
    ...
    SendMessageToSetInRange(
        data, sWorld.getConfig(IsBossEmote ? CONFIG_FLOAT_LISTEN_RANGE_YELL : CONFIG_FLOAT_LISTEN_RANGE_TEXTEMOTE),
        true);
}
...
{
  public:
    MonsterChatBuilder(WorldObject const &obj, ChatMsg msgtype, std::vector<std::string> content, Language language,
                       Unit const *target)
...
void _DoLocalizedTextAround(WorldObject const *source, std::vector<std::string> content, ChatMsg msgtype,
...
void WorldObject::MonsterText(std::vector<std::string> content, uint32 type, Language lang, Unit const *target) const
{
    ...
    {
    case CHAT_TYPE_SAY:
        _DoLocalizedTextAround(this, content, CHAT_MSG_MONSTER_SAY, lang, target,
                               sWorld.getConfig(CONFIG_FLOAT_LISTEN_RANGE_SAY));
        break;
    case CHAT_TYPE_YELL:
        _DoLocalizedTextAround(this, content, CHAT_MSG_MONSTER_YELL, lang, target,
                               sWorld.getConfig(CONFIG_FLOAT_LISTEN_RANGE_YELL));
        break;
    case CHAT_TYPE_TEXT_EMOTE:
        _DoLocalizedTextAround(this, content, CHAT_MSG_MONSTER_EMOTE, LANG_UNIVERSAL, target,
                               sWorld.getConfig(CONFIG_FLOAT_LISTEN_RANGE_TEXTEMOTE));
        break;
    case CHAT_TYPE_BOSS_EMOTE:
        _DoLocalizedTextAround(this, content, CHAT_MSG_RAID_BOSS_EMOTE, LANG_UNIVERSAL, target,
                               sWorld.getConfig(CONFIG_FLOAT_LISTEN_RANGE_YELL));
        break;
    ...
    case CHAT_TYPE_ZONE_YELL: {
        ...
        GetMap()->ExecuteMapWorkerZone(zoneId,
                                       std::bind(&MaNGOS::LocalizedPacketDo<MaNGOS::MonsterChatBuilder>::operator(),
    ...
    case CHAT_TYPE_PARTY: {
        ...
        if (Group *group = ((Player *)target)->GetGroup())
        {
            for (GroupReference *itr = group->GetFirstMember(); itr != nullptr; itr = itr->next())
            {
                ...
                MaNGOS::MonsterChatBuilder say_build(*this, CHAT_MSG_MONSTER_PARTY, content, LANG_UNIVERSAL,
                                                     groupMember);
                MaNGOS::LocalizedPacketDo<MaNGOS::MonsterChatBuilder> say_do(say_build);
                say_do(groupMember);
            }
...
Creature *WorldObject::SummonCreature(TempSpawnSettings settings, Map *map)
{
    ...
    creature->Summon(settings.spawnType,
                     settings.despawnTime); // Also initializes the AI and MMGen
...
// how much space should be left in front of/ behind a mob that already uses a
// space
...
struct WorldObjectChangeAccumulator
{
    ...
    WorldObjectChangeAccumulator(WorldObject &obj, UpdateDataMapType &d) : i_updateDatas(d), i_object(obj)
    {
        ...
        // with new camera system when player's camera too far from player, camera
        // wouldn't receive packets and changes from player
...
void WorldObject::SetActiveObjectState(bool active)
{
    ...
    // player's update implemented in a different from other active
    // worldobject's way it's considired to use generic way in future
    {
...
bool WorldObject::IsSpellReady(SpellEntry const &spellEntry, ItemPrototype const *itemProto /*= nullptr*/) const
{
    ...
    // overwrite category by provided category in item prototype during item cast
    // if need
    if (itemProto)
    {
    ...
    if (spellEntry.PreventionType == SPELL_PREVENTION_TYPE_SILENCE &&
        CheckLockout(GetSpellSchoolMask(&spellEntry), now))
        return false;
...
void WorldObject::LockOutSpells(SpellSchoolMask schoolMask, uint32 duration)
{
    ...
    {
        if (schoolMask & (1 << i))
            m_lockoutMap.emplace(SpellSchools(i),
                                 std::chrono::milliseconds(duration) + GetMap()->GetCurrentClockTime());
    }
}
...
void WorldObject::PrintCooldownList(ChatHandler &chat) const
{
    ...
    for (auto &lockoutItr : m_lockoutMap)
    {
        ...
        if (cdData > now)
        {
            ...
            continue;
        cdLine << "LOCKOUT for " << schoolName[lockoutItr.first] << " with " << durationStr.str()
               << " remaining time cd";
        chat.PSendSysMessage("%s", cdLine.str().c_str());
    }
...
int32 WorldObject::CalculateSpellEffectValue(Unit const *target, SpellEntry const *spellProto,
                                             SpellEffectIndex effect_index, int32 const *effBasePoints, bool maximum,
                                             bool finalUse) const
{
    ...
    float basePoints = effBasePoints ? *effBasePoints - baseDice : spellProto->EffectBasePoints[effect_index];
    ...
    {
        // TODO: Drastically beter than before, but still needs some additional
        // aura scaling research
        bool damage = false;
        if (uint32 aura = spellProto->EffectApplyAuraName[effect_index])
        {
            ...
            case SPELL_AURA_SCHOOL_ABSORB:
                //   SPELL_AURA_PERIODIC_DAMAGE_PERCENT: excluded, abs values
                //   only
        ...
        else if (uint32 effect = spellProto->Effect[effect_index])
        {
            ...
            case SPELL_EFFECT_WEAPON_DAMAGE_NOSCHOOL:
                //   SPELL_EFFECT_WEAPON_PERCENT_DAMAGE: excluded, abs values
                //   only
        ...
        if (damage)
        {
            GtNPCManaCostScalerEntry const *spellScaler =
                sGtNPCManaCostScalerStore.LookupEntry(spellProto->spellLevel - 1);
            GtNPCManaCostScalerEntry const *casterScaler =
                sGtNPCManaCostScalerStore.LookupEntry(unitCaster->GetLevel() - 1);
