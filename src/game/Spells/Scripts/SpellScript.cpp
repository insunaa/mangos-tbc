...
void SpellScriptMgr::LoadScripts()
{
    ...
    if (result)
    {
        do
        {
            ...
            {
                sLog.outErrorDb("Spell %u has script %s in spell_scripts but spell "
                                "does not exist. Skipping.",
                                spellId, scriptName.data());
                continue;
            }
            ...
            if (!found)
            {
                sLog.outErrorDb("Spell %u has script %s in spell_scripts but script "
                                "does not exist. Skipping.",
                                spellId, scriptName.data());
                continue;
            }
