...
bool readCamera(M2Camera const *cam, uint32 buffSize, M2Header const *header, CinematicCameraEntry const *dbcentry)
{
    ...
    for (uint32 i = 0; i < cam->positions.timestamps.number; ++i)
    {
        ...
        if (targetcam.size() > 0)
        {
            ...
            // Now, the timestamps for target cam and position can be different.
            // So, if they differ we interpolate
...
void LoadM2Cameras(std::string const &dataPath)
{
    ...
    for (uint32 i = 0; i < sCinematicCameraStore.GetNumRows(); ++i)
    {
        if (CinematicCameraEntry const *dbcentry = sCinematicCameraStore.LookupEntry(i))
        {
            ...
            {
                sLog.outError("Camera file %s is damaged. File is smaller than "
                              "header size",
                              filename.c_str());
                m2file.close();
                continue;
            ...
            {
                sLog.outError("Camera file %s is damaged. Camera references position "
                              "beyond file end (header)",
                              filename.c_str());
                continue;
            }
