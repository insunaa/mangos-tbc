...
SDName: boss_alar
SD%Complete: 99
SDComment: TODO: Make Alar Root and Hover during P1 platforms and dynamically
enable flying and hover and use spell 31514 for p2 transition SDCategory:
Tempest Keep, The Eye EndScriptData */

/* Pre-nerf Changes
Movement: Has been changed to have a random chance to either DoFlameQuills() or
to move to a random platform. There is even a small chance he will stay at the
same platform for 30 seconds. Embers: Do not do % hp damage to Al'ar when
killed.

Patches
2.1.0 - Al'ar movement was updated to the current state (platform 1, 2, 3, 4),
and embers were given the %hp burn element.
*/
...
enum
{
    ...
    SPELL_BERSERK = 26662,          // this spell is used only during phase II, increases
                                    // Attack Speed by 150% & Damage by 500%

    // NPC_EMBER_OF_ALAR       = 19551,        // scripted in Acid - summoned by
    // spell 39110 / 41824
    ...
    SPELL_ASHTONGUE_RUSE = 42090, // Player can complete 10946 quest, only if has aura 42090. If
                                  // kill Alar without this aura - quest not completed.
};
...
struct boss_alarAI : public CombatAI
{
    ...
    void Aggro(Unit * /*who*/) override
    {
        ...
        // The boss will always move to the first platform from the left side; also
        // set the movement to idle to stop the DB movement
    ...
    void DoFlameQuills()
    {
        // Move to Flame Quills position; stop range check, platform moving and
        // ember summoning
...
#else
            m_creature->CastSpell(nullptr, SPELL_SUMMON_PHOENIX_ADDS,
                                  TRIGGERED_OLD_TRIGGERED); // post 2.1
#endif
        }
    ...
    void MovementInform(uint32 motionType, uint32 pointId) override
    {
        ...
        case POINT_ID_PLATFORM:
            // When we reach the platform we start the range check and we can
            // summon the embers
            SetCombatScriptStatus(false);
            SetCombatMovement(false);
    ...
    void StartPhaseTransition()
    {
        ...
        // We set the health to 1 in order to avoid the forced death stand flag -
        // this way we can have the ressurrect animation
        ...
        if (DoCastSpellIfCan(nullptr, SPELL_EMBER_BLAST) == CAST_OK)
        {
            ...
            m_creature->GetMotionMaster()->MovePoint(POINT_ID_RESSURRECT, aCenterLocation[1].m_fX,
                                                     aCenterLocation[1].m_fY, aCenterLocation[1].m_fZ,
                                                     FORCED_MOVEMENT_RUN);
        }
    }
    ...
    void HandleDiveBomb()
    {
        ...
        case 0: {
            if (Unit *target =
                    m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, uint32(0), SELECT_FLAG_PLAYER))
            {
    ...
    void ExecuteAction(uint32 action)
    {
        ...
        case ALAR_DIVE_BOMB: {
            ...
            m_creature->SetHover(true);
            m_creature->SetLevitate(true);
            m_creature->GetMotionMaster()->MovePoint(POINT_ID_DIVE_BOMB, aCenterLocation[2].m_fX,
                                                     aCenterLocation[2].m_fY, aCenterLocation[2].m_fZ);
            SetDeathPrevention(true);
