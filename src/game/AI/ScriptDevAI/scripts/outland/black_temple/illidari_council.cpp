...
SD%Complete: 90
SDComment: Lady Malandes Reflective Shield should make her immune to physical
interrupts SDCategory: Black Temple EndScriptData */
...
struct boss_illidari_councilAI : public CombatAI
{
    ...
    {
        m_creature->GetCombatManager().SetLeashingCheck([&](Unit *, float x, float y, float z) { return x < 620.0f; });
    }
    ...
    void HealedBy(Unit * /*healer*/, uint32 &healedAmount) override
    {
        ...
        m_creature->CastCustomSpell(nullptr, SPELL_SHARED_RULE_HEAL, &healTaken, nullptr, nullptr,
                                    TRIGGERED_OLD_TRIGGERED);
    }
};
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case MALANDE_ACTION_EMPOWERED_SMITE: {
            if (DoCastSpellIfCan(m_creature->GetVictim(), SPELL_EMPOWERED_SMITE) == CAST_OK)
                ResetCombatAction(action, GetCurrentRangedMode()
                                              ? urand(2000, 3000)
                                              : urand(2000,
                                                      12000)); // ranged action with chance in spell list
            return;
        }
        }
    }
...
struct VerasDeadlyPoison : public AuraScript
{
    ...
    {
        data.target = aura->GetTarget()->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_DEADLY_POISON,
                                                               SELECT_FLAG_PLAYER);
    }
};
