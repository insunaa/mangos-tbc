...
static uint32 const causesShadowEmbraceSpellList[23] = {172,   6222,  6223,  7648,  11671, 11672, 25311, 27216,
                                                        18265, 18879, 18880, 18881, 27264, 30911, 27243, 980,
                                                        1014,  6217,  11711, 11712, 11713, 27218};
...
void RemoveShadowEmbraceIfNecessary(Aura *aura)
{
    ...
    {
        if ((*itr)->GetCasterGuid() == aura->GetCasterGuid() &&
            std::find(std::begin(causesShadowEmbraceSpellList), std::end(causesShadowEmbraceSpellList),
                      (*itr)->GetId()) != std::end(causesShadowEmbraceSpellList))
        {
    ...
    // if this is the final Shadow Embrace causing DoT being removed, also remove
    // Shadow Embrace if found
    if (!hasOtherShadowEmbraceCausingAuras)
    {
        ...
        {
            if ((*itr)->GetCasterGuid() == aura->GetCasterGuid() &&
                std::find(std::begin(shadowEmbraceSpellList), std::end(shadowEmbraceSpellList), (*itr)->GetId()) !=
                    std::end(shadowEmbraceSpellList))
            {
...
struct CurseOfAgony : public AuraScript
{
    void OnPeriodicCalculateAmount(Aura *aura, uint32 &amount) const override
    {
        ...
        // 9..12 ticks, 3/2 from normal tick damage
            ...
            amount += (amount + 1) / 2; // +1 prevent 0.5 damage possible lost at 1..4
                                        // ticks 5..8 ticks have normal tick damage
    }
    ...
    int32 OnAuraValueCalculate(AuraCalcData &data, int32 value) const override
    {
        ...
        if (Unit *caster = data.caster)
        {
            ...
            if (!data.aura->GetScriptValue())
                data.aura->SetScriptValue((target->HasAura(35693)   ? 12
                                           : target->HasAura(35692) ? 8
                                           : target->HasAura(35691) ? 4
                                                                    : 0));
            value =
                data.aura->GetScriptValue() * (caster->GetStat(STAT_STAMINA) + caster->GetStat(STAT_INTELLECT)) / 100;
        }
        return value;
    }
};
    ...
    SpellCastResult OnCheckCast(Spell *spell, bool strict) const override
    {
        ...
        if (target && caster)
        {
            ...
            for (auto itr : auras)
            {
                ...
                if (itr.second->GetTarget()->GetObjectGuid() != caster->GetObjectGuid() &&
                    spell->Dispel == DISPEL_MAGIC && IsPositiveSpell(spell) && !IsPassiveSpell(spell))
                    return SPELL_CAST_OK;
            }
        }
...
struct SeedOfCorruption : public AuraScript
{
    void OnApply(Aura *aura, bool apply) const override
    {
        ...
        if (aura->GetRemoveMode() == AURA_REMOVE_BY_DEATH)
            if (Unit *caster = aura->GetCaster())
                caster->CastSpell(aura->GetTarget(), SPELL_SEED_DAMAGE,
                                  TRIGGERED_IGNORE_GCD | TRIGGERED_IGNORE_CURRENT_CASTED_SPELL |
                                      TRIGGERED_HIDE_CAST_IN_COMBAT_LOG);
    }
    ...
    SpellAuraProcResult OnProc(Aura *aura, ProcExecutionData &procData) const override
    {
        ...
        if (mod->m_amount <= (int32)procData.damage)
        {
            ...
            if (Unit *caster = procData.triggeredByAura->GetCaster())
                caster->CastSpell(procData.victim, SPELL_SEED_DAMAGE,
                                  TRIGGERED_IGNORE_GCD | TRIGGERED_IGNORE_CURRENT_CASTED_SPELL |
                                      TRIGGERED_HIDE_CAST_IN_COMBAT_LOG);
...
struct SoulLeech : public AuraScript
{
    SpellAuraProcResult OnProc(Aura *aura, ProcExecutionData &procData) const override
    {
        ...
        target->CastCustomSpell(nullptr, 30294, &damage, nullptr, nullptr,
                                TRIGGERED_IGNORE_GCD | TRIGGERED_IGNORE_CURRENT_CASTED_SPELL |
                                    TRIGGERED_HIDE_CAST_IN_COMBAT_LOG);
        return SPELL_AURA_PROC_OK;
    }
