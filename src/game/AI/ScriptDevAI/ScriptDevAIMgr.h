...
template <typename T> UnitAI *GetNewAIInstance(Creature *creature)
{
    return new T(creature);
}

template <typename T> GameObjectAI *GetNewAIInstance(GameObject *gameobject)
{
    return new T(gameobject);
}

template <typename T> InstanceData *GetNewInstanceScript(Map *map)
{
    return new T(map);
}
...
struct Script
{
    Script()
        : pGossipHello(nullptr), pGossipHelloGO(nullptr), pGossipSelect(nullptr), pGossipSelectGO(nullptr),
          ...
          pEffectDummyItem(nullptr), pEffectScriptEffectNPC(nullptr), pEffectAuraDummy(nullptr),
