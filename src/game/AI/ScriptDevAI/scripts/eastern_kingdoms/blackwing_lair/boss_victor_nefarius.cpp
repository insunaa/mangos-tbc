...
struct boss_victor_nefariusAI : public CombatAI, private DialogueHelper
{
          ...
          m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData()))
    {
        if (m_creature->GetMapId() != MAP_ID_BWL)
            return;
        // Select the 2 different drakonids that we are going to use for all
        // instance lifetime
    ...
    void JustDidDialogueStep(int32 entry) override
    {
        ...
        if (entry == SPELL_SHADOWBLINK)
        {
            ...
            // Spawn the two spawner NPCs that will handle the drakonids spawning
            // (both brood and chromatic)
            if (m_instance)
            {
                // Left
                m_creature->SummonCreature(m_instance->GetData(TYPE_NEFA_LTUNNEL), aNefarianLocs[0].m_fX,
                                           aNefarianLocs[0].m_fY, aNefarianLocs[0].m_fZ, 5.000f,
                                           TEMPSPAWN_TIMED_OOC_OR_CORPSE_DESPAWN, HOUR * IN_MILLISECONDS);
                // Right
                m_creature->SummonCreature(m_instance->GetData(TYPE_NEFA_RTUNNEL), aNefarianLocs[1].m_fX,
                                           aNefarianLocs[1].m_fY, aNefarianLocs[1].m_fZ, 5.000f,
                                           TEMPSPAWN_TIMED_OOC_OR_CORPSE_DESPAWN, HOUR * IN_MILLISECONDS);
            }
        }
    }
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case NEFARIUS_FEAR: {
            if (Unit *target =
                    m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 1, SPELL_FEAR, SELECT_FLAG_PLAYER))
        ...
        case NEFARIUS_SHADOW_COMMAND: {
            if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 1, SPELL_SHADOW_COMMAND,
                                                                 SELECT_FLAG_PLAYER))
...
bool GossipHello_boss_victor_nefarius(Player *player, Creature *creature)
{
    ...
    player->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_NEFARIUS_1, GOSSIP_SENDER_MAIN,
