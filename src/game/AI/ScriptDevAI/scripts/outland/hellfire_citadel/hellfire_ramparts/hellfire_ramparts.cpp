...
instance_ramparts::instance_ramparts(Map *pMap) : ScriptedInstance(pMap), m_uiSentryCounter(0)
{
    Initialize();
}
    ...
    case TYPE_NAZAN:
        ...
        if (uiData == DONE && m_auiEncounter[0] == DONE)
        {
            ...
            if (GameObject *chest = GetSingleGameObjectFromStorage(
                    instance->IsRegularDifficulty() ? GO_FEL_IRON_CHEST : GO_FEL_IRON_CHEST_H))
