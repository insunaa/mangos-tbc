...
struct Modifier
{
    ...
    /**
     * Set at the beginning so that stack increases dont need to calculate spell
     * values
     */
    int32 m_baseAmount;
};
...
class SpellAuraHolder
{
  public:
    ...
    bool ModStackAmount(int32 num,
                        Unit *newCaster); // return true if last charge dropped

    Aura *GetAuraByEffectIndex(SpellEffectIndex index) const
    {
        return m_auras[index];
    }
    SpellEntry const *GetTriggeredBy() const
    {
    ...
    SpellAuraHolderState GetState() const
    {
        return m_spellAuraHolderState;
    }
    void SetState(SpellAuraHolderState state)
    {
        m_spellAuraHolderState = state;
    }
    ...
    TrackedAuraType GetTrackedAuraType() const
    {
        return m_trackedAuraType;
    }
    void SetTrackedAuraType(TrackedAuraType val)
    {
        m_trackedAuraType = val;
    }
    ...
    uint8 GetAuraLevel() const
    {
        return m_auraLevel;
    }
    void SetAuraLevel(uint8 level)
    {
        m_auraLevel = level;
    }
    uint32 GetAuraCharges() const
    {
    ...
    uint32 GetAuraApplyTime() const
    {
        return m_applyTime;
    }
    uint32 GetAuraApplyMSTime() const
    {
        return m_applyMSTime;
    } // milliseconds time
    ...
    bool IsReducedProcChancePast60()
    {
        return m_reducedProcChancePast60;
    }
    void SetReducedProcChancePast60()
    {
        m_reducedProcChancePast60 = true;
    }
    ...
    // helpers
    void PresetAuraStacks(uint32 stacks)
    {
        m_stackAmount = stacks;
    } // use only in OnHolderInit
  private:
    ...
    SpellAuraHolderState m_spellAuraHolderState; // State used to be sure init part is finished
                                                 // (ex there is still some aura to add or effect
                                                 // to process)
    ...
    float m_heartbeatResistChance;   // Chance to break this spell due to heartbeat
                                     // resistance
    ...
    TrackedAuraType m_trackedAuraType : 8; // store if the caster tracks the aura - can
                                           // change at spell steal for example
...
// speedup,
//      each setting object update field code line moved under if(Real) check
//      is significant mangos speedup, and less server->client data sends each
//      packet sending code moved under if(Real) check is _large_ mangos
...
class Aura
{
  ...
  public:
    ...
    {
        // aura not have immediate effect at add/remove and handled by ID in other
        // code place
    }
    ...
    Modifier const *GetModifier() const
    {
        return &m_modifier;
    }
    ...
    uint32 GetId() const
    {
        return GetHolder()->GetSpellProto()->Id;
    }
    ObjectGuid const &GetCastItemGuid() const
    {
    ...
    Unit *GetCaster() const
    {
        return GetHolder()->GetCaster();
    }
    Unit *GetTarget() const
    {
        return GetHolder()->GetTarget();
    }
    ...
    int32 GetAuraDuration() const
    {
        return GetHolder()->GetAuraDuration();
    }
    time_t GetAuraApplyTime() const
    {
        return m_applyTime;
    }
    uint32 GetAuraTicks() const
    {
    ...
    uint32 GetStackAmount() const
    {
        return GetHolder()->GetStackAmount();
    }
    ...
    // more limited that used in future versions (spell_affect table based only),
    // so need be careful with backporting uses
    ...
    SpellAuraHolder const *GetHolder() const
    {
        return m_spellAuraHolder;
    }
    ...
    SpellModifier *GetSpellModifier()
    {
        return m_spellmod;
    }
    ...
    bool IsMagnetUsed() const
    {
        return m_magnetUsed;
    }

    static uint32 CalculateAuraEffectValue(Unit *caster, Unit *target, SpellEntry const *spellProto,
                                           SpellEffectIndex effIdx, uint32 value);

    // Scripting system
    AuraScript *GetAuraScript() const
    {
        return GetHolder()->GetAuraScript();
    }
    // Variable storage
    ...
    uint64 GetScriptValue()
    {
        return m_scriptValue;
    }
    void SetScriptStorage(ScriptStorage *storage)
    {
        m_storage = storage;
    } // do not set more than once
    ...
    void OnAbsorb(int32 &currentAbsorb, int32 &remainingDamage, uint32 &reflectedSpellId, int32 &reflectDamage,
                  bool &preventedDeath);
