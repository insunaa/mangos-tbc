...
void SQLStorageLoaderBase<DerivedLoader, StorageClass>::Load(StorageClass &store, bool error_at_empty /*= true*/)
{
    ...
    {
        recordCount = 0;
        sLog.outError("Error in %s table, probably sql file format was updated "
                      "(there should be %d fields in sql).\n",
    ...
    do
    {
        ...
        for (uint32 x = 0, y = 0; x < store.GetDstFieldCount();)
        {
            ...
            // It is required that the input has at least as many columns set as
            // the output requires
            ...
            case FT_STRING:
                ...
                break;
            case FT_64BITINT:
                storeValue((uint64)fields[y].GetUInt64(), store, record, x, offset);
                ++x;
                break;
            case FT_NA:
            case FT_NA_BYTE:
