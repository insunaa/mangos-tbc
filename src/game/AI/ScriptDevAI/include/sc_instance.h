...
#define OUT_SAVE_INST_DATA_COMPLETE                                                                                    \
    debug_log("SD2: Saving Instance Data for Instance %s (Map %d, Instance Id "                                        \
              "%d) completed.",                                                                                        \
              instance->GetMapName(), instance->GetId(), instance->GetInstanceId())
#define OUT_LOAD_INST_DATA(a)                                                                                          \
    debug_log("SD2: Loading Instance Data for Instance %s (Map %d, Instance Id "                                       \
              "%d). Input is '%s'",                                                                                    \
              instance->GetMapName(), instance->GetId(), instance->GetInstanceId(), a)
#define OUT_LOAD_INST_DATA_COMPLETE                                                                                    \
    debug_log("SD2: Instance Data Load for Instance %s (Map %d, Instance Id: "                                         \
              "%d) is complete.",                                                                                      \
              instance->GetMapName(), instance->GetId(), instance->GetInstanceId())
#define OUT_LOAD_INST_DATA_FAIL                                                                                        \
    script_error_log("Unable to load Instance Data for Instance %s (Map %d, "                                          \
                     "Instance Id: %d).",                                                                              \
                     instance->GetMapName(), instance->GetId(), instance->GetInstanceId())
...
class ScriptedInstance : public InstanceData
{
  public:
    ...
    // Wrapper for simulating map-wide text in this instance. It is expected that
    // the Creature is stored in m_npcEntryGuidStore if loaded.
    void DoOrSimulateScriptTextForThisInstance(int32 textEntry, uint32 creatureEntry)
    {
        ...
        DoOrSimulateScriptTextForMap(textEntry, creatureEntry, instance,
                                     GetSingleCreatureFromStorage(creatureEntry, true));
    }
  ...
  protected:
    ...
    void RespawnDbGuids(std::vector<uint32> &spawns,
                        uint32 respawnDelay); // respawns all dbguid creatures
...
class DialogueHelper
{
  public:
    ...
    void SetDialogueSide(bool isFirstSide)
    {
        m_isFirstSide = isFirstSide;
    }
  ...
  protected:
    ...
    /// instances
    virtual Creature *GetSpeakerByEntry(uint32 /*entry*/)
    {
        return nullptr;
    }
