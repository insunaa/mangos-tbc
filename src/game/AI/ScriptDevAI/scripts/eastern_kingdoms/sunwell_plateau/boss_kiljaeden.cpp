...
enum
{
    ...
    // SPELL_RING_BLUE_FLAME       = 45825,        // cast by the orb targets
    // when activated
    ...
    // NPC_BLUE_ORB_TARGET         = 25640,        // dummy npc near gameobjects
    // 187869, 188114, 188115, 188116
    ...
    SPELL_PORTAL_FROM_SHATTRATH = 46801, // Riftwalker after reached point 1, summon NPC_RIFTWALKER and
                                         // cast on him this spell.
...
struct npc_kiljaeden_controllerAI : public ScriptedAI, private DialogueHelper
{
    npc_kiljaeden_controllerAI(Creature *creature)
        : ScriptedAI(creature), DialogueHelper(aOutroDialogue),
        ...
        case POINT_START_LEFT_MOVEMENT: {
            ...
            if (Creature *leader = m_creature->GetMap()->GetCreature(GUID_PREFIX + 370))
            {
                ...
                leader->GetMotionMaster()->MovePoint(1,
                                                     Position(1718.26f, 608.50726f, 28.050209f, 1.431169986724853515f));
            }
            break;
        }
        ...
        case POINT_START_RIGHT_MOVEMENT: {
            ...
            if (Creature *leader = m_creature->GetMap()->GetCreature(GUID_PREFIX + 380))
            {
                ...
                leader->GetMotionMaster()->MovePoint(
                    1, Position(1680.1309f, 649.1477f, 28.050209f, 0.331612557172775268f));
            }
            break;
        }
    ...
    void SummonedMovementInform(Creature *summoned, uint32 motionType, uint32 pointId) override
    {
        ...
        if (pointId == 1)
        {
            ...
            {
                // Cast teleport and despawn Velen, the portal and Kalec; Liadrin
                // will despawn on timer
...
struct boss_kiljaedenAI : public CombatAI, private DialogueHelper
{
    boss_kiljaedenAI(Creature *creature)
        : CombatAI(creature, KILJAEDEN_ACTION_MAX), DialogueHelper(aPhaseDialogue),
          m_instance(static_cast<instance_sunwell_plateau *>(creature->GetInstanceData())), m_uiMaxShieldOrbs(3),
          m_freeShieldOrbs(5)
    {
        InitializeDialogueHelper(m_instance);
    ...
    std::vector<uint32> m_randomOrbs = {GO_ORB_BLUE_FLIGHT_1, GO_ORB_BLUE_FLIGHT_2, GO_ORB_BLUE_FLIGHT_3,
                                        GO_ORB_BLUE_FLIGHT_4};
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case KILJAEDEN_SHIELD_ORB:
            ...
            while (m_uiShieldOrbCount < m_uiMaxShieldOrbs)
            {
                ...
                m_creature->SummonCreature(NPC_SHIELD_ORB, positions[i].x, positions[i].y, positions[i].z,
                                           positions[i].o, TEMPSPAWN_CORPSE_DESPAWN, 0);
                ++m_uiShieldOrbCount;
...
struct npc_sinister_reflectionAI : public CombatAI
{
    ...
    void ReceiveAIEvent(AIEventType eventType, Unit * /*sender*/, Unit *invoker, uint32 /*miscValue*/) override
    {
        if (eventType == AI_EVENT_CUSTOM_A)
        {
            ...
            if (m_class == CLASS_WARRIOR || m_class == CLASS_PALADIN || m_class == CLASS_HUNTER ||
                m_class == CLASS_ROGUE || m_class == CLASS_SHAMAN)
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case SINISTER_MOONFIRE: // random
            if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_MOONFIRE,
        ...
        case SINISTER_CURSE_OF_AGONY: // random
            if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_CURSE_OF_AGONY,
                                                                 SELECT_FLAG_PLAYER | SELECT_FLAG_NOT_AURA))
...
{
    npc_core_of_entropiusAI(Creature *creature)
        : CombatAI(creature, 0), m_instance(static_cast<instance_sunwell_plateau *>(creature->GetInstanceData()))
    {
    }
    ...
    void MovementInform(uint32 motionType, uint32 pointId) override
    {
        if (motionType == PATH_MOTION_TYPE && pointId == 2)
        {
            ...
            {
                // When the purified Muru reaches the ground the sunwell ignites
                // and Muru despawns
...
struct DarknessOfSouls : public AuraScript
{
    void OnApply(Aura *aura, bool apply) const override
    {
        if (aura->GetEffIndex() == EFFECT_INDEX_0 && !apply && aura->GetRemoveMode() == AURA_REMOVE_BY_EXPIRE)
        {
            ...
            {
            case 0:
                DoScriptText(SAY_DARKNESS_1, target);
                break;
            case 1:
                DoScriptText(SAY_DARKNESS_2, target);
                break;
            case 2:
                DoScriptText(SAY_DARKNESS_3, target);
                break;
            }
        }
    }
...
struct SinisterReflectionClone : public SpellScript, public AuraScript
{
    ...
    void OnEffectExecute(Spell *spell, SpellEffectIndex effIdx) const override
    {
        ...
        target->CastSpell(spell->GetCaster(), spell->m_spellInfo->CalculateSimpleValue(effIdx),
    ...
    void OnPeriodicTrigger(Aura *aura, PeriodicTriggerData &data) const override
    {
        if (aura->GetTarget()->IsCreature())
        {
            ...
            if (urand(0, lastPhase ? 1 : 2) == 0)
            {
                ...
                data.target = aura->GetTarget()->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, data.spellInfo,
                                                                       SELECT_FLAG_PLAYER);
            }
            else
