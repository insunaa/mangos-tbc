...
struct PetSpell
{
    uint8 active; // use instead enum (not good use *uint8* limited enum in case
                  // when value in enum not positive in *int8*)
...
class Pet : public Creature
{
  public:
    ...
    {
        return getPetType() == GUARDIAN_PET;
    }

    bool Create(uint32 guidlow, CreatureCreatePos &cPos, CreatureInfo const *cinfo, uint32 pet_number);
    bool CreateBaseAtCreature(Creature *creature);
    bool LoadPetFromDB(Player *owner, Position const &spawnPos, uint32 petentry = 0, uint32 petnumber = 0,
    ...
    void SetDeathState(DeathState s) override; // overwrite virtual Creature::SetDeathState and
                                               // Unit::SetDeathState
    ...
    {
        return GetByteValue(UNIT_FIELD_BYTES_1, UNIT_BYTES_1_OFFSET_PET_LOYALTY);
    }
    ...
    // return charminfo ai only when this pet is possessed. (eye of the beast
    // case for ex.)
