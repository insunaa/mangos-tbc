...
struct npc_lazy_peonAI : public ScriptedAI
{
    ...
    uint32 m_uiKneelTimer; // Time, delay kneeling to allow sound to finish
                           // playing
    uint32 m_uiGetUpTimer; // Time, npc stays at the pile so kneel animation can
                           // finish
    bool m_bIsAtPile;
    float m_fSpawnO;
    ...
    {
        m_uiWakeTimer = urand(100000, 120000); // Random wakeup timer 1m40s to 2m, retail times
                                               // unknown, 2m is duration of sleep aura
    ...
    void GoLumberPile()
    {
        if (GameObject *pLumber = GetClosestGameObjectWithEntry(m_creature, GO_LUMBERPILE, 15.0f))
        {
            ...
            script_error_log("No GameObject of entry %u was found in range or "
