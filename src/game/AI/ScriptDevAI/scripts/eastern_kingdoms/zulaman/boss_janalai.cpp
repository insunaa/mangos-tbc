...
enum
{
    ...
    // NPC_WORLD_TRIGGER_NOT_IMMUNE_PC = 21252,                // used for fire
    // wall
...
struct boss_janalaiAI : public CombatAI
{
          ...
          m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData()))
    {
        ...
        m_creature->GetCombatManager().SetLeashingCheck(
            [](Unit *, float x, float y, float z) { return x > -8.f || x < -57.f; });
...
struct npc_amanishi_hatcherAI : public ScriptedAI
{
    ...
    {
        if ((spell->Id != SPELL_HATCH_EGG_1 && spell->Id != SPELL_HATCH_EGG_2) ||
            target->GetEntry() != NPC_DRAGONHAWK_EGG)
            return;
    ...
    void ReceiveAIEvent(AIEventType eventType, Unit * /*sender*/, Unit *invoker, uint32 miscValue) override
    {
        if (eventType == AI_EVENT_CUSTOM_A)
        {
            ...
            if (Unit *target = static_cast<Creature *>(invoker)->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0,
                                                                                       nullptr, SELECT_FLAG_PLAYER))
                AttackStart(target);
...
struct HatchEggs : public SpellScript
{
    ...
    {
        if (npc_amanishi_hatcherAI *ai =
                dynamic_cast<npc_amanishi_hatcherAI *>(static_cast<Unit *>(spell->GetCaster())->AI()))
