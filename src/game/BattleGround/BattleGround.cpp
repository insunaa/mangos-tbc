  ...
  public:
    ...
    void operator()(WorldPacket &data, int32 loc_idx)
    {
        ...
        if (i_args)
        {
            ...
            ChatHandler::BuildChatPacket(data, i_msgtype, text, LANG_UNIVERSAL, CHAT_TAG_NONE, sourceGuid,
                                         sourceName.c_str(), sourceGuid, sourceName.c_str());
    }
...
void BattleGround::Update(uint32 diff)
{
    ...
    if (!m_offlineQueue.empty())
    {
        ...
        if (itr != m_players.end())
        {
            if (itr->second.offlineRemoveTime <= sWorld.GetGameTime())
            {
                ...
                CharacterDatabase.PExecute("UPDATE characters SET at_login = at_login "
                                           "| '%u' WHERE guid = '%u'",
                                           uint32(AT_LOGIN_ADD_BG_DESERTER), itr->first.GetCounter());

                RemovePlayerAtLeave(itr->first, true,
                                    true);  // remove player from BG
            ...
            if (sWorld.getConfig(CONFIG_BOOL_BATTLEGROUND_QUEUE_ANNOUNCER_START))
            {
                ...
                if (!IsArena())
                    sWorld.SendWorldText(LANG_BG_STARTED_ANNOUNCE_WORLD, GetName(),
                                         Player::GetMinLevelForBattleGroundBracketId(bracketId, BgTypeId),
                                         Player::GetMaxLevelForBattleGroundBracketId(bracketId, BgTypeId));
                else
    ...
    {
        // after 45 minutes without one team losing, the arena closes with no
        // winner and -16 rating change for both
...
/**
  Method that rewards reputation to team - called at the end of the
  battleground
...
void BattleGround::EndBattleGround(Team winner)
{
    ...
    if (IsBattleGround() && sWorld.getConfig(CONFIG_BOOL_BATTLEGROUND_SCORE_STATISTICS))
    {
        ...
        SqlStatement stmt = CharacterDatabase.CreateStatement(
            insPvPstatsBattleground, "INSERT INTO pvpstats_battlegrounds (id, winner_team, bracket_id, "
                                     "type, date) VALUES (?, ?, ?, ?, NOW())");
    ...
    if (IsArena() && IsRated())
    {
        if (winner != TEAM_NONE)
        {
            ...
            if (winner_arena_team && loser_arena_team)
            {
                ...
                DEBUG_LOG("--- Winner rating: %u, Loser rating: %u, Winner "
                          "change: %i, "
                          "Loser change: %i ---",
                          winner_rating, loser_rating, winner_change, loser_change);
    ...
    for (auto &m_Player : m_players)
    {
        ...
        if (IsBattleGround() && sWorld.getConfig(CONFIG_BOOL_BATTLEGROUND_SCORE_STATISTICS))
        {
            ...
            SqlStatement stmt = CharacterDatabase.CreateStatement(
                insPvPstatsPlayer, "INSERT INTO pvpstats_players (battleground_id, character_guid, "
...
void BattleGround::SendRewardMarkByMail(Player *player, uint32 mark, uint32 count) const
{
    ...
    if (Item *markItem = Item::CreateItem(mark, count, player))
    {
        ...
        markItem->SaveToDB(); // save for prevent lost at next mail load, if
...
/**
  Method that rewards player quest complete - called at the end of the
  battleground
...
{
    // TODO: This originally is meant to be applied/removed by the dummy aura
    // casted on battleground end NOTE: control will be automatically reset by
    // client when the player changes the map, so not needed to restore in
    // RemovePlayerAtLeave()
    player->UpdateClientControl(player, false);
    // Set the flag to indicate that server does not want player to have client
    // control
...
void BattleGround::RemovePlayerAtLeave(ObjectGuid playerGuid, bool isOnTransport, bool doSendPacket)
{
                     ...
                     // rating, update queue
    {
        ...
        if (player)
        {
            ...
            if (IsArena())
            {
                player->RemoveArenaAuras(true); // removes debuffs / dots etc., we don't want the
                                                // player to die after porting out
                bgTypeId = BATTLEGROUND_AA;     // set the bg type to all arenas (it
                                                // will be used for queue refreshing)

                // unsummon current and summon old pet if there was one and there
                // isn't a current pet
                ...
                {
                    // left a rated match while the encounter was in progress,
                    // consider as loser
                    ArenaTeam *winner_arena_team =
                        sObjectMgr.GetArenaTeamById(GetArenaTeamIdForTeam(GetOtherTeam(team)));
            ...
            {
                WorldPacket data;
                sBattleGroundMgr.BuildBattleGroundStatusPacket(data, this,
                                                               player->GetBattleGroundQueueIndex(bgQueueTypeId),
                                                               STATUS_NONE, 0, 0, ARENA_TYPE_NONE, TEAM_NONE);
                player->GetSession()->SendPacket(data);
            }

            // this call is important, because player, when joins to
            // battleground, this method is not called, so it must be called when
            // leaving bg
        ...
        if (IsBattleGround() && GetStatus() < STATUS_WAIT_LEAVE)
        {
            // a player has left the battleground, so there are free slots -> add
            // to queue
    ...
    if (player)
    {
        ...
        player->SetBattleGroundId(0,
                                  BATTLEGROUND_TYPE_NONE); // We're not in BG.
...
void BattleGround::Reset()
{
    ...
    if (m_invitedAlliance > 0 || m_invitedHorde > 0)
        sLog.outError("BattleGround system: bad counter, m_InvitedAlliance: %d, "
                      "m_InvitedHorde: %d",
...
void BattleGround::StartBattleGround()
{
    ...
    // players when first BG::Update() method is executed and it doesn't matter
    // if we call StartBattleGround() more times, because m_BattleGrounds is a
    // map and instance id never changes
    ...
    else
    {
        if (GetStatus() == STATUS_WAIT_JOIN) // not started yet
            player->CastSpell(player, SPELL_PREPARATION,
                              TRIGGERED_OLD_TRIGGERED); // reduces all mana cost of spells.
    }
...
void BattleGround::EventPlayerLoggedOut(Player *player)
{
    ...
    if (GetStatus() == STATUS_IN_PROGRESS)
    {
        ...
        if (IsArena())
            if (GetAlivePlayersCountByTeam(player->GetTeam()) <= 1 &&
                GetPlayersCountByTeam(GetOtherTeam(player->GetTeam())))
                EndBattleGround(GetOtherTeam(player->GetTeam()));
    }
}

/**
  Function that returns the number of players that can join a battleground
  based on the provided team
*/
...
/**
  Method that removes this battleground from free queue - it must be called
...
void BattleGround::OpenDoorEvent(uint8 event1, uint8 event2 /*=0*/)
{
    ...
    {
        sLog.outError("BattleGround:OpenDoorEvent this event isn't active "
                      "event1:%u event2:%u",
                      event1, event2);
        return;
    }
...
/**
  Method that handles player added to battleground when battleground is in
  progress
...
/**
  Function that returns the gameobject pointer that was stored in
...
GameObject *BattleGround::GetSingleGameObjectFromStorage(uint32 entry) const
{
    ...
    sLog.outError("BattleGround requested gameobject with entry %u, but no "
                  "gameobject of this entry was created yet, or it was not "
                  "stored by battleground script for map %u.",
...
/**
  Function that returns pointer to a loaded Creature that was stored in
  m_goEntryGuidStore. Can return nullptr
...
Creature *BattleGround::GetSingleCreatureFromStorage(uint32 entry, bool skipDebugLog /*=false*/) const
{
    ...
    // Output log, possible reason is not added GO to map, or not yet loaded;
    if (!skipDebugLog)
        script_error_log("BattleGround requested creature with entry %u, but no npc of this "
