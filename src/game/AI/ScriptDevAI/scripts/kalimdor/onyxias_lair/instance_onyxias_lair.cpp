...
SD%Complete: 80%
SDComment: Quel'Serrar forging event is missing. Whelp summoning would be
improved by GO casting. SDCategory: Onyxia's Lair EndScriptData */
...
void instance_onyxias_lair::OnCreatureDeath(Creature *pCreature)
{
    switch (pCreature->GetEntry())
    {
    // Keep track of all killed Warders to respawn them while Onyxia is in
    // combat
...
void instance_onyxias_lair::OnObjectCreate(GameObject *pGo)
{
    ...
    case GO_WHELP_SPAWNER:
        ...
        {
            pTrigger->SummonCreature(NPC_ONYXIA_WHELP, pGo->GetPositionX(), pGo->GetPositionY(), pGo->GetPositionZ(),
                                     0.0f, TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, MINUTE * IN_MILLISECONDS);
            // TODO when GO spell casting is added in core, replace above code by
            // (or something similar): pGO->CastSpell(pGO, SPELL_SUMMONWHELP,
            // TRIGGERED_NONE);
        }
        break;
    }
