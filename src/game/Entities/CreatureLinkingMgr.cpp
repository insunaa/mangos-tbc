 ...
 * DB Format:   entry, map, master_entry, flag, search_radius   for
 * `creature_linking_template` (by entry) 0      1    2             3     4 DB
 * Format:   guid, master_guid, flag                         for
 * `creature_linking` (by guid) 0     1            2
 * **************************************
 * entry/guid:                  creature_template.entry/guid
 * map:                         Map on which the NPC has to be
 * master_entry/master_guid     creature_template.entry of the npc, that shall
 * trigger the actions flag:                        flag value, of type
 * CreatureLinkingFlags search_radius:               radius, in which master
 * and slave must be spawned so that they are linked together
 *
 * ***************************************************** */
...
void CreatureLinkingMgr::LoadFromDB()
{
    ...
    uint32 count = 0;
    QueryResult *result = WorldDatabase.Query("SELECT entry, map, master_entry, flag, search_range "
                                              "FROM creature_linking_template");
    if (!result)
    {
        BarGoLink bar(1);
        bar.step();
    ...
    else
    {
        ...
        do
        {
            ...
            tmp.masterDBGuid = 0; // Will be initialized for unique mobs in
                                  // IsLinkingEntryValid (only for spawning dependend)
 ...
 *
 *  @param byEntry: is the first parameter of the function a npc entry or a npc
 * guid?
 *  @param slaveEntry: dependend on byEntry param this is either the slave's
 * npc-entry or the npc-guid
 *  @param pTmp: Information about the CreatureLinking of the npc. Note that
 * this information may be changed in some cases
 *
 *  In case of checking by entry and in case of linked spawning and searchRange
 * == 0, pTmp will be changed to keep information about the (unique!) master's
 * db-guid
 */
bool CreatureLinkingMgr::IsLinkingEntryValid(uint32 slaveEntry, CreatureLinkingInfo *pTmp, bool byEntry)
{
    ...
    if (byEntry) // Entry given
    {
        ...
        if (!pInfo)
        {
            sLog.outErrorDb("`creature_linking_template` has a non existing "
                            "slave_entry (slave: %u, master %u), skipped.",
        ...
        {
            sLog.outErrorDb("`creature_linking_template` has a non existing "
                            "master_entry (slave: %u, master %u), skipped",
                            slaveEntry, pTmp->masterId);
            return false;
        }
        if (pTmp->mapId && !sMapStore.LookupEntry(pTmp->mapId))
        {
            sLog.outErrorDb("`creature_linking_template` has a non existing map %u "
                            "(slave %u, master %u), skipped",
    ...
    else // guid given
    {
        ...
        if (!slaveData)
        {
            sLog.outErrorDb("`creature_linking` has a non existing slave (guid: %u, "
                            "master_guid %u), skipped",
                            slaveEntry, pTmp->masterId);
            return false;
        }
        if (!masterData)
        {
            sLog.outErrorDb("`creature_linking` has a non existing master (guid: "
                            "%u,, master_guid: %u), skipped",
                            slaveEntry, pTmp->masterId);
            return false;
        }
        if (slaveData->mapid != masterData->mapid)
        {
            sLog.outErrorDb("`creature_linking` has a slave and master on different "
                            "maps (guid: %u, master_guid: %u), skipped",
                            slaveEntry, pTmp->masterId);
            return false;
        }
    }
    ...
    {
        sLog.outErrorDb("`creature_linking%s` has invalid flag, (entry: %u, map: "
                        "%u, flags: %u), skipped",
    ...
    {
        sLog.outErrorDb("`creature_linking%s` has pointless "
                        "FLAG_DESPAWN_ON_RESPAWN for self, "
                        "(entry: %u, map: %u), skipped",
    ...
    if (byEntry)
    {
        // Check for uniqueness of mob whom is followed, on whom spawning is
        // dependend
        if (pTmp->searchRange == 0 &&
            pTmp->linkingFlag & (FLAG_FOLLOW | FLAG_CANT_SPAWN_IF_BOSS_DEAD | FLAG_CANT_SPAWN_IF_BOSS_ALIVE))
        {
            QueryResult *result = WorldDatabase.PQuery("SELECT guid FROM creature WHERE id=%u AND map=%u LIMIT 2",
                                                       pTmp->masterId, pTmp->mapId);
            if (!result)
            {
                sLog.outErrorDb("`creature_linking_template` has FLAG_FOLLOW, but no "
                                "master, (entry: %u, map: %u, master: %u)",
            ...
            {
                sLog.outErrorDb("`creature_linking_template` has FLAG_FOLLOW, but non "
                                "unique master, (entry: %u, map: %u, master: %u)",
                                slaveEntry, pTmp->mapId, pTmp->masterId);
                delete result;
                return false;
            }
...
bool CreatureLinkingMgr::IsLinkedEventTrigger(Creature *pCreature) const
{
    ...
    // Also return true for npcs that trigger reverse actions, or for
    // followers(needed in respawn)
...
// return true only for masters stored by entry - this prevents adding them to
// master-holder maps
...
// This gives the information of a linked NPC (describes action when its
// ActionTrigger triggers) Depends of the map
...
CreatureLinkingInfo const *CreatureLinkingMgr::GetLinkedTriggerInformation(uint32 entry, uint32 dbGuid,
                                                                           uint32 mapId) const
{
    // guid case
                                                   ...
                                                   Unit *pEnemy /* = nullptr*/)
{
    ...
    if (CreatureLinkingInfo const *pInfo = sCreatureLinkingMgr.GetLinkedTriggerInformation(pSource))
    {
        if (pInfo->linkingFlag & reverseEventFlagFilter)
        {
            ...
            if ((!pMaster || pMaster->IsCorpse()) && eventType == LINKING_EVENT_EVADE &&
                pSource->IsUsingNewSpawningSystem())
...
void CreatureLinkingHolder::ProcessSlaveGuidList(CreatureLinkingEvent eventType, Creature *pSource, uint32 flag,
                                                 uint16 searchRange,
                                                 std::list<std::pair<uint32, ObjectGuid>> &slaveGuidList, Unit *pEnemy)
{
    if (!flag)
        return;
    ...
    if (pSource->IsUsingNewSpawningSystem())
    {
        ...
        postprocessFlag =
            (postprocessFlag & ~(FLAG_RESPAWN_ON_EVADE | FLAG_RESPAWN_ON_DEATH | FLAG_RESPAWN_ON_RESPAWN));
    }
                                         ...
                                         Creature *pSlave, Unit *pEnemy)
{
    ...
    case LINKING_EVENT_RESPAWN:
        ...
        {
            // Additional check to prevent endless loops (in case whole group
            // respawns on first respawn)
...
void CreatureLinkingHolder::SetFollowing(Creature *pWho, Creature *pWhom) const
{
    ...
    // REMARK: This code needs the same distance calculation that is used for
    // following Atm this means we have to subtract the bounding radiuses
 ...
 *   in case of recursive use it is used only on _map with information of
 * dbGuid.
 *
 *  @param dbGuid (only relevant in case of recursive uses) -- db-guid of the
 * npc that is checked
 *  @param _map Map on which things are checked
 *  @param pInfo (only shipped in case of initial use) -- used as marker of
 * first use, also in first use filled directly
 *  @param sx, sy (spawn position of the checked npc with initial use)
 */
bool CreatureLinkingHolder::CanSpawn(uint32 dbGuid, Map *_map, CreatureLinkingInfo const *pInfo, float sx,
                                     float sy) const
{
