...
bool TileAssembler::convertWorld2()
{
    ...
    for (MapData::iterator map_iter = mapData.begin(); map_iter != mapData.end() && success; ++map_iter)
    {
        ...
        for (entry = map_iter->second->UniqueEntries.begin(); entry != map_iter->second->UniqueEntries.end(); ++entry)
        {
            ...
            else if (entry->second.flags & MOD_WORLDSPAWN) // WMO maps and terrain maps use different
        ...
        char isTiled = globalRange.first == globalRange.second; // only maps without terrain
                                                                // (tiles) have global WMO
...
bool TileAssembler::readMapSpawns()
{
    ...
    while (!feof(dirf))
    {
        // read mapID, tileX, tileY, Flags, adtID, ID, Pos, Rot, Scale, Bound_lo,
        // Bound_hi, name
