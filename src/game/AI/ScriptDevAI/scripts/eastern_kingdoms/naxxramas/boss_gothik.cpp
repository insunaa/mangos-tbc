...
enum
{
    ...
    // Right is right side from Gothik (eastern), i.e. right is living and left
    // is spectral
...
struct boss_gothikAI : public ScriptedAI
{
    ...
    void Aggro(Unit * /*who*/) override
    {
        ...
        m_summonTimer = 4 * IN_MILLISECONDS; // First spawn of trainees 24 secs after engage,
                                             // but the periodic summoning aura already has
                                             // an internal 20 secs timer
    }
    ...
    {
        DoCastSpellIfCan(m_creature, SPELL_RESET_GOTHIK_EVENT,
                         CAST_TRIGGERED); // Prevent more adds from spawing
    ...
    void UpdateAI(const uint32 diff) override
    {
        ...
        case PHASE_BALCONY: // Do nothing but wait to teleport down: summoning is
                            // handled by instance script
        ...
        case PHASE_TELEPORTING: // Phase is only reached if m_instance is valid
            if (m_teleportTimer < diff)
            {
                ...
                {
                    m_teleportTimer = urand(30, 45) * IN_MILLISECONDS; // Teleports between 30
            ...
            // Second time that Gothik teleports back from dead side to living side
            // or less than 30% HP: open the central gate
        ...
        case PHASE_STOP_TELEPORTING:
            ...
            {
                // Select valid target
                if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_SHADOWBOLT,
                                                                     SELECT_FLAG_IN_LOS))
...
struct CheckGothikSide : public SpellScript
{
    void OnEffectExecute(Spell *spell, SpellEffectIndex /* effIdx */) const override
    {
        if (Unit *caster = spell->GetCaster())
        {
            ...
            {
                // If central gate is open, attack any one (central gate is only
                // closed during IN_PROGRESS)
                ...
                else
                {
                    ...
                    for (const auto &playerInList : playersList)
                    {
                        if (Player *player = playerInList.getSource())
                        {
                            ...
                            if (spell->m_spellInfo->Id == SPELL_CHECK_UNRELENTING_SIDE &&
...
struct GothikSideAssault : public SpellScript
{
    void OnEffectExecute(Spell *spell, SpellEffectIndex /* effIdx */) const override
    {
        if (Unit *unitTarget = spell->GetUnitTarget())
        {
            if (instance_naxxramas *instance = (instance_naxxramas *)unitTarget->GetInstanceData())
            {
                // If for some reason one ore more creatures are not in combat when
                // the central gates open Set them in combat with zone
