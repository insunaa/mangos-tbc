...
instance_deadmines::instance_deadmines(Map *pMap)
    : ScriptedInstance(pMap), m_uiIronDoorTimer(0), m_uiDoorStep(0), m_firstEnter(true)
{
    Initialize();
}
...
void instance_deadmines::SpawnThirdDeadminesPatrol()
{
    ...
    WorldObject::SpawnCreature(GUID_PREFIX + 205,
                               instance); // Defias Taskmaster
    WorldObject::SpawnCreature(GUID_PREFIX + 206,
                               instance); // Defias Taskmaster
}
                ...
                if (GameObject *pDoor = GetSingleGameObjectFromStorage(GO_IRON_CLAD_DOOR))
                {
                    for (uint32 i = 148u; i <= 149; ++i)
                        if (Creature *guardfast = pDoor->GetMap()->GetCreature(GUID_PREFIX + i))
                            guardfast->GetMotionMaster()->MovePoint(0, pDoor->GetPositionX(), pDoor->GetPositionY(),
                                                                    pDoor->GetPositionZ(), FORCED_MOVEMENT_RUN, true);

                    if (Creature *guardlow = pDoor->GetMap()->GetCreature(GUID_PREFIX + 150))
                        guardlow->GetMotionMaster()->MovePoint(0, pDoor->GetPositionX(), pDoor->GetPositionY(),
                                                               pDoor->GetPositionZ(), FORCED_MOVEMENT_WALK, true);
                }
                ++m_uiDoorStep;
