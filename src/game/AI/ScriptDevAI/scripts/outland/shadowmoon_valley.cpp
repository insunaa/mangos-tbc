...
SD%Complete: 100
SDComment: Quest support: 10451, 10458, 10480, 10481, 10514, 10540, 10588,
10707, 10781, 10804, 10854, 10985, 11020, 11064, 11067, 11068, 11069, 11070,
11071. SDCategory: Shadowmoon Valley EndScriptData */
...
struct npc_wildaAI : public npc_escortAI
{
    ...
    void DoSpawnAssassin(uint8 uiCount = 1)
    {
        ...
        {
            m_creature->GetRandomPoint(m_creature->GetPositionX(), m_creature->GetPositionY(),
                                       m_creature->GetPositionZ(), 10.0f, fX, fY, fZ);
            m_creature->SummonCreature(NPC_COILSKAR_ASSASSIN, fX, fY, fZ, 0.0f, TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN,
                                       10000);
        }
...
struct npc_lord_illidan_stormrageAI : public Scripted_NoMovementAI
{
    ...
    void SummonWave()
    {
        ...
        for (uint8 i = 0; i < uiCount; ++i)
        {
            ...
            if (Creature *pSpawn =
                    m_creature->SummonCreature(WavesInfo[m_uiWaveCount].uiCreatureId, fLocX, fLocY, fLocZ, fOrient,
...
bool GOQuestAccept_GO_crystal_prison(Player *player, GameObject * /*go*/, Quest const *quest)
{
    if (quest->GetQuestId() == QUEST_BATTLE_OF_THE_CRIMSON_WATCH)
        if (Creature *lordIllidan = GetClosestCreatureWithEntry(player, NPC_LORD_ILLIDAN, 50.0))
            if (npc_lord_illidan_stormrageAI *illidanAI =
                    dynamic_cast<npc_lord_illidan_stormrageAI *>(lordIllidan->AI()))
...
struct npc_totem_of_spiritsAI : public ScriptedAI
{
    npc_totem_of_spiritsAI(Creature *creature) : ScriptedAI(creature)
    {
        ...
        Reset();
        m_uiElementalSieveTimer = 2500; // needs to be cast non-stop without
                                        // interference from evade and some such
    }
    ...
    bool IsPlayerOrGroupInRangeAndActiveInEvent()
    {
        ...
        if (Player *player = m_creature->GetMap()->GetPlayer(m_playerGuid)) // can find player
        {
            if (Group *group = player->GetGroup()) // player in group
            {
                for (GroupReference *ref = group->GetFirstMember(); ref != nullptr; ref = ref->next())
                {
                    Player *member = ref->getSource(); // any member gettable,
                                                       // alive & in range
                    if (member && member->IsAlive() && m_cHOFVisualTrigger &&
                        m_cHOFVisualTrigger->IsWithinDistInMap(member, MAX_PLAYER_DISTANCE))
                        return true;
        ...
        else
        {
            ...
            if (m_uiFelFireballTimer < uiDiff)
            {
                if (m_cDeathwailTrigger)
                {
                    ...
                    {
                        // TODO: creature summoning should be handled by spell, not
                        // here?
                        m_creature->SummonCreature(NPC_FELFIRE_SUMMONER, m_cDeathwailTrigger->GetPositionX(),
                                                   m_cDeathwailTrigger->GetPositionY(),
                                                   m_cDeathwailTrigger->GetPositionZ(), 0.0f, TEMPSPAWN_TIMED_DESPAWN,
                                                   6000, true);
                    }
                }
...
struct mob_shadowmoon_soulstealerAI : public ScriptedAI
{
    ...
    void Aggro(Unit *who) override
    {
        ...
        if (m_cDeathwail)
        {
            if (npc_shadowlord_deathwailAI *DeathwailAI =
                    dynamic_cast<npc_shadowlord_deathwailAI *>(m_cDeathwail->AI()))
            {
                if (!who->GetObjectGuid().IsPlayer()) // not attacked by player
                {
                    if (Unit *attackerOwner = m_creature->GetMap()->GetPlayer(who->GetOwnerGuid()))
                        who = attackerOwner;
                    else // not attacked by player, nor NPC owned by player... just
                         // ignore
                        exitCombat = true;
                }
...
struct npc_spawned_oronok_tornheartAI : public ScriptedAI, private DialogueHelper
{
    ...
    void JustRespawned() override
    {
        ...
        Creature *borak =
            m_creature->SummonCreature(NPC_BORAK_SON_OF_ORONOK, -3419.458f, 1383.739f, 228.1865f, 5.532694f,
        ...
        Creature *gromtor =
            m_creature->SummonCreature(NPC_GROMTOR_SON_OF_ORONOK, -3419.458f, 1383.739f, 228.1865f, 5.532694f,
                                       TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 300000, true, true);
    ...
    void JustDidDialogueStep(int32 iEntry) override
    {
        ...
        case NPC_CYRUKH_THE_FIRELORD: {
            Creature *cyrukh =
                m_creature->SummonCreature(NPC_CYRUKH_THE_FIRELORD, -3600.073f, 1799.803f, 39.70634f, 1.448623f,
                                           TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 300000, true, true);
        ...
        case NPC_REDEEMED_SPIRIT_OF_EARTH: {
            ...
            Creature *elemental = m_creature->SummonCreature(
                NPC_REDEEMED_SPIRIT_OF_AIR, aDamnationLocations[0].m_fX, aDamnationLocations[0].m_fY,
                aDamnationLocations[0].m_fZ, aDamnationLocations[0].m_fO, TEMPSPAWN_TIMED_DESPAWN, 32000);
    ...
    void EnterEvadeMode() override
    {
        ...
        {
            script_error_log("Npc %u couldn't be found or something really bad "
                             "happened. Epilogue event for quest %u will stop.",
...
bool GossipHello_npc_spawned_oronok_tornheart(Player *pPlayer, Creature *pCreature)
{
    ...
    {
        pPlayer->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_FIGHT, GOSSIP_SENDER_MAIN,
                                               ...
                                               uint32 uiAction)
{
    if (uiAction == GOSSIP_ACTION_INFO_DEF + 1)
    {
        ...
        pCreature->GetMotionMaster()->MovePoint(POINT_ID_ELEMENTS, aDamnationLocations[5].m_fX,
                                                aDamnationLocations[5].m_fY, aDamnationLocations[5].m_fZ);
            ...
            if (m_uiTuberTimer <= uiDiff)
            {
                ...
                case 1:
                    if (DoCastSpellIfCan(m_creature, SPELL_SHADOWMOON_TUBER) == CAST_OK)
                    {
                        ...
                        if (GameObject *pTuber =
                                GetClosestGameObjectWithEntry(m_creature, GO_SHADOWMOON_TUBER_MOUND, 3.0f))
...
struct npc_veneratus_spawn_nodeAI : public Scripted_NoMovementAI
{
    ...
    {
        // Check for the spirit hunter in order to spawn Veneratus; this will
        // replace missing spells 36614 (dummy periodic spell) and 36616 (summon
        // spell)
...
struct npc_disobedient_dragonmaw_peonAI : public ScriptedAI
{
    ...
    void HandleBooterang(Unit *caster)
    {
        ...
        DoCastSpellIfCan(nullptr,
                         SPELL_PEON_CLEAR_ALL); // clears combat and removes aura
...
struct npc_commanderAI : public CombatAI
{
    ...
    void SpawnDefenders()
    {
        ...
        {
            Creature *defender = m_creature->SummonCreature(
                deadliestScriptInfo[m_commanderId].defenderNpc, defenderSpawns[m_commanderId][i].x,
    ...
    void SpawnSpecificDragonmaw(uint32 i)
    {
        Creature *dragonmaw = m_creature->SummonCreature(
            ...
            dragonmawSpawns[m_commanderId][i].y, dragonmawSpawns[m_commanderId][i].z,
            dragonmawSpawns[m_commanderId][i].ori, TEMPSPAWN_CORPSE_DESPAWN, 1000, true, true, i);
...
{
    npc_dragonmaw_racerAI(Creature *creature, uint8 racerId) : ScriptedAI(creature), m_racerId(racerId)
    {
        Reset();
    }
    ...
    void StartRace()
    {
        ...
        if (dragonmawRacesScriptInfo[m_racerId].checkSpellId)
            DoCastSpellIfCan(m_creature,
                             dragonmawRacesScriptInfo[m_racerId].checkSpellId); // begins failure checking
    }
    ...
    virtual void AttackPlayer()
    {
        if (Player *pPlayer = m_creature->GetMap()->GetPlayer(m_playerRacerGuid))
        {
            ...
            m_creature->SummonCreature(dragonmawRacesScriptInfo[m_racerId].targetNpc, fX, fY, fZ, 0,
                                       TEMPSPAWN_TIMED_DESPAWN, 6000);
            m_creature->GetRandomPoint(pPlayer->GetPositionX(), pPlayer->GetPositionY(), pPlayer->GetPositionZ(), 10.0f,
                                       fX, fY, fZ);
            m_creature->SummonCreature(dragonmawRacesScriptInfo[m_racerId].targetNpc, fX, fY, fZ, 0,
                                       TEMPSPAWN_TIMED_DESPAWN, 6000);
        }
...
struct npc_dragonmaw_racer_skyshatterAI : public npc_dragonmaw_racerAI
{
    ...
    void AttackPlayer() override
    {
        ...
        {
            float fX, fY, fZ;
            m_creature->GetRandomPoint(pPlayer->GetPositionX(), pPlayer->GetPositionY(), pPlayer->GetPositionZ(), 5.0f,
                                       fX, fY, fZ);
            m_creature->SummonCreature(dragonmawRacesScriptInfo[m_racerId].targetNpc, fX, fY, fZ, 0,
                                       TEMPSPAWN_TIMED_DESPAWN, 6000);
        }
...
enum
{
    ...
    // Spells
    SPELL_BATTLE_FLOW_REGULATOR = 39699, // The BT Battle Sensor NPC casts this spell/aura, presumably
                                         // using it to control the constant demon/aldor/scryer fight
                                         // outside of BT, no clue how it works
...
struct mob_bt_battle_fighterAI : public ScriptedAI
{
    ...
    uint32 GetInitialActionTimer(const uint32 action) const
    {
        ...
        default:
            sLog.outDebug("Script error: mob_bt_battle_fighterAI::GetInitialActionTimer()> "
                          "Not "
                          "valid npc entry!");
            return 0;
        }
    ...
    uint32 GetSubsequentActionTimer(const uint32 action) const
    {
        ...
        default:
            sLog.outDebug("Script error: "
                          "mob_bt_battle_fighterAI::GetSubsequentActionTimer()> "
                          "Not valid npc entry!");
            return 0;
        }
    }
    ...
    void Aggro(Unit *who) override
    {
        ...
        case NPC_LIGHTSWORN_VINDICATOR: {
            if (!urand(0, 9))
            {
                DoScriptText(SAY_VINDICATOR_ON_AGGRO_1, m_creature,
                             who); // missing texts?
            }
        ...
        case NPC_ILLIDARI_RAVAGER:
            ...
            case AI_EVENT_CUSTOM_B: {
                if (Creature *senderCreature = dynamic_cast<Creature *>(sender))
                    senderCreature->GetMotionMaster()->MoveRandomAroundPoint(
                        sender->GetPositionX(), sender->GetPositionY(), sender->GetPositionZ(), 15.0f);
            ...
            case AI_EVENT_CUSTOM_EVENTAI_A: {
                ...
                if (!marker.IsEmpty())
                    if (Creature *markerCreature = m_creature->GetMap()->GetCreature(marker))
                    {
                        ...
                        sender->GetMotionMaster()->MovePoint(0, markerCreature->GetPositionX(),
                                                             markerCreature->GetPositionY(),
                ...
                if (Creature *fighterCreature = m_creature->GetMap()->GetCreature(fighter))
                    if (!fighterCreature->IsInCombat())
                        if (Creature *markerCreature = m_creature->GetMap()->GetCreature(marker))
                        {
                            ...
                            fighterCreature->GetMotionMaster()->MovePoint(0, markerCreature->GetPositionX(),
                                                                          markerCreature->GetPositionY(),
                                                                          markerCreature->GetPositionZ());
...
struct TagGreaterFelfireDiemetradon : public SpellScript, public AuraScript
{
    SpellCastResult OnCheckCast(Spell *spell, bool /*strict*/) const override
    {
        ...
        if (!target || !target->IsCreature() ||
            static_cast<Creature *>(target)->HasBeenHitBySpell(spell->m_spellInfo->Id))
