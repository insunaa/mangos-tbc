...
struct npc_skyguard_rangerAI : public ScriptedAI
{
    npc_skyguard_rangerAI(Creature *creature)
        : ScriptedAI(creature, RANGER_COMBAT_ACTION_MAX), m_spawnId(PATH_ID_DISMOUNT) // implicit default for lieutenant
    {
    ...
    void ExecuteActions() override
    {
        ...
        for (uint32 i = 0; i < FIEND_ACTION_MAX; ++i)
        {
            if (GetActionReadyStatus(i))
            {
                ...
                case FIEND_ACTION_EAT_FRIEND: {
                    // TODO:
                    Creature *slaveringSlave =
                        GetClosestCreatureWithEntry(m_creature, NPC_SLAVERING_SLAVE, 10.f, false, true);
                    if (slaveringSlave) // found corpse of ally
                    {
                        SetCombatScriptStatus(true);
                        SetCombatMovement(false);
                        float x, y, z;
                        m_creature->GetVictim()->GetNearPoint(
                            m_creature, x, y, z, m_creature->GetObjectBoundingRadius(),
                            m_creature->GetCombinedCombatReach(slaveringSlave), slaveringSlave->GetAngle(m_creature));
...
struct npc_grand_collectorAI : public ScriptedAI
{
    npc_grand_collectorAI(Creature *creature)
        : ScriptedAI(creature, COLLECTOR_COMBAT_ACTION_MAX), m_introEventId(0),
...
void AddSC_bashir_landing()
{
    ...
    pNewScript->GetAI = &GetNewAIInstance<npc_skyguard_rangerAI>; // same AI handles them until
