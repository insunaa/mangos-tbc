...
enum TempSpawnType
{
    ...
    TEMPSPAWN_CORPSE_TIMED_DESPAWN = 3,        // despawns after a specified time after
                                               // death (or when the creature disappears)
    ...
    TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN = 8,   // despawns after a specified time (OOC) OR when the creature
                                               // disappears
...
struct PlayPacketParameters
{
    ...
    PlayPacketParameters(PlayPacketSettings setting, uint32 id) : setting(setting)
    {
        this->areaOrZone.id = id;
    }
  ...
  public:
                     ...
                     uint32 categoryDuration = 0, uint32 itemId = 0, bool onHold = false)
    {
        ...
        auto resultItr = m_spellIdMap.emplace(
            spellId, std::move(std::unique_ptr<CooldownData>(new CooldownData(
                         clockNow, spellId, duration, spellCategory, categoryDuration, itemId, onHold))));
        // do not overwrite one permanent category cooldown with another permanent
        // category cooldown
    ...
    ConstIterator FindBySpellId(uint32 id) const
    {
        return m_spellIdMap.find(id);
    }
    ...
    {
        return m_spellIdMap.begin();
    }
    ConstIterator end() const
    {
        return m_spellIdMap.end();
    }
    bool IsEmpty() const
    {
        return m_spellIdMap.empty();
    }
    size_t size() const
    {
        return m_spellIdMap.size();
    }
...
class Object
{
  public:
    ...
    // must be overwrite in appropriate subclasses (WorldObject, Item currently),
    // or will crash
    ...
    void SetItsNewObject(bool enable)
    {
        m_itsNewObject = enable;
    }
    ...
    inline bool IsCreature() const
    {
        return GetTypeId() == TYPEID_UNIT;
    }
    inline bool IsUnit() const
    {
        return isType(TYPEMASK_UNIT);
    }
    inline bool IsGameObject() const
    {
        return GetTypeId() == TYPEID_GAMEOBJECT;
    }
    inline bool IsCorpse() const
    {
        return GetTypeId() == TYPEID_CORPSE;
    }
...
struct TempSpawnSettings
{
    ...
    TempSpawnSettings(WorldObject *spawner, uint32 entry, float x, float y, float z, float ori, TempSpawnType spawnType,
                      ...
                      uint32 spellId = 0, int32 movegen = -1, uint32 level = 0)
        : spawner(spawner), entry(entry), x(x), y(y), z(z), ori(ori), spawnType(spawnType), despawnTime(despawnTime),
          ...
          spawnCounting(spawnCounting), forcedOnTop(forcedOnTop), spellId(spellId), movegen(movegen), level(level)
    {
    }
};
...
class MovementInfo
{
  public:
    ...
    inline UnitMoveType GetSpeedType() const
    {
        return GetSpeedType(MovementFlags(moveFlags));
    }
    ...
    inline float GetOrientationInMotion(float o) const
    {
        return GetOrientationInMotion(MovementFlags(moveFlags), o);
    }
    ...
    void UpdateTransportData(Position pos)
    {
        t_pos = pos;
    }
    ...
    ObjectGuid const &GetTransportGuid() const
    {
        return t_guid;
    }
...
class WorldObject : public Object
{
  ...
  public:
    ...
    virtual uint32 GetHeartbeatDuration() const
    {
        return 5000;
    }
    ...
    {
        return m_transportInfo != nullptr;
    }
    ...
    /// Gives a 2d-point in distance distance2d in direction absAngle around the
    /// current position (point-to-point)
    ...
    static void GetNearPoint2dAt(const float posX, const float posY, float &x, float &y, float distance2d,
                                 float absAngle);
    /** Gives a "free" spot for searcher in distance distance2d in direction
     * absAngle on "good" height
     * @param searcher          -           for whom a spot is searched for
     * @param x, y, z           -           position for the found spot of the
     * searcher
     * @param searcher_bounding_radius  -   how much space the searcher will
     * require
    ...
    {
        return GetNearPointAt(GetPositionX(), GetPositionY(), GetPositionZ(), searcher, x, y, z,
                              searcher_bounding_radius, distance2d, absAngle, isInWater);
    }
    ...
    /** Gives a "free" spot for a searcher on the distance (including
     * bounding-radius calculation)
     * @param x, y, z           -           position for the found spot
     * @param bounding_radius   -           radius for the searcher
     * @param distance2d        -           range in which to find a free spot.
     * Default = 0.0f (which usually means the units will have contact)
     * @param angle             -           direction in which to look for a free
     * spot. Default = 0.0f (direction in which 'this' is looking
     * @param obj               -           for whom to look for a spot. Default
     * = nullptr
     */
    ...
    /** Gives a "free" spot for a searcher in contact-range of "this" (including
     * bounding-radius calculation)
     * @param x, y, z           -           position for the found spot
     * @param obj               -           for whom to find a contact position.
     * The position will be searched in direction from 'this' towards 'obj'
     * @param distance2d        -           distance which 'obj' and 'this'
     * should have beetween their bounding radiuses. Default = CONTACT_DISTANCE
     */
    ...
    // function attempts to preserve at least 80% of distance - observed on fear
    // and random spell point picking behaviour
    ...
    virtual ObjectGuid const &GetOwnerGuid() const
    {
        return GetGuidValue(OBJECT_FIELD_GUID);
    }
    ...
    // Spawner: guid of a unit, who is reponsible for starting this objects's
    // parent script (only for script-spawned objects)
    virtual ObjectGuid const GetSpawnerGuid() const
    {
        return ObjectGuid();
    }
    ...
    bool IsInMapIgnorePhase(const WorldObject *obj) const // only to be used by spells which ignore
                                                          // phase during search and similar
    {
    ...
    bool IsWithinCombatDistInMap(WorldObject const *obj, float dist2compare, bool is3D = true,
                                 bool ignorePhase = false) const
    {
        return obj && (ignorePhase ? IsInMapIgnorePhase(obj) : IsInMap(obj)) &&
               _IsWithinCombatDist(obj, dist2compare, is3D);
    }
    ...
    virtual void CleanupsBeforeDelete(); // used in destructor or explicitly before mass
                                         // creature delete to remove cross-references to
                                         // already deleted units
    ...
    virtual bool IsNotifyOnEventObject()
    {
        return m_isOnEventNotified;
    }
    ...
    int32 CalculateSpellEffectValue(Unit const *target, SpellEntry const *spellProto, SpellEffectIndex effect_index,
                                    int32 const *basePoints = nullptr, bool maximum = false,
                                    bool finalUse = true) const;

    VisibilityData const &GetVisibilityData() const
    {
    ...
    bool HaveDebugFlag(CMDebugFlags flag) const
    {
        return (uint64(m_debugFlags) & flag) != 0;
    }
    void SetDebugFlag(CMDebugFlags flag)
    {
        m_debugFlags |= uint64(flag);
    }
    void ClearDebugFlag(CMDebugFlags flag)
    {
        m_debugFlags &= ~(uint64(flag));
    }
    ...
    // Transports
    GenericTransport *GetTransport() const
    {
        return m_transport;
    }
    void SetTransport(GenericTransport *t)
    {
        m_transport = t;
    }
    ...
    virtual uint32 GetDbGuid() const
    {
        return 0;
    }
    virtual HighGuid GetParentHigh() const
    {
        return HighGuid(0);
    }
    ...
    GuidSet &GetClientGuidsIAmAt()
    {
        return m_clientGUIDsIAmAt;
    }
    ...
    // Spell mod owner: static player whose spell mods apply to this unit
    // (server-side)
