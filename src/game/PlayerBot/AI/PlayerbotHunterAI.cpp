...
{
    QueryResult *result = CharacterDatabase.PQuery("SELECT * FROM character_pet WHERE owner = '%u' AND (slot = '%u' OR "
                                                   "slot "
                                                   "= '%u')",
...
CombatManeuverReturns PlayerbotHunterAI::DoNextCombatManeuverPVE(Unit *pTarget)
{
    ...
    /*    // racial traits
       ...
       EFFECT_INDEX_0)) m_ai.CastSpell(BLOOD_FURY, *m_bot); else if
       (m_bot.getRace() == RACE_TROLL && !m_bot.HasAura(BERSERKING,
       EFFECT_INDEX_0)) m_ai.CastSpell(BERSERKING, *m_bot);
    */
    ...
    // Distance management: avoid to be in the dead zone where neither melee nor
    // range can be used: keep distance whenever possible If not in range: come
    // closer Do not do it if passive or stay orders.
    ...
    {
        // Debuff target
        if (HUNTERS_MARK > 0 && m_ai.In_Reach(pTarget, HUNTERS_MARK) &&
            !pTarget->HasAura(HUNTERS_MARK, EFFECT_INDEX_0) && m_ai.CastSpell(HUNTERS_MARK, *pTarget) == SPELL_CAST_OK)
            return RETURN_CONTINUE;
        // Buff self
        ...
        if (ARCANE_SHOT > 0 && m_bot.IsSpellReady(ARCANE_SHOT) && m_ai.In_Range(pTarget, ARCANE_SHOT) &&
            ...
            return RETURN_CONTINUE;
        // Stings: only use Viper and Serpent sting. Stats decrease (Scorpid
        // Sting) is useless in PvE and as the bot is obviously not alone and
        // assisting someone, no need to put the target to sleep (Wyvern Sting)
        // Viper sting for non-worldboss mana users
        if (pTarget->GetPower(POWER_MANA) > 0 && !m_ai.IsElite(pTarget, true))
        {
            if (VIPER_STING > 0 && m_ai.In_Range(pTarget, VIPER_STING) &&
        ...
        else
        {
            if (SERPENT_STING > 0 && m_ai.In_Range(pTarget, SERPENT_STING) &&
                ...
                m_ai.CastSpell(SERPENT_STING, *pTarget) == SPELL_CAST_OK)
                return RETURN_CONTINUE;
        }
        ...
        //       if (MULTI_SHOT > 0 && m_bot.IsSpellReady(MULTI_SHOT) &&
        //       m_ai.In_Range(pTarget,MULTI_SHOT) && m_ai.GetAttackerCount() >=
        //       3
        //       && m_ai.CastSpell(MULTI_SHOT, *pTarget) == SPELL_CAST_OK)
        //            return RETURN_CONTINUE;
        //       if (VOLLEY > 0 && m_ai.In_Range(pTarget,VOLLEY) &&
        //       m_ai.GetAttackerCount() >= 3 && m_ai.CastSpell(VOLLEY, *pTarget)
        //       == SPELL_CAST_OK)
        //           return RETURN_CONTINUE;

        // Auto shot
        // m_ai.TellMaster("target dist
        // %f",m_bot.GetCombatDistance(pTarget,true));
        if (AUTO_SHOT > 0)
        {
    ...
    else
    {
        ...
        //            return RETURN_CONTINUE;
        //        else if (m_bot.getRace() == RACE_DWARF &&
        //        m_bot.HasAuraState(AURA_STATE_DEADLY_POISON) &&
        //        m_ai.CastSpell(STONEFORM, *m_bot) == SPELL_CAST_OK)
        //            return RETURN_CONTINUE;

        /*else if(FREEZING_TRAP > 0 && !pTarget->HasAura(FREEZING_TRAP,
           EFFECT_INDEX_0) && !pTarget->HasAura(ARCANE_TRAP, EFFECT_INDEX_0) &&
           ...
           m_ai.CastSpell(FREEZING_TRAP,*pTarget) == SPELL_CAST_OK) out << " >
           Freezing Trap"; // this can trap your bots too
        */
    }
...
void PlayerbotHunterAI::DoNonCombatActions()
{
    ...
    if (PET_SUMMON > 0 && !m_petSummonFailed && m_bot.GetPetGuid())
    {
        ...
        Pet *pet = m_bot.GetPet();
        if (!pet) // Hunter has pet but it is not found: dismissed or dead and
                  // corpse was removed
        {
            // summon pet
            ...
            case SPELL_FAILED_DONT_REPORT:
                if (PET_REVIVE > 0)
                {
                    ...
                    case SPELL_CAST_OK:
                        ...
                        break;
                    case SPELL_FAILED_NO_POWER: // Not enough mana, just wait
                                                // for it
                        break;
                    default:
                ...
                break;
            case SPELL_FAILED_NO_PET: // This should not happen as if we went
                                      // this far, there is a pet entry in the
                                      // DB
                m_petSummonFailed = true;
                m_ai.TellMaster("I don't appear to have a pet. Weird...");
                break;
            default: // Also, pure sanity check: should never happen unless
        ...
        else if (pet->GetHappinessState() != HAPPY) // if pet is hungry
        {
            ...
            for (uint8 slot = INVENTORY_SLOT_ITEM_START; slot < INVENTORY_SLOT_ITEM_END; slot++)
            {
                ...
                if (pItem)
                {
                    ...
                    {
                        // DEBUG_LOG ("[PlayerbotHunterAI]: DoNonCombatActions -
                        // Food for pet: %s",pItemProto->Name1);
                        // caster->CastSpell(caster, 23355,
                        // TRIGGERED_OLD_TRIGGERED); // pet feed visual
                        ...
                        m_bot.DestroyItemCount(pItem, count,
                                               true); // remove item from inventory
                ...
                if (pBag)
                    for (uint8 slot = 0; slot < pBag->GetBagSize(); ++slot)
                    {
                        ...
                        if (pItem)
                        {
                            ...
                            if (pet->HaveInDiet(pItemProto)) // is pItem in pets diet
                            {
                                // DEBUG_LOG ("[PlayerbotHunterAI]:
                                // DoNonCombatActions - Food for pet:
                                // %s",pItemProto->Name1);
                                // caster->CastSpell(caster, 23355,
                                // TRIGGERED_OLD_TRIGGERED); // pet feed visual
                                uint32 count = 1; // number of items used
                                int32 benefit = pet->GetCurrentFoodBenefitLevel(pItemProto->ItemLevel) *
                                                15; // nutritional value of food
                                m_bot.DestroyItemCount(pItem, count,
                                                       true); // remove item from inventory
                                m_bot.CastCustomSpell(&m_bot, PET_FEED, &benefit, nullptr, nullptr,
                                                      TRIGGERED_OLD_TRIGGERED); // feed pet
