...
CombatManeuverReturns PlayerbotDruidAI::DoNextCombatManeuverPVE(Unit *pTarget)
{
    ...
    // Make sure healer stays put, don't even melee (aggro) if in range: only
    // melee if feral spec AND not healer
    ...
    // case RETURN_FAIL_WAITINGONSELFBUFF: // This is war dammit! No time for
    // silly buffs during combat...
    default:
        break;
    }
        ...
        !PlayerbotAI::IsNeutralized(newTarget)) // TODO: && party has a tank
    {
        ...
        {
            // protect the bot with barkskin: the increased casting time is
            // meaningless because bot will then avoid to cast to not angry mob
            // further
            ...
            // no other cases: cats have cower in the damage rotation and bears
            // can tank
        }
    }
    ...
    case DRUID_SPEC_RESTORATION: // There is no Resto DAMAGE rotation. If you
                                 // insist, go Balance...
    case DRUID_SPEC_BALANCE:
        if (m_bot.HasAura(BEAR) || m_bot.HasAura(CAT_FORM) || m_bot.HasAura(TREE_OF_LIFE))
            return RETURN_NO_ACTION_UNKNOWN; // Didn't shift out of inappropriate
                                             // form

        return _DoNextPVECombatManeuverSpellDPS(pTarget);

        /*if (BASH > 0 && !pTarget->HasAura(BASH, EFFECT_INDEX_0) &&
        DruidSpellCombat < 5 && CastSpell(BASH, pTarget)) return RETURN_CONTINUE;
        if (CHALLENGING_ROAR > 0 && pVictim != m_bot &&
        !pTarget->HasAura(CHALLENGING_ROAR, EFFECT_INDEX_0) &&
        !pTarget->HasAura(GROWL, EFFECT_INDEX_0) && CastSpell(CHALLENGING_ROAR,
        pTarget)) return RETURN_CONTINUE; if (ROOTS > 0 &&
        !pTarget->HasAura(ROOTS, EFFECT_INDEX_0) && CastSpell(ROOTS, pTarget))
        return RETURN_CONTINUE; if (HURRICANE > 0 &&
        ai->In_Reach(target,HURRICANE) && m_ai.GetAttackerCount()
        >= 5 && CastSpell(HURRICANE, pTarget))
        {
...
CombatManeuverReturns PlayerbotDruidAI::_DoNextPVECombatManeuverSpellDPS(Unit *pTarget)
{
    ...
    // Combat resurrection (only tanks or master. If other targets are required,
    // let master explicitly ask to)
...
CombatManeuverReturns PlayerbotDruidAI::_DoNextPVECombatManeuverHeal()
{
    ...
    // Combat resurrection (only tanks or master. If other targets are required,
    // let master explicitly requests it)
    if (ResurrectPlayer(GetResurrectionTarget(JOB_TANK_MASTER, false)) & RETURN_CONTINUE)
        return RETURN_CONTINUE;

    // Heal (try to pick a target by on common rules, than heal using each
    // PlayerbotClassAI HealPlayer() method)
...
CombatManeuverReturns PlayerbotDruidAI::HealPlayer(Player *target)
{
    ...
    // Everyone is healthy enough, return OK. MUST correlate to highest value
    // below (should be last HP check)
    if (hp >= 80)
        return RETURN_NO_ACTION_OK;
        ...
        (GetTargetJob(target) != JOB_TANK && GetTargetJob(target) != JOB_MAIN_TANK && hp < 15))
    {
        ...
        if (HEALING_TOUCH > 0 && m_bot.HasAura(NATURES_SWIFTNESS, EFFECT_INDEX_0) &&
            m_ai.In_Reach(target, HEALING_TOUCH) && CastSpell(HEALING_TOUCH, target))
            return RETURN_CONTINUE;
    ...
    // Urgent heal: target won't die next second, but first bot needs to gain
    // some time to cast Healing Touch safely
    ...
    {
        if (REGROWTH > 0 && m_ai.In_Reach(target, REGROWTH) && !target->HasAura(REGROWTH) &&
            CastSpell(REGROWTH, target))
            return RETURN_CONTINUE;
 ...
 * CheckForms()
 *
 * Returns bool - Value indicates success - shape was shifted, already shifted,
 * no need to shift.
 */
...
void PlayerbotDruidAI::DoNonCombatActions()
{
    ...
    // Revive
    // No auto-revive out of combat to preserve cooldown. Let master explicitly
    // ask bot to cast Rebirth if needed
    ...
    else
    {
        ...
        // TODO: In a group/raid with a healer you'd want this bot to focus on
        // DPS (it's not specced/geared for healing either)
        if (HealPlayer(&m_bot) & RETURN_CONTINUE)
            return; // RETURN_CONTINUE;
    }

    // Buff group
    // the check for group targets is performed by NeedGroupBuff (if group is
    // found for bots by the function)
...
{
    // RANK_1 spell ids used because this is a static method which does not have
    // access to instance. There is only one rank for these spells anyway.
...
// Return to UpdateAI the spellId usable to neutralize a target with
// creaturetype
