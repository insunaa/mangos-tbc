...
enum
{
    ...
    SPELL_MOVE_2 = 37388,       // spell which selects AI move square (for long range
                                // pieces) SPELL_MOVE_PAWN              = 37146, //
                                // individual move spells (used only by controlled
                                // npcs) SPELL_MOVE_KNIGHT            = 37144,
    ...
    SPELL_MOVE_TO_SQUARE = 30253, // spell which sends the move response from the
                                  // square to the piece
    ...
    SPELL_DISABLE_SQUARE = 32745, // used by the White / Black triggers on the
                                  // squares when a chess piece moves into place
    SPELL_IS_SQUARE_USED = 39400, // cast when a chess piece moves to another square
                                  // SPELL_SQUARED_OCCUPIED       = 39399,                    //
                                  // triggered by 39400; used to check if the square is occupied (if
                                  // hits a target); Missing in 2.4.3
    ...
    SPELL_MELEE_DAMAGE = 32247,          // melee damage spell - used by all chess pieces
                                         // SPELL_AI_SNAPSHOT_TIMER      = 37440,                    //
                                         // used to trigger spell 32260; purpose and usage unk
                                         // SPELL_DISABLE_SQUARE_SELF    = 32260,                    //
                                         // used when a piece moves to another square SPELL_AI_ACTION_TIMER
                                         // = 37504,                    // handle some kind of event check.
                                         // Cast by npc 17459. Currently the way it works is unk
                                         // SPELL_DISABLE_SQUARE         = 30271,                    // not
                                         // used SPELL_FIND_ENEMY             = 32303, // not used
                                         // SPELL_MOVE_NEAR_UNIT         = 30417, // not used
                                         // SPELL_GET_EMPTY_SQUARE       = 30418,                    // not
                                         // used SPELL_FACE_NEARBY_ENEMY      = 37787, // not used
    ...
    // SPELL_FULL_HEAL_HORDE        = 39334,                    // spells are not
    // confirmed (probably removed after 2.4.3) SPELL_FULL_HEAL_ALLIANCE     =
    // 39335,
    ...
    // generic npcs
    // NPC_SQUARE_OUTSIDE_B         = 17316,                    // used to check
    // the interior of the board NPC_SQUARE_OUTSIDE_W         = 17317, // not
    // used
    // in our script; keep for reference only
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (m_uiCheatTimer < uiDiff)
        {
            DoCastSpellIfCan(m_creature,
                             urand(0, 1) ? (m_pInstance->GetPlayerTeam() == ALLIANCE ? SPELL_HAND_OF_MEDIVH_HORDE
...
struct npc_chess_piece_genericAI : public Scripted_NoMovementAI
{
    ...
    void Reset() override
    {
        ...
        if (m_pInstance)
        {
            // Reason why != is because when player takes control, chess piece gets
            // his faction
    ...
    void ReceiveAIEvent(AIEventType eventType, Unit * /*pSender*/, Unit *pInvoker, uint32 /*uiMiscValue*/) override
    {
        ...
        else if (eventType == AI_EVENT_CUSTOM_B)
        {
            ...
            // if (Creature* pStalker =
            // m_pInstance->GetSingleCreatureFromStorage(NPC_WAITING_ROOM_STALKER))
            //    pStalker->CastSpell(pStalker, SPELL_AI_ACTION_TIMER,
            //    TRIGGERED_OLD_TRIGGERED);

            // DoCastSpellIfCan(m_creature, SPELL_AI_SNAPSHOT_TIMER,
            // CAST_TRIGGERED);
    ...
    Unit *GetTargetByType(uint8 uiType, float fRange, float fArc = M_PI_F) const
    {
        ...
        for (GuidList::const_iterator itr = lTempList.begin(); itr != lTempList.end(); ++itr)
        {
            ...
            {
                // check for specified range targets and angle; Note: to be checked
    ...
    Unit *GetMovementSquare() const
    {
        ...
        // this will replace the targeting sysmte of spells SPELL_MOVE_1 and
        // SPELL_MOVE_2 - HACK
        float fRadius = 10.0f;
        ...
        // Sort the enemies by distance and the squares compared to the distance to
        // the closest enemy
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (m_uiMoveCommandTimer)
        {
            if (m_uiMoveCommandTimer <= uiDiff)
            {
                ...
                {
                    // the npc doesn't have a 100% chance to move; also there
                    // should be some GCD check in core for this part
                    if (roll_chance_i(15))
                    {
                        // Note: in a normal case the target would be chosen using
                        // the spells above However, because the core doesn't
                        // support special targeting, we'll provide explicit target
                        ...
                        //        break;
                        //}
                        // DoCastSpellIfCan(m_creature, uiMoveSpell,
                        // CAST_TRIGGERED);
                                           ...
                                           Creature *pCreatureTarget, ObjectGuid /*originalCasterGuid*/)
{
    ...
    if (uiSpellId == SPELL_FACE_SQUARE && uiEffIndex == EFFECT_INDEX_0)
    {
        if (pCaster->GetTypeId() == TYPEID_UNIT)
        {
            pCreatureTarget->SetInFront(pCaster); // set movementinfo orientation,
                                                  // needed for next movement if any
