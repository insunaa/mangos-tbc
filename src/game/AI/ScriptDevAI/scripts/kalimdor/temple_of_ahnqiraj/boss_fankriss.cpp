...
struct boss_fankrissAI : public CombatAI
{
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case FANKRISS_SUMMON_WORM: {
            ...
            {
                --m_wormsSpawnPerWave;
                // Initialise the next wave now that all Spawn of Fankriss for
                // the current wave were spawned
                if (!m_wormsSpawnPerWave)
                {
                    // Determine how many Spawn of Fankriss will be spawned in
                    // the next wave
                    m_wormsSpawnPerWave = urand(1, 3);
                    // Randomise the summoning spells so each NPC spawns in a
                    // different location
                    std::shuffle(m_summonWormSpells.begin(), m_summonWormSpells.end(), *GetRandomGenerator());
        ...
        case FANKRISS_ENTANGLE: {
            ...
            // Teleport and untangle one random player into one of the three
            // alcoves
            ...
            // At combat start, Vekniss Hatchlings are summoned in all three
            // alcoves
