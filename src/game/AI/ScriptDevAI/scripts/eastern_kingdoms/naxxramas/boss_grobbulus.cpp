...
enum
{
    ...
    SPELL_DESPAWN_SUMMONS = 30134, // Spell ID unsure: there are several spells doing the same thing
...
{
    boss_grobbulusAI(Creature *creature)
        : CombatAI(creature, GROBBULUS_ACTION_MAX),
    ...
    uint32 GetSubsequentActionTimer(uint32 id)
    {
        ...
        case GROBBULUS_INJECTION:
            return m_creature->GetHealthPercent() > 30.0f ? urand(7, 13) * IN_MILLISECONDS
                                                          : urand(3, 7) * IN_MILLISECONDS;
        case GROBBULUS_POISON_CLOUD:
            return 15u * IN_MILLISECONDS;
        case GROBBULUS_SLIME_SPRAY:
            return urand(20, 30) * IN_MILLISECONDS;
        default:
            return 0;
        }
    }
