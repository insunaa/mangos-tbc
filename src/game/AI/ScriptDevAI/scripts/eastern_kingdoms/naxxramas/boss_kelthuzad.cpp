...
SD%Complete: 99
SDComment: Phase 1 summoning timers and/or aura stacking are to be improved
once Classic sniffs are available SDCategory: Naxxramas EndScriptData */
...
enum
{
    ...
    SPELL_GUARDIAN_INIT = 29898,     // Cast at start of phase 3 by Kel'Thuzad (start of phase 2 in
                                     // sniffs but this way we avoid running a timer needlessly)

    MAX_CONTROLLED_TARGETS = 4, // 5 targets are mind controlled by Chains of Kel'Thuzad but the main
                                // tank is always the fifth one
};

static const uint32 phaseOneAdds[] = {NPC_SOLDIER_FROZEN, NPC_UNSTOPPABLE_ABOM, NPC_SOUL_WEAVER};

// List which spell must be used at which point in phase 1 to summon a given
// NPC type
...
struct boss_kelthuzadAI : public ScriptedAI
{
    ...
    void Reset() override
    {
        ...
        m_guardiansSummonTimer = 61 * IN_MILLISECONDS; // One Guardian of Icecrown is summoned every
                                                       // 10 sec ; five of them in total
    ...
    void SpellHit(Unit * /*caster*/, const SpellEntry *spell) override
    {
        ...
        // Phase 1 periodic update every 1 second (everything in phase 1 is handled
        // here rather than in UpdateAI())
        else if (spell->Id == SPELL_CHANNEL_VISUAL_EFFECT)
        {
            ...
            case MAX_SUMMON_TICKS:
                DespawnIntroCreatures(); // Will leave those that for some reason
                                         // are in combat
                switch (urand(0, 2))
                {
                case 0:
            ...
            default:
                UpdateSummoning();
                // Kel'Thuzad does not enter combat until phase 2, so we check
                // every second if there are still players alive and force him to
                // evade otherwise
    ...
    // Called every tick (1 second) to check if the periodic summoning auras on
    // Kel'Thuzad need to be updated or removed
    void UpdateSummoning()
    {
        for (auto add : phaseOneAdds)
        {
            ...
            // Iterate the summoning auras timeline for the current add as long as
            // the tick is in range then exit with the last valid summoning aura
            ...
            // Do not update aura for current add type if summoning aura is already
            // present
            if (m_creature->HasAura(summoningAura))
                continue;

            // Clear all summoning auras for current add before applying the new
            // one (especially needed in case new one is NULL)
    ...
    bool SummonIntroCreatures(uint32 entry, uint8 count)
    {
        ...
        {
            // "How many NPCs per type" is stored in a vector:
            // {npc_entry:number_of_npcs}
            for (uint8 i = 0; i < count; ++i)
            {
                m_creature->GetRandomPoint(trigger->GetPositionX(), trigger->GetPositionY(), trigger->GetPositionZ(),
                                           12.0f, newX, newY, newZ);
                if (Creature *summoned = m_creature->SummonCreature(
                        entry, newX, newY, newZ, 0.0f, TEMPSPAWN_CORPSE_DESPAWN, 5 * MINUTE * IN_MILLISECONDS))
                {
                    if (summoned->AI())
                        summoned->AI()->SetReactState(REACT_PASSIVE); // Intro mobs only attack if engaged or
    ...
    // Every time an add dies, randomly summon the same type on one of the seven
    // corners
...
// This NPC handles the spell sync between the player that is web wrapped (with
// a DoT) and the related Web Wrap NPC
struct npc_icecrown_guardianAI : public ScriptedAI
{
    ...
    {
        m_victimGuid.Clear();
        m_checkCurrentVictimTimer = 2 * IN_MILLISECONDS; // Check every 2 seconds that we still have the
                                                         // same target
    ...
    void UpdateAI(const uint32 diff) override
    {
        ...
        if (m_checkCurrentVictimTimer < diff)
        {
            ...
            m_checkCurrentVictimTimer = 2 * IN_MILLISECONDS; // Check every 2 seconds that we still have
                                                             // the same target
        }
        else
...
// Select four random players plus the main tank and mind control them all.
// Evil.
struct ChainsKelThuzad : public SpellScript
{
    void OnEffectExecute(Spell *spell, SpellEffectIndex /* effIdx */) const override
    {
        ...
        // Prevent the mind control to happen if all remaining players would be
        // targeted
...
// Do damage onto the player equal to 26% of his/her full hit points on every
// tick
...
// Periodically inform the instance if the target is controlled through Shackle
// Undead or not
struct GuardianPeriodic : public AuraScript
{
    void OnPeriodicTrigger(Aura *aura, PeriodicTriggerData &data) const override
    {
        if (Unit *target = aura->GetTarget())
        {
            if (target->HasAuraType(SPELL_AURA_MOD_STUN)) // Shackle Undead applies
                                                          // SPELL_AURA_MOD_STUN, so we cover all
                                                          // ranks of the spell this way
