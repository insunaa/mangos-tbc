...
SDName: Boss_Cthun
SD%Complete: 95
SDComment: Transform spell has some minor core issues. Digestive Acid should be
handled in core. Spell to kill players in stomach on reset needs
implementation. SDCategory: Temple of Ahn'Qiraj EndScriptData */
...
enum
{
    ...
    SPELL_EYE_BEAM_INIT = 32950, // Cast three times on victim at encounter start,
                                 // then replaced by spell 26134 on random targets
    SPELL_DARK_GLARE = 26029,
    SPELL_ROTATE_TRIGGER = 26137, // phase switch spell - triggers 26009 or 26136.
                                  // These trigger the Dark Glare spell - 26029
    SPELL_ROTATE_360_LEFT = 26009,
    SPELL_ROTATE_360_RIGHT = 26136,

    SPELL_SUMMON_HOOK_TENTACLES_1 = 26397, // Periodically triggers spell 26398 which cast spell 26140 to
                                           // summon NPC 15725
    ...
    SPELL_GIANT_EYE_TENTACLES_1 = 26766, // Periodically triggers 26767 that cast 26768 on random target to
                                         // summon NPC 15334
    SPELL_GIANT_EYE_TENTACLES_2 = 26767,
    SPELL_SUMMON_GIANT_EYE_TENTACLE = 26768, // Summon NPC 15334
    SPELL_SUMMON_GIANT_HOOKS_1 = 26213,      // Periodically triggers 26217 that cast 26216 on random target to
                                             // summon NPC 15728
    SPELL_SUMMON_GIANT_HOOKS_2 = 26217,
    SPELL_SUMMON_GIANT_HOOK_TENTACLE = 26216, // Summon NPC 15728
    SPELL_SUMMON_MOUTH_TENTACLES_1 = 26236,   // Periodically triggers 26237 that cast 26332 on random target to
                                              // summon NPC 15910
    ...
    SPELL_EXIT_STOMACH_EFFECT = 26230,     // Complete the eject effect from the stomach towards a random
    ...
    SPELL_EXIT_STOMACH = 26221,            // Summon NPC 15800 that will handle the initial knockback outside
                                           // the stomach through spell 26224 (EventAI)

    NPC_WORLD_TRIGGER = 15384,

    DISPLAY_ID_CTHUN_BODY = 15786, // Helper display id; This is needed in order to have the proper
                                   // transform animation. ToDo: remove this when auras are fixed in
                                   // core.
...
struct boss_eye_of_cthunAI : public CombatAI
{
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case EYECTHUN_EYEBEAM:
            // The first three casts of "Eye Beam" use a specific spell that always
            // targets Eye of C'Thun's victim
            if (m_eyeBeamCount < 3)
            {
                if (m_creature->GetVictim())
                {
            ...
            // After the first three casts, random player is targeted with a second
            // specific spell
        ...
        case EYECTHUN_DARKGLARE:
            ...
            {
                // We ignore targets during Dark Glare to control Eye of C'Thun's
                // orientation ourselves in spells So we need custom AI management
                // for this sub phase
...
struct boss_cthunAI : public CombatAI
{
    ...
    {
        // There are 30 seconds offset between Giant Eye and Giant Claw Tentacles
        // which are both on a 60 seconds period handled by periodic spells
    ...
    {
        // Kill any player from the stomach on evade - this is becuase C'thun
        // cannot be soloed.
    ...
    void DoSpawnTentacles()
    {
        ...
        for (uint8 i = 0; i < MAX_FLESH_TENTACLES; ++i)
            m_creature->SummonCreature(NPC_FLESH_TENTACLE, cthunLocations[i][0], cthunLocations[i][1],
                                       cthunLocations[i][2], cthunLocations[i][3], TEMPSPAWN_DEAD_DESPAWN, 0);
    }
    ...
    {
        // Handle carapace and tentacles respawn when the vulnerability spell
        // expires
        DoCastSpellIfCan(m_creature, SPELL_CARAPACE_CTHUN, CAST_TRIGGERED | CAST_AURA_NOT_PRESENT);
        DoCastSpellIfCan(m_creature, SPELL_CHECK_RESET_AURA, CAST_TRIGGERED | CAST_AURA_NOT_PRESENT);
        DoSpawnTentacles();
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case CTHUN_CLAWTENTACLEDELAY:
            // The first Giant Claw Tentacle (at phase 2 start of after weakened
            // state) is a single isolated cast, all after are on periodic timer
            // through dedicated spell Check for valid player
                                                                 ...
                                                                 SELECT_FLAG_IN_LOS | SELECT_FLAG_PLAYER))
            {
                ...
                // All further summons are handled in periodic spell
                // SPELL_SUMMON_GIANT_HOOKS_1
                if (DoCastSpellIfCan(m_creature, SPELL_SUMMON_GIANT_HOOKS_1, CAST_TRIGGERED | CAST_AURA_NOT_PRESENT) ==
                    CAST_OK)
        ...
        case CTHUN_EYETENTACLEDELAY:
            // The first Giant Eye Tentacle (at phase 2 start of after weakened
            // state, 30 after first Giant Claw Tentacle) is a single isolated
            // cast, all after are on periodic timer through dedicated spell Check
            // for valid player
            if (Unit *player = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, uint32(0),
                                                                 SELECT_FLAG_IN_LOS | SELECT_FLAG_PLAYER))
            {
...
enum
{
    ...
    // SPELL_MASSIVE_GROUND_RUPTURE    = 26100,               // spell not
    // confirmed
...
struct npc_giant_claw_tentacleAI : public CombatAI
{
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case GIANTCLAW_CHECKTARGET:
            ...
            // If there is nobody in range, spawn a new tentacle at a new target
            // location
            if (!m_creature->SelectAttackingTarget(ATTACKING_TARGET_TOPAGGRO, 0, uint32(0),
                                                   SELECT_FLAG_IN_MELEE_RANGE) &&
                m_instance)
            {
                if (m_isLookingForMeleeTarget)
                {
                    m_creature->CastSpell(m_creature->GetVictim(), SPELL_TELEPORT_GIANT_HOOK_TRIGGER,
...
bool AreaTrigger_at_stomach_cthun(Player *player, AreaTriggerEntry const *at)
{
    ...
    if (at->id == AREATRIGGER_STOMACH_1)
    {
        ...
        // Summon the Exit Trigger NPC which will knockback the player outside
        // the stomach
    ...
    // Area trigger at the ceiling of C'Thun's stomach (exit or entrance,
    // depending on which way the "food" is going)
    ...
    {
        // We need to check for the presence of Digestive Acid aura on player so
        // the area trigger will only activate and knockback players coming from
        // C'Thun's stomach The knockback spell will remove the Digestive Acid
        // aura
...
struct ExitStomach : public SpellScript
{
    void OnEffectExecute(Spell *spell, SpellEffectIndex effIdx) const override
    {
        if (effIdx == EFFECT_INDEX_1)
        {
            if (Unit *target = spell->GetUnitTarget())
            {
                // Remove Digestive Acid aura when player is ejected from C'Thun's
                // stomach
...
struct DigestiveAcidPeriodic : public AuraScript
{
    void OnPeriodicCalculateAmount(Aura *aura, uint32 & /*amount*/) const override
    {
        if (aura->GetEffIndex() == EFFECT_INDEX_0)
        {
            if (Unit *target = aura->GetTarget())
            {
                // On every tick of the periodic damage, a new stack of Digestive
                // Acid is applied
                if (Unit *caster = aura->GetCaster())
