...
#define CHECK(exp)                                                                                                     \
    if (!(exp))                                                                                                        \
    {                                                                                                                  \
        sLog.outError("MoveSplineInitArgs::Validate: expression '%s' failed for %s", #exp,                             \
    ...
    if (time_passed >= next_timestamp())
    {
        ...
        else
        {
            if (spline.isCyclic())
            {
                ...
                // That point was the position of the unit prior to entering the
                // cycle and it shouldn't be repeated with continuous cycles.
                if (splineflags.enter_cycle)
                {
                    splineflags.enter_cycle = false;

                    MoveSplineInitArgs args{(size_t)spline.getPointCount()};
                    args.path.assign(spline.getPoints().begin() + spline.first() + 1,
                                     spline.getPoints().begin() + spline.last());
                    args.facing = facing;
                    args.flags = splineflags;
                    args.path_Idx_offset = point_Idx_offset;
                    // MoveSplineFlag::Parabolic | MoveSplineFlag::Animation not
                    // supported currently args.parabolic_amplitude = ?;
                    // args.time_perc = ?;
                    args.splineId = m_Id;
                    args.velocity = 1.0f; // Calculated below
                    if (args.Validate(nullptr))
                    {
                        // New cycle should preserve previous cycle's duration
                        // for some weird reason, even though the path is really
                        // different now. Developers are weird. Or this was just
                        // a simple oversight. Since our splines precalculate
                        // length with zspeed in mind, if we want to find the
                        // desired zspeed, we have to make a fake spline,
                        // calculate its duration and then compare it to the
                        // desired duration, thus finding out how much the zspeed
                        // has to be increased for them to match.
