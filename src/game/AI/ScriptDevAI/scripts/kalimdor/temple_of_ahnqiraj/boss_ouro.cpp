...
enum
{
    ...
    SPELL_DIRTMOUND_PASSIVE = 26092, // casts 26093 every 1 sec - removed from DBC (readded)
                                     // SPELL_SET_OURO_HEALTH   = 26075,                     // unused
                                     // - done through AI SPELL_SAVE_OURO_HEALTH  = 26076, // unused -
                                     // done through AI SPELL_TELEPORT_TRIGGER  = 26285, // unused
    ...
    // summoned npcs
    NPC_OURO = 15517,
    // NPC_OURO_SCARAB       = 15718,                       // summoned by Dirt
    // Mounds
...
struct boss_ouroAI : public CombatAI
{
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case OURO_SPAWN: {
            ...
            {
                if (Creature *trigger = GetClosestCreatureWithEntry(m_creature, NPC_OURO_TRIGGER,
                                                                    100.f)) // if not sufficient in the future, do using
                                                                            // SPELL_SET_OURO_HEALTH
        ...
        case OURO_BOULDER: {
            ...
            else
            {
                ...
                if (m_rangeCheckState > 1)
                    if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_BOULDER,
                                                                         SELECT_FLAG_PLAYER))
...
{
    npc_ouro_spawnerAI(Creature *creature) : Scripted_NoMovementAI(creature)
    {
        Reset();
    }
    ...
    void Aggro(Unit * /*who*/) override
    {
        if (!m_bHasSummoned)
        {
            m_creature->SummonCreature(NPC_OURO, m_creature->GetPositionX(), m_creature->GetPositionY(),
                                       m_creature->GetPositionZ(), m_creature->GetOrientation(), TEMPSPAWN_DEAD_DESPAWN,
                                       0, true);
            m_bHasSummoned = true;
        }
    }
