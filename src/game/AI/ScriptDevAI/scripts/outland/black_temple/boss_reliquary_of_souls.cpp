...
struct boss_reliquary_of_soulsAI : public ScriptedAI
{
    boss_reliquary_of_soulsAI(Creature *creature)
        : ScriptedAI(creature, 0), m_instance(static_cast<instance_black_temple *>(creature->GetInstanceData()))
    {
...
struct essence_base_AI : public CombatAI
{
    essence_base_AI(Creature *creature, uint32 maxActions)
        : CombatAI(creature, maxActions), m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData()))
    {
        ...
        m_creature->GetCombatManager().SetLeashingCheck(
            [&](Unit *, float x, float y, float z) { return x > 660.9f && y > 66.8902f; });
    }
...
struct boss_essence_of_angerAI : public CombatAI
{
    boss_essence_of_angerAI(Creature *creature)
        : CombatAI(creature, ANGER_ACTION_MAX), m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData()))
    {
        SetReactState(REACT_PASSIVE);
...
struct npc_enslaved_soulAI : public CombatAI
{
    npc_enslaved_soulAI(Creature *creature)
        : CombatAI(creature, 0), m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData())),
          m_postpone(false)
    {
        SetReactState(REACT_PASSIVE);
    ...
    void JustDied(Unit * /*killer*/) override
    {
        ...
        // Notify the main boss about the spirit death. Needs to be done here,
        // because the spirit is summoned with triggered spell
        if (m_instance)
        {
            if (Creature *reliquary = m_instance->GetSingleCreatureFromStorage(NPC_RELIQUARY_OF_SOULS))
            {
