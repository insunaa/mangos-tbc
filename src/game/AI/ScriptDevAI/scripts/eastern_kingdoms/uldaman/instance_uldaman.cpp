...
instance_uldaman::instance_uldaman(Map *pMap) : ScriptedInstance(pMap), m_uiKeeperCooldown(0), m_uiStoneKeepersFallen(0)
{
    Initialize();
}
...
void instance_uldaman::SetData(uint32 uiType, uint32 uiData)
{
    ...
    case TYPE_ALTAR_EVENT:
        ...
        {
            // Also do a reset before starting the event - this will respawn dead
            // Keepers
            DoResetKeeperEvent();
...
void instance_uldaman::DoResetKeeperEvent()
{
    if (m_lKeepers.empty())
    {
        script_error_log("Instance Uldaman: ERROR creature %u couldn't be found or "
...
void instance_uldaman::Update(uint32 uiDiff)
{
    ...
    if (m_uiKeeperCooldown <= uiDiff)
    {
        for (GuidList::const_iterator itr = m_lKeepers.begin(); itr != m_lKeepers.end(); ++itr)
        {
            ...
            {
                // If he's not available, then get a random player, within a
                // reasonamble distance in map
