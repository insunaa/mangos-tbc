...
enum QuestFlags
{
    ...
    QUEST_FLAGS_PARTY_ACCEPT = 0x00000002,     // If player in party, all players that can accept this quest
                                               // will receive confirmation box to accept quest
                                               // CMSG_QUEST_CONFIRM_ACCEPT/SMSG_QUEST_CONFIRM_ACCEPT
    QUEST_FLAGS_EXPLORATION = 0x00000004,      // Not used currently
    QUEST_FLAGS_SHARABLE = 0x00000008,         // Can be shared: Player::CanShareQuest()
                                               // QUEST_FLAGS_NONE2        = 0x00000010,
                                               // // Not used currently
    ...
    QUEST_FLAGS_NO_MONEY_FROM_XP = 0x00000100, // Not used currently: _DELIVER_MORE Quest needs more than
                                               // normal _q-item_ drops from mobs
    ...
    QUEST_FLAGS_AUTO_REWARDED = 0x00000400,    // These quests are automatically rewarded on quest complete
                                               // and they will never appear in quest log client side.
    QUEST_FLAGS_TBC_RACES = 0x00000800,        // Not used currently: Blood elf/Draenei starting zone quests
    QUEST_FLAGS_DAILY = 0x00001000,            // Daily quest. Can be done once a day. Quests
                                               // reset at regular intervals for all players.
    QUEST_FLAGS_FLAGS_PVP = 0x00002000,        // activates PvP on accept
    QUEST_FLAGS_UNAVAILABLE = 0x00004000,      // quests not available
    QUEST_FLAGS_WEEKLY = 0x00008000,           // Weekly quest. Can be done once a week. Quests reset at
                                               // regular intervals for all players.
};
...
enum QuestSpecialFlags
{
    ...
    QUEST_SPECIAL_FLAG_EXPLORATION_OR_EVENT = 0x002, // |2 in SpecialFlags from DB (if required area explore, spell
                                                     // SPELL_EFFECT_QUEST_COMPLETE casting, table `*_script` command
                                                     // SCRIPT_COMMAND_QUEST_EXPLORED use, set from script DLL)
    QUEST_SPECIAL_FLAG_MONTHLY = 0x004,              // |4 in SpecialFlags. Quest reset for
                                                     // player at beginning of month.
...
// (cached) quest details, all base quest information, and any utility
// functions such as generating the amount of xp to give
class Quest
{
  ...
  public:
    ...
    uint32 GetRequiredCondition() const
    {
    ...
    uint32 GetCharTitleBitIndex() const; // in 2.x in different from 3.x in some
                                         // quest packets used bit index insed id
    ...
    {
        return CompleteEmoteDelay;
    }
    uint32 GetDetailsEmoteCount() const
    {
        return m_detailsemotecount;
    }
    ...
    {
        return (m_SpecialFlags & QUEST_SPECIAL_FLAG_REPEATABLE) != 0;
    }
    ...
    bool IsDaily() const
    {
        return (m_QuestFlags & QUEST_FLAGS_DAILY) != 0;
    }
    bool IsWeekly() const
    {
        return (m_QuestFlags & QUEST_FLAGS_WEEKLY) != 0;
    }
    bool IsMonthly() const
    {
        return (m_SpecialFlags & QUEST_SPECIAL_FLAG_MONTHLY) != 0;
    }
    bool IsDailyOrWeekly() const
    {
        return (m_QuestFlags & (QUEST_FLAGS_DAILY | QUEST_FLAGS_WEEKLY)) != 0;
    }
