...
instance_zulfarrak::instance_zulfarrak(Map *pMap) : ScriptedInstance(pMap), m_uiPyramidEventTimer(0)
{
    Initialize();
}
...
void instance_zulfarrak::Update(uint32 uiDiff)
{
    if (m_uiPyramidEventTimer)
    {
        if (m_uiPyramidEventTimer <= uiDiff)
        {
            ...
            // Move the selected troll to the top of the pyramid. Note: the
            // algorythm may be more complicated than this, but for the moment
            // this will do.
            if (Creature *pTroll = instance->GetCreature(selectedGuid))
            {
                ...
                {
                    // ToDo: research if there is anything special if these guys
                    // die
                    if (!pBly->IsAlive())
                    {
                        m_uiPyramidEventTimer = 0;
                    ...
                    pBly->GetRandomPoint(pBly->GetPositionX(), pBly->GetPositionY(), pBly->GetPositionZ(), 4.0f, fX, fY,
                                         fZ);
