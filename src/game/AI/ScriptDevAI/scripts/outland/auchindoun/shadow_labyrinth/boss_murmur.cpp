...
SDName: Boss_Murmur
SD%Complete: 75
SDComment: Sonic Boom and Murmur's Touch require additional research and core
support SDCategory: Auchindoun, Shadow Labyrinth EndScriptData */
...
enum
{
    ...
    SPELL_MURMURS_TOUCH = 33711, // on expire silences the party members using shockwave - 33686 -
                                 // also related to spell 33760
...
static const uint32 SPELL_THUNDERING_STORM_MIN_TIMER = 6000;  // 15 seconds
                                                              // (Both)
            ...
            if (m_uiThunderingStormTimer < uiDiff)
            {
                                                                     ...
                                                                     m_thunderingParams))
                {
                    if (DoCastSpellIfCan(target, SPELL_THUNDERING_STORM) == CAST_OK)
                        m_uiThunderingStormTimer =
                            urand(SPELL_THUNDERING_STORM_MIN_TIMER, SPELL_THUNDERING_STORM_MAX_TIMER);
                }
            }
            else
