...
pAuraHandler AuraHandler[TOTAL_AURAS] = {
    ...
    &Aura::HandleNoImmediateEffect,                     // 15 SPELL_AURA_DAMAGE_SHIELD implemented
                                                        // in Unit::DealMeleeDamage
    &Aura::HandleModStealth,                            // 16 SPELL_AURA_MOD_STEALTH
    &Aura::HandleModStealthDetect,                      // 17 SPELL_AURA_MOD_STEALTH_DETECT
    &Aura::HandleInvisibility,                          // 18 SPELL_AURA_MOD_INVISIBILITY
    &Aura::HandleInvisibilityDetect,                    // 19
                                                        // SPELL_AURA_MOD_INVISIBILITY_DETECTION
    &Aura::HandleAuraModTotalHealthPercentRegen,        // 20
    ...
    &Aura::HandleNoImmediateEffect,                     // 28 SPELL_AURA_REFLECT_SPELLS implement
                                                        // in Unit::SpellHitResult
    ...
    &Aura::HandleNoImmediateEffect,                     // 75 SPELL_AURA_MOD_LANGUAGE implemented
                                                        // in WorldSession::HandleMessagechatOpcode
    ...
    &Aura::HandleNoImmediateEffect,                     // 94 SPELL_AURA_INTERRUPT_REGEN
                                                        // implemented in Player::RegenerateAll
    &Aura::HandleAuraGhost,                             // 95 SPELL_AURA_GHOST
    &Aura::HandleNoImmediateEffect,                     // 96 SPELL_AURA_SPELL_MAGNET implemented
                                                        // in Unit::SelectMagnetTarget
    ...
    &Aura::HandleOverrideClassScript,                   // 112 SPELL_AURA_OVERRIDE_CLASS_SCRIPTS
    ...
    &Aura::HandleNoImmediateEffect,                     // 114 SPELL_AURA_MOD_RANGED_DAMAGE_TAKEN_PCT
                                                        // implemented in Unit::MeleeDamageBonusTaken
    &Aura::HandleNoImmediateEffect,                     // 115 SPELL_AURA_MOD_HEALING implemented
                                                        // in Unit::SpellBaseHealingBonusTaken
    ...
    &Aura::HandleModHealingPCT,                         // 118 SPELL_AURA_MOD_HEALING_PCT implemented
    ...
    &Aura::HandleNoImmediateEffect,                     // 125 SPELL_AURA_MOD_MELEE_DAMAGE_TAKEN
                                                        // implemented in
                                                        // Unit::MeleeDamageBonusTaken
    &Aura::HandleNoImmediateEffect,                     // 126
    ...
    &Aura::HandleAuraModBaseResistancePercent,          // 142
    ...
    &Aura::HandleModMechanicImmunityMask,               // 147
                                                        ...
                                                        // implemented in Unit::IsImmuneToSpell
                                                        // and Unit::IsImmuneToSpellEffect
                                                        // (check part)
    ...
    &Aura::HandleNoImmediateEffect,                     // 159 SPELL_AURA_NO_PVP_CREDIT implemented
                                                        // in Player::RewardHonor
    &Aura::HandleNoImmediateEffect,                     // 160 SPELL_AURA_MOD_AOE_AVOIDANCE
                                                        // implemented in Unit::MagicSpellHitResult
    &Aura::HandleNoImmediateEffect,                     // 161
    ...
    &Aura::HandleUnused,                                // 165 SPELL_AURA_MELEE_ATTACK_POWER_ATTACKER_BONUS
                                                        // implemented in Unit::MeleeDamageBonusDone
    ...
    &Aura::HandleSpiritOfRedemption,                    // 176 SPELL_AURA_SPIRIT_OF_REDEMPTION
                                                        // only for Spirit of Redemption spell,
                                                        // die at aura end
    ...
    &Aura::HandleNoImmediateEffect,                     // 199
                                                        ...
                                                        // implemented in Unit::MagicSpellHitResult
    &Aura::HandleNoImmediateEffect,                     // 200 SPELL_AURA_MOD_XP_PCT implemented in
                                                        // Player::GiveXP
    &Aura::HandleAuraAllowFlight,                       // 201 SPELL_AURA_FLY this aura enable flight
                                                        // mode...
    ...
    &Aura::HandleNoImmediateEffect,                     // 213
    ...
    &Aura::HandlePrayerOfMending,                       // 225
    ...
    &Aura::HandleForceMoveForward,                      // 241 SPELL_AURA_FORCE_MOVE_FORWARD Forces
                                                        // the caster to move forward
    ...
    &Aura::HandleAuraMirrorImage,                       // 247 SPELL_AURA_MIRROR_IMAGE target to
                                                        // become a clone of the caster
    &Aura::HandleNoImmediateEffect,                     // 248 SPELL_AURA_MOD_COMBAT_RESULT_CHANCE
                                                        // implemented in
                                                        // Unit::CalculateEffectiveDodgeChance,
                                                        // Unit::CalculateEffectiveParryChance,
    ...
    &Aura::HandleAuraPhase                              // 261 SPELL_AURA_PHASE some phased state (44856
                                                        // spell)
};
...
Aura::Aura(SpellEntry const *spellproto, SpellEffectIndex eff, int32 const *currentDamage,
    ...
    : m_spellmod(nullptr), m_periodicTimer(0), m_periodicTick(0), m_removeMode(AURA_REMOVE_BY_DEFAULT), m_effIndex(eff),
      ...
      m_spellAuraHolder(holder), m_scriptValue(0), m_storage(nullptr)
{
    ...
    if (caster)
    {
        ...
        // scripting location for custom aura damage - needs to be moved to
        // spellscripting for proper checkcast interaction
        ...
        if (SpellEntry const *triggeredSpellInfo = sSpellTemplate.LookupEntry<SpellEntry>(triggeredSpellId))
            for (unsigned int i : triggeredSpellInfo->EffectImplicitTargetA)
                if (i == TARGET_UNIT_CHANNEL_TARGET)
                    aura = new SingleEnemyTargetAura(spellproto, eff, currentDamage, currentBasePoints, holder, target,
                                                     caster, castItem);
    }
...
void Aura::Update(uint32 diff)
{
    if (m_isPeriodic)
    {
        ...
        if (m_periodicTimer <= 0) // tick also at m_periodicTimer==0 to prevent lost last tick in
                                  // case max m_duration == (max m_periodicTimer)*N
        {
...
void AreaAura::Update(uint32 diff)
{
        ...
        if (!caster->hasUnitState(UNIT_STAT_ISOLATED))
        {
            ...
            case AREA_AURA_PARTY: {
                        ...
                        for (GroupReference *itr = pGroup->GetFirstMember(); itr != nullptr; itr = itr->next())
                        {
                                ...
                                caster->CanAssistSpell(Target, GetSpellProto()))
                            {
                                if (caster->IsWithinDistInMap(
                                        Target, m_radius, true,
                                        GetSpellProto()->HasAttribute(SPELL_ATTR_EX6_IGNORE_PHASE_SHIFT)))
                                    targets.push_back(Target);
                                Pet *pet = Target->GetPet();
                                if (pet && pet->IsAlive() &&
                                    caster->IsWithinDistInMap(
                                        pet, m_radius, true,
                                        GetSpellProto()->HasAttribute(SPELL_ATTR_EX6_IGNORE_PHASE_SHIFT)))
                ...
                else // handle aura party for creatures
                {
                    ...
                    {
                        // Due to the lack of support for NPC groups or
                        // formations, are considered of the same party NPCs
                        // with same faction than caster
                        if (target != owner && target->IsAlive() &&
                            target->GetFaction() == ((Creature *)owner)->GetFaction())
                            targets.push_back(target);
                    }
                }
                ...
                // add owner
                if (owner != caster &&
                    caster->IsWithinDistInMap(owner, m_radius, true,
                                              GetSpellProto()->HasAttribute(SPELL_ATTR_EX6_IGNORE_PHASE_SHIFT)))
                    targets.push_back(owner);
                // add caster's pet
                Unit *pet = caster->GetPet();
                if (pet && caster->IsWithinDistInMap(pet, m_radius, true,
                                                     GetSpellProto()->HasAttribute(SPELL_ATTR_EX6_IGNORE_PHASE_SHIFT)))
                    targets.push_back(pet);
            ...
            case AREA_AURA_FRIEND: {
                MaNGOS::AnyFriendlyUnitInObjectRangeCheck u_check(
                    caster, nullptr, m_radius, GetSpellProto()->HasAttribute(SPELL_ATTR_EX6_IGNORE_PHASE_SHIFT));
                MaNGOS::UnitListSearcher<MaNGOS::AnyFriendlyUnitInObjectRangeCheck> searcher(targets, u_check);
                Cell::VisitAllObjects(caster, searcher, m_radius);
                break;
            }
            case AREA_AURA_ENEMY: {
                MaNGOS::AnyAoETargetUnitInObjectRangeCheck u_check(
                    caster, nullptr, m_radius,
                    GetSpellProto()->HasAttribute(SPELL_ATTR_EX6_IGNORE_PHASE_SHIFT)); // No GetCharmer
                                                                                       // in searcher
            ...
            {
                // flag for selection is need apply aura to current iteration
                // target
                bool apply = true;

                SpellEntry const *actualSpellInfo;
                if (GetCasterGuid() == target->GetObjectGuid()) // if caster is same as target
                                                                // then no need to change rank
                                                                // of the spell
                    actualSpellInfo = GetSpellProto();
                else
                    actualSpellInfo = sSpellMgr.SelectAuraRankForLevel(GetSpellProto(),
                                                                       target->GetLevel()); // use spell id according
                                                                                            // level of the target
                if (!actualSpellInfo)
                    continue;
                ...
                for (Unit::SpellAuraHolderMap::const_iterator i = spair.first; i != spair.second; ++i)
                {
                    ...
                    case AREA_AURA_ENEMY:
                        ...
                        break;
                    case AREA_AURA_PARTY:
                        // non caster self-casted auras (stacked from diff.
                        // casters)
                        if (aur->GetModifier()->m_auraname != SPELL_AURA_NONE &&
                            i->second->GetCasterGuid() != GetCasterGuid())
                        {
                ...
                // Skip some targets (TODO: Might require better checks, also
                // unclear how the actual caster must/can be handled)
                ...
                // recalculate basepoints for lower rank (all AreaAura spell not
                // use custom basepoints?)
                if (actualSpellInfo != GetSpellProto())
                {
                    actualBasePoints = actualSpellInfo->CalculateSimpleValue(m_effIndex);
                    actualDamage =
                        caster->CalculateSpellEffectValue(target, actualSpellInfo, m_effIndex, &actualBasePoints);
                }
                ...
                AreaAura *aur = new AreaAura(actualSpellInfo, m_effIndex, &actualDamage, &actualBasePoints, holder,
                                             target, caster, nullptr, GetSpellProto()->Id);
    ...
    else // aura at non-caster
    {
        ...
        uint32 originalRankSpellId =
            m_originalRankSpellId ? m_originalRankSpellId : GetId(); // caster may have different spell id
                                                                     // if target has lower level
        ...
        if (!caster || caster->hasUnitState(UNIT_STAT_ISOLATED) ||
            !caster->HasAura(originalRankSpellId, GetEffIndex()) || !caster->IsWithinDistInMap(target, m_radius) ||
        ...
        else if (m_areaAuraType == AREA_AURA_PARTY) // check if in same sub group
        {
            ...
            // or if caster is a not player (as NPCs do not support group so aura
            // is only removed by moving out of range)
            if (caster->GetMasterGuid() != target->GetObjectGuid() &&
                caster->GetObjectGuid() != target->GetMasterGuid() && caster->IsPlayerControlled())
            {
...
bool Aura::CanProcFrom(SpellEntry const *spell, uint32 EventProcEx, uint32 procEx, bool active, bool useClassMask) const
{
    ...
    // SpellMgr::IsSpellProcEventCanTriggeredBy and it is done once for whole
    // holder note: SpellFamilyName is not checked if no spell_proc_event is
    // defined
...
void Aura::ReapplyAffectedPassiveAuras()
{
    ...
    // re-apply talents/passives/area auras applied to pet/totems (it affected by
    // player spellmods)
    GetTarget()->CallForAllControlledUnits(ReapplyAffectedPassiveAurasHelper(this), CONTROLLED_PET | CONTROLLED_TOTEMS);

    // re-apply talents/passives/area auras applied to group members (it affected
    // by player spellmods)
                                      ...
                                      SpellEntry const *spellInfo)
{
    ...
    case TARGET_UNIT_CHANNEL_TARGET: // Electrified net
        ...
        triggerTarget = triggerCaster->GetTarget(); // This will default to channel target
                                                    // for channels
        break;
    ...
    case TARGET_LOCATION_CASTER_SRC: // TODO: this needs to be done whenever
                                     // target isnt important, doing it per
                                     // case for safety
                                     //[[fallthrough]]
...
void Aura::CastTriggeredSpell(PeriodicTriggerData &data)
{
    ...
    if (data.spellInfo->Targets & TARGET_FLAG_DEST_LOCATION)
    {
        if (data.targetObject)
            ...
            targets.setDestination(data.target->GetPositionX(), data.target->GetPositionY(),
                                   data.target->GetPositionZ());
        else
            targets.setDestination(data.caster->GetPositionX(), data.caster->GetPositionY(),
                                   data.caster->GetPositionZ());
    }
...
void Aura::HandleAddModifier(bool apply, bool Real)
{
    ...
    if (apply)
    {
        ...
        case 34936: // Backlash
            ...
            break;
        case 16246: // Clearcasting - Shaman - need to be applied before
                    // elemental mastery
            priority = 100;
            break;
        }
        ...
        m_spellmod = new SpellModifier(SpellModOp(m_modifier.m_miscvalue),
                                       ...
                                       m_modifier.m_amount, this, priority,
...
void Aura::TriggerSpell()
{
    ...
    if (triggeredSpellInfo == nullptr)
    {
        ...
        case SPELLFAMILY_GENERIC: {
            ...
            {
                // Only 10% chance of triggering spell, return for the
                // remaining 90%
                if (urand(0, 9) >= 1)
                    return;
            ...
            case 24780: // Dream Fog
            {
                ...
                // script dummy effect for this spell then Select an
                // unfriendly enemy in 100y range and attack it
            ...
            case 24834: // Shadow Bolt Whirl
            {
                ...
                triggerTarget->CastSpell(triggerTarget, spellForTick[tick], TRIGGERED_OLD_TRIGGERED, nullptr, this,
                                         casterGUID);
            ...
            case 25371: // Consume
            {
                ...
                //                    // Teleport: IF/UC
                //                    case 27601: break;
                //                    // Five Fat Finger Exploding Heart
                //                    Technique case 27673: break;
            ...
            case 27819: {
                ...
                triggerTarget->CastCustomSpell(triggerTarget, 27820, &bpDamage, nullptr, nullptr,
                                               TRIGGERED_OLD_TRIGGERED, nullptr, this, triggerTarget->GetObjectGuid());
                return;
            }
                //                    // Controller Timer
                //                    case 28095: break;
            ...
            case 30427: // Extract Gas
            {
                    ...
                    ((Creature *)triggerTarget)->GetCreatureInfo()->CreatureType == CREATURE_TYPE_GAS_CLOUD)
                {
                    ...
                    // missing lootid has been reported on startup - just
                    // return
            ...
            case 31611: // Bloodmyst Tesla
                // no custom effect required; return to avoid spamming with
                // errors
                return;
            case 31944: // Doomfire
            {
                int32 damage =
                    m_modifier.m_amount * ((GetAuraDuration() + m_modifier.periodictime) / GetAuraMaxDuration());
            ...
            case 37268: // Arcane Flurry (Melee Component)
            {
                ...
                //                    // Karazhan - Chess NPC AI, Snapshot
                //                    timer case 37440: break;
                        ...
                        // Beacon item)
            {
                ...
                //                    case 39140: break;
                //                    // Quest - Dustin's Undead Dragon Visual
                //                    aura case 39259: break;
                //                    // Hellfire - The Exorcism, Jules
                //                    releases darkness, aura case 39306:
                //                    break;
                //                    // Enchanted Weapons
                //                    case 39489: break;
                //                    // Shadow Bolt Whirl
            ...
            case 39645: {
                ...
                triggerTarget->CastCustomSpell(triggerTarget, 39646, &damageForTick[GetAuraTicks() - 1], nullptr,
                                               nullptr, TRIGGERED_OLD_TRIGGERED, nullptr);
                return;
            }
            break;
            case 39857: // Tear of Azzinoth Summon Channel - it's not really
                ...
                //                    Aura case 40113: break;
            case 40157: // Spirit Lance - Teron Gorefiend - Remove stack on
                        // tick
            ...
            case 40867: // Random Periodic
            {
                ...
                if (target->GetTypeId() == TYPEID_UNIT)
                    triggerTarget = static_cast<Creature *>(target)->SelectAttackingTarget(
                        ATTACKING_TARGET_RANDOM, 0, trigger_spell_id, SELECT_FLAG_PLAYER);
                return;
            }
                //                    // Prismatic Shield
                //                    case 40879: break;
        ...
        case SPELLFAMILY_DRUID: {
            ...
            {
            case 768: // Cat Form
                // trigger_spell_id not set and unknown effect triggered in
                // this case, ignoring for while
                return;
            ...
            case 26999: {
                ...
                target->CastCustomSpell(target, 22845, &FRTriggerBasePoints, nullptr, nullptr, TRIGGERED_OLD_TRIGGERED,
                                        nullptr, this);
                return;
            }
            default:
            ...
            //                    case 30648:         // Tame Greater
            //                    Timberstrider case 30652:         // Tame
            //                    Nightstalker
            //                        return;
            //                    default:
            //                        break;
    ...
    else
    {
        ...
        // (persistent aura affects already applied to true target) come periodic
        // casts applied to targets, so need select proper caster (ex. 15790)
        ...
        case 9347: // Mortal Strike
        {
            ...
            triggerTarget =
                ((Creature *)target)->SelectAttackingTarget(ATTACKING_TARGET_TOPAGGRO, 0, triggeredSpellInfo);
            if (!triggerTarget)
                return;
        ...
        /*case 30502:                                   // Dark Spin - Only
        Effect0 s.30505 should be affect, else s.30508 doesnt work anymore
        {
            if (GetCaster()->GetTypeId() != TYPEID_UNIT)
                return;

            triggerTarget =
        ((Creature*)GetCaster())->SelectAttackingTarget(ATTACKING_TARGET_RANDOM,
        0, trigger_spell_id, SELECT_FLAG_PLAYER); if (!triggerTarget) return;
        ...
        case 39091: // Negative Charge
        {
            ...
            GetPlayerListWithEntryInWorld(playerList, target,
        ...
        {
            // Need to provide explicit target for trigger spell target
            // combination
    ...
    else if (!GetAuraScript()) // if scripter scripted spell, it is handled
                               // somehow
    {
        if (Unit *caster = GetCaster())
        {
            if (triggerTarget->GetTypeId() != TYPEID_UNIT ||
                ...
                sLog.outError("Aura::TriggerSpell: Spell %u have 0 in "
...
void Aura::TriggerSpellWithValue()
{
    ...
    // damage triggered from spell might not only be processed by first effect
    // (but always EffectDieSides equal 1)
...
void Aura::HandleAuraDummy(bool apply, bool Real)
{
    ...
    if (apply)
    {
        ...
        case SPELLFAMILY_GENERIC: {
            ...
            case 13139: // net-o-matic
                // root to self part of (root_target->charge->root_self
                // sequence
                if (Unit *caster = GetCaster())
            ...
            {
                // expected to tick with 5 sec period (tick part see in
                // Aura::PeriodicTick)
                m_isPeriodic = true;
                m_modifier.periodictime = 5 * IN_MILLISECONDS;
                m_periodicTimer = m_modifier.periodictime;
            ...
            case 28835: // Mark of Zeliek
            {
                ...
                if (Unit *caster = GetCaster())
                    caster->CastCustomSpell(target, 28836, &damage, nullptr, nullptr, TRIGGERED_OLD_TRIGGERED, nullptr,
                                            this);
                return;
            }
            ...
            case 32051:
            case 32052: {
                // TODO: Controls the random timer for the spell to be cast
                // based on aura, they need to be split up.
                return;
            }
            ...
            {
                // Possibly need some of the below to fix Vengeful Harbinger
                // flying

                // if (Unit* caster = GetCaster())
                //{
                //     caster->SetByteValue(UNIT_FIELD_BYTES_1,
                //     UNIT_BYTES_1_OFFSET_MISC_FLAGS,
                //     UNIT_BYTE1_FLAG_FLY_ANIM);
                //     caster->RemoveByteFlag(UNIT_FIELD_BYTES_1,
                //     UNIT_BYTES_1_OFFSET_MISC_FLAGS,
                //     UNIT_BYTE1_FLAG_ALWAYS_STAND); caster->SetHover(true);
            ...
            case 40856: // Wrangling Rope
            {
                ...
                if (Unit *caster = GetCaster())
                    target->CastSpell(caster, 40917,
                                      TRIGGERED_NONE); // Wrangle Aether Rays:
                                                       // Character Force Cast
        ...
        case SPELLFAMILY_WARRIOR: {
            switch (GetId())
            {
            case 23427: // Summon Infernals (Warlock class call in Nefarian
                        // encounter)
            {
                if (Unit *target = GetTarget())
    ...
    else
    {
        ...
        case 11826: // Electromagnetic Gigaflux Reactivator
            ...
            if (Unit *caster = GetCaster())
            {
                if (caster->GetTypeId() == TYPEID_PLAYER)
                    caster->CastSpell(target, 11828, TRIGGERED_OLD_TRIGGERED,
                                      ((Player *)caster)->GetItemByGuid(this->GetCastItemGuid()), this);
            }
            return;
        ...
        case 17189: // Frostwhisper's Lifeblood
            // Ras Frostwhisper gets back to full health when turned to his human
            // form
            if (Unit *caster = GetCaster())
        ...
        case 35176: // Interrupt shutdown (ara)
        {
            ...
            {
                Unit *caster = GetCaster();
                if (caster && GetAuraDuration() <= 100) // only fail if finished cast (seems to
                                                        // finish with .1 seconds left)
        ...
        case 42517: // Beam to Zelfrax
        {
            ...
            target->SummonCreature(23864, -2985.01f, -3853.649f, 44.23183f, 5.51524f,
    ...
    case SPELLFAMILY_GENERIC: {
        ...
        case 37493: // Feign Death
        {
            ...
            // Some appear to be used depending on creature location, in
            // water, at solid ground, in air/suspended, etc For now, just
            // handle all the same way if (target->GetTypeId() ==
            // TYPEID_UNIT)
        ...
        case 37025: // Coilfang Water
        {
            if (apply)
            {
                if (InstanceData *pInst = target->GetInstanceData())
                {
                    ...
                    {
                        if (pInst->CheckConditionCriteriaMeet(playerTarget, INSTANCE_CONDITION_ID_SCALDING_WATER,
                                                              nullptr, CONDITION_FROM_HARDCODED))
                            playerTarget->CastSpell(playerTarget, 37284, TRIGGERED_OLD_TRIGGERED);
                        else
                        {
                            m_isPeriodic = true;
                            m_modifier.periodictime = 2 * IN_MILLISECONDS; // Summons Coilfang
                                                                           // Frenzy
                            m_periodicTimer = 0;
                        }
                    }
        ...
        {
            // aura animate dead (fainted) state for the duration, but we
            // need to animate the death itself (correct way below?)
            if (Unit *pCaster = GetCaster())
        ...
        if (GetSpellProto()->SpellFamilyFlags & uint64(0x1000000000))
        {
                ...
                if (Unit *caster = GetCaster())
                {
                    ...
                    if (target->GetTypeId() != TYPEID_PLAYER || !((Player *)target)->GetSession()->PlayerLoading())
                    {
                        ...
                        m_modifier.m_amount = caster->SpellHealingBonusDone(target, GetSpellProto(),
                                                                            m_modifier.m_amount, SPELL_DIRECT_DAMAGE);
            ...
            else
            {
                ...
                if (target->IsInWorld())
                {
                    ...
                    target->CastCustomSpell(nullptr, 33778, &amount, nullptr, nullptr, TRIGGERED_OLD_TRIGGERED, nullptr,
                                            this, GetCasterGuid());
                }
            }
            return;
    ...
    // script has to "handle with care", only use where data are not ok to use in
    // the above code.
    if (target->GetTypeId() == TYPEID_UNIT)
...
void Aura::HandleAuraMounted(bool apply, bool Real)
{
    ...
    if (apply)
    {
        ...
        if (!ci)
        {
            sLog.outErrorDb("AuraMounted: `creature_template`='%u' not found in "
                            "database (only need it modelid)",
                            m_modifier.m_miscvalue);
            return;
        }
...
void Aura::HandleAuraModShapeshift(bool apply, bool Real)
{
    ...
    if (ssEntry->modelID_A)
    {
        ...
        // the dbc since no field in creature_templates describes wether an
        // alliance or horde modelid should be used at shapeshifting
        if (target->GetTypeId() != TYPEID_PLAYER)
    ...
    case FORM_MOONKIN: {
        ...
        for (Unit::AuraList::const_iterator iter = slowingAuras.begin(); iter != slowingAuras.end();)
        {
            ...
            if (aurMechMask == 0) // all shapeshift removing spells have some mechanic
                                  // mask. 33572 and 38132 confirmed not removed on poly
            {
                ++iter;
                continue;
            }
            ...
            if ((aurMechMask & MECHANIC_NOT_REMOVED_BY_SHAPESHIFT) ||
                // some Daze spells have these parameters instead of
                // MECHANIC_DAZE (skip snare spells)
        ...
        target->RemoveAurasDueToSpell(16591); // Patch 2.0.1 - Shapeshifting removes
                                              // Noggenfogger elixir no break here
    }
    case FORM_GHOSTWOLF: {
        // remove water walk aura. TODO:: there is probably better way to do
        // this
                ...
                for (int8 x = 0; x < MAX_SPELL_SCHOOL; ++x)
                    if (threatAura->GetModifier()->m_miscvalue & int32(1 << x))
                        ApplyPercentModFloatVar(target->m_threatModifier[x],
                                                float(threatAura->GetModifier()->m_baseAmount), apply);
            }
        }
        break;
    }
    ...
    if (apply)
    {
        ...
        // clientside process.. serverside just needs to register the new spells
        // so that player isn't kicked as cheater
        if (target->GetTypeId() == TYPEID_PLAYER)
    ...
    if (apply)
    {
        switch (form)
        {
        case FORM_CAT: // need to cast Track Humanoids if no other tracking is
                       // on
    ...
    if (apply)
    {
        ...
        if (m_modifier.m_miscvalue == 0)
        {
            ...
            case 42365: // Murloc costume
                ...
                break;
            // case 44186:                          // Gossip NPC Appearance -
            // All, Brewfest break; case 48305:                          //
            // Gossip NPC Appearance - All, Spirit of Competition break;
            ...
            // Appearance - All, Pirate Day break; case 51010: // Dire Brew
            // break;
            default:
                if (!m_modifier.m_amount) // can be set through script
                    sLog.outError("Aura::HandleAuraTransform, spell %u does not have "
        ...
        else // m_modifier.m_amount != 0
        {
            ...
            if (!cInfo)
            {
                ...
                sLog.outError("Auras: unknown creature id = %d (only need its modelid) "
...
void Aura::HandleChannelDeathItem(bool apply, bool Real)
{
    if (Real && !apply)
    {
        ...
        if (itemType == 6265)
        {
            ...
            // 1.11.0: If you cast Drain Soul while shadowburn is on the victim,
            // you will no longer receive two soul shards upon the victim's
            // death.
...
void Aura::HandleModPossess(bool apply, bool Real)
{
    ...
    Unit *caster = GetCaster();
    if (!caster || caster->GetTypeId() != TYPEID_PLAYER) // TODO:: well i know some bosses
                                                         // can take control of player???
        return;
    ...
    if (apply)
    {
        ...
        // Possess: advertised type of charm (unique) - remove existing
        // advertised charm
    ...
    if (const SpellEntry *spellInfo = GetSpellProto())
    {
        ...
        case 24937: // Using Control Console
            if (!apply)
            {
                caster->CastSpell(caster, 24938,
                                  TRIGGERED_OLD_TRIGGERED); // Close Control Console
                caster->CastSpell(caster, 27880,
                                  TRIGGERED_OLD_TRIGGERED); // Stun
                if (target->GetTypeId() == TYPEID_UNIT)
                {
                    target->CastSpell(target, 3617,
                                      TRIGGERED_OLD_TRIGGERED); // Quiet Suicide
        ...
        case 37895:
            ...
            break;
        case 37748: // Teron Gorefiend - remove aura from caster when posses is
                    // removed
            if (!apply)
            {
                static_cast<Creature *>(target)->ForcedDespawn();
            ...
            break;
        case 40268: // Spiritual Vengeance - Teron Gorefiend - kill player and
                    // despawn spirit
            if (!apply)
            {
...
void Aura::HandleModPossessPet(bool apply, bool Real)
{
    ...
    if (apply)
    {
        // Possess pet: advertised type of charm (unique) - remove existing
        // advertised charm
        caster->BreakCharmOutgoing(true);
...
void Aura::HandleModCharm(bool apply, bool Real)
{
    ...
    if (apply)
    {
        // Charm: normally advertised type of charm (unique), but with notable
        // exceptions:
        // * Seems to be non-unique for NPCs - allows overwriting advertised
        // charm by offloading existing one (e.g. Chromatic Mutation)
        // * Seems to be always unique for players - remove player's existing
        // advertised charm (no evidence against this found yet)
        if (playerCaster)
            caster->BreakCharmOutgoing(true);
...
void Aura::HandleFeignDeath(bool apply, bool Real)
{
    ...
    if (apply)
    {
        ...
        {
            // Players and player-controlled units do an additional success roll
            // for this aura on application
...
void Aura::HandleAuraModDisarm(bool apply, bool Real)
{
    ...
    // main-hand attack speed already set to special value for feral form already
    // and don't must change and reset at remove.
...
void Aura::HandleAuraModStun(bool apply, bool Real)
{
    ...
    if (apply)
    {
        ...
        case 32890: // Knockout - OHF
            ...
            break;
        case 39837: // Summon the Naj'entus Spine GameObject on target if spell
                    // is Impaling Spine
    ...
    else
    {
        ...
        case 27068: {
            ...
            case 27068:
                ...
                break;
            default:
                sLog.outError("Spell selection called for unexpected original spell "
                              "%u, new spell for this spell family?",
                              GetId());
                return;
            }
...
void Aura::HandleInvisibility(bool apply, bool Real)
{
    ...
    if (trueApply)
    {
        ...
        if (Real && target->GetTypeId() == TYPEID_PLAYER &&
            GetId() != 32727) // avoid changing this for Arena Preparation spell -
                              // it seems this should not cause the glow vision
        {
            if (((Player *)target)->IsSelfMover()) // check if the player doesnt have a mover,
                                                   // when player is hidden during MC of
                                                   // creature
            {
                // apply glow vision
...
// this method is called whenever we add / remove aura which gives m_target
// some imunity to some spell effect
void Aura::HandleAuraModEffectImmunity(bool apply, bool /*Real*/)
{
    ...
    if (target->GetTypeId() == TYPEID_PLAYER &&
        (GetSpellProto()->AuraInterruptFlags & AURA_INTERRUPT_FLAG_INVULNERABILITY_BUFF_CANCELS))
    {
        Player *player = static_cast<Player *>(target);
    ...
    {
        // Handle OPVP script condition change on aura apply; Specific for
...
void Aura::HandleAuraModSchoolImmunity(bool apply, bool Real)
{
    ...
    if (Real && apply && GetSpellProto()->HasAttribute(SPELL_ATTR_EX_DISPEL_AURAS_ON_IMMUNITY) &&
        IsPositiveSpell(GetId(), GetCaster(),
                        target)) // Only positive immunity removes auras
    {
        ...
        for (Unit::SpellAuraHolderMap::iterator iter = Auras.begin(), next; iter != Auras.end(); iter = next)
        {
            ...
            if ((GetSpellSchoolMask(spell) & school_mask)         // Check for school mask
                && !spell->HasAttribute(SPELL_ATTR_NO_IMMUNITIES) // Spells unaffected by
                                                                  // invulnerability
...
void Aura::HandleAuraProcTriggerSpell(bool apply, bool Real)
{
    ...
    case 39057: // Serpentshrine Parasite - always hit with melee
        ...
        break;
    case 40594: // Felrage - Gurtogg Bloodboil - remove Fury on removal of
                // Felrage
        if (!apply)
...
void Aura::HandlePeriodicTriggerSpell(bool apply, bool /*Real*/)
{
    ...
    if (apply)
    {
        ...
        case 43648: // Electrical Storm - Akil'zon - cast teleport self
            target->CastSpell(nullptr, 44006,
                              TRIGGERED_OLD_TRIGGERED); // done here instead of script effect
                                                        // due to auras being applied after
                                                        // effects
            break;
        default:
            break;
    ...
    else
    {
        ...
        case 23620: // Burning Adrenaline (Caster version)
            // On aura removal, the target deals AoE damage to friendlies and
            // kills himself/herself (prevent durability loss)
        ...
        case 38794:
            target->CastSpell(nullptr, 33686, TRIGGERED_OLD_TRIGGERED, nullptr,
                              this); // cast Shockwave
            target->CastSpell(nullptr, 33673,
                              TRIGGERED_OLD_TRIGGERED); // cast Shockwave knockup serverside
            return;
...
void Aura::HandlePeriodicHeal(bool apply, bool /*Real*/)
{
    ...
    if (apply)
    {
        ...
        {
        case 12939:
            m_modifier.m_amount = target->GetMaxHealth() / 10;
...
void Aura::HandlePeriodicDamage(bool apply, bool Real)
{
    ...
    if (apply)
    {
        ...
        case SPELLFAMILY_ROGUE: {
            ...
            if (spellProto->SpellFamilyFlags & uint64(0x000000000000100000))
            {
                if (caster->GetTypeId() != TYPEID_PLAYER)
                    break;
                // Dmg/tick = $AP*min(0.01*$cp, 0.03) [Like Rip: only the first
                // three CP increase the contribution from AP]
                uint8 cp = caster->GetComboPoints();
                if (cp > 3)
                    cp = 3;
                m_modifier.m_amount += int32(caster->GetTotalAttackPowerValue(BASE_ATTACK) * cp / 100);
        ...
        if (m_modifier.m_auraname == SPELL_AURA_PERIODIC_DAMAGE)
        {
            ...
            else
            {
                ...
                m_modifier.m_amount = caster->MeleeDamageBonusDone(target, m_modifier.m_amount, attackType,
                                                                   SpellSchoolMask(spellProto->SchoolMask), spellProto,
                                                                   DOT, GetStackAmount());
            }
        }
    }
    ...
    else
    {
        ...
        case 23155: // Chromaggus - Brood Affliction: Red
            if (m_removeMode == AURA_REMOVE_BY_DEATH)
                target->CastSpell(nullptr, 23168,
                                  TRIGGERED_OLD_TRIGGERED); // Heal Chromaggus
            break;
        case 43364: // Tranquilizing Poison
            if (m_removeMode == AURA_REMOVE_BY_EXPIRE)
                target->CastSpell(nullptr, 24004,
                                  TRIGGERED_OLD_TRIGGERED); // Sleep
            break;
        case 35201: // Paralytic Poison
            if (m_removeMode == AURA_REMOVE_BY_DEFAULT)
                target->CastSpell(target, 35202,
                                  TRIGGERED_OLD_TRIGGERED); // Paralysis
            break;
        case 39044: // Serpentshrine Parasite
        case 39053:
            if (m_removeMode == AURA_REMOVE_BY_DEATH && target->GetTypeId() == TYPEID_PLAYER)
                target->CastSpell(target, 39045,
                                  TRIGGERED_OLD_TRIGGERED); // Summon Serpentshrine Parasite
            break;
        }
    }
}
...
void Aura::HandlePeriodicManaLeech(bool apply, bool /*Real*/)
{
    ...
    if (!apply)
    {
        ...
        case 24003: // Tranquilizing Poison
            if (m_removeMode == AURA_REMOVE_BY_EXPIRE)
                GetTarget()->CastSpell(nullptr, 24004,
                                       TRIGGERED_OLD_TRIGGERED); // Sleep
            break;
        }
    }
...
void Aura::HandleAuraModResistanceExclusive(bool apply, bool /*Real*/)
{
    ...
    for (uint32 i = SPELL_SCHOOL_NORMAL; i < MAX_SPELL_SCHOOL; ++i)
    {
        if (m_modifier.m_miscvalue & (1 << i))
        {
            ...
            // UI malus info:
            target->ApplyResistanceBuffModsMod(SpellSchools(i), (m_modifier.m_amount > 0), apply ? change : -change,
                                               apply);
        }
    }
}
...
void Aura::HandleAuraModBaseResistancePercent(bool apply, bool /*Real*/)
{
    ...
    {
        if (m_modifier.m_miscvalue & (1 << i))
            target->HandleStatModifier(UnitMods(UNIT_MOD_RESISTANCE_START + i), BASE_PCT, float(m_modifier.m_amount),
                                       apply);
    }
}
...
void Aura::HandleAuraModStat(bool apply, bool /*Real*/)
{
    ...
    {
        sLog.outError("WARNING: Spell %u effect %u have unsupported misc value "
                      "(%i) for SPELL_AURA_MOD_STAT ",
    ...
    // Holy Strength amount decrease by 4% each level after 60 From Crusader
    // Enchant
    if (apply && GetId() == 20007)
        if (GetCaster()->GetTypeId() == TYPEID_PLAYER && GetCaster()->GetLevel() > 60)
            m_modifier.m_amount =
                int32(m_modifier.m_amount * (1 - (((float(GetCaster()->GetLevel()) - 60) * 4) / 100)));
    ...
    if (GetSpellProto()->IsFitToFamilyMask(0x0000000000008000)) // improved scorpid sting
    {
        if (apply)
        {
            ...
            if (staminaToRemove)
                GetCaster()->CastCustomSpell(target, 19486, &staminaToRemove, nullptr, nullptr,
                                             TRIGGERED_OLD_TRIGGERED);
        }
        else
            target->RemoveAurasTriggeredBySpell(GetId(), GetCasterGuid()); // just do it every time, lookup is
                                                                           // too time consuming
    }
    ...
    for (int32 i = STAT_STRENGTH; i < MAX_STATS; ++i)
    {
        ...
        {
            float amount = m_removeMode == AURA_REMOVE_BY_GAINED_STACK ? float(m_modifier.m_recentAmount)
...
void Aura::HandleAuraModResistenceOfStatPercent(bool /*apply*/, bool /*Real*/)
{
    ...
    if (m_modifier.m_miscvalue != SPELL_SCHOOL_MASK_NORMAL)
    {
        ...
        sLog.outError("Aura SPELL_AURA_MOD_RESISTANCE_OF_STAT_PERCENT(182) need "
                      "adding support for non-armor resistances!");
        return;
    }
...
void Aura::HandleAuraModIncreaseHealth(bool apply, bool Real)
{
    ...
    // Cases where we need to manually calculate the amount for the spell (by
    // percentage) recalculate to full amount at apply for proper remove

    // Cases where m_amount already has the correct value (spells cast with
    // CastCustomSpell or absolute values)
    case 4148:  // Growl of Fortitude
    case 12976: // Warrior Last Stand triggered spell (Cast with
                // percentage-value by CastCustomSpell)
    ...
    case 39625: // Elixir of Major Fortitude (i.32062 - Elixir of Major
                // Fortitude)
...
void Aura::HandleAuraModCritPercent(bool apply, bool Real)
{
    ...
    // EquippedItemInventoryTypeMask m_modifier.m_miscvalue comparison with item
...
void Aura::HandleModDamagePercentDone(bool apply, bool Real)
{
    ...
    // 1 ( 0-bit ) - normal school damage (SPELL_SCHOOL_MASK_NORMAL)
    // 126 - full bitmask all magic damages (SPELL_SCHOOL_MASK_MAGIC) including
    // wand 127 - full bitmask any damages
    //
    // mods must be applied base at equipped weapon class and subclass comparison
    // with spell->EquippedItemClass and  EquippedItemSubClassMask and
    // EquippedItemInventoryTypeMask m_modifier.m_miscvalue comparison with item
    ...
    if (target->GetTypeId() == TYPEID_PLAYER)
        for (int i = SPELL_SCHOOL_HOLY; i < MAX_SPELL_SCHOOL; ++i)
            if (m_modifier.m_miscvalue & (1 << i))
                target->ApplyModSignedFloatValue(PLAYER_FIELD_MOD_DAMAGE_DONE_PCT + i, m_modifier.m_amount / 100.0f,
                                                 apply);
            ...
            if (HotWSpellId)
            {
                ...
                for (auto i : mModTotalStatPct)
                {
                    if (i->GetSpellProto()->SpellIconID == 240 && i->GetModifier()->m_miscvalue == 3)
                    {
                        ...
                        target->CastCustomSpell(nullptr, HotWSpellId, &HotWMod, nullptr, nullptr,
                                                TRIGGERED_OLD_TRIGGERED, nullptr, this);
                        break;
                    }
                }
    ...
    if (apply)
    {
        if (Unit *caster = GetCaster())
        {
            if (Player *playerCaster = caster->IsPlayer() ? static_cast<Player *>(caster) : nullptr)
            {
                ...
                target->BuildValuesUpdateBlockForPlayerWithFlags(
                    updateData, playerCaster, UpdateFieldFlags(UF_FLAG_SPECIAL_INFO | UF_FLAG_DYNAMIC));
...
void Aura::HandleModTargetResistance(bool apply, bool Real)
{
    ...
    if (!Real)
        return;
    Unit *target = GetTarget();
    // applied to damage as HandleNoImmediateEffect in
    // Unit::CalculateAbsorbAndResist and Unit::CalcArmorReducedDamage show armor
    // penetration
...
void Aura::HandleSpiritOfRedemption(bool apply, bool Real)
{
    ...
    target->ApplySpellImmune(this, IMMUNITY_DAMAGE, 255,
                             apply); // wotlk style hack - immunity against all damage
...
{
    Unit *target = GetTarget();
    // passive periodic trigger spells should not be updated when dead, only
    // death persistent should
    ...
    case SPELL_AURA_PERIODIC_DAMAGE_PERCENT: {
        ...
        // some auras remove at specific health level or more or have damage
        // interactions
        ...
        case 41337: // Aura of Anger
        {
            ...
            // during normal immunities - ticks, only doesnt tick during
            // spite
        ...
        if (spellProto->Effect[GetEffIndex()] == SPELL_EFFECT_PERSISTENT_AREA_AURA) // safe case - caster always
                                                                                    // will exist
        {
            if (!caster)
            {
        ...
        // Aura of anger - video evidence confirms this, but attribute is legit
        // because aura is still applied during
        if (!spellProto->HasAttribute(SPELL_ATTR_NO_IMMUNITIES) || overrideImmune) // confirmed Impaling spine goes
                                                                                   // through immunity
        {
        ...
        else
        {
            ...
            pdamage =
                target->MeleeDamageBonusTaken(caster, pdamage, attackType, SpellSchoolMask(spellProto->SchoolMask),
        ...
        Unit::ProcDamageAndSpell(ProcSystemArguments(caster, target, procAttacker, procVictim, PROC_EX_NORMAL_HIT,
                                                     pdamage, BASE_ATTACK, spellProto));
    ...
    case SPELL_AURA_PERIODIC_HEALTH_FUNNEL: {
        ...
        DETAIL_FILTER_LOG(LOG_FILTER_PERIODIC_AFFECTS,
                          "PeriodicTick: %s health leech of %s for %u dmg "
        ...
        Unit::SendSpellNonMeleeDamageLog(pCaster, target, GetId(), pdamage, GetSpellSchoolMask(spellProto), absorb,
                                         resist, true, 0);
        ...
        int32 new_damage = Unit::DealDamage(pCaster, target, pdamage, &cleanDamage, DOT, GetSpellSchoolMask(spellProto),
                                            spellProto, false);
        Unit::ProcDamageAndSpell(ProcSystemArguments(pCaster, target, procAttacker, procVictim, PROC_EX_NORMAL_HIT,
                                                     pdamage, BASE_ATTACK, spellProto));
        ...
        if (m_modifier.m_auraname == SPELL_AURA_PERIODIC_LEECH)
            break;
        pCaster->getHostileRefManager().threatAssist(
            pCaster, gain * 0.5f * sSpellMgr.GetSpellThreatMultiplier(spellProto), spellProto, false, true);
        break;
    }
    ...
    case SPELL_AURA_OBS_MOD_HEALTH: {
        ...
        // don't heal target if max health or if not alive, mostly death
        // persistent effects from items
        ...
        if (target->IsImmuneToSchool(spellProto,
                                     (1 << GetEffIndex()))) // TODO: Check if we need to
                                                            // check for avoiding immune
        {
            Unit::SendSpellOrDamageImmune(GetCasterGuid(), target, spellProto->Id);
            break;
        }
        ...
        Unit::ProcDamageAndSpell(ProcSystemArguments(pCaster, target, procAttacker, procVictim, procEx, gain,
                                                     BASE_ATTACK, spellProto, nullptr, gain, true));
        ...
        if (target != pCaster && spellProto->HasAttribute(SPELL_ATTR_EX2_HEALTH_FUNNEL))
        {
            ...
            if (pCaster->GetHealth() > damage)
            {
                ...
                Unit::DealDamage(pCaster, pCaster, damage, &cleanDamage, NODAMAGE, GetSpellSchoolMask(spellProto),
                                 spellProto, true);
            }
            else
            {
                // cannot apply damage part so we have to cancel responsible
                // aura
                pCaster->RemoveAurasDueToSpell(GetId());

                // finish current generic/channeling spells, don't affect
                // autorepeat
    ...
    case SPELL_AURA_PERIODIC_MANA_LEECH: {
        ...
        // power type might have changed between aura applying and tick
        // (druid's shapeshift)
        ...
        // Check for immune (not use charges)
        if (!spellProto->HasAttribute(SPELL_ATTR_NO_IMMUNITIES)) // confirmed Impaling spine goes
                                                                 // through immunity
        {
            if (target->IsImmuneToDamage(GetSpellSchoolMask(spellProto)))
            {
        ...
        if (gainCalculated)
        {
            ...
            if (GetSpellProto()->IsFitToFamily(SPELLFAMILY_WARLOCK,
                                               0x0000000000000010)) // Drain Mana
    ...
    case SPELL_AURA_PERIODIC_ENERGIZE: {
        // don't energize target if not alive, possible death persistent
        // effects
        if (!target->IsAlive())
            break;
...
void Aura::PeriodicDummyTick()
{
    ...
    case SPELLFAMILY_GENERIC: {
            ...
            if (Unit *caster = GetCaster())
            {
                ...
                if (caster->IsAlive() && !caster->IsWithinDistInMap(target, m_radius))
                    target->CastSpell(target, (spell->Id == 21094 ? 21095 : 23492), TRIGGERED_OLD_TRIGGERED,
                                      nullptr); // Spell 21095: Separation Anxiety for
                                                // Majordomo Executus' adds, 23492:
                                                // Separation Anxiety for Garr's adds
            }
            return;
            //              // Gossip NPC Periodic - Talk
        case 32441: // Brittle Bones
            if (roll_chance_i(33))
                target->CastSpell(target, 32437, true, nullptr,
                                  this); // Rattled
            return;
            //              case 33208: break;
        ...
        case 42621: // Fire Bomb
        {
            ...
            //              // Ram Level - Neutral
            //              case 43310: break;
            //              // Headless Horseman - Maniacal Laugh, Maniacal,
            //              Delayed 17 case 43884: break;
            //              // Headless Horseman - Maniacal Laugh, Maniacal,
            //              other, Delayed 17 case 44000: break;
    ...
    case SPELLFAMILY_HUNTER: {
        ...
        case 34074: // Dummy implementation for mana restoration
        {
            ...
            if (target->GetPowerPercent() == 100.f)
                return;
            // Regen amount is max (100% from spell) on 21% or less mana and
            // min on 92.5% or greater mana (20% from spell)
            ...
            if (target->HasAura(38390)) // Improved Aspect of the Viper
                intelectPercent += 5;
            int32 restorationValue =
                (target->GetStat(STAT_INTELLECT) * intelectPercent + target->GetLevel() * 35) / 100;
...
void Aura::HandleAuraMirrorImage(bool apply, bool Real)
{
    ...
    if (apply)
    {
        // Caster can be player or creature, the unit who pCreature will become
        // an clone of.
        Unit *caster = GetCaster();
...
void Aura::HandleAuraSafeFall(bool Apply, bool Real)
{
    ...
    if (Apply && Real && GetId() == 32474 && GetTarget()->GetTypeId() == TYPEID_PLAYER &&
        ...
        ((Player *)GetTarget())
            ->ActivateTaxiPathTo(506,
                                 GetId()); // on DB load flight path is initiated
                                           // on its own after its safe to do so
}
                                      ...
                                      ObjectGuid const &targetGuid)
{
    if (casterGuid == targetGuid)
    {
        ...
        if (spellproto->Stances)
        {
            switch (spellproto->Id)
            {
            case 11327: // vanish stealth aura improvements are removed on
                        // stealth removal
    ...
    return false;
    /*TODO: investigate spellid 24864  or (SpellFamilyName = 7 and
     * EffectApplyAuraName_1 = 49 and stances = 0)*/
}
...
SpellAuraHolder::SpellAuraHolder(SpellEntry const *spellproto, Unit *target, WorldObject *caster, Item *castItem,
    ...
    : m_spellProto(spellproto), m_target(target), m_castItemGuid(castItem ? castItem->GetObjectGuid() : ObjectGuid()),
      ...
      m_skipUpdate(false), m_reducedProcChancePast60(false), m_auraScript(SpellScriptMgr::GetAuraScript(spellproto->Id))
{
    MANGOS_ASSERT(target);
    MANGOS_ASSERT(spellproto && spellproto == sSpellTemplate.LookupEntry<SpellEntry>(spellproto->Id) &&
                  "`info` must be pointer to sSpellTemplate element");
...
void SpellAuraHolder::_RemoveSpellAuraHolder()
{
    ...
    // Note: but totem can be not accessible for aura target in time remove (to
    // far for find in grid) if (m_isPassive && !(caster && caster->GetTypeId()
    // == TYPEID_UNIT && ((Creature*)caster)->IsTotem()))
    //    return;
...
bool SpellAuraHolder::IsNeedVisibleSlot(Unit const *caster) const
{
    ...
    for (int i = 0; i < MAX_EFFECT_INDEX; ++i)
    {
        ...
        case SPELL_EFFECT_APPLY_AREA_AURA_PARTY: // passive party auras shown
                                                 // even on caster
...
void SpellAuraHolder::HandleSpellSpecificBoosts(bool apply)
{
    ...
    case SPELLFAMILY_MAGE: {
        ...
        //{
        //     default:
        //         break; // Break here for poly below - 2.4.2+ only player
        //         poly regens
        // }
        break;
    }
    ...
    for (uint32 spellId : boostSpells)
    {
        ...
        ObjectGuid casterGuid = m_target->GetObjectGuid(); // caster can be nullptr, but guid is
                                                           // still valid for removal
...
{
    // note: auras in delete list won't be affected since they clear themselves
    // from holder when adding to deletedAuraslist
    for (int32 i = 0; i < MAX_EFFECT_INDEX; ++i)
        delete m_auras[i];
}
...
void SpellAuraHolder::Update(uint32 diff)
{
    ...
    if (m_duration > 0)
    {
        ...
        if (m_timeCla <= 0)
        {
            if (Unit *caster = GetCaster())
            {
                // This should not be used for health funnel (already processed
                // in PeriodicTick()).
...
{
    // NOTE: This is an experimental approximation of heartbeat resist mechanics,
    // more research is required Main points in common cited by independent
    // sources:
...
void SpellAuraHolder::UpdateHeartbeatResist(uint32 diff)
{
    ...
    while (m_heartbeatResistTimer < 0)
    {
        ...
        DEBUG_FILTER_LOG(LOG_FILTER_COMBAT,
                         "UpdateHeartbeatResist: Update tick for spell %u with "
                         "%i ms interval",
                         m_spellProto->Id, m_heartbeatResistInterval);

        const bool resist = roll_chance_f(m_heartbeatResistChance);

        DEBUG_FILTER_LOG(LOG_FILTER_COMBAT, "UpdateHeartbeatResist: Result: %s (chance %.2f)",
                         (resist ? "RESIST" : "HIT"), double(m_heartbeatResistChance));
...
uint32 Aura::CalculateAuraEffectValue(Unit *caster, Unit *target, SpellEntry const *spellProto, SpellEffectIndex effIdx,
                                      uint32 value)
{
...
void Aura::OnAbsorb(int32 &currentAbsorb, int32 &remainingDamage, uint32 &reflectedSpellId, int32 &reflectDamage,
                    bool &preventedDeath)
{
    if (AuraScript *script = GetAuraScript())
