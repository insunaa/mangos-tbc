...
enum
{
    ...
    SPELL_DESPAWN_SUMMONS = 30228, // Spell ID unsure: there are several spells doing the same thing
                                   // in DBCs within Naxxramas spell IDs range but it is not always
                                   // clear which one is for each boss so some are assigned randomly
                                   // like this one
...
struct boss_nothAI : public CombatAI
{
    boss_nothAI(Creature *creature)
        : CombatAI(creature, NOTH_ACTION_MAX), m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData()))
    {
    ...
    uint8 m_phase;
    uint8 m_phaseSub;
    uint8 m_subWavesCount;  // On balcony phase, counts if this is the first or
                            // the second wave. Used to make Noth returns early
    ...
    void SummonedCreatureJustDied(Creature *summoned) override
    {
        ...
        // Count how many wave NPCs are killed: if all of them and this was the
        // second wave, Noth returns early
    ...
    {
        // Four Plagued Champions are to be summoned, one in each of the three
        // corners and the fourth randomly along the western wall On the third
        // wave, we use specific spell to also summon a Plagued Construct in West
        // part and North-East corner
        if (DoCastSpellIfCan(m_creature, spellSummonPlaguedChampionSWList[urand(0, 1)], CAST_TRIGGERED) == CAST_OK)
            ++m_wavesNpcsCount; // We count the addition of a new NPC on each spell
                                // cast because JustSummoned() can possibly be called
                                // more than once per summoned NPC, making the count
                                // there incorrect
    ...
    {
        // Two Plagued Guardians are to be summoned, one in the south-west corner
        // and one in one of the two other corners
    ...
    void HandleBalconyPhase()
    {
        ...
        if (DoCastSpellIfCan(m_creature, SPELL_TELEPORT) == CAST_OK)
        {
            ...
            DoCastSpellIfCan(m_creature, SPELL_IMMUNE_ALL,
                             CAST_TRIGGERED); // Prevent players from damaging
                                              // Noth when he is on the balcony
            SetMeleeEnabled(false);
            StopTargeting(true);
            m_creature->SetTarget(nullptr);
            ResetCombatAction(NOTH_SUMMON,
                              10u * IN_MILLISECONDS); // Summon first wave 10
                                                      // seconds after teleport
            ++m_phaseSub;
            switch (m_phaseSub)
            {
            case PHASE_SKELETON_1:
                ResetTimer(NOTH_GROUND_PHASE, 70u * IN_MILLISECONDS);
                break;
            case PHASE_SKELETON_2:
                ResetTimer(NOTH_GROUND_PHASE, 97u * IN_MILLISECONDS);
                break;
            case PHASE_SKELETON_3:
                ResetTimer(NOTH_GROUND_PHASE, 120u * IN_MILLISECONDS);
                break;
            }
    ...
    void HandleGroundPhase()
    {
        if (DoCastSpellIfCan(m_creature, SPELL_TELEPORT_RETURN, CAST_TRIGGERED) == CAST_OK)
        {
            ...
            ResetCombatAction(NOTH_SUMMON,
                              5u * IN_MILLISECONDS); // 5 seconds before summoning
                                                     // Plagued Warriors again
            ...
            {
            case PHASE_SKELETON_1:
                ResetTimer(NOTH_BALCONY_PHASE, 110u * IN_MILLISECONDS);
                break;
            case PHASE_SKELETON_2:
                ResetTimer(NOTH_BALCONY_PHASE, 180u * IN_MILLISECONDS);
                break;
            case PHASE_SKELETON_3:
                DisableTimer(NOTH_BALCONY_PHASE);
                break;
            }
            m_phase = PHASE_GROUND;
        }
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case NOTH_SUMMON: {
            ...
            else
            {
                ...
                case PHASE_SKELETON_3: {
                    DoSummonPlaguedChampions(true); // Set parameter to true because we need to
                                                    // indicate this is the third wave and Plagued
                                                    // Construct need to be also summoned
                    DoSummonPlaguedGuardians();
