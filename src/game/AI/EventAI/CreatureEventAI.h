...
#define LOG_PROCESS_EVENT                                                                                              \
    DEBUG_FILTER_LOG(LOG_FILTER_EVENT_AI_DEV,                                                                          \
                     "CreatureEventAI: Event type %u (script %u) triggered for "                                       \
                     "%s (invoked by %s)",                                                                             \
...
enum EventAI_Type
{
    ...
    EVENT_T_TARGET_MISSING_AURA = 28,     // Param1 = SpellID, Param2 = Number of time
                                          // stacked expected, Param3/4 Repeat Min/Max
...
enum EventAI_ActionType
{
    ...
    ACTION_T_TEXT = 1,                     // TextId1, optionally TextId2, optionally TextId3(if
                                           // TextId2 exist). If more than just TextId1 is defined,
                                           // randomize. Values are broadcast_text Ids.
    ACTION_T_SET_FACTION = 2,              // FactionId (or 0 for default)
    ACTION_T_MORPH_TO_ENTRY_OR_MODEL = 3,  // Creature_template entry(param1) OR ModelId (param2) (or 0 for both
                                           // to demorph)
    ...
    ACTION_T_MOUNT_TO_ENTRY_OR_MODEL = 43, // Creature_template entry(param1) OR ModelId (param2) (or 0 for both
                                           // to unmount)
    ACTION_T_CHANCED_TEXT = 44,            // Chance to display the text, TextId1, optionally
                                           // TextId2. If more than just TextId1 is defined,
                                           // randomize. Values are broadcast_text Ids.
    ...
    ACTION_T_CHANGE_MOVEMENT = 48,         // MovementType, WanderDistance if Movement Type 1 and PathId if
                                           // Movement Type 2, asDefault
    ...
    ACTION_T_SET_IMMOBILIZED_STATE = 61,   // state (true - rooted), combatonly
                                           // (true - autoremoved on combat stop)
...
enum Target
{
    ...
    TARGET_T_HOSTILE_SECOND_AGGRO = 2,   // Second highest aggro (generaly used for
    ...
    // Invoker targets
    TARGET_T_ACTION_INVOKER = 6,       // Unit who caused this Event to occur (only works for EVENT_T_AGGRO,
                                       // EVENT_T_KILL, EVENT_T_DEATH, EVENT_T_SPELLHIT, EVENT_T_OOC_LOS,
                                       // EVENT_T_FRIENDLY_HP, EVENT_T_FRIENDLY_IS_CC,
                                       // EVENT_T_FRIENDLY_MISSING_BUFF, EVENT_T_RECEIVE_EMOTE,
                                       // EVENT_T_RECEIVE_AI_EVENT)
    TARGET_T_ACTION_INVOKER_OWNER = 7, // Unit who is responsible for Event to occur (only works for
                                       // EVENT_T_AGGRO, EVENT_T_KILL, EVENT_T_DEATH, EVENT_T_SPELLHIT,
                                       // EVENT_T_OOC_LOS, EVENT_T_FRIENDLY_HP, EVENT_T_FRIENDLY_IS_CC,
                                       // EVENT_T_FRIENDLY_MISSING_BUFF, EVENT_T_RECEIVE_EMOTE,
                                       // EVENT_T_RECEIVE_AI_EVENT)
    TARGET_T_EVENT_SENDER = 10,        // Unit who sent an AIEvent that was received
    ...
    TARGET_T_NONE = 15, // Default spell target - sets nullptr which should be
                        // most common spell fill
...
struct CreatureEventAI_Action
{
    ...
    union {
        ...
        // provided mask bits that will be set ACTION_T_REMOVE_UNIT_FLAG = 19,  //
        // value provided mask bits that will be clear
        struct
        {
            uint32 value;
            uint32 target;
        ...
        struct
        {
            uint32 state; // 0 = stop combat based movement, anything else continue
                          // attacking
            uint32 melee; // if set: at stop send melee combat stop if in combat, use
                          // for terminate melee fighting state for switch to ranged
        } combat_movement;
...
struct CreatureEventAI_Event
{
    ...
    union {
        ...
        {
            uint32 spellId;
            uint32 schoolMask; // -1 (==0xffffffff) is ok value for full mask, or
                               // must be more limited mask like (0 < 1) = 1 for
                               // normal/physical school
            uint32 repeatMin;
            uint32 repeatMax;
        } spell_hit;
        // EVENT_T_RANGE                                    = 9
        ...
        struct
        {
            uint32 eventType;   // See UnitAI.h enum AIEventType - Receive only events
                                // of this type
            uint32 senderEntry; // Optional npc from only whom this event can be
                                // received
            uint32 unused1;
            uint32 unused2;
        } receiveAIEvent;
...
class CreatureEventAI : public CreatureAI
{
  public:
    ...
    MovementGeneratorType GetDefaultMovement()
    {
        return m_defaultMovement;
    }
  ...
  protected:
    ...
    uint32 m_throwAIEventMask; // Automatically throw AIEvents that are encoded
                               // into this mask
    // Note that Step 100 means that AI_EVENT_GOT_FULL_HEALTH was sent
    // Steps 0..2 correspond to AI_EVENT_LOST_SOME_HEALTH(90%),
    // AI_EVENT_LOST_HEALTH(50%), AI_EVENT_CRITICAL_HEALTH(10%)
    uint32 m_throwAIEventStep; // Used for damage taken/ received heal
    float m_LastSpellMaxRange; // Maximum spell range that was cast during
                               // dynamic movement
