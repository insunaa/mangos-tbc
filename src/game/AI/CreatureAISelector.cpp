...
{
    // Allow scripting AI for normal creatures and not controlled pets (guardians
    // and mini-pets)
    ...
    else if (creature->HasCharmer() && !creature->IsTemporarySummon()) // for charmed creatures but not
                                                                       // possessed tempspawns
        ai_factory = ai_registry.GetRegistryItem("PetAI");
...
UnitAI *GetSpecificAI(Unit *unit, std::string const &ainame)
{
    ...
    sLog.outError("FactorySelector::GetSpecificAI> Cannot get %s AI for %s", ainame.c_str(),
...
MovementGenerator *selectMovementGenerator(Creature *creature)
{
    ...
    /* if( mv_factory == nullptr  )
    {
        ...
        for( std::vector<std::string>::iterator iter = l.begin(); iter !=
    l.end();
    ++iter)
        {
        const MovementGeneratorCreator *factory =
    mv_registry.GetRegistryItem((*iter).c_str()); const SelectableMovement *p =
    dynamic_cast<const SelectableMovement *>(factory); ASSERT( p != nullptr );
    int val = p->Permit(creature); if( val > best_val )
        {
            best_val = val;
            mv_factory = p;
        }
