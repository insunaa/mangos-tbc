...
{
    uint32 Index;                                           //          m_ID
categoryId (sent in RealmList packet) uint32 Unk1; //          m_localeMask
...
{
    uint32 Id;                                              //          m_ID
    uint32 Type;                                            // m_realmType
(sent in RealmList packet) uint32 IsPvp; // m_playerKillingAllowed uint32 IsRp;
//          m_roleplaying
};*/
...
struct CharStartOutfitEntry
{
    ...
    int32 ItemId[MAX_OUTFIT_ITEMS]; // 2-13     m_ItemID
                                    ...
                                    // m_DisplayItemID not required at server side int32
                                    // ItemInventorySlot[MAX_OUTFIT_ITEMS];           // 26-37    m_InventoryType
                                    // not required at server side uint32 Unknown1; // 38, unique values
                                    // (index-like with gaps ordered in other way as ids) uint32 Unknown2; // 39
                                    // uint32 Unknown3;                                     // 40
};
...
struct ChatChannelsEntry
{
    ...
    char *pattern[16]; // 3-18     m_name_lang
                       // 19 string flags
                       // char*       name[16];                                // 20-35
                       // m_shortcut_lang 36 string flags
};
...
struct CharSectionsEntry
{
    ...
    // char* TexturePath[3];
    uint32 Flags;
    inline bool HasFlag(CharSectionFlags flag) const
    {
        return (Flags & flag) != 0;
    }
};
...
struct ChrClassesEntry
{
    ...
    // m_name_female_lang 37 string flags char*       nameNeutralGender[16]; //
    // 38-53    m_name_male_lang 54 string flags 55       m_filename
...
struct ChrRacesEntry
{
    ...
    // m_name_female_lang 47 string flags char*       nameNeutralGender[16]; //
    // 48-63    m_name_male_lang 64 string flags 65-66
    // m_facialHairCustomization[2] 67       m_hairCustomization
...
struct CinematicCameraEntry
{
    ...
    float OriginFacing;   // 4    Orientation in map used for basis for M2
                          // co-ordinates
};
...
struct CreatureDisplayInfoExtraEntry
{
    ...
    uint32 Race;           // 1        m_DisplayRaceID
                           ...
                           // EQUIPMENT_SLOT_HEAD..EQUIPMENT_SLOT_HANDS, client show its by self uint32
                           // CanEquip;                                  // 19       m_flags 0..1 Can
...
struct CreatureTypeEntry
{
    uint32 ID; // 0        m_ID
               ...
               // 17 string flags uint32    no_expirience;                             // 18
               // m_flags
};
...
struct EmotesEntry
{
    uint32 Id; // 0        m_ID
    // char*   Name;                                        // 1
    // m_EmoteSlashCommand uint32  AnimationId; // 2        m_AnimID
...
struct FactionEntry
{
    ...
    float spilloverRateIn;      // 19       m_parentFactionMod[2] Faction gains
                                // incoming rep * spilloverRateIn
    float spilloverRateOut;     // 20       Faction outputs rep * spilloverRateOut as
                                // spillover reputation
    uint32 spilloverMaxRankIn;  // 21       m_parentFactionCap[2] The highest rank
                                // the faction will profit from incoming spillover
    uint32 spilloverRank_unk;   // 22       It does not seem to be the max standing at
                                // which a faction outputs spillover ...so no idea
    char *name[16];             // 23-38    m_name_lang
    // 39 string flags
    // char*     description[16];                           // 40-55
    // m_description_lang 56 string flags
...
/*
// NOTE: FactionGroup.dbc - currently unused, please refer to related hardcoded
enum FactionGroupMask struct FactionGroupEntry
{
    uint32      ID;                                         // 0        m_ID
    uint32      maskID;                                     // 1 m_maskID index
of the bit to check for in the faction group mask char* internalName; // 2
m_internalName
    // char*       name[16];                                // 3-18 m_name_lang
localized display name in the UI
    // 19 string flags
};
*/
...
struct FactionTemplateEntry
{
    ...
    uint32 factionGroupMask; // 3 if mask set (see FactionGroupMask) then faction
                             // included in masked group
    uint32 friendGroupMask;  // 4 if mask set (see FactionGroupMask) then faction
                             // friendly to masked group
    uint32 enemyGroupMask;   // 5 if mask set (see FactionGroupMask) then faction
                             // hostile to masked group
    ...
    {
        return (enemyGroupMask & FACTION_GROUP_MASK_PLAYER) != 0;
    }
...
#define MAX_GMSURVEY_QUESTIONS                                                                                         \
    10 // Hardcoded in all versions of the game, max amount of questions in gm
       // survey
...
//    ItemPrototype::CondExtendedCost uint32      itemextendedcostentry; //
//    ItemPrototype::ExtendedCost uint32      arenaseason; // arena season
//    number(1-4)
//};
...
struct MailTemplateEntry
{
    ...
    // m_subject_lang 17 string flags
...
struct MapEntry
{
    ...
    // m_MapDescription0_lang 44 string flags char*     allianceIntro[16]; //
    // 45-60    m_MapDescription1_lang 61 string flags
    uint32 multimap_id; // 62       m_LoadingScreenID (LoadingScreens.dbc)
    // 63-64 not used
    // float   BattlefieldMapIconScale;                     // 65
    // m_minimapIconScale chat*     unknownText1                               //
    // 66-81 unknown empty text fields, possible normal Intro text. 82 text flags
    // chat*     heroicIntroText                            // 83-98 heroic mode
    // requirement text 99 text flags chat*     unknownText2 // 100-115 unknown
    // empty text fields 116 text flags
    int32 ghost_entrance_map; // 117      m_corpseMapID map_id of entrance map in
                              // ghost mode (continent always and in most cases =
                              // normal entrance)
...
struct QuestSortEntry
{
    uint32 id; // 0        m_ID
               // char*       name[16];                                // 1-16
               // m_SortName_lang 17 string flags
};
...
struct SkillLineEntry
{
    ...
    // m_description_lang 36 string flags
...
struct SkillLineAbilityEntry
{
    ...
    uint32 racemask;  // 3
    uint32 classmask; // 4
    // uint32    racemaskNot;                               // 5 always 0
    // in 2.4.2 uint32    classmaskNot;                              // 6 always
    // 0 in 2.4.2
...
struct SoundEntriesEntry
{
    uint32 Id; // 0        m_ID
               ...
               // char* InternalName;                              // 2        m_name char*
               // FileName[10];                              // 3-12     m_File[10] uint32
               // Unk13[10];                                 // 13-22    m_Freq[10] char*
               // Path;                                      // 23       m_DirectoryBase 24
               // m_volumeFloat 25       m_flags 26       m_minDistance 27 m_distanceCutoff
               // 28       m_EAXDef
};
...
struct ClassFamilyMask
{
    ...
    bool operator==(const ClassFamilyMask &another) const
    {
...
struct SpellEntry
{
    ...
    int32 EffectBasePoints[MAX_EFFECT_INDEX];          // 80-82    m_effectBasePoints (don't
                                                       // must be used in spell/auras
                                                       // explicitly, must be used cached
    ...
    // uint32    SpellNameFlag;                         // 143      m_name_flag
    // not used
    char *Rank[16]; // 144-159  m_nameSubtext_lang
    // uint32    RankFlags;                             // 160 m_nameSubtext_flag
    // not used char*     Description[16];                       // 161-176
    // m_description_lang not used uint32    DescriptionFlags; // 177
    // m_description_flag not used char*     ToolTip[16]; // 178-193
    // m_auraDescription_lang not used uint32    ToolTipFlags; // 194
    ...
    // custom
    bool HasAttribute(SpellAttributesServerside attribute) const
    {
        return (AttributesServerside & attribute) != 0;
    }
...
struct SpellRangeEntry
{
    ...
    uint32 Flags;   // 3        m_flags
                    // char*   Name[16];                                    // 4-19
                    // m_displayName_lang uint32  NameFlags;                                   //
                    // 20 string flags char*   ShortName[16];                               //
                    // 21-36    m_displayNameShort_lang uint32  NameFlags; // 37 string flags
};
...
struct SpellShapeshiftFormEntry
{
    ...
    // (but all 0) uint32 unk3;                                         // 25
    // unused always 0 uint32 unk4;                                         // 26
    // unused always 0
...
struct TotemCategoryEntry
{
    ...
    uint32 categoryMask; // 19       m_totemCategoryMask (compatibility mask for
                         // same type: different for totems, compatible from high
                         // to low for rods)
};
...
struct WMOAreaTableEntry
{
    ...
    // uint32 field4;                                       // 4
    // m_SoundProviderPref uint32 field5; // 5 m_SoundProviderPrefUnderwater
    // uint32 field6; // 6 m_AmbienceID uint32 field7; // 7 m_ZoneMusic uint32
    // field8;                                       // 8 m_IntroSound
...
struct WorldMapAreaEntry
{
    ...
    int32 virtual_map_id; // 8        m_displayMapID -1 (map_id have correct map)
                          // other: virtual map where zone show (map_id - where
                          // zone in fact internally)
};
...
(WorldMapArea.dbc) uint32    areatableID[MAX_WORLD_MAP_OVERLAY_AREA_IDX];  //
2-5      m_areaID
                                                            // 6 m_mapPointX
                                                            // 7 m_mapPointY
...
/* Structure WorldSafeLocsEntry is no longer loaded from DBC but from DB
instead struct WorldSafeLocsEntry
{
    uint32    ID;                                           // 0        m_ID
