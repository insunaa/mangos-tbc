...
void GameObject::AddToWorld()
{
    ...
    // After Object::AddToWorld so that for initial state the GO is added to the
    // world (and hence handled correctly)
    UpdateCollisionState();

    if (IsSpawned()) // need to prevent linked trap addition due to Pool system
                     // Map::Add abuse
    {
...
void GameObject::RemoveFromWorld()
{
    ...
    if (IsInWorld())
    {
        ...
        if (ObjectGuid owner_guid = GetOwnerGuid())
        {
            ...
            {
                sLog.outError("Delete %s with SpellId %u LinkedGO %u that lost "
                              "references to owner %s GO list. Crash possible later.",
                              GetGuidStr().c_str(), m_spellId, GetGOInfo()->GetLinkedGameObjectEntry(),
                        ...
                        float rotation1, float rotation2, float rotation3, uint32 animprogress, GOState go_state)
{
    ...
    if (!IsPositionValid())
    {
        sLog.outError("Gameobject (GUID: %u Entry: %u ) not created. Suggested "
    ...
    if (!goinfo)
    {
        sLog.outErrorDb("Gameobject (GUID: %u) not created: Entry %u does not exist in "
    ...
    {
        sLog.outErrorDb("Gameobject (GUID: %u) not created: Entry %u has invalid type %u in "
                        "`gameobject_template`. It may crash client if created.",
    ...
    UpdateRotationFields(rotation2,
                         rotation3); // GAMEOBJECT_FACING, GAMEOBJECT_ROTATION+2/3
    ...
    case GAMEOBJECT_TYPE_FISHINGNODE:
        m_lootState = GO_NOT_READY; // Initialize Traps and Fishingnode delayed
                                    // in ::Update
        break;
...
void GameObject::Update(const uint32 diff)
{
    ...
    case GO_READY: {
            ...
            if (m_respawnTime <= time(nullptr)) // timer expired
            {
                ...
                case GAMEOBJECT_TYPE_BUTTON:
                    ...
                    if (GetGoState() != GO_STATE_READY)
                        ResetDoorOrButton();
                // flags in AB are type_button and we need to add them here
                // so no break!
                default:
        ...
        if (IsSpawned())
        {
            ...
            if (goInfo->type == GAMEOBJECT_TYPE_TRAP && GetGoState() == GO_STATE_READY) // traps
            {
                ...
                {
                    // FIXME: this is activation radius (in different casting
                    // radius that must be selected from spell data)
                    // TODO: move activated state code (cast itself) to
                    // GO_ACTIVATED, in this place only check activating and
                    // set state
                    float radius = float(goInfo->trap.diameter) / 2.0f;

                    // behavior verified on classic
                    // TODO: needs more research
                    if (goInfo->GetLockId() == 12) // 21 objects currently (hunter traps), all with
                                                   // 5 or less for diameter -> use diameter as
                                                   // radius instead
                    ...
                    if (!radius)
                    {
                        if (goInfo->trap.cooldown != 3) // cast in other case (at some
                                                        // triggering/linked go/etc explicit call)
                            valid = false;
                        else
                        {
                            if (m_respawnTime > 0)
                                valid = false;
                            else // battlegrounds gameobjects has data2 == 0 &&
                                 // data5 == 3
                    ...
                    {
                        // Should trap trigger?
                        Unit *target = nullptr; // pointer to appropriate
                                                // target if found any
                        ...
                        if (std::function<bool(Unit *)> *functor = sScriptDevAIMgr.OnTrapSearch(this))
                        {
                            ...
                            MaNGOS::UnitSearcher<MaNGOS::AnyUnitFulfillingConditionInRangeCheck> checker(target,
                                                                                                         u_check);
                        ...
                        else
                        {
                            ...
                            case 1: // friendly
                            {
                                ...
                                MaNGOS::UnitSearcher<MaNGOS::AnyFriendlyUnitInObjectRangeCheck> checker(target,
                                                                                                        u_check);
                                Cell::VisitAllObjects(this, checker, radius);
                                break;
                            }
                            ...
                            default: // unfriendly
                            {
                                ...
                                MaNGOS::UnitSearcher<MaNGOS::AnyUnfriendlyUnitInObjectRangeCheck> checker(target,
                                                                                                          u_check);
                                Cell::VisitAllObjects(this, checker, radius);
                                break;
                            }
                        ...
                        if (target && (!goInfo->trapCustom.triggerOn ||
                                       !target->HasFlag(UNIT_FIELD_FLAGS,
                                                        UNIT_FLAG_NOT_SELECTABLE))) // do not
                                                                                    // trigger on
                                                                                    // hostile
                                                                                    // traps if not
                                                                                    // selectable
            ...
            int32 max_charges = goInfo->GetCharges(); // Only check usable (positive) charges;
                                                      // 0 : no charge; -1 : infinite charges
    ...
    case GO_JUST_DEACTIVATED: {
        ...
        {
        case GAMEOBJECT_TYPE_GOOBER:
            // if gameobject should cast spell, then this, but some GOs (type =
            // 10) should be destroyed
            ...
            SetGoState(GO_STATE_READY);
            // research - 185861 needs to be able to despawn as well TODO:
            // fixup

            // any return here in case battleground traps
            break;
        case GAMEOBJECT_TYPE_CAPTURE_POINT:
            // there are some capture points which are used as visual GOs; we
            // allow these to be despawned
            ...
            return; // SetLootState and return because go is treated as
                    // "burning flag" due to GetGoAnimProgress() being 100 and
                    // would be removed on the client
        case GAMEOBJECT_TYPE_CHEST:
            m_despawnTimer = 0;
        ...
        if (GetGOInfo()->IsDespawnAtAction() || GetGoAnimProgress() > 0)
        {
            ...
            {
                // In Instances GO_FLAG_LOCKED, GO_FLAG_INTERACT_COND or
                // GO_FLAG_NO_INTERACT are not changed
        ...
        {
            // if part of pool, let pool system schedule new spawn instead of
            // just scheduling respawn
...
bool GameObject::LoadFromDB(uint32 dbGuid, Map *map, uint32 newGuid, uint32 forcedEntry)
{
    ...
    if (!data)
    {
        sLog.outErrorDb("Gameobject (GUID: %u) not found in table "
                        "`gameobject`, can't load. ",
                        dbGuid);
        return false;
    }
    ...
    SpawnGroupEntry *groupEntry =
        map->GetMapDataContainer().GetSpawnGroupByGuid(dbGuid, TYPEID_GAMEOBJECT); // use dynguid by default \o/
    ...
    if (!u->IsGameMaster())
    {
        ...
        case GAMEOBJECT_TYPE_TRAP: {
            ...
            if (GetGOInfo()->trap.stealthed) // stealthed traps
            {
                ...
                if (GetDistance(viewPoint, true, DIST_CALC_NONE) < visibleDistance * visibleDistance &&
                    u->HasInArc(this))
                    return true;
            }
...
bool GameObject::ActivateToQuest(Player *pTarget) const
{
    ...
    case GAMEOBJECT_TYPE_QUESTGIVER: {
        ...
        // Never expect flags|4 for these GO's? (NF-note: It doesn't appear
        // it's expected)
    ...
    case GAMEOBJECT_TYPE_GOOBER: {
        // Quest ID can be negative, so prevent error in Player::GetQuestStatus
        // by providing only positive value
...
void GameObject::TriggerLinkedGameObject(Unit *target) const
{
    ...
    // (all?) traps are probably expected to be pretty much at the same location
    // as the used GO, so it appears that using range from spell is obsolete.
    float range = 0.5f;
...
void GameObject::Use(Unit *user, SpellEntry const *spellInfo)
{
    ...
    bool scriptReturnValue =
        user->GetTypeId() == TYPEID_PLAYER && sScriptDevAIMgr.OnGameObjectUse((Player *)user, this);
    ...
    {
        // objects used mostly in battlegrounds; there are only a few
        // exceptions to this rule CanUseBattleGroundObject() is already
        // checked in the spell cast; all of these objects use the 1479 lock id
    ...
    case GAMEOBJECT_TYPE_TRAP: // 6
    {
        ...
        if (goInfo->trap.spellId)
            if (CastSpell(caster, user, goInfo->trap.spellId, TRIGGERED_OLD_TRIGGERED, nullptr, nullptr,
    ...
    case GAMEOBJECT_TYPE_CHAIR: // 7 Sitting: Wooden bench, chairs
    {
        ...
        if (info->chair.slots > 0)
        {
            ...
            for (uint32 i = 0; i < info->chair.slots; ++i)
            {
                ...
                them. Creature* helper = player->SummonCreature(14496, x_i,
                y_i, GetPositionZ(), GetOrientation(),
                TEMPSPAWN_TIMED_OR_DEAD_DESPAWN, 10000); std::ostringstream
                output; output << i << ": thisDist: " << thisDistance;
    ...
    case GAMEOBJECT_TYPE_GOOBER: // 10
    {
        ...
        // exception - 180619 - ossirian crystal - supposed to be kept from
        // despawning by a pending spellcast - to be implemented, done in db
        // for now
        ...
        if (user->GetTypeId() == TYPEID_PLAYER)
        {
            ...
            if (info->goober.eventId)
            {
                ...
                // for battleground events we need to allow the event id to be
                // forwarded Note: this exception is required in order not to
                // change the legacy even handling in DB scripts
    ...
    case GAMEOBJECT_TYPE_FISHINGNODE: // 17 fishing bobber
    {
        ...
        case GO_READY: // ready for loot
        {
            ...
            if (!zone_skill)
                sLog.outErrorDb("Fishable areaId %u are not properly defined in "
            ...
            DEBUG_LOG("Fishing check (skill: %i zone min skill: %i chance "
                      "%i roll: %i",
    ...
    case GAMEOBJECT_TYPE_SPELLCASTER: // 22
    {
        ...
        // Previously we locked all spellcasters on use with no real indication
        // why or timeout of the locking. Now only doing it on it being
        // consumed to prevent further use. spellcaster GOs like city portals
    ...
    case GAMEOBJECT_TYPE_MEETINGSTONE: // 23
    {
        ...
        // accept only use by player from same group for caster except caster
        // itself
    ...
    case GAMEOBJECT_TYPE_FLAGSTAND: // 24
    {
        ...
        if (player->CanUseBattleGroundObject())
        {
            ...
            // it's spawned by default in Warsong Gulch, Arathi Basin and Eye
            // of the Storm
            BattleGround *bg = player->GetBattleGround();
            if (bg)
                bg->HandlePlayerClickedOnFlag(player, this);

            // handle spell data if available; this usually marks the player as
            // the flag carrier in a battleground
            GameObjectInfo const *info = GetGOInfo();
    ...
    case GAMEOBJECT_TYPE_FLAGDROP: // 26
    {
        ...
        {
            // Note: object is used in battlegrounds; object only summoned by
            // spells, doesn't have static spawn data summon is triggered by
            // aura removal or aura death proc when player drop the main flag
            // (type 24) in Warsong Gulch and Eye of the Storm clicking on the
            // flag triggers an event which is handled by battleground script
            GameObjectInfo const *info = GetGOInfo();
            if (info && info->flagdrop.eventID)
            {
                StartEvents_Event(GetMap(), info->flagdrop.eventID, this, player, true, player);

                // handle spell data if available; this usually marks the
                // player as the flag carrier in a battleground
    ...
    {
        sLog.outError("WORLD: unknown spell id %u at use action for gameobject "
                      "(Entry: %u GoType: %u )",
...
void GameObject::SetLootRecipient(Unit *pUnit)
{
    ...
    // set group for group existed case including if player will leave group at
    // loot time
    if (Group *group = player->GetGroup())
...
void GameObject::TickCapturePoint()
{
    ...
    // players and minTime for maxSuperiority amount of players (linear function:
    // y = dy/dx*x+d)
    ...
    /* CONTESTED EVENTS */
    // alliance attacks tower which is in control or progress by horde (except if
    // alliance also gains control in that case)
    ...
    // horde attacks tower which is in control or progress by alliance (except if
    // horde also gains control in that case)
...
float GameObject::GetInteractionDistance() const
{
    ...
    case GAMEOBJECT_TYPE_MAILBOX:
        ...
        // failing it will start a minute-long cooldown until the next mail query
        // may be executed. And since movement info update is not sent with
        // mailbox interaction query, server may find the player outside of
...
QuaternionData GameObject::GetWorldRotation() const
{
    ...
    //    G3D::Quat worldRotationQuat(worldRotation.x, worldRotation.y,
    //    worldRotation.z, worldRotation.w); G3D::Quat
    //    localRotationQuat(localRotation.x, localRotation.y, localRotation.z,
    //    localRotation.w);

    //    G3D::Quat resultRotation = localRotationQuat * worldRotationQuat;

    //    return QuaternionData(resultRotation.x, resultRotation.y,
    //    resultRotation.z, resultRotation.w);
    //}
