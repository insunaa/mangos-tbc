                                        ...
                                        DumpTableType type)
{
    ...
    do
    {
        ...
        delete result;
    } while (guids && guids_itr != guids->end()); // not set case iterate single time,
                                                  // set case iterate for all guids
}
...
std::string PlayerDumpWriter::GetDump(uint32 guid)
{
    ...
    dump += "IMPORTANT NOTE: This sql queries not created for apply directly, "
            "use '.pdump load' command in console or client chat instead.\n";
    dump += "IMPORTANT NOTE: NOT APPLY ITS DIRECTLY to character DB or you will "
            "DAMAGE and CORRUPT character DB\n\n";
    ...
    if (result)
    {
        ...
        if (!reqName.empty())
        {
            ...
            sLog.outError("Table 'character_db_version' not have revision guard "
                          "field, revision guard query not added to pdump.");

        delete result;
    }
    else
        sLog.outError("Character DB not have 'character_db_version' table, "
                      "revision guard query not added to pdump.");
...
DumpReturn PlayerDumpReader::LoadDump(const std::string &file, uint32 account, std::string name, uint32 guid)
{
    ...
    while (!feof(fin))
    {
        ...
        case DTT_CHAR_NAME_TABLE:
            if (nameInvalidated) // ignore declined names if name will changed in
                                 // some way
            {
                execute_ok = false;
                break;
        ...
        case DTT_CHARACTER: {
            ...
            if (name.empty())
            {
                ...
                if (result)
                {
                    ...
                    if (!changenth(line, 36,
                                   "1")) // characters.at_login set to "rename
                                         // on login"
                        ROLLBACK(DUMP_FILE_BROKEN);
            ...
            {
                if (!changenth(line, 3,
                               name.c_str())) // characters.name update
                    ROLLBACK(DUMP_FILE_BROKEN);
        ...
        case DTT_INVENTORY: {
            if (!changenth(line, 1,
                           newguid)) // character_inventory.guid update
                ROLLBACK(DUMP_FILE_BROKEN);
        ...
        case DTT_ITEM: {
            ...
            if (!changenth(line, 2,
                           newguid)) // item_instance.owner_guid update
                ROLLBACK(DUMP_FILE_BROKEN);
            break;
        }
        case DTT_ITEM_GIFT: {
        ...
        case DTT_PET: {
            // store a map of old pet id to new inserted pet id for use by type
            // 5 tables
        ...
        case DTT_PET_DECL: // character_pet_declinedname
        {
            ...
            if (!changenth(line, 1,
                           newpetid)) // character_pet_declinedname.id
                ROLLBACK(DUMP_FILE_BROKEN);

            if (!changenth(line, 2,
                           newguid)) // character_pet_declinedname.owner update
                ROLLBACK(DUMP_FILE_BROKEN);
    ...
    // FIXME: current code with post-updating guids not safe for future per-map
    // threads
