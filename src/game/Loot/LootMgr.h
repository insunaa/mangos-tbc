...
struct LootStoreItem
{
    ...
    float chance;        // always positive, chance to drop for both quest and non-quest
                         // items, chance to be used for refs
...
struct LootItem
{
    ...
    // LootStoreItem entries only (mincountOrRef > 0)
...
class LootTemplate
{
  ...
  private:
    ...
    LootGroups Groups;         // groups have own (optimized) processing, grouped entries
                               // go there
};
...
class Loot
{
  public:
    ...
    void AddItem(uint32 itemid, uint32 count, uint32 randomSuffix,
                 int32 randomPropertyId); // used in item.cpp to explicitly load
                                          // a saved item
    ...
    ObjectGuid const &GetMasterLootGuid() const
    {
        return m_masterOwnerGuid;
    }
    GuidSet const &GetOwnerSet() const
    {
        return m_ownerSet;
    }
    TimePoint const &GetCreateTime() const
    {
        return m_createTime;
    }
  ...
  private:
    ...
    LootMethod m_lootMethod;         // used to know what kind of check must be done at
                                     // loot time
    ItemQualities m_threshold;       // group threshold for items
    ObjectGuid m_masterOwnerGuid;    // master loot player or round robin owner
    ObjectGuid m_currentLooterGuid;  // current player for under threshold items
                                     // (Round Robin)
    ...
    bool m_haveItemOverThreshold;    // if at least one item in the loot is over
                                     // threshold
