...
bool QuestAccept_npc_kayra_longmane(Player *pPlayer, Creature *pCreature, const Quest *pQuest)
{
    if (pQuest->GetQuestId() == QUEST_ESCAPE_FROM)
    {
        ...
        pCreature->SetFactionTemporary(FACTION_ESCORT_N_FRIEND_ACTIVE,
                                       TEMPFACTION_RESTORE_RESPAWN); // sniffed faction
...
struct npc_fhwoorAI : public npc_escortAI
{
    ...
    void WaypointReached(uint32 uiPointId) override
    {
        ...
        case 47:
            ...
            // spawn npcs
            m_creature->SummonCreature(NPC_ENCHANTRESS, 526.12f, 8136.96f, 21.64f, 0.57f,
                                       TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 60000);
            m_creature->SummonCreature(NPC_SLAVEDRIVER, 524.09f, 8138.67f, 21.49f, 0.58f,
                                       TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 60000);
            m_creature->SummonCreature(NPC_SLAVEDRIVER, 526.93f, 8133.88f, 21.56f, 0.58f,
                                       TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 60000);
            break;
        case 71:
            ...
            // spawn npcs
            m_bIsAmbush = true;
            m_creature->SummonCreature(NPC_SSSLITH, 162.91f, 8192.08f, 22.55f, 5.98f,
                                       TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 60000);
            m_creature->SummonCreature(NPC_ENCHANTRESS, 162.34f, 8193.99f, 22.85f, 5.98f,
                                       TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 60000);
            m_creature->SummonCreature(NPC_SLAVEDRIVER, 163.07f, 8187.04f, 22.71f, 0.10f,
                                       TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 60000);
