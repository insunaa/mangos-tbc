...
SDName: Netherstorm
SD%Complete: 80
SDComment: Quest support: 10191, 10198, 10299, 10310, 10321, 10322, 10323,
10329, 10330, 10337, 10338, 10365(Shutting Down Manaforge), 10406, 10425,
10438, 10924. SDCategory: Netherstorm EndScriptData */
...
{
    npc_manaforge_spawnAI(Creature *pCreature) : ScriptedAI(pCreature)
    {
        Reset();
    }
        ...
        if (m_manaforgeGuid)
        {
            ...
            if (manaforge)
            {
                ...
                if (fDistance < 20) // If within cast range
                {
                    ...
                    {
                        m_creature->GetMotionMaster()->MovePoint(0, m_creature->GetPositionX(),
                                                                 m_creature->GetPositionY(),
                                                                 m_creature->GetPositionZ()); // Why must I do this?
                                                                                              // They walk back to spawn
                                                                                              // point otherwise (and
                                                                                              // only ara mobs do this)
                        m_creature->CastSpell(m_creature, SPELL_INTERRUPT_2,
                                              TRIGGERED_OLD_TRIGGERED); // Ara mobs
                    }
                    else
                        m_creature->CastSpell(m_creature, SPELL_INTERRUPT_1,
                                              TRIGGERED_OLD_TRIGGERED); // Other consoles
                }
                else
                {
                    ...
                    uint32 uiMPID; // need to set this accoringly so creature will
                                   // cast spell after running
                    ...
                    /* Given an origin (x,y) and line distance "d" which makes an
                     * angle "a" with the x-axis, the coordinates for the other
                     * point are: x' = x + (d * cos(a)) y' = y + (d * sin(a)) */
...
struct npc_manaforge_control_consoleAI : public ScriptedAI
{
    ...
    void ResetControlConsoleAndDespawn()
    {
        for (ObjectGuid &guid : m_vSummonGuids)
        {
            if (Creature *summon = m_creature->GetMap()->GetCreature(guid))
                summon->ForcedDespawn(100); // requires delay, to prevent calling spell::cancel, as the
                                            // call can originate from channeled spell finish
        }
    ...
    void DoWaveSpawnForCreature(Creature *pCreature)
    {
        ...
        case NPC_BNAAR_C_CONSOLE:
            if (urand(0, 1))
            {
                if ((pAdd = m_creature->SummonCreature(NPC_SUNFURY_TECH, m_afBanaarTechCoords[0][0],
                                                       m_afBanaarTechCoords[0][1], m_afBanaarTechCoords[0][2], 0.0f,
        ...
        case NPC_CORUU_C_CONSOLE:
            ...
            if (route == 0)
            {
                if ((pAdd = m_creature->SummonCreature(NPC_SUNFURY_TECH, m_afCoruuTechCoords[0][0],
                                                       m_afCoruuTechCoords[0][1], m_afCoruuTechCoords[0][2], 0.0f,
                                                       TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 120000)))
                {
                    pAdd->SetWalk(false);
            ...
            {
                if ((pAdd = m_creature->SummonCreature(NPC_SUNFURY_TECH, m_afCoruuTechCoords[2][0],
                                                       m_afCoruuTechCoords[2][1], m_afCoruuTechCoords[2][2], 0.0f,
                                                       TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 120000)))
                {
                    pAdd->SetWalk(false);
            ...
            {
                if ((pAdd = m_creature->SummonCreature(NPC_SUNFURY_TECH, m_afCoruuTechCoords[4][0],
                                                       m_afCoruuTechCoords[4][1], m_afCoruuTechCoords[4][2], 0.0f,
                                                       TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 120000)))
                {
                    pAdd->SetWalk(false);
        ...
        case NPC_DURO_C_CONSOLE:
            ...
            if (route == 0)
            {
                if ((pAdd = m_creature->SummonCreature(NPC_SUNFURY_TECH, m_afDuroTechCoords[0][0],
                                                       m_afDuroTechCoords[0][1], m_afDuroTechCoords[0][2], 0.0f,
                                                       TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 120000)))
                {
                    pAdd->SetWalk(false);
            ...
            {
                if ((pAdd = m_creature->SummonCreature(NPC_SUNFURY_TECH, m_afDuroTechCoords[5][0],
                                                       m_afDuroTechCoords[5][1], m_afDuroTechCoords[5][2], 0.0f,
                                                       TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 120000)))
                {
                    pAdd->SetWalk(false);
        ...
        case NPC_ARA_C_CONSOLE:
            ...
            {
                for (uint8 i = 0; i < 3; i++)
                    if ((pAdd = m_creature->SummonCreature(NPC_ARA_TECH, m_afAraTechCoords[i][0],
                                                           m_afAraTechCoords[i][1], m_afAraTechCoords[i][2], 0.0f,
                                                           TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 120000)))
                    {
                        pAdd->SetWalk(false);
                ...
                if (!urand(0, 2)) // 1 in 3 chance to spawn a engineer
                    if ((pAdd = m_creature->SummonCreature(NPC_ARA_ENGI, m_afAraTechCoords[0][0],
                                                           m_afAraTechCoords[0][1], m_afAraTechCoords[0][2], 0.0f,
                                                           TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 120000)))
            ...
            else if (route == 1)
            {
                ...
                if (!urand(0, 2)) // 1 in 3 chance to spawn a engineer
                    if ((pAdd = m_creature->SummonCreature(NPC_ARA_ENGI, m_afAraTechCoords[6][0],
                                                           m_afAraTechCoords[6][1], m_afAraTechCoords[6][2], 0.0f,
                                                           TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 120000)))
            ...
            else
            {
                ...
                if (!urand(0, 2)) // 1 in 3 chance to spawn a engineer
                    if ((pAdd = m_creature->SummonCreature(NPC_ARA_ENGI, m_afAraTechCoords[12][0],
                                                           m_afAraTechCoords[12][1], m_afAraTechCoords[12][2], 0.0f,
                                                           TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 120000)))
    ...
    void DoFinalSpawnForCreature(Creature *pCreature)
    {
        ...
        {
        case NPC_BNAAR_C_CONSOLE:
            if ((pAdd = m_creature->SummonCreature(NPC_SUNFURY_TECH, m_afBanaarTechCoords[5][0],
                                                   m_afBanaarTechCoords[5][1], m_afBanaarTechCoords[5][2], 0.0f,
                                                   TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 120000)))
            {
                pAdd->SetWalk(false);
...
bool GossipHello_go_manaforge(Player *pPlayer, GameObject *pGo)
{
    ...
    case 3726: // b'naar
        ...
        {
            pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_ITEM_SHUTDOWN, GOSSIP_SENDER_MAIN,
    ...
    if (uiAction == GOSSIP_ACTION_INFO_DEF + 1)
    {
        ...
        case 3726: // b'naar
            if ((pPlayer->GetQuestStatus(QUEST_SHUTDOWN_BNAAR_ALDOR) == QUEST_STATUS_INCOMPLETE ||
                ...
                pPlayer->HasItemCount(ITEM_BNAAR_ACESS_CRYSTAL, 1))
                pManaforge =
                    pPlayer->SummonCreature(NPC_BNAAR_C_CONSOLE, 2918.95f, 4189.98f, 164.5f, 0.34f,
        ...
        case 3730: // coruu
            if ((pPlayer->GetQuestStatus(QUEST_SHUTDOWN_CORUU_ALDOR) == QUEST_STATUS_INCOMPLETE ||
                ...
                pPlayer->HasItemCount(ITEM_CORUU_ACESS_CRYSTAL, 1))
                pManaforge =
                    pPlayer->SummonCreature(NPC_CORUU_C_CONSOLE, 2423.36f, 2755.72f, 135.5f, 2.14f,
                                            TEMPSPAWN_TIMED_OOC_OR_CORPSE_DESPAWN, 5 * MINUTE * IN_MILLISECONDS, true);
            break;
        case 3734: // duro
            if ((pPlayer->GetQuestStatus(QUEST_SHUTDOWN_DURO_ALDOR) == QUEST_STATUS_INCOMPLETE ||
                ...
                pPlayer->HasItemCount(ITEM_DURO_ACESS_CRYSTAL, 1))
                pManaforge =
                    pPlayer->SummonCreature(NPC_DURO_C_CONSOLE, 2976.48f, 2183.29f, 166.00f, 1.85f,
                                            TEMPSPAWN_TIMED_OOC_OR_CORPSE_DESPAWN, 5 * MINUTE * IN_MILLISECONDS, true);
            break;
        case 3722: // ara
            if ((pPlayer->GetQuestStatus(QUEST_SHUTDOWN_ARA_ALDOR) == QUEST_STATUS_INCOMPLETE ||
                ...
                pPlayer->HasItemCount(ITEM_ARA_ACESS_CRYSTAL, 1))
                pManaforge =
                    pPlayer->SummonCreature(NPC_ARA_C_CONSOLE, 4013.71f, 4028.76f, 195.00f, 1.25f,
                                            TEMPSPAWN_TIMED_OOC_OR_CORPSE_DESPAWN, 5 * MINUTE * IN_MILLISECONDS, true);
            break;
        }
        ...
        if (pManaforge)
        {
                    ...
                    dynamic_cast<npc_manaforge_control_consoleAI *>(pManaforge->AI()))
            {
                ...
                std::list<Creature *> lNearbyWarpMaster; // If these are in the
                                                         // switch, causes error
                ...
                case 3726: // b'naar
                    ...
                    for (std::list<Creature *>::iterator itr = lNearbyWarpMaster.begin();
                         itr != lNearbyWarpMaster.end(); ++itr)
                        (*itr)->AI()->AttackStart(pPlayer);
                    for (std::list<Creature *>::iterator itr = lNearbyWarpEngineers.begin();
                         itr != lNearbyWarpEngineers.end(); ++itr)
                        (*itr)->AI()->AttackStart(pPlayer);
                ...
                case 3730: // coruu
                    ...
                    for (std::list<Creature *>::iterator itr = lNearbyResearchers.begin();
                         itr != lNearbyResearchers.end(); ++itr)
                        (*itr)->AI()->AttackStart(pPlayer);
                    break;
                case 3734: // duro
                    break;
...
struct npc_commander_dawnforgeAI : public ScriptedAI
{
    ...
    bool CanStartEvent(Player *pPlayer)
    {
        ...
        debug_log("SD2: npc_commander_dawnforge event already in progress, need to "
                  "wait.");
        return false;
    }
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        case 4:
            ...
            m_creature->SummonCreature(NPC_PATHALEON_THE_CALCULATOR_IMAGE, 2325.851563f, 2799.534668f, 133.084229f,
                                       6.038996f, TEMPSPAWN_TIMED_DESPAWN, 90000);
            ++m_uiPhase;
            m_uiPhaseTimer = 500;
            break;
        case 5:
...
struct npc_bessyAI : public npc_escortAI
{
    ...
    void WaypointReached(uint32 uiPointId) override
    {
        ...
        case 4:
            ...
            if (Creature *pSpirit =
                    m_creature->SummonCreature(NPC_SEVERED_SPIRIT, 2438.142f, 2200.449f, 101.5952f, 4.63704f,
                                               TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 25000, false, true))
                pSpirit->GetMotionMaster()->MoveWaypoint(1);
            break;
        case 8:
            if (Creature *pSpirit =
                    m_creature->SummonCreature(NPC_SEVERED_SPIRIT, 2279.846f, 2188.402f, 91.61183f, 6.22683f,
                                               TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 25000, false, true))
...
struct npc_maxx_a_million_escortAI : public npc_escortAI
{
    ...
    void WaypointReached(uint32 uiPoint) override
    {
        ...
        case 17:
        case 29:
            if (GameObject *pMachine =
                    GetClosestGameObjectWithEntry(m_creature, GO_DRAENEI_MACHINE, INTERACTION_DISTANCE))
            {
            ...
            if (m_uiSubEventTimer)
            {
                if (m_uiSubEventTimer <= uiDiff)
                {
                    ...
                    case 1: // Wait time before Say
                        if (Creature *pAlley = GetClosestCreatureWithEntry(m_creature, NPC_BOT_SPECIALIST_ALLEY,
...
struct npc_captured_vanguardAI : public npc_escortAI
{
    ...
    {
        // Happens only if the player helps the npc in the fight - otherwise he
        // dies
        if (m_bOfferQuest)
        {
...
struct npc_drijyaAI : public npc_escortAI
{
    ...
    void DoSpawnCreature(uint32 uiEntry)
    {
        if (Creature *pTrigger = m_creature->GetMap()->GetCreature(m_explodeTriggerGuid))
            m_creature->SummonCreature(uiEntry, pTrigger->GetPositionX(), pTrigger->GetPositionY(),
                                       pTrigger->GetPositionZ(), pTrigger->GetOrientation(),
...
{
    npc_saeed_escortAI(Creature *pCreature) : npc_escortAI(pCreature)
    {
        Reset();
    }
            ...
            if (m_uiEventTimer)
            {
                if (m_uiEventTimer <= uiDiff)
                {
                    ...
                    case 6:
                        if (Creature *dimensius = GetClosestCreatureWithEntry(m_creature, NPC_DIMENSIUS, 40.0f))
                        {
                            dimensius->RemoveFlag(UNIT_FIELD_FLAGS,
                                                  UNIT_FLAG_IMMUNE_TO_NPC | UNIT_FLAG_IMMUNE_TO_PLAYER);
                    ...
                    case 7:
                        if (Creature *avenger =
                                GetClosestCreatureWithEntry(m_creature, NPC_PROTECTORATE_AVENGER, 40.0f))
                        {
                            if (Player *player = GetPlayerForEscort())
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (!m_bSpawnsFeeding && m_creature->GetHealthPercent() < 75.0f)
        {
            ...
            for (uint8 i = 0; i < 4; ++i)
            {
                ...
                m_creature->SummonCreature(NPC_SPAWN_OF_DIMENSIUS, fX, fY, fZ, 0, TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN,
                                           120000);
            }
        ...
        if (m_uiRainTimer)
        {
            ...
            {
                if (DoCastSpellIfCan(m_creature, auiShadowRainSpells[m_uiRainIndex], CAST_INTERRUPT_PREVIOUS) ==
                    CAST_OK)
                {
                    m_uiRainIndex = urand(0, 4);
                    m_uiRainTimer = 5000;
...
struct npc_salhadaarAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (m_uiAttackTimer)
        {
            if (m_uiAttackTimer <= uiDiff) // on attack start cast spell that handles everything
            {
                ...
                if (!playerList.empty())
                {
                    ...
                    m_creature->SetFactionTemporary(FACTION_SALHADAAR_HOSTILE, TEMPFACTION_RESTORE_COMBAT_STOP |
        ...
        if (m_creature->IsInCombat())
        {
            ...
            if (!m_uiUsedMirrorImage &&
                m_creature->GetHealthPercent() < 25) // at 25% hp cast mirror image, its two spells
            {
            ...
            else
            {
                ...
                {
                    m_uiGravityFlux = 15000;
                    m_creature->CastSpell(m_creature, SPELL_GRAVITY_FLUX,
                                          TRIGGERED_NONE); // cast gravity flux every 15 seconds
                    return;
                }
                ...
                if (m_uiStasisTimer <= uiDiff)
                {
                    ...
                    {
                        if (m_creature->CastSpell(target, SPELL_STASIS,
                                                  TRIGGERED_NONE) == SPELL_CAST_OK) // cast stasis on random target
                                                                                    // every 22 seconds
...
struct npc_energy_ballAI : public ScriptedAI
{
    ...
    {
        if (pSpell->Id == SPELL_PROTECTORATE_DISRUPTOR) // recast beam after one minute if
                                                        // player doesnt engage mob
        {
...
struct npc_void_conduitAI : public Scripted_NoMovementAI
{
    ...
    void SpellHit(Unit * /*pCaster*/, const SpellEntry *pSpell) override
    {
        if (pSpell->Id == SPELL_PHASE_DISRUPTOR)
            m_uiEvadeTimer = 10000; // custom combat timer, to simulate proper
                                    // leashing of this mob
    }
...
struct npc_scrap_reaverAI : ScriptedPetAI
{
    ...
    void UpdateTimers(const uint32 diff)
    {
        ...
        if (m_areaCheckTimer && !m_despawnTimer) // Area check should not run if
                                                 // despawnTimer is already set
        {
    ...
    void UpdateAI(const uint32 diff) override
    {
        ...
        if (!m_creature->IsAlive()) // should not be needed, IsAlive is checked in
                                    // mangos before calling UpdateAI
            return;

        // UpdateAllies() is done in the generic PetAI in Mangos, but we can't do
        // this from script side. Unclear what side effects this has, but is
        // something to be resolved from Mangos.

        if (m_creature->GetVictim()) // in combat
        {
            if (!m_creature->CanAttack(m_creature->GetVictim()))
            {
                // target no longer valid for pet, so either attack stops or new
                // target are selected doesn't normally reach this, because of how
                // petAi is designed in Mangos. CombatStop are called before this
                // update diff, and then pet will already have no victim.
                ResetPetCombat();
                return;
            }
        ...
        else if (m_creature->GetCharmInfo())
        {
            ...
            if (owner->IsInCombat() && !HasReactState(REACT_PASSIVE))
            {
                ...
                // When mob initiate attack by spell, pet should not start attack
                // before spell landed.
...
struct npc_scrapped_fel_reaverAI : ScriptedAI
{
    ...
    // has to be done in both, because spawns can despawn before event ends
    // (blizzlike) without dying and then respawn
        ...
        if (m_spawnTimer <= 0)
        {
            ...
            if (i != COUNT_SPAWNING_LOCATIONS)
                m_creature->SummonCreature(NPC_ZAXXIS_AMBUSHER, spawningLocations[m_locationCounter][0],
                                           spawningLocations[m_locationCounter][1],
                                           spawningLocations[m_locationCounter][2],
                                           spawningLocations[m_locationCounter][3], TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN,
                                           240000, true, true, m_locationCounter + 1);
        }
        if (m_eventTimer <= 0)
        {
...
struct npc_adyen_the_lightwardenAI : public ScriptedAI
{
    ...
    void ReceiveAIEvent(AIEventType eventType, Unit *sender, Unit * /*invoker*/, uint32 /*miscValue*/) override
    {
        if (eventType == AI_EVENT_CUSTOM_A)
        {
            Creature *ishanah = m_creature->SummonCreature(NPC_ISHANAH, 4866.2f, 3799.016f, 199.141f, 0.4680258f,
                                                           TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 3600000, true, true, 1,
                                                           FACTION_DEATHBLOW);
            ishanah->SetUInt32Value(UNIT_NPC_FLAGS, 0);
            ishanah->SetFlag(UNIT_FIELD_FLAGS,
                             UNIT_FLAG_IMMUNE_TO_NPC); // fix for blizzlike bug
                                                       // that can occur on retail
...
struct npc_kaylaan_the_lostAI : public ScriptedAI
{
    npc_kaylaan_the_lostAI(Creature *creature)
        : ScriptedAI(creature), m_deathPrevented(false), m_avengersShieldTimer(0), m_burningLightTimer(0),
          m_healTimer(0), m_holySlamTimer(0)
    {
        m_creature->GetCombatManager().SetLeashingDisable(true);
    }
...
bool AreaTrigger_at_socrethar_seat(Player *player, AreaTriggerEntry const * /*at*/)
{
    if (player->IsCurrentQuest(QUEST_DEATHBLOW_TO_THE_LEGION))
    {
        ...
        {
            Creature *adyen = player->SummonCreature(NPC_ADYEN_THE_LIGHTWARDEN, 4805.796f, 3774.112f, 210.6172f,
                                                     5.550147f, TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 3600000, true,
                                                     false, 1, FACTION_DEATHBLOW, 0, true);
...
bool GossipHello_npc_adyen_the_lightwarden(Player *player, Creature *creature)
{
    ...
    else
    {
        ...
        {
            Creature *socrethar = ((ScriptedInstance *)creature->GetMap()->GetInstanceData())
                                      ->GetSingleCreatureFromStorage(NPC_SOCRETHAR);
...
struct Soulbind : public SpellScript
{
    void OnEffectExecute(Spell *spell, SpellEffectIndex /*effIdx*/) const override
    {
        ...
        caster->CastSpell(target, 36141,
                          TRIGGERED_OLD_TRIGGERED); // spell id is in spell effect value
    }
};
...
struct UltraDeconsolodationZapper : public SpellScript
{
    void OnEffectExecute(Spell *spell, SpellEffectIndex effIdx) const override
    {
        ...
        target->CastSpell(nullptr, 34427,
                          TRIGGERED_OLD_TRIGGERED); // Ethereal Teleport
    }
};
...
struct RingOfFlame : public AuraScript
{
    void OnApply(Aura *aura, bool apply) const override
    {
        if (!apply && aura->GetRemoveMode() != AURA_REMOVE_BY_EXPIRE)
            ...
            aura->GetTarget()->CastSpell(nullptr, 35995,
                                         TRIGGERED_OLD_TRIGGERED); // Immolation
    }
};
