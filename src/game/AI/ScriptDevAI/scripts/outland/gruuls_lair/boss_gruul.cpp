...
SDName: Boss_Gruul
SD%Complete: 60
SDComment: Ground Slam need further development (knock back effect and shatter
effect must be added to mangos) SDCategory: Gruul's Lair EndScriptData */
...
enum
{
    ...
    SPELL_GROUND_SLAM = 33525, // AoE Ground Slam applying Ground Slam to
                               // everyone with a script effect (most likely the
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case GRUUL_ACTION_GROUND_SLAM: {
            if (DoCastSpellIfCan(nullptr, SPELL_GROUND_SLAM_DUMMY) == CAST_OK)
            {
                ...
                SetActionReadyStatus(GRUUL_ACTION_SHATTER,
                                     true); // top priority, blocked by stun
                m_lookAround = true;
                return;
            }
            return;
...
struct HurtfulStrikePrimer : public SpellScript
{
    ...
    void OnEffectExecute(Spell *spell, SpellEffectIndex effIdx) const override
    {
        ...
        for (auto &targetInfo : targetInfo)
        {
            ...
            if (caster->getThreatManager().getThreat(caster->GetMap()->GetPlayer(targetInfo.targetGUID)) >
                    caster->getThreatManager().getThreat(target) ||
                target == caster->GetVictim())
                target = caster->GetMap()->GetPlayer(targetInfo.targetGUID);
        }

        caster->CastSpell(target, 33813,
