...
/*
    Small wrapper class that reduces the amount of code needed to use
   CombatActions and adds proper spell action reset functionality
*/
class CombatAI : public ScriptedAI
{
  public:
    ...
    void StopTargeting(bool state)
    {
        m_stopTargeting = state;
    }
    void OnTaunt() override;

    void AddOnKillText(int32 text);
    template <typename... Targs> void AddOnKillText(int32 value, Targs... fargs)
    {
        AddOnKillText(value);
        AddOnKillText(fargs...);
    }
