...
struct boss_archimondeAI : public CombatAI
{
    ...
    // void ReceiveAIEvent(AIEventType eventType, Unit* sender, Unit* invoker,
    // uint32 miscValue) override
    //{
        ...
        case ARCHIMONDE_ACTION_FINGER_OF_DEATH_COOLUP:
                    ...
                    m_creature->SelectAttackingTarget(ATTACKING_TARGET_NEAREST_BY, 0, nullptr, SELECT_FLAG_PLAYER))
            {
                if (!m_creature->CanReachWithMeleeAttack(closest))
                {
                    ...
                    {
                        ResetCombatAction(action, GetSubsequentActionTimer(action));
                        SetActionReadyStatus(ARCHIMONDE_ACTION_FINGER_OF_DEATH,
                                             true); // when GCD expires, cast this
                    }
                }
            }
            return;
        case ARCHIMONDE_ACTION_FINGER_OF_DEATH:
                    ...
                    m_creature->SelectAttackingTarget(ATTACKING_TARGET_NEAREST_BY, 0, nullptr, SELECT_FLAG_PLAYER))
            {
                if (!m_creature->CanReachWithMeleeAttack(closest))
                {
                    ...
                    SetActionReadyStatus(action,
                                         false); // if someone is tanking again, disable
            }
            return;
        case ARCHIMONDE_ACTION_SOUL_CHARGE:
            uint32 spellId = 0;
            ...
            ResetCombatAction(action,
                              GetSubsequentActionTimer(action)); // does not seem to reset cd only on spell cast
            if (spellId)
        ...
        if (m_leashCheckTimer)
        {
            if (m_leashCheckTimer <= diff)
            {
                ...
                if (m_creature->GetDistance2d(pos.GetPositionX(), pos.GetPositionY()) >= 155.f ||
                    (pos.GetPositionX() < 5534.752f && pos.GetPositionX() > 5381.371f &&
                     pos.GetPositionY() < -3507.099f && pos.GetPositionY() > -3587.244f))
                {
                    SetActionReadyStatus(ARCHIMONDE_ACTION_HAND_OF_DEATH, true);
                    m_leashCheckTimer = 0;
                    DoScriptText(SAY_SLAY3,
                                 m_creature); // seems to be both used as leash
                                              // yell and slay yell
                }
                else
                    m_leashCheckTimer = 2000;
    ...
    void UpdateAI(const uint32 diff) override
    {
        ...
        if (m_drainNordrassilTimer)
        {
            ...
            {
                m_creature->RemoveAurasDueToSpell(SPELL_SOUL_CHARGE_YELLOW_CHARGE); // workaround for archi not
                                                                                    // despawning after wipe
...
/* This is the script for the Doomfire Spirit Mob. This mob controls the
 * doomfire npc and allows it to move randomly around the map. */
struct npc_doomfire_targetingAI : public ScriptedAI
{
    npc_doomfire_targetingAI(Creature *creature) : ScriptedAI(creature)
    {
        Reset();
    }
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        if (m_uiChangeTargetTimer < uiDiff)
        {
            ...
            Position pos =
                m_creature->GetFirstRandomAngleCollisionPosition(8.f, nextOrientation); // both orientation and distance
                                                                                        // verified with sniffs
