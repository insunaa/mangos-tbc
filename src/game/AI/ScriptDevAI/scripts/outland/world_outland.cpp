...
// Note: Official script has 2 bunnies that randomly run around with seemingly
// no purpose - entries 23424 and 23245
...
struct world_map_outland : public ScriptedMap, public TimerManager
{
                ...
                if (m_shartuulShieldPercent == 750 || m_shartuulShieldPercent == 375)
                {
                    if (Creature *overseer = GetSingleCreatureFromStorage(NPC_OVERSEER_SHARTUUL))
                        if (Unit *demon = GetCurrentDemon())
                            DoScriptText(m_shartuulShieldPercent == 750 ? SAY_SECOND_HAMMER : SAY_FIFTH_HAMMER,
                                         overseer, demon->GetOwner());
                }
                                    ...
                                    WorldObject const *conditionSource, uint32 conditionSourceType) const override
    {
        ...
        script_error_log("world_map_outland::CheckConditionCriteriaMeet called with "
                         "unsupported "
                         "Id %u. Called with param plr %s, src %s, condition source type %u",
    ...
    {
        if (!m_apprentice) // his grid wasnt loaded yet and autoload isnt on for
                           // performance reasons
            return;
        ...
        m_apprentice->SummonCreature(NPC_SKYGUARD_AETHER_TECH, bashirSpawnPositions[0][0], bashirSpawnPositions[0][1],
                                     bashirSpawnPositions[0][2], bashirSpawnPositions[0][3],
        ...
        ranger = m_apprentice->SummonCreature(NPC_SKYGUARD_RANGER, bashirSpawnPositions[2][0],
                                              bashirSpawnPositions[2][1], bashirSpawnPositions[2][2],
                                              bashirSpawnPositions[2][3], TEMPSPAWN_CORPSE_TIMED_DESPAWN, 1000, true);
        ...
        m_apprentice->SummonCreature(NPC_SKYGUARD_LIEUTENANT, bashirSpawnPositions[3][0], bashirSpawnPositions[3][1],
                                     bashirSpawnPositions[3][2], bashirSpawnPositions[3][3],
                                     TEMPSPAWN_CORPSE_TIMED_DESPAWN, 1000, true);
        ranger = m_apprentice->SummonCreature(NPC_SKYGUARD_RANGER, bashirSpawnPositions[4][0],
                                              bashirSpawnPositions[4][1], bashirSpawnPositions[4][2],
                                              bashirSpawnPositions[4][3], TEMPSPAWN_CORPSE_TIMED_DESPAWN, 1000, true);
    ...
    {
        player->SummonCreature(NPC_SKYGUARD_LIEUTENANT, bashirCustomSpawnPositions[0][0],
                               bashirCustomSpawnPositions[0][1], bashirCustomSpawnPositions[0][2],
                               bashirCustomSpawnPositions[0][3], TEMPSPAWN_CORPSE_TIMED_DESPAWN, 1000, true);
        Creature *ranger =
            player->SummonCreature(NPC_SKYGUARD_RANGER, bashirCustomSpawnPositions[1][0],
                                   bashirCustomSpawnPositions[1][1], bashirCustomSpawnPositions[1][2],
                                   bashirCustomSpawnPositions[1][3], TEMPSPAWN_CORPSE_TIMED_DESPAWN, 1000, true);
        ranger->AI()->SendAIEvent(AI_EVENT_CUSTOM_A, ranger, ranger, 2);
        ...
        ranger->AI()->SendAIEvent(AI_EVENT_CUSTOM_A, ranger, ranger, 3);
        Creature *tech =
            player->SummonCreature(NPC_SKYGUARD_AETHER_TECH, bashirCustomSpawnPositions[3][0],
                                   bashirCustomSpawnPositions[3][1], bashirCustomSpawnPositions[3][2],
                                   bashirCustomSpawnPositions[3][3], TEMPSPAWN_CORPSE_TIMED_DESPAWN, 1000, true);
        ...
        case BASHIR_PHASE_ALL_VENDORS_SPAWNED:
            vendor =
                player->SummonCreature(NPC_AETHER_TECH_MASTER, bashirCustomSpawnPositions[7][0],
        ...
        case BASHIR_PHASE_3:
            vendor =
                player->SummonCreature(NPC_AETHER_TECH_ADEPT, bashirCustomSpawnPositions[6][0],
        ...
        case BASHIR_PHASE_2:
            vendor =
                player->SummonCreature(NPC_AETHER_TECH_ASSISTANT, bashirCustomSpawnPositions[5][0],
    ...
    void SpawnRandomBashirMob()
    {
        ...
        Creature *tech = GetSingleCreatureFromStorage(NPC_SKYGUARD_AETHER_TECH); // Has to be alive during this so we
                                                                                 // can use him for summoning
        uint32 entry;
        switch (data->phaseId)
        {
        case BASHIR_PHASE_1:
        ...
        Creature *mob =
            tech->SummonCreature(entry, data->x, data->y, data->z, data->ori, TEMPSPAWN_DEAD_DESPAWN, 0, true);
        switch (entry)
        {
    ...
    void SpawnTowerDefenders()
    {
        ...
        {
            Creature *mob = tech->SummonCreature(NPC_BASHIR_SUBPRIMAL, data.x, data.y, data.z, data.ori,
    ...
    void StartTransitionPhase()
    {
        ...
        case BASHIR_PHASE_TRANSITION_1:
            vendor =
                tech->SummonCreature(NPC_AETHER_TECH_ASSISTANT, bashirSpawnPositions[5][0], bashirSpawnPositions[5][1],
        ...
        case BASHIR_PHASE_TRANSITION_3:
            vendor =
                tech->SummonCreature(NPC_AETHER_TECH_MASTER, bashirSpawnPositions[7][0], bashirSpawnPositions[7][1],
    ...
    void StartPhase(BashirPhases phase)
    {
        ...
        case BASHIR_PHASE_TRANSITION_3:
            ...
            break;
        case BASHIR_PHASE_ALL_VENDORS_SPAWNED:
            ResetTimer(BASHIR_ACTION_OUTRO,
                       (4 * 60 + 40) * IN_MILLISECONDS); // 4 minutes 40 seconds
            break;
        default:
            break;
        }
    ...
    void SpawnBoss(BashirPhases phase)
    {
        ...
        case BASHIR_PHASE_1:
            mob = tech->SummonCreature(NPC_BASHIR_FLESH_FIEND, bashirSpawnPositions[8][0], bashirSpawnPositions[8][1],
                                       bashirSpawnPositions[8][2], bashirSpawnPositions[8][3],
        ...
        case BASHIR_PHASE_2:
            ...
            mob = tech->SummonCreature(NPC_DISRUPTOR_TOWER, bashirSpawnPositions[9][0], bashirSpawnPositions[9][1],
                                       bashirSpawnPositions[9][2], bashirSpawnPositions[9][3],
                                       TEMPSPAWN_CORPSE_TIMED_DESPAWN, 2 * 60 * IN_MILLISECONDS, true);
            mob->CastSpell(nullptr, SPELL_SPIRIT_SPAWN_IN, TRIGGERED_NONE);
        ...
        case PHASE_2_FELGUARD_DEGRADER_BOSS: {
            ...
            for (ObjectGuid guid : m_fires)
            {
                if (GameObject *fire = instance->GetGameObject(guid))
                {
                    ...
                    fire->SetRespawnTime(fire->GetRespawnDelay()); // database has timers for
                                                                   // them already
                    fire->Refresh();
                }
            }
    ...
    void HandleShartuulAddSpawns()
    {
        ...
        if (m_shartuulPhase == PHASE_3_DOOMGUARD_PUNISHER_ADDS)
        {
            ...
            if (m_shartuulWave == 0 || m_shartuulWave == 3 || m_shartuulWave == 7)
            {
                ...
                case 7: {
                    if (Creature *demon = GetCurrentDemon())
                        demon->SummonCreature(NPC_SHIVAN_ASSASSIN, m_spawnPosition.x, m_spawnPosition.y,
                                              m_spawnPosition.z, m_spawnPosition.o,
                                              TEMPSPAWN_TIMED_OOC_OR_CORPSE_DESPAWN, 300000, true);
    ...
    void HandleShartuulSpawnSequence()
    {
        ...
        {
        case 0: {
            WorldObject::SummonCreature(TempSpawnSettings(nullptr, NPC_INVISIBLE_STALKER_LARGE_AOI,
                                                          shartuulSpawnPosition.x, shartuulSpawnPosition.y,
                                                          shartuulSpawnPosition.z, shartuulSpawnPosition.o,
                                                          TEMPSPAWN_TIMED_DESPAWN, 27000),
                                        instance);
            WorldObject::SummonCreature(TempSpawnSettings(nullptr, NPC_LEGION_RING_EVENT_INVISMAN_LG,
                                                          shartuulSpawnPosition.x, shartuulSpawnPosition.y,
                                                          shartuulSpawnPosition.z + 13.f, shartuulSpawnPosition.o,
                                                          TEMPSPAWN_TIMED_DESPAWN, 27000),
                                        instance);
            timer = 2000;
            break;
        ...
        case 10: {
            if (Creature *shartuul = GetSingleCreatureFromStorage(NPC_SHARTUUL))
                if (Creature *cannon =
                    ...
                    shartuul->SummonCreature(NPC_DREADMAW, cannon->GetPositionX(), cannon->GetPositionY(),
                                             cannon->GetPositionZ(), cannon->GetOrientation(),
                                             TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 60000, true);
            timer = 3000;
            break;
        }
        case 11: {
    ...
    void OnCreatureCreate(Creature *creature) override
    {
        ...
        case NPC_WYRM_FROM_BEYOND:
            if (Creature *creature =
                    instance->GetCreature(m_npcEntryGuidStore[NPC_OSCILLATING_FREQUENCY_SCANNER_BUNNY]))
        ...
        case NPC_AETHER_TECH_MASTER:
            ...
            case NPC_SKYGUARD_AETHER_TECH:
            case NPC_SKYGUARD_LIEUTENANT:
                m_npcEntryGuidStore[creature->GetEntry()] = creature->GetObjectGuid(); // Note: we want last ranger to
                                                                                       // be stored
                break;
            }
            break;
        ...
        case NPC_FURYWING:
        case NPC_INSIDION:
            if (creature->IsTemporarySummon()) // Only dragons summoned by the
                                               // player (by using the respective
                                               // egg gameobjects) should have
    ...
    void OnCreatureRespawn(Creature *creature) override
    {
        ...
        case NPC_MOGOR:
            ...
            creature->GetCombatManager().SetLeashingCheck([](Unit *unit, float /*x*/, float /*y*/, float /*z*/) {
                return unit->GetDistance(-707.214f, 7877.495f, 45.191f,
                                         DIST_CALC_NONE) > 2500.f; // squared
            });
            if (creature->GetEntry() != NPC_MOGOR)
                m_lastRingOfBlood = creature->GetObjectGuid();
            break;
        case NPC_ETHEREUM_PRISONER: // Gameobject should close when Ethereum
                                    // Prisoner respawns
    ...
    void Update(const uint32 diff) override
    {
        ...
        if (m_shartuulTimer <= diff)
        {
            ...
            {
                bool fail = false;
                if (m_shartuulPhase == PHASE_1_FELGUARD_DEGRADER_ADDS ||
                    m_shartuulPhase == PHASE_3_DOOMGUARD_PUNISHER_ADDS)
    ...
    void ShowChatCommands(ChatHandler *handler) override
    {
        handler->SendSysMessage("This instance supports the following commands:\n bashir "
                                "(0,1,2,3,4,5,6,7) starts event at stage respectively - start event, "
                                "start phase 1, finish phase 1,"
                                "start phase 2, finish phase 2, start phase 3, finish phase 3, "
                                "despawn "
    ...
    void ExecuteChatCommand(ChatHandler *handler, char *args) override
    {
        ...
        if (val == "bashir")
        {
            ...
            switch (startPhase)
            {
            case BASHIR_PHASE_START: // Start event - equivalent to starting on
                                     // its own
                StartBashirLandingEvent();
                break;
        ...
        else if (val == "shartuulitem")
        {
            ...
            uint8 msg =
                player->CanStoreNewItem(NULL_BAG, NULL_SLOT, dest, ITEM_CRYSTALFORGED_DARKRUNE, 1, &noSpaceForCount);
            if (msg == EQUIP_ERR_OK)
            {
