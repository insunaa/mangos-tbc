...
enum
{
    ...
    // SPELL_LAVA_BREATH           = 19272,                    // Triggered by
    // SPELL_FRENZY (19451)
    SPELL_FRENZY = 19451,
    SPELL_MAGMASPIT = 19449, // This is actually a buff he gives himself
    SPELL_PANIC = 19408,
    SPELL_LAVABOMB = 19411,      // This calls a dummy server side effect that cast
                                 // spell 20494 to spawn GO 177704 for 30s
    SPELL_LAVABOMB_MANA = 20474, // This calls a dummy server side effect that
                                 // cast spell 20495 to spawn GO 177704 for 60s
                                 // SPELL_CONFLAGRATION         = 19428                     // Trap spell for
                                 // GO 177704
};
...
struct boss_magmadarAI : public CombatAI
{
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case MAGMADAR_LAVABOMB: {
            if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_LAVABOMB,
                                                                 SELECT_FLAG_POWER_NOT_MANA))
