...
enum
{
    ...
    TYPE_NETHEKURSE = 0,
    TYPE_OMROGG = 1,
    TYPE_BLADEFIST = 2, // Note: if players skip Omrogg and go straight to
    ...
    // I'm not sure if these texts are used at the execution but this is most
    // likely they are used to
    SAY_KARGATH_EXECUTE_ALLY = -1540049,
    SAY_KARGATH_EXECUTE_HORDE = -1540050,

    // AT_NETHEKURSE               = 4524,                  // Area trigger used
...
class instance_shattered_halls : public ScriptedInstance
{
  ...
  private:
    ...
    bool m_bZealotOneOrTwo;      // delay is different whether spawning first or
                                 // second zealot in wave
    ...
    uint32 m_uiShootFlamingArrowTimer_1; // timer for fire arrow ability (left
                                         // archer)
