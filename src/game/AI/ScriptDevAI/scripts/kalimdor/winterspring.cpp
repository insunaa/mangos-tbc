...
{
    npc_ranshallaAI(Creature *pCreature) : npc_escortAI(pCreature), DialogueHelper(aIntroDialogue)
    {
        Reset();
    }
    ...
    {
        // If Ranshalla cast the torch lightening spell for too long she will
        // trigger SPELL_RANSHALLA_DESPAWN (quest failed and despawn)
    ...
    {
        // Check if we are using the fire or the altar and remove the no_interact
        // flag
        if (bIsAltarWaypoint)
        {
    ...
    void DoSummonPriestess()
    {
        ...
        if (Creature *pPriestess = m_creature->SummonCreature(
                NPC_PRIESTESS_ELUNE, aWingThicketLocations[0].m_fX, aWingThicketLocations[0].m_fY,
                aWingThicketLocations[0].m_fZ, aWingThicketLocations[0].m_fO, TEMPSPAWN_CORPSE_DESPAWN, 0))
        {
        ...
        if (Creature *pPriestess = m_creature->SummonCreature(
                NPC_PRIESTESS_ELUNE, aWingThicketLocations[1].m_fX, aWingThicketLocations[1].m_fY,
                aWingThicketLocations[1].m_fZ, aWingThicketLocations[1].m_fO, TEMPSPAWN_CORPSE_DESPAWN, 0))
        {
            // Left priestess should have a distinct move point because she is the
            // one who starts the dialogue at point reach
    ...
    void SummonedMovementInform(Creature *pSummoned, uint32 uiType, uint32 uiPointId) override
    {
        ...
        // Start the dialogue when the priestess reach the altar (they should both
        // reach the point in the same time)
    ...
    void JustDidDialogueStep(int32 iEntry) override
    {
        ...
        case SAY_PRIESTESS_ALTAR_9:
            ...
            if (Creature *pPriestess = m_creature->GetMap()->GetCreature(m_firstPriestessGuid))
                pPriestess->GetMotionMaster()->MovePoint(0, aWingThicketLocations[6].m_fX,
                                                         aWingThicketLocations[6].m_fY, aWingThicketLocations[6].m_fZ);
            break;
        case SAY_PRIESTESS_ALTAR_13:
            // summon the Guardian of Elune
            if (Creature *pGuard = m_creature->SummonCreature(
                    NPC_GUARDIAN_ELUNE, aWingThicketLocations[2].m_fX, aWingThicketLocations[2].m_fY,
                    aWingThicketLocations[2].m_fZ, aWingThicketLocations[2].m_fO, TEMPSPAWN_CORPSE_DESPAWN, 0))
            {
        ...
        case SAY_VOICE_ALTAR_15:
            ...
            if (Creature *pPriestess = m_creature->GetMap()->GetCreature(m_secondPriestessGuid))
            {
                ...
                pPriestess->GetMotionMaster()->MovePoint(0, aWingThicketLocations[7].m_fX,
                                                         aWingThicketLocations[7].m_fY, aWingThicketLocations[7].m_fZ);
            }
        ...
        case SAY_PRIESTESS_ALTAR_20:
            ...
            if (Creature *pPriestess = m_creature->GetMap()->GetCreature(m_firstPriestessGuid))
            {
                pPriestess->GetMotionMaster()->MovePoint(0, aWingThicketLocations[0].m_fX,
        ...
        case SAY_PRIESTESS_ALTAR_21:
            ...
            if (Creature *pPriestess = m_creature->GetMap()->GetCreature(m_secondPriestessGuid))
            {
                pPriestess->GetMotionMaster()->MovePoint(0, aWingThicketLocations[1].m_fX,
    ...
    void UpdateEscortAI(const uint32 uiDiff) override
    {
        ...
        if (m_uiDelayTimer)
        {
            if (m_uiDelayTimer <= uiDiff)
            {
                ...
                SetEscortPaused(false);
                // Spell Ranshalla Waiting applies a root aura that triggers an
                // internal timer in the waypoint movement manager We need to
                // manually set the movement manager to the next waypoint in order
                // to reset the timer unless the NPC will wait 3 min before moving
                // again
...
struct npc_artoriusAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        {
            m_uiDemonic_Doom_Timer = 7500;
            // only attempt to cast this once every 7.5 seconds to give the hunter
            // some leeway LOWER max range for lag...
