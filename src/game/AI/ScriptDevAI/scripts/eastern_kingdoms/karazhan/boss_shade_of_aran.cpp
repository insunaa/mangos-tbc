...
SD%Complete: 95
SDComment: When drinking mana, it should remove all negative damage auras and
should sit. Timers may need adjustments. SDCategory: Karazhan EndScriptData */

/* Pre-nerf Changes
Add Dragon's Breath ability - used shortly after Flame Wreath dissipates
(random target in melee range). Patch_2.1.0 - Shade of Aran will no longer cast
Dragon's Breath.
*/
...
enum
{
    ...
    // SPELL_COMBUSTION          = 29977,                   // spell not
    // confirmed SPELL_PRESENCE_OF_MIND    = 29976,                   // spell
    // not confirmed SPELL_WATER_BREAK         = 39177,                   //
    // purpose unk
...
struct boss_aranAI : public CombatAI
{
    boss_aranAI(Creature *creature)
        : CombatAI(creature, ARAN_ACTION_MAX),
          m_instance(static_cast<instance_karazhan *>(creature->GetInstanceData())), m_atiesh(false)
    {
...
struct SummonBlizzard : public SpellScript
{
    void OnEffectExecute(Spell *spell, SpellEffectIndex /*effIdx*/) const override
    {
        if (Unit *target = spell->GetUnitTarget())
            target->CastSpell(nullptr, 29952, TRIGGERED_OLD_TRIGGERED, nullptr, nullptr,
