...
struct boss_high_astromancer_solarianAI : public CombatAI
{
    ...
    {
        // select two different numbers between 0 and 7 so we will get different
        // spawn points for the spotlights
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case SOLARIAN_SPLIT_PHASE: {
            ...
            // After these 50 seconds she portals to the middle of the room and
            // disappears, leaving 3 light portals behind. ToDo: check if there
            // are some spells involved in this event!
            SetCombatMovement(false);
            SetMeleeEnabled(false);
            SetCombatScriptStatus(true);
...
#ifdef PRENERF_2_0_3
            if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 1, SPELL_WOTA_MAIN_CAST,
                                                                 SELECT_FLAG_PLAYER))
                if (DoCastSpellIfCan(target, SPELL_WOTA_MAIN_CAST) == CAST_OK)
                    ResetCombatAction(action, urand(25000, 30000));
#else
            if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 1,
                                                                 SPELL_WRATH_OF_THE_ASTROMANCER, SELECT_FLAG_PLAYER))
...
#ifdef PRENERF_2_0_3
            // She casts this spell every 45 seconds. It is a kind of Moonfire
            // spell, which she strikes down on the whole raid simultaneously. It
            // hits everyone in the raid for 2280 to 2520 arcane damage.
        ...
        if (!apply)
            if (aura->GetRemoveMode() == AURA_REMOVE_BY_EXPIRE)
                aura->GetTarget()->CastSpell(
                    aura->GetTarget(),
                    aura->GetSpellProto()->CalculateSimpleValue(SpellEffectIndex(aura->GetEffIndex() + 1)),
