...
double Moon::phase(int mon, int day, int year)
{
    ...
    if (!(jd = Calendar2Julian(mon, day, year)))
        return (-1.0);
...
double Moon::flmoon(int n, int nph, long &jd, double &frac)
{
    ...
    // Notice that 2415020.75933 is used a referce epochs for mean time of lunar
    // cycle which is 1900 Jan 1 at 6:13 AM And 29.53058868 is the "Synodic
    // month" or the mean time in days between new moons.
    double xtra = 0.75933 + (1.53058868 * lunar_cycles) + (((1.78e-4) - (1.55e-7) * t) * t2);

    // Ok 2415020 is Julian date 1899 at noon + .75933 is the Julian date of a
    // new moon 28 + 1.53058868 is mean time of a lunar cycle , and 7 close
    // to 7.38264717 a lunar phase but unlike the first two the tractional part
    // dose not seem to be accounted for.
    jd = 2415020 + (28L * n) + (7L * nph);

    // Looks like this is all being done to adjust the variations in the maen
    // lunar cycle of 29.53058868 Which from what I understand is due the moons
    // orbit in relationship to the earths orbit around the sun.
    if (nph == 0 || nph == 2) // New or Full - sun earth moon inline
        xtra += ((0.1734 - 3.93e-4 * t) * sin(RAD * sun_anomaly)) - (0.4068 * sin(RAD * moon_anomaly));
    else // 1st quarder last quarter - moon 90 deegree left or right of the earth
         // and suns line
