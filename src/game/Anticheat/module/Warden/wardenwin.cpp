...
enum WorldEnables
{
    TerrainDoodads = 0x1,                 // default, toggled by sub at 0x673130, which is never
                                          // called.  should always be set
    Terrain = 0x2,                        // default, toggled by sub at 0x6730F0, which is never called.
    ...
    TerrainDoodadCollisionVisuals = 0x80, // toggled by sub at 0x6731C0, which is
                                          // never called.  should never be set
    MapObjects = 0x100,                   // default, toggled by sub at 0x673430, which is never
                                          // called.  should always be set
    MapObjectLighting = 0x200,            // default, toggled by sub at 0x673360, which is
                                          // never called.  should always be set
    FootPrints = 0x400,                   // showfootprints console var
    MapObjectTextures = 0x800,            // default, toggled by sub at 0x6733A0, which is
                                          // never called.  should always be set
    PortalDisplay = 0x1000,               // toggled by sub at 0x673470, which is never called.
                                          // should never be set
    PortalVisual = 0x2000,                // toggled by sub at 0x6734B0, which is never called.
                                          // should never be set
    ...
    CrappyBatches = 0x20000,              // toggled by sub at 0x6733E0, which is never
                                          // called. should never be set
    ZoneBoundaryVisuals = 0x40000,        // zoneBoundary disabled console command (should never be set,
                                          // also sends CMSG_ZONE_MAP, sub at 0x673850)
    BSPRender = 0x80000,                  // toggled by sub at 0x6730A0, which is never called.
                                          // should never be set
    ...
    TerrainDoodadAABoxVisuals = 0x400000, // toggled by sub at 0x673170, which is
                                          // never called.  should never be set
    TrilinearFiltering = 0x800000,        // trilinear console var
    Water = 0x1000000,                    // default, toggled by sub at 0x673670, which is never
                                          // called.  should always be set
    ...
    Prohibited = (TerrainDoodadCollisionVisuals | CrappyBatches | ZoneBoundaryVisuals | BSPRender | ShowQuery |
                  TerrainDoodadAABoxVisuals | Unknown6737F9 | Unknown673820),
};
...
// this function assumes that the given code begins with a valid instruction.
// in other words, that it does not begin in random data or in the middle of an
// instruction.
...
#define LSTRIP(c, l)                                                                                                   \
    ...
    do
    {
        ...
        // At the start of this loop we re-assume the entry condition that the
        // code begins at a valid instruction.
        ...
        // xchg one register with another.  in wrobot this is always
        ...
        // short conditional jumps that always are followed by far jumps to the
        // same place: "JE", "JZ", "JNE", "JNZ", "JG", "JNLE", "JGE", "JNL",
        // "JNGE", "JLE", "JNG", "JO", "JNO", "JP", "JPE", "JNP", "JPO", "JS"
        else if (code[0] == 0x70 || code[0] == 0x71 || code[0] == 0x74 || code[0] == 0x75 || code[0] == 0x78 ||
                 code[0] == 0x7A || code[0] == 0x7B || code[0] == 0x7C || code[0] == 0x7D || code[0] == 0x7E ||
                 code[0] == 0x7F)
        {
        ...
        // these are instructions that are not part of the obfuscation.  so for
        // now we will skip them and deobfuscate whatever comes after them.  this
        // will let us build up deobfuscated code. pushfd.  this is currently not
        // part of the obfuscation of wrobot but rather it appears to always be
        // the first non-obfuscated opcode.  therefore, lets skip it and
        // debofuscate what comes after it.
...
bool ValidateEndSceneHook(const std::vector<uint8> &code)
{
    ...
    sLog.outBasic("WARDEN: Deobfuscate debug.  Original code size: %lu "
                  "deobfuscated size: %lu Code:\n%s",
                  uint64(code.size()), uint64(copy.size()), str.str().c_str());

    // wrobot's deobfuscated endscene hook begins with pushfd, pushad.  if thats
    // what this starts with, assume it is wrobot, regardless of what comes after
    // it.
...
void WardenWin::LoadScriptedScans()
{
    ...
    auto const wardenSysInfo2 = std::make_shared<WindowsScan>(
        ...
        [](const Warden *warden, std::vector<std::string> &, ByteBuffer &scan) {
            ...
            scan << static_cast<uint8>(wardenWin->GetModule()->opcodes[READ_MEMORY] ^ wardenWin->GetXor())
                 << static_cast<uint8>(0) << wardenWin->_sysInfo.dwOemId + sOfsWardenWinSysInfo
        ...
        [](const Warden *warden, ByteBuffer &buff) {
            ...
            // for classic, tbc, and wotlk, the architecute should never be
            // anything other than x86 (0)
            if (!!wardenWin->_sysInfo.wProcessorArchitecture)
            {
                sLog.outBasic("WARDEN: Incorrect architecture reported (%u) for "
                              "account %u ip %s",
            ...
            // for classic, tbc, and wotlk, the cpu type should never be anything
            // other than i386, i486, or pentium (i586)
            if (wardenWin->_sysInfo.dwProcessorType != 386 && wardenWin->_sysInfo.dwProcessorType != 486 &&
            ...
            sLog.outDebug("WARDEN: Account %u IP %s read system information "
    ...
    auto const wardenSysInfo1 = std::make_shared<WindowsScan>(
        ...
        [](const Warden *warden, std::vector<std::string> &, ByteBuffer &scan) {
            ...
            scan << static_cast<uint8>(wardenWin->GetModule()->opcodes[READ_MEMORY] ^ wardenWin->GetXor())
                 << static_cast<uint8>(0) << wardenWin->_wardenAddress + sOfsWardenSysInfo
    ...
    sWardenScanMgr.AddWindowsScan(std::make_shared<WindowsScan>(
        ...
        [](const Warden *warden, std::vector<std::string> &, ByteBuffer &scan) {
            ...
            scan << static_cast<uint8>(wardenWin->GetModule()->opcodes[READ_MEMORY] ^ wardenWin->GetXor())
                 << static_cast<uint8>(0) << sOfsWardenModule << static_cast<uint8>(sizeof(wardenWin->_wardenAddress));
        },
        // checker
        [wardenSysInfo1](const Warden *warden, ByteBuffer &buff) {
            ...
            if (!!result)
            {
                sLog.outBasic("WARDEN: Failed to read s_moduleInterface from "
                              "account %u ip %s",
                              wardenWin->_session->GetAccountId(), wardenWin->_session->GetRemoteAddress().c_str());
        ...
        sizeof(uint8) + sizeof(uint8) + sizeof(uint32) + sizeof(uint8), sizeof(uint8) + sizeof(uint32), "Warden locate",
        InitialLogin | WinAllBuild));
    ...
    sWardenScanMgr.AddWindowsScan(std::make_shared<WindowsMemoryScan>(
        ...
        [](const Warden *warden, ByteBuffer &buff) {
            ...
            {
                sLog.outBasic("WARDEN: CWorld::enables expected 0x%lx prohibited "
                              "0x%lx received 0x%lx",
    ...
    sWardenScanMgr.AddWindowsScan(std::make_shared<WindowsScan>(
        ...
        [](const Warden *warden, std::vector<std::string> &, ByteBuffer &scan) {
            ...
            // NOTE: the order of these two scans is important because the client
            // will not perform the checks at the same time, and we want to safely
            // assume that the last hardware action time is not greater than the
            // current time

            // last hardware action time
            scan << static_cast<uint8>(wardenWin->GetModule()->opcodes[READ_MEMORY] ^ wardenWin->GetXor())
                 << static_cast<uint8>(0) << sOfsCSimpleTop__m_eventTime << static_cast<uint8>(sizeof(uint32));
        ...
        [](const Warden *warden, ByteBuffer &buff) {
            ...
            if (!!buff.read<uint8>())
            {
                sLog.outBasic("WARDEN: Account %u timing check failed to read "
                              "CSimpleTop::m_eventTime",
            ...
            {
                sLog.outDebug("WARDEN: Player not in world. Skipping AFK check "
                              "for account %u.",
                              session->GetAccountId());
                return false;
            }
            ...
            if (!timeResult)
            {
                // the timing check can fail for innocuous reasons like running
                // under wine, within a virtual machine, or in XP compatibility
                // mode.  there are probably other causes too.  therefore let us
                // ignore this failure, since the clock desync check will catch
                // this same case if the clock is moving at an unfair speed.
            ...
            {
                sLog.outBasic("WARDEN: Account: %u Current time: %u Last hardware "
                              "action: %u (last hardware action in the future)",
    ...
    sWardenScanMgr.AddWindowsScan(std::make_shared<WindowsScan>(
        ...
        [](const Warden *warden, std::vector<std::string> &strings, ByteBuffer &scan) {
            ...
            auto const opcode =
                static_cast<uint8>(winWarden->GetModule()->opcodes[FIND_DRIVER_BY_NAME] ^ winWarden->GetXor());
        ...
        [](const Warden *warden, ByteBuffer &buff) {
            ...
            // always return false because there is nothing necessary wrong with
            // using a hypervisor
            return false;
        },
        // TODO: Replace the magic number below with combined driver string
        // lengths
        (sizeof(uint8) + sizeof(uint32) + SHA_DIGEST_LENGTH + sizeof(uint8)) * HypervisorCount + 21,
        sizeof(uint8) * HypervisorCount, "Hypervisor check", WinAllBuild | InitialLogin));
    ...
    sWardenScanMgr.AddWindowsScan(std::make_shared<WindowsScan>(
        ...
        [](const Warden *warden, std::vector<std::string> &strings, ByteBuffer &scan) {
            ...
            static constexpr std::uint8_t pattern[] = {0x56, 0x57, 0xFC, 0x8B, 0x54, 0x24, 0x14, 0x8B, 0x74, 0x24,
                                                       0x10, 0x8B, 0x44, 0x24, 0x0C, 0x8B, 0xCA, 0x8B, 0xF8, 0xC1,
                                                       0xE9, 0x02, 0x74, 0x02, 0xF3, 0xA5, 0xB1, 0x03, 0x23, 0xCA,
                                                       0x74, 0x02, 0xF3, 0xA4, 0x5F, 0x5E, 0xC3};
        ...
        [](const Warden *warden, ByteBuffer &buff) {
            ...
            // if this is not found, it means someone has tampered with the
            // function
            return !found;
        },
        sizeof(uint8) + sizeof(uint32) + SHA_DIGEST_LENGTH + sizeof(uint32) + sizeof(uint8), sizeof(uint8),
        "Warden Memory Read check", WinBuild8606 | AdvancedScan));
    ...
    auto const endSceneHookCheck1 = std::make_shared<WindowsScan>(
        ...
        [](const Warden *warden, std::vector<std::string> &, ByteBuffer &scan) {
            ...
            scan << static_cast<uint8>(wardenWin->GetModule()->opcodes[READ_MEMORY] ^ wardenWin->GetXor())
                 << static_cast<uint8>(0) << wardenWin->_endSceneAddress << endSceneReadSize;
        },
        // checker
        [](const Warden *warden, ByteBuffer &buff) {
            auto const wardenWin = const_cast<WardenWin *>(reinterpret_cast<const WardenWin *>(warden));
            ...
            if (!!result)
            {
                sLog.outBasic("WARDEN: Failed to read EndScene (hook check stage 1) "
                              "from account %u ip %s",
                              wardenWin->_session->GetAccountId(), wardenWin->_session->GetRemoteAddress().c_str());
    ...
    auto const endSceneLocate4 = std::make_shared<WindowsScan>(
        ...
        [](const Warden *warden, std::vector<std::string> &, ByteBuffer &scan) {
            ...
            scan << static_cast<uint8>(wardenWin->GetModule()->opcodes[READ_MEMORY] ^ wardenWin->GetXor())
                 << static_cast<uint8>(0) << wardenWin->_endSceneAddress + sOfsDevice4
        ...
        [endSceneHookCheck1](const Warden *warden, ByteBuffer &buff) {
            ...
            if (!!result)
            {
                sLog.outBasic("WARDEN: Failed to read EndScene (stage 4) from "
                              "account %u ip %s",
                              wardenWin->_session->GetAccountId(), wardenWin->_session->GetRemoteAddress().c_str());
    ...
    auto const endSceneLocate3 = std::make_shared<WindowsScan>(
        ...
        [](const Warden *warden, std::vector<std::string> &, ByteBuffer &scan) {
            ...
            scan << static_cast<uint8>(wardenWin->GetModule()->opcodes[READ_MEMORY] ^ wardenWin->GetXor())
                 << static_cast<uint8>(0) << wardenWin->_endSceneAddress + sOfsDevice3
                 << static_cast<uint8>(sizeof(uint32));
        },
        // checker
        [endSceneLocate4](const Warden *warden, ByteBuffer &buff) {
            ...
            if (!!result)
            {
                sLog.outBasic("WARDEN: Failed to read EndScene (stage 3) from "
                              "account %u ip %s",
                              wardenWin->_session->GetAccountId(), wardenWin->_session->GetRemoteAddress().c_str());
    ...
    auto const endSceneLocate2 = std::make_shared<WindowsScan>(
        ...
        [](const Warden *warden, std::vector<std::string> &, ByteBuffer &scan) {
            ...
            scan << static_cast<uint8>(wardenWin->GetModule()->opcodes[READ_MEMORY] ^ wardenWin->GetXor())
                 << static_cast<uint8>(0) << wardenWin->_endSceneAddress + sOfsDevice2
                 << static_cast<uint8>(sizeof(uint32));
        },
        // checker
        [endSceneLocate3](const Warden *warden, ByteBuffer &buff) {
            ...
            if (!!result)
            {
                sLog.outBasic("WARDEN: Failed to read EndScene (stage 2) from "
                              "account %u ip %s",
                              wardenWin->_session->GetAccountId(), wardenWin->_session->GetRemoteAddress().c_str());
    ...
    sWardenScanMgr.AddWindowsScan(std::make_shared<WindowsMemoryScan>(
        ...
        [endSceneLocate2](const Warden *warden, ByteBuffer &buff) {
            ...
            if (!!result)
            {
                sLog.outBasic("WARDEN: Failed to read g_theGxDevicePtr from "
                              "account %u ip %s",
                              wardenWin->_session->GetAccountId(), wardenWin->_session->GetRemoteAddress().c_str());
    ...
    sWardenScanMgr.AddWindowsScan(std::make_shared<WindowsModuleScan>(
        "prxdrvpe.dll",
        // checker
        Scan::CheckT([](const Warden *warden, ByteBuffer &buff) {
            if (buff.read<uint8>() == WindowsModuleScan::ModuleFound)
            {
                ...
                sLog.outBasic("WARDEN: Proxifier found on account %u ip %s", wardenWin->_session->GetAccountId(),
                              wardenWin->_session->GetRemoteAddress().c_str());
            }
...
void WardenWin::BuildLuaInit(const std::string &module, bool fastcall, uint32 offset, ByteBuffer &out) const
{
    ...
    out << static_cast<uint8>(WARDEN_SMSG_MODULE_INITIALIZE) << static_cast<uint16>(len)
...
void WardenWin::BuildFileHashInit(const std::string &module, bool asyncparam, uint32 openOffset, uint32 sizeOffset,
                                  uint32 readOffset, uint32 closeOffset, ByteBuffer &out) const
{
    ...
    out << static_cast<uint8>(WARDEN_SMSG_MODULE_INITIALIZE) << static_cast<uint16>(len)
        << static_cast<uint32>(0) // checksum placeholder
        << static_cast<uint8>(1) << static_cast<uint8>(0) << static_cast<uint8>(asyncparam ? 2 : 1)
...
WardenWin::WardenWin(WorldSession *session, const BigNumber &K, SessionAnticheatInterface *anticheat)
    : _wardenAddress(0), Warden(session, sWardenModuleMgr.GetWindowsModule(), K, anticheat), _lastClientTime(0),
      _lastHardwareActionTime(0), _lastTimeCheckServer(0), _sysInfoSaved(false), _proxifierFound(false),
      _hypervisors(""), _endSceneFound(false), _endSceneAddress(0)
{
...
void WardenWin::ValidateEndScene(const std::vector<uint8> &code)
{
    ...
    if (*p == 0xCC)
    {
        sLog.outBasic("WARDEN: Detected INT3 EndScene hook for account %u IP %s "
                      "(NOP count = %d)",
    ...
    else if (*p == 0xE9)
    {
        ...
        sLog.outBasic("WARDEN: Detected JMP EndScene hook for account %u IP %s "
                      "(NOP count = %d)",
                      _session->GetAccountId(), _session->GetRemoteAddress().c_str(), nopCount);
        ...
        EnqueueScans({std::make_shared<WindowsMemoryScan>(
            ...
            [](const Warden *warden, ByteBuffer &buff) {
                ...
                if (!!result)
                {
                    sLog.outBasic("WARDEN: Failed to read EndScene hook code from "
                                  "account %u ip %s",
                                  wardenWin->_session->GetAccountId(), wardenWin->_session->GetRemoteAddress().c_str());
...
uint32 WardenWin::GetScanFlags() const
{
    ...
    case 8606:
        ...
        break;
    default:
        sLog.outBasic("WARDEN: Invalid client build %u for account %u", _session->GetGameBuild(),
...
void WardenWin::Update(uint32 diff)
{
    ...
    // 'lpMaximumApplicationAddress' should never be zero if the structure has
    // been read
    if (!_sysInfoSaved && !!_sysInfo.lpMaximumApplicationAddress)
    {
        ...
        auto stmt = LoginDatabase.CreateStatement(fingerprintUpdate,
                                                  "INSERT INTO system_fingerprint_usage (fingerprint, account, ip, "
                                                  "realm, architecture, cputype, activecpus, totalcpus, pagesize) "
                                                  "VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)");
...
{
    // if we have already recorded system information, send the packet
    // immediately.  otherwise delay
    if (_sysInfoSaved)
...
void WardenWin::SendPlayerInfo(ChatHandler *handler, bool includeFingerprint) const
{
    ...
    {
        handler->PSendSysMessage("Last hardware action: %u client time: %u idle "
                                 "time: %u seconds info age: %u seconds",
