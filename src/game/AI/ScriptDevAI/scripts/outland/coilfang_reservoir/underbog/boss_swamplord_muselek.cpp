...
struct boss_swamplord_muselekAI : public CombatAI
{
    boss_swamplord_muselekAI(Creature *creature)
        : CombatAI(creature, MUSELEK_ACTION_MAX),
    ...
    void DistancingEnded() override
    {
        ...
        SetActionReadyStatus(MUSELEK_AIMED_SHOT,
                             false); // currently never used due to not being interrupted when the
                                     // target is in his melee range (oneshotting everything)
    }
    ...
    void SpellHitTarget(Unit *target, const SpellEntry *spell) override
    {
        if (spell->Id == SPELL_FREEZING_TRAP_EFFECT)
            if (Creature *claw = m_instance->GetSingleCreatureFromStorage(NPC_CLAW))
                if (claw->IsAlive() && claw->GetEntry() != NPC_CLAW_DRUID_FORM)
                    claw->getThreatManager().modifyThreatPercent(
                        target, (0 - urand(30, 99))); // Freezing trap messes with bear aggro
    }
    ...
    {
        if (Creature *claw = m_instance->GetSingleCreatureFromStorage(NPC_CLAW))
            m_creature->CastSpell(claw, SPELL_NOTIFY_OF_DEATH,
                                  TRIGGERED_NONE); // TODO: what does this do?
