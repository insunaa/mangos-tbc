...
/* Known event issues:
 * The Kaldorei and Qiraji soldiers don't have the correct flags and factions
 * in DB The spells used by the dragons upon the Qiraji need core support
 */
...
{
    npc_anachronos_the_ancientAI(Creature *pCreature) : ScriptedAI(pCreature), DialogueHelper(aEventDialogue)
    {
        Reset();
    }
    ...
    void JustDidDialogueStep(int32 iEntry) override
    {
        ...
        case MERITHRA_TAKEOFF:
            if (Creature *pMerithra = m_creature->GetMap()->GetCreature(m_merithraGuid))
            {
                ...
                pMerithra->SetByteValue(UNIT_FIELD_BYTES_1, UNIT_BYTES_1_OFFSET_MISC_FLAGS,
        ...
        case SAY_ARYGOS_ATTACK_2:
            if (Creature *pMerithra = m_creature->GetMap()->GetCreature(m_merithraGuid))
            {
                ...
                {
                    for (GameObjectList::const_iterator itr = lMerithraWakeGOs.begin(); itr != lMerithraWakeGOs.end();
                         ++itr)
                    {
                        if ((*itr)->IsSpawned())
                            continue;
        ...
        case CAELESTRASZ_TAKEOFF:
            if (Creature *pCaelestrasz = m_creature->GetMap()->GetCreature(m_CaelestraszGuid))
            {
                ...
                pCaelestrasz->SetByteValue(UNIT_FIELD_BYTES_1, UNIT_BYTES_1_OFFSET_MISC_FLAGS,
                                           UNIT_BYTE1_FLAG_ALWAYS_STAND);
    ...
    void DoSummonKaldorei()
    {
        ...
        {
            m_creature->GetRandomPoint(aQirajiWarriors[1].m_fX, aQirajiWarriors[1].m_fY, aQirajiWarriors[1].m_fZ, 20.0f,
                                       fX, fY, fZ);
            ...
            if (m_uiEventTimer <= uiDiff)
            {
                ...
                case 3:
                    ...
                    m_creature->GetMotionMaster()->MovePoint(POINT_ID_EXIT, aEternalBoardMovement[8].m_fX,
                                                             ...
                                                             aEternalBoardMovement[8].m_fZ);
                    m_uiEventTimer = 0;
                    break;
                case 4:
                    // Take off and fly
                    m_creature->GetMotionMaster()->MovePoint(
                        0, aEternalBoardMovement[9].m_fX, aEternalBoardMovement[9].m_fY, aEternalBoardMovement[9].m_fZ);
    ...
    if (pQuest->GetQuestId() == QUEST_A_PAWN_ON_THE_ETERNAL_BOARD)
    {
                                        ...
                                        pGo->GetPositionZ(), 3.75f, TEMPSPAWN_CORPSE_DESPAWN, 0))
        {
            ...
            if (npc_anachronos_the_ancientAI *pAnachronosAI =
                    dynamic_cast<npc_anachronos_the_ancientAI *>(pAnachronos->AI()))
...
struct npc_solenorAI : public ScriptedAI
{
    ...
    {
        if (pWho->getClass () == CLASS_HUNTER
        && (m_hunterGuid.IsEmpty () || m_hunterGuid == pWho->GetObjectGuid ()) /*&& pWho->GetQuestStatus(QUEST_STAVE_OF_THE_ANCIENTS) == QUEST_STATUS_INCOMPLETE*/)
        {
