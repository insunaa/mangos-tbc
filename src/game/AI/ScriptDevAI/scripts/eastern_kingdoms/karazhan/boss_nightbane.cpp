...
enum
{
    ...
    // These points are a placeholder for the air phase movement. The dragon
    // should do some circles around the area before landing again
    POINT_ID_AIR = 1,
    POINT_ID_GROUND = 2,
...
struct boss_nightbaneAI : public CombatAI
{
        ...
        if (motionType == PATH_MOTION_TYPE)
        {
            if (m_bCombatStarted) // combat movement
            {
                if (pointId == POINT_LANDING_END)
                {
                    ...
                    m_creature->RemoveByteFlag(UNIT_FIELD_BYTES_1, UNIT_BYTES_1_OFFSET_MISC_FLAGS,
    ...
    void DoMoveToClosestTrigger(bool ground)
    {
        ...
        for (GuidList::const_iterator itr = lTriggersList.begin(); itr != lTriggersList.end(); ++itr)
        {
            ...
            {
                if (!chosenTrigger ||
                    (ground ? !m_creature->GetDistanceOrder(pTrigger, chosenTrigger, false, DIST_CALC_NONE)
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case NIGHTBANE_PHASE_2: {
            ...
            {
                // Start air phase movement (handled by
        ...
        case NIGHTBANE_DISTRACTING_ASH: {
            if (Unit *target =
                ...
                if (DoCastSpellIfCan(target,
                                     m_phase == PHASE_GROUND ? SPELL_DISTRACTING_ASH_GROUND : SPELL_DISTRACTING_ASH) ==
        ...
        case NIGHTBANE_FIREBALL_BARRAGE: {
            if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_FARTHEST_AWAY, 0,
                                                                 SPELL_FIREBALL_BARRAGE, SELECT_FLAG_PLAYER))
                if (m_creature->GetDistance(target, false, DIST_CALC_COMBAT_REACH) < 60.f ||
                    (m_creature->CastSpell(m_creature->GetVictim(), SPELL_FIREBALL_BARRAGE, TRIGGERED_NONE) ==
                     SPELL_CAST_OK))
    ...
    if (bIsStart && source->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        if (instance->GetData(TYPE_NIGHTBANE) == NOT_STARTED || instance->GetData(TYPE_NIGHTBANE) == FAIL)
        {
            ...
            if (nightbane && nightbane->IsAlive())
            {
                ...
                // values appear to be valid (see Onyxia, too)
                ...
                nightbane->RemoveByteFlag(UNIT_FIELD_BYTES_1, UNIT_BYTES_1_OFFSET_MISC_FLAGS,
                                          UNIT_BYTE1_FLAG_ALWAYS_STAND);
