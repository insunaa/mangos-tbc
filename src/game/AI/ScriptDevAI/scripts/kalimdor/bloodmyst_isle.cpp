...
struct mob_webbed_creatureAI : public Scripted_NoMovementAI
{
    ...
    void JustDied(Unit *pKiller) override
    {
        ...
        if (uiSpawnCreatureEntry)
            if (Creature *spawnedCreature =
                    m_creature->SummonCreature(uiSpawnCreatureEntry, 0.0f, 0.0f, 0.0f, m_creature->GetOrientation(),
                ...
                spawnedCreature->GetMotionMaster()->MoveRandomAroundPoint(
                    ...
                    5.f); // Needed because one or more of the spawned creatures has
                          // MovementType 2 by default, causing them to go flying to
                          // the first default waypoint
    }
};
...
struct npc_demolitionist_legosoAI : public npc_escortAI, private DialogueHelper
{
    ...
    void DoRespawnObjects(uint32 uiEntry, float fRange)
    {
        ...
        {
            script_error_log("Bloodmyst Isle: ERROR Failed to find any gameobjects "
                             "of entry %u in range.",
                             uiEntry);
            return;
        }
