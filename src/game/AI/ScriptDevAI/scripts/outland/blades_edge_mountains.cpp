...
SD%Complete: 90
SDComment: Quest support: 10503, 10504, 10506, 10512, 10545, 10556, 10609,
10674, 10859, 10998, 11058, 11080. (npc_daranelle needs bit more work before
consider complete) SDCategory: Blade's Edge Mountains EndScriptData */
...
struct mobs_nether_drakeAI : public ScriptedAI
{
    ...
    void SpellHit(Unit *pCaster, const SpellEntry *pSpell) override
    {
        if (pSpell->Id == SPELL_T_PHASE_MODULATOR && pCaster->GetTypeId() == TYPEID_PLAYER)
        {
            ...
            m_creature->CastSpell(nullptr, 35426,
                                  TRIGGERED_OLD_TRIGGERED); // arcane explosion visual
        ...
        if (m_bIsNihil)
        {
            if (m_uiNihilSpeechTimer < uiDiff)
            {
                ...
                case 4:
                    ...
                    m_creature->SetByteFlag(UNIT_FIELD_BYTES_1, UNIT_BYTES_1_OFFSET_MISC_FLAGS,
                                            UNIT_BYTE1_FLAG_FLY_ANIM);
                    m_creature->GetMotionMaster()->MovePoint(1, m_creature->GetPositionX() + 50.0f,
...
enum
{
    ...
    // other spells
    // SPELL_SWITCHED_ON_OFF        = 40512,            // decharger lock (not
    // used) SPELL_SWITCHED_ON_OFF_2      = 40499,            // decharger unlock
    // (not used)
...
struct npc_simon_game_bunnyAI : public ScriptedAI
{
    ...
    void DoPrepareLevel()
    {
        ...
        DoCastSpellIfCan(nullptr, m_bIsLargeEvent ? SPELL_PRE_GAME_BLUE_LARGE : SPELL_PRE_GAME_BLUE_AURA,
                         CAST_TRIGGERED);
        DoCastSpellIfCan(nullptr, m_bIsLargeEvent ? SPELL_PRE_GAME_GREEN_LARGE : SPELL_PRE_GAME_GREEN_AURA,
                         CAST_TRIGGERED);
        ...
        case PHASE_PLAYER_GAME:
            ...
            if (eventType == AI_EVENT_CUSTOM_C)
            {
                ...
                if (uiMiscValue == aApexisGameData[m_vColors[m_uiLevelStage]].m_uiIntrospection)
                {
                    ...
                    // if all buttons were pressed succesfully, then move to next
                    // level
                    ...
                    // cast tick sound
                        ...
                        DoCastSpellIfCan(pInvoker,
                                         m_bIsLargeEvent ? SPELL_VISUAL_GAME_TICK_LARGE : SPELL_VISUAL_GAME_TICK,
                                         CAST_TRIGGERED);
                }
                // bad button pressed
                else
                {
                    ...
                    if (!m_bIsLargeEvent && !pInvoker->IsAlive()) // if player got killed on small
                                                                  // event
                    {
            ...
            else if (eventType == AI_EVENT_CUSTOM_B)
            {
                ...
                // Not sure if this is right, but we need to keep the buttons
                // unlocked on every tick
                DoSetupPlayerLevel();
                ++m_uiPlayerStage;
            }
            break;
...
{
    npc_light_orb_collectorAI(Creature *pCreature) : ScriptedAI(pCreature)
    {
        Reset();
    }
...
{
    npc_obelisk_triggerAI(Creature *pCreature) : ScriptedAI(pCreature)
    {
        Reset();
    }
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (m_uiActiveObelisk == 5)
        {
            if (m_uiActivateTimer)
            {
                ...
                {
                    Creature *m_uiDoomcryer = m_creature->SummonCreature(NPC_DOOMCRYER, m_creature->GetPositionX(),
                                                                         m_creature->GetPositionY(), 283.65f, 0.32f,
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (!m_uiEnrage)
        {
            if (m_creature->GetHealthPercent() <= 50.0f)
            {
                ...
                {
                    float x = pMiddleBunny->GetPositionX(), y = pMiddleBunny->GetPositionY(),
                          z = pMiddleBunny->GetPositionZ();
...
struct npc_vimgol_middle_bunnyAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        if (m_uiScanTimer <= uiDiff)
        {
            ...
            {
                if (playersInsideCircles() == 5)
                    m_creature->SummonCreature(NPC_VIMGOL_THE_VILE, m_creature->GetPositionX(),
                                               m_creature->GetPositionY(), m_creature->GetPositionZ(),
                                               m_creature->GetOrientation(), TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN,
                                               90000);
                if (m_uiBeamTimer == 0)
                {
                    CastBunnySpell(nullptr, SPELL_PENTAGRAM_BEAM);
...
{
    npc_bird_spiritAI(Creature *pCreature) : ScriptedAI(pCreature)
    {
        Reset();
    }
...
{
    SPELL_REND = 13443,
    SPELL_RINAS_DIMINUTION_POWDER = 36310, // Note: spell also throws event id 13584; requires more research
                                           // on the purpose of this event
                                                        ...
                                                        Creature *pCreatureTarget, ObjectGuid /*originalCasterGuid*/)
{
    ...
    {
        // Check if creature already has aura. ToDo: check for Hibernating
        // creatures
        if (pCreatureTarget->HasAura(SPELL_RINAS_DIMINUTION_POWDER))
            return true;

        // give kill credit, change to friendly and inform the creautre about the
        // reset timer
        ...
        // Note: we can't remove all auras because it will also remove the
        // current aura; so currently we only remove periodic damage auras This
        // might be wrong and we might need to change this to something like
...
bool AreaTrigger_at_raven_prophecy(Player *pPlayer, AreaTriggerEntry const *pAt)
{
        ...
        pPlayer->GetQuestStatus(QUEST_WHISPERS_OF_THE_RAVEN_GOD) == QUEST_STATUS_INCOMPLETE)
    {
        ...
        if (prophecyIterator != prophecies.end())
        {
            ...
            // The quest required IDs are negative when they are game object so
            // we must negate the game object ID
            if (pPlayer->GetReqKillOrCastCurrentCount(QUEST_WHISPERS_OF_THE_RAVEN_GOD, prophecy.creature) == 0)
            {
                ...
                {
                    if (pPlayer->SummonCreature(NPC_VISION_RAVEN_GOD_TEMPLATE, prophecy.x, prophecy.y, prophecy.z,
                                                prophecy.o, TEMPSPAWN_TIMED_DESPAWN, 6000))
                    {
                        DoScriptText(prophecy.text, whisper, pPlayer);

                        // giving credit for the game object automatically
                        // negates the id, so we don't have to negate it
...
{
    npc_frequency_scanner(Creature *pCreature) : ScriptedAI(pCreature)
    {
        Reset();
    }
...
enum
{
    ...
    SPELL_GO_SMALL_FIRE = 49910, // serverside spells for spawning GOs - TODO:
                                 // Remove and substitute with pre-spawned gos
...
{
    npc_fel_cannon(Creature *pCreature) : Scripted_NoMovementAI(pCreature)
    {
        Reset();
    }
    ...
    void SpellHit(Unit * /*pCaster*/, const SpellEntry *pSpell) override
    {
        if (pSpell->Id == SPELL_DEATHS_DOOR_FEL_CANNON)
        {
            m_bMCed = true;
            if (Creature *target =
                    GetClosestCreatureWithEntry(m_creature, NPC_DEATHS_DOOR_FEL_CANNON_TARGET_BUNNY, 100.f))
            {
    ...
    void MoveInLineOfSight(Unit * /*pWho*/) override
    {
    } // disable changing facing of any kind
...
{
    npc_warp_gate(Creature *pCreature) : Scripted_NoMovementAI(pCreature), m_resetTimer(0)
    {
        Reset();
    }
        ...
        if (pSpell->Id == SPELL_ARTILLERY_ON_THE_WARP_GATE)
        {
            ...
            case 6: {
                ...
                if (Creature *cannon = m_creature->GetMap()->GetCreature(m_guidFelCannon))
                {
                    ...
                    {
                        player->RewardPlayerAndGroupAtEventCredit(
                            m_creature->GetEntry() == NPC_DEATHS_DOOR_NORTH_WARP_GATE ? NPC_NORTH_WARP_GATE_CREDIT
...
struct npc_soulgrinderAI : public ScriptedAI
{
    ...
    void JustRespawned() override
    {
        ...
        m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3574.525f, 5624.78f, -9.746364f, 5.707227f,
                                   TEMPSPAWN_CORPSE_DESPAWN, 1000);
        m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3558.613f, 5543.979f, -2.24089f, 0.9948376f,
                                   TEMPSPAWN_CORPSE_DESPAWN, 1000);
        m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3542.293f, 5546.184f, -2.307737f, 5.096361f,
                                   TEMPSPAWN_CORPSE_DESPAWN, 1000);
        m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3507.783f, 5607.564f, -2.115761f, 2.146755f,
                                   TEMPSPAWN_CORPSE_DESPAWN, 1000);
        m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3525.318f, 5624.862f, -5.340961f, 0.2792527f,
                                   TEMPSPAWN_CORPSE_DESPAWN, 1000);
        m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3573.157f, 5592.371f, -4.614707f, 2.984513f,
                                   TEMPSPAWN_CORPSE_DESPAWN, 1000);
        m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3542.814f, 5622.923f, -3.886532f, 0.1919862f,
                                   TEMPSPAWN_CORPSE_DESPAWN, 1000);
        m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3574.056f, 5574.628f, -5.577652f, 0.2792527f,
                                   TEMPSPAWN_CORPSE_DESPAWN, 1000);
        m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3559.653f, 5559.587f, -2.850421f, 3.01942f,
                                   TEMPSPAWN_CORPSE_DESPAWN, 1000);
        m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3574.718f, 5609.495f, -3.285291f, 1.32645f,
                                   TEMPSPAWN_CORPSE_DESPAWN, 1000);
        m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3541.38f, 5559.73f, -2.665792f, 2.827433f, TEMPSPAWN_CORPSE_DESPAWN,
                                   1000);
        m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3542.222f, 5573.395f, -2.64414f, 4.276057f,
                                   TEMPSPAWN_CORPSE_DESPAWN, 1000);
        m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3557.254f, 5591.203f, -2.993953f, 3.01942f,
                                   TEMPSPAWN_CORPSE_DESPAWN, 1000);
        m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3558.495f, 5577.132f, -3.076451f, 3.892084f,
                                   TEMPSPAWN_CORPSE_DESPAWN, 1000);
        m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3557.081f, 5612.431f, -4.562363f, 1.518436f,
                                   TEMPSPAWN_CORPSE_DESPAWN, 1000);
        m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3542.203f, 5608.064f, -3.378048f, 0.1570796f,
                                   TEMPSPAWN_CORPSE_DESPAWN, 1000);
        m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3558.009f, 5626.f, -5.146327f, 3.857178f, TEMPSPAWN_CORPSE_DESPAWN,
                                   1000);
        m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3524.67f, 5608.019f, -3.321486f, 2.042035f,
                                   TEMPSPAWN_CORPSE_DESPAWN, 1000);
        m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3510.031f, 5575.792f, -1.80589f, 4.904375f,
                                   TEMPSPAWN_CORPSE_DESPAWN, 1000);
        m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3508.348f, 5589.707f, -0.8961017f, 1.623156f,
                                   TEMPSPAWN_CORPSE_DESPAWN, 1000);
        m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3507.988f, 5561.777f, 1.993673f, 2.80998f, TEMPSPAWN_CORPSE_DESPAWN,
                                   1000);
        m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3509.509f, 5625.191f, -4.461998f, 0.06981317f,
                                   TEMPSPAWN_CORPSE_DESPAWN, 1000);
        m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3525.131f, 5573.122f, -1.622976f, 6.126106f,
                                   TEMPSPAWN_CORPSE_DESPAWN, 1000);
        m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3493.974f, 5594.064f, -0.03712566f, 2.670354f,
                                   TEMPSPAWN_CORPSE_DESPAWN, 1000);
        m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3526.678f, 5560.159f, -0.8016807f, 0.9424778f,
                                   TEMPSPAWN_CORPSE_DESPAWN, 1000);
        m_creature->SummonCreature(NPC_OGRE_SPIRIT, 3574.228f, 5559.057f, -6.009155f, 6.021386f,
                                   TEMPSPAWN_CORPSE_DESPAWN, 1000);
    }
        ...
        if (m_uiTimer)
        {
            if (m_uiTimer <= uiDiff)
            {
                ...
                case 0: {
                    ...
                    {
                        Creature *bunny = m_creature->SummonCreature(NPC_SOULGRINDER_BUNNY, 3491.224f, 5529.023f,
                                                                     17.14335f, 6.195919f, TEMPSPAWN_MANUAL_DESPAWN, 0);
                        Creature *gronn =
                            m_creature->SummonCreature(NPC_SKULLOC_SOULGRINDER, 3478.563f, 5550.74f, 7.838801f,
                    ...
                    {
                        Creature *bunny = m_creature->SummonCreature(NPC_SOULGRINDER_BUNNY, 3464.954f, 5564.684f,
                                                                     17.99977f, 5.67232f, TEMPSPAWN_MANUAL_DESPAWN, 0);
                        bunny->CastSpell(m_creature, SPELL_BEAM, TRIGGERED_NONE);
                        if (Creature *gronn = m_creature->GetMap()->GetCreature(m_skullocSoulgrinder))
                        {
                    ...
                    {
                        Creature *bunny = m_creature->SummonCreature(NPC_SOULGRINDER_BUNNY, 3515.02f, 5524.39f,
                                                                     16.8927f, 3.560477f, TEMPSPAWN_MANUAL_DESPAWN, 0);
                        bunny->CastSpell(m_creature, SPELL_BEAM, TRIGGERED_NONE);
                        if (Creature *gronn = m_creature->GetMap()->GetCreature(m_skullocSoulgrinder))
                        {
                    ...
                    {
                        Creature *bunny = m_creature->SummonCreature(NPC_SOULGRINDER_BUNNY, 3468.82f, 5581.41f,
                                                                     17.5205f, 0.0174533f, TEMPSPAWN_MANUAL_DESPAWN, 0);
                        bunny->CastSpell(m_creature, SPELL_BEAM, TRIGGERED_NONE);
                        if (Creature *gronn = m_creature->GetMap()->GetCreature(m_skullocSoulgrinder))
                        {
...
struct npc_supplicantAI : public ScriptedAI
{
    ...
    void ReceiveAIEvent(AIEventType eventType, Unit *pSender, Unit * /*pInvoker*/, uint32 /*uiMiscValue*/) override
    {
        ...
        if (eventType == AI_EVENT_CUSTOM_A) // Start doing random dance/turn/roar etc.
        {
            ...
            GetCreatureListWithEntryInGrid(list, m_creature, NPC_BLOODMAUL_SUPPLICANT,
                                           10.f); // guesswork
    ...
    void ResetOgreState(bool stopRandomStuff)
    {
        ...
        m_creature->LoadEquipment(m_creature->GetCreatureInfo()->EquipmentTemplateId,
                                  true);                                        // Set default equipment
...
{
    npc_spirit_prisoner_of_bladespire(Creature *pCreature) : ScriptedAI(pCreature)
    {
        Reset();
    }
...
struct npc_deadsoul_orb : public ScriptedAI
{
    ...
    {
        Reset();
        m_creature->SetActiveObjectState(true); // Need to be active since the area is so large they might get
                                                // unloaded on the way to the destination
        ...
        else
        {
            if (Creature *waypointTrigger = GetClosestCreatureWithEntry(m_creature, nextTrigger, 95))
            {
                m_creature->GetMotionMaster()->MovePoint(pointCount, waypointTrigger->GetPositionX(),
                                                         waypointTrigger->GetPositionY(),
                                                         waypointTrigger->GetPositionZ());
            }
            else
            {
...
struct npc_evergrove_druidAI : public ScriptedAI
{
    ...
    void ReturnToSpawn(Player *questAccepter = nullptr)
    {
        ...
        if (questAccepter) // Only return to spawn if it's the original player
                           // accepting a quest
...
struct CoaxMarmot : public AuraScript
{
    void OnApply(Aura *aura, bool apply) const override
    {
        if (!apply)
        {
            ...
            {
                if (Creature *mover = static_cast<Creature *>(pPlayer->GetMover())) // this spell uses DoSummonPossesed
                                                                                    // so remove this on removal
                {
