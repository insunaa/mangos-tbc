...
enum AVTimers
{
    ...
    BG_AV_MINE_RECLAIM_TIMER = 20 * MINUTE * IN_MILLISECONDS, // TODO: get the right value.. this is
...
enum AVGenericVariables
{
    ...
    BG_AV_MAX_NODE_DISTANCE = 25, // distance in which players are still counted in range of a banner
                                  // (for alliance towers this is calculated from the center of the
                                  // tower)
...
enum AVSpells
{
    BG_AV_SPELL_COMPLETE_AV_QUEST = 23658, // complete quests 7141 and 7142; cast individually per team,
                                           // because spell completes both quests at once
...
enum AVSounds
{
    ...
    BG_AV_SOUND_ALLIANCE_GOOD = 8173, // if something good happens for the team:  wins(maybe only through
                                      // killing the boss), captures mine or grave, destroys tower and
                                      // defends grave
    BG_AV_SOUND_HORDE_GOOD = 8213,
    BG_AV_SOUND_BOTH_TOWER_DEFEND = 8192,

    BG_AV_SOUND_ALLIANCE_CAPTAIN = 8232, // gets called when someone attacks them and at the beginning after
...
class BattleGroundAVScore : public BattleGroundScore
{
  public:
    BattleGroundAVScore()
        : graveyardsAssaulted(0), graveyardsDefended(0), towersAssaulted(0), towersDefended(0),
          secondaryObjectives(0){};
    virtual ~BattleGroundAVScore(){};

    uint32 GetAttr1() const override
    {
        return graveyardsAssaulted;
    }
    uint32 GetAttr2() const override
    {
        return graveyardsDefended;
    }
    uint32 GetAttr3() const override
    {
        return towersAssaulted;
    }
    uint32 GetAttr4() const override
    {
        return towersDefended;
    }
...
class BattleGroundAV : public BattleGround
{
  ...
  private:
    ...
    // World state helpers
    uint8 GetWorldStateType(uint8 state, PvpTeamIndex teamIdx) const
    {
