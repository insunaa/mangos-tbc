...
event NYI SDCategory: Blackrock Spire EndScriptData */
...
void instance_blackrock_spire::SetData(uint32 uiType, uint32 uiData)
{
    ...
    case TYPE_STADIUM:
        ...
        {
            // Despawn Nefarius, Rend and the spectators on fail (the others are
            // despawned OnCreatureEvade())
...
void instance_blackrock_spire::OnCreatureDeath(Creature *pCreature)
{
    ...
    case NPC_BLACKHAND_VETERAN:
        ...
        if (m_auiEncounter[TYPE_ROOM_EVENT] == IN_PROGRESS)
        {
            ...
            for (uint8 i = 0; i < MAX_ROOMS; ++i)
            {
                if (m_aRoomRuneGuid[i]) // This check is used, to ensure which
                                        // runes still need processing
                {
    ...
    case NPC_BLACKHAND_HANDLER:
        ...
        // 5% chance for Rend or Lord Victor Nefarius to taunt players when one
        // of the creature is killed (% is guesswork) Lord Victor Nefarius
        if (urand(0, 100) < 5)
        {
            if (Creature *pNefarius = GetSingleCreatureFromStorage(NPC_LORD_VICTOR_NEFARIUS))
            {
...
void instance_blackrock_spire::JustDidDialogueStep(int32 iEntry)
{
    ...
    case NPC_BLACKHAND_HANDLER:
        ...
        if (Creature *pNefarius = GetSingleCreatureFromStorage(NPC_LORD_VICTOR_NEFARIUS))
        {
            pNefarius->GetMotionMaster()->MovePoint(0, aStadiumLocs[5].m_fX, aStadiumLocs[5].m_fY,
                                                    aStadiumLocs[5].m_fZ);
            // Summon the spectators and move them to the western balcony
            for (uint8 i = 0; i < 12; i++)
            {
                Creature *pSpectator = pNefarius->SummonCreature(
                    aStadiumSpectators[i], aSpectatorsSpawnLocs[i].m_fX, aSpectatorsSpawnLocs[i].m_fY,
                    aSpectatorsSpawnLocs[i].m_fZ, aSpectatorsSpawnLocs[i].m_fO, TEMPSPAWN_DEAD_DESPAWN, 0);
                if (pSpectator)
                {
                    pSpectator->SetFacingTo(aSpectatorsTargetLocs[i].m_fO);
                    pSpectator->SetWalk(false);
                    pSpectator->GetMotionMaster()->MovePoint(
                        0, aSpectatorsTargetLocs[i].m_fX, aSpectatorsTargetLocs[i].m_fY, aSpectatorsTargetLocs[i].m_fZ);
    ...
    case NPC_REND_BLACKHAND:
        ...
        if (Creature *pNefarius = GetSingleCreatureFromStorage(NPC_LORD_VICTOR_NEFARIUS))
        {
            ...
            pNefarius->GetMotionMaster()->MovePoint(0, aStadiumLocs[6].m_fX, aStadiumLocs[6].m_fY,
                                                    aStadiumLocs[6].m_fZ);
        }
        break;
    }
    ...
    if (m_uiStadiumWaves < MAX_STADIUM_WAVES)
    {
        ...
        if (Creature *pNefarius = GetSingleCreatureFromStorage(NPC_LORD_VICTOR_NEFARIUS))
        {
            ...
            for (uint8 i = 0; i < MAX_STADIUM_MOBS_PER_WAVE; ++i)
            {
                ...
                pNefarius->GetRandomPoint(aStadiumLocs[0].m_fX, aStadiumLocs[0].m_fY, aStadiumLocs[0].m_fZ, 7.0f, fX,
                                          fY, fZ);
                fX = std::min(aStadiumLocs[0].m_fX,
                              fX); // Halfcircle - suits better the rectangular form
                if (Creature *pTemp = pNefarius->SummonCreature(aStadiumEventNpcs[m_uiStadiumWaves][i], fX, fY, fZ,
                                                                0.0f, TEMPSPAWN_DEAD_DESPAWN, 0))
                {
                    // Get some point in the center of the stadium
                    pTemp->GetRandomPoint(aStadiumLocs[2].m_fX, aStadiumLocs[2].m_fY, aStadiumLocs[2].m_fZ, 5.0f, fX,
                                          fY, fZ);
                    fX = std::min(aStadiumLocs[2].m_fX,
                                  fX); // Halfcircle - suits better the rectangular form
    ...
    else
    {
        ...
        if (Creature *pNefarius = GetSingleCreatureFromStorage(NPC_LORD_VICTOR_NEFARIUS))
        {
            ...
            if (Creature *pTemp = pNefarius->SummonCreature(NPC_GYTH, aStadiumLocs[1].m_fX, aStadiumLocs[1].m_fY,
                ...
                pTemp->GetMotionMaster()->MovePoint(0, aStadiumLocs[2].m_fX, aStadiumLocs[2].m_fY,
                                                    aStadiumLocs[2].m_fZ);
        }
...
void instance_blackrock_spire::DoSendNextFlamewreathWave()
{
    ...
    // TODO - The npcs would move nicer if they had DB waypoints, so i suggest to
    // change their default movement to DB waypoints, and random movement when
    ...
    if (m_uiFlamewreathWaveCount < 6) // Send two adds (6 waves, then boss)
    {
        ...
        {
            float fX, fY, fZ;
            pSummoner->GetRandomPoint(rookeryEventSpawnPos[0], rookeryEventSpawnPos[1], rookeryEventSpawnPos[2], 2.5f,
                                      fX, fY, fZ);
            ...
            if (pSummoned)
            {
                ...
                // pSummoned->GetMotionMaster()->MovePoint(1, fX, fY,
    ...
    else // Send Flamewreath
    {
        if (Creature *pSolakar = pSummoner->SummonCreature(NPC_SOLAKAR_FLAMEWREATH, rookeryEventSpawnPos[0],
            ...
            // pSolakar->GetMotionMaster()->MovePoint(1, pSummoner->GetPositionX(),
            // pSummoner->GetPositionY(), pSummoner->GetPositionZ());
...
bool AreaTrigger_at_blackrock_spire(Player *pPlayer, AreaTriggerEntry const *pAt)
{
    ...
    // Intro event when players enter The Furnace: three Blackhand Elite are
    // spawned, flee from the Beast and are "killed" by it The Blackhand Elite
    // scripts are handled in database by waypoints movement
        ...
        if (instance_blackrock_spire *pInstance = (instance_blackrock_spire *)pPlayer->GetInstanceData())
        {
            ...
            if (Creature *pBeast = pInstance->GetSingleCreatureFromStorage(NPC_THE_BEAST))
            {
                ...
                if (!pInstance->m_bBeastIntroDone)
                {
                    for (uint8 i = 0; i < 3; i++)
                    {
                        if (Creature *pTemp = pBeast->SummonCreature(NPC_BLACKHAND_ELITE, aBeastLocs[i].m_fX,
                                                                     aBeastLocs[i].m_fY, aBeastLocs[i].m_fZ,
                                                                     aBeastLocs[i].m_fO, TEMPSPAWN_DEAD_DESPAWN, 0))
                            pTemp->GetMotionMaster()->MoveWaypoint(i);
                    }
                    pInstance->m_bBeastIntroDone = true;
                }
