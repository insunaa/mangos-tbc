...
void instance_serpentshrine_cavern::SetData(uint32 uiType, uint32 uiData)
{
    ...
    case TYPE_THELURKER_EVENT:
        ...
        {
        case IN_PROGRESS:
            WorldObject::SummonCreature(TempSpawnSettings(nullptr, NPC_LURKER_BELOW, afLurkerSpawnPos[0],
                                                          afLurkerSpawnPos[1], afLurkerSpawnPos[2], afLurkerSpawnPos[3],
    ...
    case TYPE_LADYVASHJ_EVENT:
        ...
        if (uiData == FAIL)
        {
            ...
            for (GuidList::const_iterator itr = m_tinyTriggerGUIDList.begin(); itr != m_tinyTriggerGUIDList.end();
                 ++itr)
            {
...
void instance_serpentshrine_cavern::SetData64(uint32 uiData, uint64 uiGuid)
{
    ...
    // platform set is alive The function is triggered by eventAI on generic
    // timer
                                                               ...
                                                               uint32 conditionSourceType) const
{
    ...
    script_error_log("instance_serpentshrine_cavern::CheckConditionCriteriaMeet "
...
{
    npc_serpentshrine_parasiteAI(Creature *pCreature) : ScriptedAI(pCreature)
    {
        Reset();
    }
