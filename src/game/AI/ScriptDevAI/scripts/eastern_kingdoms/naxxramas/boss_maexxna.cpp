...
enum
{
    ...
    //    SPELL_SUMMON_SPIDERLING_2 = 30076,                   // Summons 3
    //    spiderlings
...
// This NPC is summoned by the web wrapped player and act as a visual target
// for other raid members to free the player
struct npc_web_wrapAI : public ScriptedAI
{
    npc_web_wrapAI(Creature *creature) : ScriptedAI(creature)
    {
        Reset();
    }
    ...
    void UpdateAI(const uint32 diff) override
    {
        ...
        if (m_player)
        {
            // Check if the web wrapped player is still alive, if not, clear
            // ourselves
...
struct boss_maexxnaAI : public CombatAI
{
    boss_maexxnaAI(Creature *creature)
        : CombatAI(creature, MAEXXNA_ACTION_MAX),
          m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData()))
    {
        AddTimerlessCombatAction(MAEXXNA_ENRAGE_HP_CHECK,
                                 true); // Soft enrage à 30%
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case MAEXXNA_WEBWRAP: {
            // Web Wrap
            // ToDo: the targets and triggers selection is probably done through
            // spells 29280, 29281, 29282, 29283, 29285 & 29287 But because we
            // are walking in Guessland until more reliable data are available,
            // let's do the selection manually here Randomly pick 3 targets,
            // excluding current victim (main tank)
            ...
            if (!targets.empty())
            {
                // Check we have enough summoning NPCs spawned in regards of
                // player targets
                if (m_summoningTriggers.size() < targets.size())
                {
                    script_error_log("Error in script Naxxramas::boss_maexxna: less "
                                     "summoning NPCs (entry %u) than expected targets "
                                     "(%u) for Web Wrap ability. Check your DB",
                ...
                for (uint8 i = 0; i < targets.size(); i++)
                    targets[i]->CastSpell(invisibleMen[i], SPELL_WEB_WRAP_INIT, TRIGGERED_IGNORE_UNSELECTABLE_FLAG,
...
struct npc_invisible_manAI : public ScriptedAI
{
    ...
    void HandleVictimCheck()
    {
        if (Player *victim = m_creature->GetMap()->GetPlayer(m_victimGuid))
        {
            if (victim->IsAlive())
            {
                // Make the player cast the visual effects spells with a delay to
                // ensure he/she has reach his/her destination
    ...
    void OnEffectExecute(Spell *spell, SpellEffectIndex effIdx) const override
    {
        if (effIdx == EFFECT_INDEX_0)
        {
            ...
            if (!unitTarget || unitTarget->GetTypeId() != TYPEID_UNIT ||
                spell->GetCaster()->GetTypeId() != TYPEID_PLAYER)
                return;

            float dist = spell->GetCaster()->GetDistance(unitTarget, false);
            // Switch the pull target spell based on the distance from the web wrap
            // position
            ...
            unitTarget->CastSpell(spell->GetCaster(), pullSpellId, TRIGGERED_INSTANT_CAST, nullptr, nullptr,
                                  spell->GetAffectiveCasterObject()->GetObjectGuid());
        }
    }
};
...
struct ClearWebWrap : public SpellScript
{
    void OnEffectExecute(Spell *spell, SpellEffectIndex effIdx) const override
    {
        if (effIdx == EFFECT_INDEX_0)
        {
            ...
            {
            case SPELL_CLEAR_WEB_WRAP_TARGET: // Clear Web Wrap (Maexxna: clear
                                              // effects on player)
            {
            ...
            case SPELL_CLEAR_WEB_WRAP_SELF: // Clear Web Wrap (Maexxna: kill Web
                                            // Wrap NPC)
            {
                if (unitTarget && unitTarget->GetTypeId() == TYPEID_UNIT)
                    unitTarget->CastSpell(nullptr, SPELL_KILL_WEBWRAP,
                                          TRIGGERED_OLD_TRIGGERED); // Kill Web Wrap
                break;
            }
            default:
                break;
