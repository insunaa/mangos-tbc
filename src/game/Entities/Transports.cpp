...
void MapManager::LoadTransports()
{
    ...
    do
    {
        ...
        if (!goinfo)
        {
            sLog.outErrorDb("Transport ID:%u, Name: %s, will not be loaded, "
                            "gameobject_template missing",
                            entry, name.c_str());
            continue;
        }
        ...
        {
            sLog.outErrorDb("Transport ID:%u, Name: %s, will not be loaded, "
                            "gameobject_template type wrong",
                            entry, name.c_str());
            continue;
        }
        ...
        {
            sLog.outErrorDb("Transport ID:%u, Name: %s, will not be loaded, "
                            "transport template missing (core generated)",
                            entry, name.c_str());
            continue;
        }
    ...
    result = WorldDatabase.Query("SELECT gameobject.guid,gameobject.id,transports.name FROM "
                                 "gameobject,transports WHERE gameobject.id = transports.entry");
    if (result) // wrong data found
    {
        do
        {
            ...
            sLog.outErrorDb("Transport %u '%s' have record (GUID: %u) in `gameobject`. "
                            "Transports MUST NOT have any records in `gameobject` or its "
                            "behavior will be unpredictable/bugged.",
...
void Transport::LoadTransport(TransportTemplate const &transportTemplate, Map *map)
{
    ...
    // sLog.outString("Loading transport %d between %s, %s", entry, name.c_str(),
    // goinfo->name);
...
bool Transport::Create(uint32 guidlow, uint32 mapid, float x, float y, float z, float ang, uint32 animprogress)
{
    ...
    if (!IsPositionValid())
    {
        sLog.outError("Transport (GUID: %u) not created. Suggested coordinates "
                      "isn't valid (X: %f Y: %f)",
                      guidlow, x, y);
        return false;
    }
    ...
    if (!goinfo)
    {
        sLog.outErrorDb("Transport not created: entry in `gameobject_template` not "
...
void Transport::TeleportTransport(uint32 newMapid, float x, float y, float z, float o)
{
    ...
    // we need to create and save new Map object with 'newMapid' because if not
    // done -> lead to invalid Map object reference... player far teleport would
    // try to create same instance, but we need it NOW for transport... correct
    // me if I'm wrong O.o
    if (mapChange)
    {
        RemoveModelFromMap();
...
void Transport::Update(const uint32 /*diff*/)
{
    ...
    if (m_pathProgress >= currentMsTime) // map transition and update happened in
                                         // same tick due to MT
        return;
    ...
    while (true)
    {
        ...
        // first check help in case client-server transport coordinates
        // de-synchronization
        if (m_currentFrame->Node->mapid != GetMapId() || m_currentFrame->IsTeleportFrame())
        {
            TeleportTransport(m_nextFrame->Node->mapid, m_nextFrame->Node->x, m_nextFrame->Node->y,
                              m_nextFrame->Node->z, m_nextFrame->InitialOrientation);
            return;
        }
...
void ElevatorTransport::Update(const uint32 /*diff*/)
{
    ...
    if (nodeNext && nodePrev)
    {
        ...
        // SummonCreature(1, currentPos.x, currentPos.y, currentPos.z,
        // GetOrientation(), TEMPSPAWN_TIMED_DESPAWN, 5000);
...
void GenericTransport::UpdatePassengerPosition(WorldObject *passenger)
{
    ...
    {
        sLog.outError("[TRANSPORTS] Object %s [guid %u] has invalid position "
                      "on transport.",
                      passenger->GetName(), passenger->GetGUIDLow());
        return;
    }
    ...
    case TYPEID_PLAYER:
        // relocate only passengers in world and skip any player that might be
        // still logging in/teleporting
        if (passenger->IsInWorld())
    ...
    case TYPEID_DYNAMICOBJECT:
        // GetMap()->DynamicObjectRelocation(passenger->ToDynObject(), x, y, z,
        // o);
        break;
    default:
        break;
