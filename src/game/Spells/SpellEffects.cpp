...
pEffect SpellEffects[MAX_SPELL_EFFECTS] = {
    ...
    &Spell::EffectEmpty,                    // 25 SPELL_EFFECT_WEAPON                   spell per
                                            // weapon type, in ItemSubclassmask store mask that
                                            // can be used for usability check at equip, but
                                            // current way using skill also work.
    ...
    &Spell::EffectTeleUnitsFaceCaster,      // 43
                                            // SPELL_EFFECT_TELEPORT_UNITS_FACE_CASTER
    &Spell::EffectLearnSkill,               // 44 SPELL_EFFECT_SKILL_STEP
    &Spell::EffectAddHonor,                 // 45 SPELL_EFFECT_ADD_HONOR honor/pvp related
    &Spell::EffectSpawn,                    // 46 SPELL_EFFECT_SPAWN spawn/login animation,
                                            // expected by spawn unit cast, also base points store
                                            // some dynflags
    ...
    &Spell::EffectTeleportGraveyard,        // 120 SPELL_EFFECT_TELEPORT_GRAVEYARD one
    ...
    &Spell::EffectKillCreditGroup,          // 134 SPELL_EFFECT_KILL_CREDIT_GROUP misc
    ...
    &Spell::EffectKnockBackFromPosition,    // 144
                                            // SPELL_EFFECT_KNOCKBACK_FROM_POSITION
    ...
    &Spell::EffectSummonRafFriend,          // 152 SPELL_EFFECT_SUMMON_RAF_FRIEND summon
                                            // Refer-a-Friend
    &Spell::EffectCreateTamedPet,           // 153 SPELL_EFFECT_CREATE_PET misc value is
                                            // creature entry
};

void Spell::EffectEmpty(SpellEffectIndex /*eff_idx*/)
{
    // NOT NEED ANY IMPLEMENTATION CODE, EFFECT POSISBLE USED AS MARKER OR CLIENT
    // INFORM
}
...
void Spell::EffectEnvironmentalDMG(SpellEffectIndex eff_idx)
{
    ...
    // Note: this hack with damage replace required until GO casting not
    // implemented environment damage spells already have around enemies
    // targeting but this not help in case nonexistent GO casting support
    // currently each enemy selected explicitly and self cast damage, we prevent
    // apply self casted spell bonuses/etc
    ...
    Unit::SendSpellNonMeleeDamageLog(m_trueCaster, m_caster, m_spellInfo->Id, damage, GetSpellSchoolMask(m_spellInfo),
                                     absorb, resist, false, 0, false);
    ...
    if (unitTarget && unitTarget->IsAlive())
    {
        ...
        case SPELLFAMILY_GENERIC: {
            ...
            case 37841:
                if (unitTarget->GetTypeId() == TYPEID_PLAYER &&
                    unitTarget->HasAura(37830)) // Repolarized Magneto Sphere
            ...
            case 38977: // Spore Quake Effect
                if (roll_chance_i(33))
                    m_caster->CastSpell(unitTarget, 39002,
                                        TRIGGERED_OLD_TRIGGERED); // Spore Quake Knockdown
                break;
            case 39384: {
                if (eff_idx == 0)
                {
        ...
        case SPELLFAMILY_DRUID: {
            ...
            if ((m_spellInfo->SpellFamilyFlags & uint64(0x000800000)) && m_spellInfo->SpellVisual == 6587)
            {
                ...
                if (m_caster->GetTypeId() == TYPEID_PLAYER)
                    damage +=
                        int32(m_caster->GetTotalAttackPowerValue(BASE_ATTACK) * 0.03f * m_caster->GetComboPoints());
                break;
            }
...
void Spell::EffectDummy(SpellEffectIndex eff_idx)
{
    ...
    case SPELLFAMILY_GENERIC: {
        ...
        case 8063: // Deviate Fish
        {
            if (m_caster->GetTypeId() == TYPEID_PLAYER)
            {
                // 8064 = Sleepy | 8065 = Invigorate | 8066 = Shrink | 8067 =
                // Party Time! | 8068 = Healthy Spirit
        ...
        case 8344: // Gnomish Universal Remote (ItemID: 7506)
        {
            ...
            {
                // 8345 - Control the machine | 8346 = Malfunction the
                // machine (root) | 8347 = Taunt/enrage the machine
        ...
        case 12975: // Last Stand
        {
            ...
            m_caster->CastCustomSpell(m_caster, 12976, &healthModSpellBasePoints0, nullptr, nullptr,
        ...
        case 13006: // Gnomish Shrink Ray (ItemID: 10716)
        {
            if (unitTarget && m_CastItem)
            {
                ...
                // These rates are hella random; someone feel free to correct
                // them
        ...
        case 13120: // Net-O-Matic
        {
            if (unitTarget && m_CastItem)
            {
                ...
                else if (roll < 4) // 2% for 20 sec root, charge to target
                                   // (off-like chance unknown)
        ...
        case 13567: // Dummy Trigger
        {
            ...
            if (m_triggeredByAuraSpell && unitTarget)
            {
                ...
                case 26467: // Persistent Shield
                    ...
                    break;
                default:
                    sLog.outError("EffectDummy: Non-handled case for spell 13567 for "
                                  "triggered aura %u",
        ...
        case 16378: // Temperature Reading
        {
            ...
            switch (unitTarget->GetGUIDLow()) // sadly each spawn does different texts
                                              // - meant to be db guids
            {
            ...
            default: {
                sLog.outErrorDb("Database has wrong data for q.974 - guids do not "
                                "correspond.");
                break;
            }
            }
        ...
        case 17009: // Voodoo
        {
            ...
            {
                // 16707 = Hex | 16708 = Hex | 16709 = Hex | 16711 = Grow |
                // 16712 = Special Brew | 16713 = Ghostly | 16716 = Launch
        ...
        case 18350: // Dummy Trigger
        {
            if (m_triggeredByAuraSpell && unitTarget)
            {
                ...
                case 13810: // Frost Trap Aura
                {
                    ...
                    // Chose this route so that whole proc system doesnt
                    // have to run IDs are in reverse order because its
                    // more likely someone will have max rank
                    float chance = 0.f;
                ...
                case 28820: // Lightning Shield (The Earthshatterer set
                            // trigger after cast Lighting Shield)
                {
                    // Need remove self if Lightning Shield not active
                    Unit::SpellAuraHolderMap const &auras = unitTarget->GetSpellAuraHolderMap();
                    for (const auto &aura : auras)
                ...
                case 38443: // Totemic Mastery (Skyshatter Regalia (Shaman
                            // Tier 6) - bonus)
                {
                ...
                {
                    // If your Greater Heal brings the target to full
                    // health, you gain $37595s1 mana.
        ...
        {
            // Trigger the eight summoning spells for the adds in Ragnaros
            // encounter
        ...
        {
            // randomly cast one of the nine Lava Burst spell A to I in
            // Ragnaros encounter
        ...
        case 22664: // Shadowblink
        {
            ...
            {
                // randomly cast one of the nine Shadowblink spell in
                // Nefarian encounter (phase 1)
        ...
        case 23173: // Brood Affliction - TODO: Rework to use proper
                    // targeting
        {
            // This spell selects one brood affliction amongst five and apply
            // it onto 15 targets If there are less than 15 targets, then the
            // spell loops over the targets again until there are 15
            // appliance of the chosen brood affliction
            ...
            // Get the fifteen (potentially duplicate) targets from threat
            // list
            GuidVector vGuids;
            ...
            while (targetsCount < 15)
            {
                ...
                // Prevent infinite loop: if fifteenTargets is still empty
                // after first iteration: return
                if (targetsCount == 0)
                {
                    return;
                }
            ...
            for (auto unit : fifteenTargets)
            {
                ...
                // Cast Chromatic Mutation (23174) if target is now affected
                // by all five brood afflictions
        ...
        {
            // This spell selects two breaths for Chromaggus amongst five and
            // send them to BWL instance script through events

            // Breaths selection
            // 5 possiblities for the first breath, 4 for the second: 20
            // total possiblites
        ...
        {
            // This spell selects two kinds of drakonids amongst five and
            // send them to BWL instance script through events

            // Drakonids selection
            // 5 possiblities for the first kind, 4 for the second: 20 total
            // possiblites
        ...
        case 23450: // Transporter Arrival - (Gadgetzan + Everlook)
        {
            ...
            if (!rand)                 // (1/6) - Polymorph (23456 & 23457 are both carbon
                                       // copies of this one)
        ...
        {
            // Roll chance for major malfunction (1/6); 23441 = success |
            // 23446 = malfunction (missed destination)
        ...
        case 23725: // Gift of Life (warrior bwl trinket)
        {
            ...
            // m_caster->CastSpell(nullptr, 23783, TRIGGERED_OLD_TRIGGERED);
            // // Patch 2.1 no longer heals
            return;
        }
        ...
        {
            // 24924 = Larger and Orange | 24925 = Skeleton | 24926 = Pirate
            // | 24927 = Ghost
        ...
        case 25860: // Reindeer Transformation
        {
            ...
            // 5 different spells used depending on mounted speed and if
            // mount can fly or not
            if (flyspeed >= 4.1f)
                // Flying Reindeer
                m_caster->CastSpell(m_caster, 44827,
                                    TRIGGERED_OLD_TRIGGERED); // 310% flying Reindeer
            else if (flyspeed >= 3.8f)
                // Flying Reindeer
                m_caster->CastSpell(m_caster, 44825,
                                    TRIGGERED_OLD_TRIGGERED); // 280% flying Reindeer
            else if (flyspeed >= 1.6f)
                // Flying Reindeer
                m_caster->CastSpell(m_caster, 44824,
                                    TRIGGERED_OLD_TRIGGERED); // 60% flying Reindeer
            else if (speed >= 2.0f)
                // Reindeer
                m_caster->CastSpell(m_caster, 25859,
                                    TRIGGERED_OLD_TRIGGERED); // 100% ground Reindeer
            else
                // Reindeer
                m_caster->CastSpell(m_caster, 25858,
                                    TRIGGERED_OLD_TRIGGERED); // 60% ground Reindeer
        ...
        case 26374: // Elune's Candle
        {
            if (unitTarget->GetTypeId() == TYPEID_UNIT && unitTarget->GetEntry() == 15467) // Omen
            {
                ...
                m_caster->CastSpell(unitTarget, eluneCandle[urand(0, 4)],
                                    TRIGGERED_OLD_TRIGGERED); // Damage (random visual)
                return;
            }
        ...
        case 26899: // Give Friendship Bracers
        {
            ...
            unitTarget->CastSpell(unitTarget, 26921,
                                  TRIGGERED_OLD_TRIGGERED); // cast Cancel Heartbroken, Create
                                                            // Bracelet
            return;
        }
        case 28000: // Land Mine Trigger
        {
            ...
            m_caster->CastSpell(nullptr, 25099,
                                TRIGGERED_OLD_TRIGGERED); // Detonation
        ...
        case 28110: // Feugen Tesla Effect
        {
            if (unitTarget && unitTarget->GetTypeId() == TYPEID_UNIT && unitTarget->IsAlive())
            {
                if (!m_caster->hasUnitState(UNIT_STAT_ROOT)) // This state is found in sniffs and
                                                             // is probably caused by another
                                                             // aura like 23973
                    m_caster->addUnitState(UNIT_STAT_ROOT);  // but as we are not sure (the aura
                                                             // does not show up in sniffs), we
                ...
                // Not in range and fight in progress: remove aura and cast
                // Shock onto players
        ...
        case 28359: // Trigger Teslas
        {
            if (unitTarget)
            {
                ...
                unitTarget->RemoveAllAuras();
                unitTarget->CastSpell(unitTarget, 28159,
                                      TRIGGERED_NONE); // Shock
            }
            return;
        }
        case 28414: // Call of the Ashbringer
        {
            ...
            m_caster->PlayDirectSound(AshbringerSounds[urand(0, 11)],
                                      PlayPacketParameters(PLAY_TARGET, (Player *)m_caster));
            return;
        }
        case 28697: // Forgiveness
        {
            unitTarget->CastSpell(nullptr, 29266,
                                  TRIGGERED_OLD_TRIGGERED); // guessed permanent feign death
                                                            // spell
            return;
        }
        ...
        case 28961: // Summon Corpse Scarabs (10)
        {
            ...
            unitTarget->CastSpell(unitTarget, 28864,
                                  TRIGGERED_OLD_TRIGGERED); // Actual summoning spell
        ...
        case 30023: // Periodic Trigger Dummy - triggered by many spells
        {
            ...
            case 40250: // Improved Duration - Anzu spirits
            {
                ...
                {
                    if ((*itr)->GetHolder()->GetSpellProto()->IsFitToFamily(SPELLFAMILY_DRUID,
                                                                            0x0000001000000050)) // only Rejuvenation,
                                                                                                 // Regrowth and
                                                                                                 // Lifebloom
                    {
        ...
        {
            m_caster->CastSpell(m_caster, 34810,
                                TRIGGERED_OLD_TRIGGERED); // Summon 20083 behind of the
                                                          // caster
            m_caster->CastSpell(m_caster, 34817,
                                TRIGGERED_OLD_TRIGGERED); // Summon 20078 right of the caster
            m_caster->CastSpell(m_caster, 34818,
                                TRIGGERED_OLD_TRIGGERED); // Summon 20078 left of the caster
            m_caster->CastSpell(m_caster, 34819,
                                TRIGGERED_OLD_TRIGGERED); // Summon 20078 front of the caster
            return;
        }
        case 35517: // Teleport
        {
        ...
        {
            unitTarget->CastSpell(unitTarget, 29266,
                                  TRIGGERED_IGNORE_CURRENT_CASTED_SPELL); // Permanent Feign
                                                                          // Death
            return;
        }
        ...
        {
            unitTarget->CastSpell(unitTarget, 38022,
                                  TRIGGERED_NONE); // FRS Quest Credit
            return;
        }
        ...
        case 39238: // Fumping
        {
            ...
            // 50/50 chance to summon Mature Bone Sifter instead of Sand
            // Gnome
            switch (urand(0, 1))
            {
            case 0:
        ...
        case 39487: // Crypt Scarab Hate
        {
            ...
            {
                if (Unit *spawnerTarget = spawner->GetVictim()) // Force attacking the same
                                                                // target as its spawner
                {
                    if (unitTarget->CanAttackOnSight(spawnerTarget)) // Need to make sure target is
                                                                     // attackable currently
                    {
        ...
        case 39849: // Throw Glaive (second)
        {
            ...
            unitTarget->CastSpell(unitTarget, 41466, TRIGGERED_OLD_TRIGGERED, nullptr, nullptr,
        ...
        case 40032: // Fiery Breath (Patch 2.4.2 (2008-05-13): Dragons in
                    // Blade's Edge Mountains will no longer instantly knock
                    // you off your flying mount.)
        {
            if (!unitTarget)
                return;
        ...
        case 41333: // Empyreal Equivalency
        {
            ...
            // Equilize the health of all targets based on the corresponding
            // health percent
        ...
        case 11958: // Cold Snap
        {
            ...
            auto cdCheck = [](SpellEntry const &spellEntry) -> bool {
                if (spellEntry.Id == 11958 || spellEntry.SpellFamilyName != SPELLFAMILY_MAGE)
                    return false;
                if ((GetSpellSchoolMask(&spellEntry) & SPELL_SCHOOL_MASK_FROST) &&
    ...
    case SPELLFAMILY_WARRIOR: {
        ...
        case 21977: {
            if (!unitTarget)
                return;
            m_caster->CastSpell(unitTarget, 21887,
                                TRIGGERED_OLD_TRIGGERED); // spell mod
            return;
        }
        case 30012: // Move
        {
    ...
    case SPELLFAMILY_PRIEST: {
        ...
        case 28598: // Touch of Weakness triggered spell
        {
            ...
            case 25461:
                ...
                break; // Rank 7
            default:
                sLog.outError("Spell::EffectDummy: Spell 28598 triggered "
                              "by unhandeled "
                              "spell %u",
                              m_triggeredByAuraSpell->Id);
                return;
    ...
    case SPELLFAMILY_DRUID: {
        ...
        case 29284: // Brittle Armor - dummy exists so that max stacks are
                    // added
        {
            m_caster->CastSpell(unitTarget, 24575, TRIGGERED_OLD_TRIGGERED);
            return;
        }
        case 29286: // Mercurial Shield - dummy exists so that max stacks are
                    // added
        {
    ...
    case SPELLFAMILY_PALADIN: {
        ...
        {
            if (unitTarget)
                if (SpellEntry const *spell_proto =
                        sSpellTemplate.LookupEntry<SpellEntry>(m_currentBasePoints[eff_idx]))
        ...
        case 51640: // Taunt Flag Targeting
        {
            ...
            m_caster->CastSpell(x, y, z, 52605,
                                TRIGGERED_OLD_TRIGGERED); // should also send start
            return;
        }
        }
    ...
    case SPELLFAMILY_SHAMAN: {
            ...
            for (int j = BASE_ATTACK; j <= OFF_ATTACK; ++j)
            {
                if (Item *item = ((Player *)m_caster)->GetWeaponForAttack(WeaponAttackType(j)))
                {
                    if (item->IsFitToSpellRequirements(m_spellInfo))
                    {
                        ...
                        // damage-per-sec in enchanting effect at calculation
                        // applied affect from Elemental Weapons talent real
                        // enchantment damage
        ...
        if (m_spellInfo->SpellFamilyFlags & uint64(0x0000000000200000)) // Flametongue Weapon Proc, Ranks
        {
            if (m_CastItem)
            {
                // found spelldamage coefficients of 0.381% per 0.1 speed
                // and 15.244 per 4.0 speed but own calculation say 0.385 gives
                // at most one point difference to published values
                ...
                // Does Amplify Magic/Dampen Magic influence flametongue? If
                // not, the above addition must be removed.
        ...
        if (m_spellInfo->SpellFamilyFlags & uint64(0x0000000400000000)) // Flametongue Totem Proc, Ranks
        {
            if (m_CastItem) // Does not scale with gear
            {
                float weaponSpeed = float(m_CastItem->GetProto()->Delay) / IN_MILLISECONDS;
                int32 totalDamage =
                    (damage * 0.01f * weaponSpeed * (m_CastItem->GetEnchantmentModifier() + 100.f) / 100.f);
    ...
    // implementation in core spell effects So called only for not processed
    // cases
    ...
    if (gameObjTarget)
        ...
        libraryResult = sScriptDevAIMgr.OnEffectDummy(m_caster, m_spellInfo->Id, eff_idx, (Creature *)unitTarget,
                                                      m_originalCasterGUID);
    else if (itemTarget)
        libraryResult =
            sScriptDevAIMgr.OnEffectDummy(m_caster, m_spellInfo->Id, eff_idx, itemTarget, m_originalCasterGUID);
...
void Spell::EffectTriggerSpellWithValue(SpellEffectIndex eff_idx)
{
    ...
    if (!spellInfo)
    {
        ...
        if (startDBScript)
        {
            DEBUG_FILTER_LOG(LOG_FILTER_SPELL_CAST, "Spell ScriptStart spellid %u in EffectTriggerSpell",
                             m_spellInfo->Id);
    ...
    int32 bp = damage;
    m_caster->CastCustomSpell(unitTarget, triggered_spell_id, &bp, &bp, &bp, TRIGGERED_OLD_TRIGGERED, m_CastItem,
                              nullptr, m_originalCasterGUID, m_spellInfo);
}
...
void Spell::EffectTriggerRitualOfSummoning(SpellEffectIndex eff_idx)
{
    ...
    if (!spellInfo)
    {
        sLog.outError("EffectTriggerRitualOfSummoning of spell %u: triggering "
                      "unknown spell id %i",
...
void Spell::EffectForceCast(SpellEffectIndex effIndex)
{
    ...
    if (!spellInfo)
    {
        sLog.outError("EffectForceCast of spell %u: triggering unknown spell id %i", m_spellInfo->Id,
                      triggered_spell_id);
        return;
    }
...
void Spell::EffectTriggerMissileSpell(SpellEffectIndex effect_idx)
{
    ...
    if (!spellInfo)
    {
        if (unitTarget)
        {
            ...
            sLog.outError("EffectTriggerMissileSpell of spell %u (eff: %u): "
                          "triggering unknown spell id %u",
    ...
    else if (spellInfo->EffectImplicitTargetA[0] != TARGET_LOCATION_CASTER_DEST) // TODO: Add a proper filling
                                                                                 // mechanism
        targets.setDestination(m_targets.m_destPos.x, m_targets.m_destPos.y, m_targets.m_destPos.z);
    m_caster->CastSpell(targets, spellInfo, TRIGGERED_OLD_TRIGGERED, m_CastItem, nullptr, m_originalCasterGUID,
                        m_spellInfo);
}
...
void Spell::EffectTeleportUnits(SpellEffectIndex eff_idx) // TODO - Use target settings for this effect!
{
    ...
    if (!(m_targets.m_targetMask & TARGET_FLAG_DEST_LOCATION))
    {
        sLog.outError("Spell::EffectTeleportUnits - unknown "
                      "EffectImplicitTargetB[%u] = %u for spell ID %u",
                      eff_idx, m_spellInfo->EffectImplicitTargetB[eff_idx], m_spellInfo->Id);
        return;
    }
    ...
    {
        // Wrong destination already rolled for, only handle minor malfunction
        // on sucess
        m_caster->CastSpell(m_caster, 23450,
                            TRIGGERED_OLD_TRIGGERED); // Transporter Arrival

        return;
    }
    case 23442: // Dimensional Ripper - Everlook
    {
        // Roll for major malfunction (1/6); 23450 = success | 23449 =
        // malfunction (being set afire)
...
void Spell::EffectPowerBurn(SpellEffectIndex eff_idx)
{
    ...
    // should use here effect POWER_DRAIN because POWER_BURN is not implemented
    // on client
    m_spellLog.AddLog(uint32(SPELL_EFFECT_POWER_DRAIN), unitTarget->GetPackGUID(), new_damage, uint32(powertype),
                      multiplier);
}
...
void Spell::EffectHeal(SpellEffectIndex eff_idx)
{
    if (unitTarget && unitTarget->IsAlive() && damage >= 0)
    {
        ...
        else if (m_spellInfo->TargetAuraState == AURA_STATE_SWIFTMEND && unitTarget->HasAuraState(AURA_STATE_SWIFTMEND))
        {
            ...
            if (!targetAura)
            {
                sLog.outError("Target (GUID: %u TypeId: %u) has aurastate "
        ...
        else
        {
            switch (m_spellInfo->Id)
            {
            // Crusader Enchant: Holy Strength amount decrease by 4% each level
            // after 60
            case 20007: {
...
void Spell::EffectHealthLeech(SpellEffectIndex eff_idx)
{
    ...
    if (m_caster->IsAlive())
    {
        ...
        // TODO: at hit need to schedule a (delayed) heal effect execution on
        // caster order of packets for death coil - start, go, delay, dmg log,
        // delay, heal log
...
void Spell::EffectPersistentAA(SpellEffectIndex eff_idx)
{
    ...
    if (!dynObj->Create(caster->GetMap()->GenerateLocalLowGuid(HIGHGUID_DYNAMICOBJECT), caster, m_spellInfo->Id,
                        eff_idx, m_targets.m_destPos.x, m_targets.m_destPos.y, m_targets.m_destPos.z, m_duration,
                        radius, DYNAMIC_OBJECT_AREA_SPELL, target, damage, m_currentBasePoints[eff_idx]))
    {
        delete dynObj;
        return;
    }
    ...
    // Potential Hack - at the time of channel start Dynamic Object is not
    // created yet, so have to do it here Sent in next Object Update so for
    // client its the same
...
void Spell::EffectEnergize(SpellEffectIndex eff_idx)
{
    ...
    {
    case 5530:
        if (m_caster->getClass() == CLASS_ROGUE) // Warrior and rogue use same spell, on rogue not
                                                 // supposed to give resource, WTF game devs?!
            return;
        break;
...
void Spell::SendLoot(ObjectGuid guid, LootType loottype, LockType lockType)
{
        ...
        if (gameObjTarget)
        {
            ...
            case GAMEOBJECT_TYPE_TRAP:
                ...
                case LOCKTYPE_OPEN_ATTACKING:
                    ...
                    return;
                default:
                    sLog.outError("Spell::SendLoot unhandled locktype %u for GameObject "
                                  "trap (entry %u) for spell %u.",
                                  lockType, gameObjTarget->GetEntry(), m_spellInfo->Id);
                    return;
                }
            default:
                sLog.outError("Spell::SendLoot unhandled GameObject type %u (entry %u) "
                              "for spell %u.",
    ...
    default:
        sLog.outError("Spell::SendLoot unhandled Object type %s for spell %u.", guid.GetString().c_str(),
                      m_spellInfo->Id);
        break;
    }
}
...
void Spell::EffectSummonChangeItem(SpellEffectIndex eff_idx)
{
    ...
    // prevent crash at access and unexpected charges counting with item update
    // queue corrupt
    ClearCastItem();
    ...
    if (Item *newItem = player->ConvertItem(oldItem, newitemid))
    {
        ...
        // Push packet to client so it knows we've created an item and quest can
...
void Spell::EffectApplyAreaAura(SpellEffectIndex eff_idx)
{
    ...
    Aura *aura = CreateAura(m_spellInfo, eff_idx, &damage, &m_currentBasePoints[eff_idx], m_spellAuraHolder, unitTarget,
                            m_caster, m_CastItem);
...
void Spell::EffectSummonType(SpellEffectIndex eff_idx)
{
    ...
    if (!realCaster)
    {
        sLog.outError("EffectSummonType: No Casting Object found for spell %u, "
                      "(caster = %s)",
                      m_spellInfo->Id, m_caster->GetGuidStr().c_str());
        return;
    }
    ...
    // Expected Amount: TODO - there are quite some exceptions (like totems,
    // engineering dragonlings..)
    uint32 amount = 1;
    uint32 health = 0; // totems have HP in base points
    uint32 unk = 0;    // no idea what it means, but 4000-25000 cant be right for
                       // summon count
    ...
    uint32 level;
    // Everything considered as guardian or critter pets uses its creature
    // template level by default (may change depending on SpellEffect params)
    if (summon_prop->Title == UNITNAME_SUMMON_TITLE_COMPANION)
    {
        ...
        {
            sLog.outError("Spell Effect EFFECT_SUMMON (%u) - no creature template "
                          "found for summoned NPC %u (spell id %u, effIndex %u)",
                          m_spellInfo->Effect[eff_idx], m_spellInfo->EffectMiscValue[eff_idx], m_spellInfo->Id,
                          eff_idx);
            return;
        }
    }
    ...
    {
        // If EffectMultipleValue <= 0, pets have their calculated level modified
        // by EffectMultipleValue
    ...
    else
    {
        ...
        sLog.outDebug("Spell Effect EFFECT_SUMMON (%u) - summon without "
                      "destination (spell id %u, effIndex %u)",
    ...
    for (++itr; itr != summonPositions.end(); ++itr) // In case of multiple summons around position for not-fist
                                                     // positions
    {
        if (m_targets.m_targetMask & TARGET_FLAG_DEST_LOCATION || radius > 1.0f)
        {
            realCaster->GetRandomPoint(summonPositions[0].x, summonPositions[0].y, summonPositions[0].z, radius, itr->x,
                                       itr->y, itr->z);
            if (realCaster->GetMap()->GetHitPosition(summonPositions[0].x, summonPositions[0].y, summonPositions[0].z,
                                                     itr->x, itr->y, itr->z, -0.5f))
    ...
    case SUMMON_PROP_GROUP_FRIENDLY: {
        ...
        case UNITNAME_SUMMON_TITLE_NONE: {
            // those are classical totems - effectbasepoints is their hp and
            // not summon ammount! 121: 23035, battlestands 647: 52893,
            // Anti-Magic Zone (npc used)
            if (prop_id == 121 || prop_id == 647)
                summonResult = DoSummonTotem(summonPositions, eff_idx);
            else
            {
                switch (m_spellInfo->Id) // unable to distinguish based on prop_id,
                                         // therefore spell by spell override
                {
    ...
    for (itr = summonPositions.begin(); itr != summonPositions.end(); ++itr)
    {
        ...
        if (!creature)
        {
            sLog.outError("EffectSummonType: Expected to have %u NPCs summoned, but "
                          "some failed (Spell id %u)",
                          amount, m_spellInfo->Id);
            continue;
        }
        ...
        {
            // original caster is provided by script so we have to notify it as
            // its not done in Object::SummonCreature
        ...
        if (summon_prop->Flags & SUMMON_PROP_FLAG_HELP_WHEN_SUMMONED_IN_COMBAT && m_caster)
            if (m_caster->CanEnterCombat() && creature->CanEnterCombat() && creature->CanAssist(m_caster) &&
                m_caster->GetVictim())
                creature->AI()->AttackStart(m_caster->GetVictim()); // maybe needs to help with everything
                                                                    // around not just main target
    }
}
...
bool Spell::DoSummonPet(CreatureSummonPositions &list, SummonPropertiesEntry const *prop, SpellEffectIndex effIdx)
{
    ...
    if (m_caster->IsPlayer())
    {
        ...
        if (m_caster->getClass() != CLASS_PRIEST && spawnCreature->LoadPetFromDB(_player, spawnPos, pet_entry))
        {
    ...
    {
        sLog.outErrorDb("Spell::EffectSummon: can't create creature with entry "
                        "%u for spell %u",
    ...
    if (m_caster->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PLAYER_CONTROLLED))
    {
        ...
        spawnCreature->SetByteValue(UNIT_FIELD_BYTES_2, UNIT_BYTES_2_OFFSET_DEBUFF_LIMIT,
                                    UNIT_BYTE2_PLAYER_CONTROLLED_DEBUFF_LIMIT);
    }
    else
        spawnCreature->SetByteValue(UNIT_FIELD_BYTES_2, UNIT_BYTES_2_OFFSET_DEBUFF_LIMIT,
                                    UNIT_BYTE2_CREATURE_DEBUFF_LIMIT);
...
void Spell::EffectLearnSpell(SpellEffectIndex eff_idx)
{
    ...
    uint32 spellToLearn =
        (m_spellInfo->Id == SPELL_ID_GENERIC_LEARN) ? damage : m_spellInfo->EffectTriggerSpell[eff_idx];
    ...
    if (!successList.empty())
    {
        ...
        if (m_spellInfo->SpellFamilyName == SPELLFAMILY_WARLOCK && m_spellInfo->Category == SPELLCATEGORY_DEVOUR_MAGIC)
        {
            ...
            case 27277:
                ...
                break;
            default:
                DEBUG_LOG("Spell for Devour Magic %d not handled in "
                          "Spell::EffectDispel",
                          m_spellInfo->Id);
                break;
            }
...
void Spell::EffectDistract(SpellEffectIndex /*eff_idx*/)
{
    ...
    // This is needed to change the facing server side as well (and it must be
    // after the MoveDistract call)
...
void Spell::EffectPickPocket(SpellEffectIndex /*eff_idx*/)
{
    ...
    // TODO investigate if font pickpocketing has higher failure chance then from
    // behind if (m_caster->IsFacingTargetsFront(unitTarget))
    ...
    // we need to increase the base chance for failure if target is higher level
    // then caster incremental chance to fail based on level. maximum is 97%
    // chance if level difference is dramatic (give it 3% chance to succeed?).
    ...
    {
        // Stealing successful
        // BASIC_LOG("Successfull pickpocket result %i for chance %i", result,
        // chance);
    ...
    {
        // BASIC_LOG("Failed pickpocket result %i for chance %i", result,
        // chance);
                         ...
                         uint32 level)
{
    ...
    if (!cInfo)
    {
        sLog.outErrorDb("Spell::DoSummonWild: creature entry %u not found for spell %u.", creature_entry,
                        m_spellInfo->Id);
        return false;
    }
    ...
    for (auto &itr : list)
        if (Creature *summon = WorldObject::SummonCreature(
                TempSpawnSettings(m_trueCaster, creature_entry, itr.x, itr.y, itr.z, m_trueCaster->GetOrientation(),
                                  summonType, m_duration, false, IsSpellSetRun(m_spellInfo), 0, 0, 0, false, false,
                                  m_spellInfo->Id, -1, level),
                m_trueCaster->GetMap()))
        {
            itr.creature = summon;
            itr.processed = true;
            ...
            switch (m_spellInfo->Id)
            {
            case 1122: // Warlock Infernal - requires custom code - generalized
                       // in WOTLK
            {
                // Enslave demon effect, without mana cost and cooldown
                summon->CastSpell(nullptr, 22707,
                                  TRIGGERED_OLD_TRIGGERED); // short root spell on
                                                            // infernal from sniffs
                m_caster->CastSpell(summon, 20882, TRIGGERED_OLD_TRIGGERED);
                summon->CastSpell(nullptr, 22703,
                                  TRIGGERED_NONE); // Inferno effect - in wotlk
                                                   // cast automatically
                summon->AI()->DoCastSpellIfCan(nullptr, 19483, CAST_TRIGGERED | CAST_AURA_NOT_PRESENT);
                summon->CastSpell(nullptr, 22764,
                                  TRIGGERED_NONE); // aggro spell
                break;
            }
            }
        }
                             ...
                             uint32 level)
{
    ...
    if (!cInfo)
    {
        sLog.outErrorDb("Spell::DoSummonGuardian: creature entry %u not found for spell %u.", pet_entry,
                        m_spellInfo->Id);
        return false;
    }
    ...
    // second direct cast unsummon guardian(s) (guardians without like
    // functionality have cooldown > spawn time)
    ...
    for (CreatureSummonPositions::iterator itr = list.begin(); itr != list.end(); ++itr)
    {
        ...
        {
            sLog.outError("Spell::DoSummonGuardian: can't create creature entry %u "
                          "for spell %u.",
...
void Spell::EffectLearnSkill(SpellEffectIndex eff_idx)
{
    ...
    if (WorldObject const *caster = GetCastingObject())
        DEBUG_LOG("Spell: %s teaches %s skill %u (to step %u)", caster->GetGuidStr().c_str(),
                  target->GetGuidStr().c_str(), skillid, step);
}
...
void Spell::EffectAddHonor(SpellEffectIndex /*eff_idx*/)
{
    ...
    DEBUG_FILTER_LOG(LOG_FILTER_SPELL_CAST,
                     "SpellEffect::AddHonor (spell_id %u) rewards %u honor "
                     "points (non scale) for player: %u",
...
void Spell::EffectEnchantItemTmp(SpellEffectIndex eff_idx)
{
    ...
    if (eff_idx == EFFECT_INDEX_0 && m_spellInfo->Effect[EFFECT_INDEX_1] == SPELL_EFFECT_DUMMY)
    {
        ...
        // Effect[1] base value with already applied percent bonus from Elemental
        // Weapons talent Note: damage calculated (correctly) with rounding
        // int32(float(v)) but RW enchantments applied damage
    ...
    if (!enchant_id)
    {
        sLog.outError("Spell %u Effect %u (SPELL_EFFECT_ENCHANT_ITEM_TEMPORARY) "
    ...
    {
        sLog.outError("Spell %u Effect %u (SPELL_EFFECT_ENCHANT_ITEM_TEMPORARY) "
                      "have nonexistent enchanting id %u ",
...
void Spell::EffectTameCreature(SpellEffectIndex /*eff_idx*/)
{
    ...
    if (plr->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PLAYER_CONTROLLED))
    {
        ...
        pet->SetByteValue(UNIT_FIELD_BYTES_2, UNIT_BYTES_2_OFFSET_DEBUFF_LIMIT,
                          UNIT_BYTE2_PLAYER_CONTROLLED_DEBUFF_LIMIT);
    }
    else
...
void Spell::EffectSummonPet(SpellEffectIndex eff_idx)
{
    ...
    if (m_caster->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PLAYER_CONTROLLED))
    {
        ...
        NewSummon->SetByteValue(UNIT_FIELD_BYTES_2, UNIT_BYTES_2_OFFSET_DEBUFF_LIMIT,
                                UNIT_BYTE2_PLAYER_CONTROLLED_DEBUFF_LIMIT);
...
void Spell::EffectLearnPetSpell(SpellEffectIndex eff_idx)
{
    ...
    SpellEntry const *learn_spellproto =
        sSpellTemplate.LookupEntry<SpellEntry>(m_spellInfo->EffectTriggerSpell[eff_idx]);
    if (!learn_spellproto)
        return;
...
void Spell::EffectTaunt(SpellEffectIndex eff_idx)
{
    ...
    // Also use this effect to set the taunter's threat to the taunted creature's
    // highest value
    if (unitTarget->CanHaveThreatList())
    {
        ...
        unitTarget->getThreatManager().setCurrentVictimByTarget(
            m_caster); // force changes the target to caster of taunt
    }
    // Units without threat lists but with AI are susceptible to attack target
    // interference by taunt effect:
    else if (!unitTarget->CanHaveThreatList() && !unitTarget->hasUnitState(UNIT_STAT_CAN_NOT_REACT_OR_LOST_CONTROL))
    {
        ...
        {
            // 2.4.3+: "Passive pets will no longer attack due to AoE taunt
            // spells."
            if (ai->HasReactState(REACT_PASSIVE) && !unitTarget->GetVictim())
            {
                if (SpellTargetInfoTable[m_spellInfo->EffectImplicitTargetA[eff_idx]].enumerator !=
                    TARGET_ENUMERATOR_SINGLE)
                    return;

                if (SpellTargetInfoTable[m_spellInfo->EffectImplicitTargetB[eff_idx]].enumerator !=
                    TARGET_ENUMERATOR_SINGLE)
                    return;
            }
...
void Spell::EffectWeaponDmg(SpellEffectIndex eff_idx)
{
    ...
    float weaponDamagePercentMod = 1.0f; // applied to weapon damage (and to fixed effect damage bonus if
...
void Spell::EffectInterruptCast(SpellEffectIndex eff_idx)
{
    ...
    for (uint32 i = CURRENT_FIRST_NON_MELEE_SPELL; i < CURRENT_MAX_SPELL; ++i)
    {
        if (Spell *spell = unitTarget->GetCurrentSpell(CurrentSpellTypes(i)))
        {
            ...
            if ((curSpellInfo->InterruptFlags & SPELL_INTERRUPT_FLAG_COMBAT) &&
                curSpellInfo->PreventionType == SPELL_PREVENTION_TYPE_SILENCE && spell->CanBeInterrupted())
            {
                unitTarget->LockOutSpells(GetSpellSchoolMask(curSpellInfo),
                                          unitTarget->CalculateAuraDuration(m_spellInfo, (1 << eff_idx),
                                                                            GetSpellDuration(m_spellInfo), m_caster));
...
void Spell::EffectScriptEffect(SpellEffectIndex eff_idx)
{
    ...
    case SPELLFAMILY_GENERIC: {
        ...
        {
            // Knock Away variants and derrivatives with scripted threat
            // reduction component
            if (!unitTarget)
                return;
            // Default (most of the time) reduction is 50%. TODO: Verify when
            // possible per spell
            int32 pct = -50;
        ...
        case 24195: // Grom's Tribute
        {
            ...
            case 24194:
                ...
                case RACE_GNOME:
                    spellId = 24106;
                    break;
                // next case not exist in 2.x officially (quest has been
                // broken for race until 3.x time)
                case RACE_DRAENEI:
                    spellId = 24108;
                    break;
                }
        ...
        {
            unitTarget->CastSpell(unitTarget, 24321, TRIGGERED_OLD_TRIGGERED, nullptr, nullptr,
                                  m_caster->GetObjectGuid());
            return;
        }
        case 24590: // Brittle Armor - need remove one 24575 Brittle Armor
                    // aura
        ...
        case 24717: // Pirate Costume
        {
            ...
            m_caster->CastSpell(unitTarget, unitTarget->getGender() == GENDER_MALE ? 24708 : 24709,
                                TRIGGERED_OLD_TRIGGERED);
            return;
        }
        case 24718: // Ninja Costume
        {
            ...
            m_caster->CastSpell(unitTarget, unitTarget->getGender() == GENDER_MALE ? 24711 : 24710,
                                TRIGGERED_OLD_TRIGGERED);
            return;
        }
        case 24719: // Leper Gnome Costume
        {
            ...
            m_caster->CastSpell(unitTarget, unitTarget->getGender() == GENDER_MALE ? 24712 : 24713,
                                TRIGGERED_OLD_TRIGGERED);
            return;
        }
        ...
        case 24737: // Ghost Costume
        {
            ...
            m_caster->CastSpell(unitTarget, unitTarget->getGender() == GENDER_MALE ? 24735 : 24736,
                                TRIGGERED_OLD_TRIGGERED);
            return;
        }
        ...
        case 24935: // Summon RC Tank
        {
            ...
            m_caster->CastSpell(unitTarget, 24934,
                                TRIGGERED_OLD_TRIGGERED); // Summon RC Tank
            return;
        }
        ...
        case 26465: // Mercurial Shield - need remove one 26464 Mercurial
                    // Shield aura
            unitTarget->RemoveAuraHolderFromStack(26464);
            return;
        case 26524: // Sand Trap (AQ20 - Kurinnaxx, parent spell selecting
                    // one target)
        {
            if (unitTarget)
                unitTarget->CastSpell(unitTarget, 25648, TRIGGERED_OLD_TRIGGERED, nullptr, nullptr,
                                      m_caster->GetObjectGuid());
            return;
        }
        ...
        case 26656: // Summon Black Qiraji Battle Tank
        {
            ...
            // Two separate mounts depending on area id (allows use both in
            // and out of specific instance)
        ...
        case 27548: // Valentine - Stormwind City Guard
        {
            ...
            // Guard spellIds map [Pledge of Friendship , Pledge of
            // Adoration]
            ...
            if (loveAirSpellsMapForFaction.count(m_caster->GetFaction()))
            {
                ...
                else if (!unitTarget->HasAura(26680))
                {
                    PledgeGiftOrHeartbroken =
                        loveAirSpellsMapForFaction[m_caster->GetFaction()][1]; // Pledge of Adoration
                                                                               // for related faction
                    unitTarget->CastSpell(unitTarget, 26680,
                                          TRIGGERED_OLD_TRIGGERED); // Cast Adored
                }
                else
                    PledgeGiftOrHeartbroken =
                        loveAirSpellsMapForFaction[m_caster->GetFaction()][0]; // Pledge of Friendship
                                                                               // for related faction
                ...
                ((Player *)unitTarget)
                    ->DestroyItemCount(21815, 1, true,
                                       false); // Remove 1 love token on cast from inventory
            }
            return;
        }
        ...
        case 27550: // Valentine - Alliance Civilian
        {
            ...
            if (loveAirSpellsMapForFaction.count(m_caster->GetFaction()))
            {
                ...
                {
                    PledgeGiftOrHeartbroken =
                        loveAirSpellsMapForFaction[m_caster->GetFaction()][1]; // Gift of Adoration
                                                                               // for related faction
                    unitTarget->CastSpell(unitTarget, 26680,
                                          TRIGGERED_OLD_TRIGGERED); // Cast Adored
                }
                    ...
                    PledgeGiftOrHeartbroken =
                        loveAirSpellsMapForFaction[m_caster->GetFaction()][0]; // Gift of Friendship
                                                                               // for related faction
        ...
        case 27654: // Love is in the Air Test
        {
            if (unitTarget->GetTypeId() != TYPEID_PLAYER)
            {
                unitTarget->SetFlag(UNIT_NPC_FLAGS,
                                    UNIT_NPC_FLAG_GOSSIP); // Add gossip flag for NPC missing
                                                           // it
                if (!unitTarget->HasFlag(UNIT_NPC_FLAGS,
                                         UNIT_NPC_FLAG_INNKEEPER)) // Cast Amorous if caster
                                                                   // is not an innkeeper
        ...
        case 27687: // Summon Bone Minions
        {
            ...
            for (uint8 i = 0; i < 4; ++i)
            {
                unitTarget->GetNearPoint(unitTarget, x, y, z, unitTarget->GetObjectBoundingRadius(),
                                         INTERACTION_DISTANCE, angle + i * M_PI_F / 2);
        ...
        case 27749: // Initialize Tank
        {
            ...
            unitTarget->CastSpell(nullptr, 24936,
                                  TRIGGERED_OLD_TRIGGERED); // RC Tank Control
            unitTarget->CastSpell(nullptr, 27747,
                                  TRIGGERED_OLD_TRIGGERED); // Steam Tank Passive
            return;
        }
        ...
        case 28339: // Magnetic Pull
        {
            if (unitTarget && m_caster->GetVictim())
                unitTarget->CastSpell(m_caster->GetVictim(), 28337,
                                      TRIGGERED_OLD_TRIGGERED); // target cast actual Magnetic
                                                                // Pull on caster's victim
            // ToDo research if target should also get the threat of the
            // caster for caster's victim. This is the case in WotLK version
            // but we have no proof of this in Classic/TBC and it was common
            // at these times to let players manage threat and tank
        ...
        case 30952: // Shoot Flame Arrow (Shattered Halls)
        {
            ...
            unitTarget->CastSpell(nullptr, 30953,
                                  TRIGGERED_NONE); // cast explosion
            break;
        }
        case 37142: // Karazhan - Chess NPC Action: Melee Attack: Conjured
                    // Water Elemental
        case 37143: // Karazhan - Chess NPC Action: Melee Attack: Charger
        case 37147: // Karazhan - Chess NPC Action: Melee Attack: Human
                    // Cleric
        case 37149: // Karazhan - Chess NPC Action: Melee Attack: Human
                    // Conjurer
        case 37150: // Karazhan - Chess NPC Action: Melee Attack: King Llane
        case 37220: // Karazhan - Chess NPC Action: Melee Attack: Summoned
                    // Daemon
        case 32227: // Karazhan - Chess NPC Action: Melee Attack: Footman
        case 32228: // Karazhan - Chess NPC Action: Melee Attack: Grunt
        case 37337: // Karazhan - Chess NPC Action: Melee Attack: Orc
                    // Necrolyte
        case 37339: // Karazhan - Chess NPC Action: Melee Attack: Orc Wolf
        case 37345: // Karazhan - Chess NPC Action: Melee Attack: Orc Warlock
        case 37348: // Karazhan - Chess NPC Action: Melee Attack: Warchief
                    // Blackhand
        {
            if (!unitTarget)
                return;
        ...
        case 36291: {
            ...
            unitTarget->CastSpell(nullptr, spellId,
                                  TRIGGERED_NONE); // also triggered in sniff, only
                                                   // SMSG_SPELL_GO
            return;
        }
        ...
        case 37641: // Whirlwind
        {
            ...
            {
                ai->DoResetThreat();
                if (Unit *target = static_cast<Creature *>(m_caster)->SelectAttackingTarget(
                        ATTACKING_TARGET_RANDOM, 0, nullptr, SELECT_FLAG_PLAYER))
        ...
        case 38794: // Murmur's Touch - Heroic
        {
            ...
            for (Unit *pullTarget : objectList)
                unitTarget->CastSpell(pullTarget, 28337,
                                      TRIGGERED_OLD_TRIGGERED); // this is guesswork based on
                                                                // sniff
        ...
        case 39338: // Karazhan - Chess, Medivh CHEAT: Hand of Medivh, Target
                    // Horde
        case 39342: // Karazhan - Chess, Medivh CHEAT: Hand of Medivh, Target
                    // Alliance
        {
            if (!unitTarget)
                return;
        ...
        case 39341: // Karazhan - Chess, Medivh CHEAT: Fury of Medivh, Target
                    // Horde
        case 39344: // Karazhan - Chess, Medivh CHEAT: Fury of Medivh, Target
                    // Alliance
        {
            if (!unitTarget)
                return;
        ...
        case 46271: {
            ...
            Unit *target = ((Creature *)m_caster)
                               ->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, uint32(0), SELECT_FLAG_PLAYER);
            if (target)
            {
                ThreatList const &threatList = m_caster->getThreatManager().getThreatList();
        ...
        case 43783: // Spawn Guard
        {
            ...
            m_caster->GetRandomPoint(m_caster->GetPositionX(), m_caster->GetPositionY(), m_caster->GetPositionZ(),
                                     30.0f, pointX, pointY, pointZ, 20.0f);
            ...
            {
                Creature *pGuard =
                    m_caster->SummonCreature(guardEntry, pointX, pointY, pointZ, m_caster->GetOrientation(),
    ...
    case SPELLFAMILY_POTION: {
        ...
        case 28702: // Netherbloom
        {
            ...
            //       receiving a debuff. However, this seems to be removed by
            //       a patch.
...
void Spell::EffectSanctuary(SpellEffectIndex /*eff_idx*/)
{
    ...
    // Vanish allows to remove all threat and cast regular stealth so other
    // spells can be used
...
void Spell::EffectStuck(SpellEffectIndex /*eff_idx*/)
{
    ...
    // If the player is dead, it will return them to the graveyard closest to
    // their corpse.
    ...
    // If the player is alive, and their hearthstone is in their inventory, and
    // their hearthstone is cooled down, it will activate their hearthstone. The
    // 30 minute hearthstone cooldown is activated as usual.
    if (player->IsSpellReady(8690) && player->HasItemCount(6948, 1, false))
        player->CastSpell(nullptr, 8690,
                          TRIGGERED_OLD_TRIGGERED | TRIGGERED_INSTANT_CAST); // always needs to be instant
    else
    {
        if (player->GetMap()->IsDungeon()) // teleport to safe location at entrance
                                           // - avoids abusing of terrain bugs
        {
        ...
        {
            // If the player is alive, but their hearthstone is either not in
            // their inventory (e.g. in the bank) or their hearthstone is on
            // cooldown, then the game will try to "nudge" the player in a
            // seemingly random direction.
            // @todo This check could possibly more accurately find a safe
            // position to port to, has the potential for porting underground.
            float x, y, z;
...
void Spell::EffectActivateObject(SpellEffectIndex effIdx)
{
    ...
    case GameObjectActions::ANIMATE_CUSTOM_3:
        gameObjTarget->SendGameObjectCustomAnim(gameObjTarget->GetObjectGuid(),
                                                uint32(action) - uint32(GameObjectActions::ANIMATE_CUSTOM_0));
        break;
    ...
    case GameObjectActions::OPEN:
        ...
        case 42868:
        case 45222: // Zul'Aman Object Visual - This spell is a custom GO cast
                    // spell and similar to Ritual of Souls where only channel
                    // start should be sent
            gameObjTarget->SendGameObjectCustomAnim(gameObjTarget->GetObjectGuid());
            break;
    ...
    case GameObjectActions::USE_ART_KIT_3: {
        ...
        if (artKitValue == 0)
            sLog.outError("GameObject %d hit by spell %d needs `artkit%d` in "
...
bool Spell::DoSummonTotem(CreatureSummonPositions &list, SpellEffectIndex eff_idx, uint8 slot_dbc)
{
    ...
    // Create but some Create calls can be dependent from proper position if
    // totem have creature_template_addon.auras with persistent point for example
    // or script call
    ...
    if (!cinfo)
    {
        sLog.outErrorDb("Creature entry %u does not exist but used in spell %u "
                        "totem summon.",
                              ...
                              uint32 /*level*/)
{
    ...
    if (!newUnit)
    {
        sLog.outError("Spell::DoSummonPossessed: creature entry %d for spell %u "
                      "could not be summoned.",
                      creatureEntry, m_spellInfo->Id);
        return false;
    }
...
void Spell::EffectSummonObject(SpellEffectIndex eff_idx)
{
    ...
    Map *map = m_caster->GetMap();
    if (!pGameObj->Create(map->GenerateLocalLowGuid(HIGHGUID_GAMEOBJECT), go_id, map, x, y, z,
...
void Spell::EffectResurrect(SpellEffectIndex eff_idx)
{
    ...
    case 22999: // Defibrillate (Goblin Jumper Cables XL) has 50% chance on
                // success
    {
        uint32 failChance = 0;
    ...
    if (corpseTarget)
    {
        ...
        if (!player) // thread unsafe version - corpse and player in different
                     // maps
        {
            std::string sentName = m_caster->GetTypeId() == TYPEID_PLAYER ? "" : m_caster->GetName();
            sWorld.GetMessager().AddMessage([playerGuid = corpseTarget->GetOwnerGuid(),
                                             position = m_caster->GetPosition(), casterGuid = m_caster->GetObjectGuid(),
                                             effectId, damage = damage, spellInfo = m_spellInfo, eff_idx = eff_idx,
                                             mapId = m_caster->GetMapId(), isSpiritHealer = m_caster->isSpiritHealer(),
                                             sentName, computator](World *world) {
                if (Player *player = ObjectAccessor::FindPlayer(playerGuid))
                {
                    if (player->isRessurectRequested()) // already have one
                                                        // active request
                        return;
                    uint32 health;
                    uint32 mana;
                    computator(player, health, mana, spellInfo->Effect[eff_idx], damage,
                               spellInfo->EffectMiscValue[eff_idx]);
                    char const *name = sentName.data();
                    if (!casterGuid.IsPlayer())
                        sObjectMgr.GetCreatureLocaleStrings(casterGuid.GetEntry(),
                                                            player->GetSession()->GetSessionDbLocaleIndex(), &name);
                    player->AddResurrectRequest(casterGuid, spellInfo, position, mapId, health, mana, isSpiritHealer,
                                                name);
                }
            });
        }
    ...
    if (player)
    {
        ...
        player->AddResurrectRequest(
            m_caster->GetObjectGuid(), m_spellInfo, m_caster->GetPosition(), m_caster->GetMapId(), health, mana,
            m_caster->isSpiritHealer(),
...
void Spell::EffectCharge(SpellEffectIndex /*eff_idx*/)
{
    ...
    // TODO: This is executed after spell effects. Verify if this should be
    // executed before spell effects
                            ...
                            uint32 /*level*/)
{
    ...
    if (!cInfo)
    {
        sLog.outErrorDb("Spell::DoSummonCritter: creature entry %u not found for spell %u.", pet_entry,
                        m_spellInfo->Id);
        return false;
    }
    ...
    // for (CreatureSummonPositions::iterator itr = list.begin(); itr !=
    // list.end(); ++itr)
    CreatureCreatePos pos(m_caster->GetMap(), list[0].x, list[0].y, list[0].z, m_caster->GetOrientation());
    ...
    critter->SelectLevel(); // some summoned critters have different from 1 DB
                            // data for level/hp
    const CreatureInfo *info = critter->GetCreatureInfo();

    critter->SetUInt32Value(UNIT_FIELD_FLAGS, info->UnitFlags);
    critter->SetUInt32Value(UNIT_NPC_FLAGS,
                            info->NpcFlags); // some companions may have quests,
                                             // so they need npc flags
    critter->InitPetCreateSpells();          // some companions may have spells (e.g.
                                             // disgusting oozeling)
    ...
    if (m_caster->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PLAYER_CONTROLLED))
    {
        ...
        critter->SetByteValue(UNIT_FIELD_BYTES_2, UNIT_BYTES_2_OFFSET_DEBUFF_LIMIT,
                              UNIT_BYTE2_PLAYER_CONTROLLED_DEBUFF_LIMIT);
    }
    else
        critter->SetByteValue(UNIT_FIELD_BYTES_2, UNIT_BYTES_2_OFFSET_DEBUFF_LIMIT, UNIT_BYTE2_CREATURE_DEBUFF_LIMIT);

    // NOTE: All companions should have these (creatureinfo needs to be tuned
    // accordingly before we can remove these two lines):
...
void Spell::EffectKnockBack(SpellEffectIndex eff_idx)
{
    ...
    case 37852: // Watery Grave Explosion
        if (m_triggeredByAuraSpell)
            unitTarget->RemoveAurasDueToSpell(m_triggeredByAuraSpell->Id); // Remove Watery Grave to prevent root
                                                                           // affecting knockback
        break;
    }
...
void Spell::EffectSummonDeadPet(SpellEffectIndex /*eff_idx*/)
{
    ...
    {
        pet->ResetCorpseRespawn(); // need to absolutely make sure pet is
                                   // visible - despawned corpse - invisible -
                                   // assert
    ...
    for (int slot = 0; slot < MAX_TOTEM_SLOT; ++slot)
    {
        if (Totem *totem = m_caster->GetTotem(TotemSlot(slot)))
        {
            if (damage)
            {
                ...
                {
                    uint32 manacost =
                        spellInfo->manaCost + m_caster->GetCreateMana() * spellInfo->ManaCostPercentage / 100;
...
void Spell::EffectTransmitted(SpellEffectIndex eff_idx)
{
    ...
    if (!goinfo)
    {
        sLog.outErrorDb("Gameobject (Entry: %u) not exist and not created at spell "
                        "(ID: %u) cast",
    ...
    else
    {
        ...
        // special code for fishing bobber (TARGET_LOCATION_CASTER_FISHING_SPOT),
        // should not try to avoid objects nor try to find ground level, but
        // randomly vary in angle
    ...
    // Other GO types have startOpen attribute but only GAMEOBJECT_TYPE_BUTTON
    // is used by spells
    case GAMEOBJECT_TYPE_BUTTON: {
        // Change GO state if it is supposed to be already activated at
        // summoning
...
void Spell::EffectSpiritHeal(SpellEffectIndex /*eff_idx*/)
{
    ...
    if (Player *player = static_cast<Player *>(unitTarget))
    {
        ...
        // resurrects last active minion - Imp, Voidwalker, Succubus, Felhunter,
        // or Felguard if Doomguard or Infernal were last active minion, or
        // previous minion died before Warlock - Imp summoned instead
...
void Spell::EffectCreateTamedPet(SpellEffectIndex eff_idx)
{
    ...
    {
        sLog.outErrorDb("EffectCreateTamedPet: Creature entry %u not found for spell %u.", creatureEntry,
                        m_spellInfo->Id);
        return;
    }
    ...
    if (m_caster->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PLAYER_CONTROLLED))
    {
        ...
        newTamedPet->SetByteValue(UNIT_FIELD_BYTES_2, UNIT_BYTES_2_OFFSET_DEBUFF_LIMIT,
                                  UNIT_BYTE2_PLAYER_CONTROLLED_DEBUFF_LIMIT);
    }
    else
        newTamedPet->SetByteValue(UNIT_FIELD_BYTES_2, UNIT_BYTES_2_OFFSET_DEBUFF_LIMIT,
