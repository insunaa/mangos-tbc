...
bool ChatHandler::GetDeletedCharacterInfoList(DeletedInfoList &foundList, std::string searchString)
{
    ...
    if (!searchString.empty())
    {
        ...
        if (isNumeric(searchString))
            resultChar = CharacterDatabase.PQuery("SELECT guid, deleteInfos_Name, deleteInfos_Account, deleteDate "
                                                  "FROM "
        ...
        else
        {
            ...
            resultChar = CharacterDatabase.PQuery("SELECT guid, deleteInfos_Name, deleteInfos_Account, deleteDate "
                                                  "FROM "
                                                  "characters WHERE deleteDate IS NOT NULL AND "
                                                  "deleteInfos_Name " _LIKE_ " " _CONCAT3_("'%%'", "'%s'", "'%%'"),
                                                  searchString.c_str());
        }
    }
    else
...
#ifdef __unix__
    // Set stdin IO to nonblocking - prevent Server from hanging in shutdown
    // process till enter is pressed
    int fd = fileno(stdin);
