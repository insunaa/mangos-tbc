...
bool ChatHandler::HandleMuteCommand(char *args)
{
    ...
    uint32 account_id =
        target ? target->GetSession()->GetAccountId() : sObjectMgr.GetPlayerAccountIdByGUID(target_guid);
...
void ChatHandler::ShowTriggerListHelper(AreaTriggerEntry const *atEntry)
{
    ...
    if (m_session)
    {
        ...
        PSendSysMessage(LANG_TRIGGER_LIST_CONSOLE, atEntry->id, atEntry->mapid, atEntry->x, atEntry->y, atEntry->z,
                        tavern, quest);
...
bool ChatHandler::HandleTriggerActiveCommand(char * /*args*/)
{
    ...
    for (uint32 id = 0; id < sAreaTriggerStore.GetNumRows(); ++id)
    {
        ...
        if (!IsPointInAreaTriggerZone(atEntry, pl->GetMapId(), pl->GetPositionX(), pl->GetPositionY(),
                                      pl->GetPositionZ()))
            continue;
 ...
 *                             Warning: If there is more than one mob with this
 * name you will be teleported to the first one that is found. .go creature id
 * 6109    --> TP player to the mob, that has this creature_template.entry
 *                             Warning: If there is more than one mob with this
 * "id" you will be teleported to the first one that is found.
 */
...
bool ChatHandler::HandleGoCreatureCommand(char *args)
{
    ...
    // "id" or number or [name] Shift-click form
    // |color|Hcreature:creature_id|h[name]|h|r
    int crType;
    ...
    case CREATURE_LINK_RAW: {
        ...
        else
        {
            ...
            QueryResult *result = WorldDatabase.PQuery("SELECT creature.guid, creature_spawn_entry.guid "
                                                       ...
                                                       "OR (creature_spawn_entry.entry = creature_template.entry AND "
                                                       "creature.guid = creature_spawn_entry.guid)) "
    ...
    if (!data)
    {
        ...
        //  data is nullptr so this mean either the guid/entry provided was wrong
        //  or its a summoned creature. Lets give a chance to find a summoned
        //  creature.
        ...
        if (wObj)
        {
            ...
            return HandleGoHelper(_player, wObj->GetMapId(),
        wObj->GetPositionX(), wObj->GetPositionY(), &z);
        }*/
...
bool ChatHandler::HandleGoObjectCommand(char *args)
{
    ...
    {
        // number or [name] Shift-click form
        // |color|Hgameobject_entry:creature_id|h[name]|h|r
    ...
    case GAMEOBJECT_LINK_RAW: {
        ...
        else
        {
            ...
            QueryResult *result = WorldDatabase.PQuery("SELECT gameobject.guid, gameobject_spawn_entry.guid "
                                                       ...
                                                       "OR (gameobject_spawn_entry.entry = gameobject_template.entry "
                                                       "AND "
                                                       "gameobject.guid = gameobject_spawn_entry.guid)) "
                                                       "AND gameobject_template.name LIKE '%%%s%%' LIMIT 1;",
                                                       name.c_str());
            if (!result)
            {
                SendSysMessage(LANG_COMMAND_GOOBJNOTFOUND);
...
bool ChatHandler::HandleGameObjectTargetCommand(char *args)
{
    ...
    if (*args)
    {
        ...
        {
            result =
                WorldDatabase.PQuery("SELECT guid, id, position_x, position_y, position_z, "
                                     "orientation, "
                                     "map, (POW(position_x - '%f', 2) + POW(position_y - '%f', 2) + "
                                     "POW(position_z - '%f', 2)) AS order_ FROM gameobject WHERE map "
                                     "= "
                                     "'%i' AND guid = '%u' ORDER BY order_ ASC LIMIT 1",
        ...
        else
        {
            ...
            result = WorldDatabase.PQuery("SELECT guid, id, position_x, position_y, position_z, "
                                          ...
                                          pl->GetPositionX(), pl->GetPositionY(), pl->GetPositionZ(), pl->GetMapId(),
                                          name.c_str());
        }
    }
    else
    {
        ...
        result = WorldDatabase.PQuery("SELECT gameobject.guid, id, position_x, position_y, position_z, "
                                      "orientation, map, "
    ...
    if (target)
    {
        ...
        if (target->GetGoType() == GAMEOBJECT_TYPE_DOOR)
            PSendSysMessage(LANG_COMMAND_GO_STATUS_DOOR, uint32(target->GetGoState()), uint32(target->GetLootState()),
                            GetOnOffStr(target->IsCollisionEnabled()), goI && goI->door.startOpen ? "open" : "closed");
        else
...
bool ChatHandler::HandleGameObjectAddCommand(char *args)
{
    ...
    if (gInfo->displayId && !sGameObjectDisplayInfoStore.LookupEntry(gInfo->displayId))
    {
        ...
        sLog.outErrorDb("Gameobject (Entry %u GoType: %u) have invalid displayId "
                        "(%u), not spawned.",
...
bool ChatHandler::HandleGameObjectNearCommand(char *args)
{
    ...
    Player *pl = m_session->GetPlayer();
    QueryResult *result =
        WorldDatabase.PQuery("SELECT guid, id, position_x, position_y, position_z, map, "
                             "(POW(position_x - '%f', 2) + POW(position_y - '%f', 2) + "
                             "POW(position_z "
                             "- '%f', 2)) AS order_ "
                             ...
                             "order_",
                             pl->GetPositionX(), pl->GetPositionY(), pl->GetPositionZ(), pl->GetMapId(),
                             pl->GetPositionX(), pl->GetPositionY(), pl->GetPositionZ(), distance * distance);
    ...
    if (result)
    {
        do
        {
            ...
            PSendSysMessage(LANG_GO_MIXED_LIST_CHAT, guid,
                            PrepareStringNpcOrGoSpawnInformation<GameObject>(guid).c_str(), entry, guid, name, x, y, z,
                            mapid);
...
bool ChatHandler::HandleGameObjectNearSpawnedCommand(char *args)
{
    ...
    for (GameObject *go : gameobjects)
    {
        ...
        PSendSysMessage(LANG_GO_MIXED_LIST_CHAT, guid.GetCounter(),
                        PrepareStringNpcOrGoSpawnInformation<GameObject>(guid).c_str(), entry, guid, goInfo->name, x, y,
                        z, go->GetMapId());
    }
                                        ...
                                        FactionState const *repState /*= nullptr*/, Player *target /*= nullptr */)
{
    ...
    // team] [unknown] [invisible] [inactive]" format or              "id -
    // [faction] [no reputation]" format
...
bool ChatHandler::HandleNpcMoveCommand(char *args)
{
    ...
    WorldDatabase.PExecuteLog("UPDATE creature SET position_x = '%f', position_y = '%f', position_z = "
...
bool ChatHandler::HandleNpcSpawnDistCommand(char *args)
{
    ...
    // respawn alive creature to reinitialize everything including movement
    // generator (dead one will do it anyway at respawn)
...
bool ChatHandler::HandleNpcTameCommand(char * /*args*/)
{
    ...
    player->CastSpell(creatureTarget, 13481,
                      TRIGGERED_OLD_TRIGGERED); // Tame Beast, triggered effect
    return true;
}
...
bool ChatHandler::HandleNpcFormationSwitchCommand(char *args)
{
    ...
    {
        PSendSysMessage("Formation shape id should be in 0..%u range!",
                        static_cast<uint32>(SpawnGroupFormationType::SPAWN_GROUP_FORMATION_TYPE_COUNT) - 1);
        return true;
    }
...
bool ChatHandler::HandlePInfoCommand(char *args)
{
    ...
    else
    {
        ...
        //                                                     0          1 2 3
        QueryResult *result = CharacterDatabase.PQuery("SELECT totaltime, level, money, account FROM "
    ...
    QueryResult *result = LoginDatabase.PQuery("SELECT username,gmlevel,ip,loginTime FROM account "
                                               "a JOIN account_logons b ON(a.id=b.accountId) WHERE "
                                               "a.id = '%u' ORDER BY loginTime DESC LIMIT 1",
                                               accId);
    if (result)
    {
        Field *fields = result->Fetch();
 ...
 * -> adds a waypoint to the currently selected creature, to path pathId in
 * source-storage
 *
...
bool ChatHandler::HandleWpAddCommand(char *args)
{
    ...
    if (wpDestination == PATH_NO_PATH) // No Waypoint selected, parse additional params
    {
        if (ExtractOptUInt32(&args, wpPathId, 0)) // Fill path-id and source
        {
            ...
            {
                if (wpPathId != 0)
                    wpDestination = PATH_FROM_ENTRY; // Multiple Paths must only be
                                                     // assigned by entry
            }
        }
        ...
        if (wpDestination == PATH_NO_PATH) // No overwrite params. Do best estimate
        {
            if (wpOwner->GetMotionMaster()->GetCurrentMovementGeneratorType() == WAYPOINT_MOTION_TYPE)
                if (WaypointMovementGenerator<Creature> const *wpMMGen =
                        dynamic_cast<WaypointMovementGenerator<Creature> const *>(
                            wpOwner->GetMotionMaster()->GetCurrent()))
                    wpMMGen->GetPathInformation(wpPathId, wpDestination);

            // Get information about default path if no current path. If no
            // default path, prepare data dependendy on uniqueness
    ...
    // wpOwner will get a new waypoint inserted into wpPath =
    // GetPathFromOrigin(wpOwner, wpDestination, wpPathId) at wpPointId
...
/**
 * .wp modify waittime | scriptid | orientation | del | move [dbGuid, id]
 * [value]
 *
 * waittime <Delay>
 ...
 *   Everytime the NPC comes to this waypoint, it will wait Delay
 * millieseconds.
 *
 * scriptid <scriptId>
 *   User has selected a visual waypoint before.
 ...
 *   Everytime the NPC comes to this waypoint, the DBScript scriptId is
 * executed.
 *
 * orientation [DBGuid, WpNum] <Orientation>
 *   Set the orientation of the selected waypoint or waypoint given with
 * DbGuid/ WpId to the value of <Orientation>.
 *
 * del [DBGuid, WpId]
 *   Remove the selected waypoint or waypoint given with DbGuid/ WpId.
 *
 * move [DBGuid, WpId]
 *   Move the selected waypoint or waypoint given with DbGuid/ WpId to player's
 * current positiion.
 */
bool ChatHandler::HandleWpModifyCommand(char *args)
{
    ...
    if (subCmd == "del") // Remove WP, no additional command required
    {
        ...
        if (wpPath->empty())
        {
            ...
            if (wpOwner->IsAlive()) // Dead creature will reset movement
    ...
    else if (subCmd == "scriptid")
    {
        ...
        if (!sWaypointMgr.SetNodeScriptId(wpOwner->GetEntry(), wpOwner->GetGUIDLow(), wpId, wpPathId, wpSource,
                                          scriptId))
...
bool ChatHandler::HandleWpShowCommand(char *args)
{
    ...
    {
        if (!ExtractUInt32(&args,
                           dbGuid)) // No creature selected and no dbGuid provided
            return false;
    ...
    if (subCmd == "first")
    {
        ...
        // player->GetPlayerMenu()->SendPointOfInterest(x, y, 6, 6, 0, "First
        // Waypoint");
        return true;
    }
    ...
    if (subCmd == "last")
    {
        ...
        // player->GetPlayerMenu()->SendPointOfInterest(x, y, 6, 6, 0, "Last
        // Waypoint");
        return true;
    }
...
bool ChatHandler::HandleWpExportCommand(char *args)
{
    ...
    // wpOwner is now known, in case of export by visual waypoint also the to be
    // exported path
    ...
    if (wpOrigin == PATH_NO_PATH) // No WP selected, Extract optional arguments
    {
        if (ExtractOptUInt32(&args, wpPathId, 0)) // Fill path-id and source
        {
            ...
            {
                if (wpPathId != 0)
                    wpOrigin = PATH_FROM_ENTRY; // Multiple Paths must only be
                                                // assigned by entry
            }
        }
    ...
    if (wpOrigin != PATH_FROM_EXTERNAL)
        outfile << "INSERT INTO " << table << " (" << key_field
                << ", Point, PositionX, PositionY, PositionZ, Orientation, "
                   "WaitTime, ScriptId) VALUES\n";
    else
        outfile << "INSERT INTO " << table << " (" << key_field
                << ", Point, PositionX, PositionY, positionZ, Orientation, "
                   "WaitTime) VALUES\n";
...
bool ChatHandler::HandleLookupAccountEmailCommand(char *args)
{
    ...
    //                                                 0   1         2        3 4
    QueryResult *result = LoginDatabase.PQuery("SELECT a.id, username, ip, gmlevel, expansion FROM account a join "
...
bool ChatHandler::HandleLookupAccountIpCommand(char *args)
{
    ...
    //                                                 0            1         2 3
    //                                                 4
    QueryResult *result = LoginDatabase.PQuery("SELECT distinct id, username, ip, gmlevel, expansion FROM account a "
                                               "join account_logons b on(a.id=b.accountId) WHERE ip " _LIKE_
                                               " " _CONCAT3_("'%%'", "'%s'", "'%%'"),
                                               ip.c_str());
...
bool ChatHandler::ShowAccountListHelper(QueryResult *result, uint32 *limit, bool title, bool error)
{
    ...
    do
    {
        ...
        if (m_session)
            PSendSysMessage(LANG_ACCOUNT_LIST_LINE_CHAT, account, fields[1].GetString(), char_name,
                            fields[2].GetString(), fields[3].GetUInt32(), fields[4].GetUInt32());
        else
            PSendSysMessage(LANG_ACCOUNT_LIST_LINE_CONSOLE, account, fields[1].GetString(), char_name,
                            fields[2].GetString(), fields[3].GetUInt32(), fields[4].GetUInt32());
    } while (result->NextRow());
...
bool ChatHandler::HandleLookupPlayerIpCommand(char *args)
{
    ...
    QueryResult *result = LoginDatabase.PQuery("SELECT a.id, username, distinct ip FROM account a join account_logons "
                                               "b "
                                               "on (a.id=b.accountId) WHERE b.ip " _LIKE_
                                               " " _CONCAT3_("'%%'", "'%s'", "'%%'  ORDER BY loginTime DESC LIMIT 1"),
                                               ip.c_str());
...
bool ChatHandler::HandleLookupPlayerAccountCommand(char *args)
{
    ...
    QueryResult *result = LoginDatabase.PQuery("SELECT id,username FROM account WHERE username " _LIKE_
                                               " " _CONCAT3_("'%%'", "'%s'", "'%%'"),
                                               account.c_str());
...
bool ChatHandler::LookupPlayerSearchCommand(QueryResult *result, uint32 *limit)
{
    ...
    do
    {
        ...
        QueryResult *chars = CharacterDatabase.PQuery("SELECT guid, name, race, class, level FROM "
...
void ChatHandler::ShowPoolListHelper(uint16 pool_id)
{
    ...
    if (m_session)
        PSendSysMessage(LANG_POOL_ENTRY_LIST_CHAT, uint32(pool_id), uint32(pool_id), pool_template.description.c_str(),
                        pool_template.AutoSpawn ? 1 : 0, pool_template.MaxLimit,
                        sPoolMgr.GetPoolCreatures(pool_id).size(), sPoolMgr.GetPoolGameObjects(pool_id).size(),
...
bool ChatHandler::HandleLookupPoolCommand(char *args)
{
    ...
    // spawn pools for expected map or for not initialized shared pools state for
...
bool ChatHandler::HandlePoolListCommand(char * /*args*/)
{
    ...
    if (counter == 0)
        PSendSysMessage(LANG_NO_POOL_FOR_MAP, mapState->GetMapEntry()->name[GetSessionDbcLocale()],
                        mapState->GetMapId());
            ...
            if (CreatureData const *data = sObjectMgr.GetCreatureData(crSpawn))
                if (CreatureInfo const *info = ObjectMgr::GetCreatureTemplate(data->id))
                    PSendSysMessage(LANG_CREATURE_LIST_CHAT, crSpawn,
                                    PrepareStringNpcOrGoSpawnInformation<Creature>(crSpawn).c_str(), crSpawn,
                                    info->Name, data->posX, data->posY, data->posZ, data->mapid);
            ...
            if (GameObjectData const *data = sObjectMgr.GetGOData(goSpawn))
                if (GameObjectInfo const *info = ObjectMgr::GetGameObjectInfo(data->id))
                    PSendSysMessage(LANG_GO_LIST_CHAT, goSpawn,
                                    PrepareStringNpcOrGoSpawnInformation<GameObject>(goSpawn).c_str(), goSpawn,
                                    info->name, data->posX, data->posY, data->posZ, data->mapid);
...
bool ChatHandler::HandlePoolInfoCommand(char *args)
{
    ...
    if (!poolCreaturesEx.empty())
    {
        ...
        for (auto itr : poolCreaturesEx)
        {
            if (CreatureData const *data = sObjectMgr.GetCreatureData(itr.guid))
            {
                ...
                {
                    char const *active =
                        crSpawns && crSpawns->find(itr.guid) != crSpawns->end() ? active_str.c_str() : "";
                    if (m_session)
                        PSendSysMessage(LANG_POOL_CHANCE_CREATURE_LIST_CHAT, itr.guid,
                                        PrepareStringNpcOrGoSpawnInformation<Creature>(itr.guid).c_str(), itr.guid,
                                        info->Name, data->posX, data->posY, data->posZ, data->mapid, itr.chance,
                                        active);
                    else
                        PSendSysMessage(LANG_POOL_CHANCE_CREATURE_LIST_CONSOLE, itr.guid,
                                        PrepareStringNpcOrGoSpawnInformation<Creature>(itr.guid).c_str(), info->Name,
    ...
    if (!poolGameObjectsEx.empty())
    {
        ...
        for (auto itr : poolGameObjectsEx)
        {
            if (GameObjectData const *data = sObjectMgr.GetGOData(itr.guid))
            {
                ...
                {
                    char const *active =
                        goSpawns && goSpawns->find(itr.guid) != goSpawns->end() ? active_str.c_str() : "";
                    if (m_session)
                        PSendSysMessage(LANG_POOL_CHANCE_GO_LIST_CHAT, itr.guid,
                                        PrepareStringNpcOrGoSpawnInformation<GameObject>(itr.guid).c_str(), itr.guid,
                                        info->name, data->posX, data->posY, data->posZ, data->mapid, itr.chance,
                                        active);
                    else
                        PSendSysMessage(LANG_POOL_CHANCE_GO_LIST_CONSOLE, itr.guid,
                                        PrepareStringNpcOrGoSpawnInformation<GameObject>(itr.guid).c_str(), info->name,
                                        data->posX, data->posY, data->posZ, data->mapid, itr.chance, active);
                }
            }
        }
    ...
    if (!poolPoolsEx.empty())
    {
        ...
        for (auto itr : poolPoolsEx)
        {
            ...
            char const *active =
                poolSpawns && poolSpawns->find(itr.guid) != poolSpawns->end() ? active_str.c_str() : "";
            if (m_session)
                PSendSysMessage(LANG_POOL_CHANCE_POOL_LIST_CHAT, itr.guid, itr.guid, itr_template.description.c_str(),
                                ...
                                sPoolMgr.GetPoolGameObjects(itr.guid).size(), sPoolMgr.GetPoolPools(itr.guid).size(),
                                itr.chance, active);
            else
    ...
    if (!poolPoolsEq.empty())
    {
        ...
        for (auto itr : poolPoolsEq)
        {
            ...
            if (m_session)
                PSendSysMessage(
                    LANG_POOL_POOL_LIST_CHAT, itr.guid, itr.guid, itr_template.description.c_str(),
                    itr_template.AutoSpawn ? 1 : 0, itr_template.MaxLimit, sPoolMgr.GetPoolCreatures(itr.guid).size(),
                    sPoolMgr.GetPoolGameObjects(itr.guid).size(), sPoolMgr.GetPoolPools(itr.guid).size(), active);
            else
                PSendSysMessage(
                    LANG_POOL_POOL_LIST_CONSOLE, itr.guid, itr_template.description.c_str(),
                    itr_template.AutoSpawn ? 1 : 0, itr_template.MaxLimit, sPoolMgr.GetPoolCreatures(itr.guid).size(),
                    sPoolMgr.GetPoolGameObjects(itr.guid).size(), sPoolMgr.GetPoolPools(itr.guid).size(), active);
        }
    }
    ...
    for (uint32 id = 0; id < sCharTitlesStore.GetNumRows(); ++id)
    {
        ...
        if (titleInfo)
        {
            ...
            if (loc < MAX_LOCALE)
            {
                ...
                if (m_session)
                    PSendSysMessage(LANG_TITLE_LIST_CHAT, id, titleInfo->bit_index, id, titleNameStr, localeNames[loc],
                                    knownStr, activeStr);
                else
                    PSendSysMessage(LANG_TITLE_LIST_CONSOLE, id, titleInfo->bit_index, titleNameStr, localeNames[loc],
                                    knownStr, activeStr);
    ...
    for (uint32 id = 0; id < sCharTitlesStore.GetNumRows(); ++id)
    {
        ...
        if (titleInfo && target->HasTitle(titleInfo))
        {
            ...
            if (m_session)
                ...
                PSendSysMessage(LANG_TITLE_LIST_CONSOLE, id, titleInfo->bit_index, name.c_str(), localeNames[loc],
                                knownStr, activeStr);
        }
    }
    return true;
...
bool ChatHandler::HandleMmapPathCommand(char *args)
{
    ...
    if (para)
    {
        ...
        else
        {
            ...
            PSendSysMessage("Use '.mmap path to_me' to generate path from the "
                            "target to you.");
        }
    }
