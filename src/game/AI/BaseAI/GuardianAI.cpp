...
GuardianAI::GuardianAI(Creature *creature)
    : CreatureEventAI(creature), m_followAngle(PET_FOLLOW_ANGLE), m_followDist(PET_FOLLOW_DIST)
{
...
void GuardianAI::UpdateAI(const uint32 diff)
{
    ...
    case REACT_DEFENSIVE: {
        ...
        if (!m_creature->IsInCombat() && owner->IsInCombat() &&
            ...
            AttackStart(owner->getAttackerForHelper()); // check for
                                                        // getAttackerForHelper()
                                                        // == nullpter in AttackStart()
        break;
    }
    default:
        break;
