...
enum JOB_TYPE
{
    JOB_MAIN_TANK = 0x01, // for main tank that will need specific heal focus and priority
                          // over other party members or even regular tanks
    ...
    JOB_MANAONLY = 0x40   // for buff checking (NOTE: this means any with powertype mana AND
                          // druids (who may be shifted but still have mana)
};
...
struct heal_priority
{
    ...
    // overriding the operator like this is not recommended for general use -
    // however we won't use this struct for anything else
    bool operator<(const heal_priority &a) const
    {
        return type < a.type;
    }
};
...
class PlayerbotClassAI
{
  public:
    ...
    virtual uint32 Neutralize(uint8 /*creatureType*/)
    {
        return 0;
    }
    ...
    {
        m_WaitUntil = m_ai.CurrentTime() + t;
    }
    void ClearWait()
    {
        m_WaitUntil = 0;
  ...
  protected:
    ...
    // These values are used in GetHealTarget and can be overridden per class (to
    // accomodate healing spell health checks)
