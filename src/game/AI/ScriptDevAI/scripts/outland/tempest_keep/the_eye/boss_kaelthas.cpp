...
enum
{
    ...
    SPELL_FLOATING_DROWNED = 36550, // guesswork based on video confirmation, now on retail one spell
                                    // handles all - changed visual
...
struct boss_kaelthasAI : public ScriptedAI
{
    ...
    void MoveInLineOfSight(Unit *who) override
    {
        if (m_uiPhase == PHASE_0_NOT_BEGUN && who->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PLAYER_CONTROLLED) &&
            m_creature->IsWithinDistInMap(who, m_creature->GetAttackDistance(who)) &&
            m_creature->IsWithinLOSInMap(who) && m_creature->CanAttackOnSight(who))
        {
    ...
    {
        // Handle gravity lapse teleport - each player hit has his own teleport
        // spell
        ...
        case POINT_ID_CENTER: {
            ...
            if (m_worldTriggersFirstStage.empty())
            {
                ...
                if (triggers.size() < TRIGGERS_TOTAL)
                    sLog.outErrorDb("Kaelthas Sunstrider script is missing World Trigger "
                                    "Large NPCs, current count %lu, expected count %u",
                ...
                }); // sort based on guid, first 6 are first stage
                    // triggers, rest is second stage
            ...
            if (m_actionReadyStatus[i])
            {
                ...
                case KAEL_ACTION_PHASE_5: {
                    if (m_creature->GetHealthPercent() < 50.0f)
                    {
                        ...
                        m_creature->GetMotionMaster()->MovePoint(POINT_ID_CENTER, aCenterPos[0], aCenterPos[1],
                                                                 aCenterPos[2], FORCED_MOVEMENT_RUN);
                ...
                case KAEL_ACTION_FIREBALL: // TODO: make fireball have CD in
                                           // melee mode
                {
                ...
                if (m_phaseTransitionPureNetherBeamTimer <= uiDiff)
                {
                        ...
                        for (uint32 i = 0; i < TRIGGERS_GROUP_ONE; ++i)
                            if (urand(0, 1)) // 50% chance per trigger
                                if (Creature *trigger = m_creature->GetMap()->GetCreature(
                                        m_worldTriggersSecondStage[groupOneIndices[i]]))
                    ...
                    case 1:
                        for (uint32 i = 0; i < TRIGGERS_GROUP_TWO; ++i)
                            if (urand(0, 1)) // 50% chance per trigger
                                if (Creature *trigger = m_creature->GetMap()->GetCreature(
                                        m_worldTriggersSecondStage[groupTwoIndices[i]]))
                    ...
                    case 2:
                        for (uint32 i = 0; i < TRIGGERS_GROUP_THREE; ++i)
                            if (urand(0, 1)) // 50% chance per trigger
                                if (Creature *trigger = m_creature->GetMap()->GetCreature(
                                        m_worldTriggersSecondStage[groupThreeIndices[i]]))
                    ...
                    if (m_phaseTransitionPureNetherBeamStage < 30) // 30 pure nether beam cycles on retail
                        m_phaseTransitionPureNetherBeamTimer =
                            std::max(0,
                                     400 - int32(uiDiff)); // need to roll over timer to
                                                           // achieve blizzlike behaviour
                    else
                        m_phaseTransitionPureNetherBeamTimer = 0;
                }
                else
...
{
    boss_thaladred_the_darkenerAI(Creature *creature) : advisor_base_ai(creature)
    {
        Reset();
    }
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (m_uiGazeTimer <= uiDiff)
        {
            ...
            if (Unit *target = m_creature->SelectAttackingTarget(
                    ATTACKING_TARGET_RANDOM, 0, nullptr, SELECT_FLAG_PLAYER | SELECT_FLAG_RANGE_RANGE, parameters))
            {
                DoResetThreat();
...
{
    boss_grand_astromancer_capernianAI(Creature *creature) : advisor_base_ai(creature)
    {
        Reset();
    }
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (m_uiConflagrationTimer <= uiDiff)
        {
            ...
            if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_CONFLAGRATION,
                                                                 SELECT_FLAG_PLAYER))
            {
...
when ranged, either uses shoot ability when out of range of bomb, or bomb
exclusively
*/
...
struct boss_master_engineer_telonicusAI : public advisor_base_ai
{
    ...
    {
        m_creature->SetModifierValue(UNIT_MOD_DAMAGE_OFFHAND, TOTAL_PCT,
                                     1.f); // sniff evidence seems to point out that telonicus isnt subject
                                           // to 50% offhand dmg modifier
