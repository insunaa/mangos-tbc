...
enum
{
    ...
    SPELL_FRENZY = 26051,  // triggers 26052 - Poison Bolt Volley (will hit the 15
                           // closest people)
    SPELL_BERSERK = 26068, // triggers 26052 - Poison Bolt Volley (will hit the
                           // 15 closest people)
    SPELL_NOXIOUS_POISON = 26053,
    SPELL_WYVERN_STING = 26180, // triggers 26233 on aura removal, doing 500 nature damage if
                                // 26180 expires (confirmed) and about 3K if dispelled early
                                // (amount unconfirmed)
...
struct boss_huhuranAI : public CombatAI
{
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case HUHURAN_NOXIOUS_POISON: {
            if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_NOXIOUS_POISON,
                                                                 SELECT_FLAG_PLAYER))
...
struct HuhuranWyvernSting : public AuraScript // TODO: Add spell script for filtering
{
    void OnApply(Aura *aura, bool apply) const override
    {
        if (!apply)
        {
            ...
            {
                int32 natureDamage =
                    (aura->GetRemoveMode() == AURA_REMOVE_BY_DISPEL) ? 3000 : 500; // Deal 500 nature damage on spell
                                                                                   // expire but 3000 if dispelled
                caster->CastCustomSpell(aura->GetTarget(), 26233, &natureDamage, nullptr, nullptr,
