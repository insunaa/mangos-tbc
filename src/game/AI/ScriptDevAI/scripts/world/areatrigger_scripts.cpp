...
bool AreaTrigger_at_murkdeep(Player *pPlayer, AreaTriggerEntry const * /*pAt*/)
{
    ...
    // The area trigger summons 3 Greymist Coastrunners; The rest of the event is
    // handled by world map scripts
    if (pPlayer->IsAlive() && !pPlayer->IsGameMaster() &&
        pPlayer->GetQuestStatus(QUEST_WANTED_MURKDEEP) == QUEST_STATUS_INCOMPLETE)
    {
        ...
        // Check if there are already coastrunners (dead or alive) around the
        // area
        ...
        for (uint8 i = 0; i < 3; ++i)
        {
            ...
            pPlayer->GetRandomPoint(aSpawnLocations[POS_IDX_MURKDEEP_SPAWN][0],
                                    aSpawnLocations[POS_IDX_MURKDEEP_SPAWN][1],
                                    aSpawnLocations[POS_IDX_MURKDEEP_SPAWN][2], 5.0f, fX, fY, fZ);

            if (Creature *pTemp =
                    pPlayer->SummonCreature(NPC_GREYMIST_COASTRUNNNER, fX, fY, fZ,
...
bool AreaTrigger_at_huldar_miran(Player *pPlayer, AreaTriggerEntry const * /*pAt*/)
{
    ...
    // Check if Saean is spawned and set his faction to hostile - summon him if
    // not spawned
    if (m_saean)
    ...
    // Check if any Dark Iron Ambusher are already spawned or dead, if so, do
    // nothing
    if (m_saean && !GetClosestCreatureWithEntry(pPlayer, NPC_DARK_IRON_AMBUSHER, 60.0f, false, false))
    {
        m_saean->SummonCreature(NPC_DARK_IRON_AMBUSHER, m_miranAmbushSpawns[1].m_fX, m_miranAmbushSpawns[1].m_fY,
                                m_miranAmbushSpawns[1].m_fZ, m_miranAmbushSpawns[1].m_fO,
                                TEMPSPAWN_CORPSE_TIMED_DESPAWN, 25000);
        m_saean->SummonCreature(NPC_DARK_IRON_AMBUSHER, m_miranAmbushSpawns[2].m_fX, m_miranAmbushSpawns[2].m_fY,
                                m_miranAmbushSpawns[2].m_fZ, m_miranAmbushSpawns[2].m_fO,
                                TEMPSPAWN_CORPSE_TIMED_DESPAWN, 25000);
    }
...
bool AreaTrigger_at_area_52(Player *player, AreaTriggerEntry const *at)
{
    ...
    if (!player->HasAura(SPELL_A52_NEURALYZER))
    {
        ...
        player->CastSpell(targets, sSpellTemplate.LookupEntry<SpellEntry>(SPELL_A52_NEURALYZER),
                          TRIGGERED_OLD_TRIGGERED); // mounted needs triggered
    }
...
{
    // Player is deaed, a GM, quest complete, no quest or already got item: do
    // nothing
...
bool AreaTrigger_at_hive_tower(Player *player, AreaTriggerEntry const * /*pAt*/)
{
    ...
    if (scriptedMap->GetData(TYPE_HIVE) != NOT_STARTED) // Only summon more Hive'Ashi Drones if the 5 minutes
                                                        // timer is elapsed
        return false;
    ...
    if (player->IsAlive() && !player->IsGameMaster())
    {
        ...
        for (uint8 i = POS_IDX_HIVE_DRONES_START; i <= POS_IDX_HIVE_DRONES_STOP; ++i)
            player->SummonCreature(NPC_HIVE_ASHI_DRONES, aSpawnLocations[i][0], aSpawnLocations[i][1],
                                   aSpawnLocations[i][2], aSpawnLocations[i][3], TEMPSPAWN_TIMED_OR_DEAD_DESPAWN,
