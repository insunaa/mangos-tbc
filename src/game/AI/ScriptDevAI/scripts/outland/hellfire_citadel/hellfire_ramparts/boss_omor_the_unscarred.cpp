...
SD%Complete: 90
SDComment: Temporary solution for orbital/shadow whip-ability. Needs more core
support before making it more proper. Players should be ~6 seconds airborne,
shadow whip should be casted ~3 maybe 4 times SDCategory: Hellfire Citadel,
Hellfire Ramparts EndScriptData */
...
struct boss_omor_the_unscarredAI : public CombatAI
{
    boss_omor_the_unscarredAI(Creature *creature)
        : CombatAI(creature, OMOR_ACTION_MAX), m_inRegularMode(creature->GetMap()->IsRegularDifficulty())
    {
    ...
    void HandlePullBack()
    {
        ...
        {
            // if unit dosen't have this flag, then no pulling back (script will
            // attempt cast, even if orbital strike was resisted)
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case OMOR_TREACHERY: {
            if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_TREACHEROUS_AURA,
                                                                 SELECT_FLAG_PLAYER))
            {
                if (DoCastSpellIfCan(target, m_inRegularMode ? SPELL_TREACHEROUS_AURA : SPELL_BANE_OF_TREACHERY_H) ==
                    CAST_OK)
                {
        ...
        case OMOR_SHADOW_BOLT: {
            if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_TOPAGGRO, 0, SPELL_SHADOW_BOLT,
                                                                 SELECT_FLAG_PLAYER))
            {
