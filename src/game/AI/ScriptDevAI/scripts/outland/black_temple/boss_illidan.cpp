...
enum
{
    ...
    TEXT_ID_AKAMA_ILLIDAN_PREPARE = 10465, // ToDo: fix text id - this entry is wrong -> "The time has come
                                           // to face Illidan, $N. Are you ready?"
    ...
    SPELL_PARASITIC_SHADOWFIEND_BOSS = 41917, // DoT of 3k Shadow every 2 seconds. Lasts 10 seconds. (Script
    ...
    SPELL_SUMMON_PARASITICS = 41915,          // Summons 2 Parasitic Shadowfiends on the
                                              // target. Handled in core.
    SPELL_AGONIZING_FLAMES = 40834,           // triggers 40932
    SPELL_FRENZY = 40683,                     // Increases damage by 50% and attack speed by 30%. 20
    ...
    // Demon Form
    SPELL_DEMON_TRANSFORM_1 = 40511,    // start transform animation - spell sequence: 40398, 40506, 40510
                                        // - handled in core
    ...
    SPELL_SHADOW_BLAST = 41078,         // 8k - 11k Shadow Damage. Targets highest threat. Has a splash
                                        // effect, damaging anyone in 20 yards of the target.
    SPELL_FLAME_BURST = 41126,          // triggers 41131
    SPELL_SUMMON_SHADOW_DEMONS = 41117, // summons 23375

    // Other Illidan spells
    SPELL_KNEEL_INTRO = 39656,   // Before beginning encounter, this is how he
                                 // appears (talking to Wilson).
    SPELL_SUMMMON_MAIEV = 40403, // summons 23197
    SPELL_TELEPORT_MAIEV = 41221,
    SPELL_SHADOW_PRISON = 40647,       // Illidan casts this spell to immobilize entire
                                       // raid when he summons Maiev.
    SPELL_CAGE_TRAP = 40693,           // Cast by Illidan on Maiev - teleports Maiev for the trap
    SPELL_DEATH = 41218,               // This spell doesn't do anything except stun Illidan
    ...
    SPELL_EYE_BLAST_TRIGGER = 40017,  // This summons Demon Form every few seconds
                                      // and deals ~20k damage in its radius
    // SPELL_DEMON_FIRE             = 40029,                // Blue fire trail
    // left by Eye Blast. Deals 2k per second if players stand on it.
    ...
    // Blade of Azzinoth
    SPELL_RANGE_MARKER = 41997,         // Dummy effect used by the Blade of Azzinoth to check the range
                                        // of the Azzinoth flame - needs core support
    ...
    SPELL_CHARGE = 42003,      // Flames of Azzinoth charges whoever is too far from
                               // them. They enrage after this
    ...
    /************** Creature Summons **************/
    /*NPC_ILLIDARI_ELITE              = 23226,                // attacks Akama on
    the stairs NPC_FLAME_CRASH                 = 23336,                // has
    aura 40836 NPC_PARASITIC_SHADOWFIEND       = 23498,                // has
    aura 41913 (in c_t_a) NPC_BLADE_OF_AZZINOTH           = 22996, // has aura
    41997 and summons 22997 on spawn NPC_FLAME_OF_AZZINOTH           = 22997,
    NPC_ILLIDAN_TARGET              = 23070,                // the eye blast
    target - has aura 40017 NPC_DEMON_FIRE                  = 23069, // has aura
    40029 (in EventAI) NPC_BLAZE                       = 23259, // has aura 40610
    NPC_SHADOW_DEMON                = 23375, NPC_CAGE_TRAP_DISTURB_TRIGGER =
    23304,*/
...
struct boss_illidan_stormrageAI : public CombatAI, private DialogueHelper
{
          ...
          m_instance(static_cast<instance_black_temple *>(creature->GetInstanceData()))
    {
        ...
        if (m_instance)
        {
            m_creature->GetCombatManager().SetLeashingCheck([](Unit *unit, float /*x*/, float /*y*/, float z) {
                return static_cast<ScriptedInstance *>(unit->GetInstanceData())->GetPlayerInMap(true, false) ==
                           nullptr ||
                       z < 352.0f;
            });
        }
    ...
    void JustDidDialogueStep(int32 entry) override
    {
        ...
        case SAY_AKAMA_LEAVE:
            if (m_instance)
                // Remove Akama from threat list and allow him to fight the Illidari
                // elites
                if (Creature *akama = m_instance->GetSingleCreatureFromStorage(NPC_AKAMA))
        ...
        case PHASETRANSITION_LIFTOFF: {
            ...
            case 4: {
                if (m_instance)
                {
                    ...
                    //     illidanFlightPos[firstEyeBlastPos].fY,
                    //     illidanFlightPos[firstEyeBlastPos].fZ,
                    //     DIST_CALC_NONE)
                    //         >
                    //         m_creature->GetDistance(illidanFlightPos[i].fX,
                    //         illidanFlightPos[i].fY, illidanFlightPos[i].fZ,
                    //         DIST_CALC_NONE))
                    //     {
                    //         if
                    //         (m_creature->GetDistance(illidanFlightPos[secondEyeBlastPos].fX,
                    //         illidanFlightPos[secondEyeBlastPos].fY,
                    //         illidanFlightPos[secondEyeBlastPos].fZ,
                    //         DIST_CALC_NONE)
                    //             >
                    //             m_creature->GetDistance(illidanFlightPos[firstEyeBlastPos].fX,
                    //             illidanFlightPos[firstEyeBlastPos].fY,
                    ...
                    //     }
                    // }
                    //  m_curEyeBlastLoc = urand(0, 1) ? firstEyeBlastPos :
                    //  secondEyeBlastPos;
                    m_curEyeBlastLoc = urand(0, 3);
                    m_creature->GetMotionMaster()->MovePoint(
                        POINT_ILLIDAN_FLIGHT_RANDOM, illidanFlightPos[m_curEyeBlastLoc].fX,
                        illidanFlightPos[m_curEyeBlastLoc].fY, illidanFlightPos[m_curEyeBlastLoc].fZ,
                        FORCED_MOVEMENT_RUN);
                    PreparePhaseTimers();
                }
                break;
    ...
    void MovementInform(uint32 moveType, uint32 pointId) override
    {
        if (moveType == POINT_MOTION_TYPE)
        {
            if (pointId == POINT_ILLIDAN_FLIGHT)
            {
                ...
                m_creature->SetFacingTo(4.670939f); // sets facing to world trigger in
                                                    // sniff - maybe change to this?
    ...
    bool DoCastEyeBlastIfCan()
    {
        ...
        m_creature->SummonCreature(NPC_ILLIDAN_TARGET, aEyeBlastPos[m_curEyeBlastLoc].fX,
                                   aEyeBlastPos[m_curEyeBlastLoc].fY, aEyeBlastPos[m_curEyeBlastLoc].fZ, 0,
                                   TEMPSPAWN_TIMED_DESPAWN, 15000, true, true, m_curEyeBlastLoc + 1);
        return true;
    }
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case ILLIDAN_ACTION_EYE_BLAST: {
            if (DoCastEyeBlastIfCan())
            {
                ...
                ResetCombatAction(ILLIDAN_ACTION_FIREBALL,
                                  15000u); // Check this
            }
            return;
        }
        ...
        case ILLIDAN_ACTION_SHADOW_FIEND: {
            if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0,
                                                                 SPELL_PARASITIC_SHADOWFIEND_BOSS, SELECT_FLAG_PLAYER))
...
struct npc_akama_illidanAI : public CombatAI, private DialogueHelper
{
          ...
          m_akamaStage(AKAMA_STAGE_ILLIDAN), m_outroStage(0)
    {
        ...
        AddCustomAction(AKAMA_SUMMON_ILLIDARI, true, [&]() {
            for (const Locations &aIllidariElitesPo : aIllidariElitesPos)
                m_creature->SummonCreature(NPC_ILLIDARI_ELITE, aIllidariElitesPo.fX, aIllidariElitesPo.fY,
                                           aIllidariElitesPo.fZ, 0, TEMPSPAWN_DEAD_DESPAWN, 0);
        ...
        if (moveType == WAYPOINT_MOTION_TYPE)
        {
            ...
            case PATH_ID_AKAMA_ILLIDAN: {
                if (m_instance)
                {
                    ...
                    if (Creature *illidan = m_instance->GetSingleCreatureFromStorage(NPC_ILLIDAN_STORMRAGE))
                    {
                        if (boss_illidan_stormrageAI *illidanAI =
                                dynamic_cast<boss_illidan_stormrageAI *>(illidan->AI()))
    ...
    void JustDidDialogueStep(int32 iEntry) override
    {
        ...
        case NPC_SPIRIT_OF_OLUM:
            m_creature->SummonCreature(NPC_SPIRIT_OF_OLUM, 751.6437f, 297.2233f, 312.2083f, 6.03f,
                                       TEMPSPAWN_TIMED_DESPAWN, 50000);
            m_creature->SummonCreature(NPC_SPIRIT_OF_UDALO, 751.4565f, 311.0107f, 312.19f, 0.0f,
                                       TEMPSPAWN_TIMED_DESPAWN, 50000);
            break;
...
bool GossipHello_npc_akama_illidan(Player *player, Creature *creature)
{
    ...
    {
        player->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_PREPARE, GOSSIP_SENDER_MAIN,
...
struct boss_maievAI : public CombatAI, private DialogueHelper
{
          ...
          m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData())), m_outro(false)
    {
        ...
        // Not sure if this is correct, but she seems to ignore all the shadow
        // damage inflicted
...
{
    npc_cage_trap_triggerAI(Creature *creature) : ScriptedAI(creature)
    {
        Reset();
    }
    ...
    void MoveInLineOfSight(Unit *who) override
    {
        ...
        {
            if (static_cast<boss_illidan_stormrageAI *>(who->AI())->m_phase != PHASE_4_DEMON &&
                !static_cast<boss_illidan_stormrageAI *>(who->AI())->GetCombatScriptStatus())
            {
...
struct npc_flame_of_azzinothAI : public CombatAI
{
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case FLAME_ACTION_WRATH_CHECK: {
            ...
            {
                if (DoCastSpellIfCan(nullptr, SPELL_UNCAGED_WRATH,
            CAST_TRIGGERED)
            == CAST_OK)
                {
                    DisableCombatAction(FLAME_ACTION_WRATH_CHECK);
                    if (Unit* spawner = m_creature->GetSpawner())
                        spawner->InterruptSpell(CURRENT_CHANNELED_SPELL); //
            interrupts link
                }
            }
            */
...
struct npc_shadow_demonAI : public ScriptedAI
{
    ...
    void FindNewTarget()
    {
        ...
        {
            if (Unit *target = static_cast<Creature *>(illidan)->SelectAttackingTarget(
                    ATTACKING_TARGET_RANDOM, 0, SPELL_PARALYZE, SELECT_FLAG_PLAYER))
            {
    ...
    void MovementInform(uint32 movementType, uint32 pointId) override
    {
        ...
        if (Player *player = m_creature->GetMap()->GetPlayer(m_targetGuid))
            DoCastSpellIfCan(player,
                             SPELL_CONSUME_SOUL); // will find new target on channel interrupt
    }
...
struct npc_blade_of_azzinothAI : public ScriptedAI
{
    ...
    void SummonedCreatureJustDied(Creature *summoned) override
    {
        ...
        if (summoned->GetEntry() == NPC_FLAME_OF_AZZINOTH)
        {
            ...
            // For some reason it doesn't work with Spell Hit for
            // SPELL_GLAIVE_RETURNS script effect, so we need to inform him
            // manually
            if (Creature *illidan = m_instance->GetSingleCreatureFromStorage(NPC_ILLIDAN_STORMRAGE))
            {
...
struct npc_parasitic_shadowfiendAI : public ScriptedAI
{
    npc_parasitic_shadowfiendAI(Creature *creature)
        : ScriptedAI(creature, 0), m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData()))
    {
        SetReactState(REACT_PASSIVE);
        AddCustomAction(1, 2000u, [&]() {
            ...
            if (Creature *illidan = m_instance->GetSingleCreatureFromStorage(NPC_ILLIDAN_STORMRAGE))
            {
                ...
                if (Unit *target = illidan->SelectAttackingTarget(
                        ATTACKING_TARGET_RANDOM, 0, nullptr, SELECT_FLAG_PLAYER | SELECT_FLAG_SKIP_CUSTOM, m_params))
                {
...
{
    void OnRadiusCalculate(Spell * /*spell*/, SpellEffectIndex /*effIdx*/, bool /*targetB*/,
    ...
    void OnEffectExecute(Spell *spell, SpellEffectIndex effIdx) const override
    {
        ...
        {
            spell->GetUnitTarget()->RemoveAurasDueToSpell(SPELL_PARASITIC_SHADOWFIEND_BOSS, nullptr,
                                                          AURA_REMOVE_BY_DISPEL);
            spell->GetUnitTarget()->RemoveAurasDueToSpell(SPELL_PARASITIC_SHADOWFIEND_ADD, nullptr,
                                                          AURA_REMOVE_BY_DISPEL);
        }
    }
};
