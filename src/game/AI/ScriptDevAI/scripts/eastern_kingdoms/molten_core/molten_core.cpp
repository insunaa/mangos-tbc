...
void instance_molten_core::OnObjectCreate(GameObject *pGo)
{
    ...
    case GO_RUNE_THERI:
    case GO_RUNE_KORO:
        // Activate the rune if it was previously doused by a player (encounter
        // set to SPECIAL)
    ...
    case GO_CIRCLE_GOLEMAGG:
        // Delete the Flames Circle around the rune if the boss guarding it is
        // killed
        m_goEntryGuidStore[pGo->GetEntry()] = pGo->GetObjectGuid();
        for (auto m_aMoltenCoreRune : m_aMoltenCoreRunes)
        {
...
{
    // Summon Majordomo
    // If Majordomo encounter isn't done, summon at encounter place, else near
    // Ragnaros
...
void instance_molten_core::ExecuteChatCommand(ChatHandler *handler, char *args)
{
    ...
    if (val == "spawnmajordomo")
    {
        ...
        if (summonPos > 1)
        {
            handler->PSendSysMessage("Could not spawn majordomo because spawn %u was "
                                     "invalid. Please specify a value between 0-1",
                                     summonPos);
            return;
        }
