...
int32 MoveSplineInit::Launch()
{
    ...
    if (transport)
        transport->CalculatePassengerOffset(real_position.x, real_position.y, real_position.z,
                                            &real_position.orientation);

    // there is a big chance that current position is unknown if current state is
    // not finalized, need compute it this also allows calculate spline position
    // and update map position in much greater intervals
    ...
    if (args.velocity == 0.f) // ignore swim speed and flight speed because its not used in
                              // generic scripting - always possible to override
        args.velocity = unit.GetSpeed(
            MovementInfo::GetSpeedType(MovementFlags(moveFlags & ~(MOVEFLAG_FLYING | MOVEFLAG_SWIMMING))));
    ...
    {
        sLog.outCustomLog("Invalid movement during root. Entry: %u IsImmobilized "
                          "%s, moveflags %u",
                          unit.GetEntry(), unit.IsImmobilizedState() ? "true" : "false", moveFlags);
        sLog.traceLog();
        return 0;
    }
