...
// client, item icon shift click |color|Hitemset:itemset_id|h[name]|h|r
// |color|Hplayer:name|h[name]|h|r                                        -
// client, in some messages, at click copy only name instead link, so no way
// generate it in client string send to server |color|Hpool:pool_id|h[name]|h|r
...
ChatCommand *ChatHandler::getCommandTable()
{
    ...
    //     {
    //         { "show",           SEC_GAMEMASTER,     false,
    //         &ChatHandler::HandleNpcGroupBehaviorShowCommand, "", nullptr }, {
    //         "set",            SEC_GAMEMASTER,     false,
    //         &ChatHandler::HandleNpcGroupBehaviorSetCommand, "", nullptr }, {
    //         nullptr,          0,                  false, nullptr, "", nullptr
    //         }
    //     };
    ...
    static ChatCommand npcGroupCommandTable[] = {
        ...
        //{ "behavior",       SEC_GAMEMASTER,     false, nullptr, "",
        // npcGroupBehaviorCommandTable },
        {nullptr, 0, false, nullptr, "", nullptr}};
    ...
    static ChatCommand reloadCommandTable[] = {
        ...
        {"creature_cooldowns", SEC_ADMINISTRATOR, true, &ChatHandler::HandleReloadCreatureCooldownsCommand, "",
         nullptr},
        {"creature_template_classlevelstats", SEC_ADMINISTRATOR, true, &ChatHandler::HandleReloadCreaturesStatsCommand,
         "", nullptr}, // safeguard ori against too high val in spline
        {"creature_involvedrelation", SEC_ADMINISTRATOR, true,
         &ChatHandler::HandleReloadCreatureQuestInvRelationsCommand, "", nullptr},
        ...
        {"dbscripts_on_gossip", SEC_ADMINISTRATOR, true, &ChatHandler::HandleReloadDBScriptsOnGossipCommand, "",
         nullptr},
        {"dbscripts_on_go_use", SEC_ADMINISTRATOR, true, &ChatHandler::HandleReloadDBScriptsOnGoUseCommand, "",
         nullptr},
        ...
        {"prospecting_loot_template", SEC_ADMINISTRATOR, true,
         &ChatHandler::HandleReloadLootTemplatesProspectingCommand, "", nullptr},
        ...
        {"spell_proc_item_enchant", SEC_ADMINISTRATOR, true, &ChatHandler::HandleReloadSpellProcItemEnchantCommand, "",
         nullptr},
        {"spell_script_target", SEC_ADMINISTRATOR, true, &ChatHandler::HandleReloadSpellScriptTargetCommand, "",
         nullptr},
...
void ChatHandler::CheckIntegrity(ChatCommand *table, ChatCommand *parentCommand) const
{
    for (uint32 i = 0; table[i].Name != nullptr; ++i)
    {
        ...
        if (parentCommand && command->SecurityLevel < parentCommand->SecurityLevel)
            sLog.outError("Subcommand '%s' of command '%s' have less access level "
                          "(%u) that parent (%u)",
        ...
        if (command->ChildCommands)
        {
            ...
            {
                if (parentCommand)
                    sLog.outError("Subcommand '%s' of command '%s' have handler "
                                  "and subcommands in "
                                  "same time, must be used '' subcommand for "
                                  "handler instead.",
                                  command->Name, parentCommand->Name);
                else
                    sLog.outError("First level command '%s' have handler and subcommands in "
                                  "same "
        ...
        {
            if (parentCommand)
                sLog.outError("Subcommand '%s' of command '%s' not have handler and "
                              "subcommands in same time. Must have some from its!",
                              command->Name, parentCommand->Name);
            else
                sLog.outError("First level command '%s' not have handler and "
                              "subcommands in same time. Must have some from its!",
                              command->Name);
        }
    }
 ...
 *
 * @return Pointer to found command structure or nullptr if appropriate command
 * not found
 */
 ...
 * search, it modified at return from function and pointed to not parsed tail
 * @param command       At success this is found command, at other cases this
 * is last found parent command before subcommand search fail
 * @param parentCommand Output arg for optional return parent command for
 * command arg.
 * @param cmdNamePtr    Output arg for optional return last parsed command
 * name.
 ...
 * command specific data, command store found command pointer, parentCommand
 * have parent of found command or nullptr if command found in table array
 * directly cmdNamePtr store found command name in original form from command
 * line CHAT_COMMAND_UNKNOWN  - Command not found in table directly text only
 * skip possible whitespaces, command is nullptr parentCommand is nullptr
 *                              cmdNamePtr store command name that not found as
 * it extracted from command line CHAT_COMMAND_UNKNOWN_SUBCOMMAND - Subcommand
 * not found in some deed subcomand lists text point to non parsed tail
 * including not found command name in command line, command store last found
 * parent command if any parentCommand have parent of command in command arg or
 * nullptr cmdNamePtr store command name that not found as it extracted from
 * command line
 */
                                                 ...
                                                 bool exactlyName /*= false*/) const
{
    ...
    for (uint32 i = 0; table[i].Name != nullptr; ++i)
    {
        ...
        if (table[i].ChildCommands != nullptr)
        {
            ...
            ChatCommandSearchResult res = FindCommand(table[i].ChildCommands, text, command, &parentSubcommand,
                                                      cmdNamePtr, allAvailable, exactlyName);
            ...
            default: {
                ...
                // subcommand then parentCommand can be nullptr, use known
                // value for it
                if (parentCommand)
                    *parentCommand = parentSubcommand ? parentSubcommand : &table[i];
                return res;
                                           ...
                                           std::string const &help)
{
    ...
    case CHAT_COMMAND_OK: {
        if (command->SecurityLevel != security)
            DETAIL_LOG("Table `command` overwrite for command '%s' default security "
                       "(%u) by %u",
    ...
    case CHAT_COMMAND_UNKNOWN_SUBCOMMAND: {
        ...
        if (cmdName.empty())
            sLog.outErrorDb("Table `command` have command '%s' that only used with "
                            "some subcommand selection, it can't have help or "
                            "overwritten access level, skip.",
                            cmdName.c_str());
        else
            sLog.outErrorDb("Table `command` have unexpected subcommand '%s' in "
                            "command '%s', skip.",
    ...
    */
    ...
    while (!reader.eof())
    {
        ...
        if (commandChar != '|')
        {
            ...
            {
                DEBUG_LOG("ChatHandler::isValidChatMessage invalid sequence, expected "
                          "%c but got %c",
        ...
        case 'c':
            ...
            for (int i = 0; i < 8; ++i)
            {
                ...
                if (!c)
                {
                    DEBUG_LOG("ChatHandler::isValidChatMessage got \\0 while reading "
                              "color in |c command");
                    return false;
                }
                ...
                DEBUG_LOG("ChatHandler::isValidChatMessage got non hex char "
                          "'%c' while "
                          "reading color",
                          c);
                return false;
        ...
        case 'H':
            ...
            if (strcmp(buffer, "item") == 0)
            {
                ...
                if (!linkedItem)
                {
                    DEBUG_LOG("ChatHandler::isValidChatMessage got invalid "
                              "itemID %u in "
                              "|item command",
                              atoi(buffer));
                    return false;
                ...
                {
                    DEBUG_LOG("ChatHandler::isValidChatMessage linked item has "
                              "color %u, "
                              "but user claims %u",
            ...
            else if (strcmp(buffer, "quest") == 0)
            {
                ...
                if (!linkedQuest)
                {
                    DEBUG_LOG("ChatHandler::isValidChatMessage Questtemplate "
                              "%u not found",
                              questid);
                    return false;
                }
                ...
                if (c != ':')
                {
                    DEBUG_LOG("ChatHandler::isValidChatMessage Invalid quest "
                              "link structure");
                    return false;
                }
            ...
            {
                DEBUG_LOG("ChatHandler::isValidChatMessage user sent unsupported link "
                          "type '%s'",
                          buffer);
                return false;
            }
            break;
        case 'h':
            // if h is next element in sequence, this one must contain the linked
            // text
            // :)
            if (*validSequenceIterator == 'h')
            {
                ...
                if (reader.get() != '[')
                {
                    DEBUG_LOG("ChatHandler::isValidChatMessage link caption doesn't "
                              "start with '['");
                    return false;
                }
                reader.getline(buffer, 256, ']');
                ...
                if (linkedSpell)
                {
                    ...
                    if (linkedSpell->HasAttribute(SPELL_ATTR_TRADESPELL))
                    {
                        ...
                        SkillLineAbilityMapBounds bounds =
                            sSpellMgr.GetSkillLineAbilityMapBoundsBySpellId(linkedSpell->Id);
                        ...
                        for (uint8 i = 0; i < MAX_LOCALE; ++i)
                        {
                                ...
                                strncmp(skillLine->name[i], buffer, skillLineNameLength) == 0)
                            {
                                ...
                                // spellname validation below -2 = strlen(": ")
                ...
                else if (linkedQuest)
                {
                    if (linkedQuest->GetTitle() != buffer)
                    {
                        ...
                        if (!ql)
                        {
                            DEBUG_LOG("ChatHandler::isValidChatMessage default "
                                      "questname "
                        ...
                        if (!foundName)
                        {
                            DEBUG_LOG("ChatHandler::isValidChatMessage no "
                                      "quest locale title "
                                      "matched");
                            return false;
                        }
                    ...
                    if (expectedName != buffer)
                    {
                        ...
                        for (uint8 i = LOCALE_koKR; i < MAX_LOCALE; ++i)
                        {
                            ...
                            if (dbIndex == -1 || il == nullptr || (size_t)dbIndex >= il->Name.size())
                                // using strange database/client combinations can
                                // lead to this case
                        ...
                        if (!foundName)
                        {
                            DEBUG_LOG("ChatHandler::isValidChatMessage linked "
                                      "item name "
                ...
                // been set in |H it will return false before
                else
                    return false;
            }
            break;
 ...
 * call to new position (with skipped white spaces) allowed nullptr string
 * pointer stored in *args
 */
 ...
 * is not starting substring of lit. Note: function in same way fail if no any
 * literal or literal not fit in this case. Need additional check for select
 * specific fail case
 * @return     name/number-like string without whitespaces, or nullptr if args
 ...
 * @return     quote-like string, or nullptr if args empty or not appropriate
 * content.
 */
 ...
 * call to new position (with skipped white spaces)
 * @param asis control save quote string wrappers
 * @return     quote/literal string, or nullptr if args empty or not
 * appropriate content.
 */
...
/**
 * Function extract shift-link-like string (any characters guarded by | and
 * |h|r with some additional internal structure check)
 *
 * @param args variable pointer to non parsed args string, updated at function
 * call to new position (with skipped white spaces)
 *
 * @param linkTypes  optional nullptr-terminated array of link types,
 * shift-link must fit one from link type from array if provided or extraction
 * fail
 *
 * @param found_idx  if not nullptr then at return index in linkTypes that fit
 * shift-link type, if extraction fail then non modified
 *
 * @param keyPair    if not nullptr then pointer to 2-elements array for return
 * start and end pointer for found key if extraction fail then non modified
 *
 * @param somethingPair then pointer to 2-elements array for return start and
 * end pointer if found. if not nullptr then shift-link must have data field,
 * if extraction fail then non modified
 *
 * @return     shift-link-like string, or nullptr if args empty or not
 * appropriate content.
 */
 ...
 * call to new position (with skipped white spaces)
 * @param asis control save quote string wrappers
 * @return     extracted arg string, or nullptr if args empty or not
 * appropriate content.
 */
 ...
 *
 * @param args variable pointer to non parsed args string, updated at function
 * call to new position (with skipped white spaces) if args have only single
 * arg then args still pointing to this arg (unmodified pointer)
 * @return     extracted string, or nullptr if args empty or not appropriate
 ...
 * "|color|LINKTYPE:RETURN:SOMETHING1|h[name]|h|r if linkType == LINKTYPE It
 * also extract literal/quote if not shift-link in args
 *
 * @param args       variable pointer to non parsed args string, updated at
 * function call to new position (with skipped white spaces) if args have sift
 * link with linkType != LINKTYPE then args still pointing to this arg
 * (unmodified pointer)
 *
 * @param linkType   shift-link must fit by link type to this arg value or
 * extraction fail
 *
 * @param something1 if not nullptr then shift-link must have data field and it
 * returned into this arg if extraction fail then non modified
 *
 * @return           extracted key, or nullptr if args empty or not appropriate
...
/**
 * Function extract data from shift-link
 * "|color|LINKTYPE:RETURN:SOMETHING1|h[name]|h|r if LINKTYPE in linkTypes
 * array It also extract literal/quote if not shift-link in args
 *
 * @param args       variable pointer to non parsed args string, updated at
 * function call to new position (with skipped white spaces) if args have sift
 * link with linkType != LINKTYPE then args still pointing to this arg
 * (unmodified pointer)
 *
 * @param linkTypes  nullptr-terminated array of link types, shift-link must
 * fit one from link type from array or extraction fail
 *
 * @param found_idx  if not nullptr then at return index in linkTypes that fit
 * shift-link type, for non-link case return -1 if extraction fail then non
 * modified
 *
 * @param something1 if not nullptr then shift-link must have data field and it
 * returned into this arg if extraction fail then non modified
 ...
 * "|color|LINKTYPE:RETURN|h[name]|h|r if linkType == LINKTYPE It also extract
 * direct number if not shift-link in args
 *
 * @param args       variable pointer to non parsed args string, updated at
 * function call to new position (with skipped white spaces) if args have sift
 * link with linkType != LINKTYPE then args still pointing to this arg
...
uint32 ChatHandler::ExtractSpellIdFromLink(char **text)
{
    ...
    // [name] Shift-click form |color|Hspell:spell_id|h[name]|h|r number or
    // [name] Shift-click form |color|Htalent:talent_id,rank|h[name]|h|r
    int type;
 ...
 * at least (or more).
 * @param player_guid optional arg   For function success only one from
 * provided args need get result
 * @param player_name optional arg   But if early arg get value then all later
 * args will have its (if requested) if player_guid requested and not found
 * then name also will not found So at success can be returned 2 cases:
                                     ...
                                     Player **targetIfNullArg /*= nullptr*/)
{
    ...
    if (!account_str)
    {
        ...
        /// targetPlayer!=nullptr then not console)
...
template <typename T> void ChatHandler::ShowNpcOrGoSpawnInformation(uint32 guid)
{
    if (uint16 pool_id = sPoolMgr.IsPartOfAPool<T>(guid))
    {
        ...
        else
        {
            ...
            PSendSysMessage(LANG_NPC_GO_INFO_TOP_POOL, uint32(pool_id), pool_template.description.c_str(),
                            uint32(top_pool_id), mother_template.description.c_str());
        }
...
void ChatHandler::LogCommand(char const *fullcmd) const
{
    ...
    if (m_session)
    {
        ...
        sLog.outCommand(GetAccountId(),
                        "Command: %s [Player: %s (Account: %u) X: %f Y: %f Z: %f "
                        "Map: %u Selected: %s]",
