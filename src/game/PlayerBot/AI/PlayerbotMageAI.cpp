...
CombatManeuverReturns PlayerbotMageAI::DoNextCombatManeuverPVE(Unit *pTarget)
{
    ...
    // switch to melee if in melee range AND can't shoot OR have no ranged (wand)
    // equipped
    ...
    if (newTarget && !PlayerbotAI::IsNeutralized(newTarget)) // Bot has aggro and the mob is
                                                             // not already crowd controled
    {
        if (newTarget->GetHealthPercent() > 25)
        {
            ...
            {
                // If the attacker is a beast or humanoid, let's the bot give it
                // a form more suited to the low intellect of something fool
                // enough to attack a mage No need to check if target is a player
                // as IsElite() returns false for players
                ...
                // Have threat, can't quickly lower it. 3 options remain: Stop
                // attacking, lowlevel damage (wand), keep on keeping on.
    ...
    // Bot could also cast flamestrike or blizzard for free, but the AoE could
    // break some crowd control or add threat on mobs ignoring the bot currently,
    // so only focus on the bot's current target
    ...
    case MAGE_SPEC_FROST:
        ...
        if (COLD_SNAP && m_bot.IsSpellReady(COLD_SNAP) && CheckFrostCooldowns() > 2 &&
            m_ai.SelfBuff(COLD_SNAP) == SPELL_CAST_OK) // Clear frost spell cooldowns if bot has
                                                       // more than 2 active
            return RETURN_CONTINUE;
        if (CONE_OF_COLD > 0 && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_FROST) &&
            m_bot.IsSpellReady(CONE_OF_COLD) && meleeReach)
        {
            // Cone of Cold does not require a target, so ensure that the bot
            // faces the current one before casting
            m_ai.FaceTarget(pTarget);
        ...
        /*
        if (BLIZZARD > 0 && !m_ai.IsImmuneToSchool(pTarget,
        SPELL_SCHOOL_MASK_FROST)
        && m_ai.In_Reach(pTarget,BLIZZARD) && m_ai.GetAttackerCount() >= 5 &&
    ...
    case MAGE_SPEC_FIRE:
        ...
        if (BLAST_WAVE > 0 && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_FIRE) &&
            ...
            return RETURN_CONTINUE;
        // Try to have 3 scorch stacks to let tank build aggro while getting a
        // nice crit% bonus
        if (IMPROVED_SCORCH > 0 && SCORCH > 0 && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_FIRE))
        {
            if (!pTarget->HasAura(FIRE_VULNERABILITY, EFFECT_INDEX_0) &&
                CastSpell(SCORCH, pTarget)) // no stacks: cast it
                return RETURN_CONTINUE;
            else
            {
        ...
        // All DoTs, cooldowns used, try to maximise scorch stacks (5) to get a
        // even nicer crit% bonus
        if (IMPROVED_SCORCH > 0 && SCORCH > 0 && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_FIRE))
        {
            SpellAuraHolder *holder = pTarget->GetSpellAuraHolder(FIRE_VULNERABILITY);
        ...
        /*
        if (FLAMESTRIKE > 0 && !m_ai.IsImmuneToSchool(pTarget,
        SPELL_SCHOOL_MASK_FIRE) && m_ai.In_Reach(pTarget,FLAMESTRIKE) &&
    ...
    case MAGE_SPEC_ARCANE:
        if (ARCANE_POWER > 0 && m_bot.IsSpellReady(ARCANE_POWER) && m_ai.IsElite(pTarget) &&
            m_ai.CastSpell(ARCANE_POWER) == SPELL_CAST_OK) // Do not waste Arcane Power on normal NPCs
                                                           // as the bot is likely in a group
            return RETURN_CONTINUE;
        ...
        {
            // Instant Pyroblast, yeah! Tanks will probably hate this, but what
            // do they know about power? Nothing...
        ...
        if (FIRE_BLAST > 0 && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_FIRE) &&
            m_bot.IsSpellReady(FIRE_BLAST) && CastSpell(FIRE_BLAST, pTarget))
            return RETURN_CONTINUE;
        if (SLOW > 0 && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_ARCANE) &&
            m_ai.In_Reach(pTarget, SLOW) && !pTarget->HasAura(SLOW, EFFECT_INDEX_0) && CastSpell(SLOW, pTarget))
            return RETURN_CONTINUE;
...
{
    if (FIREBALL && !PlayerbotAI::IsImmuneToSchool(pTarget, SPELL_SCHOOL_MASK_FIRE) &&
        m_ai.In_Reach(pTarget, FIREBALL) && m_ai.CastSpell(FIREBALL) == SPELL_CAST_OK)
        return RETURN_CONTINUE;
...
void PlayerbotMageAI::DoNonCombatActions()
{
    ...
    if (m_ai.CanStore())
    {
        ...
        // TODO: The beauty of a mage is not only its ability to supply itself
        // with water, but to share its water So, conjure at *least* 1.25 stacks,
        // ready to trade a stack and still have some left for self
...
// TODO: this and priest's BuffHelper are identical and thus could probably go
// in PlayerbotClassAI.cpp somewhere
