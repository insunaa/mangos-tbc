...
CombatManeuverReturns PlayerbotShamanAI::DoNextCombatManeuverPVE(Unit *pTarget)
{
    ...
    case SHAMAN_SPEC_ENHANCEMENT:
        if (STORMSTRIKE > 0 && (m_bot.IsSpellReady(STORMSTRIKE)) &&
            m_ai.CastSpell(STORMSTRIKE, *pTarget) == SPELL_CAST_OK)
            return RETURN_CONTINUE;
        if (FLAME_SHOCK > 0 && (!pTarget->HasAura(FLAME_SHOCK)) &&
            m_ai.CastSpell(FLAME_SHOCK, *pTarget) == SPELL_CAST_OK)
            return RETURN_CONTINUE;
        if (EARTH_SHOCK > 0 && (m_bot.IsSpellReady(EARTH_SHOCK)) &&
            m_ai.CastSpell(EARTH_SHOCK, *pTarget) == SPELL_CAST_OK)
            return RETURN_CONTINUE;
    ...
    case SHAMAN_SPEC_ELEMENTAL:
        ...
        /*if (FROST_SHOCK > 0 && !pTarget->HasAura(FROST_SHOCK, EFFECT_INDEX_0)
           && m_ai.CastSpell(FROST_SHOCK, *pTarget) == SPELL_CAST_OK) return
           RETURN_CONTINUE;*/
        /*if (CHAIN_LIGHTNING > 0 && m_ai.CastSpell(CHAIN_LIGHTNING, *pTarget) ==
           SPELL_CAST_OK) return RETURN_CONTINUE;*/
        /*if (HEX > 0 && !pTarget->HasAura(HEX, EFFECT_INDEX_0) &&
           m_ai.CastSpell(HEX) == SPELL_CAST_OK) return RETURN_CONTINUE;*/
    }
...
CombatManeuverReturns PlayerbotShamanAI::HealPlayer(Player *target)
{
    ...
    // Technically the best rotation is CHAIN + LHW + LHW subbing in HW for
    // trouble (bad mana efficiency)
...
CombatManeuverReturns PlayerbotShamanAI::ResurrectPlayer(Player *target)
{
    ...
    if (m_ai.IsInCombat()) // Just in case as this was supposedly checked before
...
void PlayerbotShamanAI::DropTotems()
{
    ...
    /*if (EARTH_ELEMENTAL_TOTEM > 0 && m_ai.CastSpell(EARTH_ELEMENTAL_TOTEM) ==
       SPELL_CAST_OK) return RETURN_CONTINUE;*/
    /*if (EARTHBIND_TOTEM > 0 && !pTarget->HasAura(EARTHBIND_TOTEM,
       EFFECT_INDEX_0) && !m_bot.HasAura(STRENGTH_OF_EARTH_TOTEM, EFFECT_INDEX_0)
       && m_ai.CastSpell(EARTHBIND_TOTEM) == SPELL_CAST_OK) return
       RETURN_CONTINUE;*/
    /*if (FIRE_ELEMENTAL_TOTEM > 0 && m_ai.CastSpell(FIRE_ELEMENTAL_TOTEM) ==
       SPELL_CAST_OK) return RETURN_CONTINUE;*/
    /*if (FIRE_NOVA_TOTEM > 0 && m_ai.CastSpell(FIRE_NOVA_TOTEM) ==
       SPELL_CAST_OK) return RETURN_CONTINUE;*/
    /*if (GROUNDING_TOTEM > 0 && !m_bot.HasAura(GROUNDING_TOTEM, EFFECT_INDEX_0)
       && !m_bot.HasAura(WRATH_OF_AIR_TOTEM, EFFECT_INDEX_0) &&
       !m_bot.HasAura(WIND_FURY_TOTEM, EFFECT_INDEX_0) &&
       m_ai.CastSpell(GROUNDING_TOTEM) == SPELL_CAST_OK) return
       RETURN_CONTINUE;*/
    /*if (MAGMA_TOTEM > 0 && (!m_bot.HasAura(TOTEM_OF_WRATH, EFFECT_INDEX_0)) &&
       m_ai.CastSpell(MAGMA_TOTEM)) == SPELL_CAST_OK) return RETURN_CONTINUE;*/
    /*if (SEARING_TOTEM > 0 && !pTarget->HasAura(SEARING_TOTEM, EFFECT_INDEX_0)
       && !m_bot.HasAura(TOTEM_OF_WRATH, EFFECT_INDEX_0) &&
       m_ai.CastSpell(SEARING_TOTEM) == SPELL_CAST_OK) return RETURN_CONTINUE;*/
    /*if (STONECLAW_TOTEM > 0 && m_ai.GetHealthPercent() < 51 &&
       ...
       m_ai.CastSpell(STONECLAW_TOTEM) == SPELL_CAST_OK) return
       RETURN_CONTINUE;*/
    /*if (STONESKIN_TOTEM > 0 && !m_bot.HasAura(STONESKIN_TOTEM, EFFECT_INDEX_0)
       && !m_bot.HasAura(STRENGTH_OF_EARTH_TOTEM, EFFECT_INDEX_0) &&
       m_ai.CastSpell(STONESKIN_TOTEM) == SPELL_CAST_OK) return
       RETURN_CONTINUE;*/
    /*if (TREMOR_TOTEM > 0 && !m_bot.HasAura(STRENGTH_OF_EARTH_TOTEM,
       EFFECT_INDEX_0) && m_ai.CastSpell(TREMOR_TOTEM) == SPELL_CAST_OK) return
       RETURN_CONTINUE;*/
}
...
void PlayerbotShamanAI::DoNonCombatActions()
{
    ...
    /*
           ...
           if (ROCKBITER_WEAPON > 0)
                (!m_bot.HasAura(ROCKBITER_WEAPON, EFFECT_INDEX_0) &&
       !m_bot.HasAura(WINDFURY_WEAPON, EFFECT_INDEX_0) &&
       !m_bot.HasAura(FLAMETONGUE_WEAPON, EFFECT_INDEX_0) &&
       !m_bot.HasAura(FROSTBRAND_WEAPON, EFFECT_INDEX_0) &&
       m_ai.CastSpell(ROCKBITER_WEAPON,*m_bot) == SPELL_CAST_OK); else if
       (WINDFURY_WEAPON > 0)
                (!m_bot.HasAura(WINDFURY_WEAPON, EFFECT_INDEX_0) &&
       !m_bot.HasAura(FLAMETONGUE_WEAPON, EFFECT_INDEX_0) &&
       !m_bot.HasAura(FROSTBRAND_WEAPON, EFFECT_INDEX_0) &&
       !m_bot.HasAura(ROCKBITER_WEAPON, EFFECT_INDEX_0) &&
       m_ai.CastSpell(WINDFURY_WEAPON,*m_bot) == SPELL_CAST_OK); else if
       (FLAMETONGUE_WEAPON > 0)
                (!m_bot.HasAura(FLAMETONGUE_WEAPON, EFFECT_INDEX_0) &&
       !m_bot.HasAura(WINDFURY_WEAPON, EFFECT_INDEX_0) &&
       !m_bot.HasAura(FROSTBRAND_WEAPON, EFFECT_INDEX_0) &&
       !m_bot.HasAura(ROCKBITER_WEAPON, EFFECT_INDEX_0) &&
       m_ai.CastSpell(FLAMETONGUE_WEAPON,*m_bot) == SPELL_CAST_OK); else if
       (FROSTBRAND_WEAPON > 0)
                (!m_bot.HasAura(FROSTBRAND_WEAPON, EFFECT_INDEX_0) &&
       !m_bot.HasAura(WINDFURY_WEAPON, EFFECT_INDEX_0) &&
       !m_bot.HasAura(FLAMETONGUE_WEAPON, EFFECT_INDEX_0) &&
