...
SD%Complete: 95
SDComment: Viscidus globs miss their speed update while they move to the
center. SDCategory: Temple of Ahn'Qiraj EndScriptData */
...
enum
{
    ...
    SPELL_VISCIDUS_WEAKNESS = 25926, // Aura which procs at damage - triggers the
                                     // slow spells - removed from DBC (readded)

    // When frost damage exceeds a certain limit, then boss explodes
    SPELL_VISCIDUS_EXPLODE = 25938,
    SPELL_VISCIDUS_SUICIDE_TRIGGER = 26003, // Cast when Viscidus shatters and is
                                            // below 10% HP - triggers 26002
    SPELL_VISCIDUS_SUICIDE = 26002,         // Actual suicide spell
    SPELL_HATE_TO_ZERO = 20538,

    // Spell for globs
    SPELL_REJOIN_VISCIDUS = 25896,
    SPELL_SUMMON_GLOBS = 25885,       // summons npc 15667 using spells from 25865 to
                                      // 25884; All spells have target coords
    ...
    //    SPELL_VISCIDUS_TELEPORT     = 25904,                    // Unknown
    //    usage
    //    - removed from DBC (readded)
...
struct boss_viscidusAI : public CombatAI
{
          ...
          m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData()))
    {
        ...
        m_creature->GetCombatManager().SetLeashingCheck([&](Unit * /*unit*/, float /*x*/, float /*y*/,
                                                            float /*z*/) -> bool {
            return m_creature->GetDistance(resetPoint.m_fX, resetPoint.m_fY, resetPoint.m_fZ, DIST_CALC_COMBAT_REACH) <
                   10.0f;
        });
    }
    ...
    void JustSummoned(Creature *summoned) override
    {
        if (summoned->GetEntry() == NPC_GLOB_OF_VISCIDUS)
        {
            ...
            // Check if Glob already counted/added to list to prevent dual count
            // due to hook being sometimes called twice
            if (std::find(std::begin(m_lGlobesGuidList), std::end(m_lGlobesGuidList), summoned->GetObjectGuid()) ==
                std::end(m_lGlobesGuidList))
            {
                ++m_aliveGlobs;
    ...
    void SummonedCreatureJustDied(Creature *summoned) override
    {
        if (summoned->GetEntry() == NPC_GLOB_OF_VISCIDUS)
        {
            ...
            --m_aliveGlobs;
            // Start rejoin phase for Viscidus if all Globs are dead/have reached
            // center but check that phase is not already started due to some hooks
            // being called twice
    ...
    void SetPhase(uint8 phase)
    {
        ...
        case PHASE_REJOIN: {
            ...
            m_creature->CastCustomSpell(m_creature, SPELL_VISCIDUS_GROWS, &heal, nullptr, nullptr,
                                        TRIGGERED_IGNORE_UNSELECTABLE_FLAG | TRIGGERED_IGNORE_UNATTACKABLE_FLAG);
    ...
    void HandleExplode()
    {
        // At this point, Viscidus should play an emote like dying or exploding but
        // this does not appear clearly in sniff (maybe a spell visual or stand
        // state change?)
...
struct ViscidusGrows : public SpellScript
{
    void OnEffectExecute(Spell *spell, SpellEffectIndex effIdx) const override
    {
        if (effIdx == EFFECT_INDEX_1)
        {
            ...
            Unit *target = spell->GetUnitTarget();
            int8 stacksToRemove =
                (int8)((float)spell->m_currentBasePoints[0] / ((float)target->GetMaxHealth() * 0.05f));
