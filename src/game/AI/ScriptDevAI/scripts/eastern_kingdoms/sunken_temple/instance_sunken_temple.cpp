...
void instance_sunken_temple::Update(uint32 uiDiff)
{
    ...
    if (m_suppressionTimer)
    {
        ...
        {
            // At the end of the suppression timer, if Shade of Hakkar still has
            // suppression aura : reset event
    ...
    if (m_uiAvatarSummonTimer)
    {
        if (m_uiAvatarSummonTimer <= uiDiff)
        {
            ...
            if (m_bIsFirstHakkarWave) // First wave summoned
            {
                ...
                for (GuidVector::const_iterator itr = m_evilCircleGuidList.begin(); itr != m_evilCircleGuidList.end();
                     ++itr)
                {
