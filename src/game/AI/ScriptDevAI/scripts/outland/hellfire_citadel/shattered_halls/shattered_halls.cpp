...
instance_shattered_halls::instance_shattered_halls(Map *pMap)
    : ScriptedInstance(pMap), m_uiExecutionTimer(55 * MINUTE * IN_MILLISECONDS), m_uiTeam(0), m_uiExecutionStage(0),
      m_uiPrisonersLeft(3), m_gauntletStopped(false)
{
    Initialize();
}
    ...
    case TYPE_EXECUTION:
        ...
        if (uiData == IN_PROGRESS && !GetSingleCreatureFromStorage(NPC_EXECUTIONER, true))
        {
            if (Player *pPlayer = GetPlayerInMap())
            {
                ...
                // Summon the executioner; Note: according to wowhead he
                // shouldn't be targetable until Kargath encounter is finished
                if (Creature *pExecutioner = pPlayer->SummonCreature(
                        NPC_EXECUTIONER, afExecutionerLoc[0], afExecutionerLoc[1], afExecutionerLoc[2],
...
void instance_shattered_halls::OnCreatureEnterCombat(Creature *creature)
{
    ...
    // complete the event if you engage the npc while you have only a few seconds
    // left
    if (creature->GetEntry() == NPC_EXECUTIONER)
                                                          ...
                                                          uint32 conditionSourceType) const
{
    ...
    script_error_log("instance_shattered_halls::CheckConditionCriteriaMeet "
                     "called with unsupported Id %u. Called with param plr %s, "
                     "src %s, condition source type %u",
...
{
    if (Creature *pAdd = WorldObject::SummonCreature(
            TempSpawnSettings(nullptr, NPC_SHATTERED_HAND_ZEALOT, zealotSpawnCoords[1][0], zealotSpawnCoords[1][1],
                              zealotSpawnCoords[1][2], 0.0f, TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 150000, true, true),
            instance))
    {
        pAdd->GetMotionMaster()->MovePoint(100 + m_uiNumInitialWaves, firstWaveWaypoints[0][0],
                                           firstWaveWaypoints[0][1], firstWaveWaypoints[0][2]);
        pAdd->HandleEmoteState(EMOTE_STATE_READY1H);
    }
    if (Creature *pAdd = WorldObject::SummonCreature(
            TempSpawnSettings(nullptr, NPC_SHATTERED_HAND_ZEALOT, zealotSpawnCoords[2][0], zealotSpawnCoords[2][1],
                              zealotSpawnCoords[2][2], 0.0f, TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 150000, true, true),
            instance))
    {
        pAdd->GetMotionMaster()->MovePoint(200 + m_uiNumInitialWaves, firstWaveWaypoints[1][0],
                                           firstWaveWaypoints[1][1], firstWaveWaypoints[1][2]);
        pAdd->HandleEmoteState(EMOTE_STATE_READY1H);
    }
}
...
{
    if (Creature *pAdd = WorldObject::SummonCreature(
            TempSpawnSettings(nullptr, NPC_SHATTERED_HAND_ZEALOT, zealotSpawnCoords[0][0], zealotSpawnCoords[0][1],
                              zealotSpawnCoords[0][2], 0.0f, TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 150000, true, true),
            instance))
    {
...
void instance_shattered_halls::DoBeginArcherAttack(bool leftOrRight)
{
    ...
    // some range of the players (videos from back when seem to show that
    // behavior) but I'm not sure how best to implement that and this works fine
    // for now
            ...
            for (auto &blaze : m_vBlazeTimers)
            {
                if (blaze.second <= uiDiff)
                {
                    ...
                    if (GameObject *blazeGo = instance->GetGameObject(blaze.first))
                        blazeGo->CastSpell(nullptr, nullptr, SPELL_FLAMES, TRIGGERED_NONE, nullptr, nullptr,
    ...
    if (m_uiExecutionTimer < uiDiff)
    {
        ...
        case 0:
            ...
            DoOrSimulateScriptTextForThisInstance(
                m_uiTeam == ALLIANCE ? SAY_KARGATH_EXECUTE_ALLY : SAY_KARGATH_EXECUTE_HORDE, NPC_KARGATH_BLADEFIST);
...
{
    npc_shattered_hands_zealotAI(Creature *creature)
        : CreatureEventAI(creature), m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData()))
    {
    }
    ...
    void MovementInform(uint32 motionType, uint32 data) override
    {
        if (motionType == POINT_MOTION_TYPE) // sanity check
        {
            ...
            case 0:
                m_creature->GetMotionMaster()->MovePoint(1, zealotWaypoints[1][0], zealotWaypoints[1][1],
                                                         zealotWaypoints[1][2]);
                break;
            case 1:
                m_creature->GetMotionMaster()->MovePoint(2, zealotWaypoints[2][0], zealotWaypoints[2][1],
                                                         zealotWaypoints[2][2]);
                break;
            case 2:
                m_creature->GetMotionMaster()->MovePoint(3, zealotWaypoints[3][0], zealotWaypoints[3][1],
                                                         zealotWaypoints[3][2]);
                break;
            // Rest are the initial spawns
            // Left
            case 100:
                m_creature->GetMotionMaster()->MovePoint(98, zealotDestinations[0][0], zealotDestinations[0][1],
                                                         zealotDestinations[0][2]);
                break;
            case 101:
                m_creature->GetMotionMaster()->MovePoint(98, zealotDestinations[2][0], zealotDestinations[2][1],
                                                         zealotDestinations[2][2]);
                break;
            case 102:
                m_creature->GetMotionMaster()->MovePoint(98, zealotDestinations[4][0], zealotDestinations[4][1],
                                                         zealotDestinations[4][2]);
                break;
            case 103:
                m_creature->GetMotionMaster()->MovePoint(98, zealotDestinations[6][0], zealotDestinations[6][1],
                                                         zealotDestinations[6][2]);
                break;
            // Right
            case 200:
                m_creature->GetMotionMaster()->MovePoint(99, zealotDestinations[1][0], zealotDestinations[1][1],
                                                         zealotDestinations[1][2]);
                break;
            case 201:
                m_creature->GetMotionMaster()->MovePoint(99, zealotDestinations[3][0], zealotDestinations[3][1],
                                                         zealotDestinations[3][2]);
                break;
            case 202:
                m_creature->GetMotionMaster()->MovePoint(99, zealotDestinations[5][0], zealotDestinations[5][1],
                                                         zealotDestinations[5][2]);
                break;
            case 203:
                m_creature->GetMotionMaster()->MovePoint(99, zealotDestinations[7][0], zealotDestinations[7][1],
...
struct npc_Shattered_Hand_Scout : public ScriptedAI
{
    ...
    {
        // Abuse Prevention for when people revive mid gauntlet and continue onward
        // instead of starting the gauntlet
        if (!m_bRunning)
            DoStartRunning();
    }
...
static const int32 aRandomReinf[] = {-1540056, -1540057, -1540058, -1540059, -1540060,
                                     -1540061, -1540062, 1540063,  1540064,  1540065};

struct npc_shattered_hand_legionnaire : public CombatAI
{
    npc_shattered_hand_legionnaire(Creature *creature)
        : CombatAI(creature, LEGIONNAIRE_ACTION_MAX),
          m_instance(static_cast<instance_shattered_halls *>(creature->GetInstanceData()))
    {
        Reset();
    ...
    void CallForReinforcements()
    {
        ...
        if (guid == SECOND_LEGIONNAIRE_GUID || guid == THIRD_LEGIONNAIRE_GUID || guid == FOURTH_LEGIONNAIRE_GUID ||
            guid == FIFTH_LEGIONNAIRE_GUID)
        {
            if (Creature *felorc = m_creature->SummonCreature(
                    MOB_FEL_ORC, FelOrcCoords[legionnaireGuid][0], FelOrcCoords[legionnaireGuid][1],
                    FelOrcCoords[legionnaireGuid][2], FelOrcCoords[legionnaireGuid][3],
                    TEMPSPAWN_TIMED_OOC_OR_CORPSE_DESPAWN, 60000, true, true))
            {
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case LEGIONNAIRE_PUMMEL: {
            if (DoCastSpellIfCan(m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, nullptr,
                                                                   (SELECT_FLAG_PLAYER | SELECT_FLAG_CASTING)),
                                 SPELL_PUMMEL) == CAST_OK)
