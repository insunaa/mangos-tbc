...
enum
{
    ...
    SPELL_LAVA_BURST = 21908,        // Randomly trigger one of spells 21886, 21900 -
                                     // 21907 which summons Go 178088
...
struct boss_ragnarosAI : public CombatAI
{
    ...
    void SpellHitTarget(Unit *target, const SpellEntry *spellInfo, SpellMissInfo /*missInfo*/) override
    {
        ...
        // As Majordomo is now killed, the last timer (until attacking) must be
        // handled with ragnaros script
    ...
    void HandleEnterCombat()
    {
        ...
        {
            // If we don't remove this passive flag, he will be unattackable after
            // evading, this way he will enter combat
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case RAGNAROS_LAVA_BURST: {
            if (DoCastSpellIfCan(nullptr, SPELL_LAVA_BURST) == CAST_OK)
                ResetCombatAction(action, urand(5000, 25000)); // Upper value is guess work based on
                                                               // videos. It could be up to 45 secs
            break;
        }
        }
    }
