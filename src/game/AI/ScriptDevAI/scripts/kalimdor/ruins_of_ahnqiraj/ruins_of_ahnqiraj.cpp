...
SD%Complete: 80
SDComment: It's not clear if the Rajaxx event should reset if Andorov dies, or
party wipes. SDCategory: Ruins of Ahn'Qiraj EndScriptData */
...
void instance_ruins_of_ahnqiraj::OnCreatureDeath(Creature *pCreature)
{
    ...
    case NPC_SWARMGUARD_NEEDLER: {
            ...
            m_sArmyWavesGuids[m_uiCurrentArmyWave - 1].end())
        {
            ...
            // If all the soldiers from the current wave are dead, then send
            // the next one
...
void instance_ruins_of_ahnqiraj::SetData(uint32 uiType, uint32 uiData)
{
    ...
    case TYPE_KURINNAXX:
        if (uiData == DONE)
        {
            ...
            if (Creature *ossirian = GetSingleCreatureFromStorage(NPC_OSSIRIAN))
                DoScriptText(SAY_OSSIRIAN_INTRO,
                             ossirian); // targets kurinnaxx in sniff
        }
...
void instance_ruins_of_ahnqiraj::DoSortArmyWaves()
{
    ...
    // We need to use gridsearcher for this, because coords search is too
    // complicated here
    for (uint8 i = 0; i < MAX_ARMY_WAVES; ++i)
    {
        ...
        {
            GetCreatureListWithEntryInGrid(lCreatureList, pTemp, NPC_QIRAJI_WARRIOR,
...
{
    // The next army wave is sent into battle after 2 min or after the previous
    // wave is finished
    if (GetData(TYPE_RAJAXX) != IN_PROGRESS)
        return;
    ...
    else
    {
        ...
        if (m_uiCurrentArmyWave == MAX_ARMY_WAVES)
        {
            script_error_log("Instance Ruins of Ahn'Qiraj: ERROR Something "
                             "unexpected happened. Please report to SD2 team.");
            return;
        }
        ...
        for (auto itr : m_sArmyWavesGuids[m_uiCurrentArmyWave])
        {
            if (Creature *pTemp = instance->GetCreature(itr))
            {
                ...
                pTemp->SetWalk(false);
                pTemp->GetRandomPoint(aAndorovMoveLocs[4].m_fX, aAndorovMoveLocs[4].m_fY, aAndorovMoveLocs[4].m_fZ,
                                      10.0f, fX, fY, fZ);
        ...
        // on wowwiki it states that there were 3 min between the waves, but this
        // was reduced in later patches
        m_uiArmyDelayTimer = 2 * MINUTE * IN_MILLISECONDS;
    }
