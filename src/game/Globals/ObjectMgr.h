...
struct AreaTrigger
{
    ...
    bool IsLessOrEqualThan(AreaTrigger const *l) const // Expected to have same map
    {
        ...
        return requiredLevel <= l->requiredLevel && requiredItem <= l->requiredItem &&
               requiredItem2 <= l->requiredItem2 && heroicKey <= l->heroicKey && heroicKey2 <= l->heroicKey2 &&
               requiredQuest <= l->requiredQuest && requiredQuestHeroic <= l->requiredQuestHeroic;
    }
};
...
struct BroadcastText
{
    ...
    ChatType chatTypeId; // not contained in SMSG_DB_REPLY, but added here for
                         // ease of use
    Language languageId;
    // uint32 conditionId;
    // uint32 emotesId;
...
class ObjectMgr
{
  ...
  public:
    ...
    {
        return m_creatureSpawnEntryMap;
    }
    ...
    GroupMap::iterator GetGroupMapBegin()
    {
        return mGroupMap.begin();
    }
    GroupMap::iterator GetGroupMapEnd()
    {
        return mGroupMap.end();
    }
    ...
    std::vector<uint32> const *GetAllRandomEntries(std::unordered_map<uint32, std::vector<uint32>> const &map,
                                                   uint32 dbguid) const
    {
    ...
    uint32 GetRandomGameObjectEntry(uint32 dbguid) const
    {
        return GetRandomEntry(m_gameobjectSpawnEntryMap, dbguid);
    }
    std::vector<uint32> const *GetAllRandomGameObjectEntries(uint32 dbguid) const
    {
        return GetAllRandomEntries(m_gameobjectSpawnEntryMap, dbguid);
    }

    uint32 GetRandomCreatureEntry(uint32 dbguid) const
    {
        return GetRandomEntry(m_creatureSpawnEntryMap, dbguid);
    }
    std::vector<uint32> const *GetAllRandomCreatureEntries(uint32 dbguid) const
    {
        return GetAllRandomEntries(m_creatureSpawnEntryMap, dbguid);
    }
    ...
    static Player *GetPlayer(ObjectGuid guid,
                             bool inWorld = true);             ///< Wrapper for ObjectAccessor::FindPlayer
    static CreatureInfo const *GetCreatureTemplate(uint32 id); ///< Wrapper for sCreatureStorage.LookupEntry
    static CreatureModelInfo const *GetCreatureModelInfo(
        uint32 modelid);                                              ///< Wrapper for sCreatureModelStorage.LookupEntry
    ...
    static CreatureDataAddon const *GetCreatureAddon(
        uint32 lowguid); ///< Wrapper for sCreatureDataAddonStorage.LookupEntry
    static CreatureDataAddon const *GetCreatureTemplateAddon(
        uint32 entry);                                       ///< Wrapper for sCreatureInfoAddonStorage.LookupEntry
    ...
    static CreatureConditionalSpawn const *GetCreatureConditionalSpawn(
        uint32 lowguid); ///< Wrapper for sCreatureConditionalSpawnStore.LookupEntry
    ...
    /// @param _map Map* of the map for which to load active entities. If nullptr
    /// active entities on continents are loaded
    ...
    uint32 GenerateGroupLowGuid()
    {
        return m_GroupGuids.Generate();
    }

    uint32 GenerateArenaTeamId()
    {
        return m_ArenaTeamIds.Generate();
    }
    ...
    void AddItemText(uint32 itemTextId, const std::string &text)
    {
    ...
    void GetNpcTextLocaleStringsAll(uint32 entry, int32 loc_idx, NpcTextArray *text0_Ptr,
                                    NpcTextArray *text1_Ptr) const;
    void GetNpcTextLocaleStrings0(uint32 entry, int32 loc_idx, std::string *text0_0_Ptr,
    ...
    int GetDbc2StorageLocaleIndex() const
    {
        return m_Dbc2StorageLocaleIndex;
    }
    void SetDbc2StorageLocaleIndex(LocaleConstant loc)
    {
        m_Dbc2StorageLocaleIndex = GetStorageLocaleIndexFor(loc);
    }

    int GetStorageLocaleIndexFor(LocaleConstant loc);
    int GetOrNewStorageLocaleIndexFor(LocaleConstant loc);

    // global grid objects state (static DB spawns, global spawn mods from
    // gameevent system)
    ...
    // modifiers for global grid objects state (static DB spawns, global spawn
    // mods from gameevent system) Don't must be used for modify instance
    // specific spawn state modifications
    ...
    /**
     * \brief: Data returned is used to compute health, mana, armor, damage of
     *creatures. May be nullptr. \param uint32 level               creature level
     * \param uint32 unitClass           creature class, related to
     *CLASSMASK_ALL_CREATURES \param uint32 expansion           creature
     *expansion (we could have creature exp = 0 for tbc as well as exp = 1)
     *\return: CreatureClassLvlStats const* or nullptr
     *
     * Description: GetCreatureClassLvlStats give fast access to creature stats
     *data. FullName: ObjectMgr::GetCreatureClassLvlStats Access: public
     * Qualifier: const
     **/
    ...
    std::shared_ptr<CreatureSpellListContainer> GetCreatureSpellListContainer()
    {
        return m_spellListContainer;
    }
    std::shared_ptr<SpawnGroupEntryContainer> GetSpawnGroupContainer()
    {
        return m_spawnGroupEntries;
    }
  ...
  private:
    ...
    // Array to store creature stats, Max creature level + 1 (for data alignement
    // with in game level)
...
bool LoadMangosStrings(DatabaseType &db, char const *table, int32 start_value = -1000000,
