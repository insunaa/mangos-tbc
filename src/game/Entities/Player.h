...
enum EnvironmentFlags
{
    ...
    ENVIRONMENT_MASK_LIQUID_FLAGS =
        (ENVIRONMENT_FLAG_UNDERWATER | ENVIRONMENT_MASK_IN_LIQUID | ENVIRONMENT_FLAG_HIGH_SEA |
         ENVIRONMENT_FLAG_LIQUID | ENVIRONMENT_FLAG_HIGH_LIQUID),
};
...
struct LookingForGroupInfo
{
    struct Slot
    {
        ...
        bool set(uint16 _entry, uint16 _type)
        {
            entry = _entry;
            type = _type;
            return !empty();
        }
        bool is(uint16 _entry, uint16 _type) const
        {
            return entry == _entry && type == _type;
        }
        bool isAuto() const
        {
            return entry && (type == LFG_TYPE_DUNGEON || type == LFG_TYPE_HEROIC_DUNGEON);
        }
    ...
    inline bool isLFG(LookingForGroupInfo const &info, bool autoOnly) const
    {
        return isLFG(uint16(info.more.entry), uint16(info.more.type), autoOnly);
    }
    inline bool isLFM() const
    {
        return !more.empty();
    }
    inline bool isLFM(uint32 entry, uint32 type) const
    {
        return more.is(uint16(entry), uint16(type));
    }
...
enum PlayerFlags
{
    ...
    PLAYER_FLAGS_UNK16 = 0x00008000,          // strange visual effect (2.0.1), looks like
...
enum PlayerFieldByte2Flags
{
    ...
    PLAYER_FIELD_BYTE2_DETECT_AMORE_0 = 0x02, // SPELL_AURA_DETECT_AMORE, not used as value and maybe not relcted
                                              // to, but used in code as base for mask apply
...
class MirrorTimer
{
  public:
    ...
    inline bool IsRegenerating() const
    {
        return (m_scale > 0);
    }
    inline bool IsFrozen() const
    {
        return (m_frozen && !IsRegenerating());
    }
    ...
    inline uint32 GetRemaining() const
    {
        return (m_tracker.GetInterval() - m_tracker.GetCurrent());
    }
    inline uint32 GetDuration() const
    {
        return m_tracker.GetInterval();
    }
    inline int32 GetScale() const
    {
        return m_scale;
    }
    inline uint32 GetSpellId() const
    {
        return m_spellId;
    }
...
struct BGData
{
    ...
    bool m_needSave; ///< true, if saved to DB fields modified after prev. save
                     ///< (marked as "saved" above)
};
...
class Player : public Unit
{
  ...
  public:
    ...
    const PlayerSocial *GetSocial() const
    {
        return m_social;
    }
    ...
    bool isDebuggingAreaTriggers()
    {
        return m_isDebuggingAreaTriggers;
    }
    void SetDebuggingAreaTriggers(bool on)
    {
        m_isDebuggingAreaTriggers = on;
    }
    ...
    uint32 GetMaxAttainableLevel() const
    {
    ...
    uint32 GetGrantableLevels() const
    {
        return m_grantableLevels;
    }
    void SetGrantableLevels(uint32 levels)
    {
        m_grantableLevels = levels;
    }
     ...
     * \param: bool inRestPlace  > if it was offline, is the player was in
     *city/tavern/inn? \returns: float
     **/
    ...
    bool IsTaxiDebug() const
    {
        return m_taxiTracker.m_debug;
    }
    void ToggleTaxiDebug()
    {
        m_taxiTracker.m_debug = !m_taxiTracker.m_debug;
    }
    ...
    {
        return GetWeaponForAttack(type, false, false);
    }
    ...
    void RemoveItem(uint8 bag, uint8 slot,
                    bool update); // see ApplyItemOnStoreSpell notes
    ...
    // if completed_or_not = 0 (or any other value except 1 or 2) - returns true,
    // if quest is taken and doesn't depend if quest is completed or not if
    // completed_or_not = 1 - returns true, if quest is taken but not completed
    ...
    ObjectGuid GetDividerGuid() const
    {
        return m_dividerGuid;
    }
    void SetDividerGuid(ObjectGuid guid)
    {
        m_dividerGuid = guid;
    }
    ...
    PlayerMails::reverse_iterator GetMailRBegin()
    {
        return m_mail.rbegin();
    }
    PlayerMails::reverse_iterator GetMailREnd()
    {
        return m_mail.rend();
    }
    void UpdateMail();
    ...
    const QuestStatusMap &getQuestStatusMap() const
    {
        return mQuestStatus;
    };

    ObjectGuid const &GetSelectionGuid() const override
    {
        return m_curSelectionGuid;
    }
    ...
    {
        return !m_resurrectGuid.IsEmpty();
    }
    void ResurrectUsingRequestDataInit();   // Initializes motion and schedules
                                            // rest / executes it
    void ResurrectUsingRequestDataFinish(); // Finalizes resurrection

    uint32 getCinematic() const
    {
        return m_cinematic;
    }
    ...
    SkillRaceClassInfoEntry const *GetSkillInfo(
    ...
    inline uint16 GetSkillValueBase(uint16 id) const
    {
        return GetSkill(id, true, false);
    } // skill value + perm. bonus
    inline uint16 GetSkillValuePure(uint16 id) const
    {
        return GetSkill(id, false, false);
    } // skill value
    inline uint16 GetSkillMax(uint16 id) const
    {
        return GetSkill(id, true, true, true);
    } // skill max + perm. bonus + temp bonus
    inline uint16 GetSkillMaxPure(uint16 id) const
    {
        return GetSkill(id, false, false, true);
    } // skill max
    ...
    inline int16 GetSkillBonusPermanent(uint16 id) const
    {
        return GetSkillBonus(id, true);
    } // skill perm. bonus
    inline int16 GetSkillBonusTemporary(uint16 id) const
    {
        return GetSkillBonus(id);
    } // skill temp bonus
    void UpdateSkillsForLevel(bool maximize = false);
    void UpdateSkillTrainedSpells(uint16 id,
                                  uint16 currVal); // learns/unlearns spells dependent on a skill
    void UpdateSpellTrainedSkills(uint32 spellId,
                                  bool apply); // learns/unlearns skills dependent on a spell
    ...
    {
        return m_semaphoreTeleport_Near || m_semaphoreTeleport_Far;
    }
    bool IsBeingTeleportedNear() const
    {
        return m_semaphoreTeleport_Near;
    }
    bool IsBeingTeleportedFar() const
    {
        return m_semaphoreTeleport_Far;
    }
    ...
    {
        return (m_environmentFlags & ENVIRONMENT_FLAG_UNDERWATER);
    }
    bool IsInWater() const override
    {
        return (m_environmentFlags & ENVIRONMENT_FLAG_IN_WATER);
    }
    inline bool IsInMagma() const
    {
        return (m_environmentFlags & ENVIRONMENT_FLAG_IN_MAGMA);
    }
    inline bool IsInSlime() const
    {
        return (m_environmentFlags & ENVIRONMENT_FLAG_IN_SLIME);
    }
    inline bool IsInHighSea() const
    {
        return (m_environmentFlags & ENVIRONMENT_FLAG_HIGH_SEA);
    }
    inline bool IsInHighLiquid() const
    {
        return (m_environmentFlags & ENVIRONMENT_FLAG_HIGH_LIQUID);
    }
    ...
    {
        return HasAuraType(SPELL_AURA_MOD_FLIGHT_SPEED_MOUNTED) || HasAuraType(SPELL_AURA_FLY);
    }
    ...
    bool HasAtClient(WorldObject const *u)
    {
    ...
    uint32 GetBGPetSpell() const
    {
        return m_BGPetSpell;
    }
    void SetBGPetSpell(uint32 petSpell)
    {
        m_BGPetSpell = petSpell;
    }
...
#ifdef BUILD_PLAYERBOT
    // A Player can either have a playerbotMgr (to manage its bots), or have
    // playerbotAI (if it is a bot), or neither. Code that enables bots must
    // create the playerbotMgr and set it using SetPlayerbotMgr.
  ...
  protected:
    ...
    */
    ...
    uint32 m_enchantmentFlatMod[MAX_ATTACK]; // TODO: Stat system - incorporate
                                             // generically, exposes a required
                                             // hidden weapon stat that does not
                                             // apply when unarmed
  ...
  private:
    ...
    bool IsHasDelayedTeleport() const
    {
        ...
        // been alive at teleport because we don't want player's ghost teleported
        // from graveyard
        return m_bHasDelayedTeleport &&
               (IsAlive() || !m_bHasBeenAliveAtDelayedTeleport || GetDeathState() == JUST_DIED);
    }
...
template <class T> void Player::ApplySpellMod(uint32 spellId, SpellModOp op, T &basevalue, bool finalUse)
{
    ...
    for (SpellModifier *mod : m_spellMods[op])
    {
        ...
        if (mod->isFinite && finalUse)
        {
            ...
            else if (m_consumedMods->find(SpellModifierPair(mod->spellId, mod->modId)) !=
                     m_consumedMods->end()) // if already consumed, dont consume again
                consume = false;
