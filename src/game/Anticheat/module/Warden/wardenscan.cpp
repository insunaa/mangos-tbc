    ...
    : _module(module), _wanted(wanted),
      WindowsScan(
          ...
          [this](const Warden *warden, std::vector<std::string> &, ByteBuffer &scan) {
              ...
              scan << static_cast<uint8>(winWarden->GetModule()->opcodes[FIND_MODULE_BY_NAME] ^ winWarden->GetXor())
                   << seed;
...
WindowsModuleScan::WindowsModuleScan(const std::string &module, CheckT checker, const std::string &comment,
                                     uint32 flags)
    : _module(module),
      WindowsScan(
          // builder
...
WindowsMemoryScan::WindowsMemoryScan(uint32 offset, const void *expected, size_t length, const std::string &comment,
                                     uint32 flags)
      ...
      WindowsScan(
          ...
          [this](const Warden *warden, std::vector<std::string> &, ByteBuffer &scan) {
              ...
              scan << static_cast<uint8>(winWarden->GetModule()->opcodes[READ_MEMORY] ^ winWarden->GetXor())
                   << static_cast<uint8>(0) // no string associated with this
...
WindowsMemoryScan::WindowsMemoryScan(const std::string &module, uint32 offset, const void *expected, size_t length,
                                     const std::string &comment, uint32 flags)
    : _expected(length), _offset(offset), _module(module),
      WindowsScan(
          ...
          [this](const Warden *warden, std::vector<std::string> &strings, ByteBuffer &scan) {
              ...
              scan << static_cast<uint8>(winWarden->GetModule()->opcodes[READ_MEMORY] ^ winWarden->GetXor())
                   << static_cast<uint8>(strings.size()) << this->_offset << static_cast<uint8>(this->_expected.size());
          },
          // checker
          [this](const Warden *, ByteBuffer &buff) {
              // non-zero value indicates failure
          ...
          module.length() + sizeof(uint8) + sizeof(uint8) + sizeof(uint32) + sizeof(uint8), sizeof(uint8) + length,
          comment, flags)
{
    // must fit within uint8
    MANGOS_ASSERT(_expected.size() <= 0xFF);
    ...
    // since this scan uses GetModuleHandle() rather than
    // GetFirstModule()/GetNextModule(), this is case insensitive. but still it
    // seems prudent to be consistent
    std::transform(_module.begin(), _module.end(), _module.begin(), ::toupper);
}

WindowsMemoryScan::WindowsMemoryScan(uint32 offset, size_t length, CheckT checker, const std::string &comment,
                                     uint32 flags)
    : _expected(length), _offset(offset),
      WindowsScan(
          // builder
          ...
          checker, sizeof(uint8) + sizeof(uint8) + sizeof(uint32) + sizeof(uint8), sizeof(uint8) + length, comment,
          flags)
{
    MANGOS_ASSERT(_expected.size() <= 0xFF);
}

WindowsMemoryScan::WindowsMemoryScan(const std::string &module, uint32 offset, size_t length, CheckT checker,
                                     const std::string &comment, uint32 flags)
    : _expected(length), _offset(offset), _module(module),
      WindowsScan(
          // builder
          ...
          checker, module.length() + sizeof(uint8) + sizeof(uint8) + sizeof(uint32) + sizeof(uint8),
          sizeof(uint8) + length, comment, flags)
{
}

WindowsCodeScan::WindowsCodeScan(uint32 offset, const std::vector<uint8> &pattern, bool memImageOnly, bool wanted,
                                 const std::string &comment, uint32 flags)
    : _offset(offset), _pattern(pattern), _memImageOnly(memImageOnly), _wanted(wanted),
      WindowsScan(
          ...
          [this](const Warden *warden, std::vector<std::string> &, ByteBuffer &scan) {
              ...
              scan << static_cast<uint8>(
                          winWarden->GetModule()
                              ->opcodes[this->_memImageOnly ? FIND_MEM_IMAGE_CODE_BY_HASH : FIND_CODE_BY_HASH] ^
                          winWarden->GetXor())
                   << seed;
          ...
          sizeof(uint8) + sizeof(uint32) + SHA_DIGEST_LENGTH + sizeof(uint32) + sizeof(uint8), sizeof(uint8), comment,
          flags)
{
    MANGOS_ASSERT(_pattern.size() <= 0xFF);
}

WindowsFileHashScan::WindowsFileHashScan(const std::string &file, const void *expected, bool wanted,
                                         const std::string &comment, uint32 flags)
    : _file(file), _wanted(wanted), _hashMatch(!!expected),
      WindowsScan(
          ...
          [this](const Warden *, ByteBuffer &buff) {
              ...
              // either we wanted it but didn't find it, or didn't want it but
              // did find it
              ...
              // if a hash was given, check it (some checks may only be
    ...
    : _lua(lua), _wanted(wanted),
      WindowsScan(
          ...
          [this](const Warden *, ByteBuffer &buff) {
              ...
              // if its found we have to 'read' the string, even if we don't care
              // what it is
              if (found)
              {
...
WindowsLuaScan::WindowsLuaScan(const std::string &lua, const std::string &expectedValue, const std::string &comment,
                               uint32 flags)
...
WindowsHookScan::WindowsHookScan(const std::string &module, const std::string &proc, const void *hash, uint32 offset,
                                 size_t length, const std::string &comment, uint32 flags)
    : _module(module), _proc(proc), _offset(offset), _length(length),
      WindowsScan(
          ...
          [this](const Warden *warden, std::vector<std::string> &strings, ByteBuffer &scan) {
              ...
              scan << static_cast<uint8>(strings.size() - 1) << static_cast<uint8>(strings.size()) << this->_offset
                   << static_cast<uint8>(this->_length);
          },
          // checker
          [](const Warden *, ByteBuffer &buff) { return buff.read<uint8>() == Detoured; },
          sizeof(uint8) + sizeof(uint32) + SHA_DIGEST_LENGTH + sizeof(uint8) + sizeof(uint8) + sizeof(uint32) +
              sizeof(uint8),
...
WindowsDriverScan::WindowsDriverScan(const std::string &name, const std::string &targetPath, bool wanted,
                                     const std::string &comment, uint32 flags)
    : _name(name), _wanted(wanted), _targetPath(targetPath),
      WindowsScan(
          ...
          [this](const Warden *warden, std::vector<std::string> &strings, ByteBuffer &scan) {
              ...
              scan << static_cast<uint8>(winWarden->GetModule()->opcodes[FIND_DRIVER_BY_NAME] ^ winWarden->GetXor())
                   << seed;
          ...
          sizeof(uint8) + sizeof(uint32) + SHA_DIGEST_LENGTH + sizeof(uint8) + name.length(), sizeof(uint8), comment,
          flags)
{
}
