...
SDName: Boss_Jeklik
SD%Complete: 85
SDComment: Some minor improvements are required; Bat rider movement not
implemented SDCategory: Zul'Gurub EndScriptData */
...
enum
{
    ...
    // SPELL_PSYHIC_SCREAM      = 22884,                   // spell not confirmed
    // - needs research curse of blood also unconfirmed
...
struct boss_jeklikAI : public CombatAI
{
    boss_jeklikAI(Creature *creature)
        : CombatAI(creature, JEKLIK_ACTION_MAX),
          m_instance(static_cast<instance_zulgurub *>(creature->GetInstanceData()))
    {
    ...
    void Aggro(Unit * /*pWho*/) override
    {
        ...
        // Note: on aggro the bats from the cave behind the boss should fly
        // outside!
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case JEKLIK_PHASE_BATS:
            if (m_creature->GetHealthPercent() < 35.0f)
            {
                ...
                Creature *bat =
                    m_creature->SummonCreature(NPC_BAT_RIDER, pos1.x, pos1.y, pos1.z, 0, TEMPSPAWN_DEAD_DESPAWN, 0);
...
struct npc_gurubashi_bat_riderAI : public CombatAI
{
    ...
    void Aggro(Unit * /*pWho*/) override
    {
        // Don't attack if is summoned by Jeklik - the npc gets aggro because of
        // the Liquid Fire
        if (m_bIsSummon)
            return;
