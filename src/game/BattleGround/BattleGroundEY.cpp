...
void BattleGroundEY::RemovePlayer(Player *player, ObjectGuid guid)
{
    ...
    if (IsFlagPickedUp())
    {
        if (m_flagCarrier == guid)
        {
            ...
            {
                sLog.outError("BattleGroundEY: Removing offline player who "
...
void BattleGroundEY::ProcessCaptureEvent(GameObject *go, uint32 towerId, Team team, uint32 newWorldState,
                                         uint32 message)
{
    DEBUG_LOG("BattleGroundEY: Process capture point event from gameobject entry "
              "%u, captured by team %u",
              go->GetEntry(), team);
...
// Method that respawns dropped flag; called if nobody picks the dropped flag
// after 10 seconds
...
// Method that handles the player score when flag is captured at one of
// controlled nodes
void BattleGroundEY::ProcessPlayerFlagScoreEvent(Player *source, EYNodes node)
{
    ...
    DEBUG_LOG("BattleGroundEY: Team %u has successfully delivered the flag to "
              "the node id %u",
              source->GetTeam(), node);
...
void BattleGroundEY::FillInitialWorldStates(WorldPacket &data, uint32 &count)
{
    ...
    FillInitialWorldState(data, count, WORLD_STATE_EY_BLOOD_ELF_TOWER_HORDE,
                          m_towerOwner[NODE_BLOOD_ELF_TOWER] == HORDE);
    ...
    // if you leave the bg while being in capture point radius - and later join
    // same type of bg the slider would still be displayed because the client
    // caches it
