...
improvement SDCategory: Zul'Aman EndScriptData */
...
enum
{
     ...
     * channel, which takes 10 sec The siphon soul is a channeled spell for 30
     * sec during which the boss uses some class abilities of the target
     * Basically the boss casts a dummy spell which chooses a random target on
     * which it casts the actuall channel spell The drain power spell acts as a
     * enrage timer. It's cast each 30 seconds after the boss' health is below
     * 80%
     */
...
struct boss_malacrassAI : public CombatAI
{
          ...
          m_instance(static_cast<instance_zulaman *>(creature->GetInstanceData()))
    {
        ...
        m_creature->GetCombatManager().SetLeashingCheck([](Unit *, float x, float y, float z) { return y > 1025.f; });
    ...
    void DoInitializeAdds()
    {
        ...
        if (addsList.empty())
        {
            ...
            for (uint8 i = 0; i < MAX_ACTIVE_ADDS; ++i)
            {
                ...
                Creature *creature = m_creature->SummonCreature(aSpawnEntries[i][addVersion], m_aAddPositions[i][0],
                                                                m_aAddPositions[i][1], m_aAddPositions[i][2],
                                                                m_aAddPositions[i][3], TEMPSPAWN_DEAD_DESPAWN, 0);
        ...
        else
        {
            ...
            for (uint8 i = 0; i < MAX_ACTIVE_ADDS; ++i)
            {
                ...
                m_creature->SummonCreature(addsList[i], m_aAddPositions[i][0], m_aAddPositions[i][1],
        ...
        if (target->GetTypeId() == TYPEID_PLAYER && spellInfo->Id == SPELL_SIPHON_SOUL)
        {
            ...
            for (uint8 i = 0; i < 3; ++i)
            {
                ...
                case SPELL_HU_SNAKE_TRAP:
                    ...
                    break;
                default:
                    ResetCombatAction(MALACRASS_PLAYER_ABILITY_1 + i,
                                      aMalacrassStolenAbility[m_playerClass][i].m_uiInitialTimer);
                    break;
                }
            }
        }
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case MALACRASS_PLAYER_ABILITY_4:
            if (CanUseSpecialAbility(action - MALACRASS_PLAYER_ABILITY_1))
                ResetCombatAction(
                    action, aMalacrassStolenAbility[m_playerClass][action - MALACRASS_PLAYER_ABILITY_1].m_uiCooldown);
            break;
        }
    }
};
