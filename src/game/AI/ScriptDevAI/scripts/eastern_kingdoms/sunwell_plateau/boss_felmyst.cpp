...
SDName: boss_felmyst
SD%Complete: 90%
SDComment: Intro movement NYI; Event cleanup (despawn & resummon) NYI; Breath
phase spells could use some improvements. SDCategory: Sunwell Plateau
EndScriptData */
...
enum
{
    ...
    // NPC_DEMONIC_VAPOR           = 25265,        // npc which follows the
    // player NPC_DEMONIC_VAPOR_TRAIL     = 25267,
...
struct boss_felmystAI : public CombatAI
{
    boss_felmystAI(Creature *creature)
        : CombatAI(creature, FELMYST_ACTION_MAX),
    ...
    float GetFlightOrientation(bool isLeftSide)
    {
        return isLeftSide ? 4.675057 : 1.58825;
    }
    ...
    void Aggro(Unit *who) override
    {
        ...
        if (!who) // should never happen but just a safeguard
            m_creature->GetMotionMaster()->MovePoint(POINT_AGGRO, 1483.703f, 623.2387f, 28.17801f, FORCED_MOVEMENT_RUN,
                                                     false);
        else
        {
            Position pos;
            float targetDist = who->GetCombinedCombatReach(m_creature, false);
            who->GetNearPoint(m_creature, pos.x, pos.y, pos.z, m_creature->GetObjectBoundingRadius(), targetDist,
                              who->GetAngle(m_creature));
    ...
    void MovementInform(uint32 motionType, uint32 pointId) override
    {
        ...
        case SUBPHASE_BREATH_PREPARE:
            ...
            {
                static Position landingPositions[] = {{1469.93f, 557.009f, 22.6317f, 0.f},
                                                      {1476.77f, 665.094f, 20.6423f, 0.f}};
                m_uiPhase = PHASE_TRANSITION; // TODO: figure out where to land
                m_creature->GetMotionMaster()->MovePoint(PHASE_TRANSITION, landingPositions[m_bIsLeftSide],
    ...
    void HandleLiftoff()
    {
        m_creature->GetMotionMaster()->MovePoint(SUBPHASE_VAPOR,
                                                 Position(m_creature->GetPositionX(), m_creature->GetPositionY(),
                                                          m_creature->GetPositionZ() + 20.f,
    ...
    void HandleSideMove()
    {
        if (Creature *trigger = m_instance->GetSingleCreatureFromStorage(m_bIsLeftSide ? NPC_FLIGHT_TRIGGER_LEFT
                                                                                       : NPC_FLIGHT_TRIGGER_RIGHT))
            m_creature->GetMotionMaster()->MovePoint(SUBPHASE_BREATH_SIDE,
                                                     Position(trigger->GetPositionX(), trigger->GetPositionY(),
                                                              trigger->GetPositionZ(),
                                                              GetFlightOrientation(m_bIsLeftSide)),
                                                     FORCED_MOVEMENT_RUN, 0.f, false);
    }
    ...
    void HandlePreBreath()
    {
        ...
        if (Creature *trigger = m_creature->GetMap()->GetCreature(
                m_instance->SelectFelmystFlightTrigger(m_bIsLeftSide, m_uiCorruptionIndex)))
            m_creature->GetMotionMaster()->MovePoint(SUBPHASE_BREATH_PREPARE,
                                                     Position(trigger->GetPositionX(), trigger->GetPositionY(),
                                                              trigger->GetPositionZ(),
                                                              GetFlightOrientation(m_bIsLeftSide)),
                                                     FORCED_MOVEMENT_RUN, 0.f, false);
    ...
    void HandleBreath()
    {
                ...
                m_instance->SelectFelmystFlightTrigger(!m_bIsLeftSide, m_uiCorruptionIndex)))
        {
            ...
            static const uint32 strafeSpells[] = {SPELL_TRIGGER_TOP_STRAFE, SPELL_TRIGGER_MIDDLE_STRAFE,
            ...
            m_creature->GetMotionMaster()->MovePoint(SUBPHASE_BREATH_MOVE,
                                                     Position(trigger->GetPositionX(), trigger->GetPositionY(),
                                                              trigger->GetPositionZ(),
                                                              GetFlightOrientation(!m_bIsLeftSide)),
                                                     FORCED_MOVEMENT_RUN, 50.f, false);
        }
    }
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case FELMYST_ENCAPSULATE:
            if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_ENCAPSULATE_CHANNEL,
                                                                 SELECT_FLAG_PLAYER | SELECT_FLAG_NOT_IMMUNE))
...
struct SoulSever : public SpellScript
{
    void OnEffectExecute(Spell *spell, SpellEffectIndex effIdx) const override
    {
        ...
        if (!unitTarget || unitTarget->GetTypeId() != TYPEID_PLAYER ||
            !unitTarget->HasAura(SPELL_FOG_OF_CORRUPTION_CHARM))
            return;
