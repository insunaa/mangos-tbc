...
enum SharpeningStoneDisplayId
{
    ...
    CONSECRATED_SHARPENING_DISPLAYID = 24674, // will not be used because bot can not know if it will face
                                              // undead targets
...
{
    // TODO: RETURN_NO_ACTION_UNKNOWN is not part of ANY_OK or ANY_ERROR. It's
    // also bad form and should be eliminated ASAP.
    RETURN_NO_ACTION_OK = 0x01,                 // No action taken during this combat maneuver,
                                                // as intended (just wait, etc...)
    ...
    RETURN_FINISHED_FIRST_MOVES = 0x10,         // Last action of first-combat-maneuver finished, continue onto
                                                // next-combat-maneuver
    RETURN_CONTINUE = 0x20,                     // Continue first moves; normal return value for
                                                // next-combat-maneuver
    RETURN_NO_ACTION_INSUFFICIENT_POWER = 0x40, // No action taken due to insufficient power (rage, focus, mana,
                                                // runes)
...
class PlayerbotAI
{
  public:
    ...
    {
        SCENARIO_PVE,
        SCENARIO_PVE_ELITE, // group (5 members max) when an elite is near - most
                            // likely instance (can happen in open world)
        SCENARIO_PVE_RAID,
        SCENARIO_PVP_DUEL,
        SCENARIO_PVP_BG, // You'll probably want to expand this to suit goal?
                         // (capture the flag, assault, domination, ...)
        SCENARIO_PVP_ARENA,
    ...
    enum CombatOrderType
    {
        ...
        ORDERS_TEMP_WAIT_TANKAGGRO = 0x0040, // Wait on tank to build aggro - expect healing to continue,
                                             // disable setting when tank loses focus
        ORDERS_TEMP_WAIT_OOC = 0x0080,       // Wait but only while OOC - wait only - combat will resume
                                             // healing, dps, tanking, ...
        ...
        ORDERS_MAIN_HEAL = 0x2000,           // concentrate on healing the main tank (will ignore other
                                             // targets as long as MT needs healing)
        ORDERS_NOT_MAIN_HEAL = 0x4000,       // concentrate on healing except the main
                                             // tank that will be ignored

        // Cumulative orders
        ORDERS_PRIMARY = 0x7007,
        ORDERS_SECONDARY = 0x0F78,
        ORDERS_RESIST = 0x0F00,
        ORDERS_TEMP = 0x00C0, // All orders NOT to be saved, turned off by bots (or
                              // logoff, reset, ...)
        ORDERS_RESET = 0xFFFF
    };
  ...
  public:
    ...
    // This is called from ChatHandler.cpp when there is an incoming message to
    // the bot from a whisper or from the party channel
    ...
    // Since there is no client at the other end, the packets are dropped of
    // course. For a list of opcodes that can be caught see Opcodes.cpp (SMSG_*
    // opcodes only)
    ...
    void SetCombatStyle(CombatStyle cs)
    {
        m_combatStyle = cs;
    }

    PlayerbotClassAI *GetClassAI()
    {
        return m_classAI;
    }
    void ReloadAI();
    ...
    // in priority of full text match, spells not taking reagents, and highest
    // rank
    ...
    // finds items in bots equipment and adds them to foundItemList, removes
    // found items from itemIdSearchList
    void findItemsInEquip(std::list<uint32> &itemIdSearchList, std::list<Item *> &foundItemList) const;
    // finds items in bots inventory and adds them to foundItemList, removes
    // found items from itemIdSearchList
    void findItemsInInv(std::list<uint32> &itemIdSearchList, std::list<Item *> &foundItemList) const;
    // finds nearby game objects that are specified in m_collectObjects then adds
    // them to the m_lootTargets list
    void findNearbyGO();
    // finds nearby creatures, whose UNIT_NPC_FLAGS match the flags specified in
    // item list m_itemIds
    ...
    // Used by bots to check if their target is neutralized (polymorph, shackle
    // or the like). Useful to avoid breaking crowd control
    static bool IsNeutralized(Unit *target);
    // Make the bots face their target
    void FaceTarget(Unit *target);
    // Used by bot to check if target is immune to a specific damage school
    ...
    uint32 GetCurrentSpellId()
    {
        return m_CurrentlyCastingSpellId;
    }
    ...
    void SetIgnoreUpdateTime(uint8 t = 0)
    {
        m_ignoreAIUpdatesUntilTime = time(nullptr) + t;
    };
    time_t CurrentTime()
    {
        return time(nullptr);
    ...
    {
        return m_needItemList.find(itemid) != m_needItemList.end();
    };
    bool IsInQuestCreatureList(uint32 id)
    {
        return m_needCreatureOrGOList.find(id) != m_needCreatureOrGOList.end();
    };
    ...
    bool HasCollectFlag(uint8 flag)
    {
        return (m_collectionFlags & flag) ? true : false;
    }
    ...
    bool IsRegenerating();
    bool IsGroupReady();
    Player *GetGroupTank(); // TODO: didn't want to pollute non-playerbot code
                            // but this should really go in group.cpp
    ...
    uint32 GetAttackerCount()
    {
        return m_attackerInfo.size();
    };
    ...
    CombatOrderType GetCombatOrder()
    {
        return this->m_combatOrder;
    }
    bool IsMainTank()
    {
        return (m_combatOrder & ORDERS_MAIN_TANK) ? true : false;
    }
    bool IsTank()
    {
        return (m_combatOrder & ORDERS_TANK) || IsMainTank() ? true : false;
    }
    bool IsMainHealer()
    {
        return (m_combatOrder & ORDERS_MAIN_HEAL) ? true : false;
    }
    bool IsHealer()
    {
        return (m_combatOrder & (ORDERS_HEAL | ORDERS_NOT_MAIN_HEAL)) || IsMainHealer() ? true : false;
    }
    bool HasDispelOrder()
    {
        return !(m_combatOrder & ORDERS_NODISPEL);
    }
    bool IsDPS()
    {
        return (m_combatOrder & ORDERS_ASSIST) ? true : false;
    }
    bool Impulse()
    {
        srand(time(nullptr));
    ...
    {
        return this->m_resistType;
    }
    void SetMovementOrder(MovementOrderType mo, Unit *followTarget = 0);
    MovementOrderType GetMovementOrder()
    {
        return this->m_movementOrder;
    }
    void MovementReset();
    void MovementClear();
  ...
  private:
    ...
    // owns the "this" object and m_master owns m_bot. The owner always cleans
    // up.
    PlayerbotMgr &m_mgr;
    Player *const m_bot;
    ...
    Unit *m_targetAssist;  // get new target by checking attacker list of assisted
                           // player
    ...
    float m_destX, m_destY,
        m_destZ; // latest coordinates for chase and point movement types
