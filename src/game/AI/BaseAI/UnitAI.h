...
enum CastFlags
{
    ...
    CAST_TRIGGERED = 0x02,                  // Triggered (this makes spell cost zero mana and have
                                            // no cast time)
    ...
    CAST_PLAYER_ONLY = 0x200,               // Selects only player targets - substitution for
                                            // EAI not having more params
    CAST_DISTANCE_YOURSELF = 0x400,         // If spell with this cast flag hits main aggro target, caster
                                            // distances himself - EAI only
...
class UnitAI : public CombatActions
{
  public:
    ...
    /**
     * Called if IsVisible(Unit* pWho) is true at each (relative) override pWho
     * move, reaction at visibility zone enter Note: The Unit* pWho can be out of
     * Line of Sight, usually this is only visibiliy (by state) and range
     * dependendend Note: This function is not called for creatures who are in
     * evade mode
     ...
     * function. Note: Usually called by MoveInLineOfSight, in
     * Unit::SelectHostileTarget or when the AI is forced to attack an enemy
     ...
     * @param uiType number to specify the event, default cases listed in enum
     * AIEventType
     ...
     */
    void SendAIEventAround(AIEventType eventType, Unit *invoker, uint32 delay, float radius,
                           uint32 miscValue = 0) const;

    /**
     * Send an AI Event to a Creature
     * @param eventType to specify the event, default cases listed in enum
     * AIEventType
     * @param pInvoker Unit that triggered this event (like an attacker)
     ...
     * @param resume to specify whether it launched normally or was resumed after
     * a restart
     */
    ...
    // TODO: Implement proper casterAI in EAI and remove this from Leotheras
    // script
    uint32 GetAttackDistance()
    {
    ...
    CreatureList DoFindFriendlyEligibleDispel(float range, uint32 dispelMask = 0, uint32 mechanicMask = 0,
                                              bool self = true) const;
    ...
    bool CanCastSpell();
    void SetCombatScriptStatus(bool state)
    {
        m_combatScriptHappening = state;
    };
    bool GetCombatScriptStatus() const
    {
        return m_combatScriptHappening;
    }

    void SetAIOrder(AIOrders order)
    {
        m_currentAIOrder = order;
    }
    AIOrders GetAIOrder() const
    {
        return m_currentAIOrder;
    }

    bool DoFlee(uint32 duration = 0);
    virtual bool DoRetreat()
    {
        return false;
    }                  // implemented for creatures
    void DoDistance(); // TODO
    virtual void DoCallForHelp(float /*radius*/)
    {
    } // implemented for creatures
    ...
    void SetRootSelf(bool apply,
                     bool combatOnly = false); // must call parent JustDied if this is used
    void ClearSelfRoot();

    virtual void HandleDelayedInstantAnimation(SpellEntry const *spellInfo)
    {
    }
    virtual bool IsTargetingRestricted()
    {
        return GetCombatScriptStatus();
    }
    ...
    virtual void HandleAssistanceCall(Unit *sender, Unit *invoker)
    {
    } // implemented for creatures

    virtual bool IsPreventingDeath() const
    {
        return false;
    }

    bool IsMeleeEnabled() const
    {
        return m_meleeEnabled;
    }
    ...
    void AddDistanceSpell(uint32 spellId)
    {
        m_distanceSpells.insert(spellId);
    }

    void SetRangedMode(bool state, float distance, RangeModeType type);
    void SetCurrentRangedMode(bool state);

    bool GetCurrentRangedMode()
    {
        return m_currentRangedMode;
    }
    ...
    /*
     * All units which attack at ranged need to return true. It is used for 3
     * purposes: Determining if a creature should ignore ranged targets during
     * root Attacking enemies flying overhead at aggro range Checking main spell
     * school instead of melee school for immunity suppress - TODO
     */
    bool IsRangedUnit()
    {
        return m_currentRangedMode;
    ...
    // Spell Lists
    // called when owner spell list changes - need to recalculate main spell and
    // a few other things
  ...
  protected:
    ...
    bool m_combatScriptHappening; // disables normal combat functions without
                                  // leaving combat
