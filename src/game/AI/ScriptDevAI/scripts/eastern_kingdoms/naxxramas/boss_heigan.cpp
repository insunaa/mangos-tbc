...
SDComment: Missing poison inside the eye stalk tunnel in phase 2
           Candidate spell is 30122 (correct damage range and already used in
encounter) but there is no evidence of this and of how the spell is cast and by
who (no data found in sniffs as this part was removed in WotLK) SDCategory:
...
enum
{
    ...
    SPELL_PLAGUE_WAVE_SLOW = 29351, // Activates the traps during phase 1; triggers spell 30116,
                                    // 30117, 30118, 30119 each 10 secs
    SPELL_PLAGUE_WAVE_FAST = 30114, // Activates the traps during phase 2; triggers spell 30116,
                                    // 30117, 30118, 30119 each 3 secs
...
static const float resetX = 2825.0f; // Beyond this X-line, Heigan is outside
                                     // his room and should reset (leashing)
...
struct boss_heiganAI : public CombatAI
{
    boss_heiganAI(Creature *creature)
        : CombatAI(creature, HEIGAN_ACTION_MAX),
    ...
    uint32 GetSubsequentActionTimer(uint32 id)
    {
        ...
        {
        case HEIGAN_FEVER:
            return 21u * IN_MILLISECONDS;
        case HEIGAN_MANA_BURN:
            return 10u * IN_MILLISECONDS;
        case HEIGAN_TAUNT:
            return urand(25, 90) * IN_MILLISECONDS;
        case HEIGAN_TELEPORT_PLAYERS:
            return urand(35, 45) * IN_MILLISECONDS;
        default:
    ...
    void HandlePlatformPhase()
    {
        ...
        if (DoCastSpellIfCan(m_creature, SPELL_TELEPORT_SELF) == CAST_OK)
        {
            ...
            ResetTimer(HEIGAN_ERUPTION,
                       3000u); // Small delay to let enough time for players to
                               // reach the first safe zone
...
struct npc_diseased_maggotAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 diff) override
    {
        ...
        {
            // Check if we are in range of the trigger NPC in the middle of Heigan
            // room, if so: force evade
...
struct PlagueWaveController : public AuraScript
{
    void OnPeriodicTrigger(Aura *aura, PeriodicTriggerData & /* data */) const override
    {
        ...
        triggerTarget->CastSpell(triggerTarget, spellForTick[tick], TRIGGERED_OLD_TRIGGERED, nullptr, aura,
                                 aura->GetCasterGuid(), nullptr);
    }
};
