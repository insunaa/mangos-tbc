...
void WorldSession::HandleMoveWorldportAckOpcode()
{
    ...
    {
        sLog.outError("WorldSession::HandleMoveWorldportAckOpcode: %s was teleported far "
                      "to "
                      "a not valid location "
    ...
    auto returnHomeFunc = [this, player = GetPlayer(), old_loc, loc]() {
        ...
        {
            DETAIL_LOG("WorldSession::HandleMoveWorldportAckOpcode: %s cannot be ported "
                       "to "
                       "his previous place, teleporting him to his homebind place...",
    ...
    if (mEntry->IsBattleGroundOrArena())
    {
        ...
        if (!map)
        {
            DETAIL_LOG("WorldSession::HandleMoveWorldportAckOpcode: %s was "
                       "teleported far to nonexisten battleground instance "
    ...
    {
        found = false;
        GetPlayer()->m_movementInfo.t_pos = Position(loc.coord_x, loc.coord_y, loc.coord_z,
                                                     loc.orientation); // when teleporting onto transport,
                                                                       // position is local coords
    ...
    auto lambda = [this, loc, old_loc, mEntry, mInstance](Map *map) {
        ...
        if (!map->Add(GetPlayer()))
        {
            ...
            DETAIL_LOG("WorldSession::HandleMoveWorldportAckOpcode: %s was "
                       "teleported far but couldn't be added to map "
                       " (map:%u, x:%f, y:%f, z:%f) Trying to port him to his "
                       "previous place..",
                       GetPlayer()->GetGuidStr().c_str(), loc.mapid, loc.coord_x, loc.coord_y, loc.coord_z);
            ...
            {
                DETAIL_LOG("WorldSession::HandleMoveWorldportAckOpcode: %s "
                           "cannot be ported "
                           "to his previous place, teleporting him to his "
                           "homebind place...",
        ...
        // not invited only add to bg group and object, if the player was invited
        // (else he entered through command)
...
void WorldSession::HandleForceSpeedChangeAckOpcodes(WorldPacket &recv_data)
{
    ...
    // we must inform the anticheat even if it is not for the mover, as it will
    // affect pets
    ...
    case CMSG_FORCE_FLIGHT_BACK_SPEED_CHANGE_ACK:
        ...
        break;
    default:
        sLog.outError("WorldSession::HandleForceSpeedChangeAck: Unknown move type opcode: "
                      "%u",
                      opcode);
        return;
    }
    ...
    if (!_player->GetTransport() && fabs(mover->GetSpeed(move_type) - newspeed) > 0.01f)
    {
        ...
        {
            sLog.outError("%sSpeedChange player %s is NOT correct (must be %f "
                          "instead %f), force set to correct value",
        ...
        {
            BASIC_LOG("Player %s from account id %u kicked for incorrect speed (must "
                      "be %f instead %f)",
...
void WorldSession::HandleSetActiveMoverOpcode(WorldPacket &recv_data)
{
    ...
    {
        sLog.outError("HandleSetActiveMoverOpcode: incorrect mover guid: mover is "
                      "%s and should be %s",
...
void WorldSession::HandleMoveNotActiveMoverOpcode(WorldPacket &recv_data)
{
    ...
    {
        sLog.outError("HandleMoveNotActiveMover: incorrect mover guid: mover is %s "
                      "and should be %s instead of %s",
...
void WorldSession::HandleMoveFlagChangeOpcode(WorldPacket &recv_data)
{
    ...
    case CMSG_MOVE_HOVER_ACK:
        response = MSG_MOVE_HOVER;
        break;
    case CMSG_MOVE_FEATHER_FALL_ACK:
        response = MSG_MOVE_FEATHER_FALL;
        break;
    case CMSG_MOVE_WATER_WALK_ACK:
        response = MSG_MOVE_WATER_WALK;
        break;
    case CMSG_MOVE_SET_CAN_FLY_ACK:
        response = MSG_MOVE_UPDATE_CAN_FLY;
        break;
    default:
        break;
    }
...
bool WorldSession::VerifyMovementInfo(MovementInfo const &movementInfo, Unit *mover, bool unroot) const
{
    ...
    // rooted mover sent packet without root or moving AND root - ignore, due to
    // client crash possibility
    if (!unroot)
        if (mover->IsRooted() &&
            (!movementInfo.HasMovementFlag(MOVEFLAG_ROOT) || movementInfo.HasMovementFlag(MOVEFLAG_MASK_MOVING)))
            return false;
            ...
            if (!(plMover->GetBattleGround() && plMover->GetBattleGround()->HandlePlayerUnderMap(_player)))
            {
                ...
                if (plMover->IsAlive())
                {
                    ...
                    if (!plMover->IsAlive())
                    {
                        ...
                        // timer from starting in the next player update
                        plMover->KillPlayer();
...
void WorldSession::HandleTimeSyncResp(WorldPacket &recvData)
{
    ...
    // process it and reply and for response to travel back to the server. we are
    // going to make 2 assumptions: 1) we assume that the request processing time
    // equals 0. 2) we assume that the packet took as much time to travel from
    // server to client than it took to travel from client to server.
    ...
    where
    serverTime: time that was displayed on the clock of the SERVER at the moment
    when the client processed the SMSG_TIME_SYNC_REQUEST packet. clientTime: time
    that was displayed on the clock of the CLIENT at the moment when the client
    processed the SMSG_TIME_SYNC_REQUEST packet. Once clockDelta has been
    computed, we can compute the time of an event on server clock when we know
    the time of that same event on the client clock, using the following
    relation: serverTime = clockDelta + clientTime
    */
