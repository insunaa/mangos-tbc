...
enum
{
    ...
    SPELL_QUIET_SUICIDE_UNUSED = 3617, // Currently unused. Should be used by Medivh on self when shield
                                       // runs out, 3 sec after he says his death text
};
...
struct npc_time_riftAI : public ScriptedAI
{
    ...
    void DoSummon()
    {
        ...
        if (m_bIsFirstSummon)
        {
            ...
            // On Heroic Mode if Chrono Lord and Temporus are already killed, we
            // need to summon the replacement
            switch (m_uiRiftNumber)
            {
            case 6:
                uiSpellId = (m_pInstance->GetData(TYPE_CHRONO_LORD) == DONE && !m_bIsRegularMode)
                                ? SPELL_INFINITE_CHRONO_LORD
                                : SPELL_CHRONO_LORD_DEJA;
                break;
            case 12:
                uiSpellId = (m_pInstance->GetData(TYPE_TEMPORUS) == DONE && !m_bIsRegularMode)
                                ? SPELL_INFINITE_TIMEREAVER
                                : SPELL_TEMPORUS;
                break;
            case 18:
        ...
        {
            // Reset the RiftWaveCount if we reached the maximum number of the
            // currentRiftWave is 0
        ...
        default:
            if (m_pInstance)
            {
                if (Creature *pMedivh = m_pInstance->GetSingleCreatureFromStorage(NPC_MEDIVH))
                {
                    ...
                    for (int i = 0; i < 4; i++)
                    {
                        if (afPortalLocation[i].fX == riftX && afPortalLocation[i].fY == riftY &&
                            afPortalLocation[i].fZ == riftZ)
                        {
    ...
    void SummonedCreatureJustDied(Creature *pSummoned) override
    {
        ...
        case NPC_RIFT_LORD_2:
            ...
            // No need to set the data to DONE if there is a new portal spawned
            // already
