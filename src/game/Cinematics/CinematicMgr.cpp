...
void CinematicMgr::BeginCinematic()
{
    ...
    if (itr != sFlyByCameraStore.end())
    {
        ...
        if (camitr != m_cinematicCamera->end())
        {
            ...
            m_CinematicObject = player->SummonCreature(VISUAL_WAYPOINT, pos.x, pos.y, pos.z, 0.0f,
                                                       TEMPSPAWN_TIMED_DESPAWN, 5 * MINUTE * IN_MILLISECONDS);
...
bool CinematicMgr::UpdateCinematicLocation(uint32 /*diff*/)
{
    ...
    if (m_CinematicObject &&
        MaNGOS::IsValidMapCoord(interPosition.x, interPosition.y, interPosition.z, interPosition.o))
