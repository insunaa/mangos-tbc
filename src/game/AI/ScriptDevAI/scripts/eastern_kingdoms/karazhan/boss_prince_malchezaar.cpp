...
struct boss_malchezaarAI : public CombatAI
{
    ...
    void Aggro(Unit * /*who*/) override
    {
        ...
        GetCreatureListWithEntryInGrid(creatureList,m_creature,
        NPC_INFERNAL_RELAY,400.0f);*/
        float z = 0;
    ...
    void JustSummoned(Creature *summoned) override
    {
        ...
        {
            summoned->SetForceAttackingCapability(true); // has to be able to attack even if not selectable or
                                                         // attackable
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case MALCHEZAAR_SHADOW_WORD_PAIN: {
            if (DoCastSpellIfCan(GetActionReadyStatus(MALCHEZAAR_PHASE_2) ? m_creature->GetVictim() : nullptr,
                                 GetActionReadyStatus(MALCHEZAAR_PHASE_2) ? SPELL_SW_PAIN_PHASE1
                                                                          : SPELL_SW_PAIN_PHASE3) == CAST_OK)
    ...
    void OnApply(Aura *aura, bool apply) const override
    {
        ...
        if (apply)
        {
            if (Unit *caster = aura->GetCaster())
            {
                ...
                {
                    ai->m_enfleebleMap[aura->GetTarget()->GetObjectGuid().GetRawValue()] =
                        aura->GetTarget()->GetHealth();
