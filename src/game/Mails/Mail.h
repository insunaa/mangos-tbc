...
class MailSender
{
  public: // Constructors
     ...
     * sending this mail, or the Entry of the non-player object.
...
class MailDraft
{
  ...
  private:
    MailDraft(MailDraft const &);            // trap decl, no body, mail draft must cloned
                                             // only explicitly...
    MailDraft &operator=(MailDraft const &); // trap decl, no body, ...because items clone
                                             // is high price operation
...
struct Mail
{
    ...
    uint32 itemTextId;
    /// flag mark mail that already has items, or already generate none items for
    /// template
    bool has_items;
     ...
     * from template already) or not. HasItems() checks whether the mail contains
     * items or not.
     *
     ...
     *
     */
    void prepareTemplateItems(Player *receiver); ///< called from _LoadMails for generate mailTemplateBase
                                                 ///< items not generated for offline player
};
