...
enum
{
    ...
    // GO_FIRST_GATE            = 187766,           // door between felmyst and
    // eredar twins GO_SECOND_GATE           = 187764,           // door after
    // eredar twins
    GO_MURU_ENTER_GATE = 187990, // muru gates
    GO_MURU_EXIT_GATE = 188118,
    // GO_THIRD_GATE            = 187765,           // door after muru; why
    // another?
...
class instance_sunwell_plateau : public ScriptedInstance, private DialogueHelper, public TimerManager
{
  public:
    ...
    void AddToSpectralRealm(ObjectGuid playerGuid)
    {
        m_spectralRealmPlayers.insert(playerGuid);
    }
    void RemoveFromSpectralRealm(ObjectGuid playerGuid)
    {
        m_spectralRealmPlayers.erase(playerGuid);
    }
    ...
    bool IsBrutallusIntroStarted() const
    {
        return m_brutallusIntroStarted;
    }
    void StartBrutallusIntro()
    {
        m_brutallusIntroStarted = true;
    }
