...
bool AddonHandler::BuildAddonPacket(WorldPacket &Source, WorldPacket &Target)
{
                   ...
                   const_cast<uint8 *>(Source.contents() + CurrentPosition), Source.size() - CurrentPosition) == Z_OK)
    {
        ...
        while (AddOnPacked.rpos() < AddOnPacked.size())
        {
            ...
            // sLog.outDebug("ADDON: Name:%s CRC:%x Unknown1 :%x Unknown2 :%x",
...
for reference if client switch to use server ban state information void
AddonHandler::BuildAddonPacket(WorldPacket& Source, WorldPacket& Target, uint32
Packetoffset)
{
    ByteBuffer AddOnPacked;
    uLongf AddonRealSize;
...
the packed structure
    ...
    AddonRealSize = TempValue;                              // temp value
becouse ZLIB only excepts uLongf

    CurrentPosition = Source->rpos();                       // get the position
of the pointer in the structure

    AddOnPacked.resize(AddonRealSize);                      // resize target
for zlib action
...
CurrentPosition)!= Z_OK)
    {
        bool* AddonAllowed = new bool;                      // handle addon
check and enable-ing
...
extraction while(i != AddOnPacked.size())
        {
            std::string AddonNames;
            ...
            // sLog.outDebug("ADDON:    Name:%s CRC:%x
...
addons are set from Config file *AddonAllowed = m_Addon_Default;            //
Set addon allowed on default value _AddAddon(Addonstr); sLog.outDetail("Found
new Addon, Name:%s CRC:%x Unknown:%x",AddonNames.c_str(), CRCCHECK, unk6);
            }

            if (CRCCHECK == uint64(0x4C1C776D01))          // If addon is
Standard addon CRC
            {
                                                            // value's standard
Addons *Target << uint8(0) << uint8(2) << uint8(1) << uint8(0) << uint32(0);
            }
            else if (*AddonAllowed)                         // if addon is
Custom addons
                                                            // value's enable
addon *Target << uint8(0x00) << uint8(0x01) << uint8(0x00) << uint8(0x01); else
                                                            // value's disable
addom *Target << uint8(0x00) << uint8(0x0) << uint8(0x00) << uint8(0x0);
