...
instance_stratholme::instance_stratholme(Map *map)
    : ScriptedInstance(map), m_baronRunTimer(0), m_barthilasRunTimer(0), m_mindlessSummonTimer(0),
      m_slaughterSquareTimer(0), m_slaughterDoorTimer(0), m_blackGuardsTimer(0), m_auriusSummonTimer(0),
      m_isSlaughterDoorOpen(false), m_yellCounter(0), m_mindlessCount(0), m_postboxesUsed(0), m_silverHandKilled(0),
      m_jarienKilled(false), m_sothosKilled(false)
{
    Initialize();
}
...
void instance_stratholme::OnPlayerEnter(Player *player)
{
    ...
    if (GetData(TYPE_BARON_RUN) == DONE)
        player->SummonCreature(NPC_YSIDA, stratholmeLocation[8].m_fX, stratholmeLocation[8].m_fY,
                               stratholmeLocation[8].m_fZ, stratholmeLocation[8].m_fO, TEMPSPAWN_DEAD_DESPAWN, 0);
    // Baron ultimatum failed: summon Ysida outside the cage dead
    else if (GetData(TYPE_BARON_RUN) == FAIL)
    {
        if (Creature *ysida = player->SummonCreature(NPC_YSIDA, stratholmeLocation[8].m_fX, stratholmeLocation[8].m_fY,
                                                     stratholmeLocation[8].m_fZ, stratholmeLocation[8].m_fO,
...
void instance_stratholme::OnCreatureCreate(Creature *creature)
{
    ...
    case NPC_CRIMSON_BATTLE_MAGE:
        // Store GUID of NPCs in the courtyard to spawn Timmy once they are all
        // dead
        ...
        // Iterate also over all the defense points where those NPCs are possibly
        // spawned
        for (uint8 i = BARRICADE; i <= FIRST_BARRICADES; i++)
        {
            // Do not store - again - GUIDs for Timmy spawn point, they were done
            // previously with a different range
            if (i == TIMMY)
                continue;
...
void instance_stratholme::OnObjectCreate(GameObject *go)
{
    ...
    case GO_GAUNTLET_GATE1:
        // TODO
        // weird, but unless flag is set, client will not respond as expected. DB
        // bug?
...
void instance_stratholme::SetData(uint32 type, uint32 data)
{
    ...
    case TYPE_BARON_RUN:
        ...
        case IN_PROGRESS:
            ...
            if (Creature *pBaron = GetSingleCreatureFromStorage(NPC_BARON))
            {
                ...
                if (Creature *pYsida = pBaron->SummonCreature(NPC_YSIDA, stratholmeLocation[7].m_fX,
                                                              stratholmeLocation[7].m_fY, stratholmeLocation[7].m_fZ,
                                                              stratholmeLocation[7].m_fO, TEMPSPAWN_DEAD_DESPAWN, 0))
    ...
    case TYPE_RAMSTEIN:
        if (data == SPECIAL)
        {
            ...
            {
                m_slaughterSquareTimer = 20 * IN_MILLISECONDS; // TODO - unknown, also possible that
                                                               // this is not the very correct place..
                DoUseDoorOrButton(GO_PORT_GAUNTLET);
            }
            ...
            if (!abomCount)
            {
                // Open the Slaughterhouse door and set a timer to close it after
                // 10 sec to let some time to Ramstein to move out
                ...
                if (Creature *baron = GetSingleCreatureFromStorage(NPC_BARON))
                {
                    ...
                    if (Creature *ramstein = baron->SummonCreature(
                            NPC_RAMSTEIN, stratholmeLocation[2].m_fX, stratholmeLocation[2].m_fY,
                            stratholmeLocation[2].m_fZ, stratholmeLocation[2].m_fO, TEMPSPAWN_DEAD_DESPAWN, 0))
                        ramstein->GetMotionMaster()->MovePoint(0, stratholmeLocation[5].m_fX,
                                                               stratholmeLocation[5].m_fY, stratholmeLocation[5].m_fZ);

                    debug_log("SD2: Instance Stratholme - Slaughter event: Ramstein "
                              "spawned.");
                }
            }
            else
                debug_log("SD2: Instance Stratholme - Slaughter event: %u Abomination "
                          "left to kill.",
                          abomCount);
        }
        ...
        if (data == DONE)
        {
            ...
            // Close the Slaughterhouse door in case it was open again after a
            // wipe on Ramstein
            ...
            if (Creature *baron = GetSingleCreatureFromStorage(NPC_BARON))
            {
                ...
                {
                    float fX, fY, fZ;
                    baron->GetRandomPoint(stratholmeLocation[6].m_fX, stratholmeLocation[6].m_fY,
                                          stratholmeLocation[6].m_fZ, 5.0f, fX, fY, fZ);
                    if (Creature *temp = baron->SummonCreature(NPC_BLACK_GUARD, stratholmeLocation[6].m_fX,
                                                               stratholmeLocation[6].m_fY, stratholmeLocation[6].m_fZ,
                                                               stratholmeLocation[6].m_fO, TEMPSPAWN_DEAD_DESPAWN, 0))
                        m_guardGUIDs.push_back(temp->GetObjectGuid());
                }

                debug_log("SD2: Instance Stratholme - Slaughter event: "
                          "Summoned 5 guards.");
            }
        }
    ...
    case TYPE_BARON:
        ...
        {
            // Players successfully engaged Baron within the time-limit of his
            // ultimatum
            //     Note: UpdateAI() prevents TYPE_BARON_RUN to be marked as
            //     FAILED if the Baron is already engaged (in progress) when the
            //     ultimatum timer expires
            if (m_auiEncounter[TYPE_BARON_RUN] == IN_PROGRESS)
            {
                ...
                if (Creature *ysida = GetSingleCreatureFromStorage(NPC_YSIDA))
                {
                    ...
                    ysida->GetMotionMaster()->MovePoint(0, stratholmeLocation[8].m_fX, stratholmeLocation[8].m_fY,
                                                        stratholmeLocation[8].m_fZ);
            ...
            if (GetData(TYPE_AURIUS) == DONE)
            {
                ...
                // so players can complete the quest and fake his death
    ...
    case TYPE_BARTHILAS_RUN:
        if (data == IN_PROGRESS)
        {
            ...
            if (barthilas && barthilas->IsAlive() && !barthilas->IsInCombat())
            {
                ...
                barthilas->GetMotionMaster()->MovePoint(0, stratholmeLocation[0].m_fX, stratholmeLocation[0].m_fY,
                                                        stratholmeLocation[0].m_fZ);
    ...
    case TYPE_TRUE_MASTERS:
        ...
        return;
    case TYPE_AURIUS:
        // Prevent further players to complete the quest in that instance or
        // autocomplete the follow-up quest. the flag will be set back if event
        // is succeed
        if (data == DONE)
        {
            if (Creature *aurius = GetSingleCreatureFromStorage(NPC_AURIUS))
...
void instance_stratholme::Load(const char *chrIn)
{
    ...
    // Special Treatment for the Ziggurat-Bosses, as otherwise the event couldn't
    // reload
...
void instance_stratholme::OnCreatureDeath(Creature *creature)
{
    ...
    case NPC_MINDLESS_UNDEAD:
        ...
        if (m_undeadGUIDs.empty())
            m_blackGuardsTimer = 60 * IN_MILLISECONDS; // Wait 1 min before sending the Black
                                                       // Guards outside. In Classic, this allows
    ...
    case NPC_CRIMSON_BATTLE_MAGE:
        for (uint8 i = BARRICADE; i <= FIRST_BARRICADES; i++)
        {
            ...
            // If all mobs from a defense group are dead then activate the
            // related defense event
...
void instance_stratholme::OnCreatureRespawn(Creature *creature)
{
    ...
    case NPC_BARTHILAS:
        if (GetData(TYPE_BARTHILAS_RUN) != NOT_STARTED)
            creature->NearTeleportTo(stratholmeLocation[1].m_fX, stratholmeLocation[1].m_fY, stratholmeLocation[1].m_fZ,
                                     stratholmeLocation[1].m_fO);
        break;
    case NPC_JARIEN:
...
void instance_stratholme::ThazudinAcolyteJustDied(Creature *creature)
{
    for (uint8 i = 0; i < MAX_ZIGGURATS; ++i)
    {
        ...
        if (m_zigguratStorage[i].m_zigguratAcolyteGuid.empty())
        {
            ...
            int32 aAnnounceSay[MAX_ZIGGURATS] = {SAY_ANNOUNCE_ZIGGURAT_1, SAY_ANNOUNCE_ZIGGURAT_2,
                                                 SAY_ANNOUNCE_ZIGGURAT_3};
...
void instance_stratholme::DoSpawnScarletGuards(uint8 step, Player *summoner)
{
    ...
    case INNER_BASTION_2:
        ...
        break;
    default:
        return; // avoid indexing the following tables with a wrong index. Should
                // never happen.
    }

    // Spawn the two guards and move them to where they will guard (each side of
    // a door)
    for (uint8 i = 0; i < 2; i++)
    {
        if (Creature *temp = summoner->SummonCreature(npcEntry[i], scarletGuards[index].m_fX, scarletGuards[index].m_fY,
                                                      scarletGuards[index].m_fZ, scarletGuards[index].m_fO,
                                                      TEMPSPAWN_DEAD_DESPAWN, 0))
        {
            temp->SetWalk(false);
            temp->GetMotionMaster()->MovePoint(0, scarletGuards[index + i + 1].m_fX, scarletGuards[index + i + 1].m_fY,
                                               scarletGuards[index + i + 1].m_fZ);
        }
    }
}
...
void instance_stratholme::DoSpawnScourgeInvaders(uint8 step, Player *summoner)
{
    ...
    std::vector<uint32> mobList;              // Vector holding the 5 creatures entries for
                                              // each Scourge invaders group
    ...
    {
    case 0:
        mobEntry = NPC_SKELETAL_GUARDIAN;
        break;
    case 1:
        mobEntry = NPC_SKELETAL_BERSERKER;
        break;
    }
    ...
    case CRIMSON_THRONE:
        ...
        break;
    default:
        return; // avoid indexing the following table with a wrong index. Should
                // never happen.
    }
    ...
    for (uint8 i = 0; i < 5; i++)
    {
        ...
        if (Creature *temp = summoner->SummonCreature(mobList[i], scourgeInvaders[index].m_fX,
                                                      scourgeInvaders[index].m_fY, scourgeInvaders[index].m_fZ,
                                                      scourgeInvaders[index].m_fO, TEMPSPAWN_DEAD_DESPAWN, 0))
        {
            temp->SetWalk(false);
            temp->GetRandomPoint(scourgeInvaders[index + 1].m_fX, scourgeInvaders[index + 1].m_fY,
                                 scourgeInvaders[index + 1].m_fZ, 6.0f, targetPosX, targetPosY, targetPosZ);
...
void instance_stratholme::DoMoveBackDefenders(uint8 step, Creature *creature)
{
    ...
    for (auto guardGUID : m_crimsonDefendersLowGuids[index])
    {
        ...
        if (guard && guard->IsAlive() && !guard->IsInCombat())
        {
            ...
            guard->SetWalk(false);
            guard->GetMotionMaster()->MovePoint(0, scarletLastStand[treshold + foundGuards].m_fX,
                                                scarletLastStand[treshold + foundGuards].m_fY,
                                                scarletLastStand[treshold + foundGuards].m_fZ);
            foundGuards++;
        }
...
void instance_stratholme::DoScarletBastionDefense(uint8 step, Creature *creature)
{
    ...
    case STAIRS:
        ...
        return;
    case TIMMY:
        creature->SummonCreature(NPC_TIMMY_THE_CRUEL, scourgeInvaders[0].m_fX, scourgeInvaders[0].m_fY,
                                 scourgeInvaders[0].m_fZ, scourgeInvaders[0].m_fO, TEMPSPAWN_DEAD_DESPAWN, 0);
        return;
...
{
    // Check if timer was not already set by another player/pet a few
    // milliseconds before
...
{
    // Loop over the two Gate traps, each one has up to three timers (trap reset,
    // gate opening delay, critters spawning delay)
    for (uint8 i = 0; i < 2; i++)
    {
        // Check that the trap is not on cooldown, if so check if player/pet is
        // in range
        ...
        else
        {
            ...
            for (const auto &player : players)
            {
                ...
                {
                    if (!playerSource->IsGameMaster() &&
                        playerSource->IsWithinDist2d(gateTrap[i].m_fX, gateTrap[i].m_fY, 5.5f))
                        DoGateTrap(i);

                    Pet *pet = playerSource->GetPet();
                    if (!playerSource->IsGameMaster() && pet &&
                        pet->IsWithinDist2d(gateTrap[i].m_fX, gateTrap[i].m_fY, 5.5f))
                        DoGateTrap(i);
                }
            }
        }
    ...
    // Timer to send the Black Guard Sentries out of the Slaughterhouse before
    // Baron Rivendare
    if (m_blackGuardsTimer)
    {
        if (m_blackGuardsTimer <= diff)
        {
            ...
            for (auto guardGUID : m_guardGUIDs)
            {
                ...
                {
                    float fX, fY, fZ;
                    guard->GetRandomPoint(stratholmeLocation[5].m_fX, stratholmeLocation[5].m_fY,
                                          stratholmeLocation[5].m_fZ, 10.0f, fX, fY, fZ);
    ...
    // Timer to close the gate of the Slaughterhouse before Baron Rivendare after
    // Ramstein/Black Guards moved out
    ...
    if (m_barthilasRunTimer)
    {
        if (m_barthilasRunTimer <= diff)
        {
            ...
            if (barthilas && barthilas->IsAlive() && !barthilas->IsInCombat())
            {
                ...
                barthilas->NearTeleportTo(stratholmeLocation[1].m_fX, stratholmeLocation[1].m_fY,
                                          stratholmeLocation[1].m_fZ, stratholmeLocation[1].m_fO);
            }
    ...
    if (m_auriusSummonTimer)
    {
        ...
        {
            // Teleport Aurius from the Chapel and spawn it in the Slaughter
            // House to engage Baron
            ...
            if (aurius && aurius->IsAlive() && !aurius->IsInCombat())
            {
                ...
                {
                    float fX, fY, fZ;
                    baron->GetRandomPoint(baron->GetPositionX(), baron->GetPositionY(), baron->GetPositionZ(), 4.0f, fX,
                                          fY, fZ);
    ...
    // Check changes for Baron ultimatum timer only if Baron is not already in
    // combat
    if (m_baronRunTimer && GetData(TYPE_BARON) != IN_PROGRESS)
    {
        ...
        if (m_baronRunTimer <= diff)
        {
            if (GetData(TYPE_BARON_RUN) != FAIL)
            {
                ...
                m_baronRunTimer = 8 * IN_MILLISECONDS; // We reset the timer so the speech of
                                                       // Ysida is not said at the same time
                                                       // than the Baron's one
            }
            else
            {
                ...
                debug_log("SD2: Instance Stratholme: Baron run event reached end. "
                          "Event has state %u.",
    ...
    if (m_mindlessSummonTimer)
    {
        if (m_mindlessCount < 30)
        {
            if (m_mindlessSummonTimer <= diff)
            {
                if (Creature *baron = GetSingleCreatureFromStorage(NPC_BARON))
                {
                    // Summon mindless skeletons and move them to random point in
                    // the center of the square
                    if (Creature *temp = baron->SummonCreature(NPC_MINDLESS_UNDEAD, stratholmeLocation[4].m_fX,
                                                               stratholmeLocation[4].m_fY, stratholmeLocation[4].m_fZ,
                                                               stratholmeLocation[4].m_fO, TEMPSPAWN_DEAD_DESPAWN, 0))
                    {
                        float fX, fY, fZ;
                        baron->GetRandomPoint(stratholmeLocation[5].m_fX, stratholmeLocation[5].m_fY,
                                              stratholmeLocation[5].m_fZ, 20.0f, fX, fY, fZ);
                        temp->SetWalk(false);
    ...
    if (m_slaughterSquareTimer)
    {
        if (m_slaughterSquareTimer <= diff)
        {
            ...
            for (auto abomGUID : m_abominationGUIDs)
            {
                ...
                if (!abom || !abom->IsAlive() ||
                    abom->GetMotionMaster()->GetCurrentMovementGeneratorType() == POINT_MOTION_TYPE)
                    continue;
                ...
                if (!abom->IsInCombat())
                {
                    ...
                    {
                        float fX, fY, fZ;
                        abom->GetRandomPoint(door->GetPositionX(), door->GetPositionY(), door->GetPositionZ(), 10.0f,
                                             fX, fY, fZ);
...
bool instance_stratholme::CheckConditionCriteriaMeet(Player const *player, uint32 instanceConditionId,
                                                     WorldObject const *conditionSource,
                                                     uint32 conditionSourceType) const
{
    if (instanceConditionId == INSTANCE_CONDITION_ID_NORMAL_MODE) // Aurius event during fight
                                                                  // against Baron Rivendare
        return GetData(TYPE_AURIUS) == DONE;
    else
        script_error_log("instance_stratholme::CheckConditionCriteriaMeet called with "
