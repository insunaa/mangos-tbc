...
class MovementGenerator
{
  public:
    ...
    // called after return movement generator to top position (after remove above
    // movement generator)
    ...
    virtual Unit *GetCurrentTarget() const
    {
        return nullptr;
    }
    ...
    virtual bool IsRemovedOnExpire() const
    {
        return false;
    }

    // used for check from Update call is movegen still be active (top movement
    // generator) after some not safe for this calls
...
template <class T, class D> class MovementGeneratorMedium : public MovementGenerator
{
  ...
  public:
    // Will not link if not overridden in the generators - also not generate for
    // T==Unit
