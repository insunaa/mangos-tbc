...
SD%Complete: 80
SDComment: test Threating Gaze. Script depends on ACID script for Vilebranch
Speaker SDCategory: Zul'Gurub EndScriptData */
...
struct boss_mandokirAI : public CombatAI
{
    boss_mandokirAI(Creature *creature)
        : CombatAI(creature, MANDOKIR_ACTION_MAX),
    ...
    void ReceiveAIEvent(AIEventType eventType, Unit * /*sender*/, Unit *invoker, uint32 /*miscValue*/) override
    {
        ...
        else if (eventType == AI_EVENT_CUSTOM_B)
        {
            ...
            if (watchTarget && watchTarget->IsAlive() &&
                m_creature->getThreatManager().getThreat(watchTarget) > m_targetThreat)
            {
...
struct ThreateningGaze : public AuraScript
{
    void OnApply(Aura *aura, bool apply) const override
    {
        if (Unit *caster = aura->GetCaster())
            if (caster->AI())
                caster->AI()->ReceiveAIEvent((apply ? AI_EVENT_CUSTOM_A : AI_EVENT_CUSTOM_B), aura->GetTarget(),
                                             aura->GetTarget(), 0);
    }
};
