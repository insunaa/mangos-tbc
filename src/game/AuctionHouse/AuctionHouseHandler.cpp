...
void WorldSession::HandleAuctionHelloOpcode(WorldPacket &recv_data)
{
    ...
    if (!unit)
    {
        DEBUG_LOG("WORLD: HandleAuctionHelloOpcode - %s not found or you can't "
                  "interact with him.",
...
void WorldSession::SendAuctionOwnerNotification(AuctionEntry *auction, bool sold) const
{
    ...
    data << uint32(auction->Id);
    data << uint32(auction->bid);                // if 0, client shows ERR_AUCTION_EXPIRED_S, else
                                                 // ERR_AUCTION_SOLD_S (works only when guid==0)
    data << uint32(auction->GetAuctionOutBid()); // AuctionOutBid?
    ...
    // error messages as described above if bidder!=0 client updates auctions
    // with new bid, outbid and bidderGuid
...
void WorldSession::HandleAuctionSellItem(WorldPacket &recv_data)
{
    ...
    {
        sLog.outError("AuctionError, %s is sending %s, but item is already in "
                      "another auction",
    ...
    DETAIL_LOG("selling %s to auctioneer %s with initial bid %u with buyout %u "
               "and with time %u (in sec) in auctionhouse %u",
               itemGuid.GetString().c_str(), auctioneerGuid.GetString().c_str(), bid, buyout, etime,
...
void WorldSession::HandleAuctionRemoveItem(WorldPacket &recv_data)
{
    ...
    if (!auction || auction->owner != pl->GetGUIDLow())
    {
        ...
        sLog.outError("CHEATER : %u, he tried to cancel auction (id: %u) of "
    ...
    if (auction->bid) // If we have a bid, we have to send him the money he paid
    {
        ...
        if (pl->GetMoney() < auctionCut) // player doesn't have enough money,
...
void WorldSession::HandleAuctionListBidderItems(WorldPacket &recv_data)
{
    ...
    {
        sLog.outError("Client sent bad opcode!!! with count: %u and size : %u "
                      "(must be: %u)",
                      outbiddedCount, (uint32)recv_data.size(), (16 + outbiddedCount * 4));
        outbiddedCount = 0;
    }
...
void WorldSession::HandleAuctionListItems(WorldPacket &recv_data)
{
    ...
    // DEBUG_LOG("Auctionhouse search %s list from: %u, searchedname: %s,
