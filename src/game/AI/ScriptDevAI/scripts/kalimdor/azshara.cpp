...
bool GossipHello_npc_rizzle_sprysprocket(Player *pPlayer, Creature *pCreature)
{
    if (pPlayer->GetQuestStatus(QUEST_MOONSTONE) == QUEST_STATUS_INCOMPLETE)
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_ITEM_MOONSTONE, GOSSIP_SENDER_MAIN,
    ...
    bool CanUseSpecialAbility(uint32 uiIndex)
    {
        ...
        case TARGET_TYPE_RANDOM:
            pTarget = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0,
                                                        m_aSpitelashAbility[uiIndex].m_uiSpellId, SELECT_FLAG_IN_LOS);
            break;
...
bool GossipHello_npc_loramus_thalipedes(Player *pPlayer, Creature *pCreature)
{
    ...
    if (pPlayer->GetQuestStatus(3141) == QUEST_STATUS_INCOMPLETE)
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Tell me your story", GOSSIP_SENDER_MAIN,
...
bool GossipSelect_npc_loramus_thalipedes(Player *pPlayer, Creature *pCreature, uint32 /*uiSender*/, uint32 uiAction)
{
    ...
    case GOSSIP_ACTION_INFO_DEF + 21:
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "I do not understand", GOSSIP_SENDER_MAIN,
...
{
    npc_felhound_trackerAI(Creature *pCreature) : ScriptedPetAI(pCreature)
    {
        Reset();
    }
    ...
    void DoFindNewCrystal(Player * /*pMaster*/)
    {
        ...
        for (unsigned int i : aGOList)
        {
            ...
            // If a crystal was found in range, stop the search here, else try with
            // another GO
    ...
    if (uiEventId == EVENT_ARCANITE_BUOY)
    {
        if (pSource->GetTypeId() == TYPEID_PLAYER)
        {
                                                      ...
                                                      TEMPSPAWN_MANUAL_DESPAWN, 0))
            {
                ...
                if (GameObject *lightning =
                        GetClosestGameObjectWithEntry((Player *)pSource, GO_THEATRIC_LIGHTNING, 20.0f))
                {
...
struct boss_mawsAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        // Despawn timer handled in UpdateAI because creature must perform actions
        // on despawn that cannot be done in OnDespawn() (creature is already
        // despawned)
