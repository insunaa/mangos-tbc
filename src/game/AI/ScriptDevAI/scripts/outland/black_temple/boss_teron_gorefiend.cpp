...
enum
{
    ...
    // NPC_SHADOWY_CONSTRUCT       = 23111,                 // scripted in
    // eventAI
    // NPC_VENGEFUL_SPIRIT         = 23109,                 // npc controlled by
    // the dead player
};
...
struct boss_teron_gorefiendAI : public CombatAI
{
    boss_teron_gorefiendAI(Creature *creature) : CombatAI(creature, GOREFIEND_ACTION_MAX), m_introDone(false)
    {
        ...
        m_creature->GetCombatManager().SetLeashingCheck(
            [&](Unit *, float x, float y, float z) { return x < 512.8f && y > 402.8f; });
    ...
    void OnSpellCooldownAdded(SpellEntry const *spellInfo) override
    {
        if (spellInfo->Id == SPELL_SHADOW_OF_DEATH)
            ResetCombatAction(GOREFIEND_ACTION_SHADOW_OF_DEATH,
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case GOREFIEND_ACTION_INCINERATE: {
            Unit *target =
                m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, nullptr, SELECT_FLAG_SKIP_TANK);
...
struct npc_doom_blossomAI : public ScriptedAI
{
    npc_doom_blossomAI(Creature *creature) : ScriptedAI(creature)
    {
        AddCustomAction(0, true, [&] {
            if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_SHADOW_BOLT,
                                                                 SELECT_FLAG_PLAYER))
...
struct npc_shadow_constructAI : public ScriptedAI
{
        ...
        : ScriptedAI(creature), m_instance(static_cast<instance_black_temple *>(creature->GetMap()->GetInstanceData()))
    {
        AddCustomAction(1, 0u, [&] {
            ...
            {
                if (Unit *target =
                        teron->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, nullptr, SELECT_FLAG_PLAYER))
                {
...
{
    void OnAbsorb(Aura * /*aura*/, int32 &currentAbsorb, int32 & /*remainingDamage*/, uint32 & /*reflectedSpellId*/,
                  int32 & /*reflectDamage*/, bool &preventedDeath) const override
    {
        preventedDeath = true;
        currentAbsorb = 0;
    }
    ...
    void OnApply(Aura *aura, bool apply) const override
    {
        if (!apply && aura->GetEffIndex() == EFFECT_INDEX_0 && aura->GetTarget()->IsInWorld()) // login safeguard
        {
            ...
            target->CastSpell(nullptr, SPELL_SUMMON_SPIRIT,
                              TRIGGERED_NONE); // Summon Spirit
            target->CastSpell(nullptr, SPELL_SUMMON_SKELETON_1,
                              TRIGGERED_NONE); // Summon Skeleton
            target->CastSpell(nullptr, SPELL_SUMMON_SKELETON_2,
                              TRIGGERED_NONE); // Summon Skeleton
            target->CastSpell(nullptr, SPELL_SUMMON_SKELETON_3,
                              TRIGGERED_NONE); // Summon Skeleton
            target->CastSpell(nullptr, SPELL_SUMMON_SKELETON_4,
                              TRIGGERED_NONE); // Summon Skeleton
            target->CastSpell(nullptr, SPELL_POSSESS_SPIRIT_IMMUNE,
                              TRIGGERED_NONE); // Possess Spirit Immune
            target->CastSpell(nullptr, SPELL_SPIRITUAL_VENGEANCE,
                              TRIGGERED_NONE); // Spiritual Vengeance
        }
    }
};
