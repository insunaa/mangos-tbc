...
class SpawnGroup
{
  public:
    ...
    SpawnGroupEntry const &GetGroupEntry() const
    {
        return m_entry;
    }
    ...
    virtual CreatureGroup *GetCreatureGroup()
    {
        return nullptr;
    }
...
class CreatureGroup : public SpawnGroup
{
  public:
    ...
    FormationData *GetFormationData()
    {
        return m_formationData.get();
    }
    FormationEntrySPtr GetFormationEntry() const
    {
        return m_entry.formationEntry;
    }

    virtual void Update() override;

    virtual CreatureGroup *GetCreatureGroup() override
    {
        return this;
    }
...
class FormationSlotData
{
  public:
    FormationSlotData(
        uint32 slotId, uint32 _ownerDBGuid, CreatureGroup *creatureGrp,
        SpawnGroupFormationSlotType type = SpawnGroupFormationSlotType::SPAWN_GROUP_FORMATION_SLOT_TYPE_STATIC)
        : m_slotId(slotId), m_realOwnerGuid(_ownerDBGuid), m_creatureGroup(creatureGrp), m_slotType(type),
          m_owner(nullptr), m_realAngle(0), m_realDistance(1), m_recomputePosition(true), m_angleVariation(0),
          m_distanceVariation(0), m_maxAngleVariation(0), m_maxDistanceVariation(0), m_angleVariationDest(0),
          m_distanceVariationDest(0), m_canFollow(true)
    {
    }

    uint32 GetSlotId() const
    {
        return m_slotId;
    }
    Unit *GetMaster();
    ...
    {
        m_realAngle = angle;
        m_angleVariation = angle;
    }
    void SetDistance(float distance)
    {
        m_realDistance = distance;
        m_distanceVariation = distance;
    }
    void ResetVariation()
    {
        m_angleVariation = m_realAngle;
        m_distanceVariation = m_realDistance;
    }
    void SetMaxVariation(float angleMax, float distMax)
    {
        m_maxAngleVariation = angleMax;
        m_maxDistanceVariation = distMax;
    }
    void AddPositionVariation(bool now = false);
    void Update();
    bool &GetRecomputePosition()
    {
        return m_recomputePosition;
    }
    ...
    {
        m_canFollow = canFollow;
    }

    CreatureGroup *GetCreatureGroup()
    {
        return m_creatureGroup;
    }
    FormationData *GetFormationData()
    {
        return m_creatureGroup->GetFormationData();
    }
    ...
    uint32 GetRealOwnerGuid() const
    {
        return m_realOwnerGuid;
    }
    SpawnGroupFormationSlotType GetSlotType() const
    {
        return m_slotType;
    }
...
class FormationData
{
  public:
    ...
    bool GetMirrorState() const
    {
        return m_mirrorState;
    }
    void Compact(bool set = true);
    bool SwitchFormation(SpawnGroupFormationType newShape);
    SpawnGroupFormationType GetCurrentShape() const
    {
        return m_currentFormationShape;
    }
    ...
    bool CanUseMMap() const
    {
        return !HaveOption(SPAWN_GROUP_FORMATION_OPTION_FOLLOWERS_WILL_NOT_PATHFIND_TO_LOCATION);
    }
    void Update();

    FormationSlotDataSPtr SetFormationSlot(
  ...
  private:
    ...
    void StartFollower();
    void StopFollower();
    bool HaveOption(SpawGroupFormationOptions const &option, uint32 const &options) const
    {
        return (static_cast<uint32>(options) & option) != 0;
    }
    bool HaveOption(SpawGroupFormationOptions const &option) const
    {
        return (static_cast<uint32>(m_currentOptions) & option) != 0;
    }
