...
void BattleGroundAV::HandleQuestComplete(uint32 questid, Player *player)
{
    ...
    uint32 reputation = 0; // reputation for the whole team (other reputation
                           // must be done in db)
    // TODO add events (including quest not available anymore, next quest
    // availabe, go/npc de/spawning)
    ...
    case BG_AV_QUEST_GUSES_FLEET:
        ...
        if (m_teamQuestStatus[teamIdx][1] == 120)
            DEBUG_LOG("BattleGroundAV: Quest %i completed (need to implement some "
                      "events here",
                      questid);
        break;
    ...
    case BG_AV_QUEST_GALLON_OF_BLOOD:
        m_teamQuestStatus[teamIdx][4] += 4; // there are 2 quests where you can turn in 5 or 1 item.. ( + 4
                                            // cause +1 will be done some lines below)
        reputation = 4;
    ...
    if (reputation)
        RewardReputationToTeam((player->GetTeam() == ALLIANCE) ? BG_AV_FACTION_ID_STORMPIKE
                                                               : BG_AV_FACTION_ID_FROSTWOLF,
...
void BattleGroundAV::Update(uint32 diff)
{
    ...
    // looks for all timers of the nodes and destroy the building (for graveyards
    // the building wont get destroyed, it goes just to the other team
...
void BattleGroundAV::EndBattleGround(Team winner)
{
    ...
    for (uint8 i = 0; i < PVP_TEAM_COUNT; ++i)
    {
        ...
        DEBUG_LOG("BattleGroundAV: Team %u wins the battleground. Controlled "
                  "towers: %u bonus towers honor: %u bonus towers reputation: %u",
...
void BattleGroundAV::ProcessPlayerDestroyedPoint(AVNodeIds node)
{
    ...
    uint32 newState =
        ownerTeam == ALLIANCE ? avNodeWorldStates[node].worldStateAlly : avNodeWorldStates[node].worldStateHorde;
    ...
    else
    {
        ...
        GetBgMap()->GetGraveyardManager().SetGraveYardLinkTeam(avNodeDefaults[node].graveyardId, BG_AV_ZONE_MAIN,
                                                               GetTeamIdByTeamIndex(ownerTeamIdx));
    }
}

// Process mine owner change; mineId = 0 => Irondeep Mine; mineId = 1 =>
// Coldtooth Mine;
void BattleGroundAV::ChangeMineOwner(AVMineIds mineId, PvpTeamIndex newOwnerTeamIdx)
{
    // TODO changing the owner should result in setting respawntime to infinite
    // for current creatures (they should fight the new ones), spawning new mine
    // owners creatures and changing the chest - objects so that the current
    // owning team can use them
...
void BattleGroundAV::PopulateNode(AVNodeIds node)
{
    ...
    if (!isTower && teamIdx != TEAM_INDEX_NEUTRAL)
    {
        ...
        if (m_nodes[node].state == POINT_CONTROLLED) // we can spawn the current owner event
            ...
            SpawnEvent(BG_AV_MAX_NODES + node, m_nodes[node].prevOwner * BG_AV_MAX_GRAVETYPES + graveDefenderType,
                       false);
    }
...
void BattleGroundAV::ProcessPlayerDefendsPoint(Player *player, AVNodeIds node)
{
    ...
    // ToDo: improve this part; The middle graveyard should be handled directly
    // as an assault
    if (m_nodes[node].totalOwner == TEAM_INDEX_NEUTRAL &&
        node == BG_AV_NODES_SNOWFALL_GRAVE) // initial snowfall capture
    {
        ProcessPlayerAssaultsPoint(player, node);
        return;
    }
    ...
    {
        sLog.outError("BattleGroundAV: player defends point which doesn't belong "
                      "to his team %i",
                      node);
        return;
    }
    ...
    uint32 newState = teamIdx == TEAM_INDEX_ALLIANCE ? avNodeWorldStates[node].worldStateAlly
                                                     : avNodeWorldStates[node].worldStateHorde;
...
void BattleGroundAV::ProcessPlayerAssaultsPoint(Player *player, AVNodeIds node)
{
    ...
    if (!isTower)
    {
        // before despawn of GY spirit healer - need to TP players - must be
        // after AssaultNode; opposite team of current player
        if (Creature *spiritHealer = GetClosestCreatureWithEntry(
                player, teamIdx == TEAM_INDEX_ALLIANCE ? BG_NPC_SPIRIT_GUIDE_HORDE : BG_NPC_SPIRIT_GUIDE_ALLIANCE,
                100.f))
...
{
    if (m_nodes[node].prevOwner == TEAM_INDEX_NEUTRAL) // currently only snowfall is supported as neutral
                                                       // node
                                      ...
                                      int32 mineLangId /*= 0*/)
{
    ...
    {
        if (mineLangId)
            SendYell2ToAll(textId, LANG_UNIVERSAL, pHerald,
                           (teamIdx == TEAM_INDEX_ALLIANCE) ? LANG_BG_ALLY : LANG_BG_HORDE, mineLangId);
        else
