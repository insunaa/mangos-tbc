...
enum ItemDynFlags
{
    ...
    ITEM_DYNFLAG_UNLOCKED = 0x00000004, // have meaning only for item with proto->LockId, if not set
                                        // show as "Locked, req. lockpicking N"
    ...
    ITEM_DYNFLAG_READABLE = 0x00000200, // can be open for read, it or item proto pagetText make show
                                        // "Right click to read"
...
class Item : public Object
{
  public:
    ...
    uint32 GetEnchantmentModifier()
    {
        return m_enchantmentModifier;
    }
    void SetEnchantmentModifier(uint32 value)
    {
        m_enchantmentModifier = value;
    }

    void SendTimeUpdate(Player *owner) const;
    void UpdateDuration(Player *owner, uint32 diff);

    // spell charges (negative means that once charges are consumed the item
    // should be deleted)
    ...
    {
        return m_loot && m_lootState != ITEM_LOOT_NONE && m_lootState != ITEM_LOOT_REMOVED;
    }
    bool HasTemporaryLoot() const
    {
        return m_loot && m_lootState == ITEM_LOOT_TEMPORARY;
    }
    ...
    bool IsUsedInSpell() const
    {
        return m_usedInSpell;
    }
    void SetUsedInSpell(bool state)
    {
        m_usedInSpell = state;
    }
