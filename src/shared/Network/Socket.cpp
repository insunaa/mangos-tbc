...
void Socket::OnRead(const boost::system::error_code &error, size_t length)
{
    ...
    // if there is still data to read, increase the buffer size and do so (if
    // necessary)
    ...
    // we must repeat this in case we have read in multiple messages from the
    // client
    while (m_inBuffer->m_readPosition < m_inBuffer->m_writePosition)
    {
        if (!ProcessIncomingData())
        {
            // this errno is set when there is not enough buffer data available
            // to either complete a header, or the packet length specified in the
            // header goes past what we've read.  in this case, we will reset the
    ...
    // at this point, the packet has been read and successfully processed.  reset
    // the buffer.
...
{
    // skip logging this code because it happens whenever anyone disconnects.
    // reduces spam.
...
void Socket::FlushOut()
{
    ...
    // at this point we are guarunteed that there is data to send in the primary
    // buffer.  send it.
...
// if the write state is buffering, this will cancel the running timer, which
// will immediately trigger FlushOut()
...
{
    // we must check this before locking the mutex because the connection will be
    // closed, which leads to a locked mutex being destroyed.  not good!
    if (error)
    {
        OnError(error);
        return;
