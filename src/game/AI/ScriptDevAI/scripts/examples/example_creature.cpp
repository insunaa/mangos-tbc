...
// hostile monsters Its primary purpose is to show just how much you can really
// do with scripts I recommend trying it out on both an agressive NPC and on
// friendly npc

// **** Quick Info ****
// Functions with Handled Function marked above them are functions that are
// called automatically by the core Functions that are marked Custom Function
// are functions I've created to simplify code

enum
{
    // List of text id's. The text is stored in database, also in a localized
    // version (if translation not exist for the textId, default english text
    // will
    // be used)
    SAY_AGGRO = -1999900,
...
// If (and only if) a gossip must be handled within SD2, then it should be
// moved to SD2-database!
...
struct example_creatureAI : public ScriptedAI
{
    ...
    // *** HANDLED FUNCTION ***
    // Update AI is called Every single map update (roughly once every 100ms if a
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (m_uiSpellTwoTimer < uiDiff)
        {
            ...
            if (DoCastSpellIfCan(m_creature, SPELL_TWO) == CAST_OK)
                m_uiSpellTwoTimer = 37 * IN_MILLISECONDS; // Only Update Timer, if we
                                                          // could start casting
        }
        else
        ...
        else if (m_uiPhase > 1)
        {
            ...
            if (m_uiBeserkTimer < uiDiff)
            {
                ...
                if (DoCastSpellIfCan(m_creature->GetVictim(), SPELL_BESERK) == CAST_OK)
                {
                    ...
                    // Cast our beserk spell agian in 12 seconds (if we didn't kill
                    // everyone)
