...
void PlayerbotMgr::SetInitialWorldSettings()
{
    ...
    if (!botConfig.SetSource(_PLAYERBOT_CONFIG))
        sLog.outError("Playerbot: Unable to open configuration file. Database will "
                      "be unaccessible. Configuration values will use default.");
    else
    ...
    if (botConfig.GetIntDefault("ConfVersion", 0) != PLAYERBOT_CONF_VERSION)
        sLog.outError("Playerbot: Configuration file version doesn't match "
                      "expected version. "
...
PlayerbotMgr::PlayerbotMgr(Player *const master) : m_master(master)
{
    ...
    {
        sLog.outError("Playerbot: PlayerbotAI.Collect.DistanceMax higher than "
                      "allowed. Using 100");
    ...
    {
        sLog.outError("Playerbot: PlayerbotAI.Collect.Distance higher than "
                      "PlayerbotAI.Collect.DistanceMax. Using DistanceMax value");
...
void PlayerbotMgr::HandleMasterIncomingPacket(const WorldPacket &packet)
{
    ...
    case CMSG_OFFER_PETITION: {
        ...
        QueryResult *result = CharacterDatabase.PQuery("SELECT * FROM petition_sign WHERE playerguid "
                                                       "= '%u' AND petitionguid = '%u'",
        ...
        CharacterDatabase.PExecute("INSERT INTO petition_sign (ownerguid,petitionguid, playerguid, "
                                   ...
                                   GetMaster()->GetGUIDLow(), petitionLowGuid, player->GetGUIDLow(),
                                   GetMaster()->GetSession()->GetAccountId());
    ...
    case CMSG_ACTIVATETAXI: {
        ...
        // DEBUG_LOG ("[PlayerbotMgr]: HandleMasterIncomingPacket - Received
        // CMSG_ACTIVATETAXI from %d to %d", nodes[0], nodes[1]);
    ...
    case CMSG_ACTIVATETAXIEXPRESS: {
        ...
        // DEBUG_LOG ("[PlayerbotMgr]: HandleMasterIncomingPacket - Received
        // CMSG_ACTIVATETAXIEXPRESS from %d to %d", nodes.front(),
        // nodes.back());
    ...
    // If master inspects one of his bots, give the master useful info in chat
    // window such as inventory that can be equipped
    case CMSG_INSPECT: {
        WorldPacket p(packet);
        p.rpos(0); // reset reader
        ObjectGuid guid;
    ...
    case CMSG_TEXT_EMOTE: {
        ...
        {
        case TEXTEMOTE_BOW: {
            // Buff anyone who bows before me. Useful for players not in
            // bot's group How do I get correct target??? Player* const
            // pPlayer = GetPlayerBot(m_master->GetSelection()); if
            // (pPlayer->GetPlayerbotAI()->GetClassAI())
            ...
            pBot->GetPlayer()->IsBeingTeleported();
            ch.SendSysMessage(out.str().c_str());
            }
            {
            std::ostringstream out;
        ...
        // if master has bot selected then only bot follows, else all bots
        // follow
        case 324:
        case TEXTEMOTE_WAVE: {
            Player *const bot = GetPlayerBot(m_master->GetSelectionGuid());
    ...
    case CMSG_GAMEOBJ_USE: // Used by bots to turn in quest to GameObjects when
                           // also used by master
    {
        WorldPacket p(packet);
        p.rpos(0); // reset reader
        ...
        for (PlayerBotMap::const_iterator it = GetPlayerBotsBegin(); it != GetPlayerBotsEnd(); ++it)
        {
            ...
            // If player and bot are on different maps: then player was
            // teleported by GameObject let's return and let playerbot summon
            // do its job by teleporting bots
    ...
    case CMSG_QUESTGIVER_ACCEPT_QUEST: {
        ...
        // DEBUG_LOG ("[PlayerbotMgr]: HandleMasterIncomingPacket - Received
        // CMSG_QUESTGIVER_ACCEPT_QUEST npc = %s, quest = %u, unk1 = %u",
        // guid.GetString().c_str(), quest, unk1);
        ...
        if (qInfo)
            for (PlayerBotMap::const_iterator it = GetPlayerBotsBegin(); it != GetPlayerBotsEnd(); ++it)
            {
                ...
                else if (!bot->CanTakeQuest(qInfo, false))
                {
                    ...
                    bot->GetPlayerbotAI()->TellMaster("I can't take that quest because it requires that I take "
                                                      "items, "
                                                      "but my bags are full!");
    ...
    case CMSG_QUESTGIVER_COMPLETE_QUEST: {
        ...
        // DEBUG_LOG ("[PlayerbotMgr]: HandleMasterIncomingPacket - Received
        // CMSG_QUESTGIVER_COMPLETE_QUEST npc = %s, quest = %u",
        // npcGUID.GetString().c_str(), quest);
    ...
    case CMSG_GOSSIP_HELLO: {
        ...
        for (PlayerBotMap::const_iterator it = GetPlayerBotsBegin(); it != GetPlayerBotsEnd(); ++it)
        {
            ...
            {
                DEBUG_LOG("[PlayerbotMgr]: HandleMasterIncomingPacket - Received  "
                          "CMSG_GOSSIP_HELLO %s not found or you can't interact "
                          "with him.",
            ...
            for (GossipMenuItemsMap::const_iterator itr = pMenuItemBounds.first; itr != pMenuItemBounds.second; ++itr)
            {
                ...
                case GOSSIP_OPTION_VENDOR: {
                    ...
                    // changed the SellGarbage() function to support
                    // ch.SendSysMessaage()
    ...
    case CMSG_SPIRIT_HEALER_ACTIVATE: {
        // DEBUG_LOG ("[PlayerbotMgr]: HandleMasterIncomingPacket - Received
        // CMSG_SPIRIT_HEALER_ACTIVATE SpiritHealer is resurrecting the Player
        // %s",m_master->GetName());
        for (PlayerBotMap::iterator itr = m_playerBots.begin(); itr != m_playerBots.end(); ++itr)
        {
        ...
        for (auto it = GetPlayerBotsBegin(); it != GetPlayerBotsEnd(); ++it)
        {
            ...
            if (GetMaster()->IsMounted() && !bot->IsMounted())
            {
                ...
                if (!GetMaster()->GetAurasByType(SPELL_AURA_MOUNTED).empty())
                {
                    ...
                    master_speed1 =
                        GetMaster()->GetAurasByType(SPELL_AURA_MOUNTED).front()->GetSpellProto()->EffectBasePoints[1];
                    master_speed2 =
                        GetMaster()->GetAurasByType(SPELL_AURA_MOUNTED).front()->GetSpellProto()->EffectBasePoints[2];

                    // Bot Part
                    // Step 1: find spell in bot spellbook that matches the
                    // speed change from master
                    uint32 spellMount = 0;
                    ...
                    // Step 2: no spell found or cast failed -> search for an
                    // item in inventory (mount) We start with the fastest
                    // mounts as bot will not be able to outrun its master
                    // since it is following him/her
            ...
            else if (!GetMaster()->IsMounted() && bot->IsMounted()) // only execute code if master is
                                                                    // the one who dismounted
            {
...
void PlayerbotMgr::RemoveBots()
{
    for (auto &guid : m_botToRemove)
    {
        ...
        if (bot)
        {
            ...
            m_playerBots.erase(guid);           // deletes bot player ptr inside this
                                                // WorldSession PlayerBotMap
            botWorldSessionPtr->LogoutPlayer(); // this will delete the bot Player
...
void Creature::LoadBotMenu(Player *pPlayer)
{
    ...
    do
    {
        ...
        else
        {
            ...
            // return; create the manager if it doesn't already exist
            ...
            if (pPlayer->GetPlayerbotMgr()->GetPlayerBot(guidlo) == nullptr) // add (if not already in game)
            {
                ...
                word += name;
                word += " as a Bot.";
                pPlayer->GetPlayerMenu()->GetGossipMenu().AddMenuItem((uint8)9, word, guidlo, GOSSIP_OPTION_BOT, word,
                                                                      false);
            }
            else if (pPlayer->GetPlayerbotMgr()->GetPlayerBot(guidlo) != nullptr) // remove (if in game)
            {
                ...
                word += name;
                word += " from duty.";
                pPlayer->GetPlayerMenu()->GetGossipMenu().AddMenuItem((uint8)0, word, guidlo, GOSSIP_OPTION_BOT, word,
                                                                      false);
            }
        }
...
bool ChatHandler::HandlePlayerbotCommand(char *args)
{
    ...
    if (resultchar)
    {
        ...
        if (!(m_session->GetSecurity() > SEC_PLAYER))
            ...
            {
                PSendSysMessage("|cffff0000You cannot summon anymore bots.(Current "
                                "Max: |cffffffff%u)",
                                maxnum);
                SetSentErrorMessage(true);
                delete resultchar;
    ...
    if (resultlvl)
    {
        ...
        if (!(m_session->GetSecurity() > SEC_PLAYER))
        {
            if (charlvl > maxlvl)
            {
                PSendSysMessage("|cffff0000You cannot summon |cffffffff[%s]|cffff0000, "
                                "it's level is too high.(Current Max:lvl |cffffffff%u)",
    ...
    else if (cmdStr == "remove" || cmdStr == "logout")
    {
        if (!mgr->GetPlayerBot(guid))
        {
            PSendSysMessage("|cffff0000Bot can not be removed because bot does not "
                            "exist in world.");
            SetSentErrorMessage(true);
            return false;
        }
