            ...
            pPlayer->GetQuestStatus(QUEST_ERANIKUS_TYRANT_OF_DREAMS) != QUEST_STATUS_COMPLETE)
        {
            if (pInstance->GetData(TYPE_MALFURION) != IN_PROGRESS)
            {
                pPlayer->SummonCreature(NPC_MALFURION, aSunkenTempleLocation[2].m_fX, aSunkenTempleLocation[2].m_fY,
                                        aSunkenTempleLocation[2].m_fZ, aSunkenTempleLocation[2].m_fO,
    ...
    if (pSource->GetTypeId() == TYPEID_PLAYER && pTarget->GetTypeId() == TYPEID_GAMEOBJECT)
    {
        if (instance_sunken_temple *pInstance = (instance_sunken_temple *)((Player *)pSource)->GetInstanceData())
        {
            ...
            else
            {
                ...
                else if (GameObject *pTrap2 = GetClosestGameObjectWithEntry((GameObject *)pTarget, GO_ATALAI_TRAP_2,
                                                                            INTERACTION_DISTANCE))
                    pTrap2->Use((Unit *)pSource);
                else if (GameObject *pTrap3 = GetClosestGameObjectWithEntry((GameObject *)pTarget, GO_ATALAI_TRAP_3,
                                                                            INTERACTION_DISTANCE))
    ...
    void OnEffectExecute(Spell *spell, SpellEffectIndex effIdx) const override
    {
        if (effIdx == EFFECT_INDEX_0)
        {
            if (Unit *caster = spell->GetCaster())
            {
                ...
                // Despawn all evil summoning circles now that Avatar of Hakkar is
                // summmoned
