...
enum ScourgeInvasionSpells
{
    ...
    SPELL_DESPAWNER_OTHER = 28349, // Casted by the NPC "Necropolis health" after getting hit by,
                                   // on the NPC "Necropolis" which destroys itself and the
                                   // Necropolis Object.

    // Necropolis Health
    SPELL_ZAP_NECROPOLIS = 28386, // There are always 3 Necrotic Shards spawns per Necropolis. This
                                  // Spell is castet on the NPC "Necropolis Health" if a Shard dies
                                  // and does 40 Physical damage. NPC "Necropolis Health" has 42
                                  // health. 42 health / 3 Shards = 14 damage. We have set the armor
                                  // value from the NPC "Necropolis Health" to 950 to reduce the
                                  // damage from 40 to 14.

    // Necropolis -> Proxy
    SPELL_COMMUNIQUE_TIMER_NECROPOLIS = 28395,      // Periodically triggers 28373 Communique, Necropolis-to-Proxies
                                                    // every 15 seconds.
    ...
    SPELL_COMMUNIQUE_TIMER_CAMP = 28346,    // Cast on npc_necrotic_shard on spawn? Periodically triggers
                                            // 28345 Communique Trigger every 35 seconds.
    SPELL_COMMUNIQUE_TRIGGER = 28345,       // Triggers 28281 SPELL_COMMUNIQUE_CAMP_TO_RELAY via void
                                            // Spell::EffectDummy.
    SPELL_DAMAGE_CRYSTAL = 28041,           // 100 Damage (Physical). Casted on itself, if
                                            // 16143 (Shadow of Doom) spawns.
    SPELL_SOUL_REVIVAL = 28681,             // Increases all damage caused by 10%.
    SPELL_CAMP_TYPE_GHOST_SKELETON = 28197, // Camp Type, tells the NPC "Scourge Invasion Minion, finder"
                                            // which Camp type the Shard has.
    ...
    SPELL_MINION_SPAWNER_SMALL = 27887,     // Triggers 27885 (Disturb Minion Trap, small) every 5 seconds.
                                            // Activates up to 3 unknown Objects wich spawns the Minions.
    SPELL_MINION_SPAWNER_BUTTRESS = 27888,  // Triggers 27886 (Disturb Minion Trap, Buttress) every 1 hour.
                                            // Activates unknown Objects, They may also spawn the Cultists.
    ...
    SPELL_MINION_DESPAWN_TIMER = 28090, // Triggers 28091 (Despawner, self) every 150 seconds. Triggers
                                        // 17680 SPELL_SPIRIT_SPAWN_OUT via void Spell::EffectDummy.
    SPELL_CONTROLLER_TIMER = 28095,     // Triggers 28091 (Despawner, self) every 60 seconds for 1 hour.
                                        // (Unknown who is casting this).
    ...
    SPELL_BONE_SHARDS = 17014,        // [shortest sniff CD: 16,583 seconds] Used by
                                      // 16299 (Skeletal Shocktrooper).
    SPELL_INFECTED_BITE = 7367,       // [shortest sniff CD: 13,307 seconds] Used by
                                      // 16141 (Ghoul Berserker).
    SPELL_DEMORALIZING_SHOUT = 16244, // [shortest sniff CD: 19,438 seconds] Used
                                      // by 16298 (Spectral Soldier).
    SPELL_SUNDER_ARMOR = 21081,       // [shortest sniff CD: 6,489 seconds] Used by
    ...
    SPELL_RIBBON_OF_SOULS = 16243,               // [shortest sniff CD: 1,638 seconds] Used by
                                                 // 16379 (Spirit of the Damned).
    SPELL_PSYCHIC_SCREAM = 22884,                // or 26042, used by 16379 (Spirit of the Damned).
    SPELL_MINION_DESPAWN_TIMER_UNCOMMON = 28292, // Triggers 28091 (Despawner, self) every 10 minutes. Triggers
                                                 // 17680 SPELL_SPIRIT_SPAWN_OUT via void Spell::EffectDummy.
    SPELL_ARCANE_BOLT = 13748,                   /* 20720 Used by 16380 (Bone Witch).
                                                 https://classicdb.ch/?npc=16380#abilities says 13748 but 20720 is
                                                 the only "Arcane Bolt" whichs requires no mana. Danage is very
                                                 high, so i guess it has a very long cd. Spell description in the
                                                 Bestiary is: Hurls a magical bolt at an enemy, inflicting Arcane
                                                 damage.
                                                 */

    // Cultist Engineer
    SPELL_CREATE_SUMMONER_SHIELD = 28132,    // Summon Object - Temporary (181142),
                                             // Casted exactly the same time with
                                             // 28234 (Minion Spawn-in) on spawn.
    SPELL_BUTTRESS_CHANNEL = 28078,          // Channeled by Cultist Engineer on Damaged
                                             // Necrotic Shard shortly after spawning.
    SPELL_BUTTRESS_TRAP = 28054,             // Unknown.
    SPELL_KILL_SUMMONER_SUMMON_BOSS = 28250, // Reagents, 1 Necrotic Rune

    // Probably used to spawn Shadow of Doom.   Casting sequence (All these [x]
    // spells are being casted the following order within 1-2 seconds):
    ...
    SPELL_QUIET_SUICIDE = 3617,                       // [3] Instakill, casted exactly same time as
                                                      // 31316 (Summon Boss Buff).
    SPELL_SUMMON_BOSS_BUFF = 31316,                   // [4] Summon Boss Buff, casted on Player
    SPELL_SUMMON_BOSS = 31315,                        /* [5] Reagents, 8 Necrotic Rune, Summon (Shadow of Doom) for 1
                                                         hour. The question is: What happens after this hour if the
                                                         Shadow of Doom despawns? Do the cultists respawn and channeling
                                                         again on the damaged shard or Does the Necrotic crystal respawn
                                                         without Cultists or Shadows of Doom?
                                                          */
    ...
    SPELL_DAMAGE_VS_GUARDS = 28364,                // [shortest sniff CD: 11 seconds, longest 81
                                                   // sec] hits 13839 (Royal Dreadguard).
    ...
    The [PH] in the name means it's a placeholder. Blizzard often adds that to
    the names of things they add to the game but haven't finalized. The fact that
    the [PH] is still there means the quest was never finished. (Google)
        ...
        SPELL_PH_GET_TOKEN                              = 27922, // Create Item
    "Necrotic Rune". SPELL_PH_BUTTRESS_ACTIVATOR                     = 28086,
        SPELL_PH_CRYSTAL_CORPSE_DESPAWN                 = 28020,
        SPELL_PH_CRYSTAL_CORPSE_TIMER                   = 28018, // Triggers
    28020
    ([PH] Crystal Corpse Despawn) after 2 hours. SPELL_PH_CYSTAL_BAZOOKA = 27849,
        SPELL_PH_SUMMON_BUTTRESS                        = 28024, // Summon
    (Cultist Engineer) for 1 hour. SPELL_DND_SUMMON_CRYSTAL_MINION_FINDER =
    28227,
    */
};
...
enum ScourgeInvasionNPC
{
    ...
    // https://www.youtube.com/watch?v=c0QjLqHVPRU&t=17s
...
enum ScourgeInvasionMisc
{
    ...
    // Invisible Objects
    GO_BUTTRESS_TRAP = 181112, // [Guessed] Those objects can't be sniffed and
                               // are not available in any database.

    GO_SUMMON_MINION_TRAP_GHOST_GHOUL = 181111,    // Object is not in sniffed files or any database such as
                                                   // WoWHead, but spell 28196 (Create Minion Trap: Ghost/Skeleton)
                                                   // should probably summon them.
...
enum ScourgeInvasionLang
{
    ...
    // Royal Dreadguard
    BCT_UNDERCITY_ROYAL_DREADGUARD_1 = 12337, // The Scourge are at the entrance to the Royal Quarter!  Kill
                                              // them!!
    ...
    BCT_UNDERCITY_SYLVANAS_1 = 12331, // The Scourge attack against my court has been eliminated.  You
                                      // may go about your business.
    BCT_UNDERCITY_SYLVANAS_2 = 12332, // My Royal Dreadguard, you will deal with this matter as befits
                                      // your station.  That, or you will wish that you had.
    ...
    BCT_UNDERCITY_RANDOM_3 = 12357, // Did you see that?  There's a Scourge over
                                    // there! Michael Garrett, Hannah Akeley
    BCT_STORMWIND_RANDOM_3 = 12368, // Did you see that?  There's a Scourge over there! Thomas Miller
    BCT_UNDERCITY_RANDOM_4 = 12359, // There's one of the Scourge, right over
                                    // there! Innkeeper Norman, Michael Garrett
    BCT_STORMWIND_RANDOM_4 = 12370, // There's one of the Scourge, right over there!
    BCT_UNDERCITY_RANDOM_5 = 12357, // Did you see that?  There's a Scourge over
                                    // there! Michael Garrett, Hannah Akeley
    BCT_STORMWIND_RANDOM_5 = 12368, // Did you see that?  There's a Scourge over there! Thomas Miller
    BCT_UNDERCITY_RANDOM_6 = 12361, // Will these unrelenting Scourge attacks never end? Innkeeper
                                    // Norman, William Montague
    BCT_STORMWIND_RANDOM_6 = 12372, // Will these unrelenting Scourge attacks never end?
    BCT_UNDERCITY_RANDOM_7 = 12360, // This has gone too far.  How dare the Scourge attack Undercity!
                                    // Destroy it before more come! Innkeeper Norman
    BCT_STORMWIND_RANDOM_7 = 12371, // This has gone too far.  How dare the Scourge attack Stormwind!
                                    // Destroy it before more come! Stephanie Turner
    BCT_UNDERCITY_RANDOM_8 = 12362, // Destroy the Scourge invader now, before
                                    // it's too late! Michael Garrett
    BCT_STORMWIND_RANDOM_8 = 12373, // Destroy the Scourge invader now, before
                                    // it's too late! Officer Maloof
    BCT_UNDERCITY_RANDOM_9 = 12358, // How can I get anything done with the Scourge running amok in
                                    // here?! Innkeeper Norman
    BCT_STORMWIND_RANDOM_9 = 12369, // How can I get anything done with the
                                    // Scourge running amok around
                                    // here?! Stephanie Turner
    ...
    BCT_STORMWIND_CITY_GUARD_4 = 12316, // The Scourge are at the castle entrance!
                                        // For Stormwind!  For King Anduin!

    // Highlord Bolvar Fordragon?
    BCT_STORMWIND_BOLVAR_1 = 12317, // Hold the line!  Protect the King at all costs!
    BCT_STORMWIND_BOLVAR_2 = 12318, // Good work, one and all!  The Scourge at
                                    // the castle have been defeated.
    ...
    BCT_SHADOW_OF_DOOM_TEXT_0 = 12420,   // Our dark master has noticed your
                                         // trifling, and sends me to bring
                                         // a message... of doom!
    BCT_SHADOW_OF_DOOM_TEXT_1 = 12421,   // These heroics mean nothing, $c.  Your future is sealed and your
                                         // soul is doomed to servitude!
    BCT_SHADOW_OF_DOOM_TEXT_2 = 12422,   // Your battle here is but the smallest mote of a world wide
    // invasion, whelp!  It is time you learned of the powers you face!
    BCT_SHADOW_OF_DOOM_TEXT_3 = 12243,                  // You will not stop our deepening shadow, $c.  Now... join us!
                                                        // Join the ranks of the Chosen!
