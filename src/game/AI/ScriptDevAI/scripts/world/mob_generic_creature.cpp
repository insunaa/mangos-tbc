...
struct generic_creatureAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 diff) override
    {
        ...
        if (m_creature->CanReachWithMeleeAttack(m_creature->GetVictim()))
        {
            ...
            if (m_creature->isAttackReady())
            {
                ...
                if (info)
                    ...
                    info = SelectSpell(m_creature->GetVictim(), -1, -1, SELECT_TARGET_ANY_ENEMY, 0, 0, 0, 0,
                                       SELECT_EFFECT_DONTCARE);

                // 50% chance if elite or higher, 20% chance if not, to replace our
                // white hit with a spell
        ...
        else
        {
            ...
            // No healing spell available, See if we can cast a ranged spell (Range
            // must be greater than ATTACK_DISTANCE)
            if (info)
                Healing = true;
            else
