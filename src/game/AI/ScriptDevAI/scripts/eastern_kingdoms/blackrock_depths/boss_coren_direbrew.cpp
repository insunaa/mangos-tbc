...
SD%Complete: 75
SDComment: Some parts are not complete - requires additional research.
Brewmaidens scripts handled in eventAI. SDCategory: Blackrock Depths
EndScriptData */
...
enum
{
    ...
    // SPELL_HAS_BREW                = 47331,            // triggers 47344 - aura
    // which asks for the second brew on item expire SPELL_SEND_FIRST_MUG =
    // 47333,
    // // triggers 47345
...
struct boss_coren_direbrewAI : public CombatAI
{
    boss_coren_direbrewAI(Creature *creature) : CombatAI(creature, COREN_ACTION_MAX)
    {
        ...
        AddCustomAction(COREN_ATTACK_TIMER, true, [&]() {
            if (Player *player = m_creature->GetMap()->GetPlayer(m_targetPlayer))
            {
                ...
                {
                    creature->SetFactionTemporary(FACTION_HOSTILE, TEMPFACTION_RESTORE_REACH_HOME |
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case COREN_SPAWN_ILSA:
            ...
            {
                m_creature->SummonCreature(NPC_ILSA_DIREBREW, 889.1403f, -130.9002f, -49.660107f, 5.427973747253417968f,
        ...
        case COREN_SPAWN_URSULA:
            ...
            {
                m_creature->SummonCreature(NPC_URSULA_DIREBREW, 894.956f, -127.81718f, -49.659885f,
                                           5.270894527435302734f, TEMPSPAWN_DEAD_DESPAWN, 0);
                SetActionReadyStatus(action, false);
            }
            break;
        case COREN_DISARM:
    ...
    //    SPELL_DIREBREW_CHARGE, SELECT_FLAG_NOT_IN_MELEE_RANGE |
    //    SELECT_FLAG_PLAYER))
    //    {
...
struct SummonMoleMachineTargetPicker : public SpellScript
{
    ...
    void OnEffectExecute(Spell *spell, SpellEffectIndex /*effIdx*/) const override
    {
        spell->GetCaster()->CastSpell(spell->GetUnitTarget(), SPELL_SUMMON_MOLE_MACHINE_MINION_SUMMONER,
