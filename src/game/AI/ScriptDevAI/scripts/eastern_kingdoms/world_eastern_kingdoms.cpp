...
struct world_map_eastern_kingdoms : public ScriptedMap, public TimerManager
{
    world_map_eastern_kingdoms(Map *pMap)
        : ScriptedMap(pMap), m_brewfestEvent(this), m_shadeData({AREAID_GOLDSHIRE, AREAID_KHARANOS, AREAID_BRILL})
    {
                                    ...
                                    WorldObject const *conditionSource, uint32 conditionSourceType) const override
    {
        ...
        script_error_log("world_map_eastern_kingdoms::CheckConditionCriteriaMeet called with "
    ...
    void DoSpawn(uint32 spawnId)
    {
        ...
        if (Creature *pProudwell = GetSingleCreatureFromStorage(NPC_AGENT_PROUDWELL, true))
        {
                                                              ...
                                                              spawnType, despawnTimer))
            {
                ...
                {
                    if (Creature *add = summon->SummonCreature(formationCreatureEntry, summon->GetPositionX(),
                                                               summon->GetPositionY(), summon->GetPositionZ(), 0.0f,
                                                               spawnType, despawnTimer))
...
struct go_infernaling_summoner_portal_hound : public GameObjectAI
{
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (m_uiSummonTimer <= uiDiff)
        {
            ...
            {
                pProudwell->CastSpell(pProudwell, SPELL_SUMMON_INFERNALING_PORTAL_HOUND, TRIGGERED_OLD_TRIGGERED,
                                      nullptr, nullptr, m_go->GetObjectGuid());
            }
...
// does not work currently - need to handle spell 33710 somehow when it
// hits/activates either object 183358 or 183357?
