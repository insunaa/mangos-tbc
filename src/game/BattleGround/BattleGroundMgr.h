...
{
    uint32 lastOnlineTime;     // for tracking and removing offline players from
...
class BattleGroundQueue
{
  ...
  private:
    // mutex that should not allow changing private data, nor allowing to update
    // Queue during private data change.
    std::recursive_mutex m_lock;

    typedef std::map<ObjectGuid, PlayerQueueInfo> QueuedPlayersMap;
    QueuedPlayersMap m_queuedPlayers;

    // we need constant add to begin and constant remove / add from the end,
    // therefore deque suits our problem well
...
class BattleGroundMgr
{
  public:
    ...
    BattleGround *GetBattleGround(uint32 /*instanceId*/,
                                  BattleGroundTypeId /*bgTypeId*/); // there must be uint32 because
                                                                    // MAX_BATTLEGROUND_TYPE_ID means
                                                                    // unknown
    ...
    BattleGroundQueue m_battleGroundQueues[MAX_BATTLEGROUND_QUEUE_TYPES]; // public, because we
                                                                          // need to access
                                                                          // them in BG handler
                                                                          // code
  ...
  private:
    ...
    ClientBattleGroundIdSet m_clientBattleGroundIds[MAX_BATTLEGROUND_TYPE_ID]
                                                   [MAX_BATTLEGROUND_BRACKETS]; // the instanceids
                                                                                // just visible for
                                                                                // the client
