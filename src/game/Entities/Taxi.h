...
typedef uint32 DestID; // Destination entry id (taxi flight master stop as seen
                       // by the game internals)
typedef uint32 Index;  // Index of a flight spline waypoint in a single flight route

/// Route is a struct describing a single taxi flight path between 2
/// destinations
struct Route
{
    ...
    Index nodeStart;  // Real entry index start or a modified junction point for
                      // multi-route flights
    Index nodeEnd;    // Real enrty index end or a modified junction point for
                      // multi-route flights
    uint32 cost;      // Fully pre-calculated cost of this particular route (in
                      // non-persisting multi-route flights)
};

/// Roadmap is a sequence of Routes, the source for building a spline or a
/// series of splines for the entire ride
typedef std::deque<Route> Roadmap;
/// Waypoint is an alias for TaxiPathNodeEntry, a part of the taxi flight
/// spline
typedef TaxiPathNodeEntry const *Waypoint;
/// Map is a container which describes a full proto-spline made out of taxi
/// node entries in a sequential order for a single game level
...
enum TrackerState
{
    ...
    TRACKER_TRANSFER = 4, // Container is waiting for server to apply a new taxi
                          // flight spline after map change
};

/// This is a container representing player's current taxi ride's routes and
/// destinations.
class Tracker
{
  public:
    ...
    {
        return (!m_routes.empty() ? m_routes.front().cost : 0);
    }
    /// Get total cost of the entire ride with all routes included
    uint32 GetCostTotal() const
    {
        return m_cost;
    }
    ...
    /// Get the last known node we must resume from in the current taxi spline in
    /// the case of loading
    size_t GetResumeWaypointIndex() const
    {
        return m_resumeIndex;
    }

    /// Add a taxi flight path to the container in addition to the last one,
    /// returns true if input is correct
    bool AddRoute(TaxiPathEntry const *entry, float discountMulti = 0.0f, bool requireModel = true);
    /// Add a taxi flight path to the container by ID in addition to the last
    /// one, returns true if input is correct
    bool AddRoute(PathID pathID, float discountMulti = 0.0f, bool requireModel = true);
    /// Add a taxi flight path to the container by destinations in addition to
    /// the last one, returns true if input is correct
    bool AddRoute(DestID start, DestID end, float discountMulti = 0.0f, bool requireModel = true);
    /// Replace any existing container contents with a chain of flights defined
    /// by a vector of destinations, returns true if input is correct.
    bool AddRoutes(std::vector<DestID> const &destinations, float discountMulti = 0.0f, bool requireModel = true);
    /// Replace any existing container contents with a chain of flights defined
    /// by a deque of flight path ids, returns true if input is correct.
    bool AddRoutes(std::deque<PathID> const &paths, float discountMulti = 0.0f, bool requireModel = true);
    /// Finalize the container and prepare for the tracking of the flight. Used
    /// when all routes are added. It will be impossible to alter the container
    /// until the flight ends
    bool Prepare(Index nodeResume = 0);
    /// Clear the container by removing all contents. Can't be performed if taxi
    /// flight tracking is currently active, unless forced
    bool Clear(bool forced = false);

    /// Update tick: notify the container about arrival at the taxi spline node,
    /// advances internal node tracker and provides info about changes. Returns
    /// true when current route has changed
    bool UpdateRoute(const TaxiPathNodeEntry *entry, PathID &start, PathID &end);

    /// Try to join two routes together by determining junction points. Returns
    /// true on success.
