...
enum InstanceZA
{
    ...
    NPC_WORLD_TRIGGER = 22515, // used to mark location of drums for Amani'shi
                               // Scouts and Amani'shi Reinforcement spawns
...
class instance_zulaman : public ScriptedInstance
{
  public:
    ...
    bool IsBearPhaseInProgress() const
    {
        return m_isBearPhaseInProgress;
    }
    void SetBearEventProgress(bool bIsInProgress)
    {
        m_isBearPhaseInProgress = bIsInProgress;
    }
    void SendNextBearWave(Unit *pTarget);

    bool IsAkilzonGauntletSpawning() const
    {
        return m_auiEncounter[TYPE_AKILZON_GAUNTLET] == IN_PROGRESS;
    }
    bool IsAkilzonGauntletInProgress() const
    {
        return m_auiEncounter[TYPE_AKILZON_GAUNTLET] != NOT_STARTED;
    }
    ...
    std::vector<uint32> &GetMalacrassAddsEntryList()
    {
        return m_malacrassAddsEntryList;
    }
