...
void WorldSession::HandleSplitItemOpcode(WorldPacket &recv_data)
{
    ...
    // DEBUG_LOG("STORAGE: receive srcbag = %u, srcslot = %u, dstbag = %u,
    // dstslot = %u, count = %u", srcbag, srcslot, dstbag, dstslot, count);
...
void WorldSession::HandleSwapInvItemOpcode(WorldPacket &recv_data)
{
    ...
    // DEBUG_LOG("STORAGE: receive srcslot = %u, dstslot = %u", srcslot,
    // dstslot);

    // prevent attempt swap same item to current position generated by client at
    // special cheating sequence
    if (srcslot == dstslot)
        return;
...
void WorldSession::HandleSwapItem(WorldPacket &recv_data)
{
    ...
    // DEBUG_LOG("STORAGE: receive srcbag = %u, srcslot = %u, dstbag = %u,
    // dstslot = %u", srcbag, srcslot, dstbag, dstslot);
...
void WorldSession::HandleDestroyItemOpcode(WorldPacket &recv_data)
{
    ...
    // DEBUG_LOG("STORAGE: receive bag = %u, slot = %u, count = %u", bag, slot,
    // count);
...
void WorldSession::HandleItemQuerySingleOpcode(WorldPacket &recv_data)
{
    ...
    if (pProto)
    {
        ...
        data << name;
        data << uint8(0x00); // pProto->Name2; // blizz not send name there, just
        ...
        for (const auto &Spell : pProto->Spells)
        {
            ...
            if (spell)
            {
                ...
                // let the database control the sign here.  negative means that
                // the item should be consumed once the charges are consumed.
...
void WorldSession::HandleSellItemOpcode(WorldPacket &recv_data)
{
    ...
    if (!pCreature)
    {
        DEBUG_LOG("WORLD: HandleSellItemOpcode - %s not found or you can't "
                  "interact with him.",
    ...
    for (auto i = 0; i < MAX_ITEM_PROTO_SPELLS; ++i)
    {
        ...
        // if spell charges for this item are negative, it means that the item
        // should be destroyed once the charges are consumed. it also means that
        // the value of this item is relative to how many charges are remaining.
    ...
    if (count < pItem->GetCount()) // need split items
    {
        ...
        if (!pNewItem)
        {
            sLog.outError("WORLD: HandleSellItemOpcode - could not create clone of "
                          "item %u; count = %u",
...
void WorldSession::HandleBuybackItem(WorldPacket &recv_data)
{
    ...
    if (!pCreature)
    {
        DEBUG_LOG("WORLD: HandleBuybackItem - %s not found or you can't interact "
                  "with him.",
                  vendorGuid.GetString().c_str());
...
void WorldSession::SendListInventory(ObjectGuid vendorguid) const
{
    ...
    if (!pCreature)
    {
        DEBUG_LOG("WORLD: SendListInventory - %s not found or you can't interact "
                  "with him.",
                  vendorguid.GetString().c_str());
        _player->SendSellError(SELL_ERR_CANT_FIND_VENDOR, nullptr, ObjectGuid(), 0);
        return;
        ...
        if (crItem)
        {
            ...
            if (pProto)
            {
                if (!_player->IsGameMaster())
                {
                    ...
                    if (crItem->conditionId &&
                        !sObjectMgr.IsConditionSatisfied(crItem->conditionId, _player, pCreature->GetMap(), pCreature,
...
void WorldSession::HandleItemNameQueryOpcode(WorldPacket &recv_data)
{
    ...
    else
    {
        ...
        if (sItemStore.LookupEntry(itemid))
            sLog.outErrorDb("WORLD: CMSG_ITEM_NAME_QUERY for item %u failed (item "
                            "listed in Item.dbc but not exist in DB)",
                            itemid);
        else
            sLog.outError("WORLD: CMSG_ITEM_NAME_QUERY for item %u failed (unknown "
                          "item, not listed in Item.dbc)",
                          itemid);
    }
}
...
void WorldSession::HandleWrapItemOpcode(WorldPacket &recv_data)
{
    ...
    DEBUG_LOG("WRAP: receive gift_bag = %u, gift_slot = %u, item_bag = %u, "
              "item_slot = %u",
    ...
    if (item->GetState() == ITEM_NEW) // save new item, to have alway for `character_gifts` record
                                      // in `item_instance`
    {
