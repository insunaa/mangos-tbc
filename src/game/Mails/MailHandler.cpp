...
void WorldSession::HandleSendMail(WorldPacket &recv_data)
{
    ...
    if (!rc)
    {
        DETAIL_LOG("%s is sending mail to %s (GUID: nonexistent!) with subject %s "
                   "and body %s includes %u items, %u copper and %u COD copper "
                   "with unk1 = %u, unk2 = %u",
                   pl->GetGuidStr().c_str(), receiver.c_str(), subject.c_str(), body.c_str(), items_count, money, COD,
                   unk1, unk2);
        pl->SendMailResult(0, MAIL_SEND, MAIL_ERR_RECIPIENT_NOT_FOUND);
        return;
    }

    DETAIL_LOG("%s is sending mail to %s with subject %s and body %s includes %u "
            ...
            for (uint8 i = 0; i < items_count; ++i)
            {
                ...
                if (GetSecurity() > SEC_PLAYER && sWorld.getConfig(CONFIG_BOOL_GM_LOG_TRADE))
                {
                    sLog.outCommand(GetAccountId(),
                                    ...
                                    GetPlayerName(), GetAccountId(), item->GetProto()->Name1, item->GetEntry(),
                                    item->GetCount(), receiver.c_str(), rc_account);
                }

                pl->MoveItemFromInventory(items[i]->GetBagSlot(), item->GetSlot(), true);
                CharacterDatabase.BeginTransaction();
                item->DeleteFromInventoryDB(); // deletes item from character's
                                               // inventory
                item->SaveToDB();              // recursive and not have transaction guard
                                               // into self, item not in inventory and can be
                                               // save standalone
 ...
 * It will create a new MailDraft and add the items, money, etc. associated
 * with the mail and then send the mail to the original sender.
 *
 * @param recv_data The packet containing information about the mail being
 * returned.
 *
...
void WorldSession::HandleMailReturnToSender(WorldPacket &recv_data)
{
    ...
    if (m->messageType == MAIL_NORMAL && m->sender)
    {
        MailDraft draft;
        if (m->mailTemplateId)
            draft.SetMailTemplate(m->mailTemplateId,
    ...
    if (msg == EQUIP_ERR_OK)
    {
                        ...
                        // them to sender by mail
        {
            ...
            if (GetSecurity() > SEC_PLAYER && sWorld.getConfig(CONFIG_BOOL_GM_LOG_TRADE))
            {
                ...
                sLog.outCommand(GetAccountId(),
                                "GM %s (Account: %u) receive mail item: %s (Entry: %u "
                                "Count: %u) "
                                "and send COD money: %u to player: %s (Account: %u)",
                                GetPlayerName(), GetAccountId(), it->GetProto()->Name1, it->GetEntry(), it->GetCount(),
                                m->COD, sender_name.c_str(), sender_accId);
            }
            else if (!sender)
                sender_accId = sObjectMgr.GetPlayerAccountIdByGUID(sender_guid);
...
void WorldSession::HandleItemTextQuery(WorldPacket &recv_data)
{
    ...
    /// TODO: some check needed, if player has item with guid mailId, or has mail
    /// with id mailId
 ...
 * called. It will create a new item with the text of the mail and store it in
 * the players inventory (if possible).
 *
 */
