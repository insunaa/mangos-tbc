...
class PointMovementGenerator : public MovementGenerator
{
  public:
    PointMovementGenerator(uint32 id, float x, float y, float z, float o, bool generatePath, uint32 forcedMovement,
                           float speed = 0.f, ObjectGuid guid = ObjectGuid(), uint32 relayId = 0)
        : m_x(x), m_y(y), m_z(z), m_o(o), m_speed(speed), m_generatePath(generatePath),
          m_forcedMovement(forcedMovement), m_id(id), m_speedChanged(false), m_guid(guid), m_relayId(relayId)
    {
    }
...
{
  public:
    PointTOLMovementGenerator(uint32 id, float x, float y, float z, bool takeOff, uint32 forcedMovement,
                              float speed = 0)
