...
enum AIEventType
{
    ...
    AI_EVENT_CRITICAL_HEALTH = 1,   // Sender = Hurt Npc, Invoker = DamageDealer -
                                    // Expected to be sent by 10% health
    AI_EVENT_LOST_HEALTH = 2,       // Sender = Hurt Npc, Invoker = DamageDealer -
                                    // Expected to be sent by 50% health
    AI_EVENT_LOST_SOME_HEALTH = 3,  // Sender = Hurt Npc, Invoker = DamageDealer -
                                    // Expected to be sent by 90% health
    AI_EVENT_GOT_FULL_HEALTH = 4,   // Sender = Healed Npc, Invoker = Healer
    AI_EVENT_CUSTOM_EVENTAI_A = 5,  // Sender = Npc that throws custom event, Invoker =
                                    // TARGET_T_ACTION_INVOKER (if exists)
    AI_EVENT_CUSTOM_EVENTAI_B = 6,  // Sender = Npc that throws custom event, Invoker =
                                    // TARGET_T_ACTION_INVOKER (if exists)
    AI_EVENT_GOT_CCED = 7,          // Sender = CCed Npc, Invoker = Caster that CCed
    AI_EVENT_CUSTOM_EVENTAI_C = 8,  // Sender = Npc that throws custom event, Invoker =
                                    // TARGET_T_ACTION_INVOKER (if exists)
    AI_EVENT_CUSTOM_EVENTAI_D = 9,  // Sender = Npc that throws custom event, Invoker =
                                    // TARGET_T_ACTION_INVOKER (if exists)
    AI_EVENT_CUSTOM_EVENTAI_E = 10, // Sender = Npc that throws custom event, Invoker =
                                    // TARGET_T_ACTION_INVOKER (if exists)
    AI_EVENT_CUSTOM_EVENTAI_F = 11, // Sender = Npc that throws custom event, Invoker =
                                    // TARGET_T_ACTION_INVOKER (if exists)
