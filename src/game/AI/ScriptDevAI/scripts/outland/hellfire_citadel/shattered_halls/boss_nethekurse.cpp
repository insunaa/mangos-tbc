...
enum
{
    ...
    SPELL_SHADOW_FISSURE_2 = 30744, // summons 18370 "Wild Shadow Fissure" -
                                    // targets Fel Orc Converts (TAUNT_3)
...
struct boss_grand_warlock_nethekurseAI : public ScriptedAI
{
    ...
    void DoTauntPeons()
    {
        ...
        case 0:
            if (Creature *target =
                    m_creature->GetMap()->GetCreature(m_vFelConverts[urand(0, m_vFelConverts.size() - 1)]))
            {
...
struct mob_fel_orc_convertAI : public ScriptedAI
{
    ...
    void Aggro(Unit * /*pWho*/) override
    {
        ...
        if (m_pInstance)
        {
            ...
            {
                if (boss_grand_warlock_nethekurseAI *pKurseAI =
                        dynamic_cast<boss_grand_warlock_nethekurseAI *>(pKurse->AI()))
