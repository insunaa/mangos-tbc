...
struct npc_anubisath_sentinelAI : public CombatAI
{
    ...
    {
        AddTimerlessCombatAction(ANUBSENTINEL_BERSERK_HP_CHECK,
                                 true); // Soft enrage at 30% HP
        m_assistList.clear();
    }
    ...
    void GetAIInformation(ChatHandler &reader) override
    {
        ...
        if (m_assistList.size() == MAX_BUDDY)
            ...
            reader.PSendSysMessage("Anubisath Sentinel - not correct number of mobs "
                                   "for group found. Number found %u, should be %u",
    ...
    void Aggro(Unit *who) override
    {
        ...
        if (m_assistList.empty())
        {
            ...
            if (m_assistList.size() != MAX_BUDDY)
                script_error_log("npc_anubisath_sentinel for %s found too few/too many "
                                 "buddies, expected %u.",
        ...
        if (!m_myAbility)
        {
            std::shuffle(m_abilities.begin(), m_abilities.end(),
                         *GetRandomGenerator()); // shuffle the abilities, they
                                                 // will be set to the current
                                                 // creature and its siblings
            SetAbility();
    ...
    {
        if (m_myAbility) // Do not set ability if already force set by the first
                         // Anubisath Sentinel to get aggro
            return;

        // This Anubisath Sentinel is the first one to set his ability as it is the
        // one that got initial aggro Pick the first ability in the (randomised)
        // list of abilities
        if (!forced)
            m_myAbility = m_abilities[0];
        // This Anubisath Sentinel is forced to set his ability by the first
        // sentinel to get aggro Check that the requested ability is valid
        else if (std::find(m_abilities.begin(), m_abilities.end(), abilityId) != m_abilities.end())
            m_myAbility = abilityId;
        else
        {
            script_error_log("npc_anubisath_sentinel for %s: request illegal spell "
                             "ID as ability %u.",
    ...
    void OnEffectExecute(Spell *spell, SpellEffectIndex effIdx) const override
    {
        if (effIdx == EFFECT_INDEX_0)
        {
            if (Unit *caster = spell->GetCaster())
            {
                ...
                {
                    caster->CastSpell(unitTarget, spellId,
                                      TRIGGERED_OLD_TRIGGERED); // Cast own ability on target
                    caster->CastSpell(unitTarget, SPELL_HEAL_BRETHEN,
                                      TRIGGERED_OLD_TRIGGERED); // Heal Brethren
                }
            }
        }
    }
...
struct npc_anubisath_defenderAI : public CombatAI
{
    npc_anubisath_defenderAI(Creature *creature)
        : CombatAI(creature, ANUBDEFENDER_ACTION_MAX),
          m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData()))
    {
        AddTimerlessCombatAction(ANUBDEFENDER_HP_CHECK,
                                 true); // Enrage or explode at 10% HP
