...
SD%Complete: 99
SDComment: Correct models used by Spirit Shade for each race/gender combination
are missing (Lethon) / Player summon NYI SDCategory: Emerald Dragon Bosses
EndScriptData */
...
enum
{
    ...
    SPELL_SEEPING_FOG_R = 24813, // Summons NPC 15224 (Dream Fog) that cast auras
                                 // 24777 and 24780 on spawn
...
struct boss_emerald_dragonAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (m_uiSeepingFogTimer < uiDiff)
        {
            ...
            m_uiSeepingFogTimer = urand(120000, 150000); // Rather Guesswork, but one Fog has 2min
...
enum
{
    ...
    SPELL_PUTRID_MUSHROOM = 24904,   // Summons a Putrid Mushroom (GO 180517 with
                                     // trap spell 24871) on killing a player
...
enum
{
    ...
    // SPELL_SUMMON_SPIRIT_SHADE   = 24810,                    // Summon spell
    // was removed in TBC, was using SPELL_EFFECT_SUMMON_DEMON Classic uses same
    // code than TBC and WotLK for consistency
...
struct boss_lethonAI : public boss_emerald_dragonAI
{
    ...
    void Aggro(Unit * /*pWho*/) override
    {
        ...
        // Shadow bolt wirl is a periodic aura which triggers a set of shadowbolts
        // every 2 secs
    ...
    void SpellHitTarget(Unit *pTarget, const SpellEntry *pSpell) override
    {
        ...
        if (pTarget->GetTypeId() == TYPEID_PLAYER && pSpell->Id == SPELL_DRAW_SPIRIT)
        {
            ...
            // player as original caster This is not currently supported by core
            // but this spell's visual should be dependent on player Also possible
            // that this was no problem due to the special way these NPCs had been
            // summoned in classic times
            if (Creature *pSummoned = pTarget->SummonCreature(
                    NPC_SPIRIT_SHADE, 0.0f, 0.0f, 0.0f, pTarget->GetOrientation(), TEMPSPAWN_TIMED_OR_DEAD_DESPAWN,
                    60 * IN_MILLISECONDS, false, 0, 0, false, false, true))
...
struct npc_spirit_shadeAI : public ScriptedAI
{
    ...
    {
        if (m_uiInitialMovementTimer) // Does nothing while movement is not
                                      // initiated in UpdateAI()
            return;
        ...
        if (pWho->GetEntry() == NPC_LETHON)
        {
            if (!m_bHasHealed && pWho->IsWithinDistInMap(m_creature, 3.0f))
            {
                ...
                m_creature->GetMotionMaster()->MovePoint(0, pWho->GetPositionX(), pWho->GetPositionY(),
                                                         pWho->GetPositionZ());
        }
    }
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        // NPC does not start to move when summoned, it waits 4 seconds then
        // removes invisibility and moves towards boss Lethon
...
enum
{
    ...
    SPELL_SUMMON_SHADE_1 = 24841, // Summon one NPC 15302 (Shade of Taerar) that
                                  // cast spells 24839 & 24840
...
enum
{
    ...
    SPELL_SUMMON_DRUIDS = 24796, // Summon NPC 15260 (Demented Druid Spirit) that
                                 // uses spells 6726, 16247 & 24957
};
...
struct boss_ysondreAI : public boss_emerald_dragonAI
{
    ...
    // Summon Druids, one druid per player engaged in combat (actual summon
    // handled in child spell 24795)
    bool DoSpecialDragonAbility()
    {
