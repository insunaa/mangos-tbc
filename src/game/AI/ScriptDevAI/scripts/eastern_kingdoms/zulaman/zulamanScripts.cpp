...
SDName: Zulaman
SD%Complete: 90
SDComment: Forest Frog will turn into different NPC's. Workaround to prevent
new entry from running this script SDCategory: Zul'Aman EndScriptData */
        ...
        if (m_uiEventTimer)
        {
            if (m_uiEventTimer <= diff)
            {
                ...
                case 1:
                    ...
                    break;
                case 2:
                    if (m_creature->GetEntry() != NPC_GUNTER &&
                        m_creature->GetEntry() != NPC_KYREN) // vendors don't kneel?
                ...
                case 3:
                    ...
                    if (m_creature->GetEntry() == NPC_GUNTER || m_creature->GetEntry() == NPC_KYREN)
                        m_uiEventTimer = 5 * MINUTE * IN_MILLISECONDS; // vendors wait for 5 minutes before
                                                                       // running away and despawning
                    else
                        m_uiEventTimer = 6000;
                    break;
                case 4:
...
struct npc_harrison_jones_zaAI : public npc_escortAI
{
    npc_harrison_jones_zaAI(Creature *creature)
        : npc_escortAI(creature), m_instance(static_cast<instance_zulaman *>(creature->GetInstanceData()))
    {
        Reset();
    }
    ...
    void WaypointReached(uint32 pointId) override
    {
        ...
        case 10:
            ...
            m_creature->SetVisibility(VISIBILITY_ON); // even though Harrison is stealthed, players can
                                                      // still see him
            break;
        case 12:
...
bool GossipHello_npc_harrison_jones_za(Player *player, Creature *creature)
{
    ...
    if (pInstance && pInstance->GetData(TYPE_EVENT_RUN) == NOT_STARTED)
        player->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_ID_BEGIN, GOSSIP_SENDER_MAIN,
...
{
    npc_eagle_trash_aggro_triggerAI(Creature *creature)
        : ScriptedAI(creature), m_instance(static_cast<instance_zulaman *>(creature->GetMap()->GetInstanceData()))
    {
