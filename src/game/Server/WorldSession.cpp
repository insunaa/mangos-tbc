...
WorldSession::WorldSession(uint32 id, WorldSocket *sock, AccountTypes sec, uint8 expansion, time_t mute_time,
                           LocaleConstant locale, std::string accountName, uint32 accountFlags, uint32 recruitingFriend,
                           bool isARecruiter)
    : LookingForGroup_auto_join(false), LookingForGroup_auto_add(true), m_muteTime(mute_time),
      m_accountName(accountName), _player(nullptr), m_Socket(sock ? sock->shared<WorldSocket>() : nullptr),
      m_gameBuild(0), m_clientOS(CLIENT_OS_UNKNOWN), m_accountMaxLevel(0), m_lastAnticheatUpdate(0),
      m_anticheat(nullptr), m_localAddress("127.0.0.1"), m_requestSocket(nullptr),
      ...
      m_kickSession(false), m_playerLogout(false), m_playerRecentlyLogout(false),
      ...
      m_pendingTimeSyncRequests(), m_timeSyncNextCounter(0), m_timeSyncTimer(0), m_accountFlags(accountFlags),
...
WorldSession::~WorldSession()
{
    ...
    // marks this session as finalized in the socket which references (BUT DOES
    // NOT OWN) it. this lets the socket handling code know that the socket can
    // be safely deleted
    if (m_Socket)
    {
...
void WorldSession::SetExpansion(uint8 expansion)
{
    ...
    SendAuthOk(); // this is a hack but does what we need - resets expansion
                  // setting in client
}
...
void WorldSession::SendPacket(WorldPacket const &packet, bool forcedSend /*= false*/) const
{
    ...
    {
        // sLog.outDebug("Refused to send %s to %s", packet.GetOpcodeName(),
        // _player ? _player->GetName() : "UKNOWN");
        return;
    }
    ...
    else
    {
        ...
        DETAIL_LOG("Send all time packets count: " UI64FMTD " bytes: " UI64FMTD
                   ...
                   sendPacketCount, sendPacketBytes, float(sendPacketCount) / fullTime,
...
{
    sLog.outError("SESSION: opcode %s (0x%.4X) have unprocessed tail data (read "
                  "stop at " SIZEFMTD " from " SIZEFMTD ")",
...
{
    sLog.outError("WorldSession::Update ByteBufferException occured while "
    ...
    {
        DETAIL_LOG("Disconnecting session [account id %u / address %s] for badly "
                   "formatted packet.",
...
bool WorldSession::Update(uint32 diff)
{
    ...
    {
        // sLog.outError("MOEP: %s (0x%.4X)", packet->GetOpcodeName(),
        // packet->GetOpcode());
        ...
        try
        {
            ...
            case STATUS_LOGGEDIN_OR_RECENTLY_LOGGEDOUT:
                ...
                {
                    LogUnexpectedOpcode(*packet, "the player has not logged in "
                                                 "yet and not recently logout");
                }
                else
            ...
            case STATUS_AUTHED:
                ...
                break;
            case STATUS_NEVER:
                sLog.outError("SESSION: received not allowed opcode %s (0x%.4X)", packet->GetOpcodeName(),
                              packet->GetOpcode());
                break;
            case STATUS_UNHANDLED:
                DEBUG_LOG("SESSION: received not handled opcode %s (0x%.4X)", packet->GetOpcodeName(),
                          packet->GetOpcode());
                break;
            default:
    ...
    // since Playerbots are known to the World obj only by its master's
...
void WorldSession::LogoutPlayer()
{
    ...
    if (_player)
    {
        ...
        sLog.outChar("Account: %d (IP: %s) Logout Character:[%s] (guid: %u)", GetAccountId(),
                     GetRemoteAddress().c_str(), _player->GetName(), _player->GetGUIDLow());
        ...
        if (!_player->m_InstanceValid && !_player->IsGameMaster())
        {
            ...
            // this is a bad place to call for far teleport because we need
            // player to be in world for successful logout maybe we should
            // implement delayed far teleport logout?
        }

        // FG: finish pending transfers after starting the logout
        // this should fix players beeing able to logout and login back with full
        // hp at death position
        ...
        // from world at this stage No SQL injection as AccountID is uint32
        ...
        if (!_player->GetPlayerbotAI())
        {
            ...
            SqlStatement stmt =
                LoginDatabase.CreateStatement(id, "UPDATE account SET active_realm_id = ? WHERE id = ?");
        ...
        SqlStatement stmt =
            CharacterDatabase.CreateStatement(updChars, "UPDATE characters SET online = 0 WHERE guid = ?");
        stmt.PExecute(guid);
#else
...
{
    sLog.outError("SESSION: received opcode %s (0x%.4X) that must be processed "
                  "in WorldSocket::OnRead",
                  recvPacket.GetOpcodeName(), recvPacket.GetOpcode());
}

void WorldSession::Handle_ServerSide(WorldPacket &recvPacket)
{
    sLog.outError("SESSION: received server-side opcode %s (0x%.4X)", recvPacket.GetOpcodeName(),
                  recvPacket.GetOpcode());
}

void WorldSession::Handle_Deprecated(WorldPacket &recvPacket)
{
    sLog.outError("SESSION: received deprecated opcode %s (0x%.4X)", recvPacket.GetOpcodeName(),
                  recvPacket.GetOpcode());
}

void WorldSession::SendAuthWaitQue(uint32 position) const
{
    // these SMSG_AUTH_RESPONSE structure should be used only after at least one
    // WorldSession::SendAuthOk or WorldSession::SendQueued was sent
    if (position == 0)
    {
...
void WorldSession::LoadAccountData(QueryResult *result, uint32 mask)
{
    ...
    do
    {
        ...
        if ((mask & (1 << type)) == 0)
        {
            sLog.outError("Table `%s` have non appropriate for table  account data "
                          "type (%u), ignore.",
                          mask == GLOBAL_CACHE_MASK ? "account_data" : "character_account_data", type);
            continue;
        }
...
void WorldSession::SetAccountData(AccountDataType type, time_t time_, const std::string &data)
{
    if ((1 << type) & GLOBAL_CACHE_MASK)
    {
        ...
        SqlStatement stmt =
            CharacterDatabase.CreateStatement(delId, "DELETE FROM account_data WHERE account=? AND type=?");
...
void WorldSession::SendAccountDataTimes(uint32 /*mask*/)
{
    ...
    // probably needs to be some sort of client unique identifier I was not able
    // to reverse
...
void WorldSession::LoadTutorialsData()
{
    ...
    QueryResult *result = CharacterDatabase.PQuery("SELECT tut0,tut1,tut2,tut3,tut4,tut5,tut6,tut7 "
...
void WorldSession::SaveTutorialsData()
{
    ...
    case TUTORIALDATA_CHANGED: {
        SqlStatement stmt = CharacterDatabase.CreateStatement(
            updTutorial, "UPDATE character_tutorial SET tut0=?, tut1=?, tut2=?, tut3=?, "
                         "tut4=?, "
                         "tut5=?, tut6=?, tut7=? WHERE account = ?");
        for (unsigned int m_Tutorial : m_Tutorials)
    ...
    case TUTORIALDATA_NEW: {
        SqlStatement stmt =
            CharacterDatabase.CreateStatement(insTutorial, "INSERT INTO character_tutorial "
...
// Send chat information about aborted transfer (mostly used by
// Player::SendTransferAbortedByLockstatus())
void WorldSession::SendTransferAborted(uint32 mapid, uint8 reason, uint8 arg) const
{
    ...
    case TRANSFER_ABORT_DIFFICULTY:
        data << uint8(arg);
        break;
    default: // possible not neaded (absent in 0.13, but add at backport for
             // safe)
        data << uint8(0);
        break;
    }
...
void WorldSession::ExecuteOpcode(OpcodeHandler const &opHandle, WorldPacket &packet)
{
    ...
    // do lot steps or call code that can do far teleports in some conditions
    // unexpectedly for generic way work code
    if (_player)
...
void WorldSession::SynchronizeMovement(MovementInfo &movementInfo)
{
    ...
    {
        DETAIL_LOG("The computed movement time using clockDelta is erronous. Using "
                   "fallback instead");
...
void WorldSession::SendAuthOk() const
{
    ...
    packet << uint8(GetExpansion()); // 0 - normal, 1 - TBC. Must be set in
