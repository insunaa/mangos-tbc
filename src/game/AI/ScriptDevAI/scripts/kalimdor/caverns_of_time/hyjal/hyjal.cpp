...
SD%Complete: 100
SDComment: Instance Data Scripts and functions to acquire mobs and set
encounter status for use in various Hyjal Scripts SDCategory: Caverns of Time,
Mount Hyjal EndScriptData */
...
std::array<HyjalWave, MAX_WAVES> InitHyjalWaves()
{
    ...
    // Wave 8: 4 Ghouls, 2 Crypt Fiends, 4 Abominations, 2 Banshees and 2
    // Necromancers
    ...
    // Wave 8: 4 Abomination, 4 Crypt Fiends, 2 Banshee, 2 Necromancer, 4 Fel
    // Stalker
...
std::array<InvasionWave, MAX_INVASION_WAVES> InitInvasionWaves()
{
    ...
    // Alliance Invasion
    // Wave 0: Building Triggers (These creatures do not have any creation
    // packets in sniffs (they only appear as targets of Ghouls), so coordinates
    // are guesswork based on the Ghouls position)
    ...
    // Gargoyles (ToDo: They should cast Gargoyle Strike s.31664 on Building
    // c.18304)
...
// Workaround for possible proc spell 32044 which is even scarcely in sniffs
// unless sendall is done for all auras
...
void instance_mount_hyjal::OnObjectCreate(GameObject *go)
{
    ...
    case GO_ROARING_FLAME: {
        if (go->GetPositionX() < 5360.f) // Alliance Base or Gargoyle Houses
                                         // between ally and horde
...
void instance_mount_hyjal::OnCreatureRespawn(Creature *creature)
{
    ...
    case NPC_STALK: {
        ObjectGuid spawner = creature->GetSpawnerGuid(); // not loaded in map - guid is enough
        if (!spawner)                                    // Static spawns in Scourge base should never grant XP,
                                                         // loot or reputation
        {
    ...
    case NPC_INFERNAL_TARGET:
        ...
        creature->SetCanEnterCombat(false); // on retail they enter combat, likely for some guardian
                                            // purposes for the infernals - on our end they bug out
                                            // friendlies
        break;
...
void instance_mount_hyjal::Load(const char *chrIn)
{
    ...
    for (uint8 i = 0; i < MAX_ENCOUNTER; ++i)
        if (m_encounter[i] == IN_PROGRESS) // Do not load an encounter as
...
void instance_mount_hyjal::SpawnWave(uint32 index, bool setTimer)
{
    ...
    case 26:
    case 35:
        DoUpdateWorldState(WORLD_STATE_MOUNT_HYJAL_WAVES,
                           0); // Boss wave doesnt show as a wave
        break;
    default:
    ...
    for (HyjalWaveMob &mob : m_hyjalWavesData[index].waveMobs)
    {
        ...
        if (index == 0) // The very first wave should never grant XP, loot or
                        // reputation. ToDo: According to a wowhead comment there
                        // should be a cooldown period after you've killed one
                        // boss after which the first wave of the next boss also
                        // won't grant any XP, loot or reputation
        {
...
void instance_mount_hyjal::SpawnInvasionWave(uint32 index, bool setTimer)
{
    ...
    if (Creature *spawner = GetSingleCreatureFromStorage(NPC_HYJAL_DESPAWN_TRIGGER))
    {
        ...
        {
            TempSpawnSettings settings(spawner, mob.mobEntry, mob.x, mob.y, mob.z, mob.ori, TEMPSPAWN_DEAD_DESPAWN, 0,
                                       true, true, GetRandomWavePath(mob.path));
...
void instance_mount_hyjal::RetreatBase(BaseArea index)
{
    ...
    case BASE_HORDE: {
        if (Creature *thrall = GetSingleCreatureFromStorage(NPC_THRALL))
        {
            ...
            thrall->SummonCreature(NPC_JAINA, 5486.305f, -2777.72f, 1496.687f, 1.082104f,
                                   TEMPSPAWN_TIMED_OR_DEAD_DESPAWN, 120000, true, false, 2);
...
void instance_mount_hyjal::ShowChatCommands(ChatHandler *handler)
{
    handler->SendSysMessage("This instance supports the following commands:\n despawnbase (0,1,2), "
                            "spawnbase (0,1,2), spawnwave (0-35), spawnarchimonde, retreatbase "
                            "(0,1), spawnoverrun (0,1,2), despawnoverrun (0,1,2), "
                            "despawnwavespawns, "
                            "spawninfernals (count)");
}
...
void instance_mount_hyjal::ExecuteChatCommand(ChatHandler *handler, char *args)
{
    ...
    if (val == "despawnbase")
    {
        ...
        {
            handler->PSendSysMessage("Could not despawn base %u because it doesn't "
                                     "exist. Please specify a value between 0-2",
                                     baseId);
            return;
        }
    ...
    else if (val == "spawnbase")
    {
        ...
        if (baseId > MAX_BASE - 1)
        {
            handler->PSendSysMessage("Could not spawn base %u because it doesn't "
                                     "exist. Please specify a value between 0-2",
                                     baseId);
            return;
        }
    ...
    else if (val == "spawnwave")
    {
        ...
        {
            handler->PSendSysMessage("Could not spawn wave %u because it doesn't "
                                     "exist. Please specify a value between 0-35",
                                     waveId);
            return;
        }
    ...
    else if (val == "retreatbase")
    {
        ...
        {
            handler->PSendSysMessage("Could not retreat base %u because it doesn't "
                                     "exist. Please specify a value between 0-1",
                                     baseId);
            return;
        }
    ...
    else if (val == "spawnoverrun")
    {
        ...
        if (baseId > MAX_BASE - 1)
        {
            handler->PSendSysMessage("Could not spawn overrun waves in base %u "
                                     "because it doesn't exist. "
                                     "Please specify a value between 0-2",
                                     baseId);
            return;
        }
    ...
    else if (val == "despawnoverrun")
    {
        ...
        if (baseId > MAX_BASE - 1)
        {
            handler->PSendSysMessage("Could not despawn overrun waves in base %u because it doesn't "
                                     "exist. Please specify a value between 0-2",
                                     baseId);
            return;
        }
    ...
    else if (val == "spawninfernals")
    {
        ...
        if (count > 100)
        {
            handler->PSendSysMessage("Spawn Infernals received too high number. "
                                                      ...
                                                      uint32 conditionSourceType) const
{
    ...
    case INSTANCE_CONDITION_ID_AZGALOR_STARTABLE:
        return GetData(TYPE_KAZROGAL) == DONE &&
               (GetData(TYPE_AZGALOR) == NOT_STARTED || GetData(TYPE_AZGALOR) == FAIL) && m_waveSpawns.size() <= 0;
    ...
    script_error_log("instance_mount_hyjal::CheckConditionCriteriaMeet called with "
                     "unsupported Id %u. Called with param plr %s, src %s, condition source "
                     "type %u",
