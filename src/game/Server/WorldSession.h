...
class PacketFilter
{
  public:
    ...
    virtual bool Process(WorldPacket const & /*packet*/) const
    {
        return true;
    }
...
class WorldSession
{
  ...
  public:
    ...
    // Set this session have no attached socket but keep it alive for short
    // period of time to permit a possible reconnection
    void SetOffline();
    void SetOnline();
    ...
    {
        return m_sessionState == WORLD_SESSION_STATE_OFFLINE;
    }
    WorldSessionState GetState() const
    {
    ...
    bool IsInitialZoneUpdated()
    {
        return m_initialZoneUpdated;
    }
    ...
    {
        return (m_accountFlags & flags) != 0;
    }
    void AddAccountFlag(uint32 flags)
    {
        m_accountFlags |= flags;
    }
    void RemoveAccountFlag(uint32 flags)
    {
        m_accountFlags &= ~flags;
    }
    ...
    {
        return m_Socket ? m_Socket->GetRemoteAddress() : "disconnected/bot";
    }
#else
    const std::string GetRemoteAddress() const
    {
        return m_Socket ? m_Socket->GetRemoteAddress() : "disconnected";
    }
#endif
    ...
    SessionAnticheatInterface *GetAnticheat() const
    {
        return m_anticheat.get();
    }
    ...
    void LogoutPlayer();
    void KickPlayer(bool save = false,
                    bool inPlace = false); // inplace variable needed for shutdown
    ...
    void BuildListAuctionItems(std::vector<AuctionEntry *> const &auctions, WorldPacket &data,
                               ...
                               uint32 usable, uint32 inventoryType, uint32 itemClass, uint32 itemSubClass,
                               uint32 quality, uint32 &count, uint32 &totalcount, bool isFull) const;
    ...
    // Looking For Group
    // TRUE values set by client sending CMSG_LFG_SET_AUTOJOIN and
    // CMSG_LFM_CLEAR_AUTOFILL before player login
    ...
    ClientOSType GetOS() const
    {
        return m_clientOS;
    }
    void SetOS(ClientOSType os)
    {
        m_clientOS = os;
    }
    uint32 GetGameBuild() const
    {
        return m_gameBuild;
    }
    void SetGameBuild(uint32 version)
    {
        m_gameBuild = version;
    }
    uint32 GetAccountMaxLevel() const
    {
        return m_accountMaxLevel;
    }
    void SetAccountMaxLevel(uint32 level)
    {
        m_accountMaxLevel = level;
    }
    uint32 GetOrderCounter() const
    {
        return m_orderCounter;
    }
    ...
    uint32 GetRecruitingFriendId() const
    {
        return m_recruitingFriendId;
    }
    ...
    // Thread safety
    uint32 GetCurrentPlayerLevel()
    {
        return m_currentPlayerLevel;
    }
    void SetCurrentPlayerLevel(uint32 level)
    {
    ...
    Messager<WorldSession> &GetMessager()
    {
        return m_messager;
    }
  ...
  private:
    ...
    uint32 m_GUIDLow; // set logged or recently logout player (while
                      // m_playerRecentlyLogout set)
    Player *_player;
    std::shared_ptr<WorldSocket> m_Socket;        // socket pointer is owned by the
                                                  // network thread which created it
    std::shared_ptr<WorldSocket> m_requestSocket; // a new socket for this session is requested (double
                                                  // connection)
    ...
    // True when the player is in the process of logging out
    // (WorldSession::LogoutPlayer is currently executing)
    bool m_playerLogout;
    ...
    boost::circular_buffer<std::pair<int64, uint32>>
        m_timeSyncClockDeltaQueue; // first member: clockDelta. Second member:
                                   // latency of the packet exchange that was
                                   // used to compute that clockDelta.
    int64 m_timeSyncClockDelta;
    void ComputeNewClockDelta();

    std::map<uint32, uint32> m_pendingTimeSyncRequests; // key: counter. value: server time when
                                                        // packet with that counter was sent.
    uint32 m_timeSyncNextCounter;
    uint32 m_timeSyncTimer;

    std::set<ObjectGuid> m_offlineNameQueries;                     // for name queires made when not logged in
                                                                   // (character selection screen)
    std::deque<CharacterNameQueryResponse> m_offlineNameResponses; // for responses to name queries made when not
                                                                   // logged in
