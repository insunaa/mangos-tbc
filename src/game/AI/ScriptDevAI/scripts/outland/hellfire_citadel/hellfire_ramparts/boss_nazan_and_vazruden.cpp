...
// This npc will morph into the "unmounted" dragon (nazan) after vazruden is
// summoned and continue flying
struct boss_vazruden_heraldAI : public CombatAI
{
    boss_vazruden_heraldAI(Creature *creature)
        : CombatAI(creature, NAZAN_ACTION_MAX),
    ...
    void DoMoveToCenter()
    {
        ...
        // catmulrom in sniff
        m_creature->GetMotionMaster()->MovePoint(POINT_ID_CENTER, landingPosition.x, landingPosition.y,
                                                 landingPosition.z);
    }
    ...
    void DoMoveToCombat()
    {
        ...
        m_creature->GetMotionMaster()->MovePoint(POINT_ID_LANDING, landingPhasePos.x, landingPhasePos.y,
                                                 landingPhasePos.z);
        SetCombatScriptStatus(true);
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case NAZAN_FIREBALL_FLIGHT: {
            ...
            {
                if (Unit *target =
                        vazruden->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, nullptr, SELECT_FLAG_PLAYER))
                {
                    if (DoCastSpellIfCan(target, m_inRegularMode ? SPELL_FIREBALL_FLIGHT : SPELL_FIREBALL_FLIGHT_H) ==
                        CAST_OK)
        ...
        case NAZAN_FIREBALL_GROUND: {
            ...
            {
                if (DoCastSpellIfCan(target, m_inRegularMode ? SPELL_FIREBALL_GROUND : SPELL_FIREBALL_GROUND_H) ==
                    CAST_OK)
        ...
        case NAZAN_CONE_OF_FIRE: {
            if (DoCastSpellIfCan(m_creature->GetVictim(),
                                 m_inRegularMode ? SPELL_CONE_OF_FIRE : SPELL_CONE_OF_FIRE_H) == CAST_OK)
                ResetCombatAction(action, urand(7300, 13200));
            break;
        }
