...
enum
{
    ...
    SPELL_RANDOM_TARGET = 41951, // Serverside in tbc/wotlk - sometime later
                                 // changed to Fixate and visible in client

    // NPC Volcano Spells
    SPELL_VOLCANIC_ERUPTION_SELF = 40117, // Applied to Volcano upon spawn - Cast
                                          // on self - casts 40118 & 42055
    SPELL_TARGET_RANDOM_LOCATION = 40118, // Used to select a random location within 15.00 yards around the
                                          // volcano - DUMMY - Is the visual
    SPELL_VOLCANO_AOE = 42052,            // AOE Damage used by NPC - 15yard radius -
                                          // TARGET_ALL_ENEMY_IN_AREA - Applied by 42055
...
struct boss_supremusAI : public CombatAI
{
    boss_supremusAI(Creature *creature)
        : CombatAI(creature, SUPREMUS_ACTION_MAX),
          m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData()))
    {
        ...
        AddCustomAction(SUPREMUS_ACTION_DELAY, true, [&] {
            ...
            if (!m_bTankPhase)
                ResetTimer(SUPREMUS_ACTION_SWITCH_TARGET,
                           0); // switch target immediately
...
struct SupremusRandomTarget : public AuraScript
{
    void OnApply(Aura *aura, bool apply) const override
    {
        ...
        if (apply)
        {
            ...
            caster->AI()->AttackStart(aura->GetTarget()); // visual change for immediate crosshair
                                                          // update - only safe with fixate
...
struct HatefulStrikePrimer : public SpellScript
{
    void OnEffectExecute(Spell *spell, SpellEffectIndex effIdx) const override
    {
        ...
        caster->CastSpell(target, SPELL_HATEFUL_STRIKE,
                          TRIGGERED_IGNORE_GCD | TRIGGERED_IGNORE_CURRENT_CASTED_SPELL | TRIGGERED_NORMAL_COMBAT_CAST);
    }
};
