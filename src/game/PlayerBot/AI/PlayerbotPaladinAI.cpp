...
PlayerbotPaladinAI::PlayerbotPaladinAI(Player &master, Player &bot, PlayerbotAI &ai) : PlayerbotClassAI(master, bot, ai)
{
    ...
    // m_ai.initSpell(PriestSpells::PRAYER_OF_SHADOW_PROTECTION_1);
}
...
CombatManeuverReturns PlayerbotPaladinAI::DoNextCombatManeuverPVE(Unit *pTarget)
{
    ...
    // Use Divine Protection if Divine Shield is not available and bot is not
    // tanking because of the pacify effect
        ...
        !m_ai.IsNeutralized(newTarget)) // TODO: && party has a tank
    {
        ...
        if (m_ai.IsElite(newTarget))
        {
            ...
            {
                if (DIVINE_SHIELD > 0 && m_bot.IsSpellReady(DIVINE_SHIELD) &&
    ...
    case PALADIN_SPEC_PROTECTION:
        ...
        if (CONSECRATION > 0 && m_bot.IsSpellReady(CONSECRATION) &&
            m_ai.CastSpell(CONSECRATION, *pTarget) == SPELL_CAST_OK)
            return RETURN_CONTINUE;
...
CombatManeuverReturns PlayerbotPaladinAI::HealPlayer(Player *target)
{
    ...
    // Target is a moderately wounded healer or a badly wounded not tank?
...
void PlayerbotPaladinAI::CheckAuras()
{
    ...
    else if (m_ai.GetCombatOrder() & PlayerbotAI::ORDERS_RESIST_SHADOW)
    {
        ...
        if (!m_bot.HasAura(SHADOW_RESISTANCE_AURA) && SHADOW_RESISTANCE_AURA > 0 &&
            !m_bot.HasAura(SHADOW_RESISTANCE_AURA)) // /*&&
...
// TODO: handle other paladins in group/raid, for example to cast
...
bool PlayerbotPaladinAI::CheckSealAndJudgement(Unit *target)
{
    ...
    if (target->GetTypeId() == TYPEID_UNIT)
    {
        ...
        // Prevent low health humanoid from fleeing by judging them with Seal of
        // Justice
    ...
    // Bot already defined a seal and a judgement and each is active on bot and
    // target: don't waste time to go further
    ...
    // Refresh judgement if needed by forcing paladin bot to cast seal and
    // judgement anew But first, unleash current seal if bot can do extra damage
    ...
    if (m_CurrentJudgement == 0 && m_CurrentSeal > 0 && m_bot.HasAura(m_CurrentSeal, EFFECT_INDEX_0))
    {
            ...
            m_ai.CastSpell(JUDGEMENT, *target) == SPELL_CAST_OK)
        {
            ...
            // Set current seal to 0 to force the bot to seal him/herself for
            // combat now that the target is judged
            m_CurrentSeal = 0;
            return true;
        }
    ...
    // Target already judged: bot will buff him/herself for combat according to
    // spec/orders
    uint32 spec = m_bot.GetSpec();
...
void PlayerbotPaladinAI::DoNonCombatActions()
{
    ...
    // buff group
    if (Buff(&PlayerbotPaladinAI::BuffHelper, 1) & RETURN_CONTINUE) // Paladin's BuffHelper takes care of choosing the
                                                                    // specific Blessing so just pass along a non-zero
                                                                    // value
        return;
...
/**
 * BuffHelper
 * BuffHelper is a static function, takes an AI, spellId (ignored for paladin)
 * and a target and attempts to buff them as well as their pets as best as
 * possible.
 *
...
bool PlayerbotPaladinAI::CastHoTOnTank()
{
    ...
    // Paladin: Sheath of Light (with talents), Flash of Light (with Infusion of
    // Light talent and only on a target with the Sacred Shield buff),
