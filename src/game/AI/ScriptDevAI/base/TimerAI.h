...
class CombatActions : public TimerManager
{
  public:
    ...
    // Adds a combat action which is reset to a random number between min and max
    // (inclusive)
    void AddCombatAction(uint32 id, uint32 timerMin, uint32 timerMax);
    // Adds a combat action which has no timer. It is reset to default value at
    // start. Useful for one-off actions like phase transition at HP level.
    ...
    inline void SetActionReadyStatus(uint32 index, bool state)
    {
        m_actionReadyStatus[index] = state;
    }
    inline bool GetActionReadyStatus(uint32 index)
    {
        return m_actionReadyStatus[index];
    }
    ...
    size_t GetCombatActionCount()
    {
        return m_actionReadyStatus.size();
    }
