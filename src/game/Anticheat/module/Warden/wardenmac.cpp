    ...
    : _fingerprintSaved(false), Warden(session, sWardenModuleMgr.GetMacModule(), K, anticheat)
{
    ...
    // the only known capability of the Mac Warden module is hashing a string and
    // sending back the hashed value so at least we can make the string that we
    // ask it to hash change by account, I guess...
