...
class BattleGroundScore
{
  public:
    BattleGroundScore()
        : killingBlows(0), deaths(0), honorableKills(0), bonusHonor(0), damageDone(0), healingDone(0), Team(ALLIANCE)
    {
    }
...
class BattleGround
{
  ...
  public:
    ...
    virtual void Update(uint32 diff); // must be implemented in BG subclass of BG specific update
                                      // code, but must in begginning call parent version
    ...
    {
        return m_bgMap ? GetBgMap()->GetInstanceId() : 0;
    }
    ...
    BattleGroundWinner GetWinner() const
    {
        return m_winner;
    }
    ...
    void SetArenaType(ArenaType type)
    {
        m_arenaType = type;
    }
    void SetArenaorBGType(bool isArena)
    {
        m_isArena = isArena;
    }
    void SetWinner(BattleGroundWinner winner)
    {
        m_winner = winner;
    }
    ...
    {
        m_startMaxDist = startMaxDist;
    }
    float GetStartMaxDist() const
    {
        return m_startMaxDist;
    }
    ...
    // Functions that handle battleground announcements; Used usually when
    // players capture battleground objectives
    ...
    void SendMessage2ToAll(int32 /*entry*/, ChatMsg /*type*/, Player const * /*source*/, int32 arg1 = 0,
                           int32 arg2 = 0);

    // Functions that handle creature yells in battleground; Used specifically in
    // Alterac Valley
    ...
    virtual bool HandleEvent(uint32 /*eventId*/, GameObject * /*go*/, Unit * /*invoker*/)
    {
        return false;
    }
  ...
  private:
    ...
    bool m_arenaBuffSpawned;   // to cache if arenabuff event is started (cause
                               // bool is faster than checking IsActiveEvent)
    bool m_hasBgFreeSlotQueue; // used to make sure that BG is only once inserted
                               // into the
