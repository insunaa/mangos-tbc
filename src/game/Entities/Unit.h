...
enum UnitMiscFlags
{
    ...
    UNIT_BYTE1_FLAG_FLY_ANIM = 0x02, // Creature that can fly and are not on the ground appear to have
                                     // this flag. If they are on the ground, flag is not present.
...
// byte flags value (UNIT_FIELD_BYTES_2,3)                  See enum
// ShapeshiftForm in SharedDefines.h
...
enum AuraRemoveMode
{
    ...
    AURA_REMOVE_BY_TRACKING,     // aura is removed because of a conflicting tracked
                                 // aura
...
enum DeathState
{
    ...
    DEAD = 3,        // for creature despawned state (corpse despawned), for player
                     // CORPSE/DEAD not clear way switches (FIXME), and use m_deathtimer
...
enum UnitState
{
    ...
    UNIT_STAT_FEIGN_DEATH = 0x00000004,     // Unit::SetFeignDeath - a successful
                                            // feign death is currently active
    ...
    UNIT_STAT_POSSESSED = 0x00000040,       // Aura::HandleAuraModPossess (duplicates
                                            // UNIT_FLAG_POSSESSED)
    ...
    UNIT_STAT_ROAMING = 0x00008000,       // Point/Retreat/Stay/Wander/Waypoint/Effect
                                          // MovementGenerator on stack
    ...
    // stay or scripted movement for effect( = in player case you can't move by
    // client command)
...
/// internal used flags for marking special auras - for example some
/// dummy-auras
enum UnitAuraFlags
{
...
enum UnitVisibility
{
    ...
    VISIBILITY_GROUP_INVISIBILITY = 3, // invisibility, can see and can be seen only another invisible unit
                                       // or invisible detection unit, set only if not stealthed, and in
                                       // checks not used (mask used instead)
    VISIBILITY_GROUP_NO_DETECT = 4,    // state just at stealth apply for update Grid state. Don't remove,
                                       // otherwise stealth spells will break
    VISIBILITY_REMOVE_CORPSE = 5       // special totally not detectable visibility for
                                       // force delete object while removing a corpse
};

// Value masks for UNIT_FIELD_FLAGS
enum UnitFlags
{
    UNIT_FLAG_UNK_0 = 0x00000001,               // Movement checks disabled, likely paired with
                                                // loss of client control packet. We use it to
                                                // add custom cliffwalking to GM mode until
                                                // actual usecases will be known.
    UNIT_FLAG_SPAWNING = 0x00000002,            // Spawning in - other uses are wrong
    UNIT_FLAG_CLIENT_CONTROL_LOST = 0x00000004, // Generic unspecified loss of control initiated by server
                                                // script, movement checks disabled, paired with loss of
                                                // client control packet.
    UNIT_FLAG_PLAYER_CONTROLLED = 0x00000008,   // players, pets, totems, guardians, companions, charms, any
                                                // units associated with players
    UNIT_FLAG_RENAME = 0x00000010,
    UNIT_FLAG_PREPARATION = 0x00000020, // don't take reagents for spells with
                                        // SPELL_ATTR_EX5_NO_REAGENT_WHILE_PREP
    UNIT_FLAG_UNK_6 = 0x00000040,
    UNIT_FLAG_NOT_ATTACKABLE_1 = 0x00000080, // ?? (UNIT_FLAG_PVP_ATTACKABLE | UNIT_FLAG_NOT_ATTACKABLE_1)
    ...
    UNIT_FLAG_SWIMMING = 0x00008000,         // controls water swimming animation - TODO:
                                             // confirm whether dynamic or static
    UNIT_FLAG_NON_ATTACKABLE_2 = 0x00010000, // removes attackable icon, if on yourself, cannot assist
                                             // self but can cast TARGET_UNIT_CASTER spells - added by
                                             // SPELL_AURA_MOD_UNATTACKABLE
    UNIT_FLAG_PACIFIED = 0x00020000,
    UNIT_FLAG_STUNNED = 0x00040000, // Unit is a subject to stun, turn and strafe
                                    // movement disabled
    UNIT_FLAG_IN_COMBAT = 0x00080000,
    UNIT_FLAG_TAXI_FLIGHT = 0x00100000, // Unit is on taxi, paired with a duplicate loss of client
                                        // control packet (likely a legacy serverside hack). Disables
                                        // any spellcasts not allowed in taxi flight client-side.
    UNIT_FLAG_DISARMED = 0x00200000,    // disable melee spells casting..., "Required melee weapon"
                                        // added to melee spells tooltip.
    UNIT_FLAG_CONFUSED = 0x00400000,    // Unit is a subject to confused movement, movement checks
                                        // disabled, paired with loss of client control packet.
    UNIT_FLAG_FLEEING = 0x00800000,     // Unit is a subject to fleeing movement, movement checks
                                        // disabled, paired with loss of client control packet.
    UNIT_FLAG_POSSESSED = 0x01000000,   // Unit is under remote control by another unit, movement
                                        // checks disabled, paired with loss of client control
                                        // packet. New master is allowed to use melee attack and
                                        // can't select this unit via mouse in the world (as if it
                                        // was own character).
...
enum UnitFlags2
{
    ...
    UNIT_FLAG2_HIDE_BODY = 0x00000002, // Hides body and body armor. Weapons and
    ...
    // added in 3.x, possible all later not used in pre-3.x UNIT_FLAG2_UNK8 =
    // 0x00000100, UNIT_FLAG2_UNK9                 = 0x00000200,
...
enum NPCFlags
{
    ...
    UNIT_NPC_FLAG_SPELLCLICK = 0x01000000,         // cause client to send 1015 opcode (spell click), dynamic,
 ...
 * about the idea behind diminishing returns, see:
 * http://www.wowwiki.com/Diminishing_returns \see Unit::GetDiminishing \see
 * Unit::IncrDiminishing \see Unit::ApplyDiminishingToDuration \see
...
struct DiminishingReturn
{
    ...
    uint16 stack : 16;
    /**
     * Records at what time the last hit with this DiminishingGroup was done, if
     * it's higher than 15 seconds (ie: 15 000 ms) the
     * DiminishingReturn::hitCount will be reset to
     * DiminishingLevels::DIMINISHING_LEVEL_1, which will do no difference to the
     * duration of the stun etc.
     */
    uint32 hitTime;
    /**
     * Records how many times a spell of this DiminishingGroup has hit, this in
     * turn decides how how long the duration of the stun etc is.
     */
    uint32 hitCount;
};
...
struct CleanDamage
{
    CleanDamage(uint32 _damage, WeaponAttackType _attackType, MeleeHitOutcome _hitOutCome, bool _takenOrAbsorbedDamage)
        : damage(_damage), attackType(_attackType), hitOutCome(_hitOutCome),
          takenOrAbsorbedDamage(_takenOrAbsorbedDamage)
    {
    }
...
struct SpellNonMeleeDamage
{
    SpellNonMeleeDamage(WorldObject *_attacker, Unit *_target, uint32 _SpellID, SpellSchoolMask _schoolMask,
        ...
        : target(_target), attacker(_attacker), SpellID(_SpellID), damage(0), schoolMask(_schoolMask), absorb(0),
          resist(0), periodicLog(false), unused(false), blocked(0), HitInfo(0), spell(spell)
    {
    }
...
// Unit* victim, uint32 procAttacker, uint32 procVictim, uint32 procExtra,
// uint32 amount, WeaponAttackType attType, SpellEntry const* spellInfo, bool
// dontTriggerSpecial
...
struct ProcSystemArguments
{
    ...
    explicit ProcSystemArguments(Unit *attacker, Unit *victim, uint32 procFlagsAttacker, uint32 procFlagsVictim,
                                 ...
                                 SpellEntry const *spellInfo = nullptr, Spell *spell = nullptr, uint32 healthGain = 0,
                                 bool isHeal = false)
        : attacker(attacker), victim(victim), procFlagsAttacker(procFlagsAttacker), procFlagsVictim(procFlagsVictim),
          procExtra(procExtra), damage(amount), spellInfo(spellInfo), attType(attType), spell(spell),
...
struct CharmInfo
{
  public:
    ...
    uint32 GetPetLastAttackCommandTime() const
    {
        return m_petLastAttackCommandTime;
    }
    void SetPetLastAttackCommandTime(uint32 time)
    {
        m_petLastAttackCommandTime = time;
    }
    ...
    float GetStayPosX() const
    {
        return m_stayPosX;
    }
    float GetStayPosY() const
    {
        return m_stayPosY;
    }
    float GetStayPosZ() const
    {
        return m_stayPosZ;
    }
    float GetStayPosO() const
    {
        return m_stayPosO;
    }

    uint32 GetSpellOpener() const
    {
        return m_opener;
    }
    uint32 GetSpellOpenerMinRange() const
    {
        return m_openerMinRange;
    }
    uint32 GetSpellOpenerMaxRange() const
    {
        return m_openerMaxRange;
    }
    ...
    CombatData *GetCombatData() const
    {
        return m_combatData;
    };

    void SetCharmStartPosition(Position const &position)
    {
        m_charmStartPosition = position;
    }
    Position const &GetCharmStartPosition()
    {
        return m_charmStartPosition;
    }
...
#define MAX_CREATURE_ATTACK_RADIUS                                                                                     \
    45.0f // max distance for creature aggro (use with
...
enum SelectFlags
{
    ...
    SELECT_FLAG_RANGE_RANGE = 0x0400,       // For direct targeted abilities like
                                            // charge or frostbolt but need custom data
    SELECT_FLAG_RANGE_AOE_RANGE = 0x0800,   // For AOE targeted abilities like
                                            // frost nova but need custom data
    SELECT_FLAG_POWER_NOT_MANA = 0x1000,    // Used in some dungeon encounters
    SELECT_FLAG_USE_EFFECT_RADIUS = 0x2000, // For AOE targeted abilities which
                                            // have correct data in effect index 0
...
class Unit : public WorldObject
{
  public:
    ...
    typedef std::pair<SpellAuraHolderMap::const_iterator, SpellAuraHolderMap::const_iterator>
        SpellAuraHolderConstBounds;
    ...
    void CleanupsBeforeDelete() override; // used in ~Creature/~Player (or before mass creature delete to
                                          // remove cross-references to already deleted units)
     ...
     * @param group The group that you would like to know the current diminishing
     * return level for
     ...
     * diminishing returns, ie, if the Level passed in is
     * DIMINISHING_LEVEL_IMMUNE then the duration will be zeroed out. If it is
     * DIMINISHING_LEVEL_1 then a full duration will be used
     * @param group The group to affect
     * @param duration The duration to be changed, will be updated with the new
     * duration
     * @param caster Who's casting the spell, used to decide whether anything
     * should be calculated
     * @param Level The current level of diminishing returns for the group,
     * decides the new duration
     * @param isReflected Whether the spell was reflected or not, used to
     * determine if we should do any calculations at all.
     */
     ...
     * @param time the remaining time until we can attack with the
     * WeaponAttackType again
     */
    ...
    /**
     * Resets the attack timer to the base value decided by
     * Unit::m_modAttackSpeedPct and Unit::GetAttackTime
    ...
    /**
     * Checks whether the unit can do an attack. Does this by checking the
     * attacktimer for the WeaponAttackType, can probably be thought of as a
     * cooldown for each swing/shot
     * @param type What weapon should we check for
     * @return true if the Unit::m_attackTimer is zero for the given
     * WeaponAttackType
     */
    ...
    inline bool hasMainhandWeapon() const
    {
        return hasWeapon(BASE_ATTACK);
    }
    inline bool hasOffhandWeapon() const
    {
        return hasWeapon(OFF_ATTACK);
    }
    inline bool hasRangedWeapon() const
    {
        return hasWeapon(RANGED_ATTACK);
    }
    /**
    ...
    {
        return CanUseEquippedWeapon(type);
    }
    inline bool hasMainhandWeaponForAttack() const
    {
        return hasWeaponForAttack(BASE_ATTACK);
    }
    inline bool hasOffhandWeaponForAttack() const
    {
        return hasWeaponForAttack(OFF_ATTACK);
    }
    inline bool hasRangedWeaponForAttack() const
    {
        return hasWeaponForAttack(RANGED_ATTACK);
    }
    /**
    ...
    /**
     * Check is a given equipped weapon can be used, ie the mainhand, offhand
     * etc.
     * @param attackType The attack type to check, ie: main/offhand/ranged
     * @return True if the weapon can be used, true except for shapeshifts and if
     * disarmed.
     */
    ...
    bool _addAttacker(Unit *pAttacker) //< (Internal Use) must be called only from
                                       // Unit::Attack(Unit*)
    {
    ...
    void _removeAttacker(Unit *pAttacker) //< (Internal Use) must be called only
                                          // from Unit::AttackStop()
    {
        m_attackers.erase(pAttacker);
    }
    Unit *getAttackerForHelper() //< Return a possible enemy from this unit to
                                 // help in combat
    {
    ...
    /**
     * Tries to attack a Unit/Player, also makes sure to stop attacking the
     * current target if we're already attacking someone.
    ...
    /**
     * Called when we are attacked by someone in someway, might be when a fear
     * runs out and we want to notify AI to attack again or when a spell hits.
     * @param attacker Who's attacking us
     */
    void AttackedBy(Unit *attacker);
    /**
     * Stop all spells from casting except the one give by except_spellid
     * @param except_spellid This spell id will not be stopped from casting,
     * defaults to 0 \see Unit::InterruptSpell
     */
    void CastStop(uint32 except_spellid = 0);
    /**
     * Stops attacking whatever we are attacking at the moment and tells the Unit
     * we are attacking that we are not doing that anymore.
    ...
    /**
     * Removes all attackers from the Unit::m_attackers set and logs it if
     * someone that wasn't attacking it was in the list. Does this check by
     * checking if Unit::AttackStop() returned false. \see Unit::AttackStop
     */
    ...
    /// Returns the Unit::m_attackers, that stores the units that are attacking
    /// you
    ...
    void CombatStop(bool includingCast = false,
                    bool includingCombo = true); //< Stop this unit from combat, if
                                                 // includingCast==true, also interrupt casting
    ...
    bool HasBreakableByDamageCrowdControlAura(Unit *excludeCasterChannel = nullptr,
                                              uint32 excludeAuraApplyMSTimeCutoff = 0) const;
    bool HasBreakableByDamageAuraType(AuraType type, uint32 excludeAura, uint32 excludeAuraApplyMSTimeCutoff) const;
    bool HasAuraPetShouldAvoidBreaking(Unit *excludeCasterChannel = nullptr,
                                       uint32 excludeAuraApplyMSTimeCutoff = 0) const;
    ...
    inline int32 GetArmor() const
    {
        return GetResistance(SPELL_SCHOOL_NORMAL);
    }
    inline void SetArmor(int32 val)
    {
        SetStatInt32Value(UNIT_FIELD_RESISTANCES, val);
    }

    inline int32 GetResistance(SpellSchools school) const
    {
        return GetInt32Value(UNIT_FIELD_RESISTANCES + school);
    }
    inline void SetResistance(SpellSchools school, int32 val)
    {
        SetInt32Value(UNIT_FIELD_RESISTANCES + school, val);
    }
    ...
    bool IsImmuneToPlayer() const
    {
        return HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IMMUNE_TO_PLAYER);
    }
    ...
    void SetVisFlags(uint8 flags)
    {
        SetByteFlag(UNIT_FIELD_BYTES_1, UNIT_BYTES_1_OFFSET_VIS_FLAGS, flags);
    }
    void RemoveVisFlags(uint8 flags)
    {
        RemoveByteFlag(UNIT_FIELD_BYTES_1, UNIT_BYTES_1_OFFSET_VIS_FLAGS, flags);
    }

    bool IsMounted() const
    {
        return HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_MOUNT);
    } // not used with creature non-aura mounts
    ...
    static uint32 DealDamage(Unit *dealer, Unit *victim, uint32 damage, CleanDamage const *cleanDamage,
                             ...
                             SpellEntry const *spellProto, bool durabilityLoss, Spell *spell = nullptr);
    ...
    static void DealSpellDamage(Unit *affectiveCaster, SpellNonMeleeDamage *spellDamageInfo, bool durabilityLoss,
                                bool resetLeash);
    ...
    static SpellMissInfo SpellHitResult(WorldObject *caster, Unit *victim, SpellEntry const *spell, uint8 effectMask,
                                        bool reflectable = false, bool reflected = false,
                                        uint32 *heartbeatResistChance = nullptr);
    ...
    bool CanReactInCombat() const
    {
        return (IsAlive() && !IsCrowdControlled() && !GetCombatManager().IsEvadingHome());
    }
    bool CanCastSpellInCombat() const
    {
        return (IsAlive() && !GetCombatManager().IsEvadingHome());
    } // spells are stopped by spell system
    ...
    bool CanGlanceInCombat() const
    {
        return CanGlance();
    }
    bool CanGlanceInCombat(const Unit *victim) const;
    bool CanDazeInCombat() const
    {
        return CanDaze();
    }
    bool CanDazeInCombat(const Unit *victim) const;

    // Unit Combat Reactions API for abilities (melee and ranged spells):
    // Dodge/Parry/Block/Deflect
    ...
    bool RollAbilityPartialBlockOutcome(const Unit *attacker, WeaponAttackType attType,
    ...
    virtual bool CanCrit(WeaponAttackType attType) const
    {
        return (GetCritChance(attType) >= 0.005f);
    }
    virtual bool CanCrit(SpellSchoolMask schoolMask) const
    {
        return (GetCritChance(schoolMask) >= 0.005f);
    }
    ...
    /*Hack to support always hitting creatures. TODO: investigate Serpentshrine
     * Parasite*/
    void SetAlwaysHit(bool value)
    {
        m_alwaysHit = value;
    }

    static bool RollSpellCritOutcome(Unit *caster, const Unit *victim, SpellSchoolMask schoolMask,
    ...
    bool isServiceProvider() const
    {
        return HasFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_VENDOR | UNIT_NPC_FLAG_TRAINER | UNIT_NPC_FLAG_FLIGHTMASTER |
                                           UNIT_NPC_FLAG_PETITIONER | UNIT_NPC_FLAG_BATTLEMASTER |
                                           UNIT_NPC_FLAG_BANKER | UNIT_NPC_FLAG_INNKEEPER | UNIT_NPC_FLAG_SPIRITHEALER |
    ...
    bool IsTaxiFlying() const
    {
        return hasUnitState(UNIT_STAT_TAXI_FLIGHT);
    }
    ...
    inline void SetOutOfCombatWithAggressor(Unit *aggressor)
    {
        SetInCombatWithAggressor(aggressor, true);
    }
    void SetInCombatWithAssisted(Unit *assisted, bool touchOnly = false);
    inline void SetOutOfCombatWithAssisted(Unit *assisted)
    {
        SetInCombatWithAssisted(assisted, true);
    }
    void SetInCombatWithVictim(Unit *victim, bool touchOnly = false);
    inline void SetOutOfCombatWithVictim(Unit *victim)
    {
        SetInCombatWithVictim(victim, true);
    }
    // bidirectional api - we need info about who is the attacker - handles
    // leashing
    ...
    virtual bool HasSpell(uint32 /*spellID*/) const
    {
        return false;
    }
    ...
    SpellCastResult CastSpell(Unit *Victim, SpellEntry const *spellInfo, uint32 triggeredFlags,
    ...
    SpellCastResult CastCustomSpell(Unit *Victim, SpellEntry const *spellInfo, int32 const *bp0, int32 const *bp1,
                                    ...
                                    Aura *triggeredByAura = nullptr, ObjectGuid originalCaster = ObjectGuid(),
                                    SpellEntry const *triggeredBy = nullptr);
    SpellCastResult CastSpell(float x, float y, float z, uint32 spellId, uint32 triggeredFlags,
                              Item *castItem = nullptr, Aura *triggeredByAura = nullptr,
                              ObjectGuid originalCaster = ObjectGuid(), SpellEntry const *triggeredBy = nullptr);
    ...
    SpellCastResult CastSpell(Unit *Victim, uint32 spellId, TriggerCastFlags triggeredFlags, Item *castItem = nullptr,
                              ...
                              SpellEntry const *triggeredBy = nullptr)
    {
        return CastSpell(Victim, spellId, uint32(triggeredFlags), castItem, triggeredByAura, originalCaster,
                         triggeredBy);
    }
    SpellCastResult CastCustomSpell(Unit *Victim, uint32 spellId, int32 const *bp0, int32 const *bp1, int32 const *bp2,
    ...
    SpellCastResult CastSpell(float x, float y, float z, uint32 spellId, TriggerCastFlags triggeredFlags,
                              ...
                              ObjectGuid originalCaster = ObjectGuid(), SpellEntry const *triggeredBy = nullptr)
    {
        return CastSpell(x, y, z, spellId, uint32(triggeredFlags), castItem, triggeredByAura, originalCaster,
                         triggeredBy);
    }
    ...
    SpellCastResult CastSpell(Unit *Victim, SpellEntry const *spellInfo, TriggerCastFlags triggeredFlags,
                              ...
                              ObjectGuid originalCaster = ObjectGuid(), SpellEntry const *triggeredBy = nullptr)
    {
        return CastSpell(Victim, spellInfo, uint32(triggeredFlags), castItem, triggeredByAura, originalCaster,
                         triggeredBy);
    }
    SpellCastResult CastCustomSpell(Unit *Victim, SpellEntry const *spellInfo, int32 const *bp0, int32 const *bp1,
    ...
    static void SendSpellNonMeleeDamageLog(WorldObject *attacker, Unit *target, uint32 spellID, uint32 damage,
                                           SpellSchoolMask damageSchoolMask, uint32 absorbedDamage, int32 resist,
                                           bool isPeriodic, uint32 blocked, bool criticalHit = false,
                                           bool split = false);
    ...
    static void CasterHitTargetWithSpell(Unit *realCaster, Unit *target, SpellEntry const *spellInfo, bool triggered,
                                         bool success = true);
    ...
    // recommend use MonsterMove/MonsterMoveWithSpeed for most case that
    // correctly work with movegens if used additional args in ... part then
    // floats must explicitly casted to double
    ...
    bool IsMovingIgnoreFlying() const
    {
        return m_movementInfo.HasMovementFlag(movementFlagsIgnoreFlyingMask);
    }
    bool IsMovingForward() const
    {
        return m_movementInfo.HasMovementFlag(MOVEFLAG_MASK_MOVING_FORWARD);
    }
    ...
    {
        return m_movementInfo.HasMovementFlag(MOVEFLAG_JUMPING);
    }
    bool IsFalling() const
    {
        return m_movementInfo.HasMovementFlag(MOVEFLAG_FALLING);
    }

    bool IsDebuggingMovement() const
    {
        return m_debuggingMovement;
    }
    void SetDebuggingMovement(bool state)
    {
        m_debuggingMovement = state;
    }
    ...
    // Convenience checkers/getters/setters counterparts for some of the
    // protected Unit guid fields See the comments next to protected methods for
    // meanings
    ...
    bool HasCharmer() const
    {
        return GetCharmerGuid().IsUnit();
    }
    ...
    void SetCharm(Unit *charmed)
    {
        SetCharmGuid(charmed ? charmed->GetObjectGuid() : ObjectGuid());
    }
    void SetCharmer(Unit *charmer)
    {
        SetCharmerGuid(charmer ? charmer->GetObjectGuid() : ObjectGuid());
    }
    void SetTarget(WorldObject *target)
    {
        SetTargetGuid(target ? target->GetObjectGuid() : ObjectGuid());
    }
    void SetChannelObject(WorldObject *object)
    {
        SetChannelObjectGuid(object ? object->GetObjectGuid() : ObjectGuid());
    }

    // Purely logical guid constructs getters/setters
    // Owner: automatically resolves to creator guid and additionally to summoner
    // guid
    ObjectGuid const &GetOwnerGuid() const override
    {
        return GetCreatorGuid();
    }
    void SetOwnerGuid(ObjectGuid owner) override
    {
        SetCreatorGuid(owner);
    }
    // Pet: automatically resolves to summon guid (permanent pet)
    ObjectGuid const &GetPetGuid() const
    {
        return GetSummonGuid();
    }
    void SetPetGuid(ObjectGuid pet)
    {
        SetSummonGuid(pet);
    }
    // Selection: by default resolves to target, overriden in Player
    virtual ObjectGuid const &GetSelectionGuid() const
    {
        return GetTargetGuid();
    }
    virtual void SetSelectionGuid(ObjectGuid guid)
    {
        SetTargetGuid(guid);
    }
    ...
    // Client controlled: check if unit currently is under client control (has
    // active "mover"), optionally check for specific client (server-side)
    bool IsClientControlled(Player const *exactClient = nullptr) const;
    // Controlling client: server PoV on which client (player) controls movement
    // of the unit at the moment, obtain "mover" (server-side)
    Player const *GetClientControlling() const;

    Pet *GetPet() const;
    void SetPet(Unit *pet)
    {
        SetPetGuid(pet ? pet->GetObjectGuid() : ObjectGuid());
    }

    Pet *GetMiniPet() const;
    void SetMiniPet(Unit *pet)
    {
        SetCritterGuid(pet ? pet->GetObjectGuid() : ObjectGuid());
    }
    void RemoveMiniPet();
    ...
    void _AddTotem(TotemSlot slot,
                   Totem *totem);    // only for call from Totem summon code
    ...
    int32 GetCreateResistance(SpellSchools school) const
    {
        return m_createResistances[school];
    }
    ...
    {
        return ShapeshiftForm(GetByteValue(UNIT_FIELD_BYTES_2, UNIT_BYTES_2_OFFSET_SHAPESHIFT));
    }
    void SetShapeshiftForm(ShapeshiftForm form)
    {
        SetByteValue(UNIT_FIELD_BYTES_2, UNIT_BYTES_2_OFFSET_SHAPESHIFT, form);
    }
    ...
    inline void SetRangedDamageSchool(SpellSchools school)
    {
        SetAttackDamageSchool(RANGED_ATTACK, school);
    }
    ...
    void SetVisibilityWithoutUpdate(UnitVisibility x)
    {
        m_Visibility = x;
    }
    ...
    bool IsVisibleForOrDetect(Unit const *u, WorldObject const *viewPoint, bool detect, bool inVisibleList = false,
                              bool is3dDistance = true, bool spell = false, bool ignorePhase = false) const;
    ...
    TrackedAuraTargetMap &GetTrackedAuraTargets(TrackedAuraType type)
    {
        return m_trackedAuraTargets[type];
    }
    TrackedAuraTargetMap const &GetTrackedAuraTargets(TrackedAuraType type) const
    {
        return m_trackedAuraTargets[type];
    }
    ...
    {
        return GetCombatData()->threatManager;
    }
    ThreatManager const &getThreatManager() const
    {
        return const_cast<Unit *>(this)->GetCombatData()->threatManager;
    }
    ...
    {
        return GetCombatData()->hostileRefManager;
    }
    void SetNoThreatState(bool state)
    {
        m_noThreat = state;
    }
    bool GetNoThreatState()
    {
        return m_noThreat;
    }
    ...
    {
        return const_cast<Unit *>(this)->m_combatManager;
    }
    ...
    {
        return m_spellAuraHolders;
    }
    SpellAuraHolderMap const &GetSpellAuraHolderMap() const
    {
        return m_spellAuraHolders;
    }
    ...
    uint32 SpellDamageBonusDone(Unit *victim, SpellEntry const *spellProto, uint32 pdamage, DamageEffectType damagetype,
                                uint32 stack = 1);
    uint32 SpellDamageBonusTaken(Unit *caster, SpellEntry const *spellProto, uint32 pdamage,
    ...
    // only to be used in proc handlers - basepoints is expected to be a
    // MAX_EFFECT_INDEX sized array
    SpellAuraProcResult TriggerProccedSpell(Unit *target, std::array<int32, MAX_EFFECT_INDEX> &basepoints,
                                            ...
                                            uint32 cooldown, ObjectGuid originalCaster);
    SpellAuraProcResult TriggerProccedSpell(Unit *target, std::array<int32, MAX_EFFECT_INDEX> &basepoints,
    ...
    {
        return m_lastManaUseTimer != 0 && m_lastManaUseTimer != 5000;
    }
    ...
    void CalculateAbsorbResistBlock(Unit *caster, SpellNonMeleeDamage *spellDamageInfo, SpellEntry const *spellProto,
    ...
    template <typename Func> bool HasAuraWithCondition(Func const &func) const
    {
    ...
    {
        return HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_FLEEING);
    }
    ...
    {
        return HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_STUNNED);
    }
    bool SetStunned(bool apply, ObjectGuid casterGuid = ObjectGuid(), uint32 spellID = 0, bool logout = false);

    inline bool IsStunnedByLogout() const
    {
        return hasUnitState(UNIT_STAT_LOGOUT_TIMER);
    }
    bool SetStunnedByLogout(bool apply);

    // Panic: AI reaction script, NPC flees (e.g. at low health)
    inline bool IsInPanic() const
    {
        return hasUnitState(UNIT_STAT_PANIC);
    }
    inline bool SetInPanic(uint32 duration)
    {
        return SetFleeing(true, GetObjectGuid(), 0, duration);
    }
    ...
    bool IsFeigningDeathSuccessfully() const
    {
        return hasUnitState(UNIT_STAT_FEIGN_DEATH);
    }
    void SetFeignDeath(bool apply, ObjectGuid casterGuid = ObjectGuid(), uint32 spellID = 0, bool dynamic = true,
                       bool success = true);
    virtual bool IsIgnoringFeignDeath() const
    {
        return false;
    }
    ...
    {
        return m_movementInfo.GetTransportPos().x;
    }
    float GetTransOffsetY() const
    {
        return m_movementInfo.GetTransportPos().y;
    }
    float GetTransOffsetZ() const
    {
        return m_movementInfo.GetTransportPos().z;
    }
    float GetTransOffsetO() const
    {
        return m_movementInfo.GetTransportPos().o;
    }
    ...
    bool IsAINotifyScheduled() const
    {
        return m_AINotifyEvent != nullptr;
    }
    void FinalizeAINotifyEvent()
    {
    ...
    bool IsLinkingEventTrigger() const
    {
    ...
    // Break own charm effects on all charmed units or advertised charm field
    // only
    ...
    // Uncharm (physically revert the charm effect) the unit and reset player
    // control if required
    ...
    {
        return m_canEnterCombat && !GetCombatManager().IsEvadingHome();
    }
    void SetCanEnterCombat(bool can)
    {
        m_canEnterCombat = can;
    }

    void SetIgnoreRangedTargets(bool state)
    {
        m_ignoreRangedTargets = state;
    }
    bool IsIgnoringRangedTargets()
    {
        return m_ignoreRangedTargets;
    }

    void SetSupportThreatOnly(bool state)
    {
        m_supportThreatOnly = state;
    }
    bool IsSupportThreatOnly()
    {
        return m_supportThreatOnly;
    }

    void DisableThreatPropagationToOwner()
    {
        m_ownerThreatPropagation = false;
    }
    bool IsPropagatingThreatToOwner()
    {
        return m_ownerThreatPropagation;
    } // TBC+ - Eye of Kilrogg

    float GetAttackDistance(Unit const *target) const;
    virtual uint32 GetDetectionRange() const
    {
        return 20.f;
    }
    ...
    virtual CombatData *GetCombatData()
    {
        return m_combatData;
    }
    ...
    {
        return m_baseSpeedRun;
    }

    bool IsSpellProccingHappening() const
    {
        return m_spellProcsHappening;
    }
    void AddDelayedHolderDueToProc(SpellAuraHolder *holder)
    {
        m_delayedSpellAuraHolders.push_back(holder);
    }

    void ResetAutoRepeatSpells()
    {
    ...
    std::vector<Aura *> &GetScriptedLocationAuras(AuraScriptLocation location)
    {
    ...
    uint32 GetDamageDoneByOthers()
    {
        return m_damageByOthers;
    }
    ...
    virtual bool CanCallForAssistance() const
    {
        return true;
    }
    virtual bool CanCheckForHelp() const
    {
        return true;
    }

    virtual std::vector<uint32> GetCharmSpells() const
    {
        return {};
    }

    FormationSlotDataSPtr GetFormationSlot()
    {
        return m_formationSlot;
    }
    void SetFormationSlot(FormationSlotDataSPtr fSlot)
    {
        m_formationSlot = fSlot;
    }

    virtual bool IsNoWeaponSkillGain() const
    {
        return false;
    }

  protected:
    bool MeetsSelectAttackingRequirement(Unit *target, SpellEntry const *spellInfo, uint32 selectFlags,
    ...
    SpellAuraHolderMap::iterator m_spellAuraHoldersUpdateIterator; // != end() in Unit::m_spellAuraHolders
                                                                   // update and point to next element
    AuraList m_deletedAuras;                                       // auras removed while in ApplyModifier and waiting
                                                                   // deleted
    ...
    // std::list< spellEffectPair > AuraSpells[TOTAL_AURAS];  // TODO: use this
    // if ok for mem
    ...
    void SetCharmGuid(ObjectGuid const &charm)
    {
        SetGuidValue(UNIT_FIELD_CHARM, charm);
    }
    // Summon: permanent pet unit guid (do not use: managed by
    // SetPetGuid/GetPetGuid)
    ObjectGuid const &GetSummonGuid() const
    {
        return GetGuidValue(UNIT_FIELD_SUMMON);
    }
    void SetSummonGuid(ObjectGuid const &summon)
    {
        SetGuidValue(UNIT_FIELD_SUMMON, summon);
    }
    ...
    void SetCharmerGuid(ObjectGuid const &owner)
    {
        SetGuidValue(UNIT_FIELD_CHARMEDBY, owner);
    }
    // Summoner: permanent owner unit guid for player pets [nameplate] (do not
    // use: managed by SetOwnerGuid/GetOwnerGuid)
    ObjectGuid const &GetSummonerGuid() const
    {
        return GetGuidValue(UNIT_FIELD_SUMMONEDBY);
    }
    void SetSummonerGuid(ObjectGuid const &owner)
    {
        SetGuidValue(UNIT_FIELD_SUMMONEDBY, owner);
    }
    // Creator: permanent owner unit guid for npc pets or non-pet units
    // [nameplate] (do not use: managed by SetOwnerGuid/GetOwnerGuid)
    ObjectGuid const &GetCreatorGuid() const
    {
        return GetGuidValue(UNIT_FIELD_CREATEDBY);
    }
    void SetCreatorGuid(ObjectGuid const &creator)
    {
        SetGuidValue(UNIT_FIELD_CREATEDBY, creator);
    }
    // Target: current target guid as advertised on unit frames (also known as
    // selection)
    ...
    // Persuation target: temporarily increased reputation unit guid (pre-WotLK
    // relations)
    ObjectGuid const &GetPersuadedGuid() const
    {
        return GetGuidValue(UNIT_FIELD_PERSUADED);
    }
    void SetPersuadedGuid(ObjectGuid const &persuaded)
    {
        SetGuidValue(UNIT_FIELD_PERSUADED, persuaded);
    }
    ...
    void SetChannelObjectGuid(ObjectGuid const &targetGuid)
    {
        SetGuidValue(UNIT_FIELD_CHANNEL_OBJECT, targetGuid);
    }

    // Additional server-side guid fields getters/setters
    // Critter: permanent mini-pet unit guid (pre-WotLK compatibility replacement
    // for critter guid field)
    ObjectGuid const &GetCritterGuid() const
    {
        return m_critterGuid;
    }
    void SetCritterGuid(ObjectGuid critterGuid)
    {
        m_critterGuid = critterGuid;
    }
  ...
  private:
    ...
    GuidSet m_charmedUnitsPrivate; // stores non-advertised active charmed unit
                                   // guids (e.g. aoe charms)
    ...
    bool m_ignoreRangedTargets; // Ignores ranged targets when picking someone to
                                // attack
  ...
  private: // Error traps for some wrong args using
    // this will catch and prevent build for any cases when all optional args
    // skipped and instead triggered used non boolean type no bodies expected for
    // this declarations
...
// The spell target is always first unless there is a target at _completely_
// the same position (unbelievable case)
struct TargetDistanceOrderNear
{
    WorldObject const *m_mainTarget;
    DistanceCalculation m_distcalc;

    TargetDistanceOrderNear(WorldObject const *target, DistanceCalculation distcalc = DIST_CALC_NONE)
...
struct TargetDistanceOrderFarAway
{
    WorldObject const *m_mainTarget;
    DistanceCalculation m_distcalc;
    TargetDistanceOrderFarAway(WorldObject const *target, DistanceCalculation distcalc = DIST_CALC_NONE)
        : m_mainTarget(target), m_distcalc(distcalc)
    {
    }
    // functor for operator "<"
...
struct LowestHPNearestOrder
{
    ...
    LowestHPNearestOrder(WorldObject const *target, DistanceCalculation distcalc = DIST_CALC_NONE)
        : m_mainTarget(target), m_distcalc(distcalc)
    {
    }
    // functor for operator ">"
