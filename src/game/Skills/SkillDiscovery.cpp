...
void LoadSkillDiscoveryTable()
{
    ...
    do
    {
        ...
        if (reqSkillOrSpell > 0) // spell case
        {
            ...
            if (!reqSpellEntry)
            {
                ...
                {
                    sLog.outErrorDb("Spell (ID: %u) have nonexistent spell "
                                    "(ID: %i) in `reqSpell` "
            ...
            if (reqSpellEntry->Mechanic != MECHANIC_DISCOVERY)
            {
                if (reportedReqSpells.find(reqSkillOrSpell) == reportedReqSpells.end())
                {
                    sLog.outErrorDb("Spell (ID: %u) not have MECHANIC_DISCOVERY (28) value "
                                    "in "
                                    "Mechanic field in spell.dbc but listed for spellId %u "
                                    "(and "
        ...
        else if (reqSkillOrSpell == 0) // skill case
        {
            ...
            {
                sLog.outErrorDb("Spell (ID: %u) not listed in `SkillLineAbility.dbc` but "
                                "listed "
        ...
        {
            sLog.outErrorDb("Spell (ID: %u) have negative value in `reqSpell` field "
    ...
    if (!ssNonDiscoverableEntries.str().empty())
        sLog.outErrorDb("Some items can't be successfully discovered: have in chance field "
