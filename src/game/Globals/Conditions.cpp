...
// Attention: make sure to keep this list in sync with ConditionSource to avoid
// array
//            out of bounds access! It is accessed with ConditionSource as
//            index!
...
{
    DEBUG_LOG("Condition-System: Check condition %u, type %i - called from %s "
              "with params target: %s, map %i, source %s",
    ...
    {
        sLog.outErrorDb("CONDITION %u type %u used with bad parameters, called "
                        "from %s, used with target: %s, map %i, source %s",
                        m_entry, m_condition, conditionSourceToStr[conditionSourceType],
                        target ? target->GetGuidStr().c_str() : "<nullptr>", map ? map->GetId() : -1,
                        source ? source->GetGuidStr().c_str() : "<nullptr>");
        return false;
                                     ...
                                     ConditionSource conditionSourceType) const
{
    ...
    case CONDITION_NOT: {
        // Checked on load
        return !sConditionStorage.LookupEntry<ConditionEntry>(m_value1)->Meets(target, map, source,
                                                                               conditionSourceType);
    }
    case CONDITION_OR: {
    ...
    case CONDITION_AND: {
        ...
        if (m_value3)
            extraConditionsSatisfied =
                extraConditionsSatisfied && sConditionStorage.LookupEntry<ConditionEntry>(m_value3)->Meets(
                                                target, map, source, conditionSourceType);
        if (m_value4)
            extraConditionsSatisfied =
                extraConditionsSatisfied && sConditionStorage.LookupEntry<ConditionEntry>(m_value4)->Meets(
                                                target, map, source, conditionSourceType);

        return extraConditionsSatisfied &&
               sConditionStorage.LookupEntry<ConditionEntry>(m_value1)->Meets(target, map, source,
                                                                              conditionSourceType) &&
               sConditionStorage.LookupEntry<ConditionEntry>(m_value2)->Meets(target, map, source, conditionSourceType);
    }
    case CONDITION_NONE: {
    ...
    case CONDITION_CREATURE_IN_RANGE: {
        ...
        MaNGOS::NearestCreatureEntryWithLiveStateInObjectRangeCheck creature_check(*target, m_value1, true, false,
                                                                                   m_value2, true);
    ...
    case CONDITION_LEARNABLE_ABILITY: {
        ...
        if (m_value2)
        {
            ...
            // with this condition is to have a all-in-one check regarding
            // items that learn some ability (primary/secondary tradeskills).
            // Commonly, items like this is unique and/or are not expected to
    ...
    case CONDITION_COMPLETED_ENCOUNTER: {
        ...
        if (!map->IsDungeon())
        {
            sLog.outErrorDb("CONDITION_COMPLETED_ENCOUNTER (entry %u) is used "
                            "outside of a dungeon (on Map %u) by %s",
        ...
        {
            sLog.outErrorDb("CONDITION_COMPLETED_ENCOUNTER (entry %u, "
                            "DungeonEncounterEntry "
                            "%u) "
                            "is used on wrong map (used on Map %u) by %s",
                            m_entry, m_value1, target->GetMapId(), target->GetGuidStr().c_str());
            return false;
        }
        return (completedEncounterMask & ((dbcEntry1 ? 1 << dbcEntry1->encounterIndex : 0) |
                                          (dbcEntry2 ? 1 << dbcEntry2->encounterIndex : 0))) != 0;
    }
    ...
    case CONDITION_DEAD_OR_AWAY: {
        // This condition is overcomplicated and doesn't fit neatly into the
        // target checking. Consider splitting it up into 4 separate
        // conditions.
        Player const *player =
        ...
        case 2: // All players in instance dead or out of range
            ...
            if (!map || !map->Instanceable())
            {
                sLog.outErrorDb("CONDITION_DEAD_OR_AWAY %u (Player in instance case) - "
                                "called "
                                "from "
                                "%s without map param or from non-instanceable map %i",
    ...
    case CONDITION_PVP_SCRIPT: {
        ...
        {
            sLog.outErrorDb("CONDITION_PVP_SCRIPT (entry %u) is used on non player "
                            "target. Target is %s",
                            m_entry, target->GetGuidStr().c_str());
            return false;
        }
        Player const *player = static_cast<Player const *>(target);
...
bool ConditionEntry::IsValid() const
{
    ...
    case CONDITION_NOT: {
        if (m_value1 >= m_entry)
        {
            sLog.outErrorDb("CONDITION_NOT (entry %u, type %d) has invalid value1 "
        ...
        if (!condition1)
        {
            sLog.outErrorDb("CONDITION_NOT (entry %u, type %d) has value1 %u without "
    ...
    case CONDITION_AND: {
        if (m_value1 >= m_entry)
        {
            sLog.outErrorDb("CONDITION _AND or _OR (entry %u, type %d) has invalid "
                            "value1 %u, must be lower than entry, skipped",
                            m_entry, m_condition, m_value1);
            return false;
        }
        if (m_value2 >= m_entry)
        {
            sLog.outErrorDb("CONDITION _AND or _OR (entry %u, type %d) has invalid "
                            "value2 %u, must be lower than entry, skipped",
        ...
        if (!condition1)
        {
            sLog.outErrorDb("CONDITION _AND or _OR (entry %u, type %d) has value1 %u "
                            "without proper condition, skipped",
                            m_entry, m_condition, m_value1);
            return false;
        }
        ...
        if (!condition2)
        {
            sLog.outErrorDb("CONDITION _AND or _OR (entry %u, type %d) has value2 %u "
                            "without proper condition, skipped",
                            m_entry, m_condition, m_value2);
            return false;
        }
        if (m_value3)
        {
            ...
            {
                sLog.outErrorDb("CONDITION _AND or _OR (entry %u, type %d) has invalid "
                                "value3 %u, must be lower than entry, skipped",
            ...
            if (!condition3)
            {
                sLog.outErrorDb("CONDITION _AND or _OR (entry %u, type %d) has value3 "
                                "%u without proper condition, skipped",
                                m_entry, m_condition, m_value3);
                return false;
            }
        }
        if (m_value4)
        {
            ...
            {
                sLog.outErrorDb("CONDITION _AND or _OR (entry %u, type %d) has invalid "
                                "value4 %u, must be lower than entry, skipped",
            ...
            if (!condition4)
            {
                sLog.outErrorDb("CONDITION _AND or _OR (entry %u, type %d) has value4 "
                                "%u without proper condition, skipped",
                                m_entry, m_condition, m_value4);
                return false;
            }
    ...
    case CONDITION_AURA: {
        ...
        {
            sLog.outErrorDb("Aura condition (entry %u, type %u) requires to have non "
    ...
    case CONDITION_ITEM_WITH_BANK: {
        ...
        if (!proto)
        {
            sLog.outErrorDb("Item condition (entry %u, type %u) requires to have non "
        ...
        if (m_value2 < 1)
        {
            sLog.outErrorDb("Item condition (entry %u, type %u) useless with count < 1, "
                            "skipped",
                            m_entry, m_condition);
            return false;
        }
    ...
    case CONDITION_ITEM_EQUIPPED: {
        ...
        if (!proto)
        {
            sLog.outErrorDb("ItemEquipped condition (entry %u, type %u) requires to "
                            "have non existing item (%u) equipped, skipped",
                            m_entry, m_condition, m_value1);
            return false;
        }
        break;
    ...
    case CONDITION_AREAID: {
        ...
        if (!areaEntry)
        {
            sLog.outErrorDb("Zone condition (entry %u, type %u) requires to be in "
                            "non existing area (%u), skipped",
                            m_entry, m_condition, m_value1);
            return false;
        }
        ...
        if (m_value2 > 1)
        {
            sLog.outErrorDb("Zone condition (entry %u, type %u) has invalid argument "
                            "%u (must be 0..1), skipped",
                            m_entry, m_condition, m_value2);
            return false;
        }
        break;
    ...
    case CONDITION_REPUTATION_RANK_MAX: {
        ...
        if (!factionEntry)
        {
            sLog.outErrorDb("Reputation condition (entry %u, type %u) requires to "
                            "have reputation non existing faction (%u), skipped",
                            m_entry, m_condition, m_value1);
            return false;
        }
        ...
        if (m_value2 >= MAX_REPUTATION_RANK)
        {
            sLog.outErrorDb("Reputation condition (entry %u, type %u) has invalid rank "
                            "requirement (value2 = %u) - must be between %u and %u, "
                            "skipped",
    ...
    case CONDITION_TEAM: {
        ...
        {
            sLog.outErrorDb("Team condition (entry %u, type %u) specifies unknown "
                            "team (%u), skipped",
                            m_entry, m_condition, m_value1);
            return false;
        }
    ...
    case CONDITION_SKILL_BELOW: {
        ...
        if (!pSkill)
        {
            sLog.outErrorDb("Skill condition (entry %u, type %u) specifies "
                            "non-existing skill (%u), skipped",
                            m_entry, m_condition, m_value1);
            return false;
        }
        if (m_value2 < 1 || m_value2 > sWorld.GetConfigMaxSkillValue())
        {
            sLog.outErrorDb("Skill condition (entry %u, type %u) specifies invalid "
                            "skill value (%u), skipped",
                            m_entry, m_condition, m_value2);
            return false;
        }
        break;
    ...
    case CONDITION_QUEST_NONE: {
        ...
        if (m_value2 && m_condition != CONDITION_QUESTTAKEN)
            sLog.outErrorDb("Quest condition (entry %u, type %u) has useless data in "
                            "value2 (%u)!",
                            m_entry, m_condition, m_value2);
        break;
    }
    ...
    case CONDITION_ACTIVE_GAME_EVENT: {
        ...
        {
            sLog.outErrorDb("(Not)Active event condition (entry %u, type %u) "
                            "requires existing event id (%u), skipped",
                            m_entry, m_condition, m_value1);
            return false;
        }
        break;
    ...
    case CONDITION_AREA_FLAG: {
        ...
        {
            sLog.outErrorDb("Area flag condition (entry %u, type %u) has both values "
                            "like 0, skipped",
                            m_entry, m_condition);
            return false;
        }
    ...
    case CONDITION_RACE_CLASS: {
        if (!m_value1 && !m_value2)
        {
            sLog.outErrorDb("Race_class condition (entry %u, type %u) has both "
                            "values like 0, skipped",
                            m_entry, m_condition);
            return false;
        }
        ...
        {
            sLog.outErrorDb("Race_class condition (entry %u, type %u) has invalid "
                            "player class %u, skipped",
                            m_entry, m_condition, m_value1);
            return false;
        }
    ...
    case CONDITION_LEVEL: {
        ...
        {
            sLog.outErrorDb("Level condition (entry %u, type %u)has invalid "
                            "level %u, skipped",
                            m_entry, m_condition, m_value1);
            return false;
        }
        ...
        {
            sLog.outErrorDb("Level condition (entry %u, type %u) has invalid "
                            "argument %u (must be 0..2), skipped",
                            m_entry, m_condition, m_value2);
            return false;
        }
    ...
    case CONDITION_SPELL: {
        if (!sSpellTemplate.LookupEntry<SpellEntry>(m_value1))
        {
            sLog.outErrorDb("Spell condition (entry %u, type %u) requires to have "
                            "non existing spell (Id: %d), skipped",
                            m_entry, m_condition, m_value1);
            return false;
        }
        ...
        if (m_value2 > 1)
        {
            sLog.outErrorDb("Spell condition (entry %u, type %u) has invalid "
                            "argument %u (must be 0..1), skipped",
                            m_entry, m_condition, m_value2);
            return false;
        }
    ...
    case CONDITION_LEARNABLE_ABILITY: {
        ...
        {
            sLog.outErrorDb("Learnable ability conditon (entry %u, type %u) has "
                            "spell id %u defined, but this spell is not listed in "
                            "SkillLineAbility and can not be used, skipping.",
                            m_entry, m_condition, m_value1);
            return false;
        }
        ...
        if (m_value2)
        {
            ...
            if (!proto)
            {
                sLog.outErrorDb("Learnable ability conditon (entry %u, type "
                                "%u) has item entry "
                                "%u "
                                "defined but item does not exist, skipping.",
                                m_entry, m_condition, m_value2);
                return false;
            }
        }
    ...
    case CONDITION_CREATURE_IN_RANGE: {
        ...
        {
            sLog.outErrorDb("Creature in range condition (entry %u, type %u) has an "
                            "invalid value in value1. (Creature %u does not exist in "
        ...
        if (m_value2 <= 0)
        {
            sLog.outErrorDb("Creature in range condition (entry %u, type %u) "
                            "has an invalid "
                            "value in value2. (Range %u must be greater than "
                            "0), skipping.",
                            m_entry, m_condition, m_value2);
            return false;
        }
    ...
    case CONDITION_COMPLETED_ENCOUNTER: {
        ...
        if (!dbcEntry1)
        {
            sLog.outErrorDb("Completed Encounter condition (entry %u, type %u) has an "
                            "unknown "
                            "DungeonEncounter entry %u defined (in value1), skipping.",
                            m_entry, m_condition, m_value1);
            return false;
        }
        if (m_value2 && !dbcEntry2)
        {
            sLog.outErrorDb("Completed Encounter condition (entry %u, type %u) has an "
                            "unknown "
                            "DungeonEncounter entry %u defined (in value2), skipping.",
                            m_entry, m_condition, m_value2);
            return false;
        }
        if (dbcEntry2 && dbcEntry1->mapId != dbcEntry2->mapId)
        {
            sLog.outErrorDb("Completed Encounter condition (entry %u, type %u) has "
                            "different mapIds for both encounters, skipping.",
                            m_entry, m_condition);
            return false;
        }
        break;
    ...
    case CONDITION_LAST_WAYPOINT: {
        if (m_value2 > 2)
        {
            sLog.outErrorDb("Last Waypoint condition (entry %u, type %u) has an invalid "
                            "value "
                            "in "
                            "value2. (Has %u, supported 0, 1, or 2), skipping.",
                            m_entry, m_condition, m_value2);
            return false;
        }
        break;
    ...
    case CONDITION_DEAD_OR_AWAY: {
        if (m_value1 >= 4)
        {
            sLog.outErrorDb("Dead condition (entry %u, type %u) has an invalid value "
                            "in value1. (Has %u, must be smaller than 4), skipping.",
                            m_entry, m_condition, m_value1);
            return false;
        }
        break;
    ...
    case CONDITION_SPAWN_COUNT: {
        if (!sCreatureStorage.LookupEntry<CreatureInfo>(m_value1))
        {
            sLog.outErrorDb("Spawn count condition (entry %u, type %u) has "
                            "an invalid value in "
                            "value1. (Creature %u does not exist in the "
                            "database), skipping.",
                            m_entry, m_condition, m_value1);
            return false;
        }
