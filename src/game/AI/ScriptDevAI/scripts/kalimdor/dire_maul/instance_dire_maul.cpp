...
SD%Complete: 70
SDComment: Ogre costume suit missing for Tribute Run, Cho'Rush spells
randomisation is not handled properly, Warpwood pods are not implemented, the
Maul event is not handled SDCategory: Dire Maul EndScriptData */
...
void instance_dire_maul::Update(uint32 uiDiff)
{
    if (GetData(TYPE_DREADSTEED) == IN_PROGRESS)
    {
        ...
        else
        {
            ...
            if (bell && bell->GetGoState() != GO_STATE_ACTIVE && wheel && wheel->GetGoState() != GO_STATE_ACTIVE &&
                candle && candle->GetGoState() != GO_STATE_ACTIVE)
                SetData(TYPE_DREADSTEED, FAIL);
        }
    }
}
...
void instance_dire_maul::SetData(uint32 uiType, uint32 uiData)
{
    ...
    case TYPE_ALZZIN: // This Encounter is expected to be handled within Acid
        ...
        if (uiData == DONE)
        {
            ...
            {
                for (GuidList::const_iterator itr = m_lFelvineShardGUIDs.begin(); itr != m_lFelvineShardGUIDs.end();
                     ++itr)
    ...
    case TYPE_KING_GORDOK:
        ...
        if (uiData == DONE)
        {
            ...
            if (Creature *pOgre = GetSingleCreatureFromStorage(NPC_CHO_RUSH_THE_OBSERVER))
            {
                ...
                // start WP movement for Mizzle; event handled by movement and
                // gossip dbscripts
                if (Creature *pMizzle = pOgre->SummonCreature(NPC_MIZZLE_THE_CRAFTY, afMizzleSpawnLoc[0],
                                                              afMizzleSpawnLoc[1], afMizzleSpawnLoc[2],
                                                              afMizzleSpawnLoc[3], TEMPSPAWN_DEAD_DESPAWN, 0, true))
                {
...
void instance_dire_maul::Load(const char *chrIn)
{
    ...
    loadStream >> m_auiEncounter[0] >> m_auiEncounter[1] >> m_auiEncounter[2] >> m_auiEncounter[3] >>
        ...
        m_auiEncounter[14] >> m_auiEncounter[15] >> m_auiEncounter[16] >> m_auiEncounter[17] >> m_auiEncounter[18];
                                                    ...
                                                    uint32 conditionSourceType) const
{
    ...
    script_error_log("instance_dire_maul::CheckConditionCriteriaMeet called with unsupported "
...
struct go_ai_fixed_trap : public GameObjectAI
{
    ...
    void UpdateAI(const uint32 /*uiDiff*/) override
    {
        if (m_go->IsSpawned())
        {
            ...
            // He then enters evade mode and becomes an invalid target to hostile
            // actions from players Faction is changed to 14 and restored by spell
            // 22799 (King of the Gordok) Additionnaly, Guard Slip'kik should get
            // UnitFlags 16 but purpose is unknown so we skip it for now
