...
{
    // safety check to avoid the room being reset for each OnPlayerEnter() call
    // if Rattlegore is dead
    if (m_bIsRoomReset)
        return;
    ...
    // Despawn the mobs already in the room with the exception of the necrofiend
    // (not stored, so not despawned)
    ...
    {
        std::vector<uint32> uiMobList;             // Vector holding the 4 creatures entries
                                                   // for each spawned group
        ...
        // Pick the fourth NPC in the group and randomize the four possible
        // spawns
        switch (urand(0, 1))
        {
        case 0:
    ...
    // the waypoints are handled in DB creature_movement_template table (shared
    // with the other necrofiend in the room) the two other necrofiends in the
    // instance are using DB creature_movement table
...
void instance_scholomance::SetData(uint32 uiType, uint32 uiData)
{
    ...
    {
    case TYPE_KIRTONOS:
        // This door is initially closed by DB-scripts, so only use it in case of
        // FAIL, DONE, or on aggro after wipe
...
void instance_scholomance::HandleDawnGambitEvent()
{
    ...
    // Make each of the Scholomance Student cast "Viewing Room Student Transform
    // - Effect" on himself Change faction to hostile and roam in the room
