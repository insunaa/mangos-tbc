...
SD%Complete: 80
SDComment: Spell Felfire Line Up and Wrath-Scryer's Felfire npc are summoning
are NYI and they need additional research. SDCategory: Tempest Keep, The
Arcatraz EndScriptData */
...
enum
{
    ...
    SPELL_CHARGE = 35754,           // script target on 21030; also dummy effect area
                                    // effect target on 20978 - makes the target cast 35769
...
struct boss_soccothratesAI : public ScriptedAI, private DialogueHelper
{
    ...
    void Aggro(Unit * /*pWho*/) override
    {
        ...
        GuidVector felfireVector; // at aggro felfire mobs always teleport to
                                  // respawn location
    ...
    void SpellHitTarget(Unit *target, const SpellEntry *spell) override
    {
        ...
        {
            // need to get even points between caster and target to reposition
            // felfire evenly
            float sX = m_creature->GetPositionX(),
                  sY = m_creature->GetPositionY(); // source coords
        ...
        if (iEntry == SAY_SOCCOTHRATES_INTRO_7)
        {
            ...
            if (m_pInstance)
            {
                if (Creature *pDalliah = m_pInstance->GetSingleCreatureFromStorage(NPC_DALLIAH))
                    pDalliah->GetMotionMaster()->MovePoint(1, aDalliahStartPos[0], aDalliahStartPos[1],
                                                           aDalliahStartPos[2]);
            }
        }
    }
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (m_uiFelfireShockTimer < uiDiff)
        {
            if (DoCastSpellIfCan(m_creature->GetVictim(),
                                 m_bIsRegularMode ? SPELL_FELFIRE_SHOCK : SPELL_FELFIRE_SHOCK_H) == CAST_OK)
        ...
        if (m_uiFelfireLineupTimer)
        {
            if (m_uiFelfireLineupTimer <= uiDiff)
            {
                        ...
                        m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, nullptr, SELECT_FLAG_PLAYER))
                {
                    ...
                    {
                        // ToDo: the Wrath-Scryer's Felfire npcs should be summoned
                        // at this point and aligned to the chosen target!
        ...
        if (m_uiChargeTimer)
        {
            if (m_uiChargeTimer <= uiDiff)
            {
                ...
                // Note: this spell will also light up the Wrath-Scryer's Felfire
                // npcs
