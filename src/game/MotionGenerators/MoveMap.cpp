...
bool MMapFactory::IsPathfindingEnabled(uint32 mapId, const Unit *unit = nullptr)
{
    ...
    if (unit)
    {
        ...
        // always use mmaps for pets of players (can still be disabled by
        // extra-flag for pet creature)
...
bool MMapManager::loadMap(uint32 mapId, int32 x, int32 y)
{
    ...
    ++loadedTiles;
    DEBUG_FILTER_LOG(LOG_FILTER_MAP_LOADING, "MMAP:loadMap: Loaded mmtile %03i[%02i,%02i] into %03i[%02i,%02i]", mapId,
                     x, y, mapId, header->x, header->y);
    return true;
}
...
bool MMapManager::unloadMap(uint32 mapId, int32 x, int32 y)
{
    ...
    if (dtStatusFailed(dtResult))
    {
        ...
        // but no extra memory is used we cannot recover from this error - assert
        // out
    ...
    else
    {
        ...
        --loadedTiles;
        DEBUG_FILTER_LOG(LOG_FILTER_MAP_LOADING, "MMAP:unloadMap: Unloaded mmtile %03i[%02i,%02i] from %03i", mapId, x,
                         y, mapId);
        return true;
    }
...
bool MMapManager::unloadMap(uint32 mapId)
{
    ...
    for (MMapTileSet::iterator i = mmap->mmapLoadedTiles.begin(); i != mmap->mmapLoadedTiles.end(); ++i)
    {
        ...
        if (dtStatusFailed(dtResult))
            sLog.outError("MMAP:unloadMap: Could not unload %03u%02i%02i.mmtile "
                          "from navmesh",
                          mapId, x, y);
        else
        {
            --loadedTiles;
            DEBUG_FILTER_LOG(LOG_FILTER_MAP_LOADING, "MMAP:unloadMap: Unloaded mmtile %03i[%02i,%02i] from %03i", mapId,
                             x, y, mapId);
        }
    }
        ...
        if (mmap->navMeshGOQueries.find(threadId) == mmap->navMeshGOQueries.end())
        {
            ...
            if (dtStatusFailed(query->init(mmap->navMesh, 2048)))
            {
                ...
                sLog.outError("MMAP:GetNavMeshQuery: Failed to initialize "
                              "dtNavMeshQuery for displayid %03u tid %s",
                              displayId, ss.str().data());
                return nullptr;
            }
