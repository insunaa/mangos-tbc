...
class WaypointManager
{
  public:
    ...
    //    creature.GetEntry() == creature_movement_template.entry
    ...
    //        valid for summons. Mob that can be summoned anywhere should not
    //        have creature_movement_template for example.
    ...
    void SetExternalWPTable(char const *tableName)
    {
        m_externalTable = std::string(tableName);
    }
    std::string GetExternalWPTable() const
    {
        return m_externalTable;
    }
    /// Add Nodes from external sources
    bool AddExternalNode(uint32 entry, int32 pathId, uint32 pointId, float x, float y, float z, float o,
  ...
  private:
    ...
    WaypointPathMap *getMapForPathType(WaypointPathOrigin origin)
    {
        ...
        case PATH_FROM_WAYPOINT_PATH:
            return &m_pathMovementTemplateMap;
        default:
            return nullptr;
        }
    }
