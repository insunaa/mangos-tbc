...
SDName: Arcatraz
SD%Complete: 60
SDComment: Warden Mellichar, event controller for Skyriss event. Millhouse
Manastorm. TODO: make better combatAI for Millhouse. SDCategory: Tempest Keep,
The Arcatraz EndScriptData */
...
struct npc_millhouse_manastormAI : public ScriptedAI, private DialogueHelper
{
    ...
    void EnterEvadeMode() override
    {
        ...
        if (m_creature->IsAlive())
            m_creature->GetMotionMaster()->MovePoint(1, fRoomCenterCoords[0], fRoomCenterCoords[1],
                                                     fRoomCenterCoords[2]);
...
struct npc_warden_mellicharAI : public ScriptedAI
{
    ...
    void Aggro(Unit *pWho) override
    {
        ...
        // In theory the Seal Sphere should protect the npc from being attacked,
        // but because LoS isn't enabled for Gameobjects we have to use this
        // workaround
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (m_uiFlamingWeaponTimer < uiDiff)
        {
            if (DoCastSpellIfCan(m_creature->GetVictim(),
                                 m_bIsRegularMode ? SPELL_FLAMING_WEAPON : SPELL_FLAMING_WEAPON_H) == CAST_OK)
                m_uiFlamingWeaponTimer = urand(3000, 6000);
        }
        else
            m_uiFlamingWeaponTimer -= uiDiff;

        // this spell should only be used against Protean Horror and Protean
        // Nightmare, never players
