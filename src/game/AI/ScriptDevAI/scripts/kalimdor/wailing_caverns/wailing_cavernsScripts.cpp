...
SD%Complete: 90
SDComment: Missing vipers emerge effect, Naralex doesn't fly at exit(Core
issue) SDCategory: Wailing Caverns EndScriptData */
...
struct npc_disciple_of_naralexAI : public npc_escortAI
{
    ...
    // Overwrite the evade function, to change the combat stop function (keep
    // casting some spells)
            ...
            if (m_uiEventTimer <= uiDiff)
            {
                ...
                case 16:
                    ...
                    case 2:
                        ...
                        for (uint8 i = 0; i < 3; ++i)
                            DoSpawnMob(NPC_DEVIATE_VIPER, aSummonPositions[2][0],
                                       aSummonPositions[2][1] + 2 * M_PI_F / 3 * i);
                        m_uiEventTimer = 0;
                        ++m_uiSubeventPhase;
                        break;
                    case 3:
                        // Wait for casting to be complete - TODO, this might have
                        // to be done better
                        ++m_uiSubeventPhase;
                    ...
                    case 4:
                        ...
                        m_uiEventTimer = 0;
                        ++m_uiPoint; // Increment this in order to avoid the
                                     // special case evade
                        SetEscortPaused(false);
                        break;
                    }
                ...
                case 33:
                    ...
                    case 3:
                        // First set of mobs
                        for (uint8 i = 0; i < 3; ++i)
                            DoSpawnMob(NPC_DEVIATE_MOCCASIN, aSummonPositions[3][0],
                                       aSummonPositions[3][1] + M_PI_F / 3 * i);
                        m_uiEventTimer = 20000;
                        ++m_uiSubeventPhase;
                        break;
                    case 4:
                        // Second set of mobs
                        for (uint8 i = 0; i < 7; ++i)
                            DoSpawnMob(NPC_NIGHTMARE_ECTOPLASM, aSummonPositions[3][0],
                                       aSummonPositions[3][1] + M_PI_F / 7 * i);
                        m_uiEventTimer = 0;
                        ++m_uiSubeventPhase;
                        break;
                    case 5:
...
bool GossipHello_npc_disciple_of_naralex(Player *pPlayer, Creature *pCreature)
{
    ...
    {
        pPlayer->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_BEGIN, GOSSIP_SENDER_MAIN,
    ...
    if (uiAction == GOSSIP_ACTION_INFO_DEF + 1)
    {
        if (npc_disciple_of_naralexAI *pEscortAI = dynamic_cast<npc_disciple_of_naralexAI *>(pCreature->AI()))
        {
            pEscortAI->Start(false,
                             pPlayer); // Note: after 4.0.3 set him run = true
