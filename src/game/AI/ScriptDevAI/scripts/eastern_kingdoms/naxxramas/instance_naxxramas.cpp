...
void instance_naxxramas::OnObjectCreate(GameObject *gameObject)
{
    ...
    case GO_KELTHUZAD_TRIGGER:
        if (m_auiEncounter[TYPE_KELTHUZAD] != NOT_STARTED) // Only spawn the visual trigger for Kel'Thuzad when
                                                           // encounter is not started
...
void instance_naxxramas::SetData(uint32 type, uint32 data)
{
    ...
    case TYPE_HEIGAN:
        m_auiEncounter[type] = data;
        // Open the entrance door on encounter win or failure (we specifically
        // set the GOState to avoid issue in case encounter is reset before gate
        // is closed in Heigan script)
    ...
    case TYPE_GOTHIK:
        ...
        {
        case IN_PROGRESS:
            // Encounter begins: close the gate and start timer to summon
            // unrelenting trainees
    ...
    case TYPE_THADDIUS:
        ...
        if (data == FAIL)
        {
            ...
            // Respawn: Stalagg, Feugen, their respective Tesla Coil NPCs and
            // Tesla GOs
...
{
    // Handle the continuous spawning of Living Poison blobs in Patchwerk
    // corridor
    if (m_livingPoisonTimer)
    {
        if (m_livingPoisonTimer <= diff)
        {
            ...
            {
                // Spawn 3 living poisons every 5 secs and make them cross the
                // corridor and then despawn, for ever and ever
                for (uint8 i = 0; i < 3; i++)
                    if (Creature *poison = trigger->SummonCreature(
                            NPC_LIVING_POISON, livingPoisonPositions[i].m_fX, livingPoisonPositions[i].m_fY,
                            livingPoisonPositions[i].m_fZ, livingPoisonPositions[i].m_fO, TEMPSPAWN_DEAD_DESPAWN, 0))
                    {
                        poison->GetMotionMaster()->MovePoint(0, livingPoisonPositions[i + 3].m_fX,
                                                             livingPoisonPositions[i + 3].m_fY,
                                                             livingPoisonPositions[i + 3].m_fZ);
    ...
    if (m_sapphironSpawnTimer)
    {
        ...
        {
            if (Player *player = GetPlayerInMap())
                player->SummonCreature(NPC_SAPPHIRON, sapphironPositions[0], sapphironPositions[1],
                                       sapphironPositions[2], sapphironPositions[3], TEMPSPAWN_DEAD_DESPAWN, 0);
...
// Initialize all triggers used in Gothik the Harvester encounter by flagging
// them with their position in the room and what kind of NPC they will summon
void instance_naxxramas::InitializeGothikTriggers()
{
    ...
    if (!summonList.empty())
    {
        ...
        script_error_log("No suitable summon trigger found for Gothik combat area. "
                         "Set up failed.");
}
...
// Right is right side from gothik (eastern), i.e. right is living and left is
// spectral
...
// Used in Gluth fight: move all spawned Zombie Chow towards Gluth to be
// devoured
bool instance_naxxramas::DoHandleEvent(uint32 eventId)
{
            ...
            for (auto &zombieGuid : m_zombieChowList)
            {
                if (Creature *zombie = instance->GetCreature(zombieGuid))
                {
                    if (zombie->IsAlive())
                    {
                        ...
                        zombie->GetMotionMaster()->MovePoint(0, gluth->GetPositionX(), gluth->GetPositionY(),
                                                             gluth->GetPositionZ());
                    }
                }
            }
    ...
    case EVENT_CLEAR_SHACKLES:
        m_shackledGuardians = 0;
        m_checkGuardiansTimer = 2 * IN_MILLISECONDS; // Check every two seconds how many Guardians
                                                     // of Icecrown are shackled
        return true;
...
struct npc_living_poisonAI : public ScriptedAI
{
    ...
    // Any time a player comes close to the Living Poison, it will explode and
    // kill itself while doing heavy AoE damage to the player
...
bool instance_naxxramas::DoHandleAreaTrigger(AreaTriggerEntry const *areaTrigger)
{
    ...
    {
        // Area trigger handles teleport in DB. Here we only need to check if all
        // the end wing encounters are done
