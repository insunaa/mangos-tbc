...
struct boss_ossirianAI : public CombatAI
{
    ...
    void Aggro(Unit * /*who*/) override
    {
        ...
        for (auto aSandVortexSpawnPo : aSandVortexSpawnPos)
            m_creature->SummonCreature(NPC_SAND_VORTEX, aSandVortexSpawnPo[0], aSandVortexSpawnPo[1],
                                       aSandVortexSpawnPo[2], aSandVortexSpawnPo[3], TEMPSPAWN_CORPSE_DESPAWN, 0);
    ...
    void DoSpawnNextCrystal(uint32 spawnCount)
    {
        ...
        {
            // iterate from random roll until either one (should always occur) is
            // found or we run out of crystals
    ...
    void JustSummoned(Creature *summoned) override
    {
        ...
        {
            // The movement of this isn't very clear - may require additional
            // research
            summoned->CastSpell(summoned, SPELL_SAND_STORM, TRIGGERED_OLD_TRIGGERED);
            summoned->GetMotionMaster()->MoveRandomAroundPoint(aCrystalSpawnPos[0], aCrystalSpawnPos[1],
                                                               aCrystalSpawnPos[2], 100.0f);
        }
    }
