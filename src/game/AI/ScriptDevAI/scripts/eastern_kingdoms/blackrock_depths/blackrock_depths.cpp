...
bool GOUse_go_bar_beer_keg(Player * /*pPlayer*/, GameObject *pGo)
{
    if (ScriptedInstance *pInstance = (ScriptedInstance *)pGo->GetInstanceData())
    {
        if (pInstance->GetData(TYPE_HURLEY) == IN_PROGRESS ||
            pInstance->GetData(TYPE_HURLEY) == DONE) // GOs despawning on use, this check should never be
                                                     // true but this is proper to have it there
            return false;
        // Every time we set the event to SPECIAL, the instance script increments
        // the number of broken kegs, capping at 3
...
bool AreaTrigger_at_shadowforge_bridge(Player *pPlayer, AreaTriggerEntry const * /*pAt*/)
{
    if (instance_blackrock_depths *pInstance = (instance_blackrock_depths *)pPlayer->GetInstanceData())
    {
                                                                 ...
                                                                 aGuardSpawnPositions[0][3], TEMPSPAWN_DEAD_DESPAWN, 0))
        {
            ...
            if (Creature *pSlaveGuard = pPyromancer->SummonCreature(
                    NPC_ANVILRAGE_GUARDMAN, aGuardSpawnPositions[1][0], aGuardSpawnPositions[1][1],
                    aGuardSpawnPositions[1][2], aGuardSpawnPositions[1][3], TEMPSPAWN_DEAD_DESPAWN, 0))
            {
    ...
    void WaypointReached(uint32 uiPointId) override
    {
        ...
        case 6:
            if (m_pInstance)
            {
                ...
                // debug_log("SD2: npc_grimstone: event reached end and set
                // complete.");
            }
            break;
        }
...
struct npc_phalanxAI : public npc_escortAI
{
    ...
    void Reset() override
    {
        // If reset after an fight, it means Phalanx has already started moving (if
        // not already reached door) so we made him restart right before reaching
        // the door to guard it (again)
    ...
    void WaypointReached(uint32 uiPointId) override
    {
        ...
        case 2:
            SetEscortPaused(true);
            // There are two ways of activating Phalanx: completing Rocknot event,
            // making Phalanx hostile to anyone killing Plugger making Phalanx
            // hostile to Horde (do not ask why) In the later case, Phalanx should
            // also spawn the bar patrol with some delay and only then set the
            // Plugger event to DONE. In the weird case where Plugger was
            // previously killed (event == DONE) but Phalanx is reactivated
...
struct npc_mistress_nagmaraAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        switch (m_uiPhase)
        {
        case 0: // Phase 0 : Nagmara patrols in the bar to serve patrons or is
                // following Rocknot passively
            break;
        ...
        case 2: // Phase 2 : Nagmara is "seducing" Rocknot
            ...
            m_uiPhase++;
            break;
        case 3: // Phase 3: Nagmara give potion to Rocknot and Rocknot escort AI
                // will handle the next part of the event
        ...
        case 4: // Phase 4 : make the lovers face each other
            ...
            break;
        case 5: // Phase 5 : Nagmara and Rocknot are under the stair kissing
                // (this phase repeats endlessly)
...
struct npc_rocknotAI : public npc_escortAI
{
    ...
    void WaypointReached(uint32 uiPointId) override
    {
        ...
        switch (uiPointId)
        {
        case 1: // if Nagmara and Potion of Love event is in progress, switch to
                // second part of the escort
            SetEscortPaused(true);
        ...
        case 9: // Back home stop here
            ...
            break;
        case 10: // This step is the start of the "alternate" waypoint path used
                 // with Nagmara
        ...
        case 17:
            ...
            if (pNagmara)
                pNagmara->GetMotionMaster()->MoveFollow(m_creature, 2.0f, 0);
            break;
        case 34: // Reach under the stair, make Nagmara move to her position and
                 // give the handle back to Nagmara AI script
            if (!pNagmara)
                break;

            pNagmara->GetMotionMaster()->MoveIdle();
            pNagmara->GetMotionMaster()->MovePoint(0, aPosNagmaraRocknot[0], aPosNagmaraRocknot[1],
                                                   aPosNagmaraRocknot[2]);
    ...
    void UpdateEscortAI(const uint32 uiDiff) override
    {
        ...
        // When Nagmara is in Potion of Love event and reach Rocknot, she set
        // TYPE_NAGMARA to SPECIAL in order to make Rocknot start the second part
        // of his escort quest
        ...
        if (m_uiBreakDoorTimer)
        {
            if (m_uiBreakDoorTimer <= uiDiff)
            {
                ...
                DoGo(GO_BAR_KEG_TRAP,
                     GO_STATE_ACTIVE); // doesn't work very well,
        ...
        // Several times Rocknot is supposed to perform an action (text, spell
        // cast...) followed closely by an emote we handle it here
        if (m_uiEmoteTimer)
        {
            if (m_uiEmoteTimer <= uiDiff)
            {
                // If event is SPECIAL (Rocknot moving to barrel), then we want him
                // to say a special text and start moving if not, he is still
                // accepting beers, so we want him to cheer player
...
bool QuestRewarded_npc_rocknot(Player *pPlayer, Creature *pCreature, Quest const *pQuest)
{
    ...
    if (pQuest->GetQuestId() == QUEST_ALE)
    {
        ...
        // We keep track of amount of beers given in the instance script by
        // setting data to SPECIAL Once the correct amount is reached, the script
        // will also returns SPECIAL, if not, it returns IN_PROGRESS/DONE the
        // return state and the following of the script are handled in the
        // Update->emote part of the Rocknot NPC escort AI script
...
struct npc_hurley_blackbreathAI : public npc_escortAI
{
    ...
    void Reset() override
    {
        // If reset after an fight, we made him move to the keg room (end of the
        // path)
        if (HasEscortState(STATE_ESCORT_ESCORTING) || HasEscortState(STATE_ESCORT_PAUSED))
        {
    ...
    void WaypointReached(uint32 uiPointId) override
    {
        ...
        case 6: {
            SetEscortPaused(true);
            // Make Hurley and his cronies able to attack players (and be
            // attacked)
...
struct boss_plugger_spazzringAI : public ScriptedAI
{
    ...
    void JustDied(Unit * /*pKiller*/) override
    {
        ...
        m_pInstance->SetData(TYPE_PLUGGER,
                             IN_PROGRESS); // The event is set IN_PROGRESS even if Plugger is dead
                                           // because his death triggers more actions that are part
                                           // of the event
    }
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (m_creature->SelectHostileTarget() && m_creature->GetVictim())
        {
            ...
            {
                if (Unit *pTarget = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0,
                                                                      SPELL_CURSE_OF_TONGUES, SELECT_FLAG_POWER_MANA))
                {
...
bool GOUse_go_bar_ale_mug(Player *pPlayer, GameObject *pGo)
{
    if (ScriptedInstance *pInstance = (ScriptedInstance *)pGo->GetInstanceData())
    {
        if (pInstance->GetData(TYPE_PLUGGER) == IN_PROGRESS ||
            pInstance->GetData(TYPE_PLUGGER) == DONE) // GOs despawning on use, this check should never be
                                                      // true but this is proper to have it there
            return false;
        if (Creature *pPlugger = pInstance->GetSingleCreatureFromStorage(NPC_PLUGGER_SPAZZRING))
        {
            ...
            {
                // Every time we set the event to SPECIAL, the instance script
                // increments the number of stolen mugs/boars, capping at 3
                pInstance->SetData(TYPE_PLUGGER, SPECIAL);
                // If the cap is reached the instance script changes the type
                // from SPECIAL to IN_PROGRESS Plugger then aggroes and engage
                // players, else he just warns them
                if (pInstance->GetData(TYPE_PLUGGER) == IN_PROGRESS)
