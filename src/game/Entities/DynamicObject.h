...
class DynamicObject : public WorldObject
{
  public:
    ...
    bool Create(uint32 guidlow, Unit *caster, uint32 spellId, SpellEffectIndex effIndex, float x, float y, float z,
                int32 duration, float radius, DynamicObjectType type, SpellTarget target, int32 damage,
                int32 basePoints);
    ...
    {
        return std::max(int64((m_aliveTime - GetMap()->GetCurrentClockTime()).count()), int64(0));
    }
    ...
    int32 const &GetDamage() const
    {
        return m_damage;
    }
    int32 const &GetBasePoints() const
    {
        return m_basePoints;
    }
    ...
    {
        return 0.f;
    } // dynamic object not have real interact size
