...
void AuctionHouseBot::Initialize()
{
    ...
    {
        // set buy/sell chance to 0, this prevents Update() from accessing
    ...
    if (m_chanceSell > 0 || m_chanceBuy > 0)
    {
        ...
        FillUintVectorFromQuery("SELECT entry FROM creature_template WHERE `rank` = 0 AND entry IN "
                                "(SELECT entry FROM creature_loot_template)",
                                m_creatureLootNormalTemplates);
        FillUintVectorFromQuery("SELECT entry FROM creature_template WHERE `rank` = 1 AND entry IN "
                                "(SELECT entry FROM creature_loot_template)",
                                m_creatureLootEliteTemplates);
        FillUintVectorFromQuery("SELECT entry FROM creature_template WHERE `rank` = 2 AND entry IN "
                                "(SELECT entry FROM creature_loot_template)",
                                m_creatureLootRareEliteTemplates);
        FillUintVectorFromQuery("SELECT entry FROM creature_template WHERE `rank` = 3 AND entry IN "
                                "(SELECT entry FROM creature_loot_template)",
                                m_creatureLootWorldBossTemplates);
        FillUintVectorFromQuery("SELECT entry FROM creature_template WHERE `rank` = 4 AND entry IN "
                                "(SELECT entry FROM creature_loot_template)",
        ...
        FillUintVectorFromQuery("SELECT DISTINCT entry FROM gameobject_loot_template WHERE entry IN "
                                "(SELECT data1 FROM gameobject_template WHERE entry IN (SELECT id "
                                "FROM "
                                "gameobject WHERE state = 1 AND spawntimesecsmax > 0))",
        ...
        // profession items (different than the loot above, but use similar
        // config)
        ParseLootConfig("AuctionHouseBot.Items.Profession", m_professionItemsConfig);
        FillUintVectorFromQuery("SELECT entry FROM item_template WHERE entry IN "
                                "(SELECT EffectItemType1 FROM spell_template WHERE "
                                "attributes & 32 AND attributes & 65536)",
                                m_professionItems);

        // vendor items (used to prevent items being bought from vendor and sold
        // at ah for profit)
        ...
        {
            sLog.outError("AHBot error: AuctionHouseBot.Bid.Min must be less or equal to "
                          "AuctionHouseBot.Bid.Max. Setting Bid.Min equal to Bid.Max.");
        ...
        {
            sLog.outError("AHBot error: AuctionHouseBot.Time.Min must be less or equal to "
                          "AuctionHouseBot.Time.Max. Setting Time.Min equal to Time.Max.");
        ...
        // overridden items
        QueryResult *result = CharacterDatabase.PQuery("SELECT item, value, add_chance, min_amount, "
                                                       "max_amount FROM ahbot_items");
        if (result)
        {
            do
            {
...
void AuctionHouseBot::Update()
{
    ...
    if (m_houseAction < MAX_AUCTION_HOUSE_TYPE && urand(0, 99) < m_chanceSell)
    {
        ...
        AddLootToItemMap(&LootTemplates_Skinning, m_skinningLootConfig, m_skinningLootTemplates,
                         itemMap); // skinning loot
            ...
            if (maxTemplates > 0)
            {
                for (int32 templateCounter = 0; templateCounter < maxTemplates; ++templateCounter)
                {
                    ...
                    if (!prototype || prototype->Quality == 0 || urand(0, (1 << (prototype->Quality - 1)) - 1) > 0)
                        continue; // make it decreasingly likely that crafted items
                                  // of higher quality is added to the auction
                                  // house (white: 100%, green: 50%, blue: 25%,
                                  // purple: 12.5%, ...)
        ...
        // remove items we've overridden (AddChance > 0) and add using given
        // AddChance and stack size
        for (auto &itemData : m_itemData)
        {
            if (itemData.second.AddChance > 0) // replace normal loot sources with custom chance of adding
                                               // item
                itemMap[itemData.first] = urand(0, 99) < itemData.second.AddChance
                                              ? urand(itemData.second.MinAmount, itemData.second.MaxAmount)
                                              : 0;
        }
        ...
        for (auto &itemEntry : itemMap)
        {
            ...
            uint32 itemValue = ValueWithVariance(iterator != m_itemData.end() ? iterator->second.Value
                                                                              : CalculateBuyoutPrice(prototype));
            for (uint32 stackCounter = 0; stackCounter < itemEntry.second; stackCounter += prototype->GetMaxStackSize())
            {
                uint32 count = itemEntry.second - stackCounter > prototype->GetMaxStackSize()
                ...
                if (item)
                    auctionHouse->AddAuction(
                        sAuctionHouseStore.LookupEntry(
                            houseType == AUCTION_HOUSE_ALLIANCE ? 1 : (houseType == AUCTION_HOUSE_HORDE ? 6 : 7)),
    ...
    else if (m_houseAction >= MAX_AUCTION_HOUSE_TYPE && urand(0, 99) < m_chanceBuy)
    {
        ...
        for (AuctionHouseObject::AuctionEntryMap::const_iterator itr = bounds.first; itr != bounds.second; ++itr)
        {
            ...
            if (auction->owner == 0 && auction->bid == 0)
                continue; // ignore bidding/buying auctions that were created by
                          // ahbot and not bidded on by player
            ...
            uint32 buyItemCheck = ValueWithVariance(iterator != m_itemData.end() ? iterator->second.Value
                                                                                 : CalculateBuyoutPrice(prototype));
            ...
            if (auction->buyout > 0 && buyItemCheck > auction->buyout)
                buyoutAuctions.push_back(auction); // can't buyout item here as that modifies the
                                                   // AuctionEntryMap, invalidating the iterator
...
void AuctionHouseBot::Rebuild(bool all)
{
    ...
    // refill auction house with items, simulating typical max amount of items
...
void AuctionHouseBot::SetItemData(uint32 item, AuctionHouseBotItemData &itemData, bool reset)
{
    ...
    stmt = CharacterDatabase.CreateStatement(addItem, "INSERT INTO ahbot_items (item, value, add_chance, min_amount, "
                                                      "max_amount) VALUES (?, ?, ?, ?, ?)");
    stmt.addUInt32(item);
...
AuctionHouseBotItemData AuctionHouseBot::GetItemData(uint32 item)
{
    ...
    // item data not overridden, set MinAmount/MaxAmount to 0 (those values can
    // normally never be 0) as a means to alert requester that item data is not
    // overridden
    AuctionHouseBotItemData itemData;
    ItemPrototype const *prototype = ObjectMgr::GetItemPrototype(item);
...
void AuctionHouseBot::ParseLootConfig(char const *fieldname, std::vector<int32> &lootConfig)
{
    ...
    if (lootConfig.size() > 4)
        sLog.outError("AHBot error: Too many values specified for field %s (%lu), "
                      "4 values required. Additional values ignored.",
                      fieldname, lootConfig.size());
    else if (lootConfig.size() < 4)
    {
        sLog.outError("AHBot error: Too few values specified for field %s (%lu), 4 "
                      "values required. Setting 0 for remaining values.",
                      fieldname, lootConfig.size());
    ...
    for (uint32 index = 1; index < 4; ++index)
    {
        ...
        {
            sLog.outError("AHBot error: %s value (%d) for field %s should not be a "
                          "negative number, setting value to 0.",
    ...
    {
        sLog.outError("AHBot error: First value (%d) must be less than or equal to second "
                      "value (%d) for field %s. Setting first value to second value.",
                      lootConfig[0], lootConfig[1], fieldname);
        lootConfig[0] = lootConfig[1];
    }
    if (lootConfig[2] > lootConfig[3])
    {
        sLog.outError("AHBot error: Third value (%d) must be less than or equal to fourth "
                      "value (%d) for field %s. Setting third value to fourth value.",
...
void AuctionHouseBot::AddLootToItemMap(LootStore *store, std::vector<int32> &lootConfig,
                                       std::vector<uint32> &lootTemplates, std::unordered_map<uint32, uint32> &itemMap)
{
    if (lootConfig[1] <= 0 || lootConfig[3] <= 0 || lootTemplates.size() <= 0)
        return;
    int32 maxTemplates = lootConfig[0] < 0 ? urand(0, lootConfig[1] - lootConfig[0]) + lootConfig[0]
                                           : urand(lootConfig[0], lootConfig[1]);
    if (maxTemplates <= 0)
        return;
    for (int32 templateCounter = 0; templateCounter < maxTemplates; ++templateCounter)
    {
...
uint32 AuctionHouseBot::CalculateBuyoutPrice(ItemPrototype const *prototype)
{
    ...
    // test whether we have a buy price or if buy price greatly exceed sell price
    // (causes item to be valued too high, notably arrows/shells do this) if
    // using sell price then price must be multiplied by 4 if white or gray item,
    // 5 if green or better to match expected buy price
    ...
    // if item is sold by a vendor and vendor value is forced, then multiply by
    // 100 (setting vendor price)
