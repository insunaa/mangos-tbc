...
class GenericTransport : public GameObject
{
  public:
    ...
    static void CalculatePassengerOffset(float &x, float &y, float &z, float *o, float transX, float transY,
                                         float transZ, float transO);
  ...
  protected:
    ...
    uint32 m_pathProgress; // for MO transport its full time since start for
                           // normal time in cycle
...
class Transport : public GenericTransport
{
  public:
    ...
    {
        return m_transportTemplate.keyFrames;
    }
