...
enum ConditionType
{
    ...
    CONDITION_QUESTTAKEN = 9,          // quest_id     0,1,2   for condition true while quest active (0 any
                                       // state, 1 if quest incomplete, 2 if quest completed).
    CONDITION_AD_COMMISSION_AURA = 10, // 0            0,      for condition true
    // while one from AD commission aura active
    CONDITION_UNUSED_1 = 11,
    CONDITION_ACTIVE_GAME_EVENT = 12, // event_id     0
    CONDITION_AREA_FLAG = 13,         // area_flag    area_flag_not
    CONDITION_RACE_CLASS = 14,        // race_mask    class_mask
    CONDITION_LEVEL = 15,             // level        0, 1 or 2 (0: equal to, 1: equal or
    ...
    CONDITION_QUESTAVAILABLE = 19,  // quest_id     0       for case when loot/gossip possible only if
    ...
    // SkillLineAbility). Item_id can be defined in addition, to check if
    // player has one (1) item in inventory or bank. When player has spell
    // or has item (when defined), condition return false.
    CONDITION_SKILL_BELOW = 29, // skill_id     skill_value
    // True if player has skill skill_id and skill less than (and not
    // equal) skill_value (for skill_value > 1) If skill_value == 1, then
    // true if player has not skill skill_id
    CONDITION_REPUTATION_RANK_MAX = 30, // faction_id   max_rank
    CONDITION_COMPLETED_ENCOUNTER = 31, // encounter_id encounter_id2       encounter_id[2] =
                                        // DungeonEncounter(dbc).id (if value2 provided it will return value1
                                        // OR value2)
    CONDITION_UNUSED_6 = 32,
    CONDITION_LAST_WAYPOINT = 33, // waypointId   0 = exact, 1: wp <= waypointId,
                                  // 2: wp > waypointId Use
                                  // to check what waypoint was last reached
    CONDITION_RESERVED_4 = 34,    // reserved for 3.x and later
    CONDITION_GENDER = 35,        // 0=male, 1=female, 2=none (see enum Gender)
    CONDITION_DEAD_OR_AWAY = 36,  // value1: 0=player dead, 1=player is dead (with
                                  // group dead), 2=player
    // in instance are dead, 3=creature is dead value2: if != 0 only
    // consider players in range of this value
    CONDITION_CREATURE_IN_RANGE = 37, // value1: creature entry; value2: range;
                                      // returns only alive creatures
...
enum ConditionSource // From where was the condition called?
{
    ...
    CONDITION_FROM_TRAINER = 10,              // Used to check a condition from npc_trainer
                                              // and npc_trainer_template
