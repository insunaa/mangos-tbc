...
void OutdoorPvPEP::HandlePlayerEnterZone(Player *player, bool isMainZone)
{
    ...
    case ALLIANCE:
        if (m_towersAlliance > 0)
            player->CastSpell(player, plaguelandsTowerBuffs[m_towersAlliance - 1].spellIdAlliance,
...
{
    // Remove existing buff for both teams before applying new one: this prevent
    // auras stacking
    if (m_towersAlliance != 0)
    ...
    {
        // update banner
        for (GuidList::const_iterator itr = m_towerBanners[towerId].begin(); itr != m_towerBanners[towerId].end();
             ++itr)
    ...
    // Update Echoes of Lordaeron aura for capturing team and opposite team (if
    // needed) Apply auras with updated capture towers values
    if (m_towersAlliance != 0)
    ...
    else
    {
        ...
        case TOWER_ID_NORTHPASS:
            RespawnGO(go,
                      m_towerOwner[TOWER_ID_NORTHPASS] == ALLIANCE ? m_lordaeronShrineAlliance : m_lordaeronShrineHorde,
                      false);
            break;
        case TOWER_ID_CROWNGUARD:
            SetGraveYardLinkTeam(GRAVEYARD_ID_EASTERN_PLAGUE, GRAVEYARD_ZONE_EASTERN_PLAGUE, TEAM_INVALID, 0);
