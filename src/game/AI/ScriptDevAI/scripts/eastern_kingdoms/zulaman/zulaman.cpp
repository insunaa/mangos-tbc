...
instance_zulaman::instance_zulaman(Map *map)
    : ScriptedInstance(map), m_startCheck(false), m_uiEventTimer(MINUTE * IN_MILLISECONDS), m_uiBearEventPhase(0),
      m_isBearPhaseInProgress(false), m_spiritFadeTimer(0)
{
    Initialize();
}
...
void instance_zulaman::SetData(uint32 type, uint32 data)
{
    ...
    case TYPE_RAND_VENDOR_2:
        ...
        break;
    default:
        script_error_log("Instance Zulaman: ERROR SetData = %u for type %u does "
...
void instance_zulaman::SendNextBearWave(Unit * /*target*/)
{
    ...
    for (auto itr : m_nalorakkEvent[m_uiBearEventPhase].nalorakkTrashSet)
    {
        ...
        if (temp && temp->IsAlive())
        {
            ...
            if (sWaypointMgr.GetDefaultPath(0, temp->GetGUIDLow(),
                                            nullptr)) // mob in each wave has WPs
            {
                mainGuy = temp;
                temp->SetWalk(false);
    ...
    if (mainGuy)
        for (Creature *follower : followers)
            follower->GetMotionMaster()->MoveFollow(mainGuy,
                                                    follower->GetDistance(mainGuy, true, DIST_CALC_COMBAT_REACH),
                                                  ...
                                                  WorldObject const *conditionSource, uint32 conditionSourceType) const
{
    ...
    script_error_log("instance_zulaman::CheckConditionCriteriaMeet called with unsupported "
                     "Id "
                     "%u. Called with param plr %s, src %s, condition source type %u",
...
void instance_zulaman::DoChestEvent(BossToChestIndex uiIndex)
{
    ...
    {
        creature->AI()->SendAIEvent(AI_EVENT_CUSTOM_A, creature,
                                    creature); // yell for help
...
void instance_zulaman::Update(uint32 diff)
{
    if (m_auiEncounter[TYPE_EVENT_RUN] == IN_PROGRESS)
    {
        if (m_uiEventTimer <= diff)
        {
            ...
            {
                debug_log("SD2: Instance Zulaman: event time reach end, event "
                          "failed.");
