...
struct npc_private_hendelAI : public ScriptedAI
{
    ...
    void SpellHit(Unit * /*pCaster*/, const SpellEntry *pSpell) override
    {
        ...
        // this means it is time to grant quest credit to the player previously
        // stored to this intend
        ...
        if (damage > m_creature->GetHealth() || m_creature->GetHealthPercent() < 20.0f)
        {
            if (Player *pPlayer = dealer->GetBeneficiaryPlayer())
            {
                if (pPlayer->GetQuestStatus(QUEST_MISSING_DIPLO_PT16) == QUEST_STATUS_INCOMPLETE)
                    guidPlayer = pPlayer->GetObjectGuid(); // Store the player to give
                                                           // quest credit later
            }
            ...
            for (CreatureList::const_iterator itr = lSentryList.begin(); itr != lSentryList.end(); ++itr)
            {
                if ((*itr)->IsAlive())
                {
                    ...
                    (*itr)->CombatStop(true);
                    (*itr)->SetWalk(false);
                    (*itr)->GetMotionMaster()->MovePoint(0, fSentryFleePoint[0], fSentryFleePoint[1],
                                                         fSentryFleePoint[2]);
            ...
            for (const auto &lOutroSpawn : lOutroSpawns)
            {
                Creature *pCreature = m_creature->SummonCreature(
                    lOutroSpawn.uiEntry, lOutroSpawn.fX, lOutroSpawn.fY, lOutroSpawn.fZ, lOutroSpawn.fO,
                    TEMPSPAWN_TIMED_DESPAWN, 3 * MINUTE * IN_MILLISECONDS, false, true);
                if (pCreature)
                {
                    pCreature->CastSpell(pCreature, SPELL_TELEPORT_VISUAL, TRIGGERED_NONE);
                    pCreature->GetMotionMaster()->MovePoint(0, lOutroSpawn.fDestX, lOutroSpawn.fDestY,
                                                            lOutroSpawn.fDestZ);

                    // Exception case for Archmage Tervosh: the outro event is a
                    // simple speech with visual spell cast so it will be handled
                    // by a DBScript held by NPC Archmage Tervosh
                    if (pCreature->GetEntry() == NPC_TERVOSH)
                    {
                        // Remove Gossip and Quest Giver flag from now, they will
                        // be re-added later to Archmage Tervosh in DBScript
...
bool QuestAccept_npc_private_hendel(Player *pPlayer, Creature *pCreature, const Quest *pQuest)
{
    if (pQuest->GetQuestId() == QUEST_MISSING_DIPLO_PT16)
    {
        ...
        // The two sentries are linked to Private Hendel in DB to ensure they
        // respawn together
        CreatureList lSentryList;
        ...
        for (CreatureList::const_iterator itr = lSentryList.begin(); itr != lSentryList.end(); ++itr)
        {
            if ((*itr)->IsAlive())
            {
                (*itr)->SetFactionTemporary(FACTION_HOSTILE,
        ...
        case 40:
            if (Player *pPlayer = GetPlayerForEscort())
            {
                pPlayer->RewardPlayerAndGroupAtEventExplored(
                    pPlayer->GetTeam() == ALLIANCE ? QUEST_ID_STINKYS_ESCAPE_ALLIANCE : QUEST_ID_STINKYS_ESCAPE_HORDE,
                    m_creature);
...
{
    if (pQuest->GetQuestId() == QUEST_ID_STINKYS_ESCAPE_ALLIANCE ||
        pQuest->GetQuestId() == QUEST_ID_STINKYS_ESCAPE_HORDE)
    ...
    void MovementInform(uint32 uiMotionType, uint32 uiPointId) override
    {
        if (uiMotionType == WAYPOINT_MOTION_TYPE)
        {
            ...
            case 15: {
                ...
                for (std::list<GameObject *>::const_iterator itr = lCannonsInRange.begin();
                     itr != lCannonsInRange.end(); ++itr)
                {
                ...
                for (auto itr = m_lMarksmenGUIDs.begin(); itr != m_lMarksmenGUIDs.end(); ++itr)
                {
                    if (urand(0, chance--) == 0)
                        if (Creature *target = m_creature->GetMap()->GetCreature((*itr)))
                            ...
                            {
                                // mimic boss turning because of the missing threat
                                // system
...
{
    npc_smolderwing(Creature *pCreature) : ScriptedAI(pCreature), DialogueHelper(SmolderDialogue)
    {
        Reset();
    }
