...
void instance_temple_of_ahnqiraj::OnCreatureCreate(Creature *creature)
{
    ...
    case NPC_EYE_OF_CTHUN:
        // Safeguard for C'Thun encounter in case of fight abruptly ended during
        // phase 2
...
void instance_temple_of_ahnqiraj::SetData(uint32 uiType, uint32 uiData)
{
    ...
    case TYPE_BUG_TRIO:
        ...
        if (uiData == FAIL)
        {
            ...
            // On encounter failure, despawn Vem and Princess Yauj and make Lord
            // Kri evade the two other bosses will respawn when Lord Kri reaches
            // home/respawns
            if (Creature *vem = GetSingleCreatureFromStorage(NPC_VEM))
                vem->ForcedDespawn();
            if (Creature *yauj = GetSingleCreatureFromStorage(NPC_YAUJ))
...
void instance_temple_of_ahnqiraj::Update(uint32 uiDiff)
{
    ...
    if (m_uiCthunWhisperTimer < uiDiff)
    {
        if (Player *pPlayer = GetPlayerInMap())
        {
            ...
            {
                // ToDo: also cast the C'thun Whispering charm spell - requires
                // additional research
                switch (urand(0, 7))
                {
                case 0:
