...
enum ChatNotify : uint8
{
    ...
    CHAT_THROTTLED_NOTICE = 0x1F,             //+ "[%s] The number of messages that can be sent to this channel
                                              // is
                                              // limited, please wait to send another message.";
    CHAT_NOT_IN_AREA_NOTICE = 0x20,           //+ "[%s] You are not in the correct area for this channel."; --
                                              // The
...
class Channel
{
  ...
  private:
    struct PlayerInfo
    {
        ...
        inline bool HasFlag(uint8 flag) const
        {
            return (flags & flag) != 0;
        }
        ...
        {
            return HasFlag(MEMBER_FLAG_OWNER);
        }
        inline void SetOwner(bool state)
        {
            SetFlag(MEMBER_FLAG_OWNER, state);
        }
        inline bool IsModerator() const
        {
            return HasFlag(MEMBER_FLAG_MODERATOR);
        }
        inline void SetModerator(bool state)
        {
            SetFlag(MEMBER_FLAG_MODERATOR, state);
        }
        inline bool IsMuted() const
        {
            return HasFlag(MEMBER_FLAG_MUTED);
        }
        inline void SetMuted(bool state)
        {
            SetFlag(MEMBER_FLAG_MUTED, state);
        }
    };
  ...
  public:
    ...
    {
        return (m_entry ? m_entry->ChannelID : 0);
    }
    const ChatChannelsEntry *GetChannelEntry() const
    {
        return m_entry;
    }
    ...
    bool IsLFG() const
    {
        return (GetFlags() & CHANNEL_FLAG_LFG) != 0;
    }
    ...
    static void MakeJoined(WorldPacket &data, const std::string &channel,
                           const ObjectGuid &guid); //+ 0x00
    static void MakeLeft(WorldPacket &data, const std::string &channel,
                         const ObjectGuid &guid); //+ 0x01
    static void MakeYouJoined(WorldPacket &data, const std::string &channel,
                              const Channel &which); //+ 0x02
    static void MakeYouLeft(WorldPacket &data, const std::string &channel,
                            const Channel &which); //+ 0x03
    static void MakeWrongPassword(WorldPacket &data,
                                  const std::string &channel); //? 0x04
    static void MakeNotMember(WorldPacket &data,
                              const std::string &channel); //? 0x05
    static void MakeNotModerator(WorldPacket &data,
                                 const std::string &channel); //? 0x06
    static void MakePasswordChanged(WorldPacket &data, const std::string &channel,
                                    const ObjectGuid &guid); //+ 0x07
    static void MakeOwnerChanged(WorldPacket &data, const std::string &channel,
                                 const ObjectGuid &guid); //? 0x08
    static void MakePlayerNotFound(WorldPacket &data, const std::string &channel,
                                   const std::string &name); //+ 0x09
    static void MakeNotOwner(WorldPacket &data,
                             const std::string &channel); //? 0x0A
    static void MakeChannelOwner(WorldPacket &data, const std::string &channel,
                                 const ObjectGuid &guid); //? 0x0B
    static void MakeModeChange(WorldPacket &data, const std::string &channel, const ObjectGuid &guid, uint8 oldflags,
                               uint8 newFlags); //+ 0x0C
    static void MakeAnnouncementsOn(WorldPacket &data, const std::string &channel,
                                    const ObjectGuid &guid); //+ 0x0D
    static void MakeAnnouncementsOff(WorldPacket &data, const std::string &channel,
                                     const ObjectGuid &guid); //+ 0x0E
    static void MakeModerationOn(WorldPacket &data, const std::string &channel,
                                 const ObjectGuid &guid); //+ 0x0F
    static void MakeModerationOff(WorldPacket &data, const std::string &channel,
                                  const ObjectGuid &guid); //+ 0x10
    static void MakeMuted(WorldPacket &data,
                          const std::string &channel); //? 0x11
    ...
    static void MakeBanned(WorldPacket &data,
                           const std::string &channel); //? 0x13
    ...
    static void MakePlayerNotBanned(WorldPacket &data, const std::string &channel,
                                    const std::string &name); //? 0x16
    static void MakePlayerAlreadyMember(WorldPacket &data, const std::string &channel,
                                        const ObjectGuid &guid); //+ 0x17
    static void MakeInvite(WorldPacket &data, const std::string &channel,
                           const ObjectGuid &guid); //? 0x18
    static void MakeInviteWrongFaction(WorldPacket &data,
                                       const std::string &channel); //? 0x19
    static void MakeWrongFaction(WorldPacket &data,
                                 const std::string &channel); //? 0x1A
    static void MakeInvalidName(WorldPacket &data,
                                const std::string &channel); //? 0x1B
    static void MakeNotModerated(WorldPacket &data,
                                 const std::string &channel); //? 0x1C
    static void MakePlayerInvited(WorldPacket &data, const std::string &channel,
                                  const std::string &name); //+ 0x1D
    static void MakePlayerInviteBanned(WorldPacket &data, const std::string &channel,
                                       const std::string &name); //? 0x1E
    static void MakeThrottled(WorldPacket &data,
                              const std::string &channel); //? 0x1F
    static void MakeNotInArea(WorldPacket &data,
                              const std::string &channel); //? 0x20
    static void MakeNotInLFG(WorldPacket &data,
                             const std::string &channel); //? 0x21
    static void MakeVoiceOn(WorldPacket &data, const std::string &channel,
                            const ObjectGuid &guid); //+ 0x22
    static void MakeVoiceOff(WorldPacket &data, const std::string &channel,
                             const ObjectGuid &guid); //+ 0x23
