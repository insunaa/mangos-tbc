...
enum WSSpells
{
    ...
    BG_WS_SPELL_BRUTAL_ASSAULT = 46393,  // 100% increased damage to flag carriers
                                         // after 15 minute stalemate
};
...
class BattleGroundWS : public BattleGround
{
  ...
  public:
    ...
    // Flag handler
    ObjectGuid const &GetFlagCarrierGuid(uint8 teamIdx) const
    {
        return m_flagCarrier[teamIdx];
    }

  private:
    // Flag Carrier functions
    void SetFlagCarrier(uint8 teamIdx, ObjectGuid guid)
    {
        m_flagCarrier[teamIdx] = guid;
    }
    void ClearFlagCarrier(uint8 teamIdx)
    {
        m_flagCarrier[teamIdx].Clear();
    }
    bool IsFlagPickedUp(uint8 teamIdx) const
    {
        return !m_flagCarrier[teamIdx].IsEmpty();
    }
    ...
    // Update flag world state; BG_WS_FLAG_STATE_WAIT_RESPAWN (1) for no flag
    // icon; BG_WS_FLAG_STATE_ON_PLAYER (2) for the flag icon
