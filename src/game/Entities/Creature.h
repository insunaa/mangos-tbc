...
{
    CREATURE_EXTRA_FLAG_INSTANCE_BIND = 0x00000001,     // 1 creature kill bind instance with killer and killer's
                                                        // group
    ...
    CREATURE_EXTRA_FLAG_INVISIBLE = 0x00000080,         // 128 creature is always invisible for player (mostly
                                                        // trigger creatures)
    ...
    CREATURE_EXTRA_FLAG_ACTIVE = 0x00001000,             // 4096 creature is active object. Grid of this creature will
                                                         // be loaded and creature set as active
    ...
    CREATURE_EXTRA_FLAG_CIVILIAN = 0x00010000,           // 65536 CreatureInfo->civilian
                                                         // substitute (for new expansions)
    CREATURE_EXTRA_FLAG_NO_MELEE = 0x00020000,           // 131072 creature can't melee
    CREATURE_EXTRA_FLAG_FORCE_ATTACKING_CAPABILITY = 0x00080000, // 524288 SetForceAttackingCapability(true); for
                                                                 // nonattackable, nontargetable creatures that should
                                                                 // be able to attack nontheless
    CREATURE_EXTRA_FLAG_DYNGUID = 0x00100000,            // 1048576 Temporary transition flag - spawns of this entry
                                                         // use dynguid system
...
#define USE_DEFAULT_DATABASE_LEVEL                                                                                     \
    0 // just used to show we don't want to force the new creature level and use
      // the level stored in db
...
struct CreatureInfo
{
    ...
    uint32 Rank;
    int32 Expansion; // creature expansion, important for stats, CAN BE -1 as
                     // marker for some invalid cases.
...
struct CreatureSpawnTemplate
{
    ...
    {
        return (spawnFlags & SPAWN_FLAG_RUN_ON_SPAWN) != 0;
    }
    bool IsHovering() const
    {
        return (spawnFlags & SPAWN_FLAG_HOVER) != 0;
    }
};
...
struct CreatureData
{
    ...
    bool IsNotPartOfPoolOrEvent() const
    {
        return (!gameEvent && !GuidPoolId && !EntryPoolId);
    }
};
...
struct TrainerSpellData
{
    ...
    uint32 trainerType; // trainer type based at trainer spells, can be different
                        // from creature_template value.
...
{
    TEMPFACTION_NONE = 0x00,                    // When no flag is used in temporary faction change, faction will
                                                // be persistent. It will then require manual change back to
                                                // default/another faction when changed once
    TEMPFACTION_RESTORE_RESPAWN = 0x01,         // Default faction will be restored at respawn
    TEMPFACTION_RESTORE_COMBAT_STOP = 0x02,     // ... at CombatStop() (happens at creature death, at evade or
                                                // custom scripte among others)
    TEMPFACTION_RESTORE_REACH_HOME = 0x04,      // ... at reaching home in home movement (evade), if not already
                                                // done at CombatStop()
    TEMPFACTION_TOGGLE_NON_ATTACKABLE = 0x08,   // Remove UNIT_FLAG_SPAWNING(0x02) when faction is changed (reapply
                                                // when temp-faction is removed)
    TEMPFACTION_TOGGLE_IMMUNE_TO_PLAYER = 0x10, // Remove UNIT_FLAG_IMMUNE_TO_PLAYER(0x100) when faction is changed
                                                // (reapply when temp-faction is removed)
    TEMPFACTION_TOGGLE_IMMUNE_TO_NPC = 0x20,    // Remove UNIT_FLAG_IMMUNE_TO_NPC(0x200) when faction is changed
                                                // (reapply when temp-faction is removed)
    TEMPFACTION_TOGGLE_PACIFIED = 0x40,         // Remove UNIT_FLAG_PACIFIED(0x20000) when faction is changed
                                                // (reapply when temp-faction is removed)
    TEMPFACTION_TOGGLE_NOT_SELECTABLE = 0x80,   // Remove UNIT_FLAG_NOT_SELECTABLE(0x2000000) when faction is
                                                // changed (reapply when temp-faction is removed)
    TEMPFACTION_ALL,
};
...
class Creature : public Unit
{
  public:
    ...
    // SelectLevel set creature bases stats for given level or for default levels
    // stored in db
    ...
    {
        return m_creatureInfo->CreatureType == CREATURE_TYPE_CRITTER;
    }

#ifdef BUILD_PLAYERBOT
    // Adds functionality to load/unload bots from NPC, also need to apply SQL
    // scripts
    ...
    TimePoint GetCorpseDecayTimer() const
    {
        return m_corpseExpirationTime;
    }
    ...
    {
        return (GetCreatureInfo()->ExtraFlags & CREATURE_EXTRA_FLAG_CIVILIAN) != 0;
    }
    bool IsNoAggroOnSight() const
    {
        return (GetCreatureInfo()->ExtraFlags & CREATURE_EXTRA_FLAG_NO_AGGRO_ON_SIGHT) != 0;
    }
    bool IsGuard() const
    {
        return (GetCreatureInfo()->ExtraFlags & CREATURE_EXTRA_FLAG_GUARD) != 0;
    }

    bool CanWalk() const
    {
        return (GetCreatureInfo()->InhabitType & INHABIT_GROUND) != 0;
    }
    bool CanSwim() const
    {
        return (GetCreatureInfo()->InhabitType & INHABIT_WATER) != 0;
    }
    bool IsSwimming() const
    {
        return m_movementInfo.HasMovementFlag(MOVEFLAG_SWIMMING);
    }
    ...
    uint32 GetLevelForTarget(
        Unit const *target) const override; // overwrite Unit::GetLevelForTarget for boss level support
    ...
    static float _GetHealthMod(int32 Rank);      ///< Get custom factor to scale health (default
                                                 ///< 1, CONFIG_FLOAT_RATE_CREATURE_*_HP)
    static float _GetDamageMod(int32 Rank);      ///< Get custom factor to scale damage (default
                                                 ///< 1, CONFIG_FLOAT_RATE_*_DAMAGE)
    static float _GetSpellDamageMod(int32 Rank); ///< Get custom factor to scale spell damage (default 1,
                                                 ///< CONFIG_FLOAT_RATE_*_SPELLDAMAGE)
    ...
    CreatureLootStatus GetLootStatus() const
    {
        return m_lootStatus;
    }
    ...
    void SetCorpseAccelerationDelay(uint32 delay)
    {
        m_corpseAccelerationDecayDelay = delay;
    } // in miliseconds
    ...
    {
        return m_lootGroupRecipientId != 0;
    }
    ...
    bool CanCallForAssistance() const override
    {
        return m_canCallForAssistance;
    }
    void SetCanCallForAssistance(bool state)
    {
        m_canCallForAssistance = state;
    }
    ...
    {
        m_gossipMenuId = menuId;
    }
    uint32 GetDefaultGossipMenuId() const
    {
        return m_gossipMenuId;
    }
    uint32 GetInteractionPauseTimer() const
    {
        return m_interactionPauseTimer;
    }
    ...
    {
        return (GetCreatureInfo()->RegenerateStats & REGEN_FLAG_HEALTH) != 0;
    }
    ...
    void SetCombatStartPosition(Position const &pos)
    {
        m_combatStartPos = pos;
    }
    void GetCombatStartPosition(Position &pos) const
    {
        pos = m_combatStartPos;
    }
    Position const &GetCombatStartPosition() const
    {
        return m_combatStartPos;
    }
    ...
    Position const &GetRespawnPosition() const
    {
        return m_respawnPos;
    }
    ...
    uint32 GetTemporaryFactionFlags() const
    {
    ...
    void SetIgnoreMMAP(bool ignore)
    {
        m_ignoreMMAP = ignore;
    }
    bool IsIgnoringMMAP() const
    {
        return m_ignoreMMAP;
    }
    ...
    void SetForceAttackingCapability(bool state)
    {
        m_forceAttackingCapability = state;
    }
    bool GetForceAttackingCapability() const
    {
        return m_forceAttackingCapability;
    }

    void SetSpawnCounting(bool state)
    {
        m_countSpawns = state;
    }

    uint32 GetDetectionRange() const override
    {
        return m_detectionRange;
    }
    void SetDetectionRange(uint32 range)
    {
    ...
    bool CanCheckForHelp() const override
    {
        return m_checkForHelp;
    }
    void SetCanCheckForHelp(bool state)
    {
        m_checkForHelp = state;
    }
    ...
    bool IsNoReputation()
    {
        return m_noReputation;
    }
    void SetNoReputation(bool state)
    {
        m_noReputation = state;
    }
    bool IsIgnoringFeignDeath() const
    {
        return m_ignoringFeignDeath;
    }
    void SetIgnoreFeignDeath(bool state)
    {
        m_ignoringFeignDeath = state;
    }
    ...
    uint32 GetDbGuid() const override
    {
        return m_dbGuid;
    }
    HighGuid GetParentHigh() const override
    {
    ...
    CreatureGroup *GetCreatureGroup() const
    {
        return m_creatureGroup;
    }

    ObjectGuid GetKillerGuid() const
    {
        return m_killer;
    }
    void SetKillerGuid(ObjectGuid guid)
    {
        m_killer = guid;
    }
    ...
    {
        m_noWeaponSkillGain = state;
    }
    bool IsNoWeaponSkillGain() const override
    {
        return m_noWeaponSkillGain;
    }
  ...
  protected:
    ...
    uint32 m_lootMoney;
    ObjectGuid m_lootRecipientGuid;        // player who will have rights for looting if
                                           // m_lootGroupRecipient==0 or group disbanded
    uint32 m_lootGroupRecipientId;         // group who will have rights for looting if
                                           // set and exist
    CreatureLootStatus m_lootStatus;       // loot status (used to know when we could
                                           // loot, pickpocket or skin)
    ...
    uint32 m_interactionPauseTimer; // (msecs) waypoint pause time when
                                    // interacted with
    ...
    bool m_isInvisible;
    bool m_ignoreMMAP;
    bool m_forceAttackingCapability; // can attack even if not selectable/not
                                     // attackable
    bool m_noXP;
    bool m_noLoot;
    bool m_noReputation;
