            ...
            case SAVE_ID_AHN_QIRAJ: {
                if (data.size())
                {
                    ...
                    try
                    {
                        ...
                        {
                            TimePoint timePoint =
                                std::chrono::time_point_cast<std::chrono::milliseconds>(Clock::from_time_t(time));
                            m_aqData.m_timer =
                                std::chrono::duration_cast<std::chrono::milliseconds>(timePoint - curTime).count();
                        }
            ...
            case SAVE_ID_QUEL_DANAS: {
                if (data.size())
                {
                    ...
                    catch (std::exception &e)
                    {
                        ...
                        memset(m_sunsReachData.m_sunsReachReclamationCounters, 0,
                               sizeof(m_sunsReachData.m_sunsReachReclamationCounters));
...
void WorldState::HandleGameObjectUse(GameObject *go, Unit *user)
{
    ...
    case OBJECT_EVENT_TRAP_THRALL: {
        ...
        SendWorldstateUpdate(m_loveIsInTheAirMutex, m_loveIsInTheAirCapitalsPlayers,
                             GetLoveIsInTheAirCounter(LOVE_LEADER_THRALL), WORLD_STATE_LOVE_IS_IN_THE_AIR_THRALL);
        ...
        SendWorldstateUpdate(m_loveIsInTheAirMutex, m_loveIsInTheAirCapitalsPlayers, hordeSum,
                             WORLD_STATE_LOVE_IS_IN_THE_AIR_TOTAL_HORDE);
        break;
    }
    case OBJECT_EVENT_TRAP_CAIRNE: {
        HandleExternalEvent(CUSTOM_EVENT_LOVE_IS_IN_THE_AIR_LEADER, LOVE_LEADER_CAIRNE);
        SendWorldstateUpdate(m_loveIsInTheAirMutex, m_loveIsInTheAirCapitalsPlayers,
                             GetLoveIsInTheAirCounter(LOVE_LEADER_CAIRNE), WORLD_STATE_LOVE_IS_IN_THE_AIR_CAIRNE);
        uint32 hordeSum = GetLoveIsInTheAirCounter(LOVE_LEADER_CAIRNE) + GetLoveIsInTheAirCounter(LOVE_LEADER_THRALL) +
                          GetLoveIsInTheAirCounter(LOVE_LEADER_SYLVANAS);
        SendWorldstateUpdate(m_loveIsInTheAirMutex, m_loveIsInTheAirCapitalsPlayers, hordeSum,
                             WORLD_STATE_LOVE_IS_IN_THE_AIR_TOTAL_HORDE);
    ...
    case OBJECT_EVENT_TRAP_SYLVANAS: {
        ...
        SendWorldstateUpdate(m_loveIsInTheAirMutex, m_loveIsInTheAirCapitalsPlayers,
                             GetLoveIsInTheAirCounter(LOVE_LEADER_SYLVANAS), WORLD_STATE_LOVE_IS_IN_THE_AIR_SYLVANAS);
        uint32 hordeSum = GetLoveIsInTheAirCounter(LOVE_LEADER_CAIRNE) + GetLoveIsInTheAirCounter(LOVE_LEADER_THRALL) +
                          GetLoveIsInTheAirCounter(LOVE_LEADER_SYLVANAS);
        SendWorldstateUpdate(m_loveIsInTheAirMutex, m_loveIsInTheAirCapitalsPlayers, hordeSum,
                             WORLD_STATE_LOVE_IS_IN_THE_AIR_TOTAL_HORDE);
    ...
    case OBJECT_EVENT_TRAP_BOLVAR: {
        ...
        SendWorldstateUpdate(m_loveIsInTheAirMutex, m_loveIsInTheAirCapitalsPlayers,
                             GetLoveIsInTheAirCounter(LOVE_LEADER_BOLVAR), WORLD_STATE_LOVE_IS_IN_THE_AIR_BOLVAR);
        ...
        SendWorldstateUpdate(m_loveIsInTheAirMutex, m_loveIsInTheAirCapitalsPlayers, allianceSum,
                             WORLD_STATE_LOVE_IS_IN_THE_AIR_TOTAL_ALLIANCE);
        break;
    }
    case OBJECT_EVENT_TRAP_MAGNI: {
        HandleExternalEvent(CUSTOM_EVENT_LOVE_IS_IN_THE_AIR_LEADER, LOVE_LEADER_MAGNI);
        SendWorldstateUpdate(m_loveIsInTheAirMutex, m_loveIsInTheAirCapitalsPlayers,
                             GetLoveIsInTheAirCounter(LOVE_LEADER_MAGNI), WORLD_STATE_LOVE_IS_IN_THE_AIR_MAGNI);
        uint32 allianceSum = GetLoveIsInTheAirCounter(LOVE_LEADER_BOLVAR) +
                             GetLoveIsInTheAirCounter(LOVE_LEADER_TYRANDE) +
                             GetLoveIsInTheAirCounter(LOVE_LEADER_MAGNI);
        SendWorldstateUpdate(m_loveIsInTheAirMutex, m_loveIsInTheAirCapitalsPlayers, allianceSum,
    ...
    case OBJECT_EVENT_TRAP_TYRANDE: {
        ...
        SendWorldstateUpdate(m_loveIsInTheAirMutex, m_loveIsInTheAirCapitalsPlayers,
                             GetLoveIsInTheAirCounter(LOVE_LEADER_TYRANDE), WORLD_STATE_LOVE_IS_IN_THE_AIR_TYRANDE);
        uint32 allianceSum = GetLoveIsInTheAirCounter(LOVE_LEADER_BOLVAR) +
                             GetLoveIsInTheAirCounter(LOVE_LEADER_TYRANDE) +
                             GetLoveIsInTheAirCounter(LOVE_LEADER_MAGNI);
        SendWorldstateUpdate(m_loveIsInTheAirMutex, m_loveIsInTheAirCapitalsPlayers, allianceSum,
...
void WorldState::HandlePlayerEnterZone(Player *player, uint32 zoneId)
{
    ...
    if (IsHolidayActive(HOLIDAY_FIRE_FESTIVAL)) // On midsummer starting, players can end up
                                                // without buff but with bonfire in zone -
                                                // keeping like this for simplicity and
                                                // optimization
    {
...
void WorldState::HandlePlayerLeaveZone(Player *player, uint32 zoneId)
{
    ...
    case ZONEID_MAGTHERIDON_LAIR: {
        ...
        if (position != m_magtheridonHeadPlayers.end()) // == myVector.end() means the
    ...
    case ZONEID_ARCATRAZ: {
        ...
        if (position != m_adalSongOfBattlePlayers.end()) // == myVector.end() means the
                                                         // element was not found
    ...
    if (IsHolidayActive(HOLIDAY_FIRE_FESTIVAL))
    {
        if (!player->IsInWorld() || player->GetMap()->IsContinent())
        {
            std::lock_guard<std::mutex> guard(m_mutex);
            auto position =
                std::find(m_zonePlayers[zoneId].begin(), m_zonePlayers[zoneId].end(), player->GetObjectGuid());
...
void WorldState::HandlePlayerLeaveArea(Player *player, uint32 areaId)
{
    ...
    case AREAID_FALCONWING_SQUARE: {
        ...
        if (position != m_areaPlayers[areaId].end()) // == myVector.end() means the
                                                     // element was not found
...
Map *WorldState::GetMap(uint32 mapId, Position const &invZone)
{
    ...
    if (!map)
        sLog.outError("ScourgeInvasionEvent::GetMap found no map with mapId %u, x: "
                      "%f, y: %f.",
                      mapId, invZone.x, invZone.y);
    return map;
}
        ...
        if (m_siData.m_broadcastTimer <= diff)
        {
            ...
            if (m_siData.m_state == STATE_1_ENABLED)
            {
                ...
                for (auto &zone : m_siData.m_invasionPoints)
                    HandleActiveZone(GetTimerIdForZone(zone.second.zoneId), zone.second.zoneId,
                                     zone.second.remainingVar, now);
            }
        }
        else
...
void WorldState::SpawnWarEffortGos()
{
    ...
    for (uint32 i = 0; i < RESOURCE_MAX; ++i)
        ChangeWarEffortGoSpawns(AQResources(i),
                                m_aqData.m_phase == PHASE_2_TRANSPORTING_RESOURCES ? m_aqData.m_phase2Tier : -1);
}
...
std::pair<AQResourceGroup, Team> WorldState::GetResourceInfo(AQResources resource)
{
    switch (resource)
    {
    case AQ_RAINBOW_FIN_ALBACORE:
        return {RESOURCE_GROUP_COOKING, ALLIANCE};
    case AQ_ROAST_RAPTOR:
        return {RESOURCE_GROUP_COOKING, ALLIANCE};
    case AQ_SPOTTED_YELLOWTAIL_ALLY:
        return {RESOURCE_GROUP_COOKING, ALLIANCE};
    case AQ_SPOTTED_YELLOWTAIL_HORDE:
        return {RESOURCE_GROUP_COOKING, HORDE};
    case AQ_LEAN_WOLF_STEAK:
        return {RESOURCE_GROUP_COOKING, HORDE};
    case AQ_BAKED_SALMON:
        return {RESOURCE_GROUP_COOKING, HORDE};
    case AQ_PEACEBLOOM:
        return {RESOURCE_GROUP_HERBS, HORDE};
    case AQ_FIREBLOOM:
        return {RESOURCE_GROUP_HERBS, HORDE};
    case AQ_PURPLE_LOTUS_HORDE:
        return {RESOURCE_GROUP_HERBS, HORDE};
    case AQ_PURPLE_LOTUS_ALLY:
        return {RESOURCE_GROUP_HERBS, ALLIANCE};
    case AQ_STRANGLEKELP:
        return {RESOURCE_GROUP_HERBS, ALLIANCE};
    case AQ_ARTHAS_TEARS:
        return {RESOURCE_GROUP_HERBS, ALLIANCE};
    case AQ_TIN_BAR:
        return {RESOURCE_GROUP_BARS, HORDE};
    case AQ_MITHRIL_BAR:
        return {RESOURCE_GROUP_BARS, HORDE};
    case AQ_IRON_BAR:
        return {RESOURCE_GROUP_BARS, ALLIANCE};
    case AQ_THORIUM_BAR:
        return {RESOURCE_GROUP_BARS, ALLIANCE};
    case AQ_COPPER_BAR_ALLY:
        return {RESOURCE_GROUP_BARS, ALLIANCE};
    case AQ_COPPER_BAR_HORDE:
        return {RESOURCE_GROUP_BARS, HORDE};
    case AQ_HEAVY_LEATHER:
        return {RESOURCE_GROUP_SKINNING, HORDE};
    case AQ_RUGGED_LEATHER:
        return {RESOURCE_GROUP_SKINNING, HORDE};
    case AQ_THICK_LEATHER_HORDE:
        return {RESOURCE_GROUP_SKINNING, HORDE};
    case AQ_THICK_LEATHER_ALLY:
        return {RESOURCE_GROUP_SKINNING, ALLIANCE};
    case AQ_LIGHT_LEATHER:
        return {RESOURCE_GROUP_SKINNING, ALLIANCE};
    case AQ_MEDIUM_LEATHER:
        return {RESOURCE_GROUP_SKINNING, ALLIANCE};
    case AQ_LINEN_BANDAGE:
        return {RESOURCE_GROUP_BANDAGES, ALLIANCE};
    case AQ_SILK_BANDAGE:
        return {RESOURCE_GROUP_BANDAGES, ALLIANCE};
    case AQ_RUNECLOTH_BANDAGE_ALLY:
        return {RESOURCE_GROUP_BANDAGES, ALLIANCE};
    case AQ_RUNECLOTH_BANDAGE_HORDE:
        return {RESOURCE_GROUP_BANDAGES, HORDE};
    case AQ_WOOL_BANDAGE:
        return {RESOURCE_GROUP_BANDAGES, HORDE};
    case AQ_MAGEWEAVE_BANDAGE:
        return {RESOURCE_GROUP_BANDAGES, HORDE};
    default:
        return {RESOURCE_GROUP_HERBS, HORDE};
    }
...
void WorldState::StartZoneEvent(SIZoneIds eventId)
{
    ...
    {
    default:
    case SI_ZONE_AZSHARA:
        StartNewInvasion(ZONEID_AZSHARA);
        break;
    case SI_ZONE_BLASTED_LANDS:
        StartNewInvasion(ZONEID_BLASTED_LANDS);
        break;
    case SI_ZONE_BURNING_STEPPES:
        StartNewInvasion(ZONEID_BURNING_STEPPES);
        break;
    case SI_ZONE_EASTERN_PLAGUELANDS:
        StartNewInvasion(ZONEID_EASTERN_PLAGUELANDS);
        break;
    case SI_ZONE_TANARIS:
        StartNewInvasion(ZONEID_TANARIS);
        break;
    case SI_ZONE_WINTERSPRING:
        StartNewInvasion(ZONEID_WINTERSPRING);
        break;
    case SI_ZONE_STORMWIND:
        StartNewCityAttack(ZONEID_STORMWIND_CITY);
        break;
    case SI_ZONE_UNDERCITY:
        StartNewCityAttack(ZONEID_UNDERCITY);
        break;
    }
}
...
void WorldState::StartNewInvasion(uint32 zoneId)
{
    ...
    if (!mapPtr)
    {
        sLog.outError("ScourgeInvasionEvent::StartNewInvasionIfTime unable to "
                      "access required map (%d). Retrying next update.",
                      zone.map);
        return;
    }
    ...
    {
    case ZONEID_AZSHARA:
        sGameEventMgr.StartEvent(GAME_EVENT_SCOURGE_INVASION_AZSHARA);
        break;
    case ZONEID_BLASTED_LANDS:
        sGameEventMgr.StartEvent(GAME_EVENT_SCOURGE_INVASION_BLASTED_LANDS);
        break;
    case ZONEID_BURNING_STEPPES:
        sGameEventMgr.StartEvent(GAME_EVENT_SCOURGE_INVASION_BURNING_STEPPES);
        break;
    case ZONEID_EASTERN_PLAGUELANDS:
        sGameEventMgr.StartEvent(GAME_EVENT_SCOURGE_INVASION_EASTERN_PLAGUELANDS);
        break;
    case ZONEID_TANARIS:
        sGameEventMgr.StartEvent(GAME_EVENT_SCOURGE_INVASION_TANARIS);
        break;
    case ZONEID_WINTERSPRING:
        sGameEventMgr.StartEvent(GAME_EVENT_SCOURGE_INVASION_WINTERSPRING);
        break;
    }
...
void WorldState::StartNewCityAttack(uint32 zoneId)
{
    ...
    // If any of the required maps are not available we return. Will cause the
    // invasion to be started on next update instead
    if (!mapPtr)
    {
        sLog.outError("ScourgeInvasionEvent::StartNewCityAttackIfTime unable to "
                      "access required map (%d). Retrying next update.",
                      zone.map);
        return;
    }
...
{
    // Dont have a save variable to know which necropolises had already been
    // destroyed, so we just summon the same amount, but not necessarily the same
    // necropolises
    ...
    for (uint32 i = 0; i < num_necropolises_remaining; i++)
    {
        ...
        {
            sLog.outError("ScourgeInvasionEvent::ResumeInvasion map %d not "
                          "accessible. Retry next update.",
                          zone.map);
            return false;
        }
    }
    ...
    if (!mapPtr)
    {
        sLog.outError("ScourgeInvasionEvent::ResumeInvasion failed getting map, "
                      "even after making sure they were loaded....");
        return false;
    }
...
bool WorldState::SummonMouth(Map *map, ScourgeInvasionData::InvasionZone &zone, Position position, bool newInvasion)
{
    ...
    map->GetMessager().AddMessage([=](Map *map) {
        ...
        if (Creature *mouth =
                WorldObject::SummonCreature(TempSpawnSettings(nullptr, NPC_MOUTH_OF_KELTHUZAD, position.x, position.y,
                                                              position.z, position.o, TEMPSPAWN_DEAD_DESPAWN, 0, true),
                                            map))
        {
...
bool WorldState::SummonPallid(Map *map, ScourgeInvasionData::CityAttack &zone, Position position, uint32 spawnLoc)
{
    ...
    map->GetMessager().AddMessage([=](Map *map) {
        ...
        if (Creature *pallid = WorldObject::SummonCreature(
                TempSpawnSettings(nullptr, PickRandomValue(NPC_PALLID_HORROR, NPC_PATCHWORK_TERROR), position.x,
                                  position.y, position.z, position.o, TEMPSPAWN_DEAD_DESPAWN, 0, true),
                map))
        {
...
void WorldState::HandleActiveZone(uint32 attackTimeVar, uint32 zoneId, uint32 remainingVar, TimePoint now)
{
    ...
    // Calculate the next possible attack between ZONE_ATTACK_TIMER_MIN and
    ...
    {
        map->GetMessager().AddMessage([=](Map *map) {
            // Handles the inactive zone, without a Mouth of Kel'Thuzad summoned
            // (which spawns the whole zone event).
            ...
            else if (timePoint < now && remaining == 0)
            {
                ...
                sLog.outBasic("[Scourge Invasion Event] The Scourge has been defeated "
                              "in %d, next attack starting in %ld minutes.",
                              zoneId, timeToNextAttack);
                ...
                if (mouth)
                    ...
                    sLog.outError("ScourgeInvasionEvent::HandleActiveZone ObjectGuid "
                                  "%llu not found.",
                                  (long long unsigned)zone.mouthGuid.GetRawValue());
            }
        });
    }
    else
    {
        // If more than one zones are alreay being attacked, set the timer again
        // to ZONE_ATTACK_TIMER.
        if (GetActiveZones() > 1)
            sWorldState.SetSITimer(SITimers(attackTimeVar), next_attack);
...
uint32 WorldState::GetTimerIdForZone(uint32 zoneId)
{
    ...
    {
    case ZONEID_TANARIS:
        attackTime = SI_TIMER_TANARIS;
        break;
    case ZONEID_BLASTED_LANDS:
        attackTime = SI_TIMER_BLASTED_LANDS;
        break;
    case ZONEID_EASTERN_PLAGUELANDS:
        attackTime = SI_TIMER_EASTERN_PLAGUELANDS;
        break;
    case ZONEID_BURNING_STEPPES:
        attackTime = SI_TIMER_BURNING_STEPPES;
        break;
    case ZONEID_WINTERSPRING:
        attackTime = SI_TIMER_WINTERSPRING;
        break;
    case ZONEID_AZSHARA:
        attackTime = SI_TIMER_AZSHARA;
        break;
    }
    return attackTime;
}
    ...
    sMapMgr.DoForAllMapsWithMapId(mapId, [&](Map *map) {
        WorldObject::SummonCreature(
            TempSpawnSettings(nullptr, NPC_HIGHLORD_KRUUL, highlordKruulSpawns[m_highlordKruulChosenPosition][0],
                              ...
                              highlordKruulSpawns[m_highlordKruulChosenPosition][3], TEMPSPAWN_DEAD_DESPAWN, 0, true,
                              false, m_highlordKruulChosenPosition),
            map);
    });
}
...
void WorldState::AddSunsReachProgress(uint32 questId)
{
    ...
    if (previousValue != newValue)
        SendWorldstateUpdate(m_sunsReachData.m_sunsReachReclamationMutex, m_sunsReachData.m_sunsReachReclamationPlayers,
                             newValue, worldState);
...
void WorldState::AddSunwellGateProgress(uint32 questId)
{
    ...
    case SUNWELL_ALL_GATES_CLOSED:
        counter = COUNTER_AGAMATH_THE_FIRST_GATE;
        worldState = WORLD_STATE_AGAMATH_THE_FIRST_GATE_HEALTH;
        break;
    case SUNWELL_AGAMATH_GATE1_OPEN:
        counter = COUNTER_ROHENDOR_THE_SECOND_GATE;
        worldState = WORLD_STATE_ROHENDOR_THE_SECOND_GATE_HEALTH;
        break;
    case SUNWELL_ROHENDOR_GATE2_OPEN:
        counter = COUNTER_ARCHONISUS_THE_FINAL_GATE;
        worldState = WORLD_STATE_ARCHONISUS_THE_FINAL_GATE_HEALTH;
        break;
    default:
        return;
    }
...
void WorldState::HandleSunsReachPhaseTransition(uint32 newPhase)
{
    ...
    SendWorldstateUpdate(m_sunsReachData.m_sunsReachReclamationMutex, m_sunsReachData.m_sunsReachReclamationPlayers,
                         m_sunsReachData.m_phase, WORLD_STATE_QUEL_DANAS_MUSIC);
    Save(SAVE_ID_QUEL_DANAS);
}
...
void WorldState::HandleSunwellGateTransition(uint32 newGate)
{
    ...
    case SUNWELL_ROHENDOR_GATE2_OPEN:
        worldState = WORLD_STATE_ROHENDOR_THE_SECOND_GATE_HEALTH;
        break;
    case SUNWELL_ARCHONISUS_GATE3_OPEN:
        worldState = WORLD_STATE_ARCHONISUS_THE_FINAL_GATE_HEALTH;
        break;
    }
    if (worldState)
        SendWorldstateUpdate(m_sunsReachData.m_sunsReachReclamationMutex, m_sunsReachData.m_sunsReachReclamationPlayers,
                             m_sunsReachData.m_gate, worldState);
    Save(SAVE_ID_QUEL_DANAS);
}
...
void WorldState::StopSunwellGatePhase()
{
    ...
    {
    case SUNWELL_ALL_GATES_CLOSED:
        sGameEventMgr.StopEvent(GAME_EVENT_SWP_GATES_PHASE_0);
        break;
    case SUNWELL_AGAMATH_GATE1_OPEN:
        sGameEventMgr.StopEvent(GAME_EVENT_SWP_GATES_PHASE_1);
        break;
    case SUNWELL_ROHENDOR_GATE2_OPEN:
        sGameEventMgr.StopEvent(GAME_EVENT_SWP_GATES_PHASE_2);
        break;
    case SUNWELL_ARCHONISUS_GATE3_OPEN:
        sGameEventMgr.StopEvent(GAME_EVENT_SWP_GATES_PHASE_3);
        break;
    default:
        break;
    }
...
void WorldState::StartSunwellGatePhase()
{
    ...
    {
    case SUNWELL_ALL_GATES_CLOSED:
        sGameEventMgr.StartEvent(GAME_EVENT_SWP_GATES_PHASE_0);
        break;
    case SUNWELL_AGAMATH_GATE1_OPEN:
        sGameEventMgr.StartEvent(GAME_EVENT_SWP_GATES_PHASE_1);
        break;
    case SUNWELL_ROHENDOR_GATE2_OPEN:
        sGameEventMgr.StartEvent(GAME_EVENT_SWP_GATES_PHASE_2);
        break;
    case SUNWELL_ARCHONISUS_GATE3_OPEN:
        sGameEventMgr.StartEvent(GAME_EVENT_SWP_GATES_PHASE_3);
        break;
    default:
        break;
    }
...
uint32 SunsReachReclamationData::GetPhasePercentage(uint32 phase)
{
    ...
    case SUNS_REACH_PHASE_1_STAGING_AREA:
        return uint32((m_sunsReachReclamationCounters[COUNTER_ERRATIC_BEHAVIOR] +
                      ...
                      100 / (2 * sWorld.getConfig(CONFIG_UINT32_SUNSREACH_COUNTER)));
    case SUNS_REACH_PHASE_2_SANCTUM:
...
uint32 SunsReachReclamationData::GetSubPhasePercentage(uint32 subPhase)
{
    ...
    case SUBPHASE_PORTAL:
        return uint32(m_sunsReachReclamationCounters[COUNTER_INTERCEPTING_THE_MANA_CELLS] * 100 /
                      sWorld.getConfig(CONFIG_UINT32_SUNSREACH_COUNTER));
    case SUBPHASE_ANVIL:
        return uint32(m_sunsReachReclamationCounters[COUNTER_MAKING_READY] * 100 /
                      sWorld.getConfig(CONFIG_UINT32_SUNSREACH_COUNTER));
    case SUBPHASE_ALCHEMY_LAB:
        return uint32(m_sunsReachReclamationCounters[COUNTER_DISCOVERING_YOUR_ROOTS] * 100 /
                      sWorld.getConfig(CONFIG_UINT32_SUNSREACH_COUNTER));
    case SUBPHASE_MONUMENT:
        return uint32(m_sunsReachReclamationCounters[COUNTER_A_CHARITABLE_DONATION] * 100 /
                      sWorld.getConfig(CONFIG_UINT32_SUNSREACH_COUNTER));
    default:
        return 0;
    }
...
uint32 SunsReachReclamationData::GetSunwellGatePercentage(uint32 gate)
{
    ...
    {
    case SUNWELL_ALL_GATES_CLOSED:
        return 100 - uint32(m_gateCounters[COUNTER_AGAMATH_THE_FIRST_GATE] * 100 / COUNTER_MAX_VAL_REQ_SWP_GATES);
    case SUNWELL_AGAMATH_GATE1_OPEN:
        return 100 - uint32(m_gateCounters[COUNTER_ROHENDOR_THE_SECOND_GATE] * 100 / COUNTER_MAX_VAL_REQ_SWP_GATES);
    case SUNWELL_ROHENDOR_GATE2_OPEN:
        return 100 - uint32(m_gateCounters[COUNTER_ARCHONISUS_THE_FINAL_GATE] * 100 / COUNTER_MAX_VAL_REQ_SWP_GATES);
    default:
        return 0;
    }
}
...
{
    if (m_siData.m_state != STATE_0_DISABLED) // scourge invasion active - need
                                              // to send all worldstates
    {
        uint32 victories = GetBattlesWon();
        ...
        FillInitialWorldStateData(data, count, WORLD_STATE_SCOURGE_BURNING_STEPPES,
                                  remainingBurningSteppes > 0 ? 1 : 0);
        FillInitialWorldStateData(data, count, WORLD_STATE_SCOURGE_EASTERN_PLAGUELANDS,
                                  remainingEasternPlaguelands > 0 ? 1 : 0);
        ...
        FillInitialWorldStateData(data, count, WORLD_STATE_SCOURGE_NECROPOLIS_EASTERN_PLAGUELANDS,
    ...
    case ZONEID_UNDERCITY: {
        if (sGameEventMgr.IsActiveHoliday(HOLIDAY_LOVE_IS_IN_THE_AIR))
        {
            uint32 allianceSum = GetLoveIsInTheAirCounter(LOVE_LEADER_BOLVAR) +
                                 GetLoveIsInTheAirCounter(LOVE_LEADER_TYRANDE) +
                                 GetLoveIsInTheAirCounter(LOVE_LEADER_MAGNI);
            uint32 hordeSum = GetLoveIsInTheAirCounter(LOVE_LEADER_CAIRNE) +
    ...
    case ZONEID_SHATTRATH: {
        ...
        {
        case SUNWELL_ALL_GATES_CLOSED:
            FillInitialWorldStateData(data, count, WORLD_STATE_AGAMATH_THE_FIRST_GATE_HEALTH,
                                      m_sunsReachData.GetSunwellGatePercentage(m_sunsReachData.m_gate));
            break;
        case SUNWELL_AGAMATH_GATE1_OPEN:
            FillInitialWorldStateData(data, count, WORLD_STATE_ROHENDOR_THE_SECOND_GATE_HEALTH,
                                      m_sunsReachData.GetSunwellGatePercentage(m_sunsReachData.m_gate));
            break;
        case SUNWELL_ROHENDOR_GATE2_OPEN:
            FillInitialWorldStateData(data, count, WORLD_STATE_ARCHONISUS_THE_FINAL_GATE_HEALTH,
                                      m_sunsReachData.GetSunwellGatePercentage(m_sunsReachData.m_gate));
            break;
        }
        if (m_sunsReachData.m_phase >= SUNS_REACH_PHASE_3_ARMORY &&
            (m_sunsReachData.m_subphaseMask & SUBPHASE_ANVIL) == 0)
