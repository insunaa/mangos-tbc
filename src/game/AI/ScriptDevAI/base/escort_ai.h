...
struct npc_escortAI : public ScriptedAI
{
  public:
    ...
    virtual void UpdateEscortAI(const uint32 diff); // used when it's needed to add code in
                                                    // update (abilities, scripted events, etc)
    ...
    {
        return (m_escortState & escortState) != 0;
    }
  ...
  private:
    ...
    const Quest *m_questForEscort; // generally passed in Start() when regular
                                   // escort script.

    bool m_isRunning;         // all creatures are walking by default (has flag
                              // SPLINEFLAG_WALKMODE)
    bool m_canInstantRespawn; // if creature should respawn instantly after escort
                              // over (if not, database respawntime are used)
    bool m_canReturnToStart;  // if creature can walk same path (loop) without
