...
bool ArenaTeam::Create(ObjectGuid captainGuid, ArenaType type, std::string arenaTeamName)
{
    ...
    // ArenaTeamName already assigned to ArenaTeam::name, use it to encode string
    // for DB
    ...
    CharacterDatabase.PExecute("INSERT INTO arena_team "
                               "(arenateamid,name,captainguid,type,BackgroundColor,EmblemStyle,"
                               "EmblemColor,BorderStyle,BorderColor) "
                               "VALUES('%u','%s','%u','%u','%u','%u','%u','%u','%u')",
                               m_TeamId, arenaTeamName.c_str(), m_CaptainGuid.GetCounter(), m_Type, m_BackgroundColor,
                               m_EmblemStyle, m_EmblemColor, m_BorderStyle, m_BorderColor);
    CharacterDatabase.PExecute("INSERT INTO arena_team_stats (arenateamid, rating, games_week, "
                               "wins_week, games_season, wins_season, `rank`) VALUES "
                               "('%u', '%u', '%u', '%u', '%u', '%u', '%u')",
...
bool ArenaTeam::AddMember(ObjectGuid playerGuid)
{
    ...
    // this will be prevent attempt joining player to many arenateams and corrupt
    ...
    CharacterDatabase.PExecute("INSERT INTO arena_team_member (arenateamid, "
                               "guid, personal_rating) VALUES ('%u', '%u', '%u')",
...
bool ArenaTeam::LoadMembersFromDB(QueryResult *arenaTeamMembersResult)
{
    ...
    do
    {
        ...
        // already processed and current team hasn't got any members
        if (!fields)
            break;
        ...
        if (arenaTeamId < m_TeamId)
        {
            ...
            sLog.outErrorDb("ArenaTeam %u does not exist but it has record in "
        ...
        {
            sLog.outErrorDb("ArenaTeam %u has member with empty name - probably "
                            "player %s doesn't exist, deleting him from memberlist!",
    ...
    if (Empty() || !captainPresentInTeam)
    {
        ...
        sLog.outErrorDb("ArenaTeam %u does not have any members or its captain is "
...
void ArenaTeam::Disband(WorldSession *session)
{
    ...
    CharacterDatabase.PExecute("DELETE FROM arena_team_member WHERE arenateamid = '%u'",
...
void ArenaTeam::InspectStats(WorldSession *session, ObjectGuid guid)
{
    ...
    data << uint32(member->games_season);    // played (count of all games, that the
                          ...
                          uint32 borderColor)
{
    ...
    CharacterDatabase.PExecute("UPDATE arena_team SET BackgroundColor='%u', EmblemStyle='%u', "
...
void ArenaTeam::SetStats(uint32 stat_type, uint32 value)
{
    ...
    case STAT_TYPE_RATING:
        ...
        CharacterDatabase.PExecute("UPDATE arena_team_stats SET rating = '%u' WHERE arenateamid = '%u'", value,
                                   GetId());
        break;
    case STAT_TYPE_GAMES_WEEK:
        m_stats.games_week = value;
        CharacterDatabase.PExecute("UPDATE arena_team_stats SET games_week = '%u' "
                                   "WHERE arenateamid = '%u'",
                                   value, GetId());
        break;
    case STAT_TYPE_WINS_WEEK:
        m_stats.wins_week = value;
        CharacterDatabase.PExecute("UPDATE arena_team_stats SET wins_week = '%u' WHERE arenateamid = "
                                   "'%u'",
                                   value, GetId());
        break;
    case STAT_TYPE_GAMES_SEASON:
        m_stats.games_season = value;
        CharacterDatabase.PExecute("UPDATE arena_team_stats SET games_season = "
                                   "'%u' WHERE arenateamid = '%u'",
                                   value, GetId());
        break;
    case STAT_TYPE_WINS_SEASON:
        m_stats.wins_season = value;
        CharacterDatabase.PExecute("UPDATE arena_team_stats SET wins_season = '%u' "
                                   "WHERE arenateamid = '%u'",
                                   value, GetId());
        break;
    case STAT_TYPE_RANK:
        m_stats.rank = value;
        CharacterDatabase.PExecute("UPDATE arena_team_stats SET `rank` = '%u' WHERE arenateamid = '%u'", value,
                                   GetId());
        break;
    default:
...
uint32 ArenaTeam::GetPoints(uint32 MemberRating)
{
    ...
    if (rating <= 1500)
    {
        // As of Season 6 and later, all teams below 1500 rating will earn points
        // as if they were a 1500 rated team
        if (sWorld.getConfig(CONFIG_UINT32_ARENA_SEASON_ID) >= 6)
            rating = 1500;
...
void ArenaTeam::SaveToDB()
{
    ...
    CharacterDatabase.BeginTransaction();
    CharacterDatabase.PExecute("UPDATE arena_team_stats SET rating = '%u',games_week = "
                               "'%u',games_season = '%u',`rank` = '%u',wins_week = '%u',wins_season = "
                               "'%u' WHERE arenateamid = '%u'",
                               m_stats.rating, m_stats.games_week, m_stats.games_season, m_stats.rank,
                               m_stats.wins_week, m_stats.wins_season, GetId());
    for (MemberList::const_iterator itr = m_members.begin(); itr != m_members.end(); ++itr)
    {
        CharacterDatabase.PExecute("UPDATE arena_team_member SET played_week = '%u', wons_week = '%u', "
                                   "played_season = '%u', wons_season = '%u', personal_rating = '%u' "
                                   "WHERE arenateamid = '%u' AND guid = '%u'",
                                   itr->games_week, itr->wins_week, itr->games_season, itr->wins_season,
                                   itr->personal_rating, m_TeamId, itr->guid.GetCounter());
    }
    CharacterDatabase.CommitTransaction();
}
