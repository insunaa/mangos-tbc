...
struct WarriorExecute : public SpellScript
{
    void OnCast(Spell *spell) const override // confirmed main spell can not hit and child still hits
    {
        ...
        SpellCastResult result =
            spell->GetCaster()->CastCustomSpell(spell->m_targets.getUnitTarget(), 20647, &basePoints0, nullptr, nullptr,
...
struct Devastate : public SpellScript
{
    void OnEffectExecute(Spell *spell, SpellEffectIndex effIdx) const override
    {
        ...
        if (!sunder || sunder->GetStackAmount() < sunder->GetSpellProto()->StackAmount)
        {
            ...
            if (sunderId)
                caster->CastSpell(unitTarget, sunderId,
                                  TRIGGERED_IGNORE_HIT_CALCULATION | TRIGGERED_IGNORE_CURRENT_CASTED_SPELL |
                                      TRIGGERED_IGNORE_GCD | TRIGGERED_IGNORE_COSTS);
        }
        ...
        if (sunder)
        {
            ...
            // 14 * stack
            unitTarget->AddThreat(caster, 14.0f * sunder->GetStackAmount(), false,
                                  GetSpellSchoolMask(spell->m_spellInfo), spell->m_spellInfo);
        }
    }
};
...
struct RetaliationWarrior : public AuraScript
{
    SpellAuraProcResult OnProc(Aura *aura, ProcExecutionData &procData) const override
    {
        ...
        procData.victim->CastSpell(procData.attacker, 20240,
                                   TRIGGERED_IGNORE_HIT_CALCULATION | TRIGGERED_IGNORE_CURRENT_CASTED_SPELL |
                                       TRIGGERED_IGNORE_GCD | TRIGGERED_IGNORE_COSTS);
