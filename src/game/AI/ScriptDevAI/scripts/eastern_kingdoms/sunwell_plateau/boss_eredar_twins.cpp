...
improvement; Add invulnerability in phase transition SDCategory: Sunwell
Plateau EndScriptData */
...
struct boss_alythessAI : public CombatAI
{
    boss_alythessAI(Creature *creature)
        : CombatAI(creature, ALYTHESS_ACTION_MAX),
          m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData())), m_introDialogue(aIntroDialogue)
    {
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case ALYTHESS_ENRAGE:
            if (DoCastSpellIfCan(m_creature, SPELL_TWINS_ENRAGE) == CAST_OK)
            {
                ...
                sLog.outCustomLog("Sacrolash enrage at %lu and aggro at %lu",
                                  m_creature->GetMap()->GetCurrentClockTime().time_since_epoch().count(),
                                  m_aggro.time_since_epoch().count());
            }
            break;
        ...
        case ALYTHESS_CONFLAGRATION: {
            ...
            Unit *target = source->SelectAttackingTarget(ATTACKING_TARGET_TOPAGGRO, urand(2, 7), spellId,
                                                         SELECT_FLAG_PLAYER | SELECT_FLAG_SKIP_CUSTOM, params);
            if (!target)
...
struct boss_sacrolashAI : public CombatAI
{
    boss_sacrolashAI(Creature *creature)
        : CombatAI(creature, SACROLASH_ACTION_MAX),
          m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData()))
    {
        ...
        m_creature->GetCombatManager().SetLeashingCheck(
            [](Unit *, float x, float y, float) -> bool { return x < 1800 && y < 580; });
    }
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case SACROLASH_SHADOW_NOVA: {
            ...
            if (m_creature->GetVictim())
                params.skip.guid = m_creature->GetVictim()->GetObjectGuid();
            Unit *target = source->SelectAttackingTarget(ATTACKING_TARGET_TOPAGGRO, urand(1, 7), spellId,
                                                         SELECT_FLAG_PLAYER | SELECT_FLAG_SKIP_CUSTOM, params);
            if (!target)
                target = source->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, spellId, SELECT_FLAG_PLAYER);
...
struct npc_shadow_imageAI : public ScriptedAI
{
    ...
    void AttackRandomTarget(bool shadowstep)
    {
        ...
        if (Creature *sacrolash = m_instance->GetSingleCreatureFromStorage(NPC_SACROLASH))
        {
            if (Unit *target = static_cast<boss_sacrolashAI *>(sacrolash->AI())
                                   ->GetRandomTargetAtDist(10.0f)) // Attack random range target
            {
...
struct DarkFlameShadowAura : public AuraScript
{
    ...
    bool OnCheckProc(Aura * /*aura*/, ProcExecutionData &data) const override
    {
        if (!data.spellInfo || !data.source || data.source->HasAura(SPELL_DARK_FLAME))
            return false;
        std::set<uint32> spellIds = {SPELL_CONFOUNDING_BLOW, SPELL_SHADOW_BLADES, SPELL_SHADOW_NOVA, SPELL_SHADOWFURY,
                                     SPELL_DARK_STRIKE};
...
struct DarkFlameFireAura : public AuraScript
{
    ...
    bool OnCheckProc(Aura * /*aura*/, ProcExecutionData &data) const override
    {
        if (!data.spellInfo || !data.source || data.source->HasAura(SPELL_DARK_FLAME))
            return false;
        std::set<uint32> spellIds = {SPELL_FLAME_SEAR, SPELL_CONFLAGRATION, SPELL_CONFLAGRATION_TICK, SPELL_BLAZE,
                                     SPELL_BURN};
        if (spellIds.find(data.spellInfo->Id) == spellIds.end())
            return false;
        return true;
