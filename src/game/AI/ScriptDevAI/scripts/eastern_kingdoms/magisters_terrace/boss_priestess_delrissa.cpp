...
{
    priestess_commonAI(Creature *creature, uint32 actions)
...
struct boss_priestess_delrissaAI : public priestess_commonAI
{
    boss_priestess_delrissaAI(Creature *creature)
        : priestess_commonAI(creature, DELRISSA_ACTION_MAX),
        ...
        if (m_vuiLackeyEnties.empty())
        {
            ...
            for (uint8 i = 0; i < MAX_DELRISSA_ADDS; ++i)
                m_creature->SummonCreature(m_vuiLackeyEnties[i], aLackeyLocations[i][0], aLackeyLocations[i][1],
                                           aLackeyLocations[i][2], aLackeyLocations[i][3],
                                           TEMPSPAWN_CORPSE_TIMED_DESPAWN, 7000);
        }
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case DELRISSA_DISPEL: {
            ...
            m_creature->SelectAttackingTargets(unitVector, ATTACKING_TARGET_ALL_SUITABLE, 0, SPELL_DISPEL_MAGIC,
                                               SELECT_FLAG_PLAYER);
        ...
        case DELRISSA_SHADOW_WORD_PAIN:
            if (Unit *target = m_creature->SelectAttackingTarget(
                    ATTACKING_TARGET_RANDOM, m_isRegularMode ? SPELL_SHADOW_WORD_PAIN : SPELL_SHADOW_WORD_PAIN_H,
                    m_isRegularMode ? SPELL_SHADOW_WORD_PAIN : SPELL_SHADOW_WORD_PAIN_H,
                    (SELECT_FLAG_PLAYER | SELECT_FLAG_NOT_AURA)))
                if (DoCastSpellIfCan(target, m_isRegularMode ? SPELL_SHADOW_WORD_PAIN : SPELL_SHADOW_WORD_PAIN_H) ==
                    CAST_OK)
                    ResetCombatAction(action, urand(5000, 14000));
            break;
        case DELRISSA_SCREAM:
            if (Unit *target = m_creature->SelectAttackingTarget(
                    ATTACKING_TARGET_NEAREST_BY, 0, SPELL_SCREAM,
                    (SELECT_FLAG_IN_LOS | SELECT_FLAG_PLAYER | SELECT_FLAG_RANGE_AOE_RANGE)))
...
struct npc_kagani_nightstrikeAI : public priestess_companion_commonAI
{
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case KAGANI_EVISCERATE:
            if (DoCastSpellIfCan(m_creature->GetVictim(), m_isRegularMode ? SPELL_EVISCERATE : SPELL_EVISCERATE_H) ==
                CAST_OK)
...
struct npc_ellrys_duskhallowAI : public priestess_companion_commonAI
{
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case ELLRYS_IMMOLATE:
            if (DoCastSpellIfCan(m_creature->GetVictim(), m_isRegularMode ? SPELL_IMMOLATE : SPELL_IMMOLATE_H) ==
                CAST_OK)
                ResetCombatAction(action, urand(16000, 30000));
            break;
        case ELLRYS_SHADOWBOLT:
            if (DoCastSpellIfCan(m_creature->GetVictim(), m_isRegularMode ? SPELL_SHADOW_BOLT : SPELL_SHADOW_BOLT_H) ==
                CAST_OK)
                ResetCombatAction(action, urand(2400, 3600));
            break;
        case ELLRYS_SEED_OF_CORRUPTION:
            if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_SEED_OF_CORRUPTION,
                                                                 SELECT_FLAG_PLAYER | SELECT_FLAG_NOT_AURA))
        ...
        case ELLRYS_CURSE_OF_AGONY:
            if (Unit *target = m_creature->SelectAttackingTarget(
                    ATTACKING_TARGET_RANDOM, 0, m_isRegularMode ? SPELL_CURSE_OF_AGONY : SPELL_CURSE_OF_AGONY_H,
                    SELECT_FLAG_PLAYER | SELECT_FLAG_NOT_AURA))
                if (DoCastSpellIfCan(target, m_isRegularMode ? SPELL_CURSE_OF_AGONY : SPELL_CURSE_OF_AGONY_H) ==
                    CAST_OK)
...
struct npc_eramas_brightblazeAI : public priestess_companion_commonAI
{
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case ERAMAS_KNOCKDOWN:
            if (DoCastSpellIfCan(m_creature->GetVictim(), m_isRegularMode ? SPELL_KNOCKDOWN : SPELL_KNOCKDOWN_H) ==
                CAST_OK)
...
struct npc_yazzaiAI : public priestess_companion_commonAI
{
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case YAZZAI_FROST_NOVA:
            if (m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_FROST_NOVA,
        ...
        case YAZZAI_CONE_OF_COLD:
            if (m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0,
                                                  m_isRegularMode ? SPELL_CONE_OF_COLD : SPELL_CONE_OF_COLD_H,
                                                  SELECT_FLAG_PLAYER | SELECT_FLAG_USE_EFFECT_RADIUS))
        ...
        case YAZZAI_FROSTBOLT:
            if (DoCastSpellIfCan(m_creature->GetVictim(), m_isRegularMode ? SPELL_FROSTBOLT : SPELL_FROSTBOLT_H) ==
                CAST_OK)
                ResetCombatAction(action, urand(2400, 3600));
            break;
        case YAZZAI_BLINK:
...
struct npc_warlord_salarisAI : public priestess_companion_commonAI
{
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case SALARIS_INTERCEPT_STUN:
            ...
            if (!m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, uint32(0), SELECT_FLAG_IN_MELEE_RANGE))
            {
                if (Unit *target = m_creature->SelectAttackingTarget(
                        ...
                        (SELECT_FLAG_PLAYER | SELECT_FLAG_NOT_IN_MELEE_RANGE | SELECT_FLAG_IN_LOS)))
        ...
        case SALARIS_PIERCING_HOWL:
            if (Unit *target = m_creature->SelectAttackingTarget(
                    ATTACKING_TARGET_NEAREST_BY, 0, SPELL_PIERCING_HOWL,
                    (SELECT_FLAG_IN_LOS | SELECT_FLAG_PLAYER | SELECT_FLAG_RANGE_AOE_RANGE)))
...
struct npc_apokoAI : public priestess_companion_commonAI
{
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case APOKO_WAR_STOMP:
            if (Unit *target = m_creature->SelectAttackingTarget(
                    ATTACKING_TARGET_NEAREST_BY, 0, SPELL_WAR_STOMP,
                    (SELECT_FLAG_IN_LOS | SELECT_FLAG_PLAYER | SELECT_FLAG_RANGE_AOE_RANGE)))
        ...
        case APOKO_PURGE:
            if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, nullptr,
                                                                 SELECT_FLAG_PLAYER)) // TODO: Code dispel detection
                if (DoCastSpellIfCan(target, SPELL_PURGE) == CAST_OK)
                    ResetCombatAction(action, urand(7000, 11000));
            break;
        case APOKO_HEALING_WAVE:
            if (Unit *target = DoSelectLowestHpFriendly(40.f, 25.0f, true))
                if (DoCastSpellIfCan(target, m_isRegularMode ? SPELL_LESSER_HEALING_WAVE
                                                             : SPELL_LESSER_HEALING_WAVE_H) == CAST_OK)
                    ResetCombatAction(action, urand(4000, 6000));
            break;
...
struct npc_zelfanAI : public priestess_companion_commonAI
{
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case ZELFAN_GOBLIN_DRAGON_GUN:
            if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0,
                                                                 m_isRegularMode ? SPELL_GOBLIN_DRAGON_GUN_CONE
                                                                                 : SPELL_GOBLIN_DRAGON_GUN_CONE_H,
                                                                 SELECT_FLAG_PLAYER | SELECT_FLAG_USE_EFFECT_RADIUS))
                if (DoCastSpellIfCan(target, m_isRegularMode ? SPELL_GOBLIN_DRAGON_GUN : SPELL_GOBLIN_DRAGON_GUN_H) ==
                    CAST_OK)
