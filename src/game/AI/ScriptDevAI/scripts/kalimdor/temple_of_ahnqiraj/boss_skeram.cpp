...
enum
{
    ...
    SPELL_TRUE_FULFILLMENT = 785,    // wotlk+ also casts 61286 on the target on
                                     // cast end and also on aura end 65634
    ...
    SPELL_BIRTH = 34115, // TBC+ spell on spawn - purpose of 1 sec delay before
                         // images attack
};
...
struct boss_skeramAI : public CombatAI
{
    boss_skeramAI(Creature *creature)
        : CombatAI(creature, SKERAM_ACTION_MAX),
    ...
    {
        // Prophet Skeram will only cast Arcane Explosion if a given number of
        // players are in melee range Initial value was 4+ but it was changed in
        // patch 1.12 to be less dependant on raid We assume value is number of
        // players / 10 (raid of 40 people in Classic -> value of 4)
    ...
    void HandleInitImages()
    {
        std::shuffle(m_teleports.begin(), m_teleports.end(),
                     *GetRandomGenerator()); // Shuffle the teleport spells to
                                             // ensure that boss and images have a
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case SKERAM_SPLIT: {
            ...
            if (m_creature->GetHealthPercent() < m_hpCheck)
            {
                ...
                if (DoCastSpellIfCan(m_creature, SPELL_SUMMON_IMAGES) == CAST_OK)
                {
                    m_hpCheck -= 25.0f;
                    StopAttacking();
                    // Teleport shortly after the images are summoned and set
                    // invisible to clear the selection (Workaround alert!!!)
        ...
        case SKERAM_ARCANE_EXPLOSION: {
            // Arcane Explosion is done if more than a set number of people are
            // in melee range
        ...
        case SKERAM_BLINK: {
            std::shuffle(m_teleports.begin(), m_teleports.end(),
                         *GetRandomGenerator()); // Shuffle the teleport spells to ensure
                                                 // that boss and images have a different
...
struct InitializeImages : public SpellScript
{
    void OnEffectExecute(Spell *spell, SpellEffectIndex /*effIdx*/) const override
    {
        ...
        {
            // Cast Initialize Image for each image target with orignal boss HP
            // percent as basepoints
            ...
            unitTarget->CastSpell(nullptr, SPELL_TELEPORT_IMAGE,
                                  TRIGGERED_OLD_TRIGGERED); // Teleport Image
        }
    }
};
...
struct InitializeImage : public SpellScript
{
    void OnEffectExecute(Spell *spell, SpellEffectIndex /*effIdx*/) const override
    {
        if (Unit *unitTarget = spell->GetUnitTarget())
        {
            ...
            // The max health depends on the split phase. It's percent * original
            // boss health
            if (healthPct < 25.0f)
                maxHealthPct = 0.50f;
