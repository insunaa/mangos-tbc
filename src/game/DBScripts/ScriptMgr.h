                   ...
                   // after buddy search is done
{
    ...
    // dataint = text id from broadcast_text table. dataint2-4 optional for
    // random selected texts.
    ...
    SCRIPT_COMMAND_MOVE_TO = 3,   // resSource = Creature, datalong1 = relayId, datalong2 =
                                  // travel_speed*100, datalong3 = forcedMovement, x/y/z/o
    ...
    SCRIPT_COMMAND_QUEST_EXPLORED = 7,       // one from source or target must be Player, another GO/Creature,
                                             // datalong=quest_id, datalong2=distance or 0
    SCRIPT_COMMAND_KILL_CREDIT = 8,          // source or target with Player, datalong = creature entry (or 0 for
                                             // target-entry), datalong2 = bool (0=personal credit, 1=group credit)
    ...
    SCRIPT_COMMAND_TEMP_SPAWN_CREATURE = 10, // source = any, datalong=creature entry, datalong2=despawn_delay,
                                             // datalong3=pathId, text1 - setRun - 1, text2 - faction ID, text3 -
                                             // modelId
    ...
    SCRIPT_COMMAND_ACTIVATE_OBJECT = 13, // source = unit, target=GO; data_flags &
                                         // SCRIPT_FLAG_COMMAND_ADDITIONAL send gameobject custom anim,
                                         // datalong = animId
    ...
    SCRIPT_COMMAND_PLAY_SOUND = 16,   // resSource = WorldObject, target=any/player, datalong (sound_id),
                                      // datalong2 (bitmask: 0/1=target-player, 0/2=with distance
                                      // dependent, 0/4=map wide, 0/8=zone wide; so 1|2 = 3 is target with
                                      // distance dependent)
    SCRIPT_COMMAND_CREATE_ITEM = 17,  // source or target must be player, datalong
                                      // = item entry, datalong2 = amount
    ...
    SCRIPT_COMMAND_MOVEMENT = 20,     // resSource = Creature. datalong = MovementType (0:idle, 1:random or
                                      // 2:waypoint), datalong2 = wander-distance/pathId, datalong3 =
                                      // timer/passTarget, dataint1 = forcedMovement
    // data_flags &  SCRIPT_FLAG_COMMAND_ADDITIONAL = Random-movement around
    // current position
    ...
    // data_flags & SCRIPT_FLAG_COMMAND_ADDITIONAL: terminate steps of this
    // script if npc found
    //                                        ELSE: terminate steps of this
    //                                        script if npc not found
    ...
    SCRIPT_COMMAND_TERMINATE_COND = 34, // datalong = condition_id, datalong2 = if != 0 then quest_id of
                                        // quest that will be failed for player's group if the script is
                                        // terminated
    // data_flags & SCRIPT_FLAG_COMMAND_ADDITIONAL terminate when condition is
    // false ELSE terminate when condition is true
    SCRIPT_COMMAND_SEND_AI_EVENT = 35, // resSource = Creature, resTarget = Unit
    // datalong = AIEventType
    // datalong2 = radius. If radius isn't provided and the target is a creature,
    // then send AIEvent to target
    SCRIPT_COMMAND_SET_FACING = 36, // resSource = Creature, resTarget
                                    // WorldObject. Turn resSource towards Taget
    // data_flags & SCRIPT_FLAG_COMMAND_ADDITIONAL also set TargetGuid of
    // resSource to resTarget. In this case resTarget MUST be Creature/ Player
    ...
    // datalong != 0: Move resSource to a random point between
    // datalong2..datalong around resTarget.
    //      orientation != 0: Obtain a random point around resTarget in direction
    //      of orientation
    // data_flags & SCRIPT_FLAG_COMMAND_ADDITIONAL Obtain a random point around
    // resTarget in direction of resTarget->GetOrientation + orientation for
    // resTarget == resSource and orientation == 0 this will mean resSource
    // moving forward
    SCRIPT_COMMAND_SEND_MAIL = 38, // resSource WorldObject, can be nullptr, resTarget Player
    // datalong: Send mailTemplateId from resSource (if provided) to player
    // resTarget datalong2: AlternativeSenderEntry. Use as sender-Entry dataint1:
    // Delay (>= 0) in Seconds
    SCRIPT_COMMAND_SET_HOVER = 39, // resSource = Creature
    // datalong = bool 0=off, 1=on
    // data_flags & SCRIPT_FLAG_COMMAND_ADDITIONAL set/unset byte flag
    // UNIT_BYTE1_FLAG_FLY_ANIM
    SCRIPT_COMMAND_DESPAWN_GO = 40,          // resTarget = GameObject
    SCRIPT_COMMAND_RESPAWN = 41,             // resSource = Creature. Requires SCRIPT_FLAG_BUDDY_IS_DESPAWNED to
                                             // find dead or despawned targets
    ...
    // datalong=spellid
    // datalong2=castFlags, enum TriggerCastFlags
    // dataint1-3 define the &bp value for the spell. At least one field is
    // required.
...
#define MAX_TEXT_ID                                                                                                    \
    4 // used for SCRIPT_COMMAND_TALK, SCRIPT_COMMAND_EMOTE,
      // SCRIPT_COMMAND_CAST_SPELL, SCRIPT_COMMAND_TERMINATE_SCRIPT
...
enum ScriptInfoDataFlags
{
    ...
    SCRIPT_FLAG_BUDDY_BY_GO = 0x400,          // take the buddy by GO (for commands which
                                              // can target both creature and GO)
};
...
struct ScriptInfo
{
    ...
    union {
        ...
        // datalong unsed                                   //
        // SCRIPT_COMMAND_DESPAWN_GO (40) datalong unsed // SCRIPT_COMMAND_RESPAWN
        // (41)
    ...
    uint32 searchRadiusOrGuid; // search_radius (can also be guid in case of
...
class ScriptAction
{
  public:
    ...
    bool HandleScriptStep(); // return true IF AND ONLY IF the script should be
                             // terminated
  ...
  private:
    ...
    bool GetScriptProcessTargets(WorldObject *originalSource, WorldObject *originalTarget,
