...
struct boss_twin_emperorsAI : public CombatAI
{
                     ...
                     SpellEntry const *spellInfo) override
    {
        ...
        if (Creature *twin = m_instance->GetSingleCreatureFromStorage(
                m_creature->GetEntry() == NPC_VEKLOR ? NPC_VEKNILASH : NPC_VEKLOR))
        {
            ...
            if (twin->GetHealth() <= uint32(twinDamage))
                ...
                twin->CastCustomSpell(nullptr, SPELL_TWIN_EMPATHY, &twinDamage, nullptr, nullptr,
    ...
    void SpellHit(Unit * /*caster*/, const SpellEntry *spellInfo) override
    {
        if (spellInfo->Id == SPELL_TWIN_TELEPORT)
        {
            if (Creature *veknilash = m_instance->GetSingleCreatureFromStorage(NPC_VEKNILASH))
            {
                ...
                TeleportTwin(m_creature, veknilash->GetPositionX(), veknilash->GetPositionY(),
                             veknilash->GetPositionZ(), veknilash->GetOrientation());
...
struct MutateBug : public AuraScript
{
    void OnApply(Aura *aura, bool apply) const override
    {
        ...
        {
            aura->GetTarget()->AI()->DoCastSpellIfCan(nullptr, SPELL_VIRULENT_POISON,
