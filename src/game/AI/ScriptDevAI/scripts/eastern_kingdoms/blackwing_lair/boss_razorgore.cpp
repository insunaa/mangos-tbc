...
SD%Complete: 95
SDComment: Threat management after Mind Control is released needs core support
(boss should have aggro on its previous controller and its previous victim
should have threat transfered from boss to controlling player) SDCategory:
Blackwing Lair EndScriptData */
...
enum
{
    ...
    // SPELL_POSSESS               = 19832,                    // actual possess
    // spell
...
struct npc_blackwing_orbAI : public ScriptedAI
{
    ...
    void SpellHit(Unit * /*caster*/, const SpellEntry *spellInfo) override
    {
        ...
        {
            if (Creature *pTemp = m_creature->SummonCreature(NPC_ORB_DOMINATION, m_creature->GetPositionX(),
                                                             m_creature->GetPositionY(), m_creature->GetPositionZ(), 0,
                                                             TEMPSPAWN_TIMED_DESPAWN, 5 * IN_MILLISECONDS))
    ...
    void UpdateAI(const uint32 uiDiff) override
    {
        ...
        if (m_uiIntroVisualTimer)
        {
            if (m_uiIntroVisualTimer <= uiDiff)
            {
                if (!m_instance)
                {
                    script_error_log("Instance Blackwing Lair: ERROR Failed to load "
                ...
                // If Grethok the Controller is here and spawned, start the visual,
                // else wait for him
...
struct DestroyEgg : public SpellScript
{
    void OnEffectExecute(Spell *spell, SpellEffectIndex /*effIdx*/) const override
    {
        ...
        if (ScriptedInstance *pInstance = static_cast<ScriptedInstance *>(target->GetInstanceData()))
        {
            ...
            // Store the eggs which are destroyed, in order to count them for the
            // second phase
...
struct ExplosionRazorgore : public SpellScript
{
    bool OnCheckTarget(const Spell * /*spell*/, Unit *target, SpellEffectIndex /*effIdx*/) const override
    {
        ...
        static std::set<uint32> validEntries = {NPC_GRETHOK_CONTROLLER,    NPC_BLACKWING_GUARDSMAN,
                                                NPC_BLACKWING_LEGIONNAIRE, NPC_BLACKWING_MAGE,
                                                NPC_DRAGONSPAWN,           NPC_RAZORGORE};
