...
struct boss_thaddiusAI : public CombatAI
{
    boss_thaddiusAI(Creature *creature)
        : CombatAI(creature, THADDIUS_ACTION_MAX),
...
// The superclass and add specific structure will use Feugen NPC as the master
// NPC that will initialise and synchronise both adds. This design is hinted in
// Classic spells DBCs
...
struct boss_thaddiusAddsAI : public ScriptedAI
{
    ...
    // For Stalagg returns feugen, for Feugen returns stalagg
    {
        ...
        // Check if the other add needs to be alive and return nullptr if dead of
        // faking death
    ...
    {
    } // Used for Add-specific spells

    void UpdateAI(const uint32 diff) override
    {
        if (m_areBothDead) // This is the case while fighting Thaddius
        {
            if (m_shockOverloadTimer) // Timer before triggering phase 2 with spell
                                      // Shock Overload
            {
        ...
        if (m_isFakingDeath)
        {
            if (m_reviveTimer < diff)
            {
                if (Creature *pOther = GetOtherAdd())
                {
                    ...
                    {
                        if (!otherAI->IsCountingDead()) // Raid was to slow to
                                                        // kill the second add
                        {
                            DoResetThreat();
...
struct boss_feugenAI : public boss_thaddiusAddsAI
{
    ...
    void UpdateAddAI(const uint32 diff)
    {
        ...
        {
            if (!m_isFakingDeath && GetOtherAdd(true)) // No need to check if we have a victim that Stalagg can
                                                       // pull: this is already done in super class
            {
                if (DoMagneticPullIfCan()) // This will return false if Stalagg
                                           // has no victim we can pull
                {
        ...
        if (effIdx == EFFECT_INDEX_0)
        {
            if (Unit *unitTarget = spell->GetUnitTarget())
            {
                ...
                default: // third and later random
                    unitTarget->CastSpell(unitTarget, urand(0, 1) ? SPELL_POSITIVE_CHARGE : SPELL_NEGATIVE_CHARGE,
    ...
    bool OnCheckTarget(const Spell *spell, Unit *target, SpellEffectIndex effIdx) const override
    {
        if (effIdx == EFFECT_INDEX_0)
        {
            ...
            case SPELL_POSITIVE_CHARGE_DAMAGE:              // Positive Charge
                if (target->HasAura(SPELL_POSITIVE_CHARGE)) // Only deal damage to targets
                                                            // without Positive Charge
                    return false;
                break;
            case SPELL_NEGATIVE_CHARGE_DAMAGE:              // Negative Charge
                if (target->HasAura(SPELL_NEGATIVE_CHARGE)) // Only deal damage to targets
                                                            // without Negative Charge
                    return false;
                break;
            default:
                break;
...
struct ThaddiusCharge : public AuraScript
{
    ...
    void OnPeriodicTrigger(Aura *aura, PeriodicTriggerData &data) const override
    {
        ...
        {
            uint32 buffAuraId =
                aura->GetId() == SPELL_POSITIVE_CHARGE ? SPELL_POSITIVE_CHARGE_BUFF : SPELL_NEGATIVE_CHARGE_BUFF;
            float range = 13.f; // Static value from DBC files. As the value is the
                                // same for both spells we can hardcode it instead
                                // of accessing is through sSpellRadiusStore
            ...
            // Remove previous buffs in case we have less targets of the same
            // charge near use than in previous tick
