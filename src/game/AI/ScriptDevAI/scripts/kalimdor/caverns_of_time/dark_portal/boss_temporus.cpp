...
GetSubsequentActionTimer(TEMPORUS_ACTION_HASTEN) -> 12-18 from 16-24
SDCategory: Caverns of Time, The Dark Portal EndScriptData */
...
struct boss_temporusAI : public CombatAI
{
    boss_temporusAI(Creature *creature)
        : CombatAI(creature, TEMPORUS_ACTION_MAX),
    ...
    void OnSpellCooldownAdded(SpellEntry const *spellInfo) // spells should only reset their action timer on success
    {
        ...
        case SPELL_WING_BUFFET_H:
            ...
            break;
        case SPELL_HASTEN:
            ResetCombatAction(TEMPORUS_ACTION_HASTEN,
                              GetSubsequentActionTimer(TemporusActions(TEMPORUS_ACTION_HASTEN)));
            break;
