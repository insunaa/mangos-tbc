...
SDName: Boss_Gluth
SD%Complete: 99
SDComment: Though gameplay is Blizzlike, logic in Zombie Chow Search and Call
All Zombie Chow is guess work and can probably be improved. Also, based on
similar encounters, Trigger NPCs for the summoning should probably be spawned
by a (currently unknown) spell and the despawn of all adds should also be
handled by a spell. SDCategory: Naxxramas EndScriptData */
...
struct boss_gluthAI : public CombatAI
{
    boss_gluthAI(Creature *creature)
        : CombatAI(creature, GLUTH_ACTION_MAX), m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData()))
    {
        ...
        AddCombatAction(GLUTH_BERSERK,
                        (uint32)(6.5 * MINUTE * IN_MILLISECONDS)); // ~15 seconds after the third Decimate
    }
    ...
    uint32 GetSubsequentActionTimer(uint32 id)
    {
        ...
        case GLUTH_MORTAL_WOUND:
            return 10u * IN_MILLISECONDS;
        case GLUTH_ENRAGE:
            return urand(10, 12) * IN_MILLISECONDS;
        case GLUTH_ROAR:
            return 20u * IN_MILLISECONDS;
        case GLUTH_DECIMATE:
            return urand(100, 110) * IN_MILLISECONDS;
        case GLUTH_BERSERK:
    ...
    void Aggro(Unit * /*who*/) override
    {
        ...
        DoCastSpellIfCan(m_creature, SPELL_ZOMBIE_CHOW_SEARCH,
                         CAST_TRIGGERED | CAST_AURA_NOT_PRESENT); // Aura periodically
                                                                  // looking for NPC 16360
        DoCastSpellIfCan(m_creature, SPELL_CALL_ALL_ZOMBIE_CHOW,
                         CAST_TRIGGERED | CAST_AURA_NOT_PRESENT); // Aura periodically
                                                                  // calling all NPCs 16360
        ...
        if (effIdx == EFFECT_INDEX_0)
        {
            if (Unit *unitTarget = spell->GetUnitTarget())
            {
                ...
                if (unitTarget->GetTypeId() == TYPEID_UNIT && !unitTarget->IsControlledByPlayer() &&
                    unitTarget->GetEntry() != NPC_ZOMBIE_CHOW)
                    return;

                int32 damage = unitTarget->GetHealth() - unitTarget->GetMaxHealth() * 0.05f;
                if (damage > 0)
                    spell->GetCaster()->CastCustomSpell(unitTarget, SPELL_DECIMATE_DAMAGE, &damage, nullptr, nullptr,
    ...
    void OnEffectExecute(Spell *spell, SpellEffectIndex effIdx) const override
    {
        if (effIdx == EFFECT_INDEX_0)
        {
            if (Unit *unitTarget = spell->GetUnitTarget())
            {
                ...
                {
                    float radius =
                        GetSpellRadius(sSpellRadiusStore.LookupEntry(spell->m_spellInfo->EffectRadiusIndex[effIdx]));
    ...
    void OnEffectExecute(Spell *spell, SpellEffectIndex effIdx) const override
    {
        if (effIdx == EFFECT_INDEX_0)
        {
            if (Unit *unitTarget = spell->GetUnitTarget())
            {
                ...
                if (caster->CastSpell(unitTarget, SPELL_ZOMBIE_CHOW_SEARCH_KILL,
                                      TRIGGERED_NONE) == SPELL_CAST_OK) // Zombie Chow Search - Insta kill, single
                                                                        // target
                {
...
struct ZombieChowSearchHeal : public SpellScript
{
    ...
    {
        if (effIdx == EFFECT_INDEX_0)
            spell->GetCaster()->SetHealth(spell->GetCaster()->GetHealth() +
                                          spell->GetCaster()->GetMaxHealth() * 0.05f); // Gain 5% heal
    }
};
