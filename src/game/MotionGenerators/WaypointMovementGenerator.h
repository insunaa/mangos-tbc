      ...
      public PathMovementBase<Creature, WaypointPath const *>
{
  public:
    WaypointMovementGenerator(Creature &)
        : i_nextMoveTime(0), m_scriptTime(0), m_lastReachedWaypoint(0), m_pathId(0), m_pathDuration(0), m_PathOrigin(),
          m_speedChanged(false), m_forcedMovement(FORCED_MOVEMENT_NONE)
    {
    }
    ...
    void SetForcedMovement(ForcedMovement forcedMovement)
    {
  ...
  private:
    ...
    int32 m_scriptTime; // filled with delay change when script is instantly
                        // executed and want to change node delay
 ...
 * from the DB and apply it to the creature's movement generator.
 * Creature will move in sequence from point A -> B -> C -> D -> C -> B -> A ->
 * B ..... Or if you prefer it will go back and forth all along provided nodes.
 */
template <class T> class LinearWPMovementGenerator;

template <> class LinearWPMovementGenerator<Creature> : public WaypointMovementGenerator<Creature>
{
  public:
    ...
    // return WAYPOINT_MOTION_TYPE on purpose so it act like normal waypoint for
    // large majority of the core
    // MovementGeneratorType GetMovementGeneratorType() const override { return
    // LINEAR_WP_MOTION_TYPE; }
