...
static const SummonLocation aAyamissSpawnLocs[] = {
    ...
    {-9717.18f, 1517.72f, 27.4677f}, // teleport location - need to be hardcoded because the player
                                     // is teleported after the larva is summoned
};
...
struct boss_ayamissAI : public CombatAI
{
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case AYAMISS_PARALYZE: {
            Unit *target =
                m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 1, SPELL_PARALYZE, SELECT_FLAG_PLAYER);
            if (!target)
                target =
                    m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_PARALYZE, SELECT_FLAG_PLAYER);
        ...
        case AYAMISS_POISON_STINGER: {
            if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_TOPAGGRO, 0, SPELL_POISON_STINGER,
                                                                 SELECT_FLAG_PLAYER))
