...
struct boss_zuljinAI : public CombatAI
{
          ...
          m_instance(static_cast<instance_zulaman *>(creature->GetInstanceData()))
    {
        ...
        m_creature->GetCombatManager().SetLeashingCheck([&](Unit *, float x, float y, float z) { return y > 734.0f; });
    ...
    void DamageTaken(Unit *dealer, uint32 &damage, DamageEffectType damagetype, SpellEntry const *spellInfo) override
    {
        ...
        if (m_creature->IsStunned() &&
            ((m_creature->GetHealth() - damage) * 100.0f) / m_creature->GetMaxHealth() < 40.f)
    ...
    void ExecutePhaseTransition()
    {
        ...
        m_creature->GetMotionMaster()->MovePoint(POINT_ID_CENTER, fZuljinMoveLoc[0], fZuljinMoveLoc[1],
                                                 fZuljinMoveLoc[2], FORCED_MOVEMENT_RUN);
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case ZULJIN_ACTION_PHASE_TRANSITION: {
            if (m_creature->HasAura(SPELL_LYNX_RUSH) ||
                m_creature->HasAura(SPELL_CLAW_RAGE_TRIGGER)) // do not allow phase transition
                                                              // during lynx rush/claw rage
                return;
        ...
        case ZULJIN_ACTION_FLAME_WHIRL:
            ...
            return;
        case ZULJIN_ACTION_SUMMON_PILLAR:
            if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_SUMMON_PILLAR,
                                                                 SELECT_FLAG_PLAYER))
