...
struct boss_golemaggAI : public CombatAI
{
    ...
    {
        // Send event to the Core Ragers so they know that Golemagg is dead and
        // that they must go suicide
...
struct mob_core_ragerAI : public CombatAI
{
          ...
          m_instance(static_cast<ScriptedInstance *>(creature->GetInstanceData()))
    {
        ...
        AddCombatAction(CORE_RAGER_MANGLE,
                        uint32(7 * IN_MILLISECONDS)); // These times are probably wrong
    ...
    {
        // Event sent by Golemagg at the time of his death so Core Rager knows he
        // can self-suicide
