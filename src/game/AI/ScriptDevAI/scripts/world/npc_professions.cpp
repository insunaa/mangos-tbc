...
SD%Complete: 90
SDComment: Provides learn/unlearn/relearn options for profession
specializations. Not supported: Unlearn engineering, re-learn engineering
SDCategory: NPCs
EndScriptData */
...
#define TALK_AXE_UNLEARN                                                                                               \
    ...
    "choose to abandon it you will forget all recipes that require "                                                   \
    "Axesmithing "                                                                                                     \
    "to create!"
...
#define BOX_UNLEARN_DRAGON                                                                                             \
    "Do you really want to unlearn your Dragonscale Leatherworking "                                                   \
    "specialization and lose all associated recipes?\n"
...
#define BOX_UNLEARN_ELEMENTAL                                                                                          \
    "Do you really want to unlearn your Elemental Leatherworking "                                                     \
    "specialization and lose all associated recipes?\n"
...
#define BOX_UNLEARN_TRIBAL                                                                                             \
    "Do you really want to unlearn your Tribal Leatherworking specialization "                                         \
    ...
    for (unsigned int reqSpell : spell->EffectTriggerSpell)
    {
        ...
        for (int j = EQUIPMENT_SLOT_START; j < EQUIPMENT_SLOT_END; ++j)
        {
            ...
            if (pItem)
                ...
                {
                    // pPlayer has item equipped that require specialty. Not allow
                    // to unlearn, player has to unequip first
                    debug_log("SD2: player attempt to unlearn spell %u, but item %u is "
                              "equipped.",
...
bool GossipHello_npc_prof_blacksmith(Player *pPlayer, Creature *pCreature)
{
        ...
        case N_TRAINER_WEAPON2:
            if (pPlayer->HasSpell(S_WEAPON))
            {
                ...
                if (pCreature->isTrainer())
                    pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_TRAINER, GOSSIP_TEXT_TRAIN, GOSSIP_SENDER_MAIN,
                                             GOSSIP_ACTION_TRAIN);
            }
            break;
        ...
        pPlayer->GetSkillValueBase(SKILL_BLACKSMITHING) >= 250)
    {
        ...
        case N_TRAINER_AXE:
            if (!HasWeaponSub(pPlayer))
                pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_LEARN_AXE, GOSSIP_SENDER_LEARN,
...
void SendConfirmUnlearn_npc_prof_blacksmith(Player *pPlayer, Creature *pCreature, uint32 uiAction)
{
    if (uiAction)
    {
        ...
        case N_TRAINER_AXE:
            ...
            break;
        case N_TRAINER_SWORD:
            pPlayer->ADD_GOSSIP_ITEM_EXTENDED(0, GOSSIP_UNLEARN_SWORD, GOSSIP_SENDER_CHECK, uiAction,
                                              BOX_UNLEARN_WEAPON_SPEC, GetUnlearnCostMedium(pPlayer), false);
...
armorsmith pCreature->CastSpell(pPlayer, 17451, TRIGGERED_OLD_TRIGGERED);

if ((pQuest->GetQuestId() == 5284) || (pQuest->GetQuestId() == 5302)) //
weaponsmith pCreature->CastSpell(pPlayer, 17452, TRIGGERED_OLD_TRIGGERED);
...
{
    // Allow re-learn specialization as long as player is not specialized
    // currently and has completed any specialization quest
        ...
        case N_TRAINER_DRAGON2:
            if (pPlayer->HasSpell(S_DRAGON))
            {
                pPlayer->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_CHAT, GOSSIP_UNLEARN_DRAGON, GOSSIP_SENDER_CHECK,
                                                  GOSSIP_ACTION_INFO_DEF + 1, BOX_UNLEARN_DRAGON,
        ...
        case N_TRAINER_TRIBAL2:
            if (pPlayer->HasSpell(S_TRIBAL))
            {
                pPlayer->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_CHAT, GOSSIP_UNLEARN_TRIBAL, GOSSIP_SENDER_CHECK,
                                                  GOSSIP_ACTION_INFO_DEF + 5, BOX_UNLEARN_TRIBAL,
                                                  GetUnlearnCostMedium(pPlayer), false);
                if (pCreature->isTrainer())
                    pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_TRAINER, GOSSIP_TEXT_TRAIN, GOSSIP_SENDER_MAIN,
                                             GOSSIP_ACTION_TRAIN);
...
{
pPlayer->GetPlayerMenu()->GetGossipMenu()->AddMenuItem(0,GOSSIP_LEARN_DRAGON,
