...
{
    EVADE_NONE,
    EVADE_COMBAT, // In a dungeon in combat evades all hits until target becomes
                  // reachable
...
class CombatManager
{
  public:
    ...
    {
        return m_evadeTimer > 0 || m_evadeState;
    }
    bool IsEvadingHome() const
    {
        return m_evadeState == EVADE_HOME;
    }
    ...
    {
        m_evadeTimer = 10000;
    }                 // 10 seconds after which action is taken
    ...
    bool IsLeashingDisabled()
    {
        return m_leashingDisabled;
    }
    void SetLeashingDisable(bool apply)
    {
        m_leashingDisabled = apply;
    }
  ...
  private:
    ...
    // evade handling
    uint32 m_evadeTimer;     // Used for evade during combat when mob is not running
                             // home and target isnt reachable
