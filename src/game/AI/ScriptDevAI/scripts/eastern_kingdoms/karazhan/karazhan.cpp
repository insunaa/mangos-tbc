...
{
    // Note: this is handled in Acid. The purpose is check which npc from the
    // basement set is alive The function is triggered by eventAI on generic
    // timer
...
void instance_karazhan::Load(const char *chrIn)
{
    ...
    for (uint32 &i : m_auiEncounter)
    {
        if (i == IN_PROGRESS) // Do not load an encounter as "In Progress" -
                              // reset it instead.
            i = NOT_STARTED;
    }
    ...
    case NPC_COLDMIST_WIDOW:
        ...
        if (m_sBasementMobsSet.find(pCreature->GetObjectGuid()) != m_sBasementMobsSet.end())
        {
            ...
            if (m_sBasementMobsSet.empty())
            {
                ...
                if (Creature *pBoss = pCreature->SummonCreature(
                        aBasementEnum[uiIndex].uiEntry, aBasementEnum[uiIndex].fX, aBasementEnum[uiIndex].fY,
                        aBasementEnum[uiIndex].fZ, aBasementEnum[uiIndex].fO, TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN,
                        2 * HOUR * IN_MILLISECONDS, true))
                {
                    if (aBasementEnum[uiIndex].emote2)
                        DoScriptText(urand(0, 1) ? aBasementEnum[uiIndex].emote1 : aBasementEnum[uiIndex].emote2,
                                     pBoss);
                    else
...
void instance_karazhan::DoPrepareChessEvent()
{
    ...
    for (GuidList::const_iterator itr = m_lChessPiecesAlliance.begin(); itr != m_lChessPiecesAlliance.end(); ++itr)
    {
        if (Creature *pChessPiece = instance->GetCreature(*itr))
        {
            ...
            if (!pSquare)
            {
                script_error_log("Instance Karazhan: ERROR Failed to properly load the "
                                 "Chess square for %s.",
    ...
    {
        script_error_log("Instance Karazhan: ERROR Failed to properly load the "
                         "horde side stalkers for the Chess Event.");
        return;
    }
    ...
    {
        script_error_log("Instance Karazhan: ERROR Failed to properly load the "
                         "alliance side stalkers for the Chess Event.");
        return;
    }
...
void instance_karazhan::DoMoveChessPieceToSides(uint32 uiSpellId, uint32 uiFaction, bool bGameEnd /*= false*/)
{
    ...
    {
        // need to provide specific target, in order to ensure the logic of the
        // event
...
void instance_karazhan::DoPrepareOperaStage(Creature *pOrganizer)
{
    ...
    debug_log("SD2: Barnes Opera Event - Introduction complete - preparing "
              "encounter %d",
    ...
    case OPERA_EVENT_ROMULO_AND_JUL:
        pOrganizer->SummonCreature(aOperaLocJul.uiEntry, aOperaLocJul.fX, aOperaLocJul.fY, aOperaLocJul.fZ,
                                   aOperaLocJul.fO, TEMPSPAWN_DEAD_DESPAWN, 0);
...
void instance_karazhan::Update(uint32 uiDiff)
{
    if (m_uiChessResetTimer)
    {
        ...
        {
            for (GuidList::const_iterator itr = m_lChessPiecesAlliance.begin(); itr != m_lChessPiecesAlliance.end();
                 ++itr)
            {
                if (Creature *pTemp = instance->GetCreature(*itr))
                    pTemp->Respawn();
            ...
            for (GuidList::const_iterator itr = m_lChessAllianceStalkerList.begin();
                 itr != m_lChessAllianceStalkerList.end(); ++itr)
            {
                if (Creature *pTemp = instance->GetCreature(*itr))
                {
                    pTemp->Respawn();
                ...
                for (uint32 i = 0; i < 8; i++)
                {
                    if (m_uiVictoryToolTimers[i] <= uiDiff)
                    {
                        if (!m_uiVictoryTimersPhase[i])
                        {
                            ...
                            {
                                if (Creature *square =
                                        instance->GetCreature(m_vChessSquares[urand(0, m_vChessSquares.size() - 1)]))
                                    tool->NearTeleportTo(square->GetPositionX(), square->GetPositionY(),
                                                         square->GetPositionZ(), square->GetOrientation());
                            }
