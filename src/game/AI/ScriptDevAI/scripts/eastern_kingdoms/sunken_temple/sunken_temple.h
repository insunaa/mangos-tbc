...
enum
{
    ...
    NPC_SHADE_OF_HAKKAR = 8440, // Shade of Hakkar appears when the event starts;
                                // will despawn when avatar of hakkar is summoned
    ...
    GO_ALTAR_OF_HAKKAR = 148836, // Used in order to show the player the order of
                                 // the statue activation
    ...
    GO_ATALAI_TRAP_2 = 177485, // The traps are spawned in DB randomly around the statues (we
                               // don't know exactly which statue has which trap)
    ...
    GO_EVIL_CIRCLE = 148998,   // Objects used at the avatar event. they are spawned when the
                               // event starts, and the mobs are summon atop of them
