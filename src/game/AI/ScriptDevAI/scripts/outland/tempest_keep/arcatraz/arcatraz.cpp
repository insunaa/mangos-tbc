...
void instance_arcatraz::SetData(uint32 uiType, uint32 uiData)
{
    ...
    case TYPE_HARBINGERSKYRISS:
        if (uiData == FAIL)
        {
            ...
            for (GuidList::const_iterator itr = m_lSkyrissEventMobsGuidList.begin();
                 itr != m_lSkyrissEventMobsGuidList.end(); ++itr)
            {
            ...
            if (Creature *pMillhouse = GetSingleCreatureFromStorage(NPC_MILLHOUSE))
            {
                ...
                if (!instance->IsRegularDifficulty())
                {
                    ...
                    for (const auto &itr : PlayerList)
                    {
                        ...
                        if (pPlayer &&
                            pPlayer->GetQuestStatus(QUEST_TRIAL_OF_THE_NAARU_TENACITY) == QUEST_STATUS_INCOMPLETE)
...
void instance_arcatraz::JustDidDialogueStep(int32 iEntry)
{
    ...
    case SPELL_TARGET_BETA:
        ...
        break;
    case TYPE_WARDEN_2:
        pMellichar->SummonCreature(NPC_MILLHOUSE, aSummonPosition[1].m_fX, aSummonPosition[1].m_fY,
                                   aSummonPosition[1].m_fZ, aSummonPosition[1].m_fO, TEMPSPAWN_DEAD_DESPAWN, 0);
        break;
