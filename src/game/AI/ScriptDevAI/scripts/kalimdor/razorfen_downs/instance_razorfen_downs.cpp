    ...
    case NPC_TOMB_REAVER:
        ...
        if (m_lSpawnedMobsList.empty())
        {
            if (GameObject *pGo = GetSingleGameObjectFromStorage(GO_GONG))
            {
                ...
                // Workaround - GO need to be respawned - requires core fix, GO
                // shouldn't despawn in the first place
                pGo->Respawn();
            }
        }
        break;
...
void instance_razorfen_downs::DoSpawnWaveIfCan(GameObject *pGo)
{
    ...
    for (uint8 i = 0; i < aWaveSummonInformation[m_uiWaveCounter].m_uiNPCperWave; ++i)
    {
        ...
        if (Creature *pSummoned =
                pGo->SummonCreature(aWaveSummonInformation[m_uiWaveCounter].m_uiNpcEntry, fPosX, fPosY, fPosZ,
                                    aSpawnLocations[uiPos].m_fO, TEMPSPAWN_DEAD_DESPAWN, 0))
        {
