...
typedef std::priority_queue<PrioritizeManaUnitWraper, std::vector<PrioritizeManaUnitWraper>, PrioritizeMana>
...
typedef std::priority_queue<PrioritizeHealthUnitWraper, std::vector<PrioritizeHealthUnitWraper>, PrioritizeHealth>
...
void SpellCastTargets::Update(WorldObject *caster)
{
    ...
    m_unitTarget = m_unitTargetGUID ? (m_unitTargetGUID == caster->GetObjectGuid()
                                           ? static_cast<Unit *>(caster)
...
void SpellLog::FinalizePrevious()
{
    ...
    {
        // not first effect processed so we have to finalize the previous one by
        // putting correct amount of targets
...
Spell::Spell(WorldObject *caster, SpellEntry const *info, uint32 triggeredFlags, ObjectGuid originalCasterGUID,
             SpellEntry const *triggeredBy)
    : m_spellScript(SpellScriptMgr::GetSpellScript(info->Id)), m_auraScript(SpellScriptMgr::GetAuraScript(info->Id)),
      m_spellLog(this), m_partialApplicationMask(0), m_param1(0), m_param2(0), m_trueCaster(caster)
{
...
void Spell::FillTargetMap()
{
    ...
    uint8 effToIndex[MAX_EFFECT_INDEX] = {0, 1, 2}; // Helper array, to link to another tmpUnitList, if the
                                                    // targets for both effects match
    for (uint32 i = 0; i < MAX_EFFECT_INDEX; ++i)
    {
        ...
        if (effectTargetType == TARGET_TYPE_SPECIAL_UNIT) // area auras need custom handling
        {
            ...
            // need to pick a single unit target if existant and use it for area
            // aura owner
            ...
            if (!hadTarget && m_caster) // if no single targeting available use
                                        // caster as default
        ...
        else
        {
            ...
            if (targetA == TARGET_NONE && targetB == TARGET_NONE)
            {
                ...
                if (m_spellInfo->Targets &&
                    SpellTargetMgr::CanEffectBeFilledWithMask(m_spellInfo->Id, i, m_spellInfo->Targets))
                    FillFromTargetFlags(targetingData, SpellEffectIndex(i));
                else if (uint32 defaultTarget =
                             SpellEffectInfoTable[m_spellInfo->Effect[i]].defaultTarget) // else resort to default
                                                                                         // effect type if it exists
            ...
            else // normal case, use existing spell data
            {
                ...
                if (targetB && !ignoredTargets.second)
                    SetTargetMap(SpellEffectIndex(i), targetB, true, targetingData);
                if (effectTargetType == TARGET_TYPE_UNIT_DEST) // special case - no unit target,
                                                               // but need to check for valid
                                                               // units
                    if (SpellTargetInfoTable[targetA].type != TARGET_TYPE_UNIT &&
                        SpellTargetInfoTable[targetB].type != TARGET_TYPE_UNIT) // no fill for unit out of
                                                                                // targets
                        FillFromTargetFlags(targetingData,
                                            SpellEffectIndex(i)); // inefficient call, very rare,
                                                                  // less code duplicity
            }
        }

        bool processedUnits = false; // TODO: Review area aura packet fills for
                                     // area auras with AOE targets
        ...
        case TARGET_TYPE_ITEM:
            if (targetingData.data[i].tempItemList.size() > 0) // Item case
                for (Item *item : targetingData.data[i].tempItemList)
                    AddItemTarget(item,
                                  targetMask[0]); // no spell in all of vanilla through
                                                  // wotlk has item target in B
            if (effectTargetType == TARGET_TYPE_ITEM)
                break;
            // [[fallthrough]]
        case TARGET_TYPE_GAMEOBJECT:
            processedGOs = true;
            for (uint8 rightTarget = 0; rightTarget < 2; ++rightTarget) // need to process target A and B separately due
                                                                        // to effect masks
            {
...
bool Spell::FillUnitTargets(TempTargetingData &targetingData, SpellTargetingData &data, uint32 i)
{
    ...
    auto &ignoredTargets = data.ignoredTargets[i];
    auto &filterScheme = m_filteringScheme[i];
    for (uint8 rightTarget = 0; rightTarget < 2; ++rightTarget) // need to process target A and B separately due to
                                                                // effect masks
    {
        bool ignored = rightTarget ? ignoredTargets.second : ignoredTargets.first;
        if (ignored)
                                      ...
                                      WorldObject *target)
{
    ...
    case SPELL_DAMAGE_CLASS_RANGED:
        ...
        break;
    default:
        if (IsPositiveEffectMask(m_spellInfo, effectMask, caster,
                                 target)) // Check for positive spell
        {
    ...
    if (m_spellInfo->SpellFamilyName == SPELLFAMILY_HUNTER &&
        m_spellInfo->SpellFamilyFlags & uint64(0x000020000000001C))
...
void Spell::AddUnitTarget(Unit *target, uint8 effectMask, CheckException exception)
{
    ...
    targetInfo.missCondition = m_ignoreHitResult
                                   ? SPELL_MISS_NONE
                                   : Unit::SpellHitResult(m_trueCaster, target, m_spellInfo, targetInfo.effectMask,
                                                          m_reflectable, false, &targetInfo.heartbeatResistChance);
    ...
    // Spell have speed (possible inherited from triggering spell) - need
    // calculate incoming time
    float speed = GetSpellSpeed();
    if (speed > 0.0f && affectiveObject && target != affectiveObject)
    {
        // calculate spell incoming interval
        float dist = affectiveObject->GetDistance(target->GetPositionX(), target->GetPositionY(),
                                                  target->GetPositionZ(), DIST_CALC_NONE);
        dist = sqrt(dist); // default distance calculation is raw, apply sqrt
    ...
    if (targetInfo.missCondition == SPELL_MISS_REFLECT)
    {
        ...
        {
            Unit::ProcDamageAndSpell(ProcSystemArguments(nullptr, target, PROC_FLAG_NONE, PROC_FLAG_TAKE_HARMFUL_SPELL,
                                                         PROC_EX_REFLECT, 1, BASE_ATTACK, m_spellInfo));
        ...
        else
        {
            ...
            if (m_caster)
                targetInfo.reflectResult = Unit::SpellHitResult(m_caster, m_caster, m_spellInfo, targetInfo.effectMask,
                                                                m_reflectable, true, &targetInfo.heartbeatResistChance);
            // Caster reflects back spell which was already reflected by victim
            if (targetInfo.reflectResult == SPELL_MISS_REFLECT || !m_caster)
                // Full circle: it's impossible to reflect further, "Immune" shows
                // up
    ...
    {
        bool isReflected =
            targetInfo.missCondition == SPELL_MISS_REFLECT && targetInfo.reflectResult == SPELL_MISS_NONE;
        if ((targetInfo.missCondition == SPELL_MISS_NONE || isReflected) && CanSpellDiminish())
        {
            ...
            targetInfo.diminishGroup = GetDiminishingReturnsGroupForSpell(
                m_spellInfo, m_triggeredByAuraSpell != nullptr || (m_IsTriggeredSpell && m_CastItem));
            targetInfo.diminishLevel = targetForDiminish->GetDiminishing(targetInfo.diminishGroup);

            if (m_duration > 0)
            {
                int32 duration = m_duration;
                targetForDiminish->ApplyDiminishingToDuration(targetInfo.diminishGroup, duration, realCaster,
                                                              targetInfo.diminishLevel, isReflected);
...
{
    m_targets.m_targetMask |= TARGET_FLAG_GAMEOBJECT; // all spells with GO target must have this
                                                      // flag
                                                      // TODO: do this based on target type
...
void Spell::AddDestExecution(SpellEffectIndex effIndex)
{
    if (m_destTargetInfo.effectMask == 0)
    {
        ...
        if (speed > 0.0f)
        {
            ...
            dist = sqrt(dist); // default distance calculation is raw, apply
                               // sqrt before the next step
            if (dist < 5.0f)
                dist = 5.0f;
...
void Spell::DoAllEffectOnTarget(TargetInfo *target)
{
    ...
    Unit *unit = m_trueCaster->GetObjectGuid() == target->targetGUID
                     ? m_caster
                     : ObjectAccessor::GetUnit(*m_trueCaster, target->targetGUID);
    if (!unit)
        return;
    ...
    // spell for example caster bonus or animation, except miss case where will
    // assigned PROC_EX_* flags later
    ...
    {
        if (missInfo == SPELL_MISS_REFLECT) // In case spell reflect from target,
                                            // do all effect on caster (if hit)
        {
            if (target->reflectResult == SPELL_MISS_NONE) // If reflected spell hit caster -> do all
                                                          // effect on him
            {
        ...
        // Failed hostile spell hits count as attack made against target (if
        // detected)
    ...
    if (m_healing && affectiveCaster) // does not support GO casts atm
    {
        ...
        OnHit(missInfo);       // TODO: After spell damage calc is moved to proper
                               // handler
                               // - move this before the first if
        ...
        if (affectiveCaster)
            unitTarget->getHostileRefManager().threatAssist(
                affectiveCaster, float(gain) * 0.5f * sSpellMgr.GetSpellThreatMultiplier(m_spellInfo), m_spellInfo,
                false, m_IsTriggeredSpell);

        // Do triggers for unit (reflect triggers passed on hit phase for correct
        // drop charge)
        if (m_canTrigger && missInfo != SPELL_MISS_REFLECT)
            Unit::ProcDamageAndSpell(ProcSystemArguments(
                affectiveCaster, unitTarget, affectiveCaster ? procAttacker : uint32(PROC_FLAG_NONE), procVictim,
    ...
    else if (m_damage)
    {
        ...
        if (!m_spellInfo->HasAttribute(SPELL_ATTR_EX3_IGNORE_CASTER_MODIFIERS))
        {
            if (target->isCrit) // GOs cant crit
            {
                ...
                spellDamageInfo.damage =
                    affectiveCaster->CalculateCritAmount(unitTarget, spellDamageInfo.damage, m_spellInfo);
            }
            ...
            if (spellDamageInfo.damage > 0)
            {
                ...
                if (m_spellSchoolMask & SPELL_SCHOOL_MASK_NORMAL)
                    spellDamageInfo.damage = Unit::CalcArmorReducedDamage(
                        affectiveCaster ? affectiveCaster : m_trueCaster, unitTarget, spellDamageInfo.damage);
            }
        }

        unitTarget->CalculateAbsorbResistBlock(affectiveCaster, &spellDamageInfo, m_spellInfo);

        Unit::DealDamageMods(affectiveCaster, spellDamageInfo.target, spellDamageInfo.damage, &spellDamageInfo.absorb,
    ...
    // Passive spell hits/misses or active spells only misses (only triggers if
    // proc flags set)
    else if (procAttacker || procVictim)
    {
        ...
        if (m_canTrigger && missInfo != SPELL_MISS_REFLECT)
            ...
            Unit::ProcDamageAndSpell(ProcSystemArguments(
                affectiveCaster, procAttacker & PROC_FLAG_ON_TRAP_ACTIVATION ? m_targets.getUnitTarget() : unit,
                affectiveCaster ? procAttacker : uint32(PROC_FLAG_NONE), procVictim, procEx, 0, m_attackType,
                m_spellInfo, this));
    }
    ...
    if (unit->IsCreature())
        ...
        // finished (+channel finished) ignore pets or autorepeat/melee casts for
        // speed (not exist quest for spells (hm... )
        if (affectiveCaster && !static_cast<Creature *>(unit)->IsPet() && !IsAutoRepeat() &&
            !IsNextMeleeSwingSpell(m_spellInfo) && !IsChannelActive())
    ...
    if (affectiveCaster && affectiveCaster != m_caster && affectiveCaster->IsCreature() &&
        static_cast<Creature *>(affectiveCaster)->AI())
...
void Spell::DoSpellHitOnUnit(Unit *unit, uint32 effectMask, TargetInfo *target, bool isReflected)
{
    ...
    if (traveling && !m_spellInfo->HasAttribute(SPELL_ATTR_NO_IMMUNITIES))
    {
        ...
        if (!notImmunedMask || unit->IsImmuneToDamage(GetSpellSchoolMask(m_spellInfo)) ||
    ...
    if (traveling && m_trueCaster != unit)
    {
        ...
        {
            // for delayed spells ignore not visible explicit target, if caster
            // is dead, nothing is visible for him
            if (unit == m_targets.getUnitTarget() &&
                ((m_trueCaster->IsGameObject() && !unit->IsAlive()) ||
                 !unit->IsVisibleForOrDetect(m_caster, m_caster, false, false, true, true,
                                             m_spellInfo->HasAttribute(SPELL_ATTR_EX6_IGNORE_PHASE_SHIFT))))
            {
                ResetEffectDamageAndHeal();
                return;
            }
    ...
    if (IsSpellAppliesAura(m_spellInfo, effectMask))
    {
        if (affectiveCaster)
        {
            if (traveling) // if travelling, need to recalculate diminishing
                           // level and duration
            {
                ...
                {
                    int32 duration = m_duration;
                    unit->ApplyDiminishingToDuration(target->diminishGroup, duration, m_caster, target->diminishLevel,
                                                     isReflected);
                    target->diminishDuration = duration;
                }
            }

            if (m_duration != target->diminishDuration && target->diminishDuration == 0 &&
                target->diminishLevel > DIMINISHING_LEVEL_1 && !IsSpellWithNonAuraEffect(m_spellInfo))
            {
                Unit::SendSpellMiss(m_trueCaster, unit, m_spellInfo->Id, SPELL_MISS_IMMUNE);
                ResetEffectDamageAndHeal();
                return;
            ...
            const bool pvp = (unit->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PLAYER_CONTROLLED) &&
                              affectiveCaster->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PLAYER_CONTROLLED));
            // Increase Diminishing on unit, actual durations are already
            // calculated
...
void Spell::DoAllTargetlessEffects(bool dest)
{
    ...
    if (effectMask)
    {
        ...
        if (m_caster)
        {
            ...
            Unit::ProcDamageAndSpell(ProcSystemArguments(
                m_caster, procAttacker & PROC_FLAG_ON_TRAP_ACTIVATION ? m_targets.getUnitTarget() : nullptr,
                m_caster ? procAttacker : uint32(PROC_FLAG_NONE), procVictim, procEx, 0, m_attackType, m_spellInfo,
                this));
        }
    }
}
...
void Spell::DoAllEffectOnTarget(GOTargetInfo *target)
{
    ...
    gameObjTarget = go; // redundant but for safety against future change for
                        // hook functionality
    ...
    // cast at creature (or GO) quest objectives update at successful cast
    // finished (+channel finished) ignore autorepeat/melee casts for speed (not
    // exist quest for spells (hm... )
...
void Spell::DoAllEffectOnTarget(ItemTargetInfo *target)
{
    ...
    itemTarget = target->item; // redundant but for safety against future change
...
bool Spell::IsAliveUnitPresentInTargetList()
{
    ...
    for (TargetList::const_iterator ihit = m_UniqueTargetInfo.begin(); ihit != m_UniqueTargetInfo.end(); ++ihit)
    {
        ...
        {
            Unit *unit = m_trueCaster->GetObjectGuid() == ihit->targetGUID
                             ? m_caster
                             : ObjectAccessor::GetUnit(*m_trueCaster, ihit->targetGUID);
            ...
            if (IsValidDeadOrAliveTarget(unit))
                channelAuraMask &= ~ihit->effectHitMask; // remove from need alive mask effect
...
void Spell::SetTargetMap(SpellEffectIndex effIndex, uint32 targetMode, bool targetB, TempTargetingData &targetingData)
{
    ...
    case TARGET_LOCATION_CASTER_RANDOM_CIRCUMFERENCE: {
        ...
        case TARGET_LOCATION_UNIT_RANDOM_SIDE:
        case TARGET_LOCATION_UNIT_RANDOM_CIRCUMFERENCE:
            target = m_targets.getUnitTarget() ? m_targets.getUnitTarget() : m_trueCaster;
            break;
        default:
            target = m_trueCaster;
            break;
    ...
    case TARGET_LOCATION_RANDOM_SIDE: {
        if (radius > 0.0f) // Get a random point in the circle around current
                           // dest target
        {
    ...
    case TARGET_LOCATION_UNIT_MINION_POSITION: // unknown how pet summon is
                                               // different - maybe some
                                               // formation support?
    ...
    case TARGET_LOCATION_CASTER_RIGHT: {
        ...
        case TARGET_LOCATION_UNIT_MINION_POSITION:
            caster = GetCastingObject(); // projected from original caster if
                                         // necessary
        case TARGET_LOCATION_CASTER_LEFT:
            angle += M_PI_F / 2;
            break;
    ...
    case TARGET_LOCATION_CHANNEL_TARGET_DEST:
        ...
        if (WorldObject *channelObject = m_caster->GetChannelObject())
            m_targets.setDestination(channelObject->GetPositionX(), channelObject->GetPositionY(),
                                     channelObject->GetPositionZ());
        break;
    ...
    case TARGET_LOCATION_SW: {
        if (m_targets.m_targetMask & TARGET_FLAG_DEST_LOCATION) // takes current dest and offsets it
        {
            ...
            float angle =
                currentTarget != m_trueCaster ? currentTarget->GetAngle(m_trueCaster) : m_trueCaster->GetOrientation();
    ...
    case TARGET_LOCATION_DATABASE: {
        if (SpellTargetPosition const *st = sSpellMgr.GetSpellTargetPosition(m_spellInfo->Id))
        {
            ...
            // far-teleport spells are handled in SpellEffect, elsewise report
            // an error about an unexpected map (spells are always locally)
            if (st->target_mapId != m_trueCaster->GetMapId() &&
                m_spellInfo->Effect[effIndex] != SPELL_EFFECT_TELEPORT_UNITS &&
                m_spellInfo->Effect[effIndex] != SPELL_EFFECT_BIND)
                sLog.outError("SPELL: wrong map (%u instead %u) target coordinates for "
                              "spell ID %u",
                              st->target_mapId, m_trueCaster->GetMapId(), m_spellInfo->Id);
        }
        else
    ...
    case TARGET_LOCATION_CASTER_FRONT_LEAP: {
        ...
        const float IN_OR_UNDER_LIQUID_RANGE = 0.8f; // range to make player under liquid or on liquid surface
                                                     // from liquid level
            ...
            m_caster->m_movementInfo.HasMovementFlag(MOVEFLAG_FALLING))
        {
            ...
            nextPos.z = prevPos.z - 2.0f; // little hack to avoid the impression to go up
                                          // when teleporting instead of continue to
                                          // fall. This value may need some tweak
            ...
            if (fabs(prevPos.z - nextPos.z) > 40.f) // dont move too high - magical constant - might need
                                                    // verification
            {
                nextPos.x = prevPos.x;
                nextPos.y = prevPos.y;
                ...
                m_caster->GetMap()->GetHitPosition(prevPos.x, prevPos.y, prevPos.z + 0.5f, nextPos.x, nextPos.y,
                                                   nextPos.z, -0.5f);
        }
        else
        {
            // fix origin position if player was jumping and near of the ground
            // but not in ground
            ...
            for (uint32 i = 1; i < numChecks + 1; ++i)
            {
                ...
                if (!m_caster->GetMap()->GetHeightInRange(nextPos.x, nextPos.y, nextPos.z))
                {
                    ...
                    if (!m_caster->GetMap()->GetTerrain()->IsInWater(nextPos.x, nextPos.y, nextPos.z, &liquidData))
                    {
                        // not in water and cannot get correct height, maybe
                        // flying? sLog.outString("Can't get height of point
                        // %u, point value %s", i, nextPos.toString().c_str());
                        nextPos = prevPos;
                        break;
                    }
                    isInLiquid = true;
                ...
                if (isInLiquid || (!isInLiquidTested && m_caster->GetMap()->GetTerrain()->IsInWater(
                                                            nextPos.x, nextPos.y, nextPos.z, &liquidData)))
                {
                    if (!isPrevInLiquid && fabs(liquidData.level - prevPos.z) > 2.0f)
                    {
                        // on edge of water with difference a bit to high to
                        // continue sLog.outString("Ground vs liquid edge
                        // detected!");
                        nextPos = prevPos;
                        break;
                    }
                    ...
                    if ((liquidData.level - IN_OR_UNDER_LIQUID_RANGE) > nextPos.z)
                        nextPos.z = prevPos.z; // we are under water so next z
                                               // equal prev z
                    else
                        nextPos.z = liquidData.level - IN_OR_UNDER_LIQUID_RANGE; // we are on water
                                                                                 // surface, so next z
                                                                                 // equal liquid level
                ...
                // unitTarget->SummonCreature(VISUAL_WAYPOINT, nextPos.x,
                // nextPos.y, nextPos.z, 0, TEMPSUMMON_TIMED_DESPAWN, 15000);
                float hitZ = nextPos.z + 1.5f;
                if (m_caster->GetMap()->GetHitPosition(prevPos.x, prevPos.y, prevPos.z + 1.5f, nextPos.x, nextPos.y,
                                                       hitZ, -1.0f))
                {
                ...
                if (isOnGround)
                {
                    ...
                    {
                        // sLog.outString("bad slope detected! %4.2f max %4.2f,
                        // ac(%4.2f)", slope * 180 / M_PI_F, maxSlope, ac);
                        nextPos = prevPos;
                        break;
                    }
                    // sLog.outString("slope is ok! %4.2f max %4.2f,
                    // ac(%4.2f)", slope * 180 / M_PI_F, maxSlope, ac);
                }

                // sLog.outString("point %u is ok, coords %s", i,
                // nextPos.toString().c_str());
    ...
    case TARGET_LOCATION_CASTER_FISHING_SPOT: {
        float x, y, z;
        // special code for fishing bobber
        // (TARGET_LOCATION_CASTER_FISHING_SPOT), should not try to avoid
        // objects nor try to find ground level, but randomly vary in angle
    ...
    case TARGET_UNIT_NEAR_CASTER: {
        ...
        case TARGET_UNIT_NEAR_CASTER: // TODO: Rename TARGET_UNIT_NEAR_CASTER
                                      // to something better and find real
                                      // difference with
                                      // TARGET_UNIT_FRIEND_NEAR_CASTER.
        {
    ...
    case TARGET_UNIT: {
        ...
        if (targetingData.chainTargetCount[effIndex] > 1)
        {
            ...
            UnitList tempAoeList;
            {
                ...
                tempAoeList.erase(std::remove(tempAoeList.begin(), tempAoeList.end(), newUnitTarget),
                                  tempAoeList.end());
            }
    ...
    case TARGET_UNIT_ENEMY: {
        ...
        // Check if target have Grounding Totem Aura(Magnet target). Check for
        // physical school inside included.
        if (CheckAndAddMagnetTarget(newUnitTarget, effIndex, targetB, targetingData))
            break;
        ...
        if (targetingData.chainTargetCount[effIndex] > 1)
        {
            ...
            UnitList tempAoeList;
            {
                ...
                {
                    if (!(*itr)->IsVisibleForOrDetect(m_caster, m_originalCaster, false, false, true, false,
                                                      m_spellInfo->HasAttribute(SPELL_ATTR_EX6_IGNORE_PHASE_SHIFT)))
    ...
    case TARGET_ENUM_UNITS_SCRIPT_AOE_AT_DEST_LOC: {
        ...
        SQLMultiStorage::SQLMSIteratorBounds<SpellTargetEntry> bounds =
            sSpellScriptTargetStorage.getBounds<SpellTargetEntry>(m_spellInfo->Id);
        // fill real target list if no spell script target defined
        FillAreaTargets(bounds.first != bounds.second ? tempTargetUnitMap : tempUnitList, radius, cone,
                        PUSH_DEST_CENTER, SPELL_TARGETS_ALL);
    ...
    case TARGET_ENUM_GAMEOBJECTS_SCRIPT_AOE_AT_DEST_LOC: {
        ...
        if (!entriesToUse.empty())
        {
            ...
            MaNGOS::GameObjectListSearcher<MaNGOS::AllGameObjectEntriesListInPosRangeCheck> checker(tempGOList,
                                                                                                    go_check);
    ...
    case TARGET_ENUM_UNITS_FRIEND_AOE_AT_DEST_LOC:
        ...
        break;
    // TARGET_UNIT_PARTY means that the spells can only be casted on a party
    ...
    case TARGET_UNIT_FRIEND_CHAIN_HEAL: {
        ...
        if (targetingData.chainTargetCount[effIndex] > 1)
        {
            ...
            // if target and caster are members in the same group then apply
            // member only filtering in regards to mc by player concerns the
            // mc'ed player has simple toolbar thus chain heal not available
            // TODO: in regards to mc by npc concerns this is something that
            ...
            if (m_caster->GetTypeId() == TYPEID_PLAYER)
            {
                if (Group *casterGroup = static_cast<Player *>(m_caster)->GetGroup())
                {
                    ...
                    case TYPEID_UNIT: {
                        ...
                        {
                            if (owner->GetTypeId() == TYPEID_PLAYER &&
                                casterGroup == static_cast<Player *>(owner)->GetGroup())
                            {
                                group = casterGroup;
                            }
                        }
            ...
            UnitList tempAoeList;
            MaNGOS::AnyFriendlyOrGroupMemberUnitInUnitRangeCheck u_check(m_caster, unitTarget, group, m_spellInfo,
                                                                         max_range);
            MaNGOS::UnitListSearcher<MaNGOS::AnyFriendlyOrGroupMemberUnitInUnitRangeCheck> searcher(tempAoeList,
                                                                                                    u_check);
    ...
    default:
        // sLog.outError( "SPELL: Unknown implicit target (%u) for spell ID %u",
        // targetMode, m_spellInfo->Id );
        break;
    }
                                            ...
                                            uint32 targetMode, UnitList &tempUnitList, GameObjectList &tempGOList)
{
    ...
    if (bounds.first == bounds.second)
    {
        if (m_spellInfo->EffectImplicitTargetA[effIndex] == TARGET_UNIT_SCRIPT_NEAR_CASTER ||
            ...
            sLog.outErrorDb("Spell entry %u, effect %i has "
                            "EffectImplicitTargetA/EffectImplicitTargetB = "
                            "TARGET_UNIT_SCRIPT_NEAR_CASTER, but creature are not "
                            "defined in `spell_script_target`",
        ...
        if (m_spellInfo->EffectImplicitTargetA[effIndex] == TARGET_LOCATION_SCRIPT_NEAR_CASTER ||
            ...
            sLog.outErrorDb("Spell entry %u, effect %i has "
                            "EffectImplicitTargetA/EffectImplicitTargetB = "
                            "TARGET_LOCATION_SCRIPT_NEAR_CASTER, but gameobject or "
                            "creature are not defined in `spell_script_target`",
                            m_spellInfo->Id, effIndex);
        ...
        if (m_spellInfo->EffectImplicitTargetA[effIndex] == TARGET_GAMEOBJECT_SCRIPT_NEAR_CASTER ||
            ...
            sLog.outErrorDb("Spell entry %u, effect %i has "
                            "EffectImplicitTargetA/EffectImplicitTargetB = "
                            "TARGET_GAMEOBJECT_SCRIPT_NEAR_CASTER, but gameobject "
    ...
    case SPELL_TARGET_TYPE_GAMEOBJECT_GUID: {
        ...
        else
        {
            ...
            MaNGOS::GameObjectListSearcher<MaNGOS::AllGameObjectEntriesListInObjectRangeCheck> checker(
                foundScriptGOTargets, go_check);
            Cell::VisitGridObjects(m_trueCaster, checker, radius);
        }
        ...
        if (foundScriptGOTargets.size() > targetCount) // if we have too many targets, we need to trim the
                                                       // list
    ...
    case SPELL_TARGET_TYPE_CREATURE_GUID: {
        if (Unit *target = m_targets.getUnitTarget())
        {
            if (target->GetTypeId() == TYPEID_UNIT &&
                entriesToUse.find(type == SPELL_TARGET_TYPE_CREATURE_GUID ? target->GetDbGuid() : target->GetEntry()) !=
                    entriesToUse.end())
            {
        ...
        if (foundScriptCreatureTargets.size() < targetCount)
        {
            ...
            else
            {
                MaNGOS::AllCreatureEntriesWithLiveStateInObjectRangeCheck u_check(
                    *caster, entriesToUse, type != SPELL_TARGET_TYPE_DEAD, radius,
                    type == SPELL_TARGET_TYPE_CREATURE_GUID, false, true);
                ...
                Cell::VisitAllObjects(caster, searcher,
                                      radius); // Visit all, need to find also Pet* objects
            ...
            if (foundScriptCreatureTargets.size() > targetCount) // if we have too many targets, we need to trim
                                                                 // the list
    ...
    if (!foundScriptCreatureTargets.empty())
    {
        ...
        {
            Creature *creatureScriptTarget =
                foundScriptCreatureTargets.front(); // can only have one target in this case
    ...
    {
        /* For TARGET_GAMEOBJECT_SCRIPT_NEAR_CASTER makes DB targets optional not
         * required for now
                                    ...
                                    UnitList &tempTargetUnitMap, UnitList &targetUnitMap, SpellEffectIndex effIndex)
{
    for (UnitList::const_iterator iter = tempTargetUnitMap.begin(); iter != tempTargetUnitMap.end(); ++iter)
    {
             ...
             ++i_spellST)
        {
            ...
            if (spellST->type == SPELL_TARGET_TYPE_CREATURE_GUID ? target->GetDbGuid() == spellST->targetEntry
...
SpellCastResult Spell::PreCastCheck(Aura *triggeredByAura /*= nullptr*/)
{
    ...
    if (result != SPELL_CAST_OK && (!IsAutoRepeat() || m_triggerAutorepeat)) // always cast autorepeat
                                                                             // dummy for triggering
    {
        if (triggeredByAura)
        {
            SendChannelUpdate(0);
...
SpellCastResult Spell::SpellStart(SpellCastTargets const *targets, Aura *triggeredByAura)
{
    ...
    if (!m_trueCaster->IsGameObject()) // gameobjects dont have a sense of
...
void Spell::Prepare()
{
    ...
    {
        // Channeled spell is always one per caster and needs to be tracked and
        // removed on death
...
void Spell::cancel()
{
    ...
    case SPELL_STATE_CHANNELING:
        for (auto &ihit : m_UniqueTargetInfo)
        {
            ...
            {
                Unit *unit = m_caster->GetObjectGuid() == ihit.targetGUID
                                 ? m_caster
        ...
        if (m_spellInfo->HasAttribute(SPELL_ATTR_DISABLED_WHILE_ACTIVE))
            m_caster->RemoveSpellCooldown(*m_spellInfo,
                                          true); // need to clear cooldown in client
...
SpellCastResult Spell::cast(bool skipCheck)
{
    ...
    if (!m_trueCaster->CheckAndIncreaseCastCounter())
    {
        if (m_triggeredByAuraSpell)
            sLog.outError("Spell %u triggered by aura spell %u too deep in cast chain for "
                          "cast. Cast not allowed for prevent overflow stack crash.",
                          m_spellInfo->Id, m_triggeredByAuraSpell->Id);
        else
            sLog.outError("Spell %u too deep in cast chain for cast. Cast not "
                          "allowed for prevent overflow stack crash.",
                          m_spellInfo->Id);
    ...
    if (!m_targets.getUnitTarget() && m_targets.getUnitTargetGuid() &&
        m_targets.getUnitTargetGuid() != m_trueCaster->GetObjectGuid())
    {
        cancel();
    ...
    if (m_trueCaster->IsCreature() && m_targets.getUnitTarget() && m_targets.getUnitTarget() != m_caster &&
        !m_spellInfo->HasAttribute(SPELL_ATTR_EX5_AI_DOESNT_FACE_TARGET))
    {
        Unit *charmer = m_caster->GetCharmer();
        if (charmer && !(charmer->GetTypeId() == TYPEID_PLAYER &&
                         ((Player *)charmer)->GetCamera().GetBody() == m_caster)) // need to check if target doesnt have
                                                                                  // a player controlling it
    ...
    if (m_spellState == SPELL_STATE_FINISHED) // stop cast if spell marked as
                                              // finish somewhere in FillTargetMap
    {
        m_trueCaster->DecreaseCastCounter();
        SetExecutedCurrently(false);
        return SPELL_FAILED_ERROR; // currently not propagating right error here
    ...
    if (IsDelayedSpell())
    {
                            ...
                            if (target->GetObjectGuid() == ihit.targetGUID) // Found in list
                            {
                                if (m_caster->CanAttack(target)) // can attack
                                    if ((!IsPositiveEffectMask(m_spellInfo, ihit.effectMask, m_caster, target) &&
                                         ...
                                         m_caster->CanEnterCombat() && target->CanEnterCombat())) // can see
                                                                                                  // and enter
                                                                                                  // combat
                                    {
        ...
        // 0 victim proc since there is no victim proc dependent on successfull
        // cast for caster
        Unit::ProcDamageAndSpell(ProcSystemArguments(m_caster, procTarget, PROC_EX_NORMAL_HIT, 0, PROC_EX_CAST_END, 0,
...
void Spell::handle_immediate()
{
    ...
    if (m_spellState != SPELL_STATE_CHANNELING)
        finish(); // successfully finish spell cast (not last in case autorepeat
                  // or channel spell)
}
...
{
    // (SPELL_ATTR_DISABLED_WHILE_ACTIVE) have infinity cooldown,
    // (SPELL_ATTR_PASSIVE) passive cooldown at triggering
    if (m_spellInfo->HasAttribute(SPELL_ATTR_PASSIVE) || m_channelOnly)
        return;

    m_trueCaster->AddCooldown(*m_spellInfo, m_CastItem ? m_CastItem->GetProto() : nullptr,
...
void Spell::update(uint32 difftime)
{
    ...
    {
        m_updated = true;
        if (m_creationTime == m_trueCaster->GetMap()->GetCurrentMSTime()) // do not update in the same tick as
                                                                          // created
            return;
    }
    ...
    // (exclude casting on vehicles)
    if ((m_trueCaster->IsUnit() && m_timer != 0) &&
        (m_castPositionX != m_trueCaster->GetPositionX() || m_castPositionY != m_trueCaster->GetPositionY() ||
         m_castPositionZ != m_trueCaster->GetPositionZ()) &&
        (m_spellInfo->Effect[EFFECT_INDEX_0] != SPELL_EFFECT_STUCK ||
         !m_trueCaster->m_movementInfo.HasMovementFlag(MOVEFLAG_FALLINGFAR)))
    {
        // always cancel for channeled spells
        if (m_spellState == SPELL_STATE_CHANNELING)
        {
            if (m_spellInfo->ChannelInterruptFlags & AURA_INTERRUPT_FLAG_MOVING &&
    ...
    case SPELL_STATE_CHANNELING: {
            ...
            if (m_trueCaster->IsUnit())
            {
                ...
                if (m_caster->IsCrowdControlled())
                {
                    ...
                    if (!m_spellInfo->HasAttribute(SPELL_ATTR_EX_CHANNELED_1) &&
                        !m_spellInfo->HasAttribute(SPELL_ATTR_EX3_UNK28))
                        cancel();
                }

                if (m_spellInfo->HasAttribute(SPELL_ATTR_EX2_TAME_BEAST)) // these fail on lost
                                                                          // target attention (aggro)
                {
                    if (Unit *target = dynamic_cast<Unit *>(m_caster->GetChannelObject()))
                    {
                    ...
                    m_UniqueTargetInfo.begin() != m_UniqueTargetInfo.end())
                {
                    if (WorldObject *target = m_caster->GetChannelObject())
                    {
                        if (target != m_caster)
                        {
                            ...
                            m_castOrientation = orientation; // need to update cast
                ...
                if (m_spellInfo->ChannelInterruptFlags & AURA_INTERRUPT_FLAG_TURNING)
                {
                    ...
                    case TYPEID_PLAYER: {
                        ...
                        if (m_spellInfo->HasAttribute(SPELL_ATTR_EX_CHANNEL_TRACK_TARGET)) // On
                                                                                           // start,
                                                                                           // client
                                                                                           // turns
                                                                                           // player
                            ...
                            targetOrientationDiff +=
                                M_PI_F * (std::max(1200 - (m_duration - int32(m_timer)), 0) / 1200.f);
                ...
                // need check distance for channeled target only - but only
                // when its set
        ...
        if (m_timer == 0)
        {
            ...
            // cast channel finished ignore autorepeat/melee casts for speed
            // (not exist quest for spells (hm... )
            if (!IsAutoRepeat() && !IsNextMeleeSwingSpell(m_spellInfo))
            {
                if (Player *p = m_caster->GetBeneficiaryPlayer())
                {
                    for (TargetList::const_iterator ihit = m_UniqueTargetInfo.begin(); ihit != m_UniqueTargetInfo.end();
                         ++ihit)
                    {
                    ...
                    for (GOTargetList::const_iterator ihit = m_UniqueGOTargetInfo.begin();
...
void Spell::finish(bool ok)
{
    ...
    if (m_trueCaster->IsInWorld()) // some teleport spells put caster in between
                                   // maps, need to check
    {
        ...
        if (map->IsDungeon())
            ((DungeonMap *)map)
                ->GetPersistanceState()
                ->UpdateEncounterState(ENCOUNTER_CREDIT_CAST_SPELL, m_spellInfo->Id);
    }
}
...
void Spell::SendCastResult(Player const *caster, SpellEntry const *spellInfo, uint8 cast_count, SpellCastResult result,
                           bool isPetCastResult /*=false*/, uint32 param1 /*=0*/, uint32 param2 /*=0*/)
{
    if (result == SPELL_CAST_OK)
        return;
    ...
    data << uint8(!IsPassiveSpell(spellInfo) ? result
                                             : SPELL_FAILED_DONT_REPORT); // do not report failed passive spells
    if (!isPetCastResult)
    ...
    else
    {
        ...
        for (uint8 i = 0; i < MAX_VIRTUAL_ITEM_SLOT; ++i)
        {
                    ...
                    m_caster->GetByteValue(UNIT_VIRTUAL_ITEM_INFO + (i * 2) + 0, VIRTUAL_ITEM_INFO_0_OFFSET_CLASS))
            {
                ...
                {
                    switch (m_caster->GetByteValue(UNIT_VIRTUAL_ITEM_INFO + (i * 2) + 0,
                                                   VIRTUAL_ITEM_INFO_0_OFFSET_SUBCLASS))
                    {
                    case ITEM_SUBCLASS_WEAPON_THROWN:
                        ammoDisplayID = m_caster->GetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_DISPLAY + i);
                        ammoInventoryType = m_caster->GetByteValue(UNIT_VIRTUAL_ITEM_INFO + (i * 2) + 1,
                    ...
                    default:
                        ...
                        nonRangedAmmoInventoryType = m_caster->GetByteValue(UNIT_VIRTUAL_ITEM_INFO + (i * 2) + 1,
                                                                            VIRTUAL_ITEM_INFO_1_OFFSET_INVENTORYTYPE);
                        break;
                    }
...
void Spell::WriteSpellGoTargets(WorldPacket &data)
{
    ...
    {
        sLog.outError("Spell ID %u cast by %s hit/missed too many unit targets %u. "
                      "Ignored after 255.",
                      m_spellInfo->Id, m_caster->GetObjectGuid().GetString().c_str(),
    ...
    {
        sLog.outError("Spell ID %u cast by %s hit/missed too many GO targets %u. "
                      "Ignored after 255.",
                      m_spellInfo->Id, m_caster->GetObjectGuid().GetString().c_str(),
                      (uint32)m_UniqueGOTargetInfo.size());
        m_UniqueGOTargetInfo.resize(255);
    }

    if (m_UniqueGOTargetInfo.size() + m_UniqueTargetInfo.size() > 255)
    {
        sLog.outError("Spell ID %u cast by %s hit/missed too many targets %u. Ignored "
                      "after "
                      "255.",
                      m_spellInfo->Id, m_caster->GetObjectGuid().GetString().c_str(),
    ...
    for (auto &ihit : m_UniqueTargetInfo)
    {
        if (ihit.effectHitMask == 0 && ihit.effectMask != 0) // No effect apply - all immuned add state
        {
            ...
            if (IsChanneledSpell(m_spellInfo) &&
                ihit.targetGUID == m_targets.getUnitTargetGuid()) // can happen due to DR
            ...
            if (Unit *target = dynamic_cast<Unit *>(m_caster->GetChannelObject()))
            {
                if (lastTick)
                {
                    if (SpellAuraHolder *holder =
                            target->GetSpellAuraHolder(m_spellInfo->Id, m_caster->GetObjectGuid()))
                        holder->UpdateHolder(lastTick);
                }
...
void Spell::SendChannelStart(uint32 duration)
{
    ...
    else if (!m_UniqueTargetInfo.empty())
    {
        for (TargetList::const_iterator itr = m_UniqueTargetInfo.begin(); itr != m_UniqueTargetInfo.end(); ++itr)
        {
            ...
            {
                // when immune, duration is already 0, still need to fetch data
                // for caster
                if (itr->diminishGroup > DIMINISHING_NONE &&
                    (itr->diminishDuration != duration || diminishLevel == DIMINISHING_LEVEL_IMMUNE))
                {
                ...
                if (m_spellInfo->EffectRadiusIndex[EFFECT_INDEX_0] != 0 &&
                    (m_spellInfo->EffectApplyAuraName[EFFECT_INDEX_0] == SPELL_AURA_MOD_POSSESS ||
                     m_spellInfo->EffectApplyAuraName[EFFECT_INDEX_0] == SPELL_AURA_BIND_SIGHT))
                    m_maxRange =
    ...
    // change channel duration to account for DR if neccessary, also store in
    // caster's targetInfo to use later for setting aura duration
...
void Spell::TakeCastItem()
{
    ...
    if (!proto)
    {
        ...
        // I'm not sure, if this is really an error, but I guess every item needs
        // a prototype
...
void Spell::HandleThreatSpells()
{
    ...
    if (m_negativeEffectMask & effectMask)
    {
        ...
        // check at spell_threat loading probably not perfect so abort when only
        // some effects are negative.
        if ((m_negativeEffectMask & effectMask) != effectMask)
        {
            DEBUG_FILTER_LOG(LOG_FILTER_SPELL_CAST,
                             "Spell %u, rank %u, is not clearly positive or "
                             "negative, ignoring bonus threat",
                             m_spellInfo->Id, sSpellMgr.GetSpellRank(m_spellInfo->Id));
            return;
        }
        positive = false;
    }
    ...
    for (TargetList::const_iterator ihit = m_UniqueTargetInfo.begin(); ihit != m_UniqueTargetInfo.end(); ++ihit)
    {
        ...
        Unit *target = m_trueCaster->GetObjectGuid() == ihit->targetGUID
                           ? m_caster
                           : ObjectAccessor::GetUnit(*m_trueCaster, ihit->targetGUID);
        if (!target)
            continue;
...
void Spell::AddTriggeredSpell(uint32 spellId)
{
    ...
    if (!spellInfo)
    {
        sLog.outError("Spell::AddTriggeredSpell: unknown spell id %u used as "
                      "triggred spell for spell %u)",
...
void Spell::AddPrecastSpell(uint32 spellId)
{
    ...
    if (!spellInfo)
    {
        sLog.outError("Spell::AddPrecastSpell: unknown spell id %u used as "
                      "pre-cast spell for spell %u)",
                      spellId, m_spellInfo->Id);
        return;
    }
...
SpellCastResult Spell::CheckCast(bool strict)
{
    ...
    if (m_caster)
    {
        if (!m_caster->IsAlive() && m_caster->GetTypeId() == TYPEID_PLAYER &&
            !m_spellInfo->HasAttribute(SPELL_ATTR_CASTABLE_WHILE_DEAD) &&
            ...
            VMAP::VMapFactory::createOrGetVMapManager()->isLineOfSightCalcEnabled())
        {
            if (m_spellInfo->HasAttribute(SPELL_ATTR_OUTDOORS_ONLY) &&
                ...
                return SPELL_FAILED_ONLY_OUTDOORS; // TODO: If at least one effect
                                                   // is SPELL_AURA_MOUNTED return
                                                   // mounts not allowed
        ...
        if (!m_IsTriggeredSpell && NeedsComboPoints(m_spellInfo) &&
            (!m_targets.getUnitTarget() ||
             m_targets.getUnitTarget()->GetObjectGuid() != m_caster->GetComboTargetGuid()))
            // warrior not have real combo-points at client side but use this way
            // for mark allow Overpower use
            return m_caster->getClass() == CLASS_WARRIOR ? SPELL_FAILED_CASTER_AURASTATE : SPELL_FAILED_NO_COMBO_POINTS;

        if (m_caster->GetTypeId() == TYPEID_PLAYER)
        {
            // cancel autorepeat spells if cast start when moving
            // (not wand currently autorepeat cast delayed to moving stop anyway
            // in spell update code)
            if (m_caster->IsMovingIgnoreFlying())
            {
                // skip stuck spell to allow use it in falling case and apply
                // spell limitations at movement
                if ((!m_caster->m_movementInfo.HasMovementFlag(MOVEFLAG_FALLINGFAR) ||
                     m_spellInfo->Effect[EFFECT_INDEX_0] != SPELL_EFFECT_STUCK) &&
    ...
    if (target)
    {
        // TARGET_UNIT_SCRIPT_NEAR_CASTER fills unit target per client
        // requirement but should not be checked against common things
        ...
        if (!IsSpellWithScriptUnitTarget(m_spellInfo))
        {
            ...
            if (m_spellInfo->HasAttribute(SPELL_ATTR_EX_CHANNEL_TRACK_TARGET) // TODO: Investigate this
                                                                              // condition
            ...
            {
                // target state requirements (apply to non-self only), to allow
                // cast affects to self like Dirty Deeds
                if (m_spellInfo->TargetAuraState &&
                    !target->HasAuraStateForCaster(AuraState(m_spellInfo->TargetAuraState),
                ...
                if (!IsIgnoreLosSpellCast(m_spellInfo) && !m_IsTriggeredSpell &&
                    !m_trueCaster->IsWithinLOSInMap(target, true))
                    return SPELL_FAILED_LINE_OF_SIGHT;

                if (m_trueCaster->IsPlayer())
                {
                    // auto selection spell rank implemented in
                    // WorldSession::HandleCastSpellOpcode this case can be
                    // triggered if rank not found (too low-level target for
                    // first rank)
                    if (!m_CastItem && !m_IsTriggeredSpell)
                        // spell expected to be auto-downranking in cast handle, so
                        // must be same
                        if (m_spellInfo != sSpellMgr.SelectAuraRankForLevel(m_spellInfo, target->GetLevel()))
                            return SPELL_FAILED_LOWLEVEL;

                    // Do not allow these spells to target creatures not tapped
                    // by us (Banish, Polymorph, many quest spells) Checked here,
                    // for single target spells, checked in CheckTarget to take
                    // care of AoE spells
                    if (m_spellInfo->HasAttribute(SPELL_ATTR_EX2_CANT_TARGET_TAPPED) &&
                        ...
                        if (Creature const *targetCreature = dynamic_cast<Creature *>(target))
                            if ((!targetCreature->GetLootRecipientGuid().IsEmpty()) &&
                                !targetCreature->IsTappedBy(static_cast<Player *>(m_trueCaster)))
                                return SPELL_FAILED_CANT_CAST_ON_TAPPED;

                    // Do not allow spells to complete which are targeting
                    // players that are invisible to the caster since the time of
                    // cast start
                    if (!m_trueCaster->IsGameObject() &&
                        target->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PLAYER_CONTROLLED) &&
                        !IsPositiveEffectMask(m_spellInfo, affectedMask, m_trueCaster, target) &&
                        !target->IsVisibleForOrDetect(m_caster, m_trueCaster, false, false, true, false,
                                                      m_spellInfo->HasAttribute(SPELL_ATTR_EX6_IGNORE_PHASE_SHIFT)))
                        return SPELL_FAILED_BAD_TARGETS;
                }
            ...
            if ((m_targets.m_targetMask == TARGET_FLAG_SELF || m_trueCaster == target) &&
                m_spellInfo->HasAttribute(SPELL_ATTR_EX_CANT_TARGET_SELF))
            {
                if (IsOnlySelfTargeting(m_spellInfo))
                    sLog.outCustomLog("Spell ID %u cast at self explicitly even though "
                                      "it has SPELL_ATTR_EX_CANT_TARGET_SELF",
                                      m_spellInfo->Id);
            ...
            if (m_spellInfo->HasAttribute(SPELL_ATTR_EX2_FACING_TARGETS_BACK) &&
                m_spellInfo->HasAttribute(SPELL_ATTR_EX_FACING_TARGET) && !m_trueCaster->IsFacingTargetsBack(target))
            {
                // Exclusion for Pounce: Facing Limitation was removed in 2.0.1,
                // but it still uses the same, old Ex-Flags
            ...
            if (((m_spellInfo->Attributes == (SPELL_ATTR_ABILITY | SPELL_ATTR_NOT_SHAPESHIFT |
                 ...
                 !m_trueCaster->IsFacingTargetsFront(target)))
                return SPELL_FAILED_NOT_INFRONT;

            // check if target is in combat
            if (non_caster_target && m_spellInfo->HasAttribute(SPELL_ATTR_EX_NOT_IN_COMBAT_TARGET) &&
                target->IsInCombat())
                return SPELL_FAILED_TARGET_AFFECTING_COMBAT;

            // check if target is affected by Spirit of Redemption (Aura: 27827)
            // unless death persistent
            if (target->HasAuraType(SPELL_AURA_SPIRIT_OF_REDEMPTION) &&
                !m_spellInfo->HasAttribute(SPELL_ATTR_EX3_DEATH_PERSISTENT))
                return SPELL_FAILED_BAD_TARGETS;

            // Check if more powerful spell applied on target (if spell only
            // contains non-aoe auras)
            if (IsAuraApplyEffects(m_spellInfo, SpellEffectIndexMask(affectedMask)) &&
                !IsAreaOfEffectSpell(m_spellInfo) && !HasAreaAuraEffect(m_spellInfo) && !selfTargeting)
            {
                ...
                for (auto const &pair : target->GetSpellAuraHolderMap())
                {
                    ...
                    else if (m_spellInfo == existingSpell &&
                             m_spellInfo->HasAttribute(SPELL_ATTR_EX4_NOT_CHECK_SELFCAST_POWER))
                        continue;
                    ...
                    if (!computed)
                    {
                        for (uint32 i = 0; i < MAX_EFFECT_INDEX; ++i)
                        {
                            if (affectedMask & (1 << i))
                            {
                                ...
                                amounts[i] = Aura::CalculateAuraEffectValue(m_caster, target, m_spellInfo,
                                                                            SpellEffectIndex(i), amounts[i]);
                                if (m_auraScript)
                                {
    ...
    SpellCastResult locRes = sSpellMgr.GetSpellAllowedInLocationError(
        m_spellInfo, m_trueCaster->GetMapId(), zone, area, m_caster ? m_caster->GetBeneficiaryPlayer() : nullptr);
    ...
    if (m_trueCaster->IsPlayer() && m_caster->GetMountID() && !m_IsTriggeredSpell && !IsPassiveSpell(m_spellInfo) &&
    ...
    if (m_channelOnly)
    {
        m_partialApplicationMask = EFFECT_MASK_ALL; // no effects to be executed but
                                                    // spell needs to go through
        return SPELL_CAST_OK;
    }
    ...
    for (uint32 i = 0; i < MAX_EFFECT_INDEX; ++i)
    {
        ...
        case SPELL_EFFECT_DUMMY: {
            ...
            else if (m_spellInfo->SpellIconID == 156) // Holy Shock
            {
                ...
                // Prevents usage when cant neither attack or assist and not in
                // front for shock attack
        ...
        case SPELL_EFFECT_CREATE_ITEM: {
            if (Unit *target = m_targets.getUnitTarget())
            {
                if (!target->IsPlayer() && m_spellInfo->EffectImplicitTargetA[i] != TARGET_UNIT_CASTER &&
                    m_spellInfo->EffectImplicitTargetB[i] != TARGET_UNIT_CASTER)
                    return SPELL_FAILED_BAD_TARGETS;
        ...
        case SPELL_EFFECT_TAMECREATURE: {
            // Spell can be triggered, we need to check original caster prior
            // to caster
            ...
            if (!caster || caster->GetTypeId() != TYPEID_PLAYER || !tameTarget ||
                tameTarget->GetTypeId() == TYPEID_PLAYER)
                return SPELL_FAILED_BAD_TARGETS;
        ...
        case SPELL_EFFECT_LEARN_SPELL: {
            ...
            SpellEntry const *learn_spellproto =
                sSpellTemplate.LookupEntry<SpellEntry>(m_spellInfo->EffectTriggerSpell[i]);
        ...
        case SPELL_EFFECT_CHARGE: {
            ...
            if (Unit *target = m_targets.getUnitTarget())
            {
                ...
                // if
                // (!m_caster->m_movementInfo.HasMovementFlag(MovementFlags(MOVEFLAG_FALLING
                // | MOVEFLAG_FALLINGFAR)) && (pos.coord_z <
                // m_caster->GetPositionZ())
                // && (fabs(pos.coord_z - m_caster->GetPositionZ()) < 3.0f))
                //{
        ...
        case SPELL_EFFECT_SKINNING: {
            ...
            if (creature->GetLootStatus() != CREATURE_LOOT_STATUS_LOOTED) // ||
        ...
        case SPELL_EFFECT_OPEN_LOCK: {
            ...
            if (GameObject *go = m_targets.getGOTarget())
            {
                ...
                // check if its in use only when cast is finished (called from
                // spell::cast() with strict = false)
                ...
                if (go->GetGOInfo()->CannotBeUsedUnderImmunity() &&
            ...
            SpellCastResult res =
                CanOpenLock(effIdx, lockId, m_effectSkillInfo[effIdx].skillId, m_effectSkillInfo[effIdx].reqSkillValue,
                            m_effectSkillInfo[effIdx].skillValue);
            if (res != SPELL_CAST_OK)
                return res;

            // chance for fail at orange mining/herb/LockPicking gathering
            // attempt second check prevent fail at rechecks
            if (!strict && m_effectSkillInfo[effIdx].skillId != SKILL_NONE)
            {
                bool canFailAtMax = m_effectSkillInfo[effIdx].skillId != SKILL_HERBALISM &&
                                    m_effectSkillInfo[effIdx].skillId != SKILL_MINING;
        ...
        // summon types similar SPELL_EFFECT_SUMMON_CRITTER,
        // SPELL_EFFECT_SUMMON_WILD or SPELL_EFFECT_SUMMON_GUARDIAN. These
        // won't show up in m_caster->GetPetGUID()
            ...
            if (map->IsDungeon())
            {
                ...
                if (AreaTrigger const *at = sObjectMgr.GetMapEntranceTrigger(mapId))
                {
                    ...
                    if (AREA_LOCKSTATUS_OK != target->GetAreaTriggerLockStatus(at, miscRequirement))
                        return SPELL_FAILED_BAD_TARGETS; // TODO: Verify this
                                                         // result
                }
            }
            break;
        ...
        case SPELL_EFFECT_TELEPORT_UNITS_FACE_CASTER: {
            ...
            if (m_caster->GetTypeId() == TYPEID_PLAYER)
            {
                ...
                // not allow use this effect at battleground until battleground
                // start
    ...
    for (uint32 i = 0; i < MAX_EFFECT_INDEX; ++i)
    {
        ...
        case SPELL_AURA_MOD_POSSESS: {
            ...
            if (expectedTarget) // target may not be known at CheckCast time -
                                // TODO: add support for these checks on
                                // CheckTarget
            {
                ...
                if (expectedTarget->GetOwner() &&
                    expectedTarget->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PLAYER_CONTROLLED))
                    return SPELL_FAILED_TARGET_IS_PLAYER_CONTROLLED;
            }
            break;
        }
        case SPELL_AURA_MOD_CHARM: {
            if (!m_spellInfo->HasAttribute(SPELL_ATTR_EX_DISMISS_PET) && m_caster->GetTypeId() == TYPEID_PLAYER &&
                m_caster->HasCharm())
                return SPELL_FAILED_ALREADY_HAVE_CHARM;
        ...
        case SPELL_AURA_MOUNTED: {
            ...
            // Ignore map check if spell have AreaId. AreaId already checked
            // and this prevent special mount spells
        ...
        case SPELL_AURA_WATER_WALK: {
            if (expectedTarget && expectedTarget->GetTypeId() == TYPEID_PLAYER)
            {
                ...
                // Player is not allowed to cast water walk on
                // shapeshifted/mounted player
        ...
        case SPELL_AURA_MIRROR_IMAGE: {
            if (expectedTarget)
            {
                // Target must be creature. TODO: Check if target can also be
                // player
                ...
                // It is assumed that target can not be cloned if already
                // cloned by same or other clone auras
    ...
    if (m_targets.m_targetMask & TARGET_FLAG_TRADE_ITEM)
    {
        ...
        if (TradeData *my_trade = static_cast<Player *>(m_caster)->GetTradeData())
        {
            ...
            {
                // Spell will be casted at completing the trade. Silently ignore
                // at this place
...
SpellCastResult Spell::CheckPetCast(Unit *target)
{
    ...
    if (m_caster->IsNonMeleeSpellCasted(false) && !m_ignoreConcurrentCasts) // prevent spellcast interruption by another
                                                                            // spellcast
        return SPELL_FAILED_SPELL_IN_PROGRESS;
...
std::pair<float, float> Spell::GetMinMaxRange(bool strict)
{
    ...
    if (spellRange)
    {
        ...
        else
        {
            ...
            {
                rangeMod =
                    m_caster->GetCombatReach() + (target ? target->GetCombatReach() : m_caster->GetCombatReach());
...
SpellCastResult Spell::CheckRange(bool strict)
{
    ...
    if (target && target != m_trueCaster)
    {
        ...
        float dist = m_trueCaster->GetDistance(target, true,
                                               DIST_CALC_NONE); // minRange/maxRange already contain everything
        if (dist > maxRange * maxRange)
            return SPELL_FAILED_OUT_OF_RANGE;
        if (minRange && dist < minRange * minRange)
            return SPELL_FAILED_TOO_CLOSE;
        if (m_trueCaster->IsPlayer() && (m_spellInfo->FacingCasterFlags & SPELL_FACING_FLAG_INFRONT) &&
            !m_trueCaster->HasInArc(target))
            return SPELL_FAILED_UNIT_NOT_INFRONT;
    }

    if (GameObject *goTarget = m_targets.getGOTarget())
        if (!goTarget->IsAtInteractDistance(dynamic_cast<Player *>(m_trueCaster),
                                            maxRange)) // only player casts these
            return SPELL_FAILED_OUT_OF_RANGE;

    if (m_targets.m_targetMask == TARGET_FLAG_DEST_LOCATION)
    {
        float dist = m_trueCaster->GetDistance(m_targets.m_destPos.x, m_targets.m_destPos.y, m_targets.m_destPos.z,
                                               DIST_CALC_NONE);
        if (dist > maxRange * maxRange)
            return SPELL_FAILED_OUT_OF_RANGE;
        if (minRange && dist < minRange * minRange)
    ...
    {
        float dist =
            m_trueCaster->GetDistance(m_targets.m_srcPos.x, m_targets.m_srcPos.y, m_targets.m_srcPos.z, DIST_CALC_NONE);
        if (dist > maxRange * maxRange)
            return SPELL_FAILED_OUT_OF_RANGE;
        if (minRange && dist < minRange * minRange)
            return SPELL_FAILED_TOO_CLOSE;
...
int32 Spell::CalculateSpellEffectDamage(Unit *unitTarget, int32 damage)
{
    ...
    case SPELL_DAMAGE_CLASS_MELEE: {
        ...
        if (!m_trueCaster->IsGameObject())
            damage = m_caster->MeleeDamageBonusDone(unitTarget, damage, m_attackType, m_spellSchoolMask, m_spellInfo,
                                                    SPELL_DIRECT_DAMAGE);
        damage = unitTarget->MeleeDamageBonusTaken(m_trueCaster->IsGameObject() ? nullptr : m_caster, damage,
                                                   m_attackType, m_spellSchoolMask, m_spellInfo, SPELL_DIRECT_DAMAGE);
    }
    break;
    // Magical Attacks
    ...
    case SPELL_DAMAGE_CLASS_MAGIC: {
        ...
        damage = unitTarget->SpellDamageBonusTaken(m_trueCaster->IsGameObject() ? nullptr : m_caster, m_spellInfo,
                                                   damage, SPELL_DIRECT_DAMAGE);
    }
    break;
    }
    return damage;
...
SpellCastResult Spell::CheckItems()
{
    ...
    if (m_CastItem)
    {
        ...
        if (proto->Class == ITEM_CLASS_CONSUMABLE)
        {
            ...
            for (uint32 i = 0; i < MAX_EFFECT_INDEX; ++i)
            {
                ...
                // TARGET_UNIT_CASTER_PET m_targets.getUnitTarget() is not the
    ...
    // if not item target then required item must be equipped (for triggered case
    // not report error)
    else
    {
        uint32 error = SPELL_FAILED_EQUIPPED_ITEM_CLASS;
        if (m_caster->IsPlayer() &&
            !static_cast<Player *>(m_caster)->HasItemFitToSpellReqirements(m_spellInfo, nullptr, &error))
    ...
    for (uint32 i = 0; i < MAX_EFFECT_INDEX; ++i)
    {
        ...
        case SPELL_EFFECT_CREATE_ITEM: {
            ...
            if (!m_IsTriggeredSpell && m_spellInfo->EffectItemType[i])
            {
                ...
                if (!itemProto)
                    return SPELL_FAILED_ITEM_NOT_FOUND; // custom error in case
                                                        // item template is
                                                        // missing
                ItemPosCountVec dest;
...
void Spell::UpdateOriginalCasterPointer()
{
    ...
    {
        GameObject *go =
            m_trueCaster->IsInWorld() ? m_trueCaster->GetMap()->GetGameObject(m_originalCasterGUID) : nullptr;
...
bool Spell::CheckTarget(Unit *target, SpellEffectIndex eff, bool targetB, CheckException exception) const
{
    ...
    if (target != affectiveCaster)
    {
        ...
        // A player can cast spells on his pet (or other controlled unit) though
        // in any state
        if ((!affectiveCaster || target->GetMasterGuid() != affectiveCaster->GetObjectGuid()) && !scriptTarget)
        {
            // unselectable targets skipped in all cases except targets with
            // TARGET_SCRIPT
        ...
        // Check player targets and remove if in GM mode or GM invisibility (for
        // not self casting case)
        if (target->IsPlayer())
        {
            if (static_cast<Player *>(target)->GetVisibility() == VISIBILITY_OFF)
                return false;

            if (static_cast<Player *>(target)->IsGameMaster() &&
                !IsPositiveEffect(m_spellInfo, eff, affectiveCaster, target))
                return false;
        }
        ...
        if (affectiveCaster && affectiveCaster->IsPlayer())
        {
            ...
            if (m_spellInfo->HasAttribute(SPELL_ATTR_EX2_CANT_TARGET_TAPPED))
                if (Creature const *targetCreature = dynamic_cast<Creature *>(target))
                    if ((!targetCreature->GetLootRecipientGuid().IsEmpty()) &&
                        !targetCreature->IsTappedBy(static_cast<Player *>(affectiveCaster)))
                        return false;
        }
    }
                    ...
                    if (target != m_trueCaster)
                    {
                        if (m_spellInfo->EffectImplicitTargetA[eff] == TARGET_LOCATION_CHANNEL_TARGET_DEST)
                        {
                            if (DynamicObject *dynObj = m_caster->GetDynObject(
                                    m_triggeredByAuraSpell ? m_triggeredByAuraSpell->Id : m_spellInfo->Id))
    ...
    if (targetType != TARGET_UNIT_CASTER && targetType != TARGET_UNIT_CASTER_PET &&
        target->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE_2))
        return false;
...
SpellEvent::~SpellEvent()
{
    ...
    {
        sLog.outError("~SpellEvent: %s %u tried to delete non-deletable spell %u. "
        ...
        sLog.outCustomLog("~SpellEvent: %s %u tried to delete non-deletable spell %u. Was not "
                          "deleted, causes memory leak.",
                          (m_Spell->GetCaster()->GetTypeId() == TYPEID_PLAYER ? "Player" : "Creature"),
                          m_Spell->GetCaster()->GetGUIDLow(), m_Spell->m_spellInfo->Id);
    }
}
    ...
    case SPELL_STATE_TRAVELING: {
        ...
        if (m_Spell->GetDelayStart() != 0)
        {
            ...
            if (IsChanneledSpell(m_Spell->m_spellInfo))
            {
                ...
                // casting anything else except this channeled spell and
                // autorepeat
            ...
            else
            {
                ...
                // event complete
                // finish update event will be re-added automatically at the
                // end of routine)
            }
        }
        else
                                   ...
                                   int32 &skillValue)
{
    ...
    for (int j = 0; j < 8; ++j)
    {
        ...
        case LOCK_KEY_SKILL: {
            ...
            bool oldCalc = true;
            if (tempSkillId == SKILL_NONE) // these must not be carried over to
                                           // the reference variable - unless
                                           // more research comes up
            {
            ...
            if (tempSkillId != SKILL_NONE)
            {
                ...
                if (oldCalc) // still correct but not usable for spell skill
                             // ids
                {
                    // skill bonus provided by casting spell (mostly item
                    // spells) add the damage modifier from the spell casted
                    // (cheat lock / skeleton key etc.) (use
                    // m_currentBasePoints, CalculateDamage returns wrong
                    // value)
                    uint32 spellSkillBonus = uint32(m_currentBasePoints[effIndex]);
                    reqSkillValue = lockInfo->Skill[j];

                    // castitem check: rogue using skeleton keys. the skill
                    // values should not be added in this case.
                    skillValue = m_CastItem || !m_trueCaster->IsPlayer()
                                     ? 0
                                     : static_cast<Player *>(m_trueCaster)->GetSkillValue(tempSkillId);
 ...
 * @param originalCaster       If provided set alternative original caster, if
 =nullptr then used Spell::GetAffectiveObject() return
 */
...
{
    if (m_trueCaster->IsGameObject()) // 4 spells in all of wotlk and doesnt
                                      // seem like GO casting supports
                                      // travelling delay from sniffs
        return 0.f;
    if (IsChanneledSpell(m_spellInfo))
        return 0.f;
    ...
    for (auto targetTrigger : targetTriggers)
    {
        ...
        for (TargetList::const_iterator ihit = m_UniqueTargetInfo.begin(); ihit != m_UniqueTargetInfo.end(); ++ihit)
        {
            if (ihit->missCondition == SPELL_MISS_NONE)
            {
                ...
                // Calculate chance at that moment (can be depend for example
                // from combo points)
                int32 chance =
                    m_caster->CalculateSpellEffectValue(target, auraSpellInfo, auraSpellIdx, &auraBasePoints);
                if (roll_chance_i(chance))
                {
                    Spell *spell = new Spell(m_caster, sSpellTemplate.LookupEntry<SpellEntry>(procid),
                                             TRIGGERED_OLD_TRIGGERED, ObjectGuid(), nullptr);
                ...
                if (iter->second.state != PLAYERSPELL_REMOVED)
                {
                    ...
                    for (int i = 0; i < MAX_EFFECT_INDEX; ++i)
                    {
                        if (spellInfo->EffectApplyAuraName[i] == SPELL_AURA_MOD_FLIGHT_SPEED_MOUNTED)
                        {
                            ...
                            if (mountSpeed > 280)
                            {
                                target->CastSpell(target, spellIdSpecial, TRIGGERED_NONE, nullptr, nullptr,
...
void Spell::GetSpellRangeAndRadius(SpellEffectIndex effIndex, float &radius, bool targetB, uint32 &EffectChainTarget)
{
    ...
    else if (radius == 0.f && data.type == TARGET_TYPE_LOCATION_DEST)
    {
        ...
        case TARGET_LOCATION_CASTER_LEFT:
        case TARGET_LOCATION_CASTER_RIGHT:
            if (radius == 0.f && m_spellInfo->EffectRadiusIndex[effIndex] != 36) // All shaman totems have 0 radius -
                                                                                 // need to override with proper value
                radius = 2.f;
            break;
        }
    }
...
void Spell::ProcReflectProcs(TargetInfo &targetInfo)
{
    ...
    Unit *target = m_caster->GetObjectGuid() == targetInfo.targetGUID
                       ? m_caster
                       : ObjectAccessor::GetUnit(*m_trueCaster, targetInfo.targetGUID);
    if (!target)
        return;

    // Victim reflects, apply reflect procs
    Unit::ProcDamageAndSpell(ProcSystemArguments(m_caster, target, PROC_FLAG_NONE, PROC_FLAG_TAKE_HARMFUL_SPELL,
                                                 PROC_EX_REFLECT, 1, BASE_ATTACK, m_spellInfo));
    if (targetInfo.reflectResult == SPELL_MISS_REFLECT)
        // Apply reflect procs on self
        Unit::ProcDamageAndSpell(ProcSystemArguments(m_caster, m_caster, PROC_FLAG_NONE, PROC_FLAG_TAKE_HARMFUL_SPELL,
                                                     PROC_EX_REFLECT, 1, BASE_ATTACK, m_spellInfo));
}
...
void Spell::FilterTargetMap(UnitList &filterUnitList, SpellTargetFilterScheme scheme, uint32 chainTargetCount)
{
    ...
    case SCHEME_LOWEST_HP_CHAIN: {
        ...
        while (chainTargetCount && next != filterUnitList.end())
        {
            // since we do not iterate through closest but through HP, we must
            // check all units
            if (prev->GetDistance(*next, true, DIST_CALC_NONE) > m_jumpRadius * m_jumpRadius)
            {
                ++next;
...
void Spell::FillFromTargetFlags(TempTargetingData &targetingData, SpellEffectIndex effIdx)
{
    ...
    else if (m_spellInfo->Targets & TARGET_FLAG_DEST_LOCATION)
    {
        if ((m_targets.m_targetMask & TARGET_FLAG_DEST_LOCATION) == 0)
        {
            sLog.outError("No destination for spell with flag "
                          "TARGET_FLAG_DEST_LOCATION, spell ID: %u",
...
SpellCastResult Spell::OnCheckCast(bool strict)
{
    ...
    case 31958: // Fire Bomb Halaa - Must be Taxi Flying
        ...
        break;
    case 35244: // Choking Vines - should not go through on target with Choking
                // Wound
    {
        Unit *target = m_targets.getUnitTarget();
    ...
    case 36867: // Creature - Summon Event Ethereal
        if (m_caster->GetMap()->SpawnedCountForEntry(21445) >= 1 ||
            m_caster->GetMap()->SpawnedCountForEntry(22285) >= 1)
            return SPELL_FAILED_DONT_REPORT;
        break;
    ...
    case 40157: // Spirit Lance - Teron Gorefiend - can only be used on spirits
        if (ObjectGuid target = m_targets.getUnitTargetGuid())
            if (target.GetEntry() != 23111)
                return SPELL_FAILED_BAD_TARGETS;
        break;
    case 40742: // Booterang -  Must have aura Defiant And Enraged or Lazy and
                // Good for Nothing
    {
        Unit *target = m_targets.getUnitTarget();
...
bool Spell::OnCheckTarget(Unit *target, SpellEffectIndex eff) const
{
    ...
    case 9082: // Create Containment Coffer
        if (!target->HasAura(9032))
            return false;
        break;
    case 14297: // Shadow Storm (exclude targets below 25 yards and above 40
                // yards. Max value is handled by spell effect radius)
    case 26546:
    case 26555: {
        float x, y, z;
        m_caster->GetPosition(x, y, z);
    ...
    case 16807: // Mass Teleport
        if (target->HasFlag(UNIT_FIELD_FLAGS,
                            UNIT_FLAG_PLAYER_CONTROLLED)) // spell targets only creatures
            return false;
        break;
        // Exceptions for Nefarian class calls
        // as it is not possible to filter out targets based on their class from
        // data in spells template (like spell family masks for example)
    case 23397: // Berserk
    ...
    case 30835: // Infernal Relay - Malchezaar - must only hit unused infernals
    {
        ...
        MaNGOS::NearestCreatureEntryWithLiveStateInObjectRangeCheck creature_check(*target, 17646, true, false, 2.0f,
                                                                                   true);
        MaNGOS::CreatureLastSearcher<MaNGOS::NearestCreatureEntryWithLiveStateInObjectRangeCheck> searcher(
            creature, creature_check);
        Cell::VisitGridObjects(target, searcher, 2.0f);
        if (creature)
            if (target->GetPositionX() == creature->GetPositionX() &&
                target->GetPositionY() == creature->GetPositionY())
                return false;
        break;
    }
    ...
    case 37153:
        if (target->HasAura(39400))
            return false;
        break;
    case 37433: // Spout (The Lurker Below), only players affected if its not
                // in water
        if (target->IsInWater() &&
    ...
    case 37893:
    case 37895:
        if (target->HasAura(m_spellInfo->Id)) // scorp already possessed cannot
                                              // be repossessed by a diff player
    ...
    {
        if (target->HasAura(37917) || m_caster == target) // Can't target targets with Arcano-Cloak or
                                                          // self (no attribute)
            return false;
        break;
    }
    case 38782:                                               // Druid Signal
        if (target->HasAura(38776) || target->HasAura(39158)) // Don't target druids that have already
                                                              // been summoned by another player
            return false;
        break;
    ...
    {
        float dist = sqrt(target->GetDistance(m_caster->GetPositionX(), m_caster->GetPositionY(),
                                              m_caster->GetPositionZ(), DIST_CALC_NONE));
    ...
    case 40834: // Agonizing Flames
        if (m_caster->CanReachWithMeleeAttack(target))
            return false;
        break;
    case 40869:                     // Fatal Attraction - Mother Shahraz - ignore tanks - those
                                    // which have Saber Lash
...
SpellModRAII::SpellModRAII(Spell *spell, Player *modOwner, bool success, bool onlySave)
    : m_spell(spell), m_modOwner(modOwner), m_success(success), m_onlySave(onlySave)
{
