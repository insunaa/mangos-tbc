...
struct boss_brutallusAI : public CombatAI, private DialogueHelper
{
    boss_brutallusAI(Creature *creature)
        : CombatAI(creature, BRUTALLUS_ACTION_MAX),
    ...
    void GetAIInformation(ChatHandler &reader) override
    {
        if (m_instance)
        {
            if (m_instance->GetData(TYPE_BRUTALLUS) == SPECIAL)
                ...
                reader.PSendSysMessage("Brutallus intro event is currently %s",
                                       m_instance->GetData(TYPE_BRUTALLUS) == NOT_STARTED ? "not started"
                                                                                          : "completed");
            ...
            if (m_instance->GetData(TYPE_BRUTALLUS) != NOT_STARTED)
            {
                if (Creature *madrigosa = m_instance->GetSingleCreatureFromStorage(NPC_MADRIGOSA, true))
                    reader.PSendSysMessage("Madrigosa guid is %s and has %u health.", madrigosa->GetGuidStr().c_str(),
                                           madrigosa->GetHealth());
            }
        }
    }
    ...
    void SummonedCreatureJustDied(Creature *summoned) override
    {
        ...
        if (summoned->GetEntry() == NPC_MADRIGOSA)
            script_error_log("Npc %u, %s, died unexpectedly. Felmyst won't be summoned anymore.", summoned->GetEntry(),
                             summoned->GetName());
    }
    ...
    void JustDidDialogueStep(int32 iEntry) override
    {
        ...
        {
        case NPC_MADRIGOSA:
            m_creature->SummonCreature(NPC_MADRIGOSA, 1470.362f, 738.1818f, 64.16677f, 4.625123f,
        ...
        case POINT_MOVE_ICE_BLOCK:
            ...
            if (Creature *madrigosa = m_instance->GetSingleCreatureFromStorage(NPC_MADRIGOSA))
            {
                ...
                madrigosa->GetMotionMaster()->MovePoint(POINT_MOVE_ICE_BLOCK, aMadrigosaLoc[1].x, aMadrigosaLoc[1].y,
                                                        aMadrigosaLoc[1].z);
            }
            // Temporary! This will make Brutallus not follow Madrigosa through the
            // air until mmaps are implemented
        ...
        case YELL_MADR_TRAP:
            ...
            if (Creature *madrigosa = m_instance->GetSingleCreatureFromStorage(NPC_MADRIGOSA))
            {
                ...
                // Need to remove the fire aura after 4 sec so Madrigosa won't die
                // so soon
        ...
        case POINT_MOVE_ENCAPSULATE:
            ...
            m_creature->GetMotionMaster()->MovePoint(
                POINT_MOVE_ENCAPSULATE, Position(1463.13f, 578.7533f, 44.51308f, 0.f), FORCED_MOVEMENT_RUN);
            break;
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case BRUTALLUS_BURN:
            ...
            if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_BURN_DAMAGE,
...
struct SpellBurnBrutallus : public SpellScript
{
    bool OnCheckTarget(const Spell * /*spell*/, Unit *target, SpellEffectIndex eff) const override
    {
        if (target->HasAura(SPELL_BURN_DAMAGE) ||
            target->IsImmuneToSchool(sSpellTemplate.LookupEntry<SpellEntry>(SPELL_BURN_DAMAGE), (1 << eff)))
            return false;
        return true;
    }
        ...
        if (aura->GetEffIndex() == EFFECT_INDEX_0 && !apply)
        {
            if (ScriptedInstance *instance = static_cast<ScriptedInstance *>(aura->GetTarget()->GetInstanceData()))
            {
                if (Creature *madrigosa = instance->GetSingleCreatureFromStorage(NPC_MADRIGOSA))
                {
                    ...
                    madrigosa->SetRespawnCoord(madrigosa->GetPositionX(), madrigosa->GetPositionY(),
                                               madrigosa->GetPositionZ(), madrigosa->GetOrientation());
