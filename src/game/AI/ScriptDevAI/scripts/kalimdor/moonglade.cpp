...
struct npc_clintar_dw_spiritAI : public npc_escortAI
{
    ...
    {
        // not the best way, maybe check in DummyEffect if this creature are "free"
        // and not in escort.
        if (HasEscortState(STATE_ESCORT_ESCORTING))
            return;
...
enum
{
    ...
    NPC_REMULOS = 11832,
    NPC_TYRANDE_WHISPERWIND = 15633, // appears with the priestess during the event to help the players
                                     // - should cast healing spells
    ...
    MAX_DEFENDERS = 10,    // The max number of Moonglade Defender summoned to fight
                           // the Nightmare Phantasm
...
// Spawn points of the 10 Moonglade Defenders that are periodically summoned to
// fight the Nightmare Phantasms
static EventLocations defendersLocations[] = {{7868.226f, -2556.95f, 487.07f}, {7867.39f, -2578.96f, 486.95f}};
...
struct npc_keeper_remulosAI : public npc_escortAI, private DialogueHelper
{
    ...
    void JustDied(Unit *killer) override
    {
        ...
        // Remulos is only targetable for friendly player spells during Eranikus
        // event so reset on death
    ...
    void WaypointReached(uint32 pointId) override
    {
        ...
        case 1:
            if (Player *player = GetPlayerForEscort())
            {
                ...
                // Remulos is only targetable for friendly player spells during
                // Eranikus event
    ...
    void JustDidDialogueStep(int32 entry) override
    {
        ...
        case NPC_ERANIKUS_TYRANT:
            ...
            if (Creature *eranikus = m_creature->GetMap()->GetCreature(m_eranikusGuid))
            {
                ...
                eranikus->GetMotionMaster()->MovePoint(POINT_ID_ERANIKUS_FLIGHT, eranikusLocations[1].m_fX,
                                                       eranikusLocations[1].m_fY, eranikusLocations[1].m_fZ,
    ...
    void DoHandleOutro(Creature *target)
    {
        ...
        DoDespawnSummoned();
        // Remulos is only targetable for friendly player spells during Eranikus
        // event: remove flag on quest completion
        m_creature->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP);
    ...
    void UpdateEscortAI(const uint32 diff) override
    {
        ...
        if (m_outroTimer)
        {
            if (m_outroTimer <= diff)
            {
                ...
                case 1:
                    ...
                    // respawn automatically at his home position after a few min
            ...
            if (m_shadesummonTimer <= diff)
            {
                ...
                if (m_isFirstWave)
                {
                    ...
                    for (uint8 i = 0; i < MAX_SHADOWS; ++i)
                        m_creature->SummonCreature(NPC_NIGHTMARE_PHANTASM, shadowsLocations[i].m_fX,
                                                   shadowsLocations[i].m_fY, shadowsLocations[i].m_fZ, 0,
                    ...
                    for (uint8 i = 0; i < MAX_DEFENDERS; ++i)
                    {
                        ...
                        uint8 index = i % 2;
                        float fX, fY, fZ;
                        m_creature->GetRandomPoint(defendersLocations[index].m_fX, defendersLocations[index].m_fY,
                                                   defendersLocations[index].m_fZ, 3.0f, fX, fY, fZ);
                        m_creature->SummonCreature(NPC_NIGHTHAVEN_DEFENDER, fX, fY, fZ, 0,
                                                   TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 3 * MINUTE * IN_MILLISECONDS);
                    }
                ...
                if (m_summonCount < MAX_SUMMON_TURNS)
                {
                    for (uint8 i = 0; i < MAX_SHADOWS; ++i)
                    {
                        m_creature->GetRandomPoint(shadowsLocations[summonPoint].m_fX,
                                                   shadowsLocations[summonPoint].m_fY,
                                                   shadowsLocations[summonPoint].m_fZ, 10.0f, fX, fY, fZ);
                        m_creature->SummonCreature(NPC_NIGHTMARE_PHANTASM, fX, fY, fZ, 0.0f,
                                                   TEMPSPAWN_TIMED_OOC_OR_DEAD_DESPAWN, 3 * MINUTE * IN_MILLISECONDS);
                    }

                    // Summon Moonglade Defender to patrol the area (and fight) if
                    // any are dead
                ...
                // We don't count the dead shades, because the boss is usually set
                // in combat before all shades are dead
                if (m_summonCount == MAX_SUMMON_TURNS)
                {
                    m_shadesummonTimer = 0;
                    ...
                    if (Creature *eranikus = m_creature->GetMap()->GetCreature(m_eranikusGuid))
                        ...
                        eranikus->GetMotionMaster()->MovePoint(POINT_ID_ERANIKUS_COMBAT, eranikusLocations[2].m_fX,
                                                               eranikusLocations[2].m_fY, eranikusLocations[2].m_fZ);
                }
                else
        ...
        if (m_healTimer < diff)
        {
            if (Unit *target = DoSelectLowestHpFriendly(DEFAULT_VISIBILITY_DISTANCE))
            {
                ...
                {
                case 0:
                    DoCastSpellIfCan(target, SPELL_HEALING_TOUCH);
                    break;
                case 1:
...
struct boss_eranikusAI : public ScriptedAI
{
    ...
    void EnterEvadeMode() override
    {
        ...
        {
            // There may be a core issue related to the reached home function for
            // summoned creatures so we are cleaning things up here if the creature
            // evades while the event is in progress then we despawn all the
            // summoned, including himself
    ...
    void DoSummonHealers()
    {
        ...
        {
            m_creature->GetRandomPoint(tyrandeLocations[0].m_fX, tyrandeLocations[0].m_fY, tyrandeLocations[0].m_fZ,
                                       10.0f, fX, fY, fZ);
    ...
    void JustSummoned(Creature *summoned) override
    {
        ...
        case NPC_TYRANDE_WHISPERWIND:
            ...
            summoned->SetWalk(false);
            summoned->GetMotionMaster()->MovePoint(POINT_ID_TYRANDE_HEAL, tyrandeLocations[1].m_fX,
                                                   tyrandeLocations[1].m_fY, tyrandeLocations[1].m_fZ);
            break;
        case NPC_ELUNE_PRIESTESS:
            m_priestessList.push_back(summoned->GetObjectGuid());
            float fX, fY, fZ;
            summoned->SetWalk(false);
            m_creature->GetRandomPoint(tyrandeLocations[1].m_fX, tyrandeLocations[1].m_fY, tyrandeLocations[1].m_fZ,
                                       10.0f, fX, fY, fZ);
    ...
    void SummonedMovementInform(Creature *summoned, uint32 type, uint32 pointId) override
    {
        ...
        case POINT_ID_TYRANDE_HEAL:
            ...
            {
                summoned->Unmount();
                summoned->AI()->AttackStart(m_creature); // Prietess of Elune do not really attack Eranikus
                                                         // but they need to be in combat to periodically
                                                         // cast Mass Heal
            }
            break;
    ...
    void UpdateAI(const uint32 diff) override
    {
        if (m_eventTimer)
        {
            if (m_eventTimer <= diff)
            {
                ...
                case 0:
                    ...
                    // DoCastSpellIfCan(m_creature, SPELL_MOONGLADE_TRANQUILITY);
                    // // spell id unk for the moment
                ...
                case 3:
                    ...
                    m_creature->GetMotionMaster()->MovePoint(POINT_ID_ERANIKUS_REDEEMED, eranikusLocations[3].m_fX,
                                                             eranikusLocations[3].m_fY, eranikusLocations[3].m_fZ);
                    m_eventTimer = 0;
                    break;
                case 4:
        ...
        if (m_tyrandeMoveTimer)
        {
            ...
            {
                if (Creature *tyrande = m_creature->GetMap()->GetCreature(m_tyrandeGuid))
                    tyrande->GetMotionMaster()->MovePoint(POINT_ID_TYRANDE_ABSOLUTION, tyrandeLocations[2].m_fX,
                                                          tyrandeLocations[2].m_fY, tyrandeLocations[2].m_fZ);
                m_tyrandeMoveTimer = 0;
            }
            else
        ...
        if (m_creature->GetHealthPercent() < m_healthCheck)
        {
            ...
            case 85:
                ...
                if (Creature *tyrande = m_creature->SummonCreature(NPC_TYRANDE_WHISPERWIND, tyrandeLocations[0].m_fX,
                                                                   tyrandeLocations[0].m_fY, tyrandeLocations[0].m_fZ,
                                                                   0, TEMPSPAWN_TIMED_DESPAWN, 1000))
...
struct ConjureDreamRift : public SpellScript
{
    void OnEffectExecute(Spell *spell, SpellEffectIndex effIdx) const override
    {
        if (effIdx == EFFECT_INDEX_0)
        {
            ...
            if (Creature *eranikus = spell->GetCaster()->SummonCreature(
                    NPC_ERANIKUS_TYRANT, eranikusLocations[0].m_fX, eranikusLocations[0].m_fY,
                    eranikusLocations[0].m_fZ, eranikusLocations[0].m_fO, TEMPSPAWN_DEAD_DESPAWN, 0))
            {
                eranikus->CastSpell(nullptr, SPELL_DRAGON_HOVER, TRIGGERED_OLD_TRIGGERED);
                eranikus->SetHover(true);
                eranikus->NearTeleportTo(eranikusLocations[0].m_fX, eranikusLocations[0].m_fY,
                                         eranikusLocations[0].m_fZ,
                                         eranikusLocations[0].m_fO); // Teleport back Eranikus to its spawn position in
                                                                     // case he fell to the ground/lake surface before
                                                                     // hover/fly was applied (probably only client
                                                                     // side)
            }
        }
    }
...
{
    // The Cluster Launcher gameobject used to summon Omen is the same as the one
    // that can be summoned from the Engineering-crafted launcher So we need to
    // make sure we are called from the statically spawned ones only
    if (pGo->GetSpawner())
        return false;
