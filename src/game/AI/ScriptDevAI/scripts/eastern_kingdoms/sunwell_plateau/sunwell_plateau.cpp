...
instance_sunwell_plateau::instance_sunwell_plateau(Map *pMap)
    : ScriptedInstance(pMap), DialogueHelper(aFelmystOutroDialogue), m_firstEnter(true), m_uiDeceiversKilled(0),
      m_uiSpectralRealmTimer(5000), m_uiMuruBerserkTimer(0), m_uiKiljaedenYellTimer(90000), m_impsStarted(false),
      m_miniAttackEvent(false), m_brutallusIntroStarted(false)
{
    Initialize();
}
...
{
    if (creature->GetDbGuid() >= GUID_PREFIX + 300 && creature->GetDbGuid() <= GUID_PREFIX + 322 &&
        m_gauntletRespawnTime.time_since_epoch().count() == 0)
        StartImps();
...
void instance_sunwell_plateau::OnCreatureDeath(Creature *creature)
{
    if (creature->GetEntry() == NPC_HAND_OF_THE_DECEIVER)
    {
        ...
        if (m_uiDeceiversKilled == MAX_DECEIVERS)
        {
            ...
            {
                if (Creature *kiljaeden = controller->SummonCreature(NPC_KILJAEDEN, 1699.5737f, 627.8289f, 28.400253f,
                                                                     4.049163818359375f, TEMPSPAWN_DEAD_DESPAWN, 0))
                {
                    kiljaeden->SummonCreature(NPC_KALECGOS, aKalegSpawnLoc[0], aKalegSpawnLoc[1], aKalegSpawnLoc[2],
...
void instance_sunwell_plateau::SetData(uint32 type, uint32 data)
{
    ...
    case TYPE_FELMYST:
        ...
        if (data == IN_PROGRESS && GetData(TYPE_BRUTALLUS) != DONE)
            BanPlayersIfNoGm("Player engaged felmyst without killing brutallus and "
                             "Gamemaster being present in instance.");
        if (data == DONE)
    ...
    case TYPE_EREDAR_TWINS:
        ...
        if (data == IN_PROGRESS && GetData(TYPE_FELMYST) != DONE)
            BanPlayersIfNoGm("Player engaged eredar twins without killing felmyst "
                             "and Gamemaster being present in instance.");
        if (data == SPECIAL)
        ...
        if (data == DONE)
        {
            ...
            static_cast<DungeonMap *>(instance)->GetPersistanceState()->UpdateEncounterState(ENCOUNTER_CREDIT_SCRIPT,
                                                                                             DB_ENCOUNTER_TWINS);
        }
        if (data == FAIL)
        {
    ...
    case TYPE_MURU:
        ...
        if (data == IN_PROGRESS && GetData(TYPE_EREDAR_TWINS) != DONE)
            BanPlayersIfNoGm("Player engaged muru without killing eredar twins and "
                             "Gamemaster being present in instance.");
        // combat door
        if (data != IN_PROGRESS) // closing done delayed on start
    ...
    case TYPE_KILJAEDEN:
        ...
        if (data == IN_PROGRESS && GetData(TYPE_MURU) != DONE)
            BanPlayersIfNoGm("Player engaged KJ without killing muru and Gamemaster "
                             "being present in instance.");
        if (data == FAIL)
            FailKiljaeden();
        break;
    }
...
void instance_sunwell_plateau::Update(uint32 diff)
{
    ...
    // Muru berserk timer; needs to be done here because it involves two distinct
    // creatures
    ...
    if (m_impsStarted)
    {
        ...
        for (const auto &playerRef : players)
        {
            ...
            if (player->IsAlive() && player->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PLAYER_CONTROLLED) &&
                !player->IsFeigningDeathSuccessfully() &&
                !player->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE_2) && !player->IsGameMaster())
            {
                found = true;
                break;
            }
...
void instance_sunwell_plateau::DoSortFlightTriggers()
{
    ...
    {
        script_error_log("Instance Sunwell Plateau: ERROR Failed to load flight "
                         "triggers for creature id %u.",
                         NPC_FELMYST);
        return;
    }
    ...
    // sort the flight triggers; first by position X, then group them by Y (left
    // and right)
...
{
    WorldObject::SummonCreature(TempSpawnSettings(nullptr, NPC_FELMYST, aMadrigosaLoc[0].x, aMadrigosaLoc[0].y,
                                                  aMadrigosaLoc[0].z, aMadrigosaLoc[0].o, TEMPSPAWN_MANUAL_DESPAWN, 0,
                                                  true),
                                instance);
}
...
void instance_sunwell_plateau::JustDidDialogueStep(int32 iEntry)
{
    ...
    case NPC_KALECGOS_MADRIGOSA:
        ...
        {
            if (Creature *kalec = trigger->SummonCreature(NPC_KALECGOS_MADRIGOSA, aKalecLoc[0].x, aKalecLoc[0].y,
                                                          aKalecLoc[0].z, aKalecLoc[0].o, TEMPSPAWN_CORPSE_DESPAWN, 0))
            {
                kalec->SetWalk(false);
...
void instance_sunwell_plateau::ShowChatCommands(ChatHandler *handler)
{
    handler->SendSysMessage("This instance supports the following commands:\n skipbrutallusintro, "
                            "spawnfelmyst, spawngauntlet, despawngauntlet, spawntwins, spawnmuru");
}
