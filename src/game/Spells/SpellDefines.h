...
enum SpellAttributes
{
    ...
    SPELL_ATTR_TRADESPELL = 0x00000020,                   // 5 trade spells, will be added by client
                                                          // to a sublist of profession spell
    ...
    SPELL_ATTR_TARGET_MAINHAND_ITEM = 0x00000200,         // 9 Client automatically selects item from mainhand slot as
                                                          // a cast target TODO: Implement
    ...
    SPELL_ATTR_DONT_AFFECT_SHEATH_STATE = 0x00040000,     // 18 client won't hide unit weapons in sheath on
                                                          // cast/channel
                                                          // TODO: Implement
    SPELL_ATTR_LEVEL_DAMAGE_CALCULATION = 0x00080000,     // 19 spelldamage depends on caster level
    SPELL_ATTR_STOP_ATTACK_TARGET = 0x00100000,           // 20 Stop attack after use this spell (and not begin attack
                                                          // if use)
    ...
    SPELL_ATTR_DISABLED_WHILE_ACTIVE = 0x02000000,        // 25 Activate and start cooldown after aura fade or remove
    ...
    SPELL_ATTR_HEARTBEAT_RESIST_CHECK = 0x40000000,       // 30 Chance for spell effects to break early (heartbeat
                                                          // resist)
...
enum SpellAttributesEx
{
    ...
    SPELL_ATTR_EX_UNAUTOCASTABLE_BY_CHARMED = 0x00020000,       // 17 TODO: Investigate more Chero version:
                                                          // SPELL_ATTR_EX_PLAYER_CANT_CAST_CHARMED, likely related to
                                                          // MC
    SPELL_ATTR_EX_PREVENTS_ANIM = 0x00040000,              // 18
    SPELL_ATTR_EX_CANT_TARGET_SELF = 0x00080000,           // 19 spells with area effect or friendly targets that
                                                           // exclude the caster
    ...
    SPELL_ATTR_EX_REQ_COMBO_POINTS = 0x00400000,           // 22 Use combo points (in 4.x not required combo point
                                                           // target selected)
...
enum SpellAttributesEx2
{
    ...
    SPELL_ATTR_EX2_DISPLAY_IN_STANCE_BAR = 0x00000010,   // 4 client displays icon in stance bar when learned, even if
                                                         // not shapeshift
    ...
    SPELL_ATTR_EX2_UNK13 = 0x00002000,                   // 13 TODO: Implement from TC
                                                         // SPELL_ATTR_EX2_CASTABLE_ON_ITEMS
    ...
    SPELL_ATTR_EX2_NOT_RESET_AUTO_ACTIONS = 0x00020000,  // 17 suspend weapon timer instead of resetting it, (?Hunters
                                                         // Shot and Stings only have this flag?)
    SPELL_ATTR_EX2_REQ_DEAD_PET = 0x00040000,            // 18 Only Revive pet - possible req dead pet
    SPELL_ATTR_EX2_NOT_NEED_SHAPESHIFT = 0x00080000,     // 19 does not necessary need shapeshift (pre-3.x not have
                                                         // passive spells with this attribute)
    SPELL_ATTR_EX2_FACING_TARGETS_BACK = 0x00100000,     // 20 TODO: CONFIRM!
    SPELL_ATTR_EX2_DAMAGE_REDUCED_SHIELD = 0x00200000,   // 21 for ice blocks, pala immunity buffs, priest absorb
...
{
    SPELL_ATTR_EX3_OUT_OF_COMBAT_ATTACK = 0x00000001,      // 0 Spell landed counts as hostile action against enemy even
                                                           // if it doesn't trigger combat state, propagates PvP flags
    ...
    SPELL_ATTR_EX3_RANGED_ATTACK = 0x00008000,            // 15 Spells with this attribute are processed as ranged
                                                          // attacks in client
    ...
    SPELL_ATTR_EX3_CAN_PROC_FROM_TRIGGERED = 0x04000000,  // 26 Auras with this attribute can proc off
...
enum SpellAttributesEx4
{
    ...
    SPELL_ATTR_EX4_NOT_STEALABLE = 0x00000040,          // 6 although such auras might be dispellable, they cannot be
                                                        // stolen
    SPELL_ATTR_EX4_CAN_CAST_WHILE_CASTING = 0x00000080, // 7 In theory, can use this spell while another is
                                                        // channeled/cast/autocast
    ...
    SPELL_ATTR_EX4_TRIGGER_ACTIVATE = 0x00000200,              // 9 initially disabled / trigger activate from event
                                                               // (Execute, Riposte, Deep Freeze end other)
    ...
    SPELL_ATTR_EX4_UNK29 = 0x20000000,                         // 29 Related to client check about dispel,
                                                               // CC seems to have them - dispel effect 0
...
enum SpellAttributesEx5
{
    ...
    SPELL_ATTR_EX5_DONT_SHOW_AURA_IF_SELF_CAST = 0x08000000, // 27 Auras with this attribute are not visible on units
                                                             // that are the caster
    SPELL_ATTR_EX5_DONT_SHOW_AURA_IF_NOT_SELF_CAST = 0x10000000, // 28 Auras with this attribute are not visible on
                                                                 // units that are not the caster
...
enum SpellAttributesEx6
{
    ...
    SPELL_ATTR_EX6_USE_SPELL_CAST_EVENT = 0x00000040,      // 6 Auras with this attribute trigger SPELL_CAST combat log
                                                           // event instead of SPELL_AURA_START (clientside attribute)
    ...
    SPELL_ATTR_EX6_NO_DMG_PERCENT_MODS = 0x20000000,       // 29 do not apply damage mods (usually in cases where it has
...
enum SpellCastResult
{
    ...
    SPELL_RESULT_DONT_SEND_TO_CLIENT = 0xFD, // Always has to be first - TODO: is
                                             // this still needed? not in public
...
enum SpellInterruptFlags
{
    ...
    SPELL_INTERRUPT_FLAG_STUN = 0x04,            // not implemented until more research -
                                                 // many creature spells miss it
...
enum SpellAuraInterruptFlags // also used for ChannelInterruptFlags
{
    ...
    AURA_INTERRUPT_FLAG_STANDING_CANCELS = 0x00040000, // 18   removed by standing up (used by food and drink mostly
...
enum SpellModOp
{
    ...
    SPELLMOD_CHANCE_OF_SUCCESS = 18, // Only used with SPELL_AURA_ADD_FLAT_MODIFIER and affects proc
                                     // spells
...
// Spell triggering settings for CastSpell that enable us to skip some checks
// so that we can investigate spell specific settings
enum TriggerCastFlags : uint32
{
    TRIGGERED_NONE = 0x00000000,                        // Not Triggered
    TRIGGERED_OLD_TRIGGERED = 0x00000001,               // Legacy bool support TODO: Restrict
    ...
    TRIGGERED_AUTOREPEAT = 0x00000010,                  // Will signal spell system that this is
                                                        // internal autorepeat call
    TRIGGERED_IGNORE_UNATTACKABLE_FLAG = 0x00000020,    // Ignores UNIT_FLAG_NOT_ATTACKABLE in CheckTarget
    TRIGGERED_DO_NOT_PROC = 0x00000040,                 // Spells from scripts should not proc -
                                                        // DBScripts for example
    TRIGGERED_PET_CAST = 0x00000080,                    // Spell that should report error through pet opcode
    TRIGGERED_NORMAL_COMBAT_CAST = 0x00000100,          // AI needs to be notified about change of target TODO:
                                                        // change into TRIGGERED_NONE
    ...
    TRIGGERED_IGNORE_CURRENT_CASTED_SPELL = 0x00001000, // Ignores concurrent casts and is not set as currently
                                                        // executed
    TRIGGERED_HIDE_CAST_IN_COMBAT_LOG = 0x00002000,     // Sends cast flag for ignoring combat log display - used for
                                                        // many procs - default behaviour for triggered by aura
    TRIGGERED_DO_NOT_RESET_LEASH = 0x00004000,          // Does not reset leash on cast
    TRIGGERED_CHANNEL_ONLY = 0x00008000,                // Only starts channel and no effects -
                                                        // used for summoning portal GO anims
