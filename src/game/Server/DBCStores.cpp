...
// sItemCondExtCostsStore(ItemCondExtCostsEntryfmt); DBCStorage
// <ItemDisplayInfoEntry> sItemDisplayInfoStore(ItemDisplayTemplateEntryfmt);
// -- not used currently
...
// DBC store data but sTaxiPathNodesByPath used for fast access to entries
// (it's not owner pointed data).
...
{
    sLog.outError("Size of '%s' setted by format string (%u) not equal size of "
                  "C++ structure (%u).",
...
void LoadDBCStores(const std::string &dataPath)
{
    ...
    for (uint32 i = 0; i < sCharSectionsStore.GetNumRows(); ++i)
        if (CharSectionsEntry const *entry = sCharSectionsStore.LookupEntry(i))
            if (entry->Race && ((1 << (entry->Race - 1)) & RACEMASK_ALL_PLAYABLE) != 0) // ignore Nonplayable races
                sCharSectionMap.emplace(
                    uint8(entry->BaseSection) | (uint8(entry->Gender) << 8) | (uint8(entry->Race) << 16), entry);
    ...
    for (uint32 j = 0; j < sSkillLineAbilityStore.GetNumRows(); ++j)
    {
        ...
        if (spellInfo &&
            (spellInfo->Attributes &
             (SPELL_ATTR_ABILITY | SPELL_ATTR_PASSIVE | SPELL_ATTR_DO_NOT_DISPLAY | SPELL_ATTR_HIDE_IN_COMBAT_LOG)) ==
                (SPELL_ATTR_ABILITY | SPELL_ATTR_PASSIVE | SPELL_ATTR_DO_NOT_DISPLAY | SPELL_ATTR_HIDE_IN_COMBAT_LOG))
        {
    ...
    //     sSpellItemEnchantmentStore.LookupEntry(i); if (!enchantEntry)
    //         continue;
    //     for (uint32 k = 0; k < 3; ++k)
    //     {
    //         if (enchantEntry->spellid[k])
    //             if
    //             (!sSpellTemplate.LookupEntry<SpellEntry>(enchantEntry->spellid[k]))
    //                 sLog.outErrorDb("Spell ID %u found in spell item enchant
    //                 %u does not exist.", enchantEntry->spellid[k], i);
    //     }
    // }
    ...
    // prepare fast data access to bit pos of talent ranks for use at inspecting
    {
        ...
        for (uint32 i = 1; i < sTalentStore.GetNumRows(); ++i)
        {
            ...
            sTalentBitSize[(talentInfo->Row << 24) + (talentInfo->Col << 16) + talentInfo->TalentID] =
                curtalent_maxrank;
        ...
        for (uint32 talentTabId = 1; talentTabId < sTalentTabStore.GetNumRows(); ++talentTabId)
        {
            ...
            // add total amount bits for first rank starting from talent tab first
            // talent rank pos.
            uint32 pos = 0;
    ...
    // include existing nodes that have at least single not spell base (scripted)
    // path
    {
    ...
    // dbcPath,"WorldMapOverlay.dbc"); LoadDBC(availableDbcLocales, bar,
    // bad_dbc_files, sWorldSafeLocsStore,       dbcPath, "WorldSafeLocs.dbc");

    // error checks
    if (bad_dbc_files.size() >= DBCFilesCount)
    {
        sLog.outError("\nIncorrect DataDir value in mangosd.conf or ALL required "
    ...
    {
        sLog.outError("\nYou have _outdated_ DBC files. Please re-extract DBC "
                      "files for one from client build: %s",
...
ChatChannelsEntry const *GetChatChannelsEntryFor(const std::string &name, uint32 channel_id /* = 0*/)
{
    ...
    for (uint32 i = 0; i < sChatChannelsStore.GetNumRows(); ++i)
    {
        if (ChatChannelsEntry const *entry = sChatChannelsStore.LookupEntry(i))
        {
            ...
            // try to match by name first, avoid creating custom channels with
            // same name
            if (!wname.empty())
            {
                for (uint32 i = 0; i < MAX_LOCALE; ++i)
            ...
            // name still not found, but channel id is provided: possibly no dbc
            // data for client locale
...
// DBCStorage <FactionEntry>       const* GetFactionStore()        { return
// &sFactionStore;        }
