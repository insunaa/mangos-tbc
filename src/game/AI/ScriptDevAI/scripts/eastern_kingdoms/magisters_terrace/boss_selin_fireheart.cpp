...
enum
{
    ...
    SPELL_MANA_RAGE_CHANNEL = 44320, // This spell triggers 44321, which changes scale and regens mana
                                     // Requires an entry in spell_script_target
    // SPELL_MANA_RAGE_POWER          = 44321,               // Triggered by the
    // channel
    SPELL_DUAL_WEILD = 42459,

    // Crystal spells
    // SPELL_FEL_CRYSTAL_VISUAL       = 44355,                // cosmetic -
    // defined in instance header
...
struct boss_selin_fireheartAI : public CombatAI
{
          ...
          m_isRegularMode(creature->GetMap()->IsRegularDifficulty())
    {
        ...
        m_creature->GetCombatManager().SetLeashingCheck([](Unit *, float x, float y, float z) { return x < 214.f; });
    ...
    void MovementInform(uint32 moveType, uint32 pointId) override
    {
        ...
        script_error_log("Selin Fireheart unable to drain crystal as the crystal "
                         "is either dead or deleted..");
    }
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case SELIN_ACTION_DRAIN_LIFE: {
            if (m_creature->GetPower(POWER_MANA) >= m_felExplosionCost) // do not cast if enough mana for fel
                                                                        // explosion
                return;
    ...
    void OnApply(Aura *aura, bool apply) const override
    {
        if (!apply && aura->GetEffIndex() == EFFECT_INDEX_0)
        {
            ...
            if (UnitAI *ai = aura->GetTarget()->AI())
                ai->SendAIEvent(AI_EVENT_CUSTOM_A, caster, aura->GetTarget(),
                                aura->GetRemoveMode() == AURA_REMOVE_BY_EXPIRE);
        }
    }
};
