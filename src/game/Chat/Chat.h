...
class ChatHandler
{
  public:
     ...
     * Returns:   void
     *
     * \param WorldPacket& data             : Provided packet will be filled with
     *requested info \param ChatMsg msgtype               : Message type from
     *ChatMsg enum from SharedDefines.h \param ChatTagFlags chatTag          :
     *Chat tag from PlayerChatTag in Chat.h \param char const* message : Message
     *to send \param Language language             : Language from Language enum
     *in SharedDefines.h \param ObjectGuid const& senderGuid  : May be null in
     *some case but often required for ignore list \param char const* senderName
     *: Required for type *MONSTER* or *BNET*, but also if GM is true \param
     *ObjectGuid const& targetGuid  : Often null, but needed for type *MONSTER*
     *or *BNET* or *BATTLEGROUND* or *ACHIEVEMENT* \param char const* targetName
     *: Often null, but needed for type *MONSTER* or *BNET* or *BATTLEGROUND*
    ...
    bool ExtractUInt32(char **args, uint32 &val) const
    {
    ...
    static char *ExtractArg(char **args,
                            bool asis = false);     // any name/number/quote/shift-link strings
    static char *ExtractOptNotLastArg(char **args); // extract name/number/quote/shift-link arg
                                                    // only if more data in args for parse
  ...
  protected:
    ...
    bool SetDataForCommandInTable(ChatCommand *commandTable, const char *text, uint32 security,
