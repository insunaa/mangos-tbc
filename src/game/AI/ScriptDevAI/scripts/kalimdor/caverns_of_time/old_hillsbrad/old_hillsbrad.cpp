...
SD%Complete: 90
SDComment: Quest support: 10283, 10284. All friendly NPC's. Thrall escort event
is complete, possible a few details are still missing. SDCategory: Caverns of
Time, Old Hillsbrad Foothills EndScriptData */
...
bool GossipHello_npc_erozion(Player *pPlayer, Creature *pCreature)
{
    ...
    if (pInstance && pInstance->GetData(TYPE_EPOCH) == DONE)
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Take me back to my time.", GOSSIP_SENDER_MAIN,
...
enum
{
    ...
    TEXT_ID_SKARLOC_1 = 9578,         // Thank you friends, I owe my freedom to you.
                                      // Where is Taretha? I hoped to see her
    GOSSIP_ITEM_SKARLOC_2 = -3560003, // "The situation is rather complicated,
                                      // Thrall. It would be best for you..."
...
struct npc_thrall_old_hillsbradAI : public npc_escortAI, private DialogueHelper
{
    ...
    void JustRespawned() override
    {
        ...
        if (m_pInstance->GetData(TYPE_THRALL_EVENT) == IN_PROGRESS)
        {
            ...
            else if (m_pInstance->GetData(TYPE_ESCORT_BARN) != DONE)
            {
                ...
                // resummon the mount
                m_creature->SummonCreature(NPC_SKARLOC_MOUNT, 2047.775f, 253.4088f, 62.91183f, 5.37f,
            ...
            if (m_lTarrenMillSoldiersGuids.empty())
            {
                ...
                case 1:
                    ...
                    m_creature->SummonCreature(NPC_INFINITE_SABOTEOR, 2602.208f, 678.2955f, 56.34682f, 6.07f,
                                               TEMPSPAWN_DEAD_DESPAWN, 0);
                    m_creature->SummonCreature(NPC_INFINITE_SLAYER, 2602.8f, 686.2845f, 55.79315f, 5.95f,
                                               TEMPSPAWN_DEAD_DESPAWN, 0);
                    ++m_uiEpochWaveId;
                    break;
                case 2:
    ...
    void JustDidDialogueStep(int32 iEntry) override
    {
        ...
        case SAY_PROTECTOR_BARN_3:
            ...
            float fX, fY, fZ;
            for (GuidList::const_iterator itr = m_lTarrenMillSoldiersGuids.begin();
                 itr != m_lTarrenMillSoldiersGuids.end(); ++itr)
            {
                if (Creature *pTemp = m_creature->GetMap()->GetCreature(*itr))
                {
                    pTemp->SetWalk(false);
    ...
    void WaypointReached(uint32 uiPoint) override
    {
        ...
        case 21:
            m_creature->SummonCreature(NPC_MAGE, 2186.909f, 139.8108f, 88.21628f, 5.75f, TEMPSPAWN_TIMED_OOC_DESPAWN,
                                       5000);
            m_creature->SummonCreature(NPC_WARDEN, 2187.943f, 141.6124f, 88.21628f, 5.73f, TEMPSPAWN_TIMED_OOC_DESPAWN,
                                       5000);
            ...
            break;
        case 24:
            m_creature->SummonCreature(NPC_MAGE, 2149.463f, 104.9756f, 73.63239f, 1.71f, TEMPSPAWN_TIMED_OOC_DESPAWN,
                                       5000);
            m_creature->SummonCreature(NPC_SENTRY, 2147.642f, 105.0251f, 73.99422f, 1.52f, TEMPSPAWN_TIMED_OOC_DESPAWN,
                                       5000);
            m_creature->SummonCreature(NPC_VETERAN, 2149.212f, 107.2005f, 74.15676f, 1.71f, TEMPSPAWN_TIMED_OOC_DESPAWN,
                                       5000);
            m_creature->SummonCreature(NPC_WARDEN, 2147.328f, 106.7235f, 74.34447f, 1.69f, TEMPSPAWN_TIMED_OOC_DESPAWN,
                                       5000);
            break;
        case 27:
            m_creature->SummonCreature(NPC_MAGE, 2142.363f, 172.4260f, 66.30494f, 2.54f, TEMPSPAWN_TIMED_OOC_DESPAWN,
                                       5000);
            m_creature->SummonCreature(NPC_SENTRY, 2138.177f, 168.6046f, 66.30494f, 2.47f, TEMPSPAWN_TIMED_OOC_DESPAWN,
                                       5000);
            m_creature->SummonCreature(NPC_SENTRY, 2142.372f, 174.2907f, 66.30494f, 2.56f, TEMPSPAWN_TIMED_OOC_DESPAWN,
                                       5000);
            m_creature->SummonCreature(NPC_VETERAN, 2140.146f, 169.2364f, 66.30494f, 2.49f, TEMPSPAWN_TIMED_OOC_DESPAWN,
                                       5000);
            break;
        case 29:
            m_creature->SummonCreature(NPC_MAGE, 2107.938f, 192.0753f, 66.30494f, 2.54f, TEMPSPAWN_TIMED_OOC_DESPAWN,
                                       5000);
            m_creature->SummonCreature(NPC_MAGE, 2109.852f, 195.1403f, 66.30493f, 2.42f, TEMPSPAWN_TIMED_OOC_DESPAWN,
                                       5000);
        ...
        case 68:
            m_creature->SummonCreature(NPC_SKARLOC_MOUNT, 2488.779f, 623.9724f, 58.07383f, 1.37f,
                                       TEMPSPAWN_TIMED_DESPAWN, 30000);
            m_creature->Unmount();
            m_bHadMount = false;
            break;
        case 69:
        ...
        case 111:
            ...
            m_creature->SummonCreature(NPC_TARREN_MILL_LOOKOUT, 2648.96f, 662.59f, 61.93f, 0.79f,
                                       TEMPSPAWN_TIMED_OOC_DESPAWN, 5000);
            m_creature->SummonCreature(NPC_TARREN_MILL_LOOKOUT, 2657.36f, 662.34f, 61.93f, 2.68f,
                                       TEMPSPAWN_TIMED_OOC_DESPAWN, 5000);
        ...
        case 114:
            m_creature->SummonCreature(NPC_EPOCH, 2639.9202f, 700.25867f, 65.13583f, 4.74729f, TEMPSPAWN_DEAD_DESPAWN,
                                       0);
        ...
        case 123:
            // begin fight
            m_lTarrenMillSoldiersGuids.clear();
            if (Creature *pGuardsman = m_creature->SummonCreature(NPC_TARREN_MILL_GUARDSMAN, 2629.452f, 716.2737f,
                                                                  56.55614f, 4.73f, TEMPSPAWN_DEAD_DESPAWN, 0))
            ...
            if (Creature *pProtector = m_creature->SummonCreature(NPC_TARREN_MILL_PROTECTOR, 2655.716f, 698.5595f,
                                                                  57.72154f, 3.17f, TEMPSPAWN_DEAD_DESPAWN, 0))
...
bool GossipHello_npc_thrall_old_hillsbrad(Player *pPlayer, Creature *pCreature)
{
    ...
    if (instance_old_hillsbrad *pInstance = (instance_old_hillsbrad *)pCreature->GetInstanceData())
    {
        ...
        {
            pPlayer->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_START, GOSSIP_SENDER_MAIN,
                                        GOSSIP_ACTION_INFO_DEF + 1);
...
enum
{
    ...
    GOSSIP_ITEM_EPOCH_2 = -3560006, // "We'll get you out. Taretha. Don't worry. I doubt the wizard
                                    // would wander too far away."
    ...
    SPELL_SHADOW_PRISON = 33071, // in creature_template_addon - remove from
                                 // Taretha on event complete
};
...
struct npc_tarethaAI : public npc_escortAI, private DialogueHelper
{
    ...
    void JustSummoned(Creature *pSummoned) override
    {
        // Remove flags from the npc - the quest will be handled by the entrance
        // version
...
bool GossipHello_npc_taretha(Player *pPlayer, Creature *pCreature)
{
    ...
    {
        pPlayer->ADD_GOSSIP_ITEM_ID(GOSSIP_ICON_CHAT, GOSSIP_ITEM_EPOCH_1, GOSSIP_SENDER_MAIN,
                                    GOSSIP_ACTION_INFO_DEF + 1);
...
{
    npc_bartolo_ginsetti(Creature *creature) : ScriptedAI(creature)
    {
        Reset();
    }
