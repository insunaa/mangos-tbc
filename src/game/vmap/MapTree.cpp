...
//=========================================================
//! Make sure to call unloadMap() to unregister acquired model references
//! before destroying
        ...
        if (!iIsTiled)
        {
            ...
            while (ModelSpawn::readFromFile(rf, spawn))
            {
                ...
                if (model)
                    ...
                    ERROR_LOG("StaticMapTree::LoadMapTile() could not acquire "
                              "WorldModel "
                              "pointer for '%s'!",
                              spawn.name.c_str());
...
bool StaticMapTree::LoadMapTile(uint32 tileX, uint32 tileY, VMapManager2 *vm)
{
    ...
    if (!iTreeValues)
    {
        ERROR_LOG("StaticMapTree::LoadMapTile(): Tree has not been "
                  "initialized! [%u,%u]",
                  tileX, tileY);
        return false;
    }
...
void StaticMapTree::UnloadMapTile(uint32 tileX, uint32 tileY, VMapManager2 *vm)
{
    ...
    {
        // ERROR_LOG("StaticMapTree::UnloadMapTile(): Trying to unload non-loaded
        // tile. Map:%u X:%u Y:%u", iMapID, tileX, tileY);
        return;
    }
