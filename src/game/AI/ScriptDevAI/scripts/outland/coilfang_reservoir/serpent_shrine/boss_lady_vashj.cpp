...
SD%Complete: 80
SDComment: Some details are not very clear: the usage of Shoot and Multishot
spells; the summons positions. Tainted Core paralize NYI. Timers need
improvements. SDCategory: Coilfang Resevoir, Serpent Shrine Cavern
EndScriptData */
...
enum
{
    ...
    SPELL_PARALYZE = 38132,  // aura which should apply to the player which picked
                             // the tainted core
...
struct boss_lady_vashjAI : public CombatAI
{
    boss_lady_vashjAI(Creature *creature)
        : CombatAI(creature, VASHJ_ACTION_MAX),
    ...
    void MovementInform(uint32 uiType, uint32 uiPointId) override
    {
        ...
        if (uiPointId == POINT_MOVE_CENTER)
        {
            ...
            if (m_instance)
            {
                ...
                for (GuidList::const_iterator itr = lShieldGeneratorsGuid.begin(); itr != lShieldGeneratorsGuid.end();
                     ++itr)
                {
            ...
            {
                // set creature facing to PI for guid fills (proper
    ...
    void JustSummoned(Creature *summoned) override
    {
        ...
        case NPC_TAINTED_ELEMENTAL:
            ...
            summoned->ForcedDespawn(16000,
                                    true); // only despawn after 15 seconds if still alive, different
                                           // rules on death, one second more for appearing
        ...
        case NPC_ENCHANTED_ELEMENTAL:
            // summoned->GetMotionMaster()->MovePoint(POINT_MOVE_CENTER,
            // afMiddlePos[0], afMiddlePos[1], afMiddlePos[2]);
    ...
    void DoInformGeneratorStopped()
    {
        ...
        // Check if all generators have been deactivated, or the creature doesn't
        // have the spell barrier aura (in order to avoid eventual aura stacking
        // bugs)
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case VASHJ_ACTION_STATIC_CHARGE: {
            if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_STATIC_CHARGE,
                                                                 SELECT_FLAG_PLAYER))
            {
...
{
    // Interrupt Magic barrier spell casting, inform the boss and make the GO
    // unusable
