...
// Texts used during the encounter: they are ordered so we can easily use them
// in a parent class
...
// Spells used during the encounter: they are ordered so we can easily use them
// in a parent class
...
// Coordinates at the entrance of the Four Horsemen room. Beyond these, the
// Horsemen reset (leashing)
...
struct boss_horsmenAI : public ScriptedAI
{
    ...
    uint32 m_horsmenIndex; // Used to differentiate each of the Four Horsemen in
    ...
    void Reset() override
    {
        ...
        m_markCounter = 0;
        m_horsmenIndex = INDEX_BLAUMEUX; // Default: Lady Blaumeux (1: Highlord Morgraine, 2:
                                         // Thane Korth'azz, 3: Sir Zeliek)
...
{
    boss_lady_blaumeuxAI(Creature *creature) : boss_horsmenAI(creature)
    {
        Reset();
    }
...
{
    boss_alexandros_mograineAI(Creature *creature) : boss_horsmenAI(creature)
    {
        Reset();
    }
...
{
    boss_thane_korthazzAI(Creature *creature) : boss_horsmenAI(creature)
    {
        Reset();
    }
...
{
    boss_sir_zeliekAI(Creature *creature) : boss_horsmenAI(creature)
    {
        Reset();
    }
