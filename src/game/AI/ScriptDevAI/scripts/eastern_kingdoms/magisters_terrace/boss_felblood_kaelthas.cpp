...
enum
{
    ...
    SPELL_GRAVITY_LAPSE_VISUAL = 44251, // Channeled; blue beam animation to every enemy in range - when
                                        // removed the Gravity Lapse auras are removed from players
    SPELL_TELEPORT_CENTER = 44218,      // Teleport the boss in the center. Requires
                                        // DB entry in spell_target_position.
    SPELL_GRAVITY_LAPSE_FLY = 44227,    // Hastens flyspeed and allows flying for 1 minute. Requires aura
                                        // stacking exception for 44226.
...
struct boss_felblood_kaelthasAI : public CombatAI
{
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case KAEL_ACTION_FLAMESTRIKE: {
            if (Unit *target = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0, SPELL_FLAME_STRIKE,
                                                                 SELECT_FLAG_PLAYER))
            {
        ...
        case KAEL_ACTION_FIREBALL: {
            if (DoCastSpellIfCan(m_creature->GetVictim(), m_isRegularMode ? SPELL_FIREBALL : SPELL_FIREBALL_H) ==
                CAST_OK)
...
struct spell_gravity_lapse_mgt : public SpellScript
{
    void OnEffectExecute(Spell *spell, SpellEffectIndex /*effIdx*/) const override
    {
        ...
        spell->GetCaster()->CastSpell(unitTarget, aGravityLapseSpells[spell->GetScriptValue()],
