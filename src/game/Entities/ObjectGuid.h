...
class ObjectGuid
{
  ...
  private:
    ...
    ObjectGuid(HighGuid, uint32,
               uint64 counter); // no implementation, used for catch wrong type assign
    ObjectGuid(HighGuid,
               uint64 counter); // no implementation, used for catch wrong type assign
  ...
  public: // accessors
    ...
    {
        return HasEntry() ? uint32((m_guid >> 24) & uint64(0x0000000000FFFFFF)) : 0;
    }
...
template <HighGuid high> class ObjectGuidGenerator
{
  ...
  public: // accessors
    uint32 GetNextAfterMaxUsed() const
    {
        return m_nextGuid;
    }
