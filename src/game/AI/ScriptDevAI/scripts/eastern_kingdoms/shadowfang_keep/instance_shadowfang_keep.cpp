...
void instance_shadowfang_keep::OnCreatureCreate(Creature *creature)
{
    ...
    case NPC_SLAVERING_WORG:
        // Only store the wolves/worgs that are static spawn on the top level of
        // the instance
...
void instance_shadowfang_keep::OnCreatureDeath(Creature *creature)
{
    ...
    case NPC_SLAVERING_WORG:
        ...
        if (m_lNandosWolvesGuids.empty())
        {
            ...
            {
                if (nandos->IsInCombat()) // Wolf Master Nandos already joined
                                          // the fight: no need to go further
                    return;
...
void instance_shadowfang_keep::SetData(uint32 type, uint32 data)
{
    ...
    case TYPE_FENRUS:
        if (data == DONE)
        {
            ...
            {
                fenrus->SummonCreature(NPC_ARCHMAGE_ARUGAL, -136.89f, 2169.17f, 136.58f, 2.794f,
