    ...
    void HandleStart()
    {
        ...
        case 2: // wps start
            if (m_creature->GetOwner() && m_creature->GetOwner()->GetTypeId() == TYPEID_PLAYER)
                ...
                script_error_log("npc_khadgars_servant can not obtain owner or owner "
                                 "is not a player.");
            break;
        }
        ++m_startPhase;
...
bool QuestRewarded_npc_adal(Player *player, Creature *creature, Quest const *quest)
{
    ...
    case QUEST_KAELTHAS_AND_THE_VERDANT_SPHERE:
        ...
        player->GetMap()->ScriptsStart(sRelayScripts, SCRIPT_RELAY_ID, creature, player,
                                       Map::SCRIPT_EXEC_PARAM_UNIQUE_BY_SOURCE); // only once active per
                                                                                 // adal
        return true;                                                             // handled
    }
...
struct DemonBroiledSurprise : public SpellScript
{
    SpellCastResult OnCheckCast(Spell *spell, bool strict) const
    {
        if (strict)
        {
            ...
            return spell->CheckScriptTargeting(EFFECT_INDEX_1, 1, radius, TARGET_ENUM_UNITS_SCRIPT_AOE_AT_SRC_LOC,
