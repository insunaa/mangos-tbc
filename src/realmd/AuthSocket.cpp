...
{
    // benchmarking has demonstrated that this lookup method is faster than
    // std::map
    ...
    // the purpose of this loop is to handle multiple opcodes in the same tcp
    // packet, which presumably the client will never do, but lets support it
    // anyway! \o/
    while (ReadLengthRemaining() > 0)
    {
        ...
        // did we iterate over the entire command table, finding nothing? if so,
        // punt!
        if (i == tableLength)
        {
    ...
    // if we reach here, it means that a valid opcode was found and the handler
...
bool AuthSocket::_HandleLogonChallenge()
{
    ...
    ///- Read the first 4 bytes (header) to get the length of the remaining of
    /// the
    /// packet
    ...
    // Convert uint8[4] to string, restore string order as its byte order is
    // reversed
    ch->os[3] = '\0';
    m_os = (char *)ch->os;
    ...
    // utf8ToUpperOnlyLatin(_login); -- client already send account in expected
    // form
    ...
    std::unique_ptr<QueryResult> ip_banned_result(LoginDatabase.PQuery("SELECT expires_at FROM ip_banned "
                                                                       "WHERE (expires_at = banned_at OR expires_at > "
    ...
    else
    {
        ...
        QueryResult *result = LoginDatabase.PQuery("SELECT id,locked,lockedIp,gmlevel,v,s,token FROM "
                                                   "account WHERE username = '%s'",
                                                   _safelogin.c_str());
        if (result)
        {
            Field *fields = result->Fetch();

            ///- If the IP is 'locked', check that the player comes indeed from
            /// the
            /// correct IP address
            bool locked = false;
            ...
            {
                pkt << uint8(AUTH_LOGON_FAILED_FAIL_NOACCESS);
                DEBUG_LOG("[AuthChallenge] Broken v/s values in database for "
                          "account %s!",
                          _login.c_str());
                broken = true;
            }
            ...
            if (!locked && !broken)
            {
                ...
                QueryResult *banresult = LoginDatabase.PQuery("SELECT banned_at,expires_at FROM account_banned WHERE "
                                                              "account_id = %u AND active = 1 AND (expires_at > "
                                                              "UNIX_TIMESTAMP() "
                ...
                if (banresult)
                {
                    ...
                    {
                        pkt << uint8(AUTH_LOGON_FAILED_SUSPENDED);
                        BASIC_LOG("[AuthChallenge] Temporarily banned account "
                                  "%s tries to login!",
                                  _login.c_str());
                    }
                ...
                else
                {
                    ...
                    // B may be calculated < 32B so we force minimal length to
                    // 32B
...
bool AuthSocket::_HandleLogonProof()
{
    ...
    if (!FindBuildInfo(_build))
    {
        ...
        BASIC_LOG("[AuthChallenge] Account %s tried to login with invalid client "
                  "version %u!",
                  _login.c_str(), _build);
        Write((const char *)pkt.contents(), pkt.size());
        return true;
    ...
    if (!srp.Proof(lp.M1, 20))
    {
        if (lp.securityFlags & SECURITY_FLAG_AUTHENTICATOR || !_token.empty())
        {
            ...
            {
                BASIC_LOG("[AuthChallenge] Account %s tried to login with wrong "
                          "pincode! Given %u Expected %u Pin Count: %u",
        ...
        {
            BASIC_LOG("[AuthChallenge] Account %s tried to login with modified "
                      "client!",
                      _login.c_str());
        ...
        ///- Update the sessionkey, current ip and login time and reset number of
        /// failed logins in the account table for this account
        // No SQL injection (escaped user input) and IP address as received by
        // socket
        const char *K_hex = srp.GetStrongSessionKey().AsHexStr();
        LoginDatabase.PExecute("UPDATE account SET sessionkey = '%s', locale = "
                               "'%s', failed_logins = "
                               "0, os = '%s' WHERE username = '%s'",
                               K_hex, _safelocale.c_str(), m_os.c_str(), _safelogin.c_str());
        if (QueryResult *loginfail =
                LoginDatabase.PQuery("SELECT id FROM account WHERE username = '%s'", _safelogin.c_str()))
            LoginDatabase.PExecute("INSERT INTO account_logons(accountId,ip,loginTime,loginSource) "
                                   "VALUES('%u','%s',NOW(),'%u')",
    ...
    else
    {
        ...
        if (MaxWrongPassCount > 0)
        {
            ...
            LoginDatabase.PExecute("UPDATE account SET failed_logins = failed_logins "
                                   "+ 1 WHERE username = '%s'",
                                   _safelogin.c_str());

            if (QueryResult *loginfail = LoginDatabase.PQuery("SELECT id, failed_logins FROM account "
                                                              "WHERE username = '%s'",
                                                              _safelogin.c_str()))
            {
                ...
                if (failed_logins >= MaxWrongPassCount)
                {
                    ...
                    if (WrongPassBanType)
                    {
                        ...
                        LoginDatabase.PExecute("INSERT INTO account_banned(account_id, banned_at, "
                                               "expires_at, "
                                               "banned_by, reason, active)"
                                               "VALUES "
                                               "('%u',UNIX_TIMESTAMP(),UNIX_TIMESTAMP()+'%u','"
                                               "MaNGOS "
                                               "realmd','Failed login autoban',1)",
                                               acc_id, WrongPassBanTime);
                        BASIC_LOG("[AuthChallenge] account %s got banned for '%u' "
                                  "seconds "
                    ...
                    else
                    {
                        ...
                        LoginDatabase.PExecute("INSERT INTO ip_banned VALUES "
                                               ...
                                               "u','MaNGOS realmd','Failed login autoban')",
                                               current_ip.c_str(), WrongPassBanTime);
                        BASIC_LOG("[AuthChallenge] IP %s got banned for '%u' seconds "
                                  "because account %s failed to authenticate '%u' "
                                  "times",
...
bool AuthSocket::_HandleReconnectChallenge()
{
    ...
    if (!result)
    {
        sLog.outError("[ERROR] user %s tried to login and we cannot find his "
...
bool AuthSocket::_HandleRealmList()
{
    ...
    QueryResult *result =
        LoginDatabase.PQuery("SELECT id, gmlevel FROM account WHERE username = '%s'", _safelogin.c_str());
    if (!result)
    {
        sLog.outError("[ERROR] user %s tried to login and we cannot find him in "
                      "the database.",
                      _login.c_str());
        Close();
        return false;
...
void AuthSocket::LoadRealmlist(ByteBuffer &pkt, uint32 acctid, uint8 securityLevel)
{
    ...
    case 6141: // 1.12.3
    {
        ...
        for (const auto &i : sRealmList)
        {
            ...
            QueryResult *result = LoginDatabase.PQuery("SELECT numchars FROM realmcharacters WHERE "
                                                       "realmid = '%d' AND acctid='%u'",
            ...
            bool ok_build = std::find(i.second.realmbuilds.begin(), i.second.realmbuilds.end(), _build) !=
            ...
            // 1.x clients not support explicitly REALM_FLAG_SPECIFYBUILD, so
            // manually form similar name as show in more recent clients
            std::string name = i.first;
            if (realmflags & REALM_FLAG_SPECIFYBUILD)
            {
                char buf[20];
                snprintf(buf, 20, " (%u,%u,%u)", buildInfo->major_version, buildInfo->minor_version,
