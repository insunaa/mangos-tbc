...
void GameEventMgr::LoadFromDB()
{
    ...
    std::unique_ptr<QueryResult> result(WorldDatabase.Query("SELECT "
                                                            "entry,schedule_type,occurence,length,holiday,"
                                                            "linkedTo,EventGroup,description FROM game_event"));
    if (!result)
    {
        m_gameEvents.clear();
        sLog.outString(">> Table game_event is empty!");
...

    {
        ...
        do
        {
            ...
            if (event_id == 0)
            {
                sLog.outErrorDb("`game_event` game event id (%i) is reserved and "
                                "can't be used.",
                                event_id);
                continue;
            }
            ...
            if (gameEvent.length == 0 && gameEvent.scheduleType != GAME_EVENT_SCHEDULE_SERVERSIDE) // length>0 is
                                                                                                   // validity check
            {
                sLog.outErrorDb("`game_event` game event id (%i) have length 0 "
                                "and can't be used.",
                                event_id);
                gameEvent.start = time_t(FAR_FUTURE);
            }

            if (gameEvent.occurence < gameEvent.length) // occurence < length is useless. This also
                                                        // asserts that occurence > 0!
            {
                sLog.outErrorDb("`game_event` game event id (%i) has occurence %u  < "
                                "length %u and can't be used.",
                                event_id, gameEvent.occurence, gameEvent.length);
                gameEvent.start = time_t(FAR_FUTURE);
            }
        ...
        for (uint16 itr = 1; itr < m_gameEvents.size(); ++itr)
        {
            if (m_gameEvents[itr].isValid() && m_gameEvents[itr].linkedTo != 0 &&
                (m_gameEvents[itr].linkedTo >= m_gameEvents.size() ||
                 !m_gameEvents[m_gameEvents[itr].linkedTo].isValid()))
            {
                sLog.outErrorDb("`game_event` game event id (%i) is Linked to "
                                "invalid event %u",
                                itr, m_gameEvents[itr].linkedTo);
    ...
    else
    {
        do
        {
            ...
            if (gameEvent.scheduleType != GAME_EVENT_SCHEDULE_DATE &&
                gameEvent.scheduleType != GAME_EVENT_SCHEDULE_YEARLY)
            {
                sLog.outErrorDb("`game_event` game event id (%i) has game_event_time "
                                "but is not date scheduled - ignoring",
                                event_id);
                continue;
            }
    ...
    result.reset(WorldDatabase.Query("SELECT creature.guid, game_event_creature.event "
                                     "FROM creature JOIN game_event_creature ON "
                                     "creature.guid = game_event_creature.guid"));
    ...
    else
    {
        ...
        do
        {
            ...
            {
                sLog.outErrorDb("`game_event_creature` game event id (%i) not exist in "
                                "`game_event`",
                                event_id);
                continue;
            }
                ...
                if (uint16 topPoolId = sPoolMgr.IsPartOfTopPool<Creature>(guid))
                {
                    ...
                    if (eventRef != 0)
                    {
                        if (eventRef != event_id)
                            sLog.outErrorDb("`game_event_creature` have creature "
                                            "(GUID: %u) for event %i "
                                            "from pool or subpool of pool (ID: "
                                            "%u) but pool have already "
                                            "content from event %i. Pool don't "
                                            "must have content for "
                                            "different events!",
    ...
    else
    {
        ...
        do
        {
            ...
            if (!IsValidEvent(std::abs(event_id)))
            {
                sLog.outErrorDb("`game_event_gameobject` game event id (%i) not exist "
                                "in `game_event`",
                                event_id);
                continue;
            }
            ...
            if (event_id > 0)
            {
                ...
                // not list explicitly gameobjects from pools in event gameobject
                // list
                if (uint16 topPoolId = sPoolMgr.IsPartOfTopPool<GameObject>(guid))
                {
                    ...
                    {
                        if (eventRef != event_id)
                            sLog.outErrorDb("`game_event_gameobject` have "
                                            "gameobject (GUID: %u) for "
                                            "event %i from pool or subpool of "
                                            "pool (ID: %u) but pool "
                                            "have already content from event %i. "
                                            "Pool don't must have "
    ...
    //                                   0              1 2
    result.reset(WorldDatabase.Query("SELECT creature.guid, game_event_creature_data.event, "
                                     "game_event_creature_data.modelid,"
                                     //   3                                      4
                                     "game_event_creature_data.equipment_id, "
                                     "game_event_creature_data.vendor_id, "
                                     //   5                                     6
    ...
    else
    {
        ...
        do
        {
            ...
            if (event_id == 0)
            {
                sLog.outErrorDb("`game_event_creature_data` game event id (%i) is "
            ...
            {
                sLog.outErrorDb("`game_event_creature_data` game event id (%u) not "
                                "exist in `game_event`",
                                event_id);
                continue;
            }
            ...
            {
                sLog.outErrorDb("Table `game_event_creature_data` have creature (Guid: %u) "
                                "with "
            ...
            if (newData.vendor_id)
            {
                ...
                {
                    sLog.outErrorDb("Table `game_event_creature_data` has a creature with "
                                    "(Guid: %u) "
                                    "and vendor_id %u which was not found in table "
                                    "`npc_vendor_template`, set to no vendor.",
            ...
            {
                sLog.outErrorDb("Table `game_event_creature_data` have creature (Guid: "
                                "%u) with event time entry %u not found in table "
    ...
    else
    {
        ...
        do
        {
            ...
            if (event_id == 0)
            {
                sLog.outErrorDb("`game_event_quest` game event id (%i) is reserved and "
                                "can't be used.",
                                event_id);
                continue;
            }
            ...
            if (!IsValidEvent(event_id))
            {
                sLog.outErrorDb("`game_event_quest` game event id (%u) not "
                                "exist in `game_event`",
                                event_id);
                continue;
            }
            ...
            if (!pQuest)
            {
                sLog.outErrorDb("Table `game_event_quest` contain entry for quest %u "
    ...
    result.reset(WorldDatabase.Query("SELECT event, raceMask, quest, mailTemplateId, "
                                     "senderEntry FROM game_event_mail"));
    ...
    else
    {
        ...
        do
        {
            ...
            if (!IsValidEvent(event_id))
            {
                sLog.outErrorDb("`game_event_mail` game event id (%u) not "
                                "exist in `game_event`",
                                event_id);
                continue;
            }
            ...
            {
                sLog.outErrorDb("Table `game_event_mail` have raceMask (%u) requirement for "
                                "game "
            ...
            {
                sLog.outErrorDb("Table `game_event_mail` have nonexistent quest (%u) "
            ...
            {
                sLog.outErrorDb("Table `game_event_mail` have invalid "
                                "mailTemplateId (%u) for game "
                                "event %i that invalid not include any player "
                                "races, ignoring.",
            ...
            {
                sLog.outErrorDb("Table `game_event_mail` have nonexistent sender "
                                "creature entry (%u) for game event %i that invalid "
...
uint32 GameEventMgr::Update(ActiveEvents const *activeAtShutdown /*= nullptr*/)
{
    ...
    return (nextEventDelay + 1) * IN_MILLISECONDS; // Add 1 second to be sure event has
                                                   // started/stopped at next call
}
...
void GameEventMgr::GameEventSpawn(int16 event_id)
{
    ...
    if (event_id > 0)
    {
        ...
        {
            sLog.outError("GameEventMgr::GameEventSpawn attempt access to out of "
                          "range mGameEventSpawnPoolIds element %i (size: " SIZEFMTD ")",
...
void GameEventMgr::GameEventUnspawn(int16 event_id)
{
    ...
    if (internal_event_id < 0 || (size_t)internal_event_id >= m_gameEventCreatureGuids.size())
    {
        sLog.outError("GameEventMgr::GameEventUnspawn attempt access to out of "
                      "range mGameEventCreatureGuids element %i (size: " SIZEFMTD ")",
                      internal_event_id, m_gameEventCreatureGuids.size());
        return;
    }
    ...
    {
        sLog.outError("GameEventMgr::GameEventUnspawn attempt access to out of "
                      "range mGameEventGameobjectGuids element %i (size: " SIZEFMTD ")",
                      internal_event_id, m_gameEventGameobjectGuids.size());
        return;
    }
...
{
    GameEventUpdateCreatureDataInMapsWorker(ObjectGuid guid, CreatureData const *data,
                                            GameEventCreatureData *event_data, bool activate)
    ...
    void operator()(Map *map)
    {
        if (Creature *pCreature = map->GetCreature(i_guid))
        {
            ...
            // spells not casted for event remove case (sent nullptr into update),
            // do it
            if (!i_activate)
...
void GameEventMgr::SendEventMails(int16 event_id)
{
    ...
    for (auto mail : mails)
    {
        if (mail.questId)
        {
            ...
            sMassMailMgr.AddMassMailTask(new MailDraft(mail.mailTemplateId),
...
template <> int16 GameEventMgr::GetGameEventId<Creature>(uint32 guid_or_poolid)
{
    for (uint16 i = 0; i < m_gameEventCreatureGuids.size(); ++i) // 0 <= i <= 2*(S := m_gameEvents.size()) - 2
        for (GuidList::const_iterator itr = m_gameEventCreatureGuids[i].begin();
             itr != m_gameEventCreatureGuids[i].end(); ++itr)
...
void GameEventMgr::ComputeEventStartAndEndTime(GameEventData &data, time_t today)
{
    ...
    case GAME_EVENT_SCHEDULE_DMF_BUILDING_STAGE_2_3: {
        ...
        mktime(&firstMonday);
        firstMonday.tm_mday =
            (firstMonday.tm_mday - 1 - (firstMonday.tm_wday - 1) + 7) % 7 + 1; // get this weeks monday
        switch (data.scheduleType)
        {
        case GAME_EVENT_SCHEDULE_DMF_BUILDING_STAGE_1_1:
        case GAME_EVENT_SCHEDULE_DMF_BUILDING_STAGE_1_2:
    ...
    case GAME_EVENT_SCHEDULE_LUNAR_NEW_YEAR: {
        Moon moon;
        tm todayT = *localtime(&today); // take year from today
        --todayT.tm_year;               // need to start at winter solstice - 21st Dec of
                                        // previous year
        todayT.tm_year += 1900;
        todayT.tm_mday = 21;
