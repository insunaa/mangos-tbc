...
enum UpdateFieldFlags : uint16
{
    ...
    UF_FLAG_DYNAMIC = 0x100,      // visible to everyone, but different values can be
                                  // sent to different observers
};

struct UpdateFieldData
{
    UpdateFieldData() = default;
    constexpr UpdateFieldData(uint8 objectTypeMask_, const char *name_, uint16 offset_, uint16 size_,
                              UpdateFieldValueTypes valueType_, uint16 flags_)
        : objectTypeMask(objectTypeMask_), name(name_), offset(offset_), size(size_), valueType(valueType_),
          flags(flags_)
    {
    }
