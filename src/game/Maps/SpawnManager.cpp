...
{
    m_spawns.emplace_back(m_map.GetCurrentClockTime() + std::chrono::seconds(respawnDelay), dbguid,
                          HIGHGUID_GAMEOBJECT);
    std::sort(m_spawns.begin(), m_spawns.end());
}
...
void SpawnManager::Update()
{
    ...
    {
        auto &spawnInfo = *itr;
        if (spawnInfo.IsUsed() || (spawnInfo.GetRespawnTime() <= now && spawnInfo.ConstructForMap(m_map)))
...
std::string SpawnManager::GetRespawnList()
{
    ...
    {
        output += "DBGuid: " + std::to_string(data.GetDbGuid()) +
                  "HighGuid: " + (data.GetHighGuid() == HIGHGUID_UNIT ? "Creature" : "GameObject") + "Respawn Time ";
