...
pAuraProcHandler AuraProcHandler[TOTAL_AURAS] = {
    ...
    &Unit::HandleProcTriggerDamageAuraProc,           // 43
    ...
    &Unit::HandleNULLProc,                            // 193 SPELL_AURA_HASTE_ALL (in fact combat (any
                                                      // type attack) speed pct)
    ...
    &Unit::HandleNULLProc,                            // 201 SPELL_AURA_FLY this aura enable flight
                                                      // mode...
    ...
    &Unit::HandleRaidProcFromChargeWithValueAuraProc, // 225
    ...
    &Unit::HandleNULLProc,                            // 261 SPELL_AURA_PHASE some phased state (44856
                                                      // spell)
};
                                  ...
                                  WeaponAttackType attType)
{
    ...
    if (procFlags & MELEE_BASED_TRIGGER_MASK)
    {
        ...
        if (GetTypeId() == TYPEID_PLAYER)
        {
            ...
            // attacker)
            if (procEx & (PROC_EX_NORMAL_HIT | PROC_EX_CRITICAL_HIT | PROC_EX_MISS | PROC_EX_DODGE | PROC_EX_PARRY |
                          PROC_EX_BLOCK))
            {
...
void Unit::ProcDamageAndSpell(ProcSystemArguments &&data)
{
    ...
    if (data.attacker)
    {
        if (data.attacker->m_spellProcsHappening)
            currentLevel = false; // triggered spell in proc system should not make
                                  // holders ready
        data.attacker->m_spellProcsHappening = true;
        if (data.procFlagsAttacker)
            data.attacker->ProcSkillsAndReactives(false, data.victim, data.procFlagsAttacker, data.procExtra,
                                                  data.attType);
    }
    ...
    if (data.attacker)
    {
        ...
        if (data.attacker->GetTypeId() == TYPEID_PLAYER &&
            ...
            if ((data.procFlagsAttacker & PROC_FLAG_DEAL_HARMFUL_PERIODIC) == 0) // do not proc this on DOTs
                if (!data.spellInfo || (data.spellInfo->EquippedItemClass == ITEM_CLASS_WEAPON &&
                                        !data.spellInfo->HasAttribute(SPELL_ATTR_STOP_ATTACK_TARGET)))
                    static_cast<Player *>(data.attacker)
                        ->CastItemCombatSpell(data.victim, data.attType,
                                              data.spellInfo ? !IsNextMeleeSwingSpell(data.spellInfo) : false);
        ...
        if (currentLevel)
        {
            ...
            for (SpellAuraHolder *holder : data.attacker->m_delayedSpellAuraHolders)
                if (holder->GetState() == SPELLAURAHOLDER_STATE_CREATED) // if deleted by some unknown
                                                                         // circumstance
...
ProcExecutionData::ProcExecutionData(ProcSystemArguments &data, bool isVictim)
    : attacker(data.attacker), victim(data.victim), isVictim(isVictim), procExtra(data.procExtra),
      attType(data.attType), damage(data.damage), spellInfo(data.spellInfo), spell(data.spell),
      healthGain(data.healthGain), isHeal(data.isHeal), triggeredByAura(nullptr), cooldown(0), triggeredSpellId(0),
      procOnce(false), triggerTarget(nullptr)
{
    if (isVictim)
    {
        source = data.victim;
...
void Unit::ProcDamageAndSpellFor(ProcSystemArguments &argData, bool isVictim)
{
    ...
    {
        // Some auras can be deleted in function called in this loop (except
        // first, ofc)
            ...
            if (execData.spellInfo)
            {
                if (spellProcEvent)
                {
                    if (spellProcEvent->spellFamilyMask[i])
                    {
                        ...
                        if (IsCastEndProcModifierAura(triggeredByHolder->GetSpellProto(), SpellEffectIndex(i),
                                                      execData.spellInfo))
                        {
                            if (useCharges && execData.procExtra != PROC_EX_CAST_END &&
                                spellProcEvent->procEx == PROC_EX_NONE)
                                continue;
                        }
                    ...
                    else if (!execData.triggeredByAura->CanProcFrom(execData.spellInfo, spellProcEvent->procEx,
                                                                    execData.procExtra, execData.damage != 0,
                                                                    !spellProcEvent->schoolMask))
                        continue;
                }
                else if (!execData.triggeredByAura->CanProcFrom(execData.spellInfo, PROC_EX_NONE, execData.procExtra,
                                       ...
                                       SpellProcEventEntry const *&spellProcEvent)
{
    ...
    if (!SpellMgr::IsSpellProcEventCanTriggeredBy(spellProcEvent, EventProcFlag, data.spellInfo, data.procFlags,
                                                  data.procExtra))
        return false;
    ...
    // triggers) But except periodic triggers (can triggered from self)
    ...
    if (data.spell)
    {
        ...
        {
            if (!data.spell->m_spellInfo->HasAttribute(SPELL_ATTR_EX2_TRIGGERED_CAN_TRIGGER_PROC) &&
                !data.spell->m_spellInfo->HasAttribute(SPELL_ATTR_EX3_TRIGGERED_CAN_TRIGGER_SPECIAL))
                return false;
        }
                                              ...
                                              uint32 cooldown, ObjectGuid originalCaster)
{
    ...
    if (!triggerEntry)
    {
        sLog.outError("Unit::TriggerProccedSpell: Script has nonexistent "
                      "triggered spell %u",
                      triggeredSpellId);
                                              ...
                                              uint32 cooldown, ObjectGuid originalCaster)
{
    ...
    if (basepoints[EFFECT_INDEX_0] || basepoints[EFFECT_INDEX_1] || basepoints[EFFECT_INDEX_2])
        CastCustomSpell(target, spellInfo, basepoints[EFFECT_INDEX_0] ? &basepoints[EFFECT_INDEX_0] : nullptr,
                        ...
                        TRIGGERED_OLD_TRIGGERED | TRIGGERED_INSTANT_CAST | TRIGGERED_DO_NOT_RESET_LEASH, castItem,
                        triggeredByAura, originalCaster);
    else
        CastSpell(target, spellInfo, TRIGGERED_OLD_TRIGGERED | TRIGGERED_INSTANT_CAST | TRIGGERED_DO_NOT_RESET_LEASH,
...
SpellAuraProcResult Unit::HandleDummyAuraProc(ProcExecutionData &data)
{
    ...
    case SPELLFAMILY_GENERIC: {
        ...
        case 35429: {
            ...
            if (spellInfo)
            {
                ...
                else if (spellInfo->TargetAuraState == AURA_STATE_HEALTHLESS_20_PERCENT)
                {
                    ...
                    else // execute procs normalized damage if target is
                         // above 20 percent
                        triggered_spell_id = 26654;
                }
                else
        ...
        case 32052: // Red
        {
            ...
            return SPELL_AURA_PROC_CANT_TRIGGER; // Strangely, stacks and
                                                 // charges are removed on
                                                 // cast finish
        }
        // Mark of Malice
        case 33493: {
        ...
        case 36035: {
            ...
            // aura only affect the spirit totem, since this is the one that
            // need to be in range. It is possible though, that player is the
            // one who should actually have the aura and check for presense
            // of spirit totem, but then we can't script the dummy.
            if (!pCaster)
                return SPELL_AURA_PROC_FAILED;
        ...
        // 40997 Delusional: +70 attack power (Rogue, Hunter, Paladin,
        // Warrior, Druid) 40998 Kleptomania: +35 agility (Warrior, Rogue,
        // Paladin, Hunter, Druid) 40999 Megalomania: +41 damage/healing
        // (Druid, Shaman, Priest, Warlock, Mage, Paladin) 41002 Paranoia:
        // +35 spell/melee/ranged crit strike rating (All classes) 41005
        // Manic: +35 haste (spell, melee and ranged) (All classes) 41009
        // Narcissism: +35 intellect (Druid, Shaman, Priest, Warlock, Mage,
        // Paladin, Hunter) 41011 Martyr Complex: +35 stamina (All classes)
        // 41406 Dementia: Every 5 seconds either gives you +5%
        // damage/healing. (Druid, Shaman, Priest, Warlock, Mage, Paladin)
        // 41409 Dementia: Every 5 seconds either gives you -5%
        // damage/healing. (Druid, Shaman, Priest, Warlock, Mage, Paladin)
        case 39446: {
            if (GetTypeId() != TYPEID_PLAYER)
                return SPELL_AURA_PROC_FAILED;
        ...
        case 45481: {
            ...
            {
                // triggered at positive/self casts also, current attack
                // target used then
                if (!CanAttack(target))
                {
                    target = GetVictim();
    ...
    case SPELLFAMILY_MAGE: {
        ...
        case 11129: {
            ...
            return (procEx & PROC_EX_CRITICAL_HIT) ? SPELL_AURA_PROC_OK : SPELL_AURA_PROC_FAILED; // charge update only
                                                                                                  // at crit hits, no
                                                                                                  // hidden cooldowns
        }
        }
    ...
    case SPELLFAMILY_WARRIOR: {
        ...
        {
            // only for spells and hit/crit (trigger start always) and not
            // start from self casted spells (5530 Mace Stun Effect for
            // example)
    ...
    case SPELLFAMILY_PRIEST: {
        ...
        if (dummySpell->SpellFamilyFlags & uint64(0x0000040000000000))
        {
            ...
            pVictim->CastCustomSpell(nullptr, 34919, &basepoints[0], nullptr, nullptr,
        ...
        case 15286: {
            ...
            // heal amount
            basepoints[0] = triggerAmount * damage / 100;
            pVictim->CastCustomSpell(nullptr, 15290, &basepoints[0], nullptr, nullptr,
                                     TRIGGERED_IGNORE_GCD | TRIGGERED_IGNORE_CURRENT_CASTED_SPELL |
                                         TRIGGERED_HIDE_CAST_IN_COMBAT_LOG);
            return SPELL_AURA_PROC_OK; // no hidden cooldown
        }
        ...
        // Frozen Shadoweave (Shadow's Embrace set) warning! its not only
        // priest set
        case 39372: {
            if (!spellInfo ||
                (GetSpellSchoolMask(spellInfo) & (SPELL_SCHOOL_MASK_FROST | SPELL_SCHOOL_MASK_SHADOW)) == 0)
                return SPELL_AURA_PROC_FAILED;
    ...
    case SPELLFAMILY_PALADIN: {
            ...
            triggeredByAura->GetEffIndex() == EFFECT_INDEX_0)
        {
            ...
            int32 damagePoint = int32(damageBasePoints + 0.03f *
                                                             (GetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE) +
                                                              GetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE)) /
                                                             2.0f) +
                                1;
            ...
            if (damagePoint >= 0)
            {
                // currently uses same spell damage fetch as flametongue - need
                // to verify whether SP is supposed to be applied pre-triggered
                // spell bonuses or post
                ...
                if (Aura *aura = GetAura(43743,
                                         EFFECT_INDEX_0)) // Improved Seal of Righteousness
        ...
        case 31785:
        case 33776: {
            // Only if healed by another unit, not spells like First Aid and
            // only when actual heal occured
        ...
        case 31801: {
            ...
            // Since Patch 2.2.0 Seal of Vengeance does additional damage
            // against fully stacked targets Add 5-stack effect from Holy
            // Vengeance
    ...
    case SPELLFAMILY_SHAMAN: {
        ...
        case 33757: {
            ...
            case 2636:
                ...
                break; // 5 Rank
            default: {
                sLog.outError("Unit::HandleDummyAuraProc: non handled item "
                              "enchantment "
                              "(rank?) "
            ...
            {
                sLog.outError("Unit::HandleDummyAuraProc: nonexistent spell id: %u "
                              "(Windfury)",
                              spellId);
                return SPELL_AURA_PROC_FAILED;
            }
            ...
            for (uint32 i = 0; i < 2; ++i)
                CastCustomSpell(pVictim, triggered_spell_id, &basepoints[0], nullptr, nullptr,
                                TRIGGERED_IGNORE_GCD | TRIGGERED_IGNORE_CURRENT_CASTED_SPELL, castItem,
                                triggeredByAura);
        ...
        if (dummySpell->SpellFamilyFlags & uint64(0x0000000000200000))
        {
            ...
            Item *usedWeapon =
                ((Player *)this)
                    ->GetWeaponForAttack(procFlags & PROC_FLAG_OFF_HAND_WEAPON_SWING ? OFF_ATTACK : BASE_ATTACK, true,
                                         true);
        ...
        // Lightning Overload
        if (dummySpell->SpellIconID == 2018) // only this spell have SpellFamily Shaman SpellIconID ==
                                             // 2018 and dummy aura
        {
...
SpellAuraProcResult Unit::HandleProcTriggerSpellAuraProc(ProcExecutionData &data)
{
    ...
    case SPELLFAMILY_GENERIC:
        ...
        // case 7377:  break:                   // Take Immune Periodic Damage
        // <Not Working> case 13358: break;                   // Defensive
        // State (DND) case 16092: break;                   // Defensive State
        // (DND) case 18943: break;                   // Double Attack case
        // 19194: break;                   // Double Attack case 19817: break;
        // // Double Attack case 19818: break;                   // Double
        // Attack case 22835: break;                   // Drunken Rage
        ...
        case 36096:                    // Spell Reflection
            return SPELL_AURA_PROC_OK; // Missing Trigger spell with no evidence
                                       // to tell what to trigger, need to return
                                       // to trigger consumption
        // case 36207: break:                   // Steal Weapon
        // case 36576: break:                   // Shaleskin (Shaleskin Flayer,
        // Shaleskin Ripper) 30023 trigger case 37030: break; // Chaotic
        // Temperament
        case 38164: // Unyielding Knights - this can only proc in hellfire
                    // peninsula with a maximum of 2 guardians against fel orc
                    // faction only
        ...
        case 39832: // Light of the Naaru - proc only hits demons near Black
                    // Temple entrance
        ...
        // case 43504: break;                   // Alterac Valley OnKill Proc
        // Aura
        ...
        // sec) case 44527: break;                       // Hate Monster (Spar
        // Buddy) (30 sec) case 44819: break;                       // Hate
        // Monster (Spar Buddy) (>30% Health) case 44820: break; // Hate
        // Monster (Spar) (<30%)
        ...
        case 48473:                                    // Capture Soul - Doom Lord Kazzak
            if (pVictim->GetTypeId() != TYPEID_PLAYER) // only player death procs
                return SPELL_AURA_PROC_FAILED;
            if (Player *lootRecipient = ((Creature *)this)->GetLootRecipient()) // only same team as the
                                                                                // one that tagged procs
    ...
    case SPELLFAMILY_MAGE:
        if (auraSpellInfo->SpellIconID == 2127) // Blazing Speed
        {
            ...
            case 31642: // Rank 2
                ...
                break;
            default:
                sLog.outError("Unit::HandleProcTriggerSpellAuraProc: Spell %u miss "
    ...
    case SPELLFAMILY_WARRIOR:
        ...
        if (!auraSpellInfo->SpellFamilyFlags && auraSpellInfo->SpellIconID == 243)
        {
            ...
            // TODO: check if using offhand damage is correct and if it should be
            // divided by 2
            if (hasOffhandWeaponForAttack() && getAttackTimer(BASE_ATTACK) > getAttackTimer(OFF_ATTACK))
                weaponDamage =
                    (GetFloatValue(UNIT_FIELD_MINOFFHANDDAMAGE) + GetFloatValue(UNIT_FIELD_MAXOFFHANDDAMAGE)) / 2;
            else
            ...
            // Impossible case
            default:
                sLog.outError("Unit::HandleProcTriggerSpellAuraProc: DW "
                              "unknown spell rank "
                              "%u",
                              auraSpellInfo->Id);
                return SPELL_AURA_PROC_FAILED;
        ...
        if (auraSpellInfo->SpellIconID == 2006 && auraSpellInfo->IsFitToFamilyMask(uint64(0x0000000000100000)))
        {
            ...
            case 30033:
                ...
                break; // Rank 3
            default:
                sLog.outError("Unit::HandleProcTriggerSpellAuraProc: Spell %u not "
                              "handled in Rampage",
                              auraSpellInfo->Id);
                return SPELL_AURA_PROC_FAILED;
            }
    ...
    case SPELLFAMILY_WARLOCK: {
        ...
        else if (auraSpellInfo->SpellFamilyFlags & uint64(0x0000000000004000))
        {
            ...
            {
                if (i->GetModifier()->m_miscvalue == SPELLMOD_CHANCE_OF_SUCCESS &&
    ...
    case SPELLFAMILY_PALADIN: {
        ...
        if (auraSpellInfo->SpellIconID == 2137)
        {
            switch (auraSpellInfo->Id)
            {
                ...
                default:
                    sLog.outError("Unit::HandleProcTriggerSpellAuraProc: Spell
        %u miss posibly Blessed Life", auraSpellInfo->Id); return
        ...
        */
        ...
        else if (auraSpellInfo->SpellIconID == 241)
        {
            ...
            if (!originalSpell)
            {
                sLog.outError("Unit::HandleProcTriggerSpellAuraProc: Spell %u unknown "
    ...
    {
        // Need add combopoint AFTER finishing move (or they get dropped in
        // finish phase)
    ...
    case 35095: {
        if (!spellInfo || spellInfo->powerType != POWER_MANA ||
            (spellInfo->manaCost == 0 && spellInfo->ManaCostPercentage == 0 && spellInfo->manaCostPerlevel == 0))
            return SPELL_AURA_PROC_FAILED;
        break;
    }
    }
    ...
    if (!triggerEntry)
    {
        ...
        // sLog.outError("Unit::HandleProcTriggerSpellAuraProc: Spell %u have 0
        // in EffectTriggered[%d], not handled custom
    ...
    if (target == nullptr)
        target = !(procFlags & (PROC_FLAG_DEAL_HELPFUL_SPELL | PROC_FLAG_DEAL_HELPFUL_ABILITY)) &&
                         IsPositiveSpellTargetMode(triggerEntry, this, pVictim)
                     ? this
                     : pVictim;

    // Quick check for target modes for procs: do not cast offensive procs on
    // friendly targets and in reverse
    if (target != nullptr)
    {
        if (!(procEx & PROC_EX_REFLECT))
        {
            // TODO: add neutral target handling, neutral targets should still be
            // able to go through
...
SpellAuraProcResult Unit::HandleProcTriggerDamageAuraProc(ProcExecutionData &data)
{
    ...
    DEBUG_FILTER_LOG(LOG_FILTER_SPELL_CAST,
                     "ProcDamageAndSpell: doing %u damage from spell id %u "
                     "(triggered by auratype %u of spell %u)",
    ...
    if (SpellMissInfo missInfo =
            SpellHitResult(this, victim, spellInfo, uint8(1 << triggeredByAura->GetEffIndex()), false))
    {
...
SpellAuraProcResult Unit::HandleOverrideClassScriptAuraProc(ProcExecutionData &data)
{
    ...
    case 3656: // Corrupted Healing (Priest class call in Nefarian encounter)
    {
        ...
        // Check that only priest class can proc it is done in
        // Spell::CheckTargetScript() for aura 23401
...
SpellAuraProcResult Unit::HandleRaidProcFromChargeAuraProc(ProcExecutionData &data)
{
    uint32 damage = data.damage;
    Aura *triggeredByAura = data.triggeredByAura;
    // if no damage then mending proc has failed, we dont need to do unecessary
    // steps
    if (!damage)
        return SPELL_AURA_PROC_FAILED;
        ...
        if (Unit *caster = triggeredByAura->GetCaster())
        {
            if (Player *target = GetNextRandomRaidMember(radius, SPELL_AURA_RAID_PROC_FROM_CHARGE_WITH_VALUE))
            {
                ...
                for (int32 i = 0; i < MAX_EFFECT_INDEX; ++i)
                {
                    ...
                    Aura *new_aur =
                        CreateAura(spellProto, aur->GetEffIndex(), &damage, &basePoints, new_holder, target, caster);
                    new_holder->AddAura(new_aur, new_aur->GetEffIndex());
                }
                new_holder->SetAuraCharges(jumps, false);

                // lock aura holder (currently SPELL_AURA_RAID_PROC_FROM_CHARGE
                // is single target spell, so will attempt removing from old
                // target when applied to new one)
...
SpellAuraProcResult Unit::HandleRaidProcFromChargeWithValueAuraProc(ProcExecutionData &data)
{
    ...
    if (jumps > 0 && GetTypeId() == TYPEID_PLAYER && caster_guid.IsPlayer())
    {
        ...
        if (Player *caster = static_cast<Player *>(triggeredByAura->GetCaster()))
        {
            ...
            {
                // This custom nonsense was later removed in 3.0.2 for this proc
                // effect only and replaced by casting the original spell
                SpellAuraHolder *holder = GetSpellAuraHolder(spellProto->Id, caster->GetObjectGuid());
                SpellAuraHolder *new_holder = CreateSpellAuraHolder(spellProto, target, caster);
                ...
                // lock aura holder (currently
                // SPELL_AURA_RAID_PROC_FROM_CHARGE_WITH_VALUE is single target
                // spell, so will attempt removing from old target when applied
                // to new one)
                if (!target->AddSpellAuraHolder(new_holder))
                    delete new_holder;
                else
    ...
    // heal
    SpellCastResult result = CastCustomSpell(nullptr, 33110, &heal, nullptr, nullptr,
                                             TRIGGERED_IGNORE_GCD | TRIGGERED_IGNORE_CURRENT_CASTED_SPELL |
                                                 TRIGGERED_HIDE_CAST_IN_COMBAT_LOG);
...
SpellAuraProcResult Unit::HandleModCastingSpeedNotStackAuraProc(ProcExecutionData &data)
{
    ...
    return !(spellInfo == nullptr || GetSpellCastTime(spellInfo, this) == 0) ? SPELL_AURA_PROC_OK
                                                                             : SPELL_AURA_PROC_FAILED;
}
...
SpellAuraProcResult Unit::HandleReflectSpellsSchoolAuraProc(ProcExecutionData &data)
{
    ...
    return !(spellInfo == nullptr || (triggeredByAura->GetModifier()->m_miscvalue & spellInfo->SchoolMask) == 0)
               ? SPELL_AURA_PROC_OK
               : SPELL_AURA_PROC_FAILED;
}
...
SpellAuraProcResult Unit::HandleModPowerCostSchoolAuraProc(ProcExecutionData &data)
{
    ...
    return !(spellInfo == nullptr || (spellInfo->manaCost == 0 && spellInfo->ManaCostPercentage == 0) || // Cost check
             (triggeredByAura->GetModifier()->m_miscvalue & spellInfo->SchoolMask) == 0)
               ? SPELL_AURA_PROC_OK
...
SpellAuraProcResult Unit::HandlePeriodicAuraProc(ProcExecutionData &data)
{
    ...
    case 35244: // Choking Vines
        if (triggeredByAura->GetHolder()->GetStackAmount() ==
            triggeredByAura->GetHolder()->GetSpellProto()->StackAmount)
        {
            RemoveSpellAuraHolder(triggeredByAura->GetHolder(), AURA_REMOVE_BY_DEFAULT);
            CastSpell(nullptr, 35247,
                      TRIGGERED_OLD_TRIGGERED); // constricting wound
        }
        break;
    }
