...
enum
{
    ...
    SPELL_CAPTURESOUL = 21053,    // procs 21054 on kill (Doom Lord Kazzak uses
                                  // 48473 Capture Soul, procs 32966 on kill)
    SPELL_TWISTED_REFLECTION = 21063,

    SPELL_SUMMON_INFERNALING = 33533,            // summons creature Infernal Hound 19207
    SPELL_CREATE_INFERNALING_SUMMONER = 33536,   // summons gameobjects 183283 Infernaling Summoner Visual and
                                                 // 183282 Infernaling Summoner Damage Trap - triggers spell 29115
                                                 // Flames
    SPELL_CREATE_INFERNALING_SUMMONER_C = 33541, // triggered by 33536, summons trap gameobject 183278 Infernaling
                                                 // Summoner (go_infernaling_summoner)
...
struct boss_highlord_kruulAI : public ScriptedAI
{
    ...
    {
        if (Creature *hand1 = m_creature->SummonCreature(NPC_HAND_OF_THE_HIGHLORD, m_creature->GetPositionX(),
                                                         m_creature->GetPositionY(), m_creature->GetPositionZ(), 0.0f,
                                                         TEMPSPAWN_DEAD_DESPAWN, 1800000, true))
        {
            hand1->GetMotionMaster()->MoveFollow(m_creature, 15.f, 220.f * float(M_PI) / 180.0f, true);
        }
        if (Creature *hand2 = m_creature->SummonCreature(NPC_HAND_OF_THE_HIGHLORD, m_creature->GetPositionX(),
                                                         m_creature->GetPositionY(), m_creature->GetPositionZ(), 0.0f,
                                                         TEMPSPAWN_DEAD_DESPAWN, 1800000, true))
        {
