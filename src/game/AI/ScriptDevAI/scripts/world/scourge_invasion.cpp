...
inline uint32 GetCampType(Creature *unit)
{
    return unit->HasAura(SPELL_CAMP_TYPE_GHOST_SKELETON) || unit->HasAura(SPELL_CAMP_TYPE_GHOST_GHOUL) ||
           unit->HasAura(SPELL_CAMP_TYPE_GHOUL_SKELETON);
};

inline bool IsGuardOrBoss(Unit *unit)
{
    return unit->GetEntry() == NPC_ROYAL_DREADGUARD || unit->GetEntry() == NPC_STORMWIND_ROYAL_GUARD ||
           unit->GetEntry() == NPC_UNDERCITY_ELITE_GUARDIAN || unit->GetEntry() == NPC_UNDERCITY_GUARDIAN ||
           unit->GetEntry() == NPC_DEATHGUARD_ELITE || unit->GetEntry() == NPC_STORMWIND_CITY_GUARD ||
           unit->GetEntry() == NPC_HIGHLORD_BOLVAR_FORDRAGON || unit->GetEntry() == NPC_LADY_SYLVANAS_WINDRUNNER ||
           unit->GetEntry() == NPC_VARIMATHRAS;
}
...
Unit *SelectRandomFlameshockerSpawnTarget(Creature *unit, Creature *except, float radius)
{
    ...
    {
        // || !(*tIter)->ToCreature()->CanSummonGuards() -
        // CREATURE_FLAG_EXTRA_SUMMON_GUARD
        if ((*tIter)->IsCivilian() || (*tIter)->GetZoneId() != unit->GetZoneId() || !unit->CanAttack((*tIter)) ||
            GetClosestCreatureWithEntry((*tIter), NPC_FLAMESHOCKER, VISIBILITY_DISTANCE_TINY))
        {
            auto tIter2 = tIter;
            ++tIter;
...
void ChangeZoneEventStatus(Creature *mouth, bool on)
{
    ...
    sWorld.GetMessager().AddMessage([](World * /*world*/) { sWorldState.Save(SAVE_ID_SCOURGE_INVASION); });
}
...
void DespawnEventDoodads(Creature *shard)
{
    ...
    GetGameObjectListWithEntryInGrid(doodadList, shard,
                                     {GO_SUMMON_CIRCLE, GO_UNDEAD_FIRE, GO_UNDEAD_FIRE_AURA, GO_SKULLPILE_01,
                                      GO_SKULLPILE_02, GO_SKULLPILE_03, GO_SKULLPILE_04, GO_SUMMONER_SHIELD},
                                     60.0f);
...
void SummonCultists(Unit *shard)
{
    ...
    // We don't have all positions sniffed from the Cultists, so why not using
    // this code which placing them almost perfectly into the circle while
    // blizzards positions are sometimes way off?
    if (GameObject *gameObject = GetClosestGameObjectWithEntry(shard, GO_SUMMON_CIRCLE, CONTACT_DISTANCE))
    {
        for (int i = 0; i < 4; ++i)
        {
            ...
            if (Creature *cultist =
                    shard->SummonCreature(NPC_CULTIST_ENGINEER, x, y, z, angle - M_PI_F,
                                          TEMPSPAWN_TIMED_OR_DEAD_DESPAWN, IN_MILLISECONDS * HOUR, true))
...
uint32 HasMinion(Creature *summoner, float /*range*/)
{
    ...
    GetCreatureListWithEntryInGrid(minionList, summoner,
                                   {NPC_SKELETAL_SHOCKTROOPER, NPC_GHOUL_BERSERKER, NPC_SPECTRAL_SOLDIER,
...
bool UncommonMinionspawner(Creature *pSummoner) // Rare Minion Spawner.
{
    ...
    GetCreatureListWithEntryInGrid(uncommonMinionList, pSummoner,
                                   {NPC_LUMBERING_HORROR, NPC_BONE_WITCH, NPC_SPIRIT_OF_THE_DAMNED}, 100.0f);
    for (const auto pMinion : uncommonMinionList)
        if (pMinion)
            return false; // Already a rare found (dead or alive).

    /*
    The chance or timer for a Rare minion spawn is unknown, and I don't see an
    exact pattern for a spawn sequence. Sniffed are: 19669 Minions and 90 Rares
    (Ratio: 217 to 1).
    */
...
struct MouthAI : public ScriptedAI
{
    MouthAI(Creature *creature) : ScriptedAI(creature)
    {
        AddCustomAction(EVENT_MOUTH_OF_KELTHUZAD_YELL, urand((IN_MILLISECONDS * 150), (IN_MILLISECONDS * HOUR)), [&]() {
            DoBroadcastText(PickRandomValue(BCT_MOUTH_OF_KELTHUZAD_RANDOM_1, BCT_MOUTH_OF_KELTHUZAD_RANDOM_2,
                                            BCT_MOUTH_OF_KELTHUZAD_RANDOM_3, BCT_MOUTH_OF_KELTHUZAD_RANDOM_4),
    ...
    void ReceiveAIEvent(AIEventType eventType, Unit * /*sender*/, Unit * /*invoker*/, uint32 miscValue) override
    {
        if (eventType == AI_EVENT_CUSTOM_A)
        {
            ...
            case EVENT_MOUTH_OF_KELTHUZAD_ZONE_START: {
                ...
                DoBroadcastText(PickRandomValue(BCT_MOUTH_OF_KELTHUZAD_ZONE_ATTACK_START_1,
                                                BCT_MOUTH_OF_KELTHUZAD_ZONE_ATTACK_START_2),
                                m_creature, nullptr, CHAT_TYPE_ZONE_YELL);
                break;
            }
            case EVENT_MOUTH_OF_KELTHUZAD_ZONE_STOP: {
                DoBroadcastText(PickRandomValue(BCT_MOUTH_OF_KELTHUZAD_ZONE_ATTACK_ENDS_1,
                                                BCT_MOUTH_OF_KELTHUZAD_ZONE_ATTACK_ENDS_2,
                                                BCT_MOUTH_OF_KELTHUZAD_ZONE_ATTACK_ENDS_3),
                                m_creature, nullptr, CHAT_TYPE_ZONE_YELL);
...
struct NecropolisAI : public ScriptedAI
{
    ...
    void SpellHit(Unit *caster, SpellEntry const *spell) override
    {
        ...
        if (spell->Id == SPELL_COMMUNIQUE_PROXY_TO_NECROPOLIS)
            m_creature->CastSpell(m_creature, SPELL_COMMUNIQUE_TIMER_NECROPOLIS,
                                  TRIGGERED_OLD_TRIGGERED); // m_creature->AddAura(SPELL_COMMUNIQUE_TIMER_NECROPOLIS);
    }

    void UpdateAI(uint32 const diff) override
    {
        if (m_creature && m_creature->IsAlive() &&
            m_creature->GetPositionZ() < (m_creature->GetRespawnPosition().GetPositionZ() - 10))
        {
            Position respawn = m_creature->GetRespawnPosition();
            m_creature->NearTeleportTo(respawn.GetPositionX(), respawn.GetPositionY(), respawn.GetPositionZ(),
                                       respawn.GetPositionO());
        }
    }
};
    ...
    NecropolisHealthAI(Creature *creature) : ScriptedAI(creature)
    {
        AddCustomAction(0, 5000u, [&]() {
            if (m_creature->GetHealthPercent() > 99.f)
            {
                ...
                for (ScourgeInvasionMisc t_necId : {GO_NECROPOLIS_BIG, GO_NECROPOLIS_HUGE, GO_NECROPOLIS_MEDIUM,
                                                    GO_NECROPOLIS_SMALL, GO_NECROPOLIS_TINY})
                {
...
struct NecropolisProxyAI : public ScriptedAI
{
    ...
    void SpellHitTarget(Unit *target, SpellEntry const *spellInfo) override
    {
        // Make sure m_creature despawn after SPELL_COMMUNIQUE_CAMP_TO_RELAY_DEATH
        // hits the target to avoid getting hit by Purple bolt again.
...
struct NecropolisRelayAI : public ScriptedAI
{
    ...
    {
        if (m_creature && m_creature->IsAlive() &&
            m_creature->GetPositionZ() < (m_creature->GetRespawnPosition().GetPositionZ() - 5))
        {
            Position respawn = m_creature->GetRespawnPosition();
            m_creature->NearTeleportTo(respawn.GetPositionX(), respawn.GetPositionY(), respawn.GetPositionZ(),
                                       respawn.GetPositionO());
...
struct NecroticShard : public ScriptedAI
{
    ...
    {
        m_creature->SetActiveObjectState(true);
        AddCustomAction(EVENT_SHARD_MINION_SPAWNER_SMALL, true,
                        [&]() // Spawn Minions every 5 seconds.
                        { HandleShardMinionSpawnerSmall(); });
        AddCustomAction(
            11, 25000u,
            [&]() // Check if there are a summoning circle and a necropolis
                  // nearby, otherwise despawn
            {
                if (!m_creature)
                    return;
        ...
        if (m_creature->GetEntry() == NPC_DAMAGED_NECROTIC_SHARD)
        {
            ...
            ResetTimer(EVENT_SHARD_MINION_SPAWNER_SMALL,
                       5000); // Spawn Minions every 5 seconds.
            AddCustomAction(EVENT_SHARD_MINION_SPAWNER_BUTTRESS, 5000u,
                            [&]() // Spawn Cultists every 60 minutes.
                            {
                                /*
                                This is a placeholder for
                                SPELL_MINION_SPAWNER_BUTTRESS [27888] which also
                                activates unknown, not sniffable gameobjects and
                                happens every hour if a Damaged Necrotic Shard is
                                active. The Cultists despawning after 1 hour, so
                                this just resets everything and spawn them again and
                                Refill the Health of the Shard.
                                */
                                m_creature->SetHealthPercent(100.f); // m_creature->SetFullHealth();
                                // Despawn all remaining Shadows before respawning
                                // the Cultists?
        ...
        else
        {
            ...
            GetCreatureListWithEntryInGrid(shardList, m_creature, {NPC_NECROTIC_SHARD, NPC_DAMAGED_NECROTIC_SHARD},
                                           CONTACT_DISTANCE);
            ...
            AddCustomAction(10, 5000u,
                            [&]() // Check if Doodads are spawned 5 seconds after
                                  // spawn. If not: spawn them
                            {
                                GameObjectList objectList;
                                GetGameObjectListWithEntryInGrid(objectList, m_creature,
                                                                 {GO_UNDEAD_FIRE, GO_UNDEAD_FIRE_AURA, GO_SKULLPILE_01,
                                                                  GO_SKULLPILE_02, GO_SKULLPILE_03, GO_SKULLPILE_04},
                                                                 50.f);
    ...
    void SpellHit(Unit *caster, SpellEntry const *spell) override
    {
        ...
        case SPELL_ZAP_CRYSTAL_CORPSE: {
            Creature::DealDamage(m_creature, m_creature, (m_creature->GetMaxHealth() / 4), nullptr, DIRECT_DAMAGE,
        ...
        case SPELL_CHOOSE_CAMP_TYPE: {
            m_camptype = PickRandomValue(SPELL_CAMP_TYPE_GHOUL_SKELETON, SPELL_CAMP_TYPE_GHOST_GHOUL,
        ...
        case SPELL_CAMP_RECEIVES_COMMUNIQUE: {
            if (!GetCampType(m_creature) && m_creature->GetEntry() == NPC_NECROTIC_SHARD)
            {
                ...
                ResetTimer(EVENT_SHARD_MINION_SPAWNER_SMALL,
                           0); // Spawn Minions every 5 seconds.
            }
            break;
        }
    ...
    void JustDied(Unit *killer) override
    {
        ...
        case NPC_NECROTIC_SHARD:
            if (Creature *pShard = m_creature->SummonCreature(
                    NPC_DAMAGED_NECROTIC_SHARD, m_creature->GetPositionX(), m_creature->GetPositionY(),
                    m_creature->GetPositionZ(), m_creature->GetOrientation(), TEMPSPAWN_MANUAL_DESPAWN, 0))
            {
        ...
        case NPC_DAMAGED_NECROTIC_SHARD:
            ...
            // Remove Objects from the event around the Shard (Yes this is
            // Blizzlike).
            DespawnEventDoodads(m_creature);
            sWorld.GetMessager().AddMessage([](World * /*world*/) { sWorldState.Save(SAVE_ID_SCOURGE_INVASION); });
            break;
    ...
    {
        /*
        This is a placeholder for SPELL_MINION_SPAWNER_SMALL [27887] which also
        activates unknown, not sniffable objects, which possibly checks whether a
        minion is in his range and happens every 15 seconds for both, Necrotic
        Shard and Damaged Necrotic Shard.
        */
        ...
        // On a fresh camp, first the minions are spawned close to the shard and
        // then further and further out.
        ...
        for (const auto &pFinder : finderList)
        {
            ...
            /*
            A finder disappears after summoning the spawner NPC (which summons the
            minion). after 160-185 seconds a finder respawns on the same position
            as before.
            */
            if (pFinder->AI()->DoCastSpellIfCan(m_creature, SPELL_FIND_CAMP_TYPE, CAST_TRIGGERED) == CAST_OK)
            {
                pFinder->SetRespawnDelay(urand(150,
                                               200)); // Values are from Sniffs (rounded). Shortest and
                                                      // Longest respawn time from a finder on the same spot.
                pFinder->ForcedDespawn();
                finderCounter++;
            }
        }
...
struct MinionspawnerAI : public ScriptedAI
{
    ...
    {
        AddCustomAction(
            EVENT_SPAWNER_SUMMON_MINION, 2000, 5000,
            [&]() // Spawn Minions every 5 seconds.
            {
                ...
                case NPC_SCOURGE_INVASION_MINION_SPAWNER_GHOST_GHOUL:
                    Entry = UncommonMinionspawner(m_creature)
                                ? PickRandomValue(NPC_SPIRIT_OF_THE_DAMNED, NPC_LUMBERING_HORROR)
                                : PickRandomValue(NPC_SPECTRAL_SOLDIER, NPC_GHOUL_BERSERKER);
                    break;
                case NPC_SCOURGE_INVASION_MINION_SPAWNER_GHOST_SKELETON:
                    Entry = UncommonMinionspawner(m_creature)
                                ? PickRandomValue(NPC_SPIRIT_OF_THE_DAMNED, NPC_BONE_WITCH)
                                : PickRandomValue(NPC_SPECTRAL_SOLDIER, NPC_SKELETAL_SHOCKTROOPER);
                    break;
                case NPC_SCOURGE_INVASION_MINION_SPAWNER_GHOUL_SKELETON:
                    Entry = UncommonMinionspawner(m_creature)
                                ? PickRandomValue(NPC_LUMBERING_HORROR, NPC_BONE_WITCH)
                                : PickRandomValue(NPC_GHOUL_BERSERKER, NPC_SKELETAL_SHOCKTROOPER);
                    break;
                }
                if (Creature *pMinion = m_creature->SummonCreature(
                        Entry, m_creature->GetPositionX(), m_creature->GetPositionY(), m_creature->GetPositionZ(),
                        m_creature->GetOrientation(), TEMPSPAWN_TIMED_OR_DEAD_DESPAWN, IN_MILLISECONDS * HOUR, true))
                {
                    pMinion->GetMotionMaster()->MoveRandomAroundPoint(pMinion->GetPositionX(), pMinion->GetPositionY(),
                                                                      pMinion->GetPositionZ(),
                                                                      1.0f); // pMinion->SetWanderDistance(1.0f);
...
struct ScourgeMinion : public CombatAI
{
    ...
    void ReceiveAIEvent(AIEventType eventType, Unit * /*sender*/, Unit *invoker, uint32 miscValue) override
    {
        ...
        if (eventType == AI_EVENT_CUSTOM_A)
        {
            if (miscValue == NPC_SHADOW_OF_DOOM)
            {
                ...
                // Pickup random emote like here:
                // https://youtu.be/evOs9aJa2Jw?t=229
                DoBroadcastText(PickRandomValue(BCT_SHADOW_OF_DOOM_TEXT_0, BCT_SHADOW_OF_DOOM_TEXT_1,
                                                BCT_SHADOW_OF_DOOM_TEXT_2, BCT_SHADOW_OF_DOOM_TEXT_3),
                                m_creature, invoker);
    ...
    void MoveInLineOfSight(Unit *pWho) override
    {
        if (m_creature->GetEntry() == NPC_FLAMESHOCKER)
            if (pWho->IsCreature() && m_creature->IsWithinDistInMap(pWho, VISIBILITY_DISTANCE_TINY) &&
                m_creature->IsWithinLOSInMap(pWho) && !pWho->GetVictim())
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case EVENT_MINION_FLAMESHOCKERS_TOUCH: {
            DoCastSpellIfCan(m_creature->GetVictim(),
                             PickRandomValue(SPELL_FLAMESHOCKERS_TOUCH, SPELL_FLAMESHOCKERS_TOUCH2), CAST_TRIGGERED);
    ...
    void UpdateAI(uint32 const diff) override
    {
        ...
        // Instakill every mob nearby, except Players, Pets or NPCs with the same
        // faction. m_creature->IsValidAttackTarget(m_creature->GetVictim(), true)
        if (m_creature->GetEntry() != NPC_FLAMESHOCKER &&
            m_creature->IsWithinDistInMap(m_creature->GetVictim(), 30.0f) &&
            !m_creature->GetVictim()->IsControlledByPlayer() && m_creature->CanAttack(m_creature->GetVictim()))
...
struct PallidHorrorAI : public CombatAI
{
    ...
    {
        AddCustomAction(25, 2500u,
                        [&]() // Hacky solution for flameshockers to remain in formation
                        {
                            for (std::pair<ObjectGuid, uint32> follower : m_flameshockFollowers)
                            {
                                ...
                                {
                                    if (!f->IsInCombat() &&
                                        f->GetMotionMaster()->GetCurrentMovementGeneratorType() != FOLLOW_MOTION_TYPE)
                                    {
                                        float angle = (float(follower.second) *
                                                       (M_PI_F / (float(m_flameshockFollowers.size()) / 2.f))) +
        ...
        if (m_creature->GetHealthPercent() == 100.0f)
        {
            ...
            {
                if (Creature *pFlameshocker =
                        m_creature->SummonCreature(NPC_FLAMESHOCKER, m_creature->GetPositionX(),
                                                   m_creature->GetPositionY(), m_creature->GetPositionZ(), 0.0f,
                                                   TEMPSPAWN_TIMED_OOC_OR_CORPSE_DESPAWN, HOUR * IN_MILLISECONDS, true))
                {
                    float angle = (float(i) * (M_PI_F / (float(amount) / 2.f))) + m_creature->GetOrientation();
                    // pFlameshocker->JoinCreatureGroup(m_creature, 5.0f, angle -
                    // M_PI, OPTION_FORMATION_MOVE); // Perfect Circle around the
                    // Pallid.
    ...
    void JustDied(Unit *unit) override
    {
        if (Creature *creature =
                GetClosestCreatureWithEntry(m_creature, NPC_HIGHLORD_BOLVAR_FORDRAGON, VISIBILITY_DISTANCE_NORMAL))
            DoBroadcastText(BCT_STORMWIND_BOLVAR_2, creature, m_creature, CHAT_TYPE_ZONE_YELL);

        if (Creature *creature =
                GetClosestCreatureWithEntry(m_creature, NPC_LADY_SYLVANAS_WINDRUNNER, VISIBILITY_DISTANCE_NORMAL))
        ...
        m_creature->CastSpell(m_creature,
                              (m_creature->GetZoneId() == ZONEID_UNDERCITY_A ? SPELL_SUMMON_FAINT_NECROTIC_CRYSTAL
                                                                             : SPELL_SUMMON_CRACKED_NECROTIC_CRYSTAL),
        ...
        m_creature->GetMap()->SetWeather(m_creature->GetZoneId(), WEATHER_TYPE_RAIN, 0.0f, false);
        sLog.outBasic("[Scourge Invasion Event] The Scourge has been defeated in "
                      "%s, next attack starting in %ld minutes",
                      m_creature->GetZoneId() == ZONEID_UNDERCITY_A ? "Undercity" : "Stormwind", timeToNextAttack);
    }
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        case EVENT_PALLID_RANDOM_YELL: {
            DoBroadcastText(PickRandomValue(BCT_PALLID_HORROR_YELL1, BCT_PALLID_HORROR_YELL2, BCT_PALLID_HORROR_YELL3,
                                            BCT_PALLID_HORROR_YELL4, BCT_PALLID_HORROR_YELL5, BCT_PALLID_HORROR_YELL6,
                                            BCT_PALLID_HORROR_YELL7, BCT_PALLID_HORROR_YELL8),
                            m_creature, nullptr, CHAT_TYPE_ZONE_YELL);
        ...
        case EVENT_PALLID_SUMMON_FLAMESHOCKER: {
            ...
            {
                if (Unit *target =
                        SelectRandomFlameshockerSpawnTarget(m_creature, nullptr, DEFAULT_VISIBILITY_BGARENAS))
                {
                    float x, y, z;
                    target->GetNearPoint(target, x, y, z, 5.0f, 5.0f, 0.0f);
                    if (Creature *pFlameshocker =
                            m_creature->SummonCreature(NPC_FLAMESHOCKER, x, y, z, target->GetOrientation(),
                                                       TEMPSPAWN_TIMED_OR_DEAD_DESPAWN, IN_MILLISECONDS * HOUR, true))
                    {
                        m_flameshockers.insert(pFlameshocker->GetObjectGuid());
                        pFlameshocker->CastSpell(pFlameshocker, SPELL_MINION_SPAWN_IN, TRIGGERED_OLD_TRIGGERED);
                        pFlameshocker->AI()->SendAIEvent(AI_EVENT_CUSTOM_A, pFlameshocker, pFlameshocker,
                                                         NPC_FLAMESHOCKER);
                    }
                }
            }
